...
// Возвращает пустую таблицу. Реализован для совместимости API с нового менеджера расчета			
// 
// Параметры:
...
Функция ТаблицаНачислений() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаНачислений();
...
Функция СтрокаТаблицыНачислений() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.СтрокаТаблицыНачислений(Зарплата.Начисления);
...
Функция ТаблицаУдержаний() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаУдержаний();
...
Функция СтрокаТаблицыУдержаний() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.СтрокаТаблицыУдержаний(Зарплата.Удержания);
...
Функция ТаблицаПоказателей() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаПоказателей();
...
Функция ТаблицаВзносы() Экспорт
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаВзносы();
...
Процедура ЗаполнитьВыявленнымиНачислениями(ТаблицаСотрудников, Начисления, Отборы = Неопределено)
	...
	ОписаниеВТСотрудникиПериодыИсходные = ОписаниеВТСПериодами();
	ОписаниеВТСотрудникиПериодыИсходные.ИмяВТ = "ВТСотрудникиПериодыИсходные";
	ОписаниеВТСотрудникиПериодыИсходные.ИмяПоляНачалоПериода = "ДатаНачала";
	ОписаниеВТСотрудникиПериодыИсходные.ИмяПоляОкончаниеПериода = "ДатаОкончания";
	ОписаниеВТСотрудникиПериодыИсходные.Измерения.Добавить("Сотрудник");
	ОписаниеВТСотрудникиПериодыИсходные.Измерения.Добавить("Организация");
	
	СоздатьВТСотрудникиПериоды(ТаблицаСотрудников, ОписаниеВТСотрудникиПериодыИсходные.ИмяВТ);
	УдалитьВТ.Добавить(ОписаниеВТСотрудникиПериодыИсходные.ИмяВТ);
	
	ИнициализироватьМенеджерУчетаВремени(ОписаниеВТСотрудникиПериодыИсходные);
	
	ЗаполнитьКадровыеСведенияВТСПериодами(ОписаниеВТСотрудникиПериодыИсходные);
	
	ВыделитьСоответствующиеКадровомуОтбору(ОписаниеВТСотрудникиПериодыИсходные, Отборы);
	...
	УдалитьВТ.Добавить(ОписаниеВТСотрудникиПериоды.ИмяВТ);
	
	ВычеркнутьРанееОплаченныеПериоды(ОписаниеВТСотрудникиПериоды);
	
	ДополнитьВТСотрудникиПериодыДляРасчетаВЦеломЗаМесяц(ОписаниеВТСотрудникиПериоды);
	...
	ВычеркнутьПериодыДляРасчетаВЦеломЗаМесяцНеИмеющиеНачислений(ОписаниеВТСотрудникиПериоды, Начисления);
...
Процедура ДополнитьВТСотрудникиПериодыДляРасчетаВЦеломЗаМесяц(ОписаниеВТСотрудникиПериоды)
	
	Запрос = НовыйЗапрос();
	Запрос.Текст =
		"ВЫБРАТЬ
		|	&ИменаПолей,
		|	МИНИМУМ(СотрудникиПериоды.Вычеркнуть) КАК Вычеркнуть
		|ПОМЕСТИТЬ ВТВсеСотрудникиПериоды
		|ИЗ
		|	(ВЫБРАТЬ
		|		&ИменаПолей,
		...
		|		ИСТИНА
		|	ИЗ
		|		ВТСотрудникиПериодыИсходные КАК СотрудникиПериоды
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТСотрудникиПериоды КАК СотрудникиПериодыОтобранные
		|			ПО (СотрудникиПериодыОтобранные.Сотрудник = СотрудникиПериоды.Сотрудник)
		|				И (СотрудникиПериодыОтобранные.Организация = СотрудникиПериоды.Организация)
		|	ГДЕ
		|		(СотрудникиПериоды.ДатаНачала >= СотрудникиПериодыОтобранные.ДатаОкончания
		|				ИЛИ СотрудникиПериоды.ДатаОкончания <= СотрудникиПериодыОтобранные.ДатаНачала)) КАК СотрудникиПериоды	
		|СГРУППИРОВАТЬ ПО
		|	&ИменаПолей";
...
Процедура ДобавитьПлановыеНачисления(ОписаниеВТСотрудникиПериоды, Начисления)
	...
	Запрос.Текст = 
		...
		|	СотрудникиПериоды.НачислятьОплатуТруда КАК НачислятьОплатуТруда,
		...
		|				И (НЕ ПлановыеНачисления.Начисление.НачисляетсяВЦеломЗаМесяц
		|					ИЛИ ПлановыеНачисления.Начисление.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		|	СотрудникиПериоды.Вычеркнуть КАК Вычеркнуть
		|ИЗ
		|	ВТСотрудникиПериоды КАК СотрудникиПериоды
		...
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		|			И (НЕ СотрудникиПериоды.Вычеркнуть
		|				ИЛИ ПлановыеНачисления.Начисление.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
			...
			Пока Выборка.СледующийПоЗначениюПоля("Начисление") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("ДокументОснование") Цикл
					...
					Пока Выборка.Следующий() Цикл
						ДобавитьНачисление(Начисления, Выборка, ПредыдущаяСтрока,, Истина);
					КонецЦикла;
				КонецЦикла;
			КонецЦикла;
		КонецЦикла;
...
Процедура ДобавитьНачисление(Начисления, Выборка, ПредыдущаяСтрока, ПроверятьНаличие = Ложь, ПомечатьВычеркиваемые = Ложь)
	
	Если ПредыдущаяСтрока = Неопределено Тогда
		// Это первая строка, ее просто добавляем.
		ДобавитьСтрокуНачисления(Начисления, Выборка, ПредыдущаяСтрока, ПроверятьНаличие, ПомечатьВычеркиваемые);
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьСтрокуНачисления(Начисления, Выборка, ПредыдущаяСтрока, ПроверятьНаличие = Ложь, ПомечатьВычеркиваемые = Ложь)
	...
	Если ПомечатьВычеркиваемые И Выборка.Вычеркнуть Тогда
		НоваяСтрока.Дополнительно.Вставить("Вычеркнуть", Истина);
	КонецЕсли;
...
Процедура ДобавитьНачисленияПоЗначениямОперативныхПоказателей(ОписаниеВТСотрудникиПериоды, Начисления)	
	...
	Запрос.Текст = 
		...
		|	СотрудникиПериоды.ДатаОкончания КАК ДатаОкончания,
		|	ПоказателиРасчета.ВидРасчета КАК Начисление,
		|	НЕ ПоказателиРасчета.ВидРасчета.НачисляетсяВЦеломЗаМесяц
		|		ИЛИ ПоказателиРасчета.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц КАК РазбиватьСтроку,
		|	СотрудникиПериоды.Вычеркнуть КАК Вычеркнуть
		|ИЗ
		|	ВТСотрудникиПериоды КАК СотрудникиПериоды
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПоказателиРасчета КАК ПоказателиРасчета
		...
		|				ИЛИ ПоказателиРасчета.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
		|			И (НЕ СотрудникиПериоды.Вычеркнуть
		|				ИЛИ ПоказателиРасчета.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
		|
		|УПОРЯДОЧИТЬ ПО
...
Процедура ДобавитьНачисленияПоВидамВремени(ОписаниеВТСотрудникиПериоды, Начисления)
	...
	Запрос.Текст = 
		...
		|			ПО (ВидыРасчета.Ссылка = НачисленияПоВидамВремени.КлючНабораВидовВремени)
	...
	Запрос.Текст = 
		...
		|	НачисленияПоВидамВремени.КатегорияНачисления
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
			...
			МодульУправленческаяЗарплата.ПриДобавленииНачисленияПоВидамВремени(Начисления, Выборка, СтандартнаяОбработка);
		КонецЕсли;
...
Процедура ЗаполнитьВыявленнымиУдержаниями(ТаблицаСотрудников, Удержания)
	...
	СоздатьВТКадровыеИзмененияИнтервалыПриНеобходимости(ОписаниеВТСотрудникиПериоды);
...
Процедура СоздатьВТЗадолженностьПоУдержаниямФизическихЛиц(ОписаниеВТФизическиеЛицаПериоды)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл 
		...
		КонецЦикла;
		СписокСотрудников = Обработки.МенеджерРасчетаЗарплатыАрхивный.ИсключаемыеИзРасчетаВзысканийСотрудники(ГоловнаяОрганизация, МесяцНачисления, ФизическиеЛица);
	...
	Пока Выборка.СледующийПоЗначениюПоля("ИдентификаторСтроки") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(Строка.Дополнительно) = Тип("Структура") Тогда
				НоваяСтрока.Дополнительно = ОбщегоНазначения.СкопироватьРекурсивно(Строка.Дополнительно);
			КонецЕсли;
			НоваяСтрока.ДатаНачала = Выборка.Дата;
...
Процедура ВычеркнутьСтрокиПоПоказателям(Строки, СтрокиПоИдентификатору = Неопределено, НаборЗаписей = Неопределено)
	...
	ВычеркнутьСтроки(ВычеркиваемыеСтроки, Строки, СтрокиПоИдентификатору, НаборЗаписей);
	ВычеркиваемыеСтроки.Очистить();
	
КонецПроцедуры

Процедура ВычеркнутьСтрокиДляРасчетаВЦеломЗаМесяц(Строки, СтрокиПоИдентификатору = Неопределено, НаборЗаписей = Неопределено)
	
	// Вычеркиваем строки, сохраненные для поддержки расчета накапливаемых показателей в целом за месяц.
	ИдентификаторыСтрок = Новый Массив;
	Вычеркнуть = Ложь;
	
	Для Каждого Строка Из Строки Цикл
		Если Строка.Дополнительно.Свойство("Вычеркнуть", Вычеркнуть) И Вычеркнуть Тогда
			ИдентификаторыСтрок.Добавить(Строка.ИдентификаторСтроки);
		КонецЕсли;
	КонецЦикла;	
	
	ВычеркнутьСтроки(ИдентификаторыСтрок, Строки, СтрокиПоИдентификатору, НаборЗаписей);
	
КонецПроцедуры

Процедура ВычеркнутьСтроки(ВычеркиваемыеСтроки, Строки, СтрокиПоИдентификатору = Неопределено, НаборЗаписей = Неопределено)
	
	Если ВычеркиваемыеСтроки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ВыделитьСоответствующиеКадровомуОтбору(ОписаниеВТСотрудникиПериоды, Отборы)
	...
	Запрос.Текст = 
		...
		|	ВТСотрудникиПериодыИсходные КАК СотрудникиПериоды
		|ГДЕ
		|	СотрудникиПериоды.Организация = &Организация
		...
		|	ВТСотрудникиПериодыИсходные КАК СотрудникиПериоды
		|ГДЕ
		|	СотрудникиПериоды.БезусловноСоответствуетКадровымДанным";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТСотрудникиПериодыИсходные", ОписаниеВТСотрудникиПериоды.ИмяВТ);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаНачала", "СотрудникиПериоды." + ОписаниеВТСотрудникиПериоды.ИмяПоляНачалоПериода);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаОкончания", "СотрудникиПериоды." + ОписаниеВТСотрудникиПериоды.ИмяПоляОкончаниеПериода);
...
Процедура ВычеркнутьПериодыДляРасчетаВЦеломЗаМесяцНеИмеющиеНачислений(ОписаниеВТСотрудникиПериоды, Начисления)
	...
	Запрос.Текст =
		...
		|	Начисления.ДатаОкончания КАК ДатаОкончания
		|ПОМЕСТИТЬ ВТИнтервалыНачислений
		|ИЗ
		...
		|	ВТСотрудникиПериоды КАК СотрудникиПериоды
		|ГДЕ
		|	(НЕ СотрудникиПериоды.Вычеркнуть
		|			ИЛИ ИСТИНА В
		|				(ВЫБРАТЬ ПЕРВЫЕ 1
		|					ИСТИНА
		|				ИЗ
		|					ВТИнтервалыНачислений КАК ИнтервалыНачислений
		|				ГДЕ
		|					ИнтервалыНачислений.Сотрудник = СотрудникиПериоды.Сотрудник
		|					И ИнтервалыНачислений.Организация = СотрудникиПериоды.Организация
		|					И ИнтервалыНачислений.ДатаНачала = СотрудникиПериоды.ДатаНачала
		|					И ИнтервалыНачислений.ДатаОкончания = СотрудникиПериоды.ДатаОкончания))";
	...
	УдалитьВТ = Новый Массив;
	УдалитьВТ.Добавить(ОписаниеВТСотрудникиПериоды.ИмяВТ);
	УдалитьВТ.Добавить("ВТИнтервалыНачислений");
	УничтожитьВТ(УдалитьВТ);
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТСотрудникиПериоды
		|ИЗ
		|	ВТВсеСотрудникиПериоды КАК ВсеСотрудникиПериоды
		|;
...
Процедура СоздатьВТПоказателиРасчетаНачисления(ОписаниеВТНачисления)
	...
	Запрос.УстановитьПараметр("КалендарныеДниМесяцаБезПраздников", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяцаБезПраздников"));
	...
	Запрос.Текст = 
		...
		|		КОГДА ПоказателиРасчета.Показатель = &КалендарныеДниБезПраздников
		|			ТОГДА ИСТИНА
		|		КОГДА ПоказателиРасчета.Показатель = &КалендарныеДниМесяцаБезПраздников
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
...
Процедура ЗаполнитьЗначенияПоказателяДоляНеполногоРабочегоВремени(ТребованияНачислений, СтрокиПоИдентификатору)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ДоляНеполногоРабочегоВремени");
	...
	Запрос.Текст =
		...
		|	ДополнительныеПоказатели.ИдентификаторСтроки,
		|	ДополнительныеПоказатели.Показатель,
		|	ЕСТЬNULL(КадровыеДанныеСотрудников.ДоляНеполногоРабочегоВремени, 1) КАК Значение
		|ИЗ
		|	ВТДополнительныеПоказатели КАК ДополнительныеПоказатели
...
Процедура ЗаполнитьЗначенияПоказателейСтоимостьЧасаДня(ТребованияНачислений, СтрокиПоИдентификатору)
	...
	СтоимостьЧасаДня = Обработки.МенеджерРасчетаЗарплатыАрхивный.СтоимостьЧасаДня(МенеджерВременныхТаблиц, "ВТДополнительныеПоказатели", МенеджерУчетаВремени());
...
Процедура ЗаполнитьЗначенияПоказателяПроцентСевернойНадбавки(ТребованияНачислений, СтрокиПоИдентификатору)
	...
	Запрос.Текст =
		...
		|	ДополнительныеПоказатели.ИдентификаторСтроки,
		|	ДополнительныеПоказатели.Показатель,
		|	ЕСТЬNULL(ПроцентНадбавки.ПроцентСевернойНадбавки, 0) КАК Значение
		|ИЗ
		|	ВТДополнительныеПоказатели КАК ДополнительныеПоказатели
...
Процедура РассчитатьЗначенияПоказателейНормыВремени(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	НормаДней = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДней");
	НормаВремениВДнях = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВДнях");
	НормаВремениВЧасах = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВЧасах");
	НормаДнейЧасов = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДнейЧасов");
	
	Показатели = Новый Массив;
	Показатели.Добавить(НормаДней);
	Показатели.Добавить(НормаВремениВДнях);
	Показатели.Добавить(НормаВремениВЧасах);
	Показатели.Добавить(НормаДнейЧасов);
	...
	Запрос.УстановитьПараметр("НормаДнейЧасов", НормаДнейЧасов);
	...
	Запрос.Текст = 
		...
		|					ИНАЧЕ НормаВремени.НормаДнейПолная
		|				КОНЕЦ
		|		КОГДА ПоказателиРасчета.Показатель = &НормаВремениВДнях
		|			ТОГДА НормаВремени.НормаДнейПолная
		|		КОГДА ПоказателиРасчета.Показатель = &НормаВремениВЧасах
...
Процедура РассчитатьЗначенияПоказателейРабочегоВремени(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	Запрос.Текст = 
		...
		|	И НЕ Начисления.ВремяВЦеломЗаПериод
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВТРегистрРасчета_Начисления КАК Начисления
		|ГДЕ
		|	(НЕ Начисления.ВидРасчета.НачисляетсяВЦеломЗаМесяц
		|			ИЛИ Начисления.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
		|	И НЕ Начисления.ФиксСторно
		|	И НЕ Начисления.ФиксРасчетВремени
		|	И Начисления.ВремяВЦеломЗаПериод";
	Запрос.Выполнить();
	...
	ОписаниеВидовВремени.ПолеУчитыватьРабочееВремяДляОтклонений = "Ссылка.СпособВыполненияНачисления = ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ПоОтдельномуДокументуДоОкончательногоРасчета)";
	...
	ВремяВДнях = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВДнях");
	ВремяВДняхЧасах = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВДняхЧасах");
	ВремяВЧасах = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВЧасах");
	...
	Запрос.Текст = 
		...
		|				ИЛИ ВидыРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗарегистрированноеВремяПоНаборамВидовВремени КАК РабочееВремя
		|		ПО (РабочееВремя.ИдентификаторСтроки = Начисления.ИдентификаторСтроки)
		|ГДЕ
		|	Начисления.РассчитываемыйНабор
...
Процедура РассчитатьЗначенияПоказателейСверхурочно(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	ОтработаноСверхурочно = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноСверхурочно");
	ОтработаноСверхурочноВПределах2Часов = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноСверхурочноВПределах2Часов");
	...
	Запрос.Текст = 
		...
		|	ВТРегистрРасчета_Начисления КАК Начисления
		|ГДЕ
		|	Начисления.ВремяВЦеломЗаПериод
		|	И НЕ Начисления.ФиксРасчетВремени
		|	И НЕ Начисления.ФиксСторно
		|	И ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	Запрос.УстановитьПараметр("СверхурочныеБезПовышеннойОплаты", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.СверхурочныеБезПовышеннойОплаты"));
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВидыВремени.Ссылка КАК Начисление,
...
Процедура РассчитатьЗначенияПоказателяОтработаноСмен(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	ОтработаноСмен = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноСмен");
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДни(СтрокиПоИдентификатору, ОписаниеВТНачисления, ОтборПоРассчитываемомуНабору = Истина)
	...
	КалендарныеДни = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДни");
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниМесяца(СтрокиПоИдентификатору, ОписаниеВТНачисления)
	...
	КалендарныеДниМесяца = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяца");
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниБезУчетаПраздников(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	КалендарныеДниБезПраздников = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниБезПраздников");
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниМесяцаБезУчетаПраздников(СтрокиПоИдентификатору, ТребованияНачислений)
	
	Если Не ТребованияНачислений.ТребуетсяПоказательКалендарныеДниБезПраздников Тогда
		Возврат;
	КонецЕсли;
	
	КалендарныеДниМесяцаБезПраздников = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяцаБезПраздников");
	
	Показатели = Новый Массив;
	Показатели.Добавить(КалендарныеДниМесяцаБезПраздников);
	
	УдалитьВТ = Новый Массив;
	
	Запрос = НовыйЗапрос();
	Запрос.УстановитьПараметр("ПоказателиКалендарныеДниМесяца", Показатели);
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Начисления.ИдентификаторСтроки,
		|	Начисления.ОбщийГрафик,
		|	НАЧАЛОПЕРИОДА(Начисления.ПериодДействияНачало, МЕСЯЦ) КАК ДатаНачала,
		|	КОНЕЦПЕРИОДА(Начисления.ПериодДействияНачало, МЕСЯЦ) КАК ДатаОкончания
		|ПОМЕСТИТЬ ВТФильтрДанныеПроизводственногоКалендаряПоСотрудникам
		|ИЗ
		|	ВТРегистрРасчета_Начисления КАК Начисления
		|ГДЕ
		|	НЕ Начисления.ФиксРасчетВремени
		|	И НЕ Начисления.ФиксСторно
		|	И ИСТИНА В
		|			(ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА
		...
		|			ГДЕ
		|				ПоказателиРасчета.ВидРасчета = Начисления.ВидРасчета
		|				И ПоказателиРасчета.Показатель В (&ПоказателиКалендарныеДниМесяца))";
	Запрос.Выполнить();
	УдалитьВТ.Добавить("ВТФильтрДанныеПроизводственногоКалендаряПоСотрудникам");
...
Процедура РассчитатьЗначенияПоказателейОтработаноВсего(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	ОтработаноДней = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноДней");
	ОтработаноЧасов = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноЧасов");
...
Процедура РассчитатьЗначенияПоказателейКоличествоДнейОтпуска(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	КоличествоДнейОтпуска = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейОтпуска");
...
Процедура РассчитатьЗначенияПоказателейКоэффициентИндексацииСреднегоЗаработка(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	КоэффициентИндексацииСреднегоЗаработка = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСреднегоЗаработка");
	КоэффициентИндексацииСохраняемогоДенежногоСодержания = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоСодержания");
...
#Область НакапливаемыеПоказатели

Процедура РассчитатьЗначенияОперативныхПоказателейСотрудников(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	Запрос.Текст = 
		...
		|	Начисления.ПериодДействия КАК Месяц,
		|	ВЫБОР
		|		КОГДА Начисления.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц
		|			ТОГДА НАЧАЛОПЕРИОДА(Начисления.ПериодДействияНачало, МЕСЯЦ)
		|		ИНАЧЕ Начисления.ПериодДействияНачало
		|	КОНЕЦ КАК ДатаНачала,
		|	ВЫБОР
		|		КОГДА Начисления.ВидРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц
		|			ТОГДА КОНЕЦПЕРИОДА(Начисления.ПериодДействияКонец, МЕСЯЦ)
	...
	Запрос.Текст =
		...
		|	ДанныеДляРасчета.ФиксЗаполнение КАК ФиксЗаполнение,
		|	ДанныеДляРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц КАК ИспользоватьОперативныеПоказателиВЦеломЗаМесяц,
		|	ПоказателиРасчета.Показатель КАК Показатель,
		|	СУММА(ЕСТЬNULL(ЗначенияОперативногоПоказателя.Значение, 0)) КАК Значение
		|ПОМЕСТИТЬ ВТНакопленныеЗначенияПоказателей
		|ИЗ
		|	ВТДанныеДляРасчета КАК ДанныеДляРасчета
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТПоказателиРасчета КАК ПоказателиРасчета
		|		ПО (ПоказателиРасчета.ВидРасчета = ДанныеДляРасчета.Начисление)
		|			И (ПоказателиРасчета.ОперативныйПоказательСотрудника = ИСТИНА)
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТЗначенияОперативныхПоказателейСотрудников КАК ЗначенияОперативногоПоказателя
		...
		|			И (ЗначенияОперативногоПоказателя.Показатель = ПоказателиРасчета.Показатель)
		...
		|ГДЕ
		|	ВЫБОР
		|			КОГДА ДанныеДляРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц
		|				ТОГДА &РассчитатьПоказателиНакапливаемыеВЦеломЗаМесяц
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ДанныеДляРасчета.ФиксЗаполнение,
		|	ДанныеДляРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц,
		|	ПоказателиРасчета.Показатель
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	НакопленныеЗначенияПоказателей.Месяц КАК Месяц,
		|	НакопленныеЗначенияПоказателей.ИдентификаторСтроки КАК ИдентификаторСтроки,
		|	НакопленныеЗначенияПоказателей.Сотрудник КАК Сотрудник,
		|	НакопленныеЗначенияПоказателей.Показатель КАК Показатель,
		|	НакопленныеЗначенияПоказателей.Значение КАК Значение
		|ИЗ
		|	ВТНакопленныеЗначенияПоказателей КАК НакопленныеЗначенияПоказателей
		|ГДЕ
		|	НЕ НакопленныеЗначенияПоказателей.ФиксЗаполнение
		|
		|УПОРЯДОЧИТЬ ПО
		|	НакопленныеЗначенияПоказателей.ИдентификаторСтроки,
		|	НакопленныеЗначенияПоказателей.Показатель";
	
	Запрос.УстановитьПараметр("РассчитатьПоказателиНакапливаемыеВЦеломЗаМесяц",
		НастройкиРасчета.РассчитатьПоказателиНакапливаемыеВЦеломЗаМесяц);
	
	ЗаполнитьЗначенияПоказателейПоРезультатуЗапроса(Запрос, СтрокиПоИдентификатору, Истина);
	УдалитьВТ.Добавить("ВТНакопленныеЗначенияПоказателей");
	
	Если НастройкиРасчета.РассчитатьПоказателиНакапливаемыеВЦеломЗаМесяц Тогда
		РаспределитьЗначенияПропорциональноОтработанномуВремени(СтрокиПоИдентификатору);
	КонецЕсли;
...
Процедура РаспределитьЗначенияПропорциональноОтработанномуВремени(СтрокиПоИдентификатору)
	...
	Запрос.Текст =
		...
		|	НакопленныеЗначенияПоказателей.Месяц КАК Месяц,
		|	НакопленныеЗначенияПоказателей.Сотрудник КАК Сотрудник,
		|	НакопленныеЗначенияПоказателей.Начисление КАК Начисление,
		|	НакопленныеЗначенияПоказателей.Показатель КАК Показатель,
		|	НакопленныеЗначенияПоказателей.Значение КАК Значение
		|ИЗ
		|	ВТНакопленныеЗначенияПоказателей КАК НакопленныеЗначенияПоказателей
		|ГДЕ
		|	НакопленныеЗначенияПоказателей.Значение ЕСТЬ НЕ NULL 
		|	И НакопленныеЗначенияПоказателей.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц
		|
		|УПОРЯДОЧИТЬ ПО
		|	НакопленныеЗначенияПоказателей.Месяц,
		|	НакопленныеЗначенияПоказателей.Сотрудник,
		|	НакопленныеЗначенияПоказателей.Начисление,
		|	НакопленныеЗначенияПоказателей.Показатель";
	...
	КоэффициентыНачислений = Новый Массив;
		...
		Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Начисление") Цикл
				Пока Выборка.СледующийПоЗначениюПоля("Показатель") Цикл
					...
					Пока Выборка.Следующий() Цикл
						СтрокаНачисления = СтрокиПоИдентификатору[Выборка.ИдентификаторСтроки];
						СтрокиНачислений.Добавить(СтрокаНачисления);
						
						КоэффициентыНачислений.Добавить(
							?(СтрокаНачисления.ВремяВЧасах,	СтрокаНачисления.ОплаченоЧасов, СтрокаНачисления.ОплаченоДней));
					КонецЦикла;
					
					РаспределениеСуммы = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(
						Выборка.Значение, КоэффициентыНачислений, 0);
					
					Если РаспределениеСуммы <> Неопределено Тогда
						Сч = 0;
						Для Каждого СтрокаНачисления Из СтрокиНачислений Цикл
							Если Не СтрокаНачисления.ФиксЗаполнение Тогда
								ЗаполнитьЗначениеПоказателяСтрокиРасчета(
									СтрокаНачисления, Выборка.Показатель, РаспределениеСуммы[Сч], Истина);
							КонецЕсли;
							Сч = Сч + 1;
						КонецЦикла;
					КонецЕсли;
					КоэффициентыНачислений.Очистить();
					СтрокиНачислений.Очистить();
...
Процедура СоздатьВТПоказателиРасчетаУдержания(ОписаниеВТУдержания)
	...
	Запрос = НовыйЗапрос();
	Запрос.УстановитьПараметр("РасчетнаяБазаСтраховыеВзносы", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РасчетнаяБазаСтраховыеВзносы"));
	Запрос.УстановитьПараметр("РасчетнаяБазаИсполнительногоЛиста", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РасчетнаяБазаИсполнительногоЛиста"));
	Запрос.УстановитьПараметр("РасчетнаяБаза", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РасчетнаяБаза"));
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВидыРасчета.Ссылка КАК ВидРасчета,
...
Процедура РассчитатьЗначенияПоказателяРасчетнаяБазаСтраховыеВзносы(ОчередностьРасчета, СтрокиПоИдентификатору)
	...
	РасчетнаяБазаСтраховыеВзносы = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РасчетнаяБазаСтраховыеВзносы");
...
Процедура РассчитатьЗначениеПоказателяРасчетнаяБазаУдержаний(ОчередностьРасчета, СтрокиПоИдентификатору)	
	...
	РасчетнаяБазаИсполнительногоЛиста = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.РасчетнаяБазаИсполнительногоЛиста");
...
Функция ДополнениеРасчетнойБазыУдержаний(ОписаниеВТ, НалогПоДоходам)
	...
	Запрос.УстановитьПараметр("ВидыНачисленийДополнения", Обработки.МенеджерРасчетаЗарплатыАрхивный.ВидыНачисленийДополненияРасчетнойБазыУдержаний());
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура ЗаполнитьЗначенияПериодическихПоказателейОрганизаций(ТребованияНачислений, СтрокиПоИдентификатору, ОписаниеВТНачисления = Неопределено)
	...
	Запрос.Текст = 
		...
		|					ТОГДА Начисления.ДатаОкончания МЕЖДУ ЗначенияПоказателейОрганизации.НачалоПериода И КОНЕЦПЕРИОДА(ЗначенияПоказателейОрганизации.КонецПериода, МЕСЯЦ)
...
Процедура ЗаполнитьЗначениеПоказателяСтрокиРасчета(Строка, Показатель, Значение, Замещать = Ложь)
	...
	Если Замещать Тогда
		...
		НайденныеСтроки = ВыявленныеПоказатели.НайтиСтроки(Отбор);
		Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
			ВыявленныеПоказатели.Удалить(НайденнаяСтрока);
		КонецЦикла;
	КонецЕсли;
...
Процедура ЗаполнитьОплаченноеВремяКоличествомДней(Строка, Показатель, Значение, СтандартнаяОбработка)
	...
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДни"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниБезПраздников"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейКомпенсации"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейОтпуска"));
...
Процедура ЗаполнитьОплаченнымВременемКоличествоДней(Строка)
	...
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДни"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниБезПраздников"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяца"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяцаБезПраздников"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейКомпенсации"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейОтпуска"));
...
Процедура ЗаполнитьОплаченноеВремяКоличествомЧасов(Строка, Показатель, Значение, СтандартнаяОбработка)
	...
	ПоказателиДней = Новый Массив;
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ОтработаноСверхурочно"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ПереработаноПоСуммированномуУчету"));
...
Процедура ЗаполнитьОплаченноеВремяДнейЧасовВыходногоПособия(Строка, Показатель, Значение, СтандартнаяОбработка)
	...
	Если Показатель <> ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ДнейЧасовВыходногоПособия") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОплаченнымВременемВремяВДнях(Строка)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВДнях");
	ЗаполнитьЗначениеПоказателяСтрокиРасчета(Строка, Показатель, Строка.ОплаченоДней, Истина);
...
Процедура ЗаполнитьОплаченнымВременемНормуДней(Строка)
	...
	ПоказателиДней = Новый Массив;
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДней"));
	ПоказателиДней.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВДнях"));
...
Процедура ЗаполнитьОплаченнымВременемНормуЧасов(Строка)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВЧасах");
	ЗаполнитьЗначениеПоказателяСтрокиРасчета(Строка, Показатель, Строка.НормаЧасов, Истина);
	
КонецПроцедуры

Процедура ЗаполнитьОплаченнымВременемНормуДнейЧасов(Строка)
	
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДнейЧасов");
	Если Строка.ВремяВЧасах = Истина Тогда
		ЗаполнитьЗначениеПоказателяСтрокиРасчета(Строка, Показатель, Строка.НормаЧасов, Истина);
	Иначе
		ЗаполнитьЗначениеПоказателяСтрокиРасчета(Строка, Показатель, Строка.НормаДней, Истина);
	КонецЕсли;
...
Процедура ЗаполнитьПоказателиНачисленийПоОплаченномуВремени(СтрокиПоИдентификатору)
	...
	Для Каждого КлючИЗначение Из СтрокиПоИдентификатору Цикл
		...
		ЗаполнитьОплаченнымВременемНормуДнейЧасов(Строка);
...
Процедура ЗаполнитьСведенияНачисленийДляРасчетаСлужебный(Начисления, ТребованияЗаполнения) 
	...
	Запрос.Текст = 
		...
		|	ИсточникиДанныхВремени.ВремяВведеноСводно,
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаполнитьПериодРегистрацииСтрокиНачисления(Строка, Выборка, ТребованияЗаполнения);
...
Процедура ЗаполнитьПериодРегистрацииСтрокиНачисления(Строка, Выборка, ТребованияЗаполнения)
	
	Если Не ТребованияЗаполнения.ПериодРегистрации Тогда
		Возврат;
	КонецЕсли;
...
Процедура РассчитатьНачисления()
	...
	ВычеркнутьСтрокиДляРасчетаВЦеломЗаМесяц(Начисления, СтрокиПоИдентификатору, НаборыЗаписей.Начисления);
...
Процедура ПослеРасчетаНачислений(СтрокиПоИдентификатору, НаборЗаписей)
	УчетПособийСоциальногоСтрахованияРасширенный.ПослеРасчетаНачислений(
		МенеджерВременныхТаблиц,
		Организация,
		МесяцНачисления,
...
Процедура РассчитатьЗначенияПоказателейНачислений(СтрокиПоИдентификатору, ТребованияНачислений)
	...
	РассчитатьЗначенияПоказателейКалендарныеДниМесяцаБезУчетаПраздников(СтрокиПоИдентификатору, ТребованияНачислений);
	...
	РассчитатьЗначенияОперативныхПоказателейСотрудников(СтрокиПоИдентификатору, ТребованияНачислений);
...
Процедура ВычеркнутьПолностьюВытесненныеНачисления(СтрокиПоИдентификатору, НаборЗаписей)
	...
	Если НастройкиРасчета.ИсключатьСтрокиПолностьюВытесненныхНачислений = Ложь Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И НЕ 1 В
		|				(ВЫБРАТЬ ПЕРВЫЕ 1
		|					1
		|				ИЗ
		|					ПланВидовРасчета.Начисления КАК ВидРасчета
		|				ГДЕ
		|					ВидРасчета.Ссылка = Начисления.ВидРасчета
		|					И ВидРасчета.ЗачетНормыВремени = ИСТИНА
		|					И ВидРасчета.ЗачетОтработанногоВремени = ИСТИНА)";
	КонецЕсли;
...
Процедура РассчитатьЗначениеПоказателяРасчетнаяБазаНачислений(ОчередностьРасчета, СтрокиПоИдентификатору, РасчитанныеНачисленияПредыдущихИтераций = Неопределено)
	...
	|	Начисления.ИдентификаторСтроки КАК ИдентификаторСтроки,
	|	ПоказателиРасчета.Показатель КАК Показатель,
	|	ВЫРАЗИТЬ(СУММА(ЕСТЬNULL(РасчетнаяБаза.РезультатБаза, 0)) КАК ЧИСЛО(15, 2)) КАК Значение
	|ИЗ
	|	ВТРегистрРасчета_Начисления КАК Начисления
...
Процедура РассчитатьУдержанияПоФормуле(ОчередностьРасчета, СтрокиПоИдентификатору)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	ПогашениеЗадолженности = Обработки.МенеджерРасчетаЗарплатыАрхивный.ОснованияПогашенияЗадолженности(МенеджерВременныхТаблиц);
	Пока Выборка.СледующийПоЗначениюПоля("ИдентификаторСтроки") Цикл
		Строка = СтрокиПоИдентификатору[Выборка.ИдентификаторСтроки];
...
Процедура СоздатьВТНаборыВидовВремениДляРаспределенияПоТерриториямУсловиямТруда(ОписаниеВТ)
	...
	Запрос.Текст = 
		...
		|	ВидыВремениНачислений.ВидВремени КАК ВидВремени,
		|	ВидыВремениНачислений.ОпределяющийЗаПраздничныеДни КАК УчитыватьТолькоПраздники,
		|	ВидыВремениНачислений.Ссылка.СпособВыполненияНачисления = ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ПоОтдельномуДокументуДоОкончательногоРасчета) КАК УчитыватьРабочееВремяДляОтклонений
	...
	ОписаниеВТ.ПолеУчитыватьРабочееВремяДляОтклонений = "УчитыватьРабочееВремяДляОтклонений";
...
Функция ТаблицаТерриторийУсловийТруда()
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаТерриторийУсловийТруда();
...
Процедура ОграничитьРазмерУдержанийДоПредела(УдалитьВТ, СтрокиПоИдентификатору)
	...
	Запрос.Текст = 
		...
		|		ПО Удержания.ФизическоеЛицо = ПредельныеСуммыУдержанийСотрудников.ФизическоеЛицо
		|			И Удержания.ДокументОснование = ПредельныеСуммыУдержанийСотрудников.ДокументОснование
...
Процедура СоздатьВТКадровыеИзмененияИнтервалы(ОписаниеВТСотрудникиПериоды)
	...
	УдалитьВТ.Добавить("ВТИнтервалыВидыЗанятостиСотрудников");
	ЗарплатаКадрыПериодическиеРегистры.СоздатьВТИмяРегистраПериоды(
	...
	УдалитьВТ.Добавить("ВТВсеДатыНачала");
	УдалитьВТ.Добавить("ВТКадровыеИзмененияСДатамиНачала");
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
...
Процедура СоздатьВТКадровыеИзмененияИнтервалыПриНеобходимости(ОписаниеВТСотрудникиПериоды)
	
	Если Не ВТСуществует("ВТКадровыеИзмененияИнтервалы") Тогда
		СоздатьВТКадровыеИзмененияИнтервалы(ОписаниеВТСотрудникиПериоды);
		Возврат;
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	ВТСотрудникиПериоды КАК СотрудникиПериоды
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТКадровыеИзмененияИнтервалы КАК КадровыеИзмененияИнтервалы
		|		ПО (КадровыеИзмененияИнтервалы.Сотрудник = СотрудникиПериоды.Сотрудник)
		|			И (КадровыеИзмененияИнтервалы.ДатаНачала = СотрудникиПериоды.ДатаНачала)
		|			И (КадровыеИзмененияИнтервалы.ДатаОкончания = СотрудникиПериоды.ДатаОкончания)";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "ВТСотрудникиПериоды", ОписаниеВТСотрудникиПериоды.ИмяВТ);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаНачала", "СотрудникиПериоды." + ОписаниеВТСотрудникиПериоды.ИмяПоляНачалоПериода);
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "СотрудникиПериоды.ДатаОкончания", "СотрудникиПериоды." + ОписаниеВТСотрудникиПериоды.ИмяПоляОкончаниеПериода);
	
	Если Не Запрос.Выполнить().Пустой() Тогда
		УничтожитьВТ("ВТКадровыеИзмененияИнтервалы");
		СоздатьВТКадровыеИзмененияИнтервалы(ОписаниеВТСотрудникиПериоды);	
	КонецЕсли;		
...
Функция РезультатРасчетаНДФЛ()
	...
	УстановитьПривилегированныйРежим(Истина);
	НДФЛ = УчетНДФЛ.РассчитатьНалогПоОсновнойСтавкеБезОптимизации(
		ИсключаемыйРегистратор, 
		Организация, 
		МесяцНачисления, 
		МенеджерВременныхТаблиц, , , , 
...
Процедура ЗаполнитьВычетыКДоходамНДФЛ(Начисления)
	...
	ДоходыСоСкидками = УчетСтраховыхВзносов.ВидыДоходовСоСкидками();
...
Процедура РассчитатьКорректировкиВыплаты()
	...
	ИсчисленныйФизическиеЛица = ИсчисленныйНалог.ВыгрузитьКолонку("ФизическоеЛицо");
	ВозвратНДФЛПрошлыхЛет = УчетНДФЛ.ВозвратНДФЛПрошлыхЛет(Организация, МесяцНачисления, ИсчисленныйФизическиеЛица);
	
	ТекущиеКорректировки = ВзаиморасчетыССотрудниками.КорректировкиВыплаты(
		Организация, 
		МесяцНачисления, 
		ИсчисленныйФизическиеЛица, 
		ИсключаемыйРегистратор);
...
Функция ТаблицаКорректировокВыплаты()
	...
	Возврат Обработки.МенеджерРасчетаЗарплатыАрхивный.ТаблицаКорректировокВыплаты();
...
Процедура ВыполнитьОтражениеУдержанийИЗаймовВБухучете()
	...
	ИсходныеДанные.ВидыНачисленийДополненияРасчетнойБазы = Обработки.МенеджерРасчетаЗарплатыАрхивный.ВидыНачисленийДополненияРасчетнойБазыУдержаний();
...
Процедура СоздатьВТНачисленияДляОтраженияВБухучете(Начисления, ИмяВТНачисления)
	...
	|	Начисления.КорректировкаРанееВыполненногоНачисления
	|ПОМЕСТИТЬ ВТНачисленияТаблица
	|ИЗ
	|	&Начисления КАК Начисления
	|;
...
Процедура СоздатьВТУсловияРазбиенияСтрокНачислений()
	...
	Запрос.Текст = 
		...
		|	НЕ ЕСТЬNULL(ВидыРасчета.НачисляетсяВЦеломЗаМесяц, ЛОЖЬ)
		|		ИЛИ ЕСТЬNULL(ВидыРасчета.ЕстьНакапливаемыеПоказатели, ЛОЖЬ) КАК УчитыватьКадровыеИзменения,
		|	ВЫБОР
		...
		|				ИЛИ ЕСТЬNULL(ВидыРасчета.ИспользоватьОперативныеПоказателиВЦеломЗаМесяц, ЛОЖЬ))
		|				И ИСТИНА В
		|					(ВЫБРАТЬ ПЕРВЫЕ 1
		|						ИСТИНА
		|					ИЗ
...
Процедура ЗаполнитьКадровыеСведенияВТСПериодами(ОписаниеВТСПериодами, РазбиватьВТПериоды = Истина)
	...
	Если РазбиватьВТПериоды Тогда
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	&ИменаПолей,
			|	ЕСТЬNULL(КадровыеИзменения.Организация, &Организация) КАК Организация,
			|	ЕСТЬNULL(КадровыеИзменения.Организация.ГоловнаяОрганизация, &ГоловнаяОрганизация) КАК ГоловнаяОрганизация,
			|	КадровыеИзменения.Подразделение,
			|	КадровыеИзменения.ДолжностьПоШтатномуРасписанию,
			|	КадровыеИзменения.ВидСобытия,
			|	ВЫБОР
			|		КОГДА СотрудникиПериоды.БезусловноСоответствуетКадровымДанным = ИСТИНА
			|			ТОГДА СотрудникиПериоды.ДатаНачала
			|		ИНАЧЕ ЕСТЬNULL(КадровыеИзменения.НачалоПериода, СотрудникиПериоды.ДатаНачала)
			|	КОНЕЦ КАК ДатаНачала,
			|	ВЫБОР
			|		КОГДА СотрудникиПериоды.БезусловноСоответствуетКадровымДанным = ИСТИНА
			|			ТОГДА СотрудникиПериоды.ДатаОкончания
			|		ИНАЧЕ ЕСТЬNULL(КадровыеИзменения.КонецПериода, СотрудникиПериоды.ДатаОкончания)
			|	КОНЕЦ КАК ДатаОкончания
			|ПОМЕСТИТЬ ВТВсеСотрудникиПериоды
			|ИЗ
			|	ВТСотрудникиПериоды КАК СотрудникиПериоды
...
Процедура СоздатьВТЗначенияПериодическихПоказателейПодразделенийИнтервалы(ОписаниеВТПодразделенияПериоды)
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(ОписаниеВТПодразделенияПериоды.ИмяВТ, "Подразделение,Показатель");
	ЗарплатаКадрыПериодическиеРегистры.СоздатьВТИмяРегистраПериоды(
...
Процедура СоздатьВТЗначенияОперативныхПоказателейСотрудников(ОписаниеВТ)
	...
	// и вычисления накапливаемых показателей, в том числе в целом за месяц.
	Запрос = НовыйЗапрос();
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	НАЧАЛОПЕРИОДА(СотрудникиПериоды.НачалоПериода, МЕСЯЦ) КАК НачалоПериода,
		|	КОНЕЦПЕРИОДА(СотрудникиПериоды.ОкончаниеПериода, МЕСЯЦ) КАК ОкончаниеПериода,
	...
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	ЗначенияОперативныхСотрудников.Период КАК Период,
		|	ЗначенияОперативныхСотрудников.Сотрудник КАК Сотрудник,
		|	ЗначенияОперативныхСотрудников.Показатель КАК Показатель,
		|	ЗначенияОперативныхСотрудников.Организация КАК Организация,
		|	ЗначенияОперативныхСотрудников.Организация.ГоловнаяОрганизация КАК ГоловнаяОрганизация,
		|	ЗначенияОперативныхСотрудников.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ЗначенияОперативныхСотрудников.Значение КАК Значение
		...
		|			И (ЗначенияОперативныхСотрудников.ИспользоватьПриРасчетеПервойПоловиныМесяца)";
	
	Если Не НастройкиРасчета.ПерваяПоловинаМесяца Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "И (ЗначенияОперативныхСотрудников.ИспользоватьПриРасчетеПервойПоловиныМесяца)", "И ИСТИНА");
	КонецЕсли;
...
Процедура СоздатьВТПлановыеНачисленияИнтервалы(ОписаниеВТСотрудникиПериоды)
	...
	ОписаниеФильтра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеФильтраДляСоздатьВТИмяРегистра(
		"ВТФильтрПлановыеНачисления", ИзмеренияФильтра);
...
Процедура ИнициализироватьМенеджерУчетаВремени(ОписаниеВТ)
	...
	Описание.ТаблицаИсточникДанных = ОписаниеВТ.ИмяВТ;
...
Процедура ИнициализироватьРасчетБазыНачислений()
	...
	ОписаниеРассчитываемогоНабора = РасчетЗарплатыРасширенный.ОписаниеРассчитываемогоНабораНачислений();
	ОписаниеРассчитываемогоНабора.ИмяВТРассчитываемыеЗаписи = "ВТОсновныеЗаписиНачислений";
	ОписаниеРассчитываемогоНабора.ИмяТаблицыБазовыеЗаписи = "ПланВидовРасчета.Начисления.БазовыеВидыРасчета";
	ОписаниеРассчитываемогоНабора.ИмяПоляОчередностьРасчета = "ОчередностьРасчета";
	...
	ОписаниеРассчитываемогоНабора.ОписаниеТекущегоНабораРегистраНачисления = ОписаниеТекущегоНабораРегистраНачисления;
	
	РасчетБазыНачислений = РасчетЗарплатыРасширенный.РасчетБазыНачислений(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабора, МенеджерДанныхУчетаВремени, МенеджерКадровогоУчета());
...
Процедура ИнициализироватьРасчетБазыУдержаний()
	...
	ОписаниеРассчитываемогоНабора = РасчетЗарплатыРасширенный.ОписаниеРассчитываемогоНабораУдержаний();
	ОписаниеРассчитываемогоНабора.ИмяВТРассчитываемыеЗаписи = "ВТОсновныеЗаписиУдержаний";
	ОписаниеРассчитываемогоНабора.ИмяТаблицыБазовыеЗаписи = "ПланВидовРасчета.Удержания.БазовыеВидыРасчета";
	ОписаниеРассчитываемогоНабора.ИмяПоляОчередностьРасчета = "ОчередностьРасчета";
	...
	УстановитьПривилегированныйРежим(Истина);
	РасчетБазыУдержаний = РасчетЗарплатыРасширенный.РасчетБазыУдержаний(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабора, МенеджерДанныхУчетаВремени, МенеджерКадровогоУчета());
	УстановитьПривилегированныйРежим(Ложь);
КонецПроцедуры
