...
#Область ПолучениеКлючаАвторизации
	Если ПустаяСтрока(ЭтотОбъект.authKey) Тогда
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ОблачныйАрхив.ПолучитьПроксиВебСервисаБэкап1СПриватный(ВложенныйКонтекстВыполненияПолученияКлючаАвторизации);
...
Процедура ПриОткрытии(Отказ)
	...
	#Если НЕ ВебКлиент И НЕ МобильныйКлиент Тогда
		...
		Иначе
			...
			УправлениеФормой(ЭтотОбъект);
			ЭтотОбъект.ПодключитьОбработчикОжидания("Подключаемый_ЗагрузитьФайлРезервнойКопии", 0.1, Истина);
...
Процедура КомандаПодробности(Команда)
	...
	Если НЕ ПустаяСтрока(ТекущаяПодсказка) Тогда
		Текст = Новый ТекстовыйДокумент; // АПК:561 форма не открывается ни в мобильном клиенте, ни в веб-клиенте.
...
//
&НаКлиенте
Процедура Подключаемый_ЗагрузитьФайлРезервнойКопии()
	...
	#Если НЕ ВебКлиент Тогда
		...
		Попытка
			ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ХТТПСоединение = Новый HTTPСоединение(
					...
					ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(), // ЗащищенноеСоединение
		...
		Если ЕстьОшибки = Ложь Тогда
			ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла"; // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			// Если это повторная загрузка, то счетчики количества попыток загрузки (БылоПопытокЗагрузки) должны быть сброшены.
			ЭтотОбъект.ПодключитьОбработчикОжидания("Подключаемый_ЗагрузитьОчереднуюЧастьФайлаРезервнойКопии", 0.1, Истина);
		Иначе
...
//
&НаКлиенте
Процедура Подключаемый_ЗагрузитьОчереднуюЧастьФайлаРезервнойКопии()
		...
		Если ЭтотОбъект.ИдентификаторСостоянияЗагрузкиФайла = "ЗагрузкаФайла" Тогда // Идентификатор: ЗагрузкаФайла, ОтменаЗагрузки, ФайлЗагружен, ОшибкиЗагрузки, РаспаковкаФайла, ФайлРаспакован, ОшибкиРаспаковки.
			...
			Если ТекущаяСтрока <> Неопределено Тогда
				...
				Иначе
					...
					ИначеЕсли КодВозврата = "7" Тогда // Получить повторно ключ авторизации.
						ПолучитьНовыйКлючАвторизацииНаСервере();
						ОписаниеРезультата = ОписаниеРезультата
							+ НСтр("ru='Процесс загрузки был слишком длительный, ключ авторизации был просрочен и будет получен заново.'")
								+ Символы.ПС;
					Иначе
						ОписаниеРезультата = ОписаниеРезультата
							+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Попытка
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					ТекстКомандногоФайла = Новый ЗаписьТекста(
						...
						ТекстКомандногоФайла.ЗаписатьСтроку(СтрокаКоманды);
					...
					//@skip-warning
					РезультатЗапускаПрограммы = ОбщегоНазначенияКлиентСервер.ЗапуститьПрограмму(ПутьККомандномуФайлу, ПараметрыЗапускаПрограммы);
					КодВозвратаПрограммы = РезультатЗапускаПрограммы.КодВозврата;
				...
				Иначе
					...
					ПодробноеПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Код возврата программы разархивации отличается от 0.
							|Строка команды:
							|%1
							|Код возврата: %2
							|Поток ошибок:
...
Процедура ЗаписатьСообщениеВЛогНаСервере()
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Облачный архив. Веб-сервисы. Загрузка файла резервной копии'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
