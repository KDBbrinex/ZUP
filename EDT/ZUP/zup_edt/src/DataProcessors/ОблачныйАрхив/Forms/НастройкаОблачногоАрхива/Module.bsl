...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МассивПодстрок.Добавить(
		Новый ФорматированнаяСтрока(
			НСтр("ru='Журнал работы'"),,ЦветаСтиля.ГиперссылкаЦвет,,"backup1C:BackupAgentEventLog"));
	МассивПодстрок.Добавить(" ");
	...
	ИначеЕсли ПроверкаНеобходимостиОбновленияАгента.ТребуетсяОбновление = 1 Тогда // Доступно обновление.
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				НСтр("ru='Обновить'"),,ЦветаСтиля.ГиперссылкаЦвет,,"backup1C:UpdateAgentOnThisPC"));
		МассивПодстрок.Добавить(".");
		МассивПодстрок.Добавить(Символы.ПС);
		МассивПодстрок.Добавить(Символы.ПС);
	...
	Элементы.ДекорацияЖурналРаботыРезервногоКопированияИВерсияАгента.Заголовок
		= Новый ФорматированнаяСтрока(МассивПодстрок); // АПК:1356 Слишком сложно, чтобы выразить в одну строку.
	...
	УправлениеФормойНаСервере(ЭтотОбъект);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора) = ТипФормаКлиентскогоПриложения Тогда
		Если ИсточникВыбора.КлючНазначенияИспользования = "НастройкаРасписания" Тогда
			Если ТипЗнч(ВыбранноеЗначение) = ТипСтруктура Тогда
				...
				УправлениеФормойНаКлиенте(); // Перерисовать надписи.
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура ИнициализироватьРасписание()
	...
	Иначе
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ИмяВременногоФайлаНастроекИБ = ПолучитьИмяВременногоФайла("xml"); // АПК:441 Отсутствует удаление файла.
				...
				ЗаписьХМЛ.ЗаписатьНачалоЭлемента("DatabaseBackupConfiguration");
					...
					ЗаписьХМЛ.ЗаписатьНачалоЭлемента("Schedule");
						...
						Если ЭтотОбъект.РасписаниеАвтоматическогоРезервногоКопирования.Вариант = "Ежедневно_ОдинРазВДень" Тогда
							...
							ЗаписьХМЛ.ЗаписатьАтрибут("time", Формат(УниверсальноеВремя(ТекущееРасписание.Время), "ДФ=HH:mm:ss")); // АПК:335 Специальный формат даты для сторонней программы.
							ЗаписьХМЛ.ЗаписатьКонецЭлемента();
						ИначеЕсли ЭтотОбъект.РасписаниеАвтоматическогоРезервногоКопирования.Вариант = "Ежедневно_НесколькоРазВДень" Тогда
							...
							ЗаписьХМЛ.ЗаписатьНачалоЭлемента("BackupDaily");
							ЗаписьХМЛ.ЗаписатьАтрибут("time", Формат(УниверсальноеВремя(ТекущееРасписание.ВремяНачала), "ДФ=HH:mm:ss")); // АПК:335 Специальный формат даты для сторонней программы.
							ЗаписьХМЛ.ЗаписатьАтрибут("endTime", Формат(УниверсальноеВремя(ТекущееРасписание.ВремяОкончания), "ДФ=HH:mm:ss")); // АПК:335 Специальный формат даты для сторонней программы.
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			УдалитьФайлы = Новый Массив;
				УдалитьФайлы.Добавить(ИмяВременногоФайлаНастроекИБ);
			ДополнительныеПараметры = Новый Структура;
				ДополнительныеПараметры.Вставить("УдалитьФайлы", УдалитьФайлы);
			СтрокаКоманды =
				""""
				+ ЭтотОбъект.КаталогУстановкиАгентаКопирования
				...
				+ " update_task_config -path="
					...
					+ ИмяВременногоФайлаНастроекИБ
					+ """";
			ОблачныйАрхивКлиент.ВыполнитьСтрокуКомандыАсинхронно(
				СтрокаКоманды,
				ЭтотОбъект.КаталогУстановкиАгентаКопирования,
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Запуск утилиты.
					|Строка команды: %1
					|Каталог установки агента копирования: %2
					|Код возврата: недоступно при асинхронном вызове'"),
				СтрокаКоманды,
				ЭтотОбъект.КаталогУстановкиАгентаКопирования),
			КонтекстВыполненияВложенный);
	...
	ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru='Облачный архив. Работа с внешними утилитами. Запуск Агента резервного копирования'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Процедура УправлениеФормойНаКлиенте()
	...
	ФорматированныеСтроки = Новый Структура;
	ФорматированныеСтроки.Вставить("ДекорацияРасписание_РасписаниеОтключено",
		СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru='Выполнять автоматическое резервное копирование <a href = ""backup1C:EditBackupScheduler"">по расписанию</a>.'")));

	УправлениеФормой(ЭтотОбъект, ФорматированныеСтроки);

КонецПроцедуры

&НаСервереБезКонтекста
Процедура УправлениеФормойНаСервере(Форма)
	...
	УправлениеФормой(Форма, ФорматированныеСтроки);

КонецПроцедуры

&НаКлиентеНаСервереБезКонтекста
Процедура УправлениеФормой(Форма, ФорматированныеСтроки)
	...
	Если Форма.РасписаниеВключено = Истина Тогда
		...
		МассивПодстрок.Добавить(
			Новый ФорматированнаяСтрока(
				...
				, // Шрифт
				Форма.ГиперссылкаЦвет, // ЦветТекста
				, // ЦветФона
	...
	Иначе
		Элементы.ДекорацияРасписание.Заголовок = ФорматированныеСтроки.ДекорацияРасписание_РасписаниеОтключено;
