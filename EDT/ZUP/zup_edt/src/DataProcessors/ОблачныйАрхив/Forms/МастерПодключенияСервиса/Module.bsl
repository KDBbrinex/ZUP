			...
			Если (ЭтотОбъект.АгентКопированияУстановлен) И (ЭтотОбъект.ТребуетсяОбновление = 0) Тогда
				...
				Иначе
					...
					Иначе
						...
						МассивПодстрок.Добавить(НСтр("ru='Недостаточно свободных лицензий для активации Агента резервного копирования.'"));
						МассивПодстрок.Добавить(Символы.ПС);
						МассивПодстрок.Добавить(Символы.ПС);
						МассивПодстрок.Добавить(
							СтроковыеФункции.ФорматированнаяСтрока(
								НСтр("ru='Купить дополнительные лицензии на использование Агента резервного копирования на этом компьютере можно в <a href = ""backup1C:OpenWebPersonalAccount_BuyAgent"">личном кабинете</a>.'")));
						МассивПодстрок.Добавить(Символы.ПС);
						МассивПодстрок.Добавить(НСтр("ru='Также в личном кабинете можно освободить лицензии с других компьютеров.'"));
						МассивПодстрок.Добавить(Символы.ПС);
						МассивПодстрок.Добавить(
							СтроковыеФункции.ФорматированнаяСтрока(
								НСтр("ru='Для этого из <a href = ""backup1C:OpenWebPersonalAccount_DeactivateAgent"">личного кабинета</a> необходимо выбрать неиспользуемого Агента резервного копирования и нажать кнопку <b>Освободить</b>.'")));
						МассивПодстрок.Добавить(Символы.ПС);
						Элементы.ДекорацияУстановкаАгентаРезервногоКопированияЗавершенаСОшибкамиТекст.Заголовок =
							Новый ФорматированнаяСтрока(МассивПодстрок);
						ПереключитьсяНаСтраницуНаСервере("СтраницаУстановкаАгентаРезервногоКопированияЗавершенаСОшибками");
					КонецЕсли;
				КонецЕсли;
			Иначе
...
Процедура ДекорацияЗарегистрироватьсяНаПортале1СИТСНажатие(Элемент)
	...
	ФайловаяСистемаКлиент.ОткрытьНавигационнуюСсылку("https://login.1c.ru/registration");
...
#Область СозданиеФайловНастроек
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			// Например, при инсталляции всегда создается локальный администратор 1CBackupAgent, и в настройках
			...
			// Возможна ситуация, когда в конце инсталляции файл настроек еще не успел сохраниться
			//  в файловой системе - дождаться, когда файл будет создан и записан.
			ТекущееВремяПроверки = ТекущаяУниверсальнаяДатаВМиллисекундах();
			ВремяКонцаПроверки  = ТекущееВремяПроверки + 20000; // Не более 20 секунд проверок.
			ОсталосьПроверок = 200; // Не более 200 проверок.
			Пока (ТекущееВремяПроверки <= ВремяКонцаПроверки) И (ОсталосьПроверок > 0) Цикл
				ФайлНастроекАгента = Новый Файл(ИмяФайлаНастроекАгента);
				Если ФайлНастроекАгента.Существует() Тогда
					РазмерФайла = ФайлНастроекАгента.Размер();
					Если РазмерФайла > 0 Тогда
						Прервать;
					КонецЕсли;
				КонецЕсли;
				ТекущееВремяПроверки = ТекущаяУниверсальнаяДатаВМиллисекундах();
				ОсталосьПроверок = ОсталосьПроверок - 1;
			КонецЦикла;
			ФайлНастроекАгента = Новый Файл(ИмяФайлаНастроекАгента);
			Если ФайлНастроекАгента.Существует() Тогда
...
#Область АктивацияАгентаКопирования
		...
		Если ЕстьОшибки <> Истина Тогда
			...
			ОписаниеРезультата = Новый ФорматированнаяСтрока(
				ОписаниеРезультата,
				Символы.ПС,
				НСтр("ru='Активация Агента резервного копирования: успешно получены коды запуска, завершён первый запуск'")); // АПК:163 выводится пользователю.
