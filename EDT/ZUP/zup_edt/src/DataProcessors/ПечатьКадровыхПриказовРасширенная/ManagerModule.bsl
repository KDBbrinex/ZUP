...
Функция ДанныеДляПечатиТрудовогоДоговора(МассивОбъектов, ИмяМакета)
	...
	Пока Выборка.Следующий() Цикл
		...
		РезультатСклонения = "";
		Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ПараметрыТрудовогоДоговора.РуководительФИОПолные), 2, РезультатСклонения, ПараметрыТрудовогоДоговора.РуководительПол, Выборка.Руководитель) Тогда
...
Процедура ВывестиДанныеСправкиВТабличныйДокумент(Макет, ТабличныйДокумент, МассивДанныхЗаполнения, ОбъектыПечати, ЭтоСправкаОДоходах = Ложь) Экспорт
	...
	Если Макет <> Неопределено Тогда
		...
		Для каждого ПараметрыМакета Из МассивДанныхЗаполнения Цикл
			...
			РезультатСклонения = "";
			Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ПараметрыМакета.ФИОПолные), 3, РезультатСклонения, ПараметрыМакета.Пол, ПараметрыМакета.ФизическоеЛицо) Тогда
...
Процедура СоздатьВТОбщиеДанныеСправок(МенеджерВременныхТаблиц, МассивОбъектов)
	...
	Если МассивОрганизаций.Количество() > 0 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ЗапрашиваемыеЗначения.ОснованиеПодписиРуководителя) Тогда
				НоваяСтрокаСведенияОбОрганизациях.ДолжностьРуководителя = ЗапрашиваемыеЗначения.ОснованиеПодписиРуководителя;
			ИначеЕсли ЗначениеЗаполнено(ЗапрашиваемыеЗначения.ДолжностьРуководителя) Тогда
			...
			Если ЗначениеЗаполнено(ЗапрашиваемыеЗначения.ОснованиеПодписиГлавногоБухгалтера) Тогда
				НоваяСтрокаСведенияОбОрганизациях.ДолжностьГлавногоБухгалтера = ЗапрашиваемыеЗначения.ОснованиеПодписиГлавногоБухгалтера;
			ИначеЕсли ЗначениеЗаполнено(ЗапрашиваемыеЗначения.ДолжностьГлавногоБухгалтера) Тогда
...
Функция ДанныеДляПечатиЗапискиРасчетаТ61(МассивОбъектов)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			РезультатСклонения = "";
			Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(Параметры.Работник), 5, РезультатСклонения, Параметры.Пол, Выборка.ФизическоеЛицо) Тогда
	...
	Пока ВыборкаПоСсылке.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ВыборкаПоВидуОтпуска.НайтиСледующий(Новый Структура("ЭтоОсновнойОтпуск", Истина)) Тогда
				...
				РезультатСклонения = "";
				Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(Параметры.Работник), 3, РезультатСклонения, Параметры.Пол, ВыборкаПоВидуОтпуска.ФизическоеЛицо) Тогда
					Параметры.Работник	= РезультатСклонения
				КонецЕсли;
	...
	Пока ДанныеДляПечати.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Иначе
			ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ДанныеДляПечати.ФИОГлавногоБухгалтера),
				...
				ДанныеДляПечати.ГлавныйБухгалтер);
		КонецЕсли;
	...
	Пока ДанныеПечати.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		Пока ДанныеПечати.СледующийПоЗначениюПоля("Сотрудник") Цикл 
			...
			ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ДанныеПечати.СотрудникФИОПолные), 3,
				ОбластьМакета.Параметры.СотрудникФИО, ДанныеПечати.Пол, ДанныеПечати.ФизическоеЛицо);
		...
		Если Сотрудники.Количество() > 0 Тогда
			...
			Если ДанныеСотрудников.Количество() > 0 Тогда
				...
				ФизическиеЛицаЗарплатаКадры.Просклонять(
					...
					ДанныеСотрудников[0].ФизическоеЛицо);
		...
		Если ЗначениеЗаполнено(ДанныеПечати.РуководительКадровойСлужбы) Тогда
			...
			Если ДанныеФизическогоЛица.Количество() > 0 Тогда
				...
				ФизическиеЛицаЗарплатаКадры.Просклонять(
					...
					ДанныеФизическогоЛица[0].ФизическоеЛицо);
...
Функция ТабличныйДокументПриказОПрекращенииОтпускаПоУходуЗаРебенком(Макет, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ДанныеПечати Из ДанныеПечатиОбъектов Цикл
		...
		Если ДанныеПечати.Значение.ЕстьВыплатыПособий Тогда
			...
			ОбалстьПодписи = Макет.ПолучитьОбласть("ПодписиСПособиями");
		Иначе
			ОбластьМакетаПриказ = Макет.ПолучитьОбласть("ПриказБезПособий");
			ОбалстьПодписи = Макет.ПолучитьОбласть("ПодписиБезПособий");
		КонецЕсли;
		...
		ОбалстьПодписи.Параметры.Заполнить(ДанныеПечати.Значение);
		ДокументРезультат.Вывести(ОбалстьПодписи);
...
//
Функция ТабличныйДокументПриказОбОплатеПоСреднемуЗаработку(Макет, МассивОбъектов, ОбъектыПечати)
