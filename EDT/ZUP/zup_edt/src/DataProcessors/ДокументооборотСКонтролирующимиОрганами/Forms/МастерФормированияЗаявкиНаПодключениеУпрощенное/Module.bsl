...
&НаКлиенте
Процедура СоглашениеСПФРНажатие(Элемент)
	ОткрытьФормуСоглашенияСПФР();
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьФормуСоглашенияСПФР()
	
	СохранитьЗаявление(Истина);
	СнятьМодифицированность(ЭтотОбъект);
	КонтекстЭДОКлиент.ОткрытьФормуСоглашенияСПФР(ДокументЗаявление.Ссылка);
...
Процедура ДействияФормыПечатьЗаявка(Команда)
	...
	КонтекстЭДОКлиент.ПечатьЗаявленияНаПодключение(ДокументЗаявление.Ссылка);
...
Процедура ИзменитьОформлениеСтраницыУспешнойОтправки()
	...
	ПоказыватьБлокПФР  = СдаватьВПФР И НЕ ЕстьСоглашениеСПФР;
...
&НаСервере
Процедура СохранитьЗаявление(ЗаписатьЗаявление = Истина, НовыйGUID = Ложь)
	...
	Иначе
		...
		НовыйДокументЗаявление.ПодписатьЭП							= СпособПодписания = 1 ИЛИ ИспользоватьСуществующий(ЭтотОбъект);
...
Процедура ДобавитьОтсканированныеФайлы(ПрисоединенныйФайл, ВидДокумента)
	...
	ОписаниеФайла.Вставить("Имя",    ДанныеФайла.ИмяФайла);
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ДобавитьОтсканированныеФайлы_ПослеОбработки", 
		ЭтотОбъект, 
		ВидДокумента);
...
&НаКлиенте
Процедура ДобавитьОтсканированныеФайлы_ПослеОбработки(Результат, ВидДокумента) Экспорт
	...
	Расширение = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(Имя);
	
	НовоеИмя = ВидыДокументов(ЭтотОбъект)[ВидДокумента].Представление + "." + Расширение;
...
&НаКлиенте
Функция ФайлыДляПредпросмотра(ВидДокумента)
...
Процедура ДобавитьФайлыСПредпросмотром(ВидДокумента)
	...
	ПараметрыФункции = ПараметрыПроцедурыВыбораФайлов(ВидДокумента, Описание.Представление);
	...
	ПараметрыФункции.Требования.Вставить("Требования",     Требования);
	ПараметрыФункции.Требования.Вставить("КлючТребований", "Заявление на подключение к 1С-Отчетности");
...
Процедура ВыбратьОдинФайлДокумента_ПослеПомещения(Результат, Адрес, ВыбранноеИмяФайла, ВходящийКонтекст) Экспорт
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ВыбратьОдинФайлДокумента_ПослеОбработки", 
		ЭтотОбъект, 
		ВходящийКонтекст);
...
&НаКлиенте
Процедура ВыбратьОдинФайлДокумента_ПослеОбработки(Результат, ВходящийКонтекст) Экспорт	
	...
	ФайлыДокумента = ОписаниеФайла(Имя, Адрес);
...
&НаКлиенте
Функция ПараметрыПроцедурыВыбораФайлов(ВидДокумента = "", ЗаголовокФормыПредпросмотра = "")
	...
	ПараметрыФункции.Вставить("Файлы",               ФайлыДляПредпросмотра(ВидДокумента));
	ПараметрыФункции.Вставить("Требования",          Требования);
	ПараметрыФункции.Вставить("РежимТолькоПросмотр", ЗапретитьИзменение);
	ПараметрыФункции.Вставить("Документ",            ЗаголовокФормыПредпросмотра);
...
Процедура ИзменитьОформлениеДокументов(Форма, СкрытьДокументы = Ложь)
	...
	Если СкрытьДокументы ИЛИ НЕ ИздатьНовый Тогда
		Возврат;
	КонецЕсли;
...
Функция ПроверитьНаправления(МастерДалее = Истина, ВыводитьСообщения = Истина)
	...
	Иначе
		...
		Если ВыводитьСообщения Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Уточните направления сдачи отчетности'"), ,"УказательНаправлений");
		КонецЕсли;
...
Процедура ИзменитьОформление1СЭДО()
	...
	ИначеЕсли ЗапретитьИзменение И ПодключитьЭДО И НЕ ЭтоНоваяСхемаЭДО Тогда

		Элементы.ОткрытьПараметрыПодключения.Заголовок   = НСтр("ru = 'Настройки'");
		Элементы.ОткрытьПараметрыПодключения.ЦветТекста  = СинийЦвет;
	...
	Иначе
		...
		Иначе
			...
			Если ЗначениеЗаполнено(ОператорЭДО) И ЗначениеЗаполнено(КодНалоговогоОрганаЭДО) 
				И КонтекстЭДОСервер.ПроверитьПараметры1СЭДО(ЭтотОбъект, Истина) Тогда
	...
	ИначеЕсли ОткрытьВСтаромФормате Тогда
		...
		Если ЗначениеЗаполнено(СтруктураПараметров.МассивОператоровЭДО) Тогда
			...
			Если СтруктураПараметров.ЕстьПодключениеЭДО = Ложь Тогда
				...
				ОператорыЭДО.ЗагрузитьЗначения(СтруктураПараметров.МассивОператоровЭДО); // различаются
				...
				Иначе
					ПодключитьЭДО = НЕ ВыбраноОблачноеЭП; // различаются
				КонецЕсли;
...
Процедура ИнициализироватьПараметрыУЦ()
	...
	ПризнакПоддержкиФСРАР = ПараметрыСпецоператора.ФСРАРПризнак = Истина 
...
&НаСервере
Функция ИнициализироватьТаблицуОрганов()
	
	Направления = Новый ТаблицаЗначений;
	Направления.Колонки.Добавить("Орган");
	Направления.Колонки.Добавить("Подключить");
	Направления.Колонки.Добавить("ПризнакПоддержки");
	Направления.Колонки.Добавить("Детализация");
	
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'ФНС'"), СдаватьВФНС, ПризнакПоддержкиФНС, ПолучателиФНС);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'ПФР'"), СдаватьВПФР, ПризнакПоддержкиПФР);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'Росстат'"), СдаватьВРосстат, ПризнакПоддержкиРосстат, ПолучателиФСГС);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'ФСС'"), СдаватьВФСС, ПризнакПоддержкиФСС);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'ФСРАР'"), ПодатьЗаявкуНаСертификатДляФСРАР, ПризнакПоддержкиФСРАР);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'РПН'"), ПодатьЗаявкуНаПодключениеРПН, ПризнакПоддержкиРПН);
	ДобавитьСтрокуНаправления(Направления, НСтр("ru = 'ФТС'"), ПодатьЗаявкуНаПодключениеФТС, ПризнакПоддержкиФТС);
...
&НаСервере
Процедура ДобавитьСтрокуНаправления(Направления, Орган, Подключить, ПризнакПоддержки, Детализация = Неопределено)
	
	НоваяСтрока = Направления.Добавить();
	НоваяСтрока.Орган = Орган;
	НоваяСтрока.Подключить = Подключить;
	НоваяСтрока.ПризнакПоддержки = ПризнакПоддержки;
	НоваяСтрока.Детализация = Детализация;
	
КонецПроцедуры

&НаСервере
Функция КоличествоВыбраныхОрганов(Направления, Подключить = Неопределено, ПризнакПоддержки = Неопределено)
	
	Отбор = Новый Структура();
	
	Если Подключить <> Неопределено Тогда
		Отбор.Вставить("Подключить", Подключить);
	КонецЕсли;
	
	Если ПризнакПоддержки <> Неопределено Тогда
		Отбор.Вставить("ПризнакПоддержки", ПризнакПоддержки);
	КонецЕсли;
	
	НайденныеСтроки = Направления.НайтиСтроки(Отбор);
...
&НаСервере
Функция НеВыбранНиОдинОрган(Направления)
	
	Возврат КоличествоВыбраныхОрганов(Направления, Истина, Истина) = 0;
	
КонецФункции

&НаСервере
Функция ВыбранТолькоОдинОрган(Направления)
	
	Возврат КоличествоВыбраныхОрганов(Направления, Истина, Истина) = 1;
	
КонецФункции

&НаСервере
Функция ВыбраныВсеНаправления(Направления)
	
	Поддерживаемые = КоличествоВыбраныхОрганов(Направления, ,Истина);
	Подключенные   = КоличествоВыбраныхОрганов(Направления, Истина,);
	ПодключенныеИПоддерживаемые = КоличествоВыбраныхОрганов(Направления, Истина, Истина);
	
	Возврат Поддерживаемые = Подключенные И Подключенные = ПодключенныеИПоддерживаемые;
	
КонецФункции

&НаСервере
Функция ПредставлениеОрганов(Направления)
	
	// Выбрана часть органов
	Органы = Новый Массив;
	Для каждого Направление Из Направления Цикл
		
		Если Направление.Подключить И Направление.ПризнакПоддержки Тогда
		
			ПредставлениеОргана = Направление.Орган;
			
			Если Направление.Детализация <> Неопределено И Направление.Детализация.Количество() > 1 Тогда
				ПредставлениеОргана = ПредставлениеОргана + "(" + Строка(Направление.Детализация.Количество()) + ")";
			КонецЕсли;
...
Процедура ИзменитьОформлениеНаправлений()
	...
	Направления = ИнициализироватьТаблицуОрганов();
	
	Если НеВыбранНиОдинОрган(Направления) ИЛИ НЕ ПроверитьНаправления(, Ложь) Тогда
	...
	ИначеЕсли ВыбраныВсеНаправления(Направления) И НЕ ВыбранТолькоОдинОрган(Направления) Тогда
	...
	Иначе
		...
		// Выбрана часть органов
		Элементы.УказатьНаправления.Заголовок  = ПредставлениеОрганов(Направления);
		Элементы.УказатьНаправления.ЦветТекста = Новый Цвет();
...
Процедура ИзменитьОформлениеТелефонаИПочтыДляПаролей(Форма)
	...
	ИздатьНовый = Форма.СпособПолученияСертификата = ПредопределенноеЗначение("Перечисление.СпособПолученияСертификата.ИздатьНовый");
	Элементы.ГруппаПроверкаТелефона.Видимость = ИздатьНовый;
...
Процедура ЗаполнитьСведенияИзСертификатаДляРеквизитов_ПослеЭкспортаСертификата(Результат, ВходящийКонтекст) Экспорт
	...
	Если СертификатДляРеквизитов.Свойство("ВладелецСтруктура") Тогда
		ВладелецСтруктура = СертификатДляРеквизитов.ВладелецСтруктура;
	ИначеЕсли СертификатДляРеквизитов.Свойство("Субъект") Тогда
		ВладелецСтруктура = СертификатДляРеквизитов.Субъект;
	Иначе
...
Процедура ИзменитьОформлениеСоглашенияСПФР()
	...
	Элементы.СоглашениеСПФР.Видимость = ЕстьСоглашениеСПФР И СдаватьВПФР;
