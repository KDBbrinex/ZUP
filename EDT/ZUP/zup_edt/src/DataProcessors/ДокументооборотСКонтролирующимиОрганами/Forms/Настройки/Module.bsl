...
Процедура СоздатьМенеджерКриптографииЗавершениеПослеПолученияХранилища(Хранилище, КонтекстВозврата) Экспорт
	...
	КонтекстВозврата.Вставить("НайденоСертификатов", 0);
	КонтекстВозврата.Вставить("ДобавленоСертификатов", 0);
	Для Каждого СтруктураУчетнойЗаписи Из ДанныеДляТестовогоКонтура1С Цикл
		ОтпечатокДвоичныеДанные = ПолучитьДвоичныеДанныеИзHexСтроки(СтруктураУчетнойЗаписи.СертификатРуководителя);
		КонтекстВозврата.Вставить("ТекущийСертификатРуководителя", СтруктураУчетнойЗаписи.СертификатРуководителя);
		Оповещение = Новый ОписаниеОповещения("СоздатьМенеджерКриптографииЗавершениеПослеПоискаСертификата", ЭтотОбъект, КонтекстВозврата);
		Хранилище.НачатьПоискПоОтпечатку(Оповещение, ОтпечатокДвоичныеДанные);
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьМенеджерКриптографииЗавершениеПослеПоискаСертификата(Сертификат, КонтекстВозврата) Экспорт
	
	КонтекстВозврата.ТекущийНомер = КонтекстВозврата.ТекущийНомер + 1;
	
	Если Сертификат <> Неопределено Тогда
		КонтекстВозврата.НайденоСертификатов = КонтекстВозврата.НайденоСертификатов + 1;
		Оповещение = Новый ОписаниеОповещения("СоздатьМенеджерКриптографииЗавершениеПослеВыгрузкиСертификата", ЭтотОбъект, КонтекстВозврата);
		Сертификат.НачатьВыгрузку(Оповещение);
	Иначе
		СоздатьМенеджерКриптографииЗавершениеПослеВыгрузкиСертификата(Неопределено, КонтекстВозврата);	
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура СоздатьМенеджерКриптографииЗавершениеПослеВыгрузкиСертификата(СертификатДвоичныеДанные, КонтекстВозврата) Экспорт
	
	Если СертификатДвоичныеДанные <> Неопределено Тогда
		СертификатРуководителяBase64 = Base64Строка(СертификатДвоичныеДанные);		
		Для Каждого СтруктураУчетнойЗаписи Из КонтекстВозврата.ДанныеДляТестовогоКонтура1С Цикл
			Если СтруктураУчетнойЗаписи.СертификатРуководителя = КонтекстВозврата.ТекущийСертификатРуководителя Тогда
				СтруктураУчетнойЗаписи.Вставить("СертификатРуководителяBase64", СертификатРуководителяBase64);
				КонтекстВозврата.ДобавленоСертификатов = КонтекстВозврата.ДобавленоСертификатов + 1;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Если КонтекстВозврата.ТекущийНомер = КонтекстВозврата.Количество
		И КонтекстВозврата.ДобавленоСертификатов = КонтекстВозврата.НайденоСертификатов Тогда
...
Функция ДанныеДляТестовогоКонтура1С()
	...
	ЗапросУчетныхЗаписей = Новый Запрос("ВЫБРАТЬ
