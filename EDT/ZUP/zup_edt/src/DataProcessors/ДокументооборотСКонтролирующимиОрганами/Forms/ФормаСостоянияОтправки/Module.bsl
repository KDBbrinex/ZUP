	...
	Иначе
		...
		Если ТекущийЭтап <> Неопределено Тогда
			...
			Если ДокументооборотСФССКлиентСервер.ЭтоРеестрДанныхЭЛНЗаполняемыхРаботодателем(Ссылка,
				ОбъектПоУмолчаниюРеестрЭЛНСуществует)
				ИЛИ ТипЗнч(Ссылка) = Тип("СправочникСсылка.ЭлектронныеПредставленияРегламентированныхОтчетов")
...
Процедура ОтпределитьСоставМенюВыгрузка()
	...
	Если ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС Тогда
		...
		Если ЭтоДанныеЭЛН ИЛИ ЭтоПодтверждениеВидаДеятельности Тогда
	...
	Если ВидКонтролирующегоОргана = Перечисления.ТипыКонтролирующихОрганов.ФСС
		...
		Если ЗначениеЗаполнено(Отправка) Тогда
			...
			Если (Отправка.СтатусОтправки <> Перечисления.СтатусыОтправки.Отправлен
				...
				ИЛИ ЭтоДанныеЭЛН ИЛИ ЭтоПодтверждениеВидаДеятельности Тогда
...
Функция ПолучитьАдресФайлаКвитанции(
	...
	Если ТипЗнч(ОтправкаСсылка) <> Тип("СправочникСсылка.ОтправкиФТС") Тогда
		...
		ИмяФайлаКвитанции = ОтправкаСсылка.ИдентификаторОтправкиНаСервере
			+ ?(ЭтоДанныеЭЛН ИЛИ ЭтоПодтверждениеВидаДеятельности, ".xml", ".p7e");
