...
Процедура ЗаполнитьСводку(РезультатыДиагностики, ВсегоОшибок, ВсегоПредупреждений)
	...
	ИначеЕсли НЕ РезультатыДиагностики.Отчет.Проверки.Свойство("ПроверкаКриптоопераций") Тогда
		Если РезультатыДиагностики.Параметры.УчетнаяЗаписьОбмена.ЭлектроннаяПодписьВМоделиСервиса Тогда
			СсылкаГруппы = ДобавитьНадпись("Проверка криптоопераций не производится при ключах в ""облаке""", 1);
			ВсегоПредупреждений = ВсегоПредупреждений + 1;
			Сообщение = НайтиОписаниеПоКоду("130_4", МакетПодсказки, Ссылка1,);
			ДобавитьНадпись(Сообщение, СсылкаГруппы,,, Ссылка1,,, Истина);
	...
	Если Нерасшифрованные <> Неопределено И Нерасшифрованные.Количество() > 0 Тогда
		...
		Если Безнадежных > 0 Тогда 			
			...
			Сообщение = НайтиОписаниеПоКоду("130_3", МакетПодсказки, Ссылка1,);
			ДобавитьНадпись(Сообщение, СсылкаГруппы,,, Ссылка1,,, Истина);
...
Процедура ПриОткрытии(Отказ)
	...
	Если Элементы.Организация.СписокВыбора.Количество() = 0 Тогда 
		Сообщение = "Запуск средства диагностики возможен только при наличии подключения к 1С-Отчетности";
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (НЕ ОсновнаяОрганизация.Пустая()) И (ИспользуетсяОднаОрганизация) Тогда 
		...
		Если Учетка <> Неопределено И Учетка.ОбменВУниверсальномФормате И СпецоператорСвязиПоддерживается(Учетка.СпецоператорСвязи) Тогда 
			...
			ОбновитьЭлементыПриИзмененииОрганизации();
			Возврат;
		КонецЕсли;		
	КонецЕсли;
...
&НаСервере
Процедура ОбновитьЭлементыПриИзмененииОрганизации()
	
	ОсновнаяУчетнаяЗапись = ПолучитьУчетнуюЗапись(Организация);
	
	Если ЗначениеЗаполнено(ОсновнаяУчетнаяЗапись) И ОсновнаяУчетнаяЗапись.ЭлектроннаяПодписьВМоделиСервиса Тогда
		Элементы.ДекорацияТекстИнфо.Заголовок =
...
Функция ВыполнитьПроверку(Вид, КриптокомпонентГотов = Ложь, ПроверкаПодключенийЗавершена = Ложь)
	...
	Если Вид = 2 И НЕ РезультатыДиагностики.НастройкиДиагностики[Вид].Пометка Тогда
		Если ПроверкаПодключенийЗавершена Тогда
...
Функция ПоместитьИнформациюОбУчетнойЗаписиВОтчет()
	...
	РезультатыДиагностики.Отчет.Абонент.Вставить("НазваниеАбонента", ?(ЕстьРеквизитОрганизацииНаименованиеСокращенное,
		ОрганизацияКлиент.НаименованиеСокращенное, ОрганизацияКлиент.Наименование));
...
Функция ПолучитьИнформациюФайлаАвтонастройки()
	...
	Если КонтекстЭДОКлиент.РегистрацияПолучитьДанныеССервераКлиент(ПараметрыФункции, Ложь) Тогда
...
Процедура НачатьВызовPKCS7ПолучитьСвойстваПолучателейЗавершение(Результат, ПараметрыВызова, ВхПараметры) Экспорт
	...
	Для Каждого ВременныйФайл Из ПараметрыВызова Цикл 
		...
		Если НЕ ЭтоВеб И НЕ ЭтоLinux Тогда
...
Процедура ИзвлечьСведенияИзКонтейнеров(Результат, Проверяемые) Экспорт	
	...
	Если ЭтоВеб ИЛИ ЭтоLinux Тогда
	...
	Иначе
		// предотвращение ошибки автоматической проверки ПолучитьИмяВременногоФайла()
...
Функция ПолучитьДанныеТКС(ТССсылка, УИД)
	...
	Пока ВсеТК.Следующий() Цикл 
		...
		ФайлОписания = ЧтениеАрхива.Элементы.Найти(ИмяФайлаОписания);
		Если ФайлОписания = Неопределено Тогда
			РеквизитыЦиклаОбмена = ?(ЗначениеЗаполнено(ТССсылка.ЦиклОбмена),
				ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТССсылка.ЦиклОбмена, "Вид, ФорматДокументооборота"),
			...
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПервичноеСообщениеСодержащееОтчетностьПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПодтверждениеПолученияОтчетностиПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПротоколПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПротоколКвитанцияПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПисьмоИсходящееКвитанцияПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПодтверждениеПолученияЗапросаПФР
			ИЛИ ТССсылка.Тип = Перечисления.ТипыТранспортныхСообщений.ОтветНаЗапросПФР Тогда
				// обход ошибки платформы для архива zip с кодировкой имен файлов cp866, сформированного в Linux
				Попытка
					ЧтениеАрхива.Закрыть();
					МассивОписанийИДанных = ОбщегоНазначенияЭДКО.МассивОписанийИДанныхZipФайла(ПолныйПуть);
					Если МассивОписанийИДанных <> Неопределено Тогда
						ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ПолныйПуть);
						ПолныйПуть = ПолныйПуть + ".zip";
						ОбщегоНазначенияЭДКО.СформироватьZip(МассивОписанийИДанных, ПолныйПуть);
					КонецЕсли;
					ЧтениеАрхива = Новый ЧтениеZipФайла(ПолныйПуть);
				Исключение
				...
				Для каждого ЭлементКонтейнера Из ЧтениеАрхива.Элементы Цикл
					...
					Если Лев(ИмяФайлаВРег, 4) = "ПФР_" И СтрНайти(ИмяФайлаВРег, "_ОСП_") > 0 И нрег(РасширениеФайла) = "xml" Тогда
						...
						ФайлОписания = ЭлементКонтейнера;
		...
		ЧтениеОписания.ОткрытьФайл(КаталогКонтейнера + ИмяФайлаОписания);
		...
		Если ФорматДокументооборота = Перечисления.ФорматыДокументооборотаСПФР.ЭДОК Тогда
			...
			ДеревоОписанияСведений = КонтекстЭДОСервер.ЗагрузитьXMLВДеревоЗначений(КаталогКонтейнера + ИмяФайлаОписания);
			Если ДеревоОписанияСведений = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			
			УзелОСП = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(ДеревоОписанияСведений,,, "ОСП", "Э");
			Если НЕ ЗначениеЗаполнено(УзелОСП) Тогда
				ТранспортныеКонтейнеры[ТранспортныеКонтейнеры.Количество()-1].ОшибкаКонтейнера = 5;
				Продолжить;
			КонецЕсли;
			
			УзелЗашифровано = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелОСП,,, "Зашифровано", "Э", Ложь);
			ДокументыЗашифрованы = ?(ЗначениеЗаполнено(УзелЗашифровано),
				КонтекстЭДОСервер.БулевоПоСтроке(УзелЗашифровано.Значение), Неопределено);
			
			УзлыДокумент = Новый Массив;
			КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелОСП,, УзлыДокумент, "Документ", "Э", Ложь);
			УзелДополнительныеДокументы = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелОСП,,,
			...
			Если ЗначениеЗаполнено(УзелДополнительныеДокументы) Тогда
				...
				КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелДополнительныеДокументы,,
			...
			Для НомерУзловДокументов = 1 По ?(УзлыДополнительныйДокумент = Неопределено, 1, 2) Цикл
				...
				Для каждого УзелДокумент Из УзлыДокумент Цикл
					УзелКод = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелДокумент,,, "Код", "Э", Ложь);
					КодДокумента = ?(ЗначениеЗаполнено(УзелКод), СокрЛП(УзелКод.Значение), "");
					
					УзелФайл = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелДокумент,,, "Файл", "Э", Ложь);
					...
					УзелТипФайла = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(УзелДокумент,,, "ТипФайла", "Э", Ложь);
					ТипФайлаДокумента = ?(ЗначениеЗаполнено(УзелТипФайла), СокрЛП(УзелТипФайла.Значение), "");
					Если ТипФайлаДокумента = "application/gzip" Тогда // Обход ошибки ПФР
						ТипФайлаДокумента = "application/xml";
					КонецЕсли;
					...
					Иначе
						...
						ДокументЗашифрован = (РасширениеДокументаНРег = ".enc"
							...
							И КонтекстЭДОСервер.ТипСодержимогоПоИмениФайла(ИмяФайлаДокумента) =
							Перечисления.ТипыСодержимогоФайлов.Неизвестный));
					КонецЕсли;
					
					ИмяФайла = ИмяФайлаДокумента;
					
					ТипСодержимогоИРасширение = КонтекстЭДОСервер.ТипСодержимогоИРасширениеПоMIMEТипу(ТипФайлаДокумента);
					
					ПозицияРазделителя = СтрНайти(ИмяФайлаДокумента, ".");
					Если ПозицияРазделителя > 1 Тогда
						ИмяФайлаДокумента = Лев(ИмяФайлаДокумента, ПозицияРазделителя - 1);
					КонецЕсли;
					
					Если ЗначениеЗаполнено(ТипСодержимогоИРасширение.Расширение) Тогда
						ИмяФайлаДокумента = ИмяФайлаДокумента + "." + ТипСодержимогоИРасширение.Расширение;
					КонецЕсли;
					
					ИсходноеИмяФайла = ИмяФайлаДокумента;
	...
	Для Каждого ТС из ВсеСообщения Цикл 
		...
		Иначе
			...
			Если Стр = Неопределено Тогда
				Если ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПервичноеСообщениеСодержащееОтчетностьПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПодтверждениеПолученияОтчетностиПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПротоколПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПротоколКвитанцияПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПисьмоИсходящееКвитанцияПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ПодтверждениеПолученияЗапросаПФР
				ИЛИ ТС.Ссылка.Тип = Перечисления.ТипыТранспортныхСообщений.ОтветНаЗапросПФР Тогда
					Стр = ТС.Ссылка.ДополнительныеРеквизитыЗаголовка.Найти("X-PFR-Message-ID", "Тип");
				КонецЕсли;
				Если Стр = Неопределено Тогда
...
Функция ВыполнитьПроверкуПодключений() //2
	...
	Для Каждого Направление Из РезультатыДиагностики.Автонастройка.Получатели Цикл
		...
		Если Направление.Значение = "ФСРАР" Тогда 
			...
			Адрес = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("service.fsrar.ru");
			Порт = 443;
			...
			Адрес = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("service.alcolicenziat.ru");
			Порт = 443;
		...
		ИначеЕсли Направление.Значение = "ФТС" Тогда 
			...
			// Добавляем URL, так как при запросе только к хосту возвращается статус 301.
			Урл = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("/FtsPersonalCabinetWeb2017", "{FTS}/FtsPersonalCabinetWeb2017");
			ДанныеХоста = Новый Структура("Проверен, Адрес, Порт, Псевдоним, Урл", Ложь, Адрес, Порт, "ФТС", Урл);
			РезультатыДиагностики.Отчет.Проверки.ПроверяемыеХосты.Добавить(ДанныеХоста);
	...
	Адрес = КонтекстЭДОКлиент.СерверПриемаОтчетностиФСС(ВернутьТипДокументовФСС(2));
	Порт = 443;
	Урл = ПолучитьЗаменяемоеЗначениеТестовогоСервера1С("/ws-insurer-crypto-v11/FileOperationsLnPort", "{FSS}/ws-insurer-crypto-v11/FileOperationsLnPort");
...
&НаСервереБезКонтекста
Функция ПолучитьЗаменяемоеЗначениеТестовогоСервера1С(ИсходноеЗначение, ЗначениеДляОтправки = Неопределено) Экспорт
	
	Возврат ДокументооборотСКО.ПолучитьЗаменяемоеЗначениеТестовогоСервера1С(ИсходноеЗначение, ЗначениеДляОтправки, Неопределено);
