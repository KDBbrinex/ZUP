...
Процедура ТаблицаИсходящиеДокументыПФРПриАктивизацииСтроки(Элемент)
	...
	Если Элемент.Видимость Тогда
		Если ЦиклыОбменаИсходящиеПФР.Параметры.Элементы.Найти(Новый ПараметрКомпоновкиДанных("СсылкаДляОтбора")) <> Неопределено
			...
			Ссылка = Элементы.ТаблицаИсходящиеДокументыПФР.ТекущиеДанные.Ссылка;
			
			ЭтоЗаявление = ТипЗнч(Ссылка) = Тип("ДокументСсылка.ЗаявленияПоЭлДокументооборотуСПФР");
			Элементы.ТаблицаЦиклыОбменаИсходящиеПФРКолонка5.Видимость = ЭтоЗаявление;
			Элементы.ТаблицаЦиклыОбменаИсходящиеПФРКолонка6.Видимость = ЭтоЗаявление;
			
			ЦиклыОбменаИсходящиеПФР.Параметры.УстановитьЗначениеПараметра("СсылкаДляОтбора", Ссылка);
...
Процедура ТаблицаЦиклыОбменаВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбраннаяСтрока) = Тип("Число") Тогда
		Если Элемент.ТекущиеДанные <> Неопределено И Элемент.ТекущиеДанные.Свойство("ЦиклОбмена") Тогда
			ЦиклОбмена = Элемент.ТекущиеДанные.ЦиклОбмена;
		Иначе
			Возврат;
		КонецЕсли;
	Иначе
		ЦиклОбмена = ВыбраннаяСтрока;
	КонецЕсли;
		
	Результат = ВызватьВыборВТабличномПолеЦикловОбмена(ЦиклОбмена, КолонкаВыбора, СтандартнаяОбработка);
...
&НаКлиенте
Процедура КомандаИсходящиеДокументыПФРСоздатьЗаявлениеОНазначенииПенсии(Команда)
	
	ОткрытьФорму("Справочник.ЗаявлениеОНазначенииПенсии.ФормаОбъекта");
	
КонецПроцедуры

&НаКлиенте
Процедура КомандаИсходящиеДокументыПФРСоздатьЗаявлениеНаПодключениеКЭДОсПФР(Команда)
	
	Вид = ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаПодключение");
	ДокументооборотСКОКлиент.СоздатьНовоеИлиОткрытьЗаявлениеВПФР(Вид);
		
КонецПроцедуры

&НаКлиенте
Процедура КомандаИсходящиеДокументыПФРСоздатьЗаявлениеНаОтключениеОтЭДОсПФР(Команда)
...
&НаКлиенте
Процедура КомандаИсходящиеДокументыПФРСоздатьЗаявлениеНаНовыйСертификат(Команда)
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИмяСобытияЗаписиДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ =
		?(НЕ ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ), "", "Запись_" + ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ);
	...
	ИначеЕсли ИмяСобытия = "Запись_ЭлектронныеПредставленияРегламентированныхОтчетов" 
		...
		ИЛИ ИмяСобытия = ИмяСобытияЗаписиДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ
	...
	ИначеЕсли ИмяСобытия = "Запись_ОписиИсходящихДокументовВНалоговыеОрганы" 
		...
		И ТипЗнч(Источник) = Тип("СправочникСсылка.МакетыПенсионныхДел")
		ИЛИ ИмяСобытия = "Запись_ЗаявлениеОНазначенииПенсии"
		И ТипЗнч(Источник) = Тип("СправочникСсылка.ЗаявлениеОНазначенииПенсии")
		ИЛИ ИмяСобытия = "Запись_ЗаявленияВПФР"
		И ТипЗнч(Источник) = Тип("ДокументСсылка.ЗаявленияПоЭлДокументооборотуСПФР") Тогда
	...
	ИначеЕсли ИмяСобытия = "Показать циклы обмена уведомления" 
		...
		Если ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) Тогда
			Если Тип("ДокументСсылка." + ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) = ТипЗначенияПараметра Тогда 
				СменитьЗакладкуУведомленийПриНеобходимости("АвансыНДФЛ", Ложь);
			КонецЕсли;
		КонецЕсли;
...
&НаКлиенте
Процедура КоманднаяПанельОтчетностьФССВыгрузитьОтправленныйПодписанныйПакет(Команда)
	ВыгрузитьСодержимоеОтправкиФСС(3);
КонецПроцедуры
...
&НаКлиенте
Процедура КоманднаяПанельОтчетностьФССВыгрузитьПолученныйПакетКвитанции(Команда)
	ВыгрузитьСодержимоеОтправкиФСС(4);
КонецПроцедуры
...
Функция СодержимоеОтправкиФССВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФСС)
	
	Результат = Новый Структура;
	Результат.Вставить("АдресВоВременномХранилище", Неопределено);
	Результат.Вставить("ИмяФайлаДляКлиента", 		"");
	Результат.Вставить("ВидОтчета", 				СсылкаНаОбъектОтправкиФСС["ВидОтчета"]);
	
	Если РежимВыгрузки = 0 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФСС["ЗашифрованныйПакет"];
		Результат.ИмяФайлаДляКлиента = СсылкаНаОбъектОтправкиФСС["ИмяФайлаПакета"];
	ИначеЕсли РежимВыгрузки = 1 Тогда
		ЭтоПодтверждениеВидаДеятельности = (Результат.ВидОтчета =
		...
		Результат.ИмяФайлаДляКлиента = СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"]
	...
	ИначеЕсли РежимВыгрузки = 2 Тогда
		...
		Результат.ИмяФайлаДляКлиента = НСтр("ru = 'Протокол'")
			+ "_" + СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"] + ".html";
	ИначеЕсли РежимВыгрузки = 3 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФСС["ПодписанныйПакет"];
		Результат.ИмяФайлаДляКлиента = СсылкаНаОбъектОтправкиФСС["ИмяФайлаПакета"];
	ИначеЕсли РежимВыгрузки = 4 Тогда
		СсылкаНаСодержимое = СсылкаНаОбъектОтправкиФСС["ПакетКвитанции"];
		Результат.ИмяФайлаДляКлиента = НСтр("ru = 'Зашифрованная_квитанция'")
			+ "_" + СсылкаНаОбъектОтправкиФСС["ИдентификаторОтправкиНаСервере"] + ".xml";;
	Иначе
		Возврат Результат;
	КонецЕсли;
	...
	Результат.АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(Содержимое, Новый УникальныйИдентификатор);
...
Процедура ВыгрузитьСодержимоеОтправкиФСС(РежимВыгрузки)
	...
	СодержимоеОтправки = СодержимоеОтправкиФССВоВременномХранилищеНаСервере(
		РежимВыгрузки,
		Элементы.ТаблицаОтправкиОтчетностьФСС.ВыделенныеСтроки[0]);
	АдресВоВременномХранилище 	= СодержимоеОтправки.АдресВоВременномХранилище;
	ИмяФайла 					= СодержимоеОтправки.ИмяФайлаДляКлиента;
	ВидОтчетаОтправки 			= СодержимоеОтправки.ВидОтчета;
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			ЭтоПодтверждениеВидаДеятельности = (ВидОтчетаОтправки =
				ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПодтверждениеВидаДеятельности"));
			ПоказатьПредупреждение(, ?(ЭтоПодтверждениеВидаДеятельности, НСтр("ru = 'Квитанция отсутствует.'"),
				НСтр("ru = 'Квитанция об успехе отсутствует.'")));
			Возврат;
		КонецЕсли;
	...
	ИначеЕсли РежимВыгрузки = 3 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			...
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru = 'Выберите каталог для выгрузки отправленного отчета с подписью'");
	...
	ИначеЕсли РежимВыгрузки = 4 Тогда
		...
		Если АдресВоВременномХранилище = Неопределено Тогда
			...
			Возврат;
		КонецЕсли;
		ТекстЗаголовка = НСтр("ru = 'Выберите каталог для выгрузки полученной зашифрованной квитанции'");
...
&НаСервереБезКонтекста
Функция СуществуетРегламентированныйОтчетСтрановойОтчет()
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетСтрановойОтчет") <> Неопределено Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
&НаСервереБезКонтекста
Функция СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний()
	Если Метаданные.Отчеты.Найти("РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний") <> Неопределено Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
	КонецЕсли;
...
Процедура КомандаУведомленияСоздать(Команда)
	...
	СписокВариантовВыбора.Добавить("РеестрыАкцизы", НСтр("ru='Реестры по акцизам'"));
	
	Если ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) Тогда
		СписокВариантовВыбора.Добавить("АвансыНДФЛ", НСтр("ru = 'Заявление о подтверждении права на зачет авансов по НДФЛ'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетСтрановойОтчет() Тогда
		СписокВариантовВыбора.Добавить("СтрановыеОтчеты", НСтр("ru='Страновые отчеты'"));
	КонецЕсли;
	
	Если СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
		СписокВариантовВыбора.Добавить("УведомленияОбУчастииВМеждународнойГруппеКомпаний", НСтр("ru='Уведомления об участии в международной группе компаний'"));
	КонецЕсли;
...
Процедура КомандаУведомленияСоздатьЗавершение(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора <> Неопределено Тогда
		...
		ИначеЕсли РезультатВыбора.Значение = "АвансыНДФЛ" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("АвансыНДФЛ");
			ОткрытьФорму("Документ." + ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ + ".ФормаОбъекта", Новый Структура("Организация", Организация));
			
		ИначеЕсли РезультатВыбора.Значение = "СтрановыеОтчеты" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("СтрановыеОтчеты");
			
			ИсточникОтчета = "РегламентированныйОтчетСтрановойОтчет";
			
			ОткрытьФорму("Отчет." + ИсточникОтчета + ".Форма.ОсновнаяФорма", Новый Структура("Организация", Организация));
			
		ИначеЕсли РезультатВыбора.Значение = "УведомленияОбУчастииВМеждународнойГруппеКомпаний" Тогда
			СменитьЗакладкуУведомленийПриНеобходимости("УведомленияОбУчастииВМеждународнойГруппеКомпаний");
			
			ИсточникОтчета = "РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний";
		...
		ИначеЕсли РезультатВыбора.Значение = "РеестрыАкцизы" Тогда
			
			СменитьЗакладкуУведомленийПриНеобходимости("РеестрыАкцизы");
			
			ТекстВопроса = НСтр("ru = 'Создание реестров по акцизам выполняется из формы ""1С-Отчетность"".
                                 |Перейти к форме ""1С-Отчетность""?'");
...
Функция СодержимоеОтправкиФТСВоВременномХранилищеНаСервере(РежимВыгрузки, СсылкаНаОбъектОтправкиФТС, ИмяФайлаДляКлиента)
	...
	ИначеЕсли РежимВыгрузки = 1 Тогда
		...
		ИмяФайлаДляКлиента = НСтр("ru = 'Квитанция'") + " " + СсылкаНаОбъектОтправкиФТС["ИмяФайлаВыгрузки"] + ".bin";
	ИначеЕсли РежимВыгрузки = 2 Тогда
...
&НаКлиенте
Процедура КомандаОткрытьРеестрыАкцизы(Команда)
	УправлениеЗакладкойУведомлений("РеестрыАкцизы");
КонецПроцедуры
...
&НаКлиенте
Процедура КомандаОткрытьАвансыНДФЛ(Команда)
	УправлениеЗакладкойУведомлений("АвансыНДФЛ");
КонецПроцедуры

&НаКлиенте
Процедура КомандаОткрытьСтрановыеОтчеты(Команда)
	УправлениеЗакладкойУведомлений("СтрановыеОтчеты");
КонецПроцедуры

&НаКлиенте
Процедура КомандаОткрытьУведомленияОбУчастииВМеждународнойГруппеКомпаний(Команда)
	УправлениеЗакладкойУведомлений("УведомленияОбУчастииВМеждународнойГруппеКомпаний");
КонецПроцедуры
...
Функция АдресТаблицыПредметовИзЦикловОбмена(ИмяДинамическогоСписка)
	...
	МенеджерВременыхТаблиц = Новый МенеджерВременныхТаблиц;
	
	ЗапросПоЦикламОбмена = Новый Запрос();
	ЗапросПоЦикламОбмена.МенеджерВременныхТаблиц = МенеджерВременыхТаблиц;
	
	// Используем запрос и параметры динамического списка, чтобы получить временную таблицу циклов обмена, которые находятся в списке.
	СхемаЗапроса = Новый СхемаЗапроса;
	СхемаЗапроса.УстановитьТекстЗапроса(ТекстЗапроса);
	КоличествоЗапросов = СхемаЗапроса.ПакетЗапросов.Количество();
	СхемаЗапроса.ПакетЗапросов[КоличествоЗапросов - 1].ТаблицаДляПомещения = "втЦиклыОбмена";
	ТекстЗапроса = СхемаЗапроса.ПолучитьТекстЗапроса();
	ЗапросПоЦикламОбмена.Текст = ТекстЗапроса;
	...
	Для каждого ПараметрДинамическогоСписка Из ДинамическийСписок.Параметры.Элементы Цикл
		Если ПараметрДинамическогоСписка.Использование Тогда
			ЗапросПоЦикламОбмена.УстановитьПараметр(ПараметрДинамическогоСписка.Параметр, ПараметрДинамическогоСписка.Значение);
		КонецЕсли;
	КонецЦикла;
	
	ЗапросПоЦикламОбмена.Выполнить();
	
	// Выполняем запрос к временной таблице, чтобы получить получить список Предметов.
	ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               |	ЦиклыОбмена.Предмет КАК Предмет,
	               |	ЦиклыОбмена.Организация КАК Организация
	               |ИЗ
	               |	втЦиклыОбмена КАК ЦиклыОбмена
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ЦиклыОбмена.Предмет
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |УНИЧТОЖИТЬ втЦиклыОбмена";
					
	ЗапросПоПредметам = Новый Запрос(ТекстЗапроса);
	ЗапросПоПредметам.МенеджерВременныхТаблиц = МенеджерВременыхТаблиц;
...
Процедура УстановитьОтборПоПредмету(Форма)
	...
	Если КнопкаОтбора.Пометка Тогда
		...
		Если ЗначениеЗаполнено(Предмет) Тогда
			...
			// Применяем отбор
			Если ИмяДинамическогоСписка <> "ЦиклыОбменаОтчетностьФНС" И ИмяДинамическогоСписка <> "ЦиклыОбменаЗаявление"
				И ДинамическийСписок.Параметры.Элементы.Найти(Новый ПолеКомпоновкиДанных("Организация")) <> Неопределено Тогда
...
Функция ПолучитьНастройкиДляДинамическогоСписка(ИмяДинамическогоСписка)
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЗапросНаИнформационноеОбслуживаниеНалогоплательщика" Тогда
		...
		|					ТОГДА ВЫБОР
		|							КОГДА СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.УдалитьСтатусыРегламентированныхОтчетов.ВРаботе)
		|								ТОГДА 0
		|							ИНАЧЕ ВЫБОР
		|									КОГДА СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.УдалитьСтатусыРегламентированныхОтчетов.Подготовлен)
		|										ТОГДА 1
		|									ИНАЧЕ ВЫБОР
		...
		|																					ИЛИ СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.УдалитьСтатусыРегламентированныхОтчетов.Сдан)
		|																				ТОГДА 6
		|																		КОНЕЦ
		|																КОНЕЦ
		|														КОНЕЦ
	...
	ИначеЕсли ИмяДинамическогоСписка = "ИсходящиеДокументыПФР" Тогда
		...
		|	ПерепискаСКО.Получатель КАК Получатель,
		|	ПерепискаСКО.Наименование КАК Тема,
		|	ПерепискаСКО.Организация КАК Организация,
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ЗаявленияПоЭлДокументооборотуСПФР.ПометкаУдаления
		|			ТОГДА ВЫБОР
		|					КОГДА СтатусПереписки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПисем.Полученное)
		|						ТОГДА 7
		|					ИНАЧЕ ВЫБОР
		...
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ЗаявленияПоЭлДокументооборотуСПФР.Дата,
		|	ЗаявленияПоЭлДокументооборотуСПФР.Получатель,
		|	ЗаявленияПоЭлДокументооборотуСПФР.Вид,
		|	ЗаявленияПоЭлДокументооборотуСПФР.Организация,
		|	ЗаявленияПоЭлДокументооборотуСПФР.Ссылка
		|ИЗ
		|	Документ.ЗаявленияПоЭлДокументооборотуСПФР КАК ЗаявленияПоЭлДокументооборотуСПФР
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			СтатусыОтправки.Объект КАК Объект,
		|			СтатусыОтправки.Статус КАК Статус
		|		ИЗ
		|			РегистрСведений.СтатусыОтправки КАК СтатусыОтправки) КАК СтатусПереписки
		|		ПО (СтатусПереписки.Объект = ЗаявленияПоЭлДокументооборотуСПФР.Ссылка)
		|ГДЕ
		|	ЗаявленияПоЭлДокументооборотуСПФР.Организация В(&Организация)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА ЗаявлениеОНазначенииПенсии.ПометкаУдаления
		|			ТОГДА ВЫБОР
		|					КОГДА СтатусПереписки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПисем.Полученное)
		|						ТОГДА 7
		|					ИНАЧЕ ВЫБОР
		...
		|			КОНЕЦ
		|	КОНЕЦ,
		|	ЗаявлениеОНазначенииПенсии.ДатаСоздания,
		|	ЗаявлениеОНазначенииПенсии.Получатель,
		|	ЗаявлениеОНазначенииПенсии.Наименование,
		|	ЗаявлениеОНазначенииПенсии.Организация,
		|	ЗаявлениеОНазначенииПенсии.Ссылка
		|ИЗ
		|	Справочник.ЗаявлениеОНазначенииПенсии КАК ЗаявлениеОНазначенииПенсии
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			СтатусыОтправки.Объект КАК Объект,
		|			СтатусыОтправки.Статус КАК Статус
		|		ИЗ
		|			РегистрСведений.СтатусыОтправки КАК СтатусыОтправки) КАК СтатусПереписки
		|		ПО (СтатусПереписки.Объект = ЗаявлениеОНазначенииПенсии.Ссылка)
		|ГДЕ
		|	ЗаявлениеОНазначенииПенсии.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыПерепискиСКонтролирующимиОрганами.ПерепискаСПФР)
		|	И НЕ ЗаявлениеОНазначенииПенсии.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыПисем.Полученное)
		|	И ЗаявлениеОНазначенииПенсии.Организация В(&Организация)";
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаВходящиеФСГС" Тогда
		...
		|					РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|				ГДЕ
		|					СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение.Тип В (ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоВходящееФСГС), ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РассылкаФСГС), ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РассылкаШаблоновФСГС))
		|				
		|				СГРУППИРОВАТЬ ПО
		|					СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение) КАК СодержимоеТранспортногоКонтейнера
		|				ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|		ГДЕ
		|			ТранспортноеСообщение.Тип В (ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоВходящееФСГС), ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РассылкаФСГС), ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РассылкаШаблоновФСГС))) КАК ЗапросКолонка1
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка1.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ТранспортноеСообщение.Ссылка КАК Ссылка,
		|			ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена,
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаЗапросыИОН" Тогда
		...
		#Область ЗапросЦиклыОбменаЗапросыИОН
		Настройки.ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
		|ПОМЕСТИТЬ втНовыеСообщения
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|ГДЕ
		|	ТранспортноеСообщение.Ссылка В(&ПолученныеТранспортныеСообщения)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втТранспортныйКонтейнер
		|ИЗ
		|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
		|ГДЕ
		...
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ТранспортныйКонтейнерСообщение
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втЗапросКолонка1
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ЗапросНП)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|
		|СГРУППИРОВАТЬ ПО
		|	КрайнееСообщение.ЦиклОбмена
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена,
		|	СсылкаКрайнегоСообщения
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втСодержимоеТранспортногоКонтейнера
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втЗапросКолонка2
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втСодержимоеТранспортногоКонтейнера КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПодтверждениеЗапросНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втТранспортныйКонтейнер1
		|ИЗ
		|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втЗапросКолонка3
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение1 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер1 КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ИзвещениеПодтверждениеНП)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втСодержимоеТранспортногоКонтейнера1
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втЗапросКолонка4
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение2 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втСодержимоеТранспортногоКонтейнера1 КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ИзвещениеЗапросНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втСодержимоеТранспортногоКонтейнера2
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втЗапросКолонка5
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение3 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втСодержимоеТранспортногоКонтейнера2 КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РезультатПриемаЗапросНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втТранспортныйКонтейнер2
		|ИЗ
		|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втЗапросКолонка6
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение4 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер2 КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втСодержимоеТранспортногоКонтейнера3
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ втЗапросКолонка7
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение5 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втСодержимоеТранспортногоКонтейнера3 КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РезультатОбработкиЗапросНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втТранспортныйКонтейнер3
		|ИЗ
		|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ втЗапросКолонка8
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение6 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер3 КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ИзвещениеРезультатОбработкиНП)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|	ТранспортноеСообщение.Тип КАК Тип,
		|	ТранспортноеСообщение.Статус КАК Статус
		|ПОМЕСТИТЬ втПротоколСУведомлениемОшибкой
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение7 КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|ГДЕ
		|	(ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеФНС)
		...
		|	ЦиклыОбмена.ДатаЗакрытия КАК ДатаЗакрытия,
		|	ЦиклыОбмена.Предмет КАК Предмет,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(НовыеСообщения.ЦиклОбмена, 0) = 0
		|			ТОГДА ЛОЖЬ
		...
		|																		ИЛИ ЗапросКолонка1.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеФНС)
		|																	ТОГДА ИСТИНА
		|																ИНАЧЕ ЗапросКолонка1.Ссылка.ПротоколСОшибкой
		|																		ИЛИ ЗапросКолонка1.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеФНС)
		|															КОНЕЦ
		|													КОНЕЦ
		|											КОНЕЦ
		...
		|ИЗ
		|	Справочник.ЦиклыОбмена КАК ЦиклыОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втНовыеСообщения КАК НовыеСообщения
		|		ПО ЦиклыОбмена.Ссылка = НовыеСообщения.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка1 КАК ЗапросКолонка1
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка1.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка2 КАК ЗапросКолонка2
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка2.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка3 КАК ЗапросКолонка3
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка3.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка4 КАК ЗапросКолонка4
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка4.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка5 КАК ЗапросКолонка5
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка5.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка6 КАК ЗапросКолонка6
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка6.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка7 КАК ЗапросКолонка7
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка7.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка8 КАК ЗапросКолонка8
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка8.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ втПротоколСУведомлениемОшибкой КАК ПротоколСУведомлениемОшибкой
		|		ПО ЦиклыОбмена.Ссылка = ПротоколСУведомлениемОшибкой.ЦиклОбмена
		|ГДЕ
		|	ЦиклыОбмена.Вид = ЗНАЧЕНИЕ(Перечисление.ВидыЦикловОбмена.ЦиклОбменаСФНС)
		|	И ЦиклыОбмена.Предмет = &СсылкаДляОтбора";
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаЗаявление" Тогда
		...
		Если ВидДокументаЗаявленияОВвозе = "Заявление" Тогда
			...
			#Область ЗапросЦиклыОбменаЗаявление
			Настройки.ТекстЗапроса = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
			|ПОМЕСТИТЬ втНовыеСообщения
			...
			|	И ТранспортноеСообщение.ЦиклОбмена.Организация В (&Организация)
			|
			|ИНДЕКСИРОВАТЬ ПО
			|	ЦиклОбмена
			|;
			...
			|	И КрайнееСообщение.ЦиклОбмена.Организация В (&Организация)
			|
			|СГРУППИРОВАТЬ ПО
			|	КрайнееСообщение.ЦиклОбмена
			|
			...
			|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
			|ПОМЕСТИТЬ втСодержимоеТранспортногоКонтейнера4
			|ИЗ
			|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
			|ГДЕ
			...
			|	ТранспортноеСообщение.Статус КАК Статус,
			|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
			|ПОМЕСТИТЬ втЗапросКолонка9
			|ИЗ
			|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение7 КАК КрайнееСообщение
			|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
			|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
			|		ЛЕВОЕ СОЕДИНЕНИЕ втСодержимоеТранспортногоКонтейнера4 КАК СодержимоеТранспортногоКонтейнера
			|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
			|ГДЕ
			...
			|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
			|ПОМЕСТИТЬ втТранспортныйКонтейнер4
			|ИЗ
			|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
			|ГДЕ
			...
			|	ТранспортноеСообщение.Статус КАК Статус,
			|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
			|ПОМЕСТИТЬ втЗапросКолонка10
			|ИЗ
			|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение8 КАК КрайнееСообщение
			|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
			|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
			|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер4 КАК ТранспортныйКонтейнер
			|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
			|ГДЕ
			...
			|ИЗ
			|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКрайнееСообщение9 КАК КрайнееСообщение
			|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
			|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
			|ГДЕ
			|	(ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеФНС)
			...
			|	ЦиклыОбмена.ВидОтчета КАК ВидОтчета,
			...
			|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка8 КАК ЗапросКолонка8
			|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка8.ЦиклОбмена
			|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка9 КАК ЗапросКолонка9
			|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка9.ЦиклОбмена
			|		ЛЕВОЕ СОЕДИНЕНИЕ втЗапросКолонка10 КАК ЗапросКолонка10
			...
			|	И ЦиклыОбмена.ПометкаУдаления = ЛОЖЬ
			|	И ЦиклыОбмена.Организация В (&Организация)";
			#КонецОбласти
			
		ИначеЕсли ВидДокументаЗаявленияОВвозе = "ПереченьЗаявлений" Тогда
			
			#Область ЦиклыОбменаПереченьЗаявлений
			Настройки.ТекстЗапроса = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
			|ПОМЕСТИТЬ втНовыеСообщения
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаИсходящиеПФР" Тогда
		...
		"ВЫБРАТЬ
		|	ЦиклыОбмена.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ЦиклыОбменаПредмета
		|ИЗ
		|	Справочник.ЦиклыОбмена КАК ЦиклыОбмена
		|ГДЕ
		|	ЦиклыОбмена.Предмет = &СсылкаДляОтбора
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ТранспортноеСообщение.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ТранспортныеСообщенияПредмета
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|ГДЕ
		|	ТранспортноеСообщение.ЦиклОбмена В
		|			(ВЫБРАТЬ
		|				ЦиклыОбменаПредмета.Ссылка
		|			ИЗ
		|				ЦиклыОбменаПредмета)
		|;
		|
		...
		|	КрайнееСообщение.ЦиклОбмена КАК ЦиклОбмена,
		|	МАКСИМУМ(КрайнееСообщение.Ссылка) КАК СсылкаКрайнегоСообщения
		|ПОМЕСТИТЬ КрайнееСообщениеСОшибкой
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК КрайнееСообщение
		|ГДЕ
		|	(КрайнееСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР)
		|			ИЛИ КрайнееСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.КритическаяОшибка))
		|	И КрайнееСообщение.Ссылка В
		|			(ВЫБРАТЬ
		|				ТранспортныеСообщенияПредмета.Ссылка
		|			ИЗ
		|				ТранспортныеСообщенияПредмета)
		|
		|СГРУППИРОВАТЬ ПО
		|	КрайнееСообщение.ЦиклОбмена
		|;
		...
		|	ТранспортноеСообщение.Ссылка КАК Ссылка,
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
		|ПОМЕСТИТЬ СвойстваКрайнегоСообщенияСОшибкой
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ КрайнееСообщениеСОшибкой КАК КрайнееСообщениеСОшибкой
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщениеСОшибкой.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщениеСОшибкой.СсылкаКрайнегоСообщения
		|ГДЕ
		|	(ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.УведомлениеОбОшибкеПФР)
		|			ИЛИ ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.КритическаяОшибка))
		|;
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ КоличествоВложенийПодтвержденияДоставки
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	И СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение В
		|			(ВЫБРАТЬ
		|				ТранспортныеСообщенияПредмета.Ссылка
		|			ИЗ
		|				ТранспортныеСообщенияПредмета)
		...
		|ВЫБРАТЬ
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена,
		|	МАКСИМУМ(ТранспортноеСообщение.Ссылка) КАК СсылкаКрайнегоСообщения
		|ПОМЕСТИТЬ ПоследнееПодтверждениеДоставки
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|ГДЕ
		|	ТранспортноеСообщение.Тип В (ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоИсходящееКвитанцияПФР))
		|	И ТранспортноеСообщение.Ссылка В
		|			(ВЫБРАТЬ
		|				ТранспортныеСообщенияПредмета.Ссылка
		|			ИЗ
		|				ТранспортныеСообщенияПредмета)
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ СвойстваПоследегоПодтвержденияДоставки
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПоследнееПодтверждениеДоставки КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПодтвержденияДоставки КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоИсходящееКвитанцияПФР)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ СвойстваЦиклаОбменаИсходящегоПисьма
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПисьма КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПисьмоИсходящееПФР)
		|	И ТранспортноеСообщение.Ссылка В
		|			(ВЫБРАТЬ
		|				ТранспортныеСообщенияПредмета.Ссылка
		|			ИЗ
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
		|ПОМЕСТИТЬ ПолученныеТранспортныеСообщения
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|ГДЕ
		|	ТранспортноеСообщение.Ссылка В(&ПолученныеТранспортныеСообщения)
		...
		|	КОНЕЦ КАК ПротоколСУведомлениемОшибкой,
		|	ЦиклыОбмена.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ЦиклыОбменаПисьма
		|ИЗ
		|	Справочник.ЦиклыОбмена КАК ЦиклыОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПолученныеТранспортныеСообщения КАК НовыеСообщения
		|		ПО ЦиклыОбмена.Ссылка = НовыеСообщения.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваЦиклаОбменаИсходящегоПисьма КАК ЗапросКолонка1
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка1.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваПоследегоПодтвержденияДоставки КАК ЗапросКолонка4
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка4.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваКрайнегоСообщенияСОшибкой КАК ПротоколСУведомлениемОшибкой
		|		ПО ЦиклыОбмена.Ссылка = ПротоколСУведомлениемОшибкой.ЦиклОбмена
		|ГДЕ
		|	ЦиклыОбмена.Вид = ЗНАЧЕНИЕ(Перечисление.ВидыЦикловОбмена.ЦиклОбменаСПФР)
		|	И ЦиклыОбмена.Предмет = &СсылкаДляОтбора
		|	И ТИПЗНАЧЕНИЯ(ЦиклыОбмена.Предмет) <> ТИП(Документ.ЗаявленияПоЭлДокументооборотуСПФР)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	КрайнееСообщение.ЦиклОбмена КАК ЦиклОбмена,
		|	МАКСИМУМ(КрайнееСообщение.Ссылка) КАК СсылкаКрайнегоСообщения
		|ПОМЕСТИТЬ СамыйПоследнийПротоколКвитанцияПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК КрайнееСообщение
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ СвойстваКрайнегоПротоколаКвитанцияПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СамыйПоследнийПротоколКвитанцияПФР КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПротоколаКвитанцииПФР КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ КоличествоВложенийПротоколаПФР
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	КрайнееСообщение.ЦиклОбмена КАК ЦиклОбмена,
		|	МАКСИМУМ(КрайнееСообщение.Ссылка) КАК СсылкаКрайнегоСообщения
		|ПОМЕСТИТЬ СамыйПоследнийПротоколПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК КрайнееСообщение
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ СвойстваКрайнегоПротоколПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СамыйПоследнийПротоколПФР КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПротоколаПФР КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		...
		|	СодержимоеТранспортныхКонтейнеров.ТранспортноеСообщение КАК ТранспортноеСообщениеСсылка,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ СодержимоеТранспортныхКонтейнеров.ИмяФайла) КАК ЧислоСодержимого
		|ПОМЕСТИТЬ КоличествоВложенийПодтвержденияПолученияОтчетностиПФР
		|ИЗ
		|	РегистрСведений.СодержимоеТранспортныхКонтейнеров КАК СодержимоеТранспортныхКонтейнеров
		|ГДЕ
		...
		|	КрайнееСообщение.ЦиклОбмена КАК ЦиклОбмена,
		|	МАКСИМУМ(КрайнееСообщение.Ссылка) КАК СсылкаКрайнегоСообщения
		|ПОМЕСТИТЬ СамыйПоследнееПодтверждениеПолученияОтчетностиПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК КрайнееСообщение
		|ГДЕ
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	СодержимоеТранспортногоКонтейнера.ЧислоСодержимого КАК ЧислоСодержимого
		|ПОМЕСТИТЬ СвойстваКрайнегоПодтвержденияПолученияОтчетностиПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СамыйПоследнееПодтверждениеПолученияОтчетностиПФР КАК КрайнееСообщение
		|		ПО ТранспортноеСообщение.ЦиклОбмена = КрайнееСообщение.ЦиклОбмена
		|			И ТранспортноеСообщение.Ссылка = КрайнееСообщение.СсылкаКрайнегоСообщения
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПодтвержденияПолученияОтчетностиПФР КАК СодержимоеТранспортногоКонтейнера
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		...
		|	ТранспортныеКонтейнеры.ТранспортноеСообщение КАК ТранспортныйКонтейнерСообщение,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ТранспортныеКонтейнеры.ИмяФайла) КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ КоличествоВложенийПервичногоСообщенияСодержащееОтчетностьПФР
		|ИЗ
		|	РегистрСведений.ТранспортныеКонтейнеры КАК ТранспортныеКонтейнеры
		|ГДЕ
		...
		|	И ТранспортныеКонтейнеры.ТранспортноеСообщение В
		|			(ВЫБРАТЬ
		|				ТранспортныеСообщенияПредмета.Ссылка
		|			ИЗ
		|				ТранспортныеСообщенияПредмета)
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров
		|ПОМЕСТИТЬ СвойстваПервичноеСообщениеСодержащееОтчетностьПФР
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВложенийПервичногоСообщенияСодержащееОтчетностьПФР КАК ТранспортныйКонтейнер
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		...
		|	КОНЕЦ КАК ПротоколСУведомлениемОшибкой,
		|	ЦиклыОбмена.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ЦиклыОбменаЗаявления
		|ИЗ
		|	Справочник.ЦиклыОбмена КАК ЦиклыОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ ПолученныеТранспортныеСообщения КАК НовыеСообщения
		|		ПО ЦиклыОбмена.Ссылка = НовыеСообщения.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваПервичноеСообщениеСодержащееОтчетностьПФР КАК ЗапросКолонка1
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка1.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваКрайнегоПодтвержденияПолученияОтчетностиПФР КАК ЗапросКолонка4
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка4.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваКрайнегоПротоколПФР КАК ЗапросКолонка5
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка5.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваКрайнегоПротоколаКвитанцияПФР КАК ЗапросКолонка6
		|		ПО ЦиклыОбмена.Ссылка = ЗапросКолонка6.ЦиклОбмена
		|		ЛЕВОЕ СОЕДИНЕНИЕ СвойстваКрайнегоСообщенияСОшибкой КАК ПротоколСУведомлениемОшибкой
		|		ПО ЦиклыОбмена.Ссылка = ПротоколСУведомлениемОшибкой.ЦиклОбмена
		|ГДЕ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЦиклыОбменаЗаявления.ДатаСоздания КАК ДатаСоздания,
		|	ЦиклыОбменаЗаявления.ДатаЗакрытия КАК ДатаЗакрытия,
		|	ЦиклыОбменаЗаявления.Предмет КАК Предмет,
		|	ЦиклыОбменаЗаявления.Новое КАК Новое,
		|	ЦиклыОбменаЗаявления.Колонка1 КАК Колонка1,
		|	ЦиклыОбменаЗаявления.Колонка4 КАК Колонка4,
		|	ЦиклыОбменаЗаявления.Колонка5 КАК Колонка5,
		|	ЦиклыОбменаЗаявления.Колонка6 КАК Колонка6,
		|	ЦиклыОбменаЗаявления.ПротоколСОшибкойУведомлением КАК ПротоколСОшибкойУведомлением,
		|	ЦиклыОбменаЗаявления.ПротоколСУведомлениемОшибкой КАК ПротоколСУведомлениемОшибкой,
		|	ЦиклыОбменаЗаявления.Ссылка КАК ЦиклОбмена
		|ИЗ
		|	ЦиклыОбменаЗаявления КАК ЦиклыОбменаЗаявления
		|
		|ОБЪЕДИНИТЬ
		|
		|ВЫБРАТЬ
		|	ЦиклыОбменаПисьма.ДатаСоздания,
		|	ЦиклыОбменаПисьма.ДатаЗакрытия,
		|	ЦиклыОбменаПисьма.Предмет,
		|	ЦиклыОбменаПисьма.Новое,
		|	ЦиклыОбменаПисьма.Колонка1,
		|	ЦиклыОбменаПисьма.Колонка4,
		|	ЦиклыОбменаПисьма.Колонка5,
		|	ЦиклыОбменаПисьма.Колонка6,
		|	ЦиклыОбменаПисьма.ПротоколСОшибкойУведомлением,
		|	ЦиклыОбменаПисьма.ПротоколСУведомлениемОшибкой,
		|	ЦиклыОбменаПисьма.Ссылка
		|ИЗ
		|	ЦиклыОбменаПисьма КАК ЦиклыОбменаПисьма";

	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаИсходящиеФНС" Тогда
		Настройки.ОсновнаяТаблица = "Справочник.ЦиклыОбмена";
		Настройки.ДинамическоеСчитываниеДанных = Истина;
		
		#Область ЗапросЦиклыОбменаИсходящиеФНС
		Настройки.ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
		|ПОМЕСТИТЬ втНовыеСообщения
		...
		|		ПО (ТранспортныйКонтейнер.ТранспортныйКонтейнерСообщение = ТранспортноеСообщение.Ссылка)
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ОбращениеНП)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ПодтверждениеОбращениеНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.ИзвещениеОбращениеНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
		...
		|		ПО ТранспортноеСообщение.Ссылка = СодержимоеТранспортногоКонтейнера.ТранспортноеСообщениеСсылка
		|ГДЕ
		|	ТранспортноеСообщение.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыТранспортныхСообщений.РезультатПриемаОбращениеНО)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ЦиклОбмена
		|;
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаОтчетностьПФР" Тогда
		...
		|	И ЦиклыОбмена.ПометкаУдаления = ЛОЖЬ
		|	И ЦиклыОбмена.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыЦикловОбмена.ОтчетностьПФР)
		|	И ЦиклыОбмена.ВидОтчета <> ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ЗаявленияПоЭлДокументооборотуСПФР)
		|{ГДЕ
	...
	ИначеЕсли ИмяДинамическогоСписка = "ЦиклыОбменаОтчетностьФНС" Тогда
		...
		#Область ЗапросЦиклыОбменаОтчетностьФНС
		Настройки.ТекстЗапроса = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ТранспортноеСообщение.ЦиклОбмена КАК ЦиклОбмена
		|ПОМЕСТИТЬ втНовыеСообщения
		...
		|	ТранспортноеСообщение.Статус КАК Статус,
		|	ТранспортныйКонтейнер.ЧислоКонтейнеров КАК ЧислоКонтейнеров,
		|	ТранспортноеСообщение.ПротоколСОшибкой КАК ПротоколСОшибкой
		|ПОМЕСТИТЬ втЗапросКолонка1
		|ИЗ
		|	Документ.ТранспортноеСообщение КАК ТранспортноеСообщение
		|		ЛЕВОЕ СОЕДИНЕНИЕ втТранспортныйКонтейнер КАК ТранспортныйКонтейнер
		...
		|	ЦиклыОбмена.Период КАК Период,
		|	ЦиклыОбмена.ПредставлениеПериода КАК ПредставлениеПериода,
		|	ЦиклыОбмена.ВидДокумента КАК ВидДокумента,
		|	ЦиклыОбмена.ПредставлениеВидаДокумента КАК ПредставлениеВидаДокумента,
		|	ЦиклыОбмена.ДатаСоздания КАК ДатаСоздания,
		|	ЦиклыОбмена.ДатаЗакрытия КАК ДатаЗакрытия,
		|	ЦиклыОбмена.ВнешняяОрганизация КАК НалоговыйОрган,
		|	ЦиклыОбмена.Наименование КАК Наименование,
		...
		|	КОНЕЦ КАК Колонка8,
		|	ВЫБОР
		|		КОГДА ЗапросКолонка8.ПротоколСОшибкой
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ЗапросКолонка7.ПротоколСОшибкой
		|					ТОГДА ИСТИНА
		|				ИНАЧЕ ВЫБОР
		|						КОГДА ЗапросКолонка6.ПротоколСОшибкой
		|							ТОГДА ИСТИНА
		|						ИНАЧЕ ВЫБОР
		|								КОГДА ЗапросКолонка5.ПротоколСОшибкой
		|									ТОГДА ИСТИНА
		|								ИНАЧЕ ВЫБОР
		|										КОГДА ЗапросКолонка4.ПротоколСОшибкой
		|											ТОГДА ИСТИНА
		|										ИНАЧЕ ВЫБОР
		|												КОГДА ЗапросКолонка3.ПротоколСОшибкой
		|													ТОГДА ИСТИНА
		|												ИНАЧЕ ВЫБОР
		|														КОГДА ЗапросКолонка2.ПротоколСОшибкой
		|															ТОГДА ИСТИНА
		|														ИНАЧЕ ЗапросКолонка1.ПротоколСОшибкой
		|													КОНЕЦ
		|											КОНЕЦ
		|									КОНЕЦ
		|							КОНЕЦ
...
Процедура ОсновнаяКоманднаяПанельОбменяться(Команда)
	...
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ОсновнаяКоманднаяПанельОбменятьсяПослеПолученияКонтекста",
		ЭтотОбъект);
...
&НаКлиенте
Процедура ОсновнаяКоманднаяПанельОбменятьсяПослеПолученияКонтекста(Результат, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли НЕ КонтекстЭДОКлиент.ЕстьВозможностьОбмена(ВыбранныеОрганизации, Истина) Тогда
		Возврат;
	ИначеЕсли ВыбранныеОрганизации.Количество() = 1 Тогда
		
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Произвести обмен сообщениями по учетной записи ""%1""?'"), 
			ВыбранныеОрганизации[0]);
			
	Иначе
		ТекстВопроса = НСтр("ru = 'Произвести обмен сообщениями по выбранным учетным записям?'");
	КонецЕсли;
	
	ОписаниеОповещения = Новый ОписаниеОповещения("ВопросОбменятьсяЗавершение", ЭтотОбъект);
...
Процедура ВопросОбменятьсяЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	...
	ПараметрыДлительногоОбмена.ЭтоОбменИзЖурналаОбмена 		= Истина;
	
	Если НЕ ДлительнаяОтправкаКлиент.ПоказатьФормуДлительногоОбмена(ПараметрыДлительногоОбмена) Тогда // Обмен из журнала обмена.
		Возврат;
	КонецЕсли;
	
	ПодключитьОбработчикОжидания("Подключаемый_ОбменятьсяИзЖурналаОбмена", 0.1, Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура Подключаемый_ОбменятьсяИзЖурналаОбмена() Экспорт
...
Процедура ОсуществитьОбменПоВыбраннойУчетнойЗаписи(ВыбранныеУчетныеЗаписи, СписокПолученныхТС, НомерОбъекта)
	...
	Если НомерОбъекта + 1 > ВыбранныеУчетныеЗаписи.Количество() Тогда
		...
		ДлительнаяОтправкаКлиент.ОповеститьОЗавершенииОбмена(); // из журнала обмена
...
Процедура УправлениеЗакладкойУведомлений(Вид = Неопределено)
	...
	Элементы.ТаблицаУведомленияКомандаОткрытьАвансыНДФЛ.Видимость 							= Истина; // заявление о подтверждении права на зачет авансов по НДФЛ
	Элементы.ТаблицаУведомленияКомандаОткрытьСтрановыеОтчеты.Видимость 						= Истина; // страновые отчеты (регламентированный отчет)
	Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОбУчастииВМеждународнойГруппеКомпаний.Видимость = Истина; // уведомления об участии в международной группе компаний (регламентированный отчет)
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьАвансыНДФЛ.Видимость = Ложь; // документ Заявление о подтверждении права на зачет авансов по НДФЛ
	КонецЕсли;	
	
	Если НЕ СуществуетРегламентированныйОтчетСтрановойОтчет() Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьСтрановыеОтчеты.Видимость = Ложь; // страновые отчеты (регламентированный отчет)
	КонецЕсли;
	
	Если НЕ СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
		Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОбУчастииВМеждународнойГруппеКомпаний.Видимость = Ложь; // уведомления об участии в международной группе компаний (регламентированный отчет)
	КонецЕсли;
	...
	ИначеЕсли Вид = "РеестрыФНС" Тогда
		...
		Элементы.ТаблицаУведомленияОтчетныйГод.Видимость = Ложь;
	ИначеЕсли Вид = "РеестрыАкцизы" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетРеестрАкцизыПриложение*
		Элементы.ТаблицаУведомленияКомандаОткрытьРеестрыАкцизы.Пометка = Истина;// нажатость подменю
		Элементы.ТаблицаУведомленияОтчетныйГод.Видимость = Ложь;
	ИначеЕсли Вид = "АвансыНДФЛ" Тогда
		// документ ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ
		Элементы.ТаблицаУведомленияКомандаОткрытьАвансыНДФЛ.Пометка = Истина;// нажатость подменю
	ИначеЕсли Вид = "СтрановыеОтчеты" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетСтрановойОтчет
		Элементы.ТаблицаУведомленияКомандаОткрытьСтрановыеОтчеты.Пометка = Истина;// нажатость подменю
	ИначеЕсли Вид = "УведомленияОбУчастииВМеждународнойГруппеКомпаний" Тогда
		// документ РегламентированныйОтчет с источником РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний
		Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОбУчастииВМеждународнойГруппеКомпаний.Пометка = Истина;// нажатость подменю
	...
	Если НЕ ЗначениеЗаполнено(ИмяДокументаУведомлениеОКонтролируемыхСделках) И НЕ СуществуетРегламентированныйОтчетРозничныеЦеныНаТабак()
		И НЕ ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ)
		И НЕ СуществуетРегламентированныйОтчетСтрановойОтчет()
		И НЕ СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() Тогда
	...
	// запрос для электронных представлений с типом УведомлениеОКонтролируемыхСделках, ИсходящееУведомлениеФНС,
	// УведомлениеОРозничныхЦенахНаТабак, СтрановойОтчет и УведомлениеОбУчастииВМеждународнойГруппеКомпаний, будет в любом случае
	...
	Если Вид = "РеестрыФНС" Тогда
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления("РеестрНДС");
		Уведомления.ОсновнаяТаблица = "Документ.РегламентированныйОтчет";
	КонецЕсли;
	
	Если Вид = "РеестрыАкцизы" Тогда
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления("РеестрАкцизыПриложение");
		Уведомления.ОсновнаяТаблица = "Документ.РегламентированныйОтчет";
	КонецЕсли;
	
	// Если в системе существует документ ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ
	// тогда добавляем в запрос для динамического списка запрос по документу Заявление о подтверждении права на зачет авансов по НДФЛ.
	Если ЗначениеЗаполнено(ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) И Вид = "АвансыНДФЛ" Тогда
		
		ТекстЗапроса = ТекстЗапроса + "ВЫБРАТЬ
									  |	ВЫБОР
									  |		КОГДА ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.ПометкаУдаления
									  |			ТОГДА ВЫБОР
									  |					КОГДА СтатусОтправки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыОтправки.Отправлен)
									  |						ТОГДА 10
									  |					ИНАЧЕ ВЫБОР
									  ...
									  |			КОНЕЦ
									  |	КОНЕЦ КАК СтатусКартинка,
									  |	ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.Дата КАК ДатаСоздания,
									  |	ВЫРАЗИТЬ(ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.Организация КАК Справочник.Организации) КАК Организация,
									  |	ГОД(ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.НалоговыйПериодДата) КАК ОтчетныйГод,
									  |	ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.Ссылка,
									  |	ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.КодИФНС КАК НалоговыйОрган,
									  |	ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ) КАК ВидУведомления
									  |ИЗ
									  ...
									  |		ИЗ
									  |			РегистрСведений.СтатусыОтправки КАК СтатусыОтправки) КАК СтатусОтправки
									  |		ПО (СтатусОтправки.Объект = ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.Ссылка)
									  |ГДЕ
									  |	ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ.Организация В(&Организация)";
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ", ИмяДокументаЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ);
		
		Уведомления.ОсновнаяТаблица = "Документ.ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ";
		
	КонецЕсли;
	
	// Добавляем в запрос для динамического списка запрос по документу РегламентированныйОтчет с источником РегламентированныйОтчетСтрановойОтчет.
	Если СуществуетРегламентированныйОтчетСтрановойОтчет() И Вид = "СтрановыеОтчеты" Тогда
		
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления("РегламентированныйОтчетСтрановойОтчет");
		Уведомления.ОсновнаяТаблица = "Документ.РегламентированныйОтчет";
		
	КонецЕсли;
	
	// Добавляем в запрос для динамического списка запрос по документу РегламентированныйОтчет с источником РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний.
	Если СуществуетРегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний() И Вид = "УведомленияОбУчастииВМеждународнойГруппеКомпаний" Тогда
		
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления("РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний");
		Уведомления.ОсновнаяТаблица = "Документ.РегламентированныйОтчет";
	...
	Если Вид = "Файлы" Тогда
		...
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.РеестрНДСДокументыДляКомпенсацииНалога);
		ВидыУведомленийРеестровАкцизов = ДокументооборотСКОКлиентСервер.ВидыОтправляемыхДокументовРеестровАкцизов();
		Для каждого ВидУведомленияРеестраАкцизов Из ВидыУведомленийРеестровАкцизов Цикл
			ВидыУведомлений.Добавить(ВидУведомленияРеестраАкцизов);
		КонецЦикла;
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.ЗаявлениеОПодтвержденииПраваНаЗачетАвансовПоНДФЛ);
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.СтрановойОтчет);
		ВидыУведомлений.Добавить(Справочники.ВидыОтправляемыхДокументов.УчастиеВМеждународнойГруппеКомпаний);
...
Функция ТекстЗапросаДляОтчетовНаЗакладкеИсходящиеУведомления(ИсточникОтчета)
	 ...
	 ТекстЗапроса = "ВЫБРАТЬ
	                ...
	                |	ВЫБОР
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетСтрановойОтчет""
	                |			ИЛИ РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний""
	                |			ТОГДА ГОД(РегламентированныйОтчет.ДатаНачала)
	                |		ИНАЧЕ 0
	                |	КОНЕЦ КАК ОтчетныйГод,
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДСДокументыДляКомпенсацииНалога""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДСДокументыДляКомпенсацииНалога)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДСДекларацииИТаможенныеДекларации23""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДСДекларацииИТаможенныеДекларации23)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрНДСДекларацииНаЭкспрессГрузы""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрНДСДекларацииНаЭкспрессГрузы)
	                ...
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрАкцизыПриложение1""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрАкцизыПриложение1)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетРеестрАкцизыПриложение2""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.РеестрАкцизыПриложение2)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетСтрановойОтчет""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.СтрановойОтчет)
	                |		КОГДА РегламентированныйОтчет.ИсточникОтчета = ""РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний""
	                |			ТОГДА ЗНАЧЕНИЕ(Справочник.ВидыОтправляемыхДокументов.УчастиеВМеждународнойГруппеКомпаний)
...
Процедура УправлениеЗакладкойОтветыНаТребования(Вид = Неопределено)
	...
	Если Вид = "ТребованиеОПредставленииДокументов" Тогда
		...
		|	ОтветыНаТребование.Основание КАК Требование,
	...
	ИначеЕсли Вид = "ТребованиеПоясненийКДекларацииНДС" Тогда
		...
		Элементы.ИнформацияОРежимеОтображения.Заголовок = 
			"Выведены ответы на требования пояснений. Для отображения прочих видов ответов воспользуйтесь переключателем ""Режим отображения""";
...
Процедура СменитьЗакладкуУведомленийПриНеобходимости(ИмяСобытия, ВыводитьПредупреждение = Истина)
	...
	ИначеЕсли ИмяСобытия = "РеестрыФНС" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьРеестрыФНС.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "РеестрыАкцизы" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьРеестрыАкцизы.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "АвансыНДФЛ" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьАвансыНДФЛ.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "СтрановыеОтчеты" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьСтрановыеОтчеты.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
	ИначеЕсли ИмяСобытия = "УведомленияОбУчастииВМеждународнойГруппеКомпаний" И НЕ Элементы.ТаблицаУведомленияКомандаОткрытьУведомленияОбУчастииВМеждународнойГруппеКомпаний.Пометка Тогда
		НужноВыполнитьСменуЗакладки = Истина;
...
Процедура ТаблицаУведомленияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ИсхУФ = Неопределено;
	ИсхСсылка = Элементы.ТаблицаУведомления.ТекущиеДанные.Ссылка;
	Попытка
		Для Каждого ТекОкно Из ПолучитьОкна() Цикл 
			Для Каждого УФ Из ТекОкно.Содержимое Цикл 
				Если ТипЗнч(УФ) = Тип("ФормаКлиентскогоПриложения") Тогда 
					Попытка
						Если УФ.Объект.Ссылка = ИсхСсылка Тогда 
							ИсхУФ = УФ;
						КонецЕсли;
					Исключение
					КонецПопытки;
	...
	Если ИсхУФ = Неопределено Тогда 
		ПоказатьЗначение(, ИсхСсылка);
	Иначе
		ИсхУФ.Активизировать();
	КонецЕсли;
	
	Если ТипЗнч(ИсхСсылка) = Тип("ДокументСсылка.УведомлениеОСпецрежимахНалогообложения") Тогда 
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры
