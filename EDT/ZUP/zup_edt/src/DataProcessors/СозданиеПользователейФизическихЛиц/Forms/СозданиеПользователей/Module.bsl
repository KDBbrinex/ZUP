...
Процедура ЗаполнитьСписокСотрудников()
	...
	Если СотрудникиОрганизации.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Ни одного сотрудника пока не принято на работу.'");
	КонецЕсли;
...
Процедура СоздатьПользователейФизическихЛиц(ТолькоВыделенныеСтроки = Ложь)
	...
	Для Каждого СтрокаКоллекции Из СтрокиКоллекции Цикл 
		...
		Если ПользовательФизическогоЛица <> Неопределено Тогда 
			...
			Продолжить;
		КонецЕсли;
		НастройкиПользователей.ПользовательОС = ДанныеСотрудника.ПользовательОС;
...
Процедура ОтправитьСообщенияПользователям()
	...
	Для Каждого ИдентификаторСтроки Из Элементы.Сотрудники.ВыделенныеСтроки Цикл 
		...
		Если ЗначениеЗаполнено(ДанныеСотрудника.ИдентификаторПользователяИБ) Тогда 
			...
			СвойстваПользователяИБ = Пользователи.СвойстваПользователяИБ(ДанныеСотрудника.ИдентификаторПользователяИБ);
			ТекстПисьма = СамообслуживаниеСотрудников.ТекстПисьмаУведомления(СвойстваПользователяИБ.Имя, ОбновляемыеСвойства.Пароль, НастройкиСообщений);
