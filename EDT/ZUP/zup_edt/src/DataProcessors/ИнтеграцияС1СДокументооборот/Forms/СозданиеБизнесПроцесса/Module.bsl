...
Процедура ДеревоШаблоновПриАктивизацииСтроки(Элемент)
	...
	Сводка = HTMLПодсказка(Элементы.ДеревоШаблонов.ТекущиеДанные);
...
Процедура ЗаполнитьДеревоШаблонов(ПолучатьВидДокумента = Ложь)
	...
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(РезультатДеревоШаблонов, "formHeader") Тогда
		ЭтаФорма.Заголовок = РезультатДеревоШаблонов.formHeader;
	КонецЕсли;
	
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(РезультатДеревоШаблонов, "commandHeader") Тогда
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1") Тогда
		ОбработатьРезультат_2_1_16_2(Дерево, РезультатДеревоШаблонов);
	Иначе
...
&НаСервере
Процедура ОбработатьРезультат_2_1_16_2(Дерево, РезультатДеревоШаблонов)
	...
	Для Каждого Раздел из РезультатДеревоШаблонов.BusinessProcessTemplatesTree Цикл
		...
		Если Раздел.name = НСтр("ru = 'Рекомендованные шаблоны'") Тогда
			...
			Для Каждого Шаблон из Раздел.elements Цикл
				...
				СтрокаРаздела.Наименование = Шаблон.name;
				Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Шаблон, "hintHTML") Тогда
		...
		Иначе
			...
			РазделДерева = Дерево.Строки.Добавить();
			РазделДерева.Наименование = Раздел.name;
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Раздел, "hintHTML") Тогда
