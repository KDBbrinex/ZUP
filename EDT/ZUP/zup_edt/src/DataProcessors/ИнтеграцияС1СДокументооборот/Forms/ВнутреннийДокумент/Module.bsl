...
Процедура ОрганизацияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ВыбранноеЗначение.Свойство("VATpayer") Тогда
		УчитыватьНДС = ВыбранноеЗначение.VATpayer;
	Иначе
		УчитыватьНДС = Истина;
	КонецЕсли;
...
Процедура СоставСтрокаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ИнтеграцияС1СДокументооборотКлиент.ОткрытьСоставДокумента(ЭтаФорма);
...
Процедура НоменклатураДелНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ИнтеграцияС1СДокументооборотКлиент.ОткрытьНоменклатуруДел(ЭтаФорма);
...
Процедура ДелоНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ИнтеграцияС1СДокументооборотКлиент.ОткрытьДелоХраненияДокументов(ЭтаФорма);
...
Процедура ФайлыПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	ПараметрыОповещения = Новый Структура;
		...
		Оповещение = Новый ОписаниеОповещения("ФайлыПеретаскиваниеЗавершение", ЭтаФорма, ПараметрыОповещения);
...
Процедура ПодписатьФайл(Команда)
	...
	Оповещение = Новый ОписаниеОповещения("ПодписатьФайлЗавершение", ЭтаФорма, Неопределено);
	Если ПустаяСтрока(ID) Тогда 
		ЗаписатьИВозможноЗакрыть(Ложь, Оповещение);
	Иначе
...
Процедура ЗарегистрироватьНаСервере()
	...
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтаФорма, "ДатаРегистрации", Результат.document, "regDate");
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтаФорма, "РегистрационныйНомер", Результат.document, "regNumber");
	ИнтеграцияС1СДокументооборот.
		ЗаполнитьРеквизитФормыИзXDTO(ЭтаФорма, "Состояние", Результат.document, "status");
	...
	Обработки.ИнтеграцияС1СДокументооборот.УстановитьДоступностьРегистрации(ЭтаФорма, Результат.document);
...
Процедура ПрочитатьОбъектВФорму(ОбъектXDTO)
	...
	СоответствиеРеквизитов = Справочники.ПравилаИнтеграцииС1СДокументооборотом.
		СоответствиеСвойствXDTOиРеквизитовФормыОбъектаДО(Тип);
	ИнтеграцияС1СДокументооборот.ЗаполнитьФормуИзОбъектаXDTO(ЭтаФорма, ОбъектXDTO, СоответствиеРеквизитов);
	...
	ВестиУчетПоСтатьямДДС =
		ОбъектXDTO.documentType <> Неопределено
		И ОбъектXDTO.documentType.Свойства().Получить("cashFlowDetailsEnabled") <> Неопределено
		И ОбъектXDTO.documentType.cashFlowDetailsEnabled = Истина;
	Элементы.ГруппаСтатьиДДС.Видимость = ВестиУчетПоСтатьямДДС;
	
	ВестиУчетТоваровИУслуг =
		ОбъектXDTO.documentType <> Неопределено
		И ОбъектXDTO.documentType.Свойства().Получить("productRowsEnabled") <> Неопределено
		И ОбъектXDTO.documentType.productRowsEnabled = Истина;
	Элементы.СтраницаТовары.Видимость = ВестиУчетТоваровИУслуг;
	
	УчитыватьНДС = Истина
		Или ОбъектXDTO.organization = Неопределено
		Или ОбъектXDTO.organization.Свойства().Получить("VATpayer") = Неопределено
		Или ОбъектXDTO.organization.VATpayer = Ложь;
	Элементы.СуммаНДС.Видимость =
		УчитыватьНДС
		И ОбъектXDTO.documentType <> Неопределено
		И ОбъектXDTO.documentType.sumEnabled = Истина;
	...
	Если ОбъектXDTO.Свойства().Получить("titleBlockedByTemplate") <> Неопределено
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.13.1") Тогда
		Элементы.Адресат.Видимость =
			(ОбъектXDTO.documentType <> Неопределено И ОбъектXDTO.documentType.addresseeEnabled = Истина);
	Иначе
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1") Тогда
		...
		Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO, "useCaseFiles")
			...
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO, "documentType")
					И ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO.documentType,
						"accountingForCaseFilesEnabled")
					И ОбъектXDTO.documentType.accountingForCaseFilesEnabled Тогда
	...
	Обработки.ИнтеграцияС1СДокументооборот.УстановитьДоступностьРегистрации(ЭтаФорма, ОбъектXDTO);
...
Процедура ОбновитьСостояние()
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1.CORP") Тогда
		Запрос.columnSet.Добавить("statusSigning");
	КонецЕсли;
...
Процедура ЗаписатьОбъект(ПроверятьСвязи = Истина)
	...
	ИнтеграцияС1СДокументооборотПереопределяемый.ПередЗаписьюДокумента(Прокси, ОбъектXDTO, ЭтаФорма, Отказ);
	Если Отказ Тогда
		Возврат;
	КонецЕсли;
...
Процедура УстановитьДоступностьПоСостоянию(ОбъектXDTO)
	...
	Для Каждого СтрокаСоответствия Из СоответствиеРеквизитов Цикл
		...
		Если ДоступныеПоля.Количество() = 0 Или ДоступныеПоля.Найти(НРег(СтрокаСоответствия.Ключ)) <> Неопределено Тогда
	...
	СостояниеРазрешаетДобавлениеСканКопии = (ДоступныеПоля.Количество() = 0
...
Процедура ТоварыНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСпискаВТаблице(
		"DMProduct", "Номенклатура", Элемент.Родитель.ТекущиеДанные, ЭтаФорма);
...
Процедура ТоварыНоменклатураАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ДанныеДляАвтоПодбора("DMProduct", ДанныеВыбора, Текст, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ТоварыНоменклатураОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
				"Номенклатура", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ТоварыНоменклатураОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
		"Номенклатура", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
	СтандартнаяОбработка = Ложь;
...
Процедура ТоварыЕдиницаИзмеренияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзСпискаВТаблице(
		"DMMeasurementUnit", "ЕдиницаИзмерения", Элемент.Родитель.ТекущиеДанные, ЭтаФорма);
...
Процедура ТоварыЕдиницаИзмеренияАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ДанныеДляАвтоПодбора("DMMeasurementUnit", ДанныеВыбора, Текст, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ТоварыЕдиницаИзмеренияОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
				"ЕдиницаИзмерения", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ТоварыЕдиницаИзмеренияОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
		"ЕдиницаИзмерения", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
	СтандартнаяОбработка = Ложь;
...
Процедура ТоварыСтавкаНДСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ВыбратьЗначениеИзВыпадающегоСписка(
		"DMVATRate", "СтавкаНДС", ЭтаФорма,, Истина, Элемент);
...
Процедура ТоварыСтавкаНДСАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		ИнтеграцияС1СДокументооборотВызовСервера.ДанныеДляАвтоПодбора("DMVATRate", ДанныеВыбора, Текст, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ТоварыСтавкаНДСОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		Если ДанныеВыбора.Количество() = 1 Тогда 
			ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
				"СтавкаНДС", ДанныеВыбора[0].Значение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
			СтандартнаяОбработка = Истина;
		КонецЕсли;
	КонецЕсли;
...
Процедура ТоварыСтавкаНДСОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ИнтеграцияС1СДокументооборотКлиент.ОбработкаВыбораДанныхДляАвтоПодбора(
		"СтавкаНДС", ВыбранноеЗначение, СтандартнаяОбработка, ЭтаФорма, Истина, Элемент);
	СтандартнаяОбработка = Ложь;
