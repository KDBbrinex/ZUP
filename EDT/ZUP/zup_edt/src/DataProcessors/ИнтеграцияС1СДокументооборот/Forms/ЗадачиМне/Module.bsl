	...
	Иначе
		...
		Если СтрокаПредмета.Тип = "DMFile" Тогда
			...
			Элементы.Редактировать.Доступность = ДоступенЗахватФайлов И
				(Не СтрокаПредмета.Редактируется Или СтрокаПредмета.РедактируетсяТекущимПользователем);
...
&НаКлиенте
Процедура ДекорацияНетПредметовОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	Если СтрокаЗадачи.ПроцессТип = "DMBusinessProcessInvitation"
			И РезультатID = "ПринятоОбязательнымиУчастниками" Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ТочкаМаршрута = "РассмотрениеИнициатором"
			И ВидВопросаID = "Иное" Тогда
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
	
	// Требует ли тип задачи открытия сложной формы?
	Если ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Ознакомиться"
			И (РезультатID = "НеПринятоОбязательнымиУчастниками"
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	Если (ПроцессТип = "DMBusinessProcessApproval" И ТочкаМаршрута = "Согласовать")
			Или (ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Пригласить") Тогда
		ТребоватьРезультатВыполнения = Истина;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ТочкаМаршрута = "РассмотрениеИнициатором"
			И ВидВопросаID = "ПереносСрока" Тогда
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessInvitation" И ТочкаМаршрута = "Ознакомиться"
			И РезультатID = "ПринятоОбязательнымиУчастниками" Тогда
		ОткрытьФорму("Обработка.ИнтеграцияС1СДокументооборот.Форма.БизнесПроцессПриглашение",
			ПараметрыФормы,,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ВыполнитьЗадачуТретья(Команда)
	...
	// Требует ли тип задачи ввода результата выполнения?
	Если ПроцессТип = "DMBusinessProcessApproval" И ТочкаМаршрута = "Согласовать" Тогда
		ТребоватьРезультатВыполнения = Истина;
	КонецЕсли;
...
Процедура ДобавитьПредмет(Команда)
	...
	ПараметрыОповещения.Вставить("РольПредмета", "Вспомогательный");
...
Процедура ДобавитьПредметЗавершение(ОписаниеПредмета, ПараметрыОбработчика) Экспорт
	...
	ПараметрыЗадачи = Новый Структура("ID, Тип, ПроцессТип, Комментарий, ДоступнаМультипредметность",
		ПараметрыОбработчика.СтрокаЗадачи.ЗадачаID,
		ПараметрыОбработчика.СтрокаЗадачи.ЗадачаТип,
		ПараметрыОбработчика.СтрокаЗадачи.ПроцессТип,
		Комментарий,
		ДоступнаМультипредметность);
...
Процедура ЗаполнитьСтрокуЗадачиXDTO(СтрокаЗадачи, ЗадачаXDTO)
	...
	Иначе
		...
		Если ПроцессXDTO.Свойства().Получить("executionResult") <> Неопределено
				И ПроцессXDTO.Установлено("executionResult") Тогда
			Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(СтрокаЗадачи,
				ПроцессXDTO.executionResult, "Результат");
		КонецЕсли;
		
		СтрокаЗадачи.Состояние = ?(ПроцессXDTO.state = Неопределено, "Активен", ПроцессXDTO.state.name);
	КонецЕсли;
	...
	СтрокаЗадачи.Предметы.Колонки.Добавить("ДоступноУдаление");
	СтрокаЗадачи.Предметы.Колонки.Добавить("Редактируется");
	СтрокаЗадачи.Предметы.Колонки.Добавить("РедактируетсяТекущимПользователем");
	...
	Если СтрокаЗадачи.ПроцессТип = "DMBusinessProcessIssuesSolution" Тогда
		...
		Для Каждого ФайлXDTO Из ЗадачаXDTO.files Цикл
			...
			СтрокаПредмета.РольПредмета = "Вспомогательный";
...
Процедура ЗаполнитьДанныеБПУтверждение(СтрокаЗадачи, ЗадачаXDTO)
	...
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ЗадачаXDTO.parentBusinessProcess, "processType") Тогда
		СтрокаЗадачи.ЭтоПодписание = (ЗадачаXDTO.parentBusinessProcess.processType.objectID.id = "Подписание");
	Иначе
		СтрокаЗадачи.ЭтоПодписание = Ложь;
	КонецЕсли;
	
	Если СтрокаЗадачи.ЭтоПодписание Тогда
		Если СтрокаЗадачи.РезультатID = "Утверждено" Тогда
...
Процедура ЗаполнитьДанныеБПРешениеВопросов(СтрокаЗадачи, ЗадачаXDTO)
	...
	ИначеЕсли ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1") Тогда
		СтрокаЗадачи.ВидВопроса = "Иное";
		СтрокаЗадачи.ВидВопросаID = "Иное";
	...
	Для Каждого СтрокаПредмета Из РеквизитыЗадачи.Предметы Цикл
		...
		Иначе // объект иного типа, который может иметь приложенные файлы
			...
			Если ЗначениеЗаполнено(Предмет.Ссылка) Тогда
				...
				Попытка
					Предмет.Вставить("Представление", Строка(Предмет.Ссылка) + НСтр("ru = ', объект ИС'"));
				Исключение
...
Процедура ОбновитьПредставлениеЗадачиНаСервере(РеквизитыЗадачи, ДетальныеСведенияЗадачи)
	...
	Элементы.Комментарий.ПодсказкаВвода = НСтр("ru = 'Ваш комментарий'");
	...
	Предметы.Очистить();
	Для Каждого Предмет Из ДетальныеСведенияЗадачи.Предметы Цикл
		Если Предмет.РольПредмета = "Основной" И ОсновнойПредметID = "" Тогда
...
Процедура ОбновитьПредставлениеЗадачиБППриглашение(РеквизитыЗадачи)
	...
	ИначеЕсли РеквизитыЗадачи.ТочкаМаршрутаКратко = "Ознакомиться" Тогда
		Элементы.Комментарий.Видимость = Ложь;
		Элементы.РезультатВыполнения.Видимость = Истина;
		Если РеквизитыЗадачи.РезультатID = "НеПринятоОбязательнымиУчастниками"
...
Процедура ЗаполнитьСвойстваОбъектаПоТипуЗадачи(Прокси, Задача, Комментарий, НомерКнопки, РезультатID)
	...
	ИначеЕсли ИмяТипа = "DMBusinessProcessInvitationTaskAcquaint" Тогда
		...
		Если РезультатID = "НеПринятоОбязательнымиУчастниками"
				Или РезультатID = "НеПринятоВсемиУчастниками" Тогда
