...
Процедура ДеревоСтатейПриАктивизацииСтроки(Элемент)
	...
	Если ДоступенВыбор Тогда
		...
		ДоступенВыбор = ДоступенВыбор И 
			(ВыбранныеСтатьи.НайтиСтроки(СтруктураПоиска).Количество() = 0);
	КонецЕсли;
...
Процедура ЗаполнитьЭлементДереваСтатей(НоваяСтрока, Элемент)
	...
	Иначе	
		НоваяСтрока.НомерКартинки = ИнтеграцияС1СДокументооборотПереопределяемый.ИндексКартинкиЭлемента();
	КонецЕсли;
	
	Если Элемент.isFolder  Тогда
		НоваяСтрока.ПодпапкиСчитаны = Ложь;
		ФиктивныйЛист = НоваяСтрока.ПолучитьЭлементы().Добавить();
	Иначе
