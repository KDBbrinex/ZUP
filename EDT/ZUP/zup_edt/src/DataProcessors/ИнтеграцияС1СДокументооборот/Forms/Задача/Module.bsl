...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РезультатВыполнения.ПодсказкаВвода = НСтр("ru = 'Ваш комментарий'");
	...
	ИнтеграцияС1СДокументооборотПереопределяемый.ДополнительнаяОбработкаФормыЗадачи(
		ЭтаФорма,
		Отказ,
...
Процедура HTMLПредставлениеПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Найти(ДанныеСобытия.Href, "ИзменитьСрокВыполнения") Тогда
...
Процедура ДеревоПриложенийВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Иначе // объект ДО
		ИнтеграцияС1СДокументооборотКлиент.ОткрытьОбъект(Строка.ПредметТип, Строка.ПредметID, ЭтаФорма);
...
Процедура ВыполнитьЗадачуПервая(Команда)
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessConsideration" и ТочкаМаршрута = "Рассмотреть"
			И Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
		ОчиститьСообщения();
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Поле ""Резолюция"" не заполнено'"),,
			"РезультатВыполнения");
		Возврат;
	КонецЕсли;
...
Процедура ВыполнитьЗадачуВторая(Команда)
	...
	Если (ПроцессТип = "DMBusinessProcessApproval" И ЭтоТочкаМаршрута("Согласовать"))
			Или (ПроцессТип = "DMBusinessProcessInvitation" И ЭтоТочкаМаршрута("Пригласить")) Тогда
	...
	ИначеЕсли ПроцессТип = "DMBusinessProcessIssuesSolution" И ЭтоТочкаМаршрута("РассмотрениеИнициатором") Тогда
		Если ВидВопросаID = "ПереносСрока" И Не ЗначениеЗаполнено(РезультатВыполнения) Тогда
			ОчиститьСообщения();
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				ОбщегоНазначенияКлиентСервер.ТекстОшибкиЗаполнения(,, НСтр("ru = 'Комментарий'")),,
				"РезультатВыполнения");
...
Процедура СогласоватьПереносСрока(Команда)
	...
	ПараметрыФормы.Вставить("id", ID);
	ПараметрыФормы.Вставить("type", Тип);
	ПараметрыФормы.Вставить("ВидВопроса", "Перенос срока");
	ПараметрыФормы.Вставить("ВидВопросаID", "ПереносСрока");
...
Процедура ИзменитьДатуВыполнения(Команда)
	...
	ПоказатьВводДаты(
		Новый ОписаниеОповещения("ПослеВводаДатаИсполнения", ЭтаФорма),
		ДатаИсполнения,
...
Процедура ЗаполнитьФормуЗадачи(Параметры)
	...
	Для Каждого Элемент Из ДеревоПриложений.ПолучитьЭлементы() Цикл
		Если Элемент.РольПредмета = "Основной" Тогда
	...
	Если СостояниеID = "Остановлен" Тогда
		...
		КартинкаСостояние = 1;
	ИначеЕсли СостояниеID = "Прерван" Тогда
		Элементы.ГруппаСведенияОЗадаче.Видимость = Истина;
	...
	// Заполним описание.
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ОбъектXDTO, "htmlView") Тогда
	...
	Иначе
		...
		Элементы.ФормаСогласоватьПереносСрока.Видимость = Не Выполнена
			...
			И ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1.CORP");
	КонецЕсли;
...
Процедура ДополнитьСтрокуПредметаСвязаннымОбъектомИС(СтрокаПредмета, ПредметXDTO)
	...
	Если ПредметXDTO.Установлено("externalObject") Тогда
		СсылкиПредметов = ИнтеграцияС1СДокументооборот.СсылкиПоВнешнимОбъектам(ПредметXDTO);
	...
	Попытка
		Если ЗначениеЗаполнено(СтрокаПредмета.Ссылка) Тогда // представление как в ИС
			СтрокаПредмета.Представление = Строка(СтрокаПредмета.Ссылка) + НСтр("ru = ', объект ИС'");
...
Процедура ЗаполнитьДанныеБПУтверждение(ЗадачаОбъект)
	...
	Если ИнтеграцияС1СДокументооборот.СвойствоСуществует(ЗадачаОбъект.parentBusinessProcess, "processType") Тогда
		ЭтоПодписание = (ЗадачаОбъект.parentBusinessProcess.processType.objectID.id = "Подписание");
	Иначе
		ЭтоПодписание = Ложь;
	КонецЕсли;
	...
	Если ЭтоТочкаМаршрута("Утвердить") Тогда
		...
		Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтаФорма,
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		...
		Если ЭтоПодписание Тогда
			...
			Элементы.Утверждающий.Заголовок = "Подписант";
			Элементы.ДатаУтверждения.Заголовок = "Дата подписания";
		Иначе
...
Процедура ЗаполнитьДанныеБПКомплексныйПроцесс(Прокси, ПроцессОбъект)
	...
	Если ЭтоТочкаМаршрута("Контролировать") Тогда
		...
		Для Каждого Этап Из ПроцессОбъект.stages Цикл
			...
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(Этап, "importance") Тогда
				Если Этап.importance.objectId.id = "Низкая" Тогда
					НоваяСтрока.Важность = 0;
				ИначеЕсли Этап.importance.objectId.id = "Обычная" Тогда
					НоваяСтрока.Важность = 1;
				ИначеЕсли Этап.importance.objectId.id = "Высокая" Тогда
...
Процедура ЗаполнитьДанныеБПРешениеВопросов(ЗадачаОбъект)
	...
	Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтаФорма,
		...
		"ПредметРассмотрения",
		Истина);
	Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ЗадачаОбъект, "issueType") Тогда
		Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(ЭтаФорма,
	...
	ИначеЕсли ЭтоТочкаМаршрута("РассмотрениеИнициатором") И ВидВопросаID = "ПереносСрока" Тогда
...
Процедура ЗаполнитьДанныеБППриглашение(ЗадачаОбъект)
	...
	ИначеЕсли ЭтоТочкаМаршрута("Ознакомиться") Тогда
		...
		Для каждого СтрокаРезультата из ЗадачаОбъект.invitationResults Цикл
			...
			Обработки.ИнтеграцияС1СДокументооборот.ЗаполнитьОбъектныйРеквизит(СтрИсполнения,
...
Процедура ЗаполнитьЗадачуПредметамиИзДереваПриложений(Прокси, СтрокиДерева, Задача)
	...
	Если ИнтеграцияС1СДокументооборот.ПроверитьТип(Прокси, Задача, "DMBusinessProcessIssuesSolutionTaskQuestion")
			Или ИнтеграцияС1СДокументооборот.ПроверитьТип(Прокси, Задача, "DMBusinessProcessIssuesSolutionTaskAnswer")Тогда
...
Функция ПолучитьПодписьДлительностиПереносаСрока(ПарамтерыЗапроса)
	...
	Запрос.user.name = ПарамтерыЗапроса.Автор;
	Запрос.user.objectId = ИнтеграцияС1СДокументооборот.СоздатьObjectID(Прокси,
		ПарамтерыЗапроса.АвторID, ПарамтерыЗапроса.АвторТип);
	Запрос.oldDueDate = ПарамтерыЗапроса.СтарыйСрок;
	Запрос.newDueDate = ПарамтерыЗапроса.НовыйСрок;
