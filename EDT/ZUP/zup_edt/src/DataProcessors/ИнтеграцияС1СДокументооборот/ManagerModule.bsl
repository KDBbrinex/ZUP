...
//	ЗаполнятьПредставление - Булево - признак необходимости заполнения реквизита представления по свойству objectId.presentation
//
...
Процедура ПриИзмененииВидаНаФормеДокумента(Форма) Экспорт
	...
	Если Не ПроверитьВозможностьСозданияДокументаНеПоШаблону(Форма, documentType) Тогда
		Возврат;
	КонецЕсли;
	НастроитьФормуДокументаСогласноВидуДокументаXDTO(Форма, documentType);
	Прокси = ИнтеграцияС1СДокументооборотПовтИсп.ПолучитьПрокси();
...
//
Процедура НастроитьФормуДокументаСогласноВидуДокументаXDTO(Форма, documentType) Экспорт
	...
	ИначеЕсли ИмяФормы = "Обработка.ИнтеграцияС1СДокументооборот.Форма.ВнутреннийДокумент" Тогда
		...
		Элементы.Организация.Видимость		= (documentType <> Неопределено И documentType.organizationEnabled = Истина);
		Элементы.СрокИсполнения.Видимость	= (documentType <> Неопределено И documentType.performanceDateEnabled = Истина);
		Элементы.ГруппаКонтрагент.ОтображатьЗаголовок =
											(documentType <> Неопределено И documentType.correspondentEnabled = Истина);
		...
		Элементы.Адресат.Видимость = documentType <> Неопределено
			И documentType.Свойства().Получить("addresseeEnabled") <> Неопределено
			И documentType.addresseeEnabled = Истина;
		
		Форма.ВестиУчетПоСтатьямДДС =
			documentType <> Неопределено
			И documentType.Свойства().Получить("cashFlowDetailsEnabled") <> Неопределено
			И documentType.cashFlowDetailsEnabled = Истина;
		Элементы.ГруппаСтатьиДДС.Видимость = Форма.ВестиУчетПоСтатьямДДС;
		
		Форма.ВестиУчетТоваровИУслуг =
			documentType <> Неопределено
			И documentType.Свойства().Получить("productRowsEnabled") <> Неопределено
			И documentType.productRowsEnabled = Истина;
	...
	Если ИнтеграцияС1СДокументооборотПовтИсп.ДоступенФункционалВерсииСервиса("2.1.18.1.CORP") Тогда
		...
		Если Форма.Элементы.Дело.Видимость Тогда
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(documentType, "accountingForCaseFilesEnabled")
...
Процедура ЗаполнитьСтандартнуюШапкуБизнесПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если Форма.Элементы.Найти("ГруппаСостояние") <> Неопределено Тогда
		...
		ИначеЕсли Форма.СостояниеID = "Прерван" Тогда
			Форма.Элементы.ГруппаСостояние.Видимость = Истина;
	...
	Если Форма.Элементы.Найти("ВидПроцессаПредставление") <> Неопределено Тогда
		...
		Иначе
			...
			Форма.ВидПроцессаID = "Утверждение";
...
Процедура УстановитьВидимостьКомандИзмененияСостоянияПроцесса(Форма, ОбъектXDTO) Экспорт
	...
	Если ОбъектXDTO.Установлено("leadingTask") Тогда
	...
	ИначеЕсли Не ОбъектXDTO.Установлено("state") Тогда
		Форма.Элементы.ФормаОстановитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПродолжитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПрерватьПроцесс.Видимость = Ложь;
	ИначеЕсли ОбъектXDTO.Установлено("completed") И ОбъектXDTO.completed Тогда
		Форма.Элементы.ФормаОстановитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПродолжитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПрерватьПроцесс.Видимость = Ложь;
	ИначеЕсли Не ОбъектXDTO.Установлено("started") Или Не ОбъектXDTO.started Тогда
		Форма.Элементы.ФормаОстановитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПродолжитьПроцесс.Видимость = Ложь;
		Форма.Элементы.ФормаПрерватьПроцесс.Видимость = Ложь;
...
//   Форма - УправляемаяФорма - содержит реквизиты - источники заполнения
