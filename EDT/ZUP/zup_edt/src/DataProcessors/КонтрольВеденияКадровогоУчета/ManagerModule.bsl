...
Функция РезультатПроверкиКорректностиВидовЗанятостиСотрудников()
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(КадроваяИсторияСотрудников.ВидСобытия, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ВидыКадровыхСобытий.Увольнение)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК Уволен
		...
		Для Каждого ВозрастЗамены Из ВозрастыЗамены Цикл
			...
			Если ЗначениеЗаполнено(Выборка.ДатаУвольнения) Тогда
				Если Выборка.ДатаУвольнения >= ДатаВозрастаЗамены 
					...
					И ДатаВозрастаЗамены < ТекущаяДатаСеанса() Тогда
					ПроблемныеСотрудники.Вставить(Выборка.Сотрудник, ВозрастЗамены);
				КонецЕсли;
			Иначе
				Если Выборка.ДатаВыдачиПаспорта < ДатаВозрастаЗамены
					И ДобавитьМесяц(ДатаВозрастаЗамены, МесяцевНаЗамену) < ТекущаяДатаСеанса() Тогда
					ПроблемныеСотрудники.Вставить(Выборка.Сотрудник, ВозрастЗамены);
				КонецЕсли;
			КонецЕсли;
			Прервать;
