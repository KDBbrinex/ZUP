...
Процедура ПоказатьБланк(Команда)
	...
	Если СтрНайти(НаименованиеОтчета, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
		...
		Если ВыбраннаяФорма <> "ФормаОтчета2016Кв2" Тогда
			...
			Если ОбщийМодульАЛКОКлиент = Неопределено Тогда
			    Возврат;	
			КонецЕсли;
			
			ДлитОперация = ОбщийМодульАЛКОКлиент.ПолучитьПеречислениеВидыДлительныхОперацийРеглОтчетов("Печать");
	...
	// АлкоОтчеты с реализацией печати с использованием длительных опреаций,
	// 11-я и 12-я алкодекларация.
	Если СтрНайти(НаименованиеОтчета, "Прил. 11: Розничная продажа алкогольной") > 0
		или 
		СтрНайти(НаименованиеОтчета, "Прил. 12: Розничная продажа пива и пивных напитков") > 0					
		Тогда
	...
	// АлкоОтчеты с реализацией печати с использованием длительных опреаций,
	// Заявление о представлении корректирующих деклараций в ФСРАР.
	Если СтрНайти(НаименованиеОтчета, "Заявление о представлении корректирующих деклараций в ФСРАР.") > 0 Тогда
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Доходы, выплаченные иностранным организациям" Тогда
		...
		Если (ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2020Кв1"
		  ИЛИ ОписаниеДохИно.ВыбраннаяФорма = "ФормаОтчета2016Кв1")
			...
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Уведомление об участии в международной группе компаний"
		ИЛИ Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Страновой отчет" Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
...
Процедура Выгрузить(Команда)
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если СтрНайти(НаименованиеОтчета, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
			...
			ДлитОперация = ОбщийМодульАЛКОКлиент.ПолучитьПеречислениеВидыДлительныхОперацийРеглОтчетов("Выгрузка");
		...
		// АлкоОтчеты с реализацией выгрузки с использованием длительных опреаций,
		// 11-я и 12-я алкодекларация.	
		Если СтрНайти(НаименованиеОтчета, "Прил. 11: Розничная продажа алкогольной") > 0
			или 
			СтрНайти(НаименованиеОтчета, "Прил. 12: Розничная продажа пива и пивных напитков") > 0
		...
		Если СтрНайти(НаименованиеОтчета, "Заявление о представлении корректирующих деклараций в ФСРАР.") > 0 Тогда
			...
			ТекстПредупреждения = НСтр("ru = 'Проверка выгрузки, выгрузка и отправка редакции формы, 
												|утвержденной Приказом Минфина России от 20 июня 2017г. № 97н., 
												|не предусмотрена.'");
			ПоказатьПредупреждение(, ТекстПредупреждения);
			Возврат;
		...
		Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
			= "Движение средств по счету в банке за пределами РФ" Тогда
			ОписаниеРегОтчета = ОписаниеРегОтчета(Ссылка);
			Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв2" Тогда
				ПоказатьПредупреждение(, НСтр(
				"ru='Выгрузка в электронном виде формы отчета
				|в редакции постановления Правительства РФ
				|от 17.04.2020 № 528 не предусмотрена
				|законодательством. Форма сдается в бумажном виде'"));
				Возврат;
			ИначеЕсли ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2015Кв3" Тогда
				ПоказатьПредупреждение(, НСтр(
				"ru='Выгрузка в электронном виде формы отчета
				|в редакции постановления Правительства РФ
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
		= "Сведения о доле доходов от туристско-рекреационной деятельности" Тогда
		Если ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока).ВыбраннаяФорма = "ФормаОтчета2018Кв1" Тогда
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
			"ru='Выгрузка, проверка выгрузки, печать с двухмерным штрихкодом PDF417 формы
			|отчета ""%1"",
...
Процедура ПроверитьВыгрузку(Команда)
	...
	Если СтрНайти(НаименованиеОтчета, "Прил. 13: Объем собранного винограда для производства винодельческой продукции") > 0
		...
		ДлитОперация = ОбщийМодульАЛКОКлиент.ПолучитьПеречислениеВидыДлительныхОперацийРеглОтчетов("ПроверкаВыгрузки");
		ДлительныеОперацииАлко(ДокументСсылкаАлкоОтчет, ДлитОперация);
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета
		= "Движение средств по счету в банке за пределами РФ"
		И ТипЗнч(Элементы.ЖурналОтчетов.ТекущаяСтрока) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		ОписаниеРегОтчета = ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока);
		Если ОписаниеРегОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв2" Тогда
			ПоказатьПредупреждение(, НСтр(
...
Процедура ОбработатьАктивизациюСтрокиОсновнойТаблицы()
	...
	ИсточникиОтчетовФСС.Добавить("РегламентированныйОтчетПодтверждениеВидаДеятельности");
	...
	ИсточникиОтчетовФСРАР.Добавить("РегламентированныйОтчетАлкоПриложение16");
	ИсточникиОтчетовФСРАР.Добавить("РегламентированныйОтчетАлкоПриложение17");
	ИсточникиОтчетовФСРАР.Добавить("РегламентированныйОтчетАлкоПриложение18");
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДСДокументыДляКомпенсацииНалога");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДСДекларацииИТаможенныеДекларации23");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрНДСДекларацииНаЭкспрессГрузы");
	...
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрАкцизыПриложение1");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетРеестрАкцизыПриложение2");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетСтрановойОтчет");
	ИсточникиОтчетовПредставление.Добавить("РегламентированныйОтчетУчастиеВМеждународнойГруппеКомпаний");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1Фермер");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ИПАвтогруз");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2ДМ");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2ДМприл");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ИПуслуги");
	...
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФормаМПмикронатура");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаПрочиеФормы");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма4ОС");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1Конъюнктура");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1КонъюнктураОпт");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФормаЗПФизическаяКультураИСпорт");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ЛХ");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1РЦ");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2РЦ");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2ТПОхота");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФормаПМ1Автогруз");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ПР");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма6Нефть");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма14ФОМС");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма2Наука");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФормаИАП");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ТГС");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1ТМС");
	ИсточникиОтчетовФСГС.Добавить("РегламентированныйОтчетСтатистикаФорма1КХ");
	...
	ИсточникиОтчетовРПН.Добавить("РегламентированныйОтчетСтатистикаФорма2ТПВоздух");
	ИсточникиОтчетовРПН.Добавить("РегламентированныйОтчетРасчетЭкологическийСбор");
	ИсточникиОтчетовРПН.Добавить("РегламентированныйОтчетДекларацияПлатаНВОС");
	ИсточникиОтчетовРПН.Добавить("РегламентированныйОтчетЭкологическийКонтроль");
	ИсточникиОтчетовРПН.Добавить("РегламентированныйОтчетФорма2ТПРекультивация");
...
Процедура ПоказатьСДвухмернымШтрихкодомPDF417(Команда)
	...
	Если Элементы.ЖурналОтчетов.ТекущиеДанные.НаименованиеОтчета = "Декларация 3-НДФЛ" Тогда
		Описание3НДФЛ = ОписаниеРегОтчета(Элементы.ЖурналОтчетов.ТекущаяСтрока);
...
Процедура ПоказатьРезультатПроверкиОбновленияИнформацииМОС(
	...
	Попытка
		Если НЕ ЗаданиеВыполнено(ИдентификаторЗаданияМОС) Тогда // задание не выполнено
			Возврат;
		КонецЕсли;
	Исключение
...
Процедура ДлительныеОперацииАлко(ДокументСсылкаАлкоОтчет, ВидДлОперации, ВидПечати = Неопределено)
	...
	Если ОбщегоНазначенияКлиент.ОбщийМодуль("РегламентированнаяОтчетностьАЛКОКлиент") = Неопределено Тогда	
		Возврат;	
	КонецЕсли;
