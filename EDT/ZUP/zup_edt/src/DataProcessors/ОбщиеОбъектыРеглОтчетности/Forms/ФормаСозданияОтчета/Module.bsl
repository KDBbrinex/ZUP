...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// Ограничиваем исходное дерево отчетов для ИП на УСН 6%, или для сервиса ЭТК.
	УпроститьФормуРегОтчетностиФильтрОтчетов(ДеревоВсехИспользуемыхОтчетовПоКатегориям, КоличествоЭлементовВДеревеОтчетов);
	
	#Область РасширенныйФункционал
	// Ограничиваем исходное дерево отчетов при отключенной функциональной опции "Расширенный функционал".
	РегламентированнаяОтчетностьПереопределяемый.УдалитьОтчетыВходяшиеВРасширенныйФункционал(ДеревоВсехИспользуемыхОтчетовПоКатегориям, КоличествоЭлементовВДеревеОтчетов);
	#КонецОбласти
	...
	// Скрываем некоторые элементы формы для ИП на УСН 6%, или для сервиса ЭТК.
...
Функция ДеревоВсехИспользуемыхОтчетов()
	...
	ДополнитьДеревоЭлектроннымиПредставлениями(ДеревоВсехИспользуемыхОтчетов, ПараметрыОтбораУОС);
...
Функция ДеревоИзСправочникаРегламентированныеОтчеты()
	...
	|			И (ВидыОтправляемыхДокументов.Источник ПОДОБНО ""%[^ ]%"")
	|ГДЕ
...
&НаСервере
Процедура УпроститьФормуСозданияОтчета() Экспорт
	...
	Если ФлагиУчета.ИнтеграцияСБанком ИЛИ ФлагиУчета.СервисЭлектронныхТрудовыхКнижек Тогда
		
		//Доступна и активна упрощенная отчетность или сервис ЭТК.
		Элементы.ГруппаВидыСписков.ОтображениеСтраниц = ОтображениеСтраницФормы.Нет;
		
		Элементы.ГруппаДеревоОтчетовГруппировка.Видимость = Ложь;
		Элементы.ГруппаРанееСозданные.Видимость = Ложь;
		
		Элементы.ДеревоОтчетов.Шапка = Ложь;
		
		ДеревоОтчетовГруппировка = 3; // без группировки, упрощенный интерфейс
...
Процедура УпроститьФормуРегОтчетностиФильтрОтчетов(ДеревоОтчетов, КоличествоЭлементовВДеревеОтчетов) Экспорт
	...
	Если ФлагиУчета.ИнтеграцияСБанком ИЛИ ФлагиУчета.СервисЭлектронныхТрудовыхКнижек Тогда
		...
		Если ФлагиУчета.ИнтеграцияСБанком Тогда
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.ПолучитьНазванияРегламентированныхОтчетовИПнаУСН6(
				ОтчетыДляВыводаВУпрощеннойФорме);
		ИначеЕсли ФлагиУчета.СервисЭлектронныхТрудовыхКнижек Тогда
			ЭлектронныйДокументооборотСКонтролирующимиОрганамиПереопределяемый.ПолучитьНазванияРегламентированныхОтчетовСервисаЭТК(
				ОтчетыДляВыводаВУпрощеннойФорме);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция СоответствиеСвойствВидовОтправляемыхДокументов(ИдТипаДокумента = Неопределено)
	
	ОтборПоТипуДокумента = (ИдТипаДокумента <> Неопределено);
	
	РезультатСоответствие = Новый Соответствие;
	
	// Получаеим макет со списком документов.
	...
	Для Инд = 0 По МакетСписокДокументов.Области.Количество() - 1 Цикл
		...
		Если НЕ ПустаяСтрока(ИмяГруппы) Тогда
			...
			Для Ном = ТекОбласть.Верх По ТекОбласть.Низ Цикл
				...
				Источник     = СокрЛП(МакетСписокДокументов.Область(Ном, 4).Текст);
				ТипДокумента = СокрЛП(МакетСписокДокументов.Область(Ном, 6).Текст);
				
				Если ОтборПоТипуДокумента Тогда
					Если ИдТипаДокумента <> ТипДокумента Тогда
						Продолжить;
					КонецЕсли;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(Источник) Тогда
					ИмяЭлемента = Источник;
				КонецЕсли;
				
				Наименование = СокрЛП(МакетСписокДокументов.Область(Ном, 2).Текст);
				
				РезультатСоответствие.Вставить(ИмяЭлемента, Наименование);
			КонецЦикла;
...
Функция ПолучитьТаблицуКодовФСГСНереализованныхОтчетов()
	...
	СвойстваВидовОтправляемыхДокументов = СоответствиеСвойствВидовОтправляемыхДокументов("ОтчетФСГС");
	
	Результат = Новый ТаблицаЗначений;
	Результат.Колонки.Добавить("ОКУД", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(7)));
	Результат.Колонки.Добавить("Наименование", Новый ОписаниеТипов("Строка", , Новый КвалификаторыСтроки(254)));
	Результат.Колонки.Добавить("ЕстьВПрограмме", Новый ОписаниеТипов("Булево"));
	...
	Для Стр = 2 По Макет.ВысотаТаблицы Цикл 
		...
		Если ЗначениеЗаполнено(ОКУД) Тогда
			ИДОтчета = Макет.Область(Стр, 3, Стр, 3).Текст;
			НовСтр = Результат.Добавить();
			НовСтр.ОКУД = ОКУД;
			НовСтр.Наименование = СвойстваВидовОтправляемыхДокументов[ИДОтчета];
			Если Метаданные.Отчеты.Найти(ИДОтчета) = Неопределено Тогда 
				НовСтр.ЕстьВПрограмме = Ложь;
			Иначе
				НовСтр.ЕстьВПрограмме = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Функция ПолучитьТаблицуНереализованныхЭлектронныхПредставлений()
	...
	ОтображатьВДеревеРеализованныеУсловие = "ИСТИНА";
	Если НЕ ОтображатьВДеревеРеализованные Тогда
		// Значения, удовлетворяющие условию: "ЕстьВПрограмме ЕСТЬ НЕ NULL" - не будут выводиться в дереве отчетов.
		ОтображатьВДеревеРеализованныеУсловие = "НЕ ЕСТЬNULL(ВТ_КодыФСГС.ЕстьВПрограмме, ЛОЖЬ)";
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_КодыФСГС.Наименование КАК Наименование,
	|	ВТ_КодыФСГС.ОКУД КАК ОКУДФормы,
	|	ШаблоныЭВФОтчетовСтатистики.Наименование КАК НаименованиеШаблона,
	|	ШаблоныЭВФОтчетовСтатистики.ОКУД КАК ОКУДШаблона,
	|	ШаблоныЭВФОтчетовСтатистики.ИмяФайлаШаблона,
	|	" + ОтображатьВДеревеРеализованныеУсловие + " КАК ОтображатьВДереве
	|ИЗ
	|	ВТ_КодыФСГС КАК ВТ_КодыФСГС
	|		ПОЛНОЕ СОЕДИНЕНИЕ РегистрСведений.ШаблоныЭВФОтчетовСтатистики КАК ШаблоныЭВФОтчетовСтатистики
	...
	|		ПО (ИСТИНА)
	|ГДЕ
	|	ЕСТЬNULL(ВТ_ЕстьУниверсальныйОтчет.Количество, 0) <> 0"; //вывод содержит реализованные и нереализованные отчеты
	
	Возврат Запрос.Выполнить().Выгрузить();
КонецФункции

&НаСервереБезКонтекста
Процедура ДополнитьДеревоЭлектроннымиПредставлениями(ДеревоВсехИспользуемыхОтчетов, ПараметрыОтбораУОС)
	...
	СписокЗагрузкиШаблонов = Новый СписокЗначений;
	...
	Для Каждого СтрокаПервогоУровня Из ДеревоВсехИспользуемыхОтчетов.Строки Цикл
		Если СтрокаПервогоУровня.Строки.Найти(Перечисления.ТипыКонтролирующихОрганов.ФСГС, "ТипПолучателя", Ложь) <> Неопределено Тогда 
			...
			ОтобранныеОтчеты = СтрокаПервогоУровня.Строки.НайтиСтроки(ПараметрыОтбораУОС);
			Если ЗначениеЗаполнено(ОтобранныеОтчеты) Тогда // перемещаем отчет УОС в группу "Прочие формы"
				ОтчетУОС = ОтобранныеОтчеты[0];
				ГруппаОтчетаУОС = ОтчетУОС.Родитель;
				НовыйОтчетУОС = ГруппаЭлектронныеПредставления.Строки.Добавить();
				ЗаполнитьЗначенияСвойств(НовыйОтчетУОС, ОтчетУОС,, "Родитель");
				ГруппаОтчетаУОС.Строки.Удалить(ОтчетУОС);
			КонецЕсли;
			
			ПрефиксОтчета = "Статистика: ";
			
			Для Каждого Стр Из ТаблицаПредставлений Цикл
				
				Наименование = СокрЛП(?(ПустаяСтрока(Стр.Наименование), Стр.НаименованиеШаблона, Стр.Наименование));
				Наименование = ?(Лев(Наименование, СтрДлина(ПрефиксОтчета)) = ПрефиксОтчета, "", ПрефиксОтчета) + Наименование;
				
				Если ЗначениеЗаполнено(Стр.ОКУДФормы) И НЕ ЗначениеЗаполнено(Стр.ОКУДШаблона) Тогда
					
					СписокЗагрузкиШаблонов.Добавить(Стр.ОКУДФормы, Наименование);
					
				ИначеЕсли Стр.ОтображатьВДереве И ЗначениеЗаполнено(Стр.ИмяФайлаШаблона) Тогда
					
					Если ЗначениеЗаполнено(Стр.ОКУДФормы) Тогда
						Если СтрНачинаетсяС(СокрЛ(Стр.НаименованиеШаблона), "(") Тогда
							НовСтр = ГруппаЭлектронныеПредставления.Строки.Добавить();
						Иначе
							НовСтр = СтрокаПервогоУровня.Строки.Добавить();
						КонецЕсли;
					Иначе
						НовСтр = ГруппаЭлектронныеПредставления.Строки.Добавить();
					КонецЕсли;
					
					НовСтр.Наименование = Наименование + ".xml";
	...
	Если ЗначениеЗаполнено(СписокЗагрузкиШаблонов) Тогда
		ОбновитьШаблоныЧерезВебСервис(СписокЗагрузкиШаблонов); // обновляет в фоновом задании
	КонецЕсли;
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОбновитьШаблоныЧерезВебСервис(СписокШаблонов)
	
	ПараметрыЗапуска = ДлительныеОперации.ПараметрыВыполненияВФоне(Новый УникальныйИдентификатор);
	...
	Отбор = Новый Структура("Ключ,Состояние", ПараметрыЗапуска.КлючФоновогоЗадания, СостояниеФоновогоЗадания.Активно);
	Задания = ФоновыеЗадания.ПолучитьФоновыеЗадания(Отбор);
	
	Если Задания.Количество() = 0 Тогда
		ПараметрыВыполнения = Новый Структура("СписокШаблонов", СписокШаблонов);
		Результат = ДлительныеОперации.ВыполнитьВФоне(
			"РегламентированнаяОтчетность.ОбновитьШаблоныСпискомЧерезВебСервисНаСервереВФоне",
