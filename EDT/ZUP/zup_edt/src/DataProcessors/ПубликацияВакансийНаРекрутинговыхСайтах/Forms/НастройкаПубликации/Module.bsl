...
Процедура ДеревоНастроекПубликацияВакансииНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ЭлементыСоСпециальнойФормойРедактированияСписка)
		...
		Если(ТекущиеДанные.Путь = "contacts.phones" 
			...
			ИменаСвойств = ИнтеграцияРекрутинговыхСайтовКлиент.ИменаСвойствТелефона(Сайт);
...
Процедура Опубликовать(Команда)
	...
	Если ЗначениеЗаполнено(РезультатДействия) Тогда
		...
		Успешно = РезультатДействия.Успешно;
		
		Если Успешно Тогда
			ИнтеграцияРекрутинговыхСайтовКлиент.ОпубликоватьВакансиюЗавершение(ЭтотОбъект);
		ИначеЕсли ПовторнаяПубликация Тогда
...
Процедура ОбновитьПубликацию(Команда)
	...
	Если ЗначениеЗаполнено(РезультатДействия) Тогда
		...
		Если РезультатДействия.Успешно Тогда
			ИнтеграцияРекрутинговыхСайтовКлиент.ОбновитьПубликациюВакансииЗавершение(ЭтотОбъект);
		Иначе
			ПредложитьРазмещениеНовойВакансии(РезультатДействия.ТекстОшибки);
		КонецЕсли;
...
Процедура ЗагрузитьКлассификаторы(Сайт)
	...
	ДлительнаяОперация = ЗагрузитьКлассификаторыНаСервере(Сайт);
	...
	Элементы.ГруппаЗагрузкаКлассификаторов.Видимость = Истина;
	Элементы.ГруппаЗагрузкаКлассификаторов.ТекущаяСтраница = Элементы.ГруппаКлассификаторыЗагружаются;
...
&НаСервере
Функция ЗагрузитьКлассификаторыНаСервере(Сайт)
	
	Если Не Сайт = ПредопределенноеЗначение("Справочник.ИсточникиИнформацииОКандидатах.Zarplata") Тогда
	...
	Возврат ДлительныеОперации.ВыполнитьВФоне("ИнтеграцияРекрутинговыхСайтов.ЗагрузитьКлассификаторыСайта",
...
Процедура ЗагрузитьКлассификаторыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат.Статус = "Отменено" Тогда
		Элементы.ГруппаЗагрузкаКлассификаторов.ТекущаяСтраница = Элементы.ГруппаОшибкаЗагрузкиКлассификаторов;
		Элементы.НадписьОшибкаЗагрузкиКлассификаторов.Заголовок = СтрШаблон(НСтр("ru = 'Загрузка классификаторов отменена. Обратитесь к администратоу.'"));
	Иначе
		Возврат;
	КонецЕсли;
...
Процедура НастроитьКомандыИДеревоНастроек()
	...
	ВидыПолей = ИнтеграцияРекрутинговыхСайтов.ВидыПолей(Сайт, ОбновлениеПубликации);
...
Функция ПовторноОпубликоватьВакансию()
	...
	Если ПроверитьЗаполнениеПолей(Дерево) Тогда
		Возврат ИнтеграцияРекрутинговыхСайтов.ПовторноОпубликоватьВакансию(Вакансия, Сайт, Дерево, ИдентификаторВакансии);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Функция ОбновитьПубликациюВакансии()
	...
	Результат = ИнтеграцияРекрутинговыхСайтов.НовыйРезультатДействияСВакансией();
	Дерево = РеквизитФормыВЗначение("ДеревоНастроек", Тип("ДеревоЗначений"));
	
	Если ПроверитьЗаполнениеПолей(Дерево) Тогда
		Возврат ИнтеграцияРекрутинговыхСайтов.ОбновитьПубликациюВакансии(Вакансия, Сайт, Дерево, ИдентификаторВакансии);
	Иначе
		Возврат Неопределено;
	КонецЕсли;
...
Процедура СинхронизироватьСвойстваПубликацииВакансии()
	...
	Если ИнтеграцияРекрутинговыхСайтов.СинхронизироватьСвойстваПубликацииВакансии(Вакансия, Сайт, ИдентификаторВакансии) Тогда
		
		ОбновленныеСвойства = ИнтеграцияРекрутинговыхСайтов.СвойстваПубликацииВакансии(Вакансия, Сайт);
		ПовторнаяПубликация = ЗначениеЗаполнено(ОбновленныеСвойства.ДатаСнятияСПубликации);
		
		Если ПовторнаяПубликация И Сайт = ИнтеграцияРекрутинговыхСайтовКлиентСервер.HeadHunter() Тогда
...
Процедура ОбновитьДерево(ОбновлениеПубликации)
	...
	Если Сайт = ИнтеграцияРекрутинговыхСайтовКлиентСервер.HeadHunter() Тогда
		...
		МассивПолейВакансии = ИнтеграцияРекрутинговыхСайтов.ПоляВакансииHeadHunter(ОбновлениеПубликации);
		ЗаполнитьДерево(МассивПолейВакансии);
		
	ИначеЕсли Сайт = ИнтеграцияРекрутинговыхСайтовКлиентСервер.Rabota() Тогда
		
		МассивПолейВакансии = ИнтеграцияРекрутинговыхСайтов.ПоляВакансииRabota();
		ЗаполнитьДерево(МассивПолейВакансии);
	...
	ИначеЕсли Сайт = ИнтеграцияРекрутинговыхСайтовКлиентСервер.Zarplata() Тогда
		...
		МассивПолейВакансии = ИнтеграцияРекрутинговыхСайтов.ПоляВакансииZarplata();
		ЗаполнитьДерево(МассивПолейВакансии);
