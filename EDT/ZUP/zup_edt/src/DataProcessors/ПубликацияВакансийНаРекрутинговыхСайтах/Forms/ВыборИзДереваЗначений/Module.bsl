...
Процедура ПометитьВыбранныеСтроки(ДеревоОбъект, ВыбранныеСтроки)
	...
	Для Каждого ВыбраннаяСтрока Из ВыбранныеСтроки Цикл
		...
		КолонкаПоиска = "Идентификатор";
		
		Если ТипЗнч(ВыбраннаяСтрока.Значение) = Тип("Структура") Тогда
			Если ВыбраннаяСтрока.Значение.Свойство("ИдентификаторОбщий") Тогда
				ЗначениеПоиска = ВыбраннаяСтрока.Значение.ИдентификаторОбщий;
			Иначе
				КолонкаПоиска = "Значение";
				ЗначениеПоиска = ВыбраннаяСтрока.Представление;
			КонецЕсли;
		Иначе
			ЗначениеПоиска = ВыбраннаяСтрока.Значение;
		КонецЕсли;
		
		СтрокаДерева = ДеревоОбъект.Строки.Найти(ЗначениеПоиска, КолонкаПоиска, Истина);
...
Процедура ОбработатьСтроку(Строки, ВыбранныеСтроки)
	...
	Для Каждого Строка Из Строки Цикл
		...
		Если Строка.Пометка = 1 
			...
			Если Строка.ИдентификаторДоп = Неопределено Тогда
				ВыбранныеСтроки.Добавить(Строка.Идентификатор, Строка.Значение);
			Иначе
				Если ТипЗнч(Строка.ИдентификаторДоп) = Тип("ФиксированнаяСтруктура") Тогда
					Идентификаторы = Новый Структура(Строка.ИдентификаторДоп);
				Иначе
					Идентификаторы = Новый Структура;
					Идентификаторы.Вставить("ИдентификаторДоп", Строка.ИдентификаторДоп);
