...
Процедура ДобавитьСтрокуВыявленныхНачислений(ИсточникНачислений, СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений, КатегорииПособий, ПериодыЦеломесячныхНачислений)
	
	Если ИсточникНачислений = "ПлановыеНачисления" Тогда
		ДобавитьСтрокуПлановыхНачислений(СвойстваПериодаВыявления, СтрокаДобавляемыхНачислений, СвойстваНачислений, КатегорииПособий, ПериодыЦеломесячныхНачислений);
...
Процедура ДобавитьСтрокуПлановыхНачислений(СвойстваПериодаВыявления, СтрокаПлановыхНачислений, СвойстваНачислений, КатегорииПособий, ПериодыЦеломесячныхНачислений)
	...
	Если НастройкиНачислений.РассчитыватьЛьготы
		И СвойстваТекущегоНачисления.ЯвляетсяЛьготой Тогда
		
		ДобавлятьНачисление = Истина;
	ИначеЕсли НастройкиНачислений.РассчитыватьПособия
		И КатегорииПособий[СвойстваТекущегоНачисления.КатегорияНачисленияИлиНеоплаченногоВремени] = Истина Тогда
...
Процедура ВычеркнутьРанееОплаченныеПериоды(СотрудникиПериоды)
	...
	СотрудникиПериоды.Сортировать("Сотрудник, ДатаНачала", Новый СравнениеЗначений);
	...
	ВычеркнутьРанееОплаченныеПериодыПерерасчетов(СотрудникиПериоды, ВычеркиваемыеПериоды);
	ВычеркнутьРанееОплаченныеОсновныеПериоды(СотрудникиПериоды, ВычеркиваемыеПериоды);
КонецПроцедуры

Процедура ВычеркнутьРанееОплаченныеПериодыПерерасчетов(СотрудникиПериоды, ВычеркиваемыеПериоды)
...
Процедура ДобавитьКорректировкуРанееВыполненныхНачислений(СотрудникиПериоды)
	...
	ОбходТаблицыПериодов = ОбходТаблицы(ПериодыКорректировки, "Сотрудник", "ДатаНачала", "ДатаОкончания", Истина);
	...
	Для Каждого СтрокаДобавляемогоНачисления Из ДобавляемыеНачисления Цикл
		Если Не ЗначенияИзмеренийСовпадают(СтрокаДобавляемогоНачисления, ПредыдущаяСтрокаДобавляемыхНачислений, "Сотрудник, Подразделение, ДокументОснование, Начисление") Тогда
			...
			СброситьОбходТаблицыНаПервуюСтрокуПоКлючу(ОбходТаблицыПериодов);
		КонецЕсли;				
		
		СтрокаАктуальногоПериода = СледующаяАктуальнаяСтрока(СтрокаДобавляемогоНачисления, ОбходТаблицыПериодов, СтрокаАктуальногоПериода);
		Пока СтрокаАктуальногоПериода <> Неопределено Цикл
			...
			Иначе	
				...
				СтрокаАктуальногоПериода = СледующаяАктуальнаяСтрока(СтрокаДобавляемогоНачисления, ОбходТаблицыПериодов, Неопределено);	
			КонецЕсли;	
		КонецЦикла;	
	КонецЦикла;		
КонецПроцедуры	
	...
	Для Каждого РанееВыполненноеНачисление Из РанееВыполненныеНачисления Цикл
		...
		Если ЗначениеЗаполнено(ДатаПриемаУвольненияСотрудника.ДатаУвольнения) Тогда
			Если РанееВыполненноеНачисление.ДатаОкончания > ДатаПриемаУвольненияСотрудника.ДатаУвольнения Тогда
				УточненныйПериод.ДатаОкончания = ДатаПриемаУвольненияСотрудника.ДатаУвольнения; 	
			КонецЕсли;
...
Процедура СкорректироватьБазуРанееНачисленныхСумм(РассчитанныеНачисления)
	...
	Отбор = Новый Структура("ИдентификаторСтрокиБазы");
...
Процедура СкорректироватьРанееВыполненныеНачисления(СтрокиНачислений)
	...
	Для Каждого СтрокаНачислений Из СтрокиНачислений Цикл
		Если РасчетНачисленияЗафиксирован(СтрокаНачислений) Тогда
...
Процедура ЗаполнитьРаспределениеРанееНачисленныхСумм()
	...
	ОтборРапределенияБазовых = Новый Структура("РегистраторИзмерение, ИдентификаторСтроки");
	...
	Для Каждого СтрокаНачисления Из Начисления Цикл
		...
		Для Каждого СтрокаБазы Из СтрокиБазы Цикл
			ОтборРапределенияБазовых.РегистраторИзмерение = СтрокаБазы.Регистратор;
			ОтборРапределенияБазовых.ИдентификаторСтроки = СтрокаБазы.ИдентификаторСтрокиБазы;

			СтрокиРаспределения = РаспределениеБазовыхНачислений.НайтиСтроки(ОтборРапределенияБазовых);
...
Процедура РазбитьНачисленийПоДатам(ТаблицаНачислений, ДатыРазбиения, РазбиватьБезусловно = Ложь)
	...
	Пока ИндексТекущейСтроки < ТаблицаНачислений.Количество()  Цикл
		...
		Пока Истина Цикл
			СтрокаТалицыРазбиения = СледующаяСтрокаПоВхождениюВИнтервал(ОбходТаблицыДат, СтрокаТаблицыНачислений, СтрокаТаблицыНачислений.ДатаНачала, СтрокаТаблицыНачислений.ДатаОкончания);
...
Процедура РазбитьУдержанийПоДатам(ТаблицаУдержаний, ДатыРазбиения, РазбиватьБезусловно = Ложь)
	...
	Пока ИндексТекущейСтроки < ТаблицаУдержаний.Количество()  Цикл
		...
		Пока Истина Цикл
			СтрокаТалицыРазбиения = СледующаяСтрокаПоВхождениюВИнтервал(ОбходТаблицыДат, СтрокаТаблицыУдержаний, СтрокаТаблицыУдержаний.ДатаНачала, СтрокаТаблицыУдержаний.ДатаОкончания);
			Если СтрокаТалицыРазбиения = Неопределено Тогда
				Прервать;
			КонецЕсли;
...
Процедура ПометитьВычеркиваемыеПериоды(СтрокиИсходныхИнтервалов, СтрокиВычеркиваемыхИнтервалов, Знач ИндексПервойСтроки = 0, Знач ИндексПоследнейСтроки = Неопределено)
	...
	ОбходТаблицыВычеркиваемыхПериодов = ОбходТаблицы(СтрокиВычеркиваемыхИнтервалов, "Сотрудник", "ДатаНачала", "ДатаОкончания", Истина);
...
Процедура ЗаполнитьФиксированныеСуммыНачислений()
	...
	Для Каждого СтрокаНачислений Из Начисления Цикл
		...
		ТекущаяСтрокаПлановыхНачислений = СледующаяСтрокаВключающаяПериод(ОбходПлановыхНачислений, СтрокаНачислений, Период, ТекущаяСтрокаПлановыхНачислений);
...
Процедура ЗаполнитьЗначенияПоказателейПроцентДоплатыЗаРаботуВНочноеВечернееВремя(ПоказателиСотрудников, СтрокиНачисленийПоИдентификаторам)
	...
	Для Каждого СтрокаПоказателя Из ЗаполняемыеПоказатели Цикл
		...
		Если ЗначениеЗаполнено(ПроцентДоплатыЗаНочные) И СтрокаПоказателя.Показатель = ПроцентДоплатыЗаНочные Тогда
			...
			ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаНачисления, СтрокаПоказателя.Показатель, Значение);	
		ИначеЕсли ЗначениеЗаполнено(ПроцентДоплатыЗаВечерние) И СтрокаПоказателя.Показатель = ПроцентДоплатыЗаВечерние Тогда
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
			Если ЗначениеЗаполнено(ПроцентДоплатыЗаНочныеВоеннослужащим) И СтрокаПоказателя.Показатель = ПроцентДоплатыЗаНочныеВоеннослужащим Тогда
				...
				ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаНачисления, СтрокаПоказателя.Показатель, Значение);	
			ИначеЕсли ЗначениеЗаполнено(ПроцентДоплатыЗаВечерниеВоеннослужащим) И СтрокаПоказателя.Показатель = ПроцентДоплатыЗаВечерниеВоеннослужащим Тогда
...
Процедура ЗаполнитьЗначенияПереопределяемыхПоказателейНачислений(ПоказателиСотрудников, СтрокиНачисленийПоИдентификаторам)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		Модуль.ЗаполнитьЗначенияПоказателейРасчетаЗарплатыПоТаблицеЗначений(ПоказателиСотрудников.ДополнительныеПоказатели, ЗаполняемыеПоказатели, ЭтотОбъект);
	КонецЕсли;
...
Функция СтоимостьЧасаДня(ЗаполняемыеПоказатели)
	...
	ОбходТаблицыПорядкаРасчета = ОбходТаблицы(ПорядокРасчетаСтоимостиЕдиницыВремени, "Сотрудник", "ДатаНачала", "ДатаОкончания", Истина);
...
Функция ПоказателиСтоимостиЧасаДня()
	...
	ПоказателиСтоимости.СтоимостьДняПоказатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СтоимостьДня");
	ПоказателиСтоимости.СтоимостьДняЧасаПоказатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СтоимостьДняЧаса");
	ПоказателиСтоимости.СтоимостьЧасаПоказатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СтоимостьЧаса");
...
Процедура ЗаполнитьОтборДляПроверкиТерриторий(Отбор, СтрокаНабора)
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниБезУчетаПраздников(СтрокиНачисленийКРасчету)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниБезПраздников");
...
Процедура РассчитатьЗначенияПоказателейКалендарныеДниМесяцаБезУчетаПраздников(СтрокиНачисленийКРасчету)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяцаБезПраздников");
	...
	Для Каждого СтрокаНачисления Из СтрокиНачисленийКРасчету Цикл
		...
		ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаНачисления, Показатель, Значение, Истина);
	КонецЦикла			
КонецПроцедуры
...
Процедура РассчитатьЗначенияПоказателейКоличествоДнейОтпуска(СтрокиНачисленийКРасчету)
	...
	Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоличествоДнейОтпуска");
...
Функция ЗначениеОперативногоПоказателя(СтрокаНачисления, СтрокиФПД, ЗначенияПоказателей, ОтборПоказателей, ПоказательВЦеломЗаМесяц)
	...
	Если ПоказательВЦеломЗаМесяц Тогда
		...
		Если СтрокиЗначений.Количество() > 0 Тогда
			...
			Если НастройкиРасчета.РассчитатьПоказателиНакапливаемыеВЦеломЗаМесяц Тогда
				СтруктураПоиска = Новый Структура("Сотрудник,ПериодДействия,Начисление,Сторно");
...
Процедура ЗаполнитьЗначенияПереопределяемыхПоказателейУдержаний(ПоказателиФизическихЛиц, СтрокиПоИдентификатору)	
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба");
		Модуль.ЗаполнитьЗначенияПоказателейРасчетаЗарплатыПоТаблицеЗначений(ПоказателиФизическихЛиц.ДополнительныеПоказатели, ЗаполняемыеПоказатели, ЭтотОбъект);
	КонецЕсли;
...
Процедура ЗаполнитьФиксированныеСуммыУдержаний()
	...
	Для Каждого СтрокаУдержаний Из Удержания Цикл
		...
		ТекущаяСтрокаПлановыхУдержаний = СтрокаВключающаяДату(СтрокиПлановыхУдержаний, СтрокаУдержаний.ДатаНачала);
...
Процедура ЗаполнитьОплаченноеВремяКоличествомЧасов(Строка, Показатель, Значение, СтандартнаяОбработка)
	...
	ПоказателиДней.Добавить(ВремяВЧасах);
	...
	Строка.ОплаченоЧасов = Значение;
	Если Показатель = ВремяВЧасах Тогда
...
Процедура ЗаполнитьОплаченнымВременемВремяВДнях(Строка, Показатель)
	...
	Если Показатель <> ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВДнях") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОплаченнымВременемВремяВДняхЧасах(Строка, Показатель)
	...
	Если Показатель <> ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.ВремяВДняхЧасах") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОплаченнымВременемНормуЧасов(Строка, Показатель)
	...
	Если Показатель <> ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВЧасах") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьОплаченнымВременемНормуДнейЧасов(Строка, Показатель)
	...
	Если Показатель <> ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДнейЧасов") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьСведенияНачисленийДляРасчетаСлужебный(Требования = Неопределено) 
	...
	Для Каждого СтрокаТаблицыНачислений Из Начисления Цикл
		...
		Если СтрокиИсточниковУчетаВремени <> Неопределено Тогда
			СтрокаИсточниковУчетаВремени = СтрокаВключающаяДату(СтрокиИсточниковУчетаВремени, СтрокаТаблицыНачислений.ДатаНачала);
		КонецЕсли;
...
Функция СтрокиНабораНачисленияПоИдентификаторам()
...
Функция СтрокиНачисленийПоТребованиям()
	...
	ПоказателиКалендарныеДни.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДни"), Истина);
	ПоказателиКалендарныеДни.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КалендарныеДниМесяца"), Истина);
	
	ПоказателиНормыВремени = Новый Соответствие;
	ПоказателиНормыВремени.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДней"), Истина);
	ПоказателиНормыВремени.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВДнях"), Истина);
	ПоказателиНормыВремени.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаВремениВЧасах"), Истина);
	ПоказателиНормыВремени.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.НормаДнейЧасов"), Истина);
	...
	ПоказателиКоэффициентыИндексации.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСреднегоЗаработка"), Истина);
	ПоказателиКоэффициентыИндексации.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоСодержания"), Истина);
...
Процедура ОграничитьРазмерУдержанийДоПредела(УдалитьВТ, СтрокиПоИдентификатору)
	...
	ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(МенеджерВременныхТаблиц, ДанныеТекущегоНабора.ТаблицаНабора, "ВТРегистрРасчета_Удержания", Истина);
...
Процедура ОграничитьРазмерВзысканий(СтрокиПоИдентификатору);
	...
	РезультатВзысканий = ОграничениеВзысканий.РезультатОграниченияВзысканийДокумента(ДанныеНачислений, ДанныеУдержаний, Зарплата.НДФЛ);
...
Процедура СформироватьДоходыНДФЛПоНачислениям(Начисления, ДатаОперации)
	...
	Если НастройкиНДФЛ.ЗасчитыватьДанныеАвансов Тогда
		УчетНДФЛРасширенный.ДополнитьДвиженияСписаниемАванса(ИсключаемыйРегистратор, НаборыЗаписей, Организация, ДатаОперации);
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗаполнитьВычетыКДоходамНДФЛ(Начисления)
	...
	Запрос.Текст =
		...
		|	И НЕ Начисления.ФиксСуммаВычета
		...
		|ГДЕ
		|	Начисления.Начисление.КодДоходаНДФЛ В(&ДоходыСВычетами)
		|	И (Начисления.Сторно = ИСТИНА ИЛИ Начисления.ФиксСуммаВычета)
	...
	|	КОНЕЦ КАК КодВычета,
	|	ВЫБОР
	|		КОГДА Начисления.Сторно ИЛИ Начисления.ФиксСуммаВычета
...
Процедура ВыполнитьОтражениеНДФЛИКорректировокВыплатыВБухучете()
	...
	Если Не БухучетВыполняемыеДействия.ВыполнятьБухучетНДФЛ И Не БухучетВыполняемыеДействия.ВыполнятьБухучетКорректировокВыплаты Тогда
		Возврат;
	КонецЕсли;
...
Функция ПредельныеСуммыУдержаний()
	...
	ПерерасчетЗарплаты.УдалитьПредельныеСуммыПерерассчитываемыхУдержаний(ПредельныеСуммыУдержаний, ЭтотОбъект);
...
Процедура ИнициализироватьРасчетБазыНачислений()
	...
	НастройкиРасчетаБазы.РасчетПоДаннымТекущегоНабора = НастройкиРасчета.РассчитыватьНачисленияТолькоПоТекущемуДокументу;
...
Функция ДатыПриемаУвольненияПоКадровойИстории(КадроваяИсторияСотрудника, Сотрудник)
	...
	Для Каждого СтрокаКадровойИстории Из КадроваяИсторияСотрудника Цикл
		...
		ИначеЕсли СтрокаКадровойИстории.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Увольнение 
			...
			ДатыПриемаУвольнения.ДатаУвольнения = НачалоДня(СтрокаКадровойИстории.ПериодЗаписи);
		КонецЕсли;
	КонецЦикла;	
...
Функция КоэффициентыИндексацииСохраняемогоДенежногоСодержания()
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
		Модуль = ОбщегоНазначения.ОбщийМодуль("РасчетДенежногоСодержания");
		КэшКоэффициентыИндексацииСохраняемогоДенежногоСодержания = Модуль.КоэффициентИндексацииДенежногоСодержания(Фильтр);
	Иначе
		КэшКоэффициентыИндексацииСохраняемогоДенежногоСодержания = Новый ТаблицаЗначений;
	КонецЕсли;
...
Процедура ЗаполнитьДатыРазбиенияПоИзменениямКлассовУсловийТрудаПоДолжностям(Начисления, ДатыРазбиения, СвойстваВидовРасчета)
	...
	Для Каждого СтрокаКадровойИстории Из КадроваяИсторияСотрудников Цикл
		...
		КонецЕсли;
	КонецЦикла;
	ТаблицаФильтра = КадроваяИсторияСотрудников.Скопировать(,"Должность,ДолжностьПоШтатномуРасписанию");
	КлассыУсловияТрудаПоДолжностям = КадровыйУчетРасширенный.ТаблицаДатИзмененийКлассовУсловийТрудаПоДолжностям(ТаблицаФильтра, ДатаНачала, ДатаОкончания);
	
	Если КлассыУсловияТрудаПоДолжностям.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(КадроваяИсторияСотрудников, "Должность,ДолжностьПоШтатномуРасписанию");
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(Начисления, "Сотрудник");
	СтруктураПоискаКадровойИстории = Новый Структура("Должность,ДолжностьПоШтатномуРасписанию");
	КлассыУсловияТрудаПоДолжностям.Сортировать("Должность,ДолжностьПоШтатномуРасписанию,Период", Новый СравнениеЗначений);
	ПредыдущаяСтрока = Неопределено;
	
	Для Каждого ТекущаяСтрока Из КлассыУсловияТрудаПоДолжностям Цикл
		Если ПредыдущаяСтрока = Неопределено
			Или ПредыдущаяСтрока.Должность <> ТекущаяСтрока.Должность
			Или ПредыдущаяСтрока.ДолжностьПоШтатномуРасписанию <> ТекущаяСтрока.ДолжностьПоШтатномуРасписанию Тогда
...
Функция СледующаяАктуальнаяСтрока(ТекущаяСтрокаИсходнойТаблицы, ОбходТаблицыРазбиения, ТекущаяАктуальнаяСтрока = Неопределено)
...
Функция СледующаяСтрокаПоВхождениюВИнтервал(ОбходТаблицы, ЗначениеКлюча, ДатаНачалаИнтервала, ДатаОкончанияИнтервала = Неопределено)
...
Процедура СброситьОбходТаблицыНаПервуюСтрокуПоКлючу(ОбходТаблицы)
	ОбходТаблицы.ИндексТекущейСтроки = Неопределено;
