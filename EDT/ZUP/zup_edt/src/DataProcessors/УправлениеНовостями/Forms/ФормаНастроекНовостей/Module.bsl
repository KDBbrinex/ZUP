...
Функция ВыполнитьШагПроверки_ЗаполненностьОбъектовМетаданных()
	...
	Результат = ИнтернетПоддержкаПользователейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(ЛогШаговВыполнения, Символы.Таб, Истина, Ложь);
...
#Область АдминистративныеОтборы
	...
	СписокСтрокой = ИнтернетПоддержкаПользователейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(Список, Символы.Таб, Истина, Ложь);
		...
		Пока Выборка.Следующий() Цикл
			...
			Попытка
				...
				Если ВременныйФайл.Существует() Тогда
					ПервыеСтрокиФайла = ИнтернетПоддержкаПользователей.СодержимоеТекстовогоФайла(ВременныйФайл.ПолноеИмя, 100);
			...
			РезультатШага = 
				"### " + Выборка.Код + ", " + Выборка.Наименование + ":"
				+ ИнтернетПоддержкаПользователейКлиентСервер.ВставитьОтступВМногострочнуюСтроку(ЛогШаговВыполненияВложенный, Символы.Таб, Истина, Ложь);
...
Функция ВыполнитьШагПроверки_РегламентноеЗадание()
	...
	Если ТипЗнч(ПоследнееЗадание) = ТипФоновоеЗадание Тогда
		ПоследнееЗаданиеПредставление = Новый Структура;
			...
			Если ТипЗнч(ПоследнееЗадание.ИнформацияОбОшибке) = ТипОшибка Тогда
				ПоследнееЗаданиеПредставление.Вставить("ИнформацияОбОшибке", ПодробноеПредставлениеОшибки(ПоследнееЗадание.ИнформацияОбОшибке));
			Иначе
	...
	Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		...
		ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(РасписаниеРегламентногоЗадания, "=", Символы.ПС, 1),
		ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(ПоследнееЗаданиеПредставление, "=", Символы.ПС, 1));
