...
Процедура ЗагрузитьФайлыНовостейССервера(ЛентыНовостей, КонтекстВыполнения = Неопределено) Экспорт
	...
	Перем HTTPСоединение, HTTPОтвет;
							...
							Пока (ФайлПолучен <> Истина) И (ПопыткаПолученияФайла < НастройкиПолученияНовостей.HTTPСоединениеКоличествоПопытокПолученияФайла) Цикл
								Попытка
									...
									// Непосредственно загрузка файла, начало.
										...
										HTTPЗапрос = Новый HTTPЗапрос(лкИмяФайлаНаСервере, лкЗаголовки);
										...
										сЗаголовки = "";
										Для каждого ТекущийЗаголовок Из HTTPОтвет.Заголовки Цикл
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											...
											ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(HTTPОтвет.Заголовки, "=", Символы.ПС),
											ТекущееХТТПСоединение.Протокол + "://" + ТекущаяЛентаНовостей.Сайт + "/" + лкИмяФайлаНаСервере,
										...
										Если ВременныйФайл.Существует() Тогда
											...
											Если (HTTPОтвет.КодСостояния >= 500) И (HTTPОтвет.КодСостояния <= 599) Тогда
												Попытка
												...
												Исключение
													...
													ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
														...
														НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка получения текста ошибки сервера'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
									...
									Если (HTTPОтвет.КодСостояния >= 300) И (HTTPОтвет.КодСостояния <= 399) Тогда
										// ////? В случае ошибки HTTP 301, 302, 307, 308 пытаться получить файл по новому адресу
									ИначеЕсли (HTTPОтвет.КодСостояния >= 500) И (HTTPОтвет.КодСостояния <= 599) Тогда
										...
										Если НЕ ПустаяСтрока(ТекстХТМЛРегламентныхРабот) Тогда
											...
											Исключение
												...
												ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
													НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
													НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей. Ошибка расшифровки текста ошибки сервера'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
													УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
													, // ОбъектМетаданных
													, // Данные
													ТекстСообщения, // Комментарий
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка новостей. Загрузка файлов новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
...
#Область ОсновныеОбработки
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Сохранение даты последних загрузок'"));
			...
			// Прочитать файлы
			ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Обработка лент новостей:'"));
			Для каждого ТекущаяСтрока Из ЛентыНовостей Цикл
				...
				Если (ТекущаяСтрока.Пометка) ИЛИ (ТекущаяСтрока.ОбязательныйКанал) Тогда
					...
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Обрабатывается...'"));
				...
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Не помечена для обработки'"));
				КонецЕсли;
...
#Область ВспомогательныеОбработкиОсновные_ПересчитатьОтборыПоНовостям_ДляОбластиДанных
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Если ОбрабатыватьОбластиДанных = Истина Тогда
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Выполнение пересчета отборов по новостям для области данных.'"));
				ОбработкаНовостей.ПересчитатьОтборыПоНовостям_ДляОбластиДанных(КонтекстВыполненияВложенныйВложенный);
			Иначе
				ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Пропуск пересчета отборов по новостям для области данных.'"));
			КонецЕсли;
...
#Область ВспомогательныеОбработкиОсновные_СбросУНовостейПризнакаТребуетсяПересчетОтборов

		//  Только что загруженным или обновленным новостям проставляется признак "ТребуетсяПересчетОтборов",
		//  чтобы они не отображались, пока не будет произведен пересчет отборов.
		// Т.к. пересчет уже был совершен, то необходимо сбросить этот признак.

		ИдентификаторШага1 = "ВспомогательныеОбработкиОсновные.СбросУНовостейПризнакаТребуетсяПересчетОтборов"; // Идентификатор.
		КодРезультатаВложенный = 0;
		ЛогШаговВыполненияВложенный = Новый Массив;
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			КонтекстВыполненияВложенный,
			...
			КонтекстВыполненияВложенныйВложенный = ИнтернетПоддержкаПользователейКлиентСервер.НоваяЗаписьРезультатовВыполненияОпераций(); // Контекст "по шагам".
			ОбработкаНовостей.СбросУНовостейПризнакаТребуетсяПересчетОтборов(КонтекстВыполненияВложенныйВложенный);
...
#КонецОбласти
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Функция ОбновитьСтандартныйСписокССервера(Список, ТекущаяВерсияНаКлиенте, Прокси = Неопределено, КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
						...
						Если ТипЗнч(СписокВсехДанных.channel) = ТипСписокXDTO Тогда // Правильно сработает, даже если передали только одно значение.
							...
							Для Каждого ТекущийЭлементСписка Из СписокВсехЗагруженныхСерверныхЛентНовостей Цикл
								Если ТекущийЭлементСписка.Пометка = Ложь Тогда
									...
									ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
										НСтр("ru='Лента новостей [%1] помечена на удаление, т.к. она была отключена на сервере 1С'"),
										ЛентаНовостейОбъект.Наименование);
									ЛогШаговВыполнения.Добавить(ТекстСообщения);
								КонецЕсли;
							КонецЦикла;
		...
		Иначе
			БылиОшибки = Истина;
			ЛогШаговВыполнения.Добавить(НСтр("ru='Не удалось подключиться к веб-сервисам.'"));
		КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ОбновитьСписокСправочниковИВерсииССервера(КонтекстВыполнения = Неопределено) Экспорт
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка классификаторов. Список обновлений классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ПарсингХМЛФайлаНовостей(ОбъектХДТО, ФорматНовостей, ЛентаНовостей, Прокси, КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Если ТипЗнч(ОбъектХДТО.Свойства().Получить("channel")) = ТипСвойствоXDTO
				...
				Если РазрешенаРаботаСНовостямиЧерезИнтернет = Истина Тогда
					...
					Для каждого ТекущиеОбновляемыеДанные Из ТаблицаОбновляемыхДанных Цикл
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='Обновление стандартного списка %1 (версия %2) с сервера. Результат: %3'"),
								ТекущиеОбновляемыеДанные.Список,
								ТекущиеОбновляемыеДанные.ТекущаяВерсияНаКлиенте,
								Результат));
					КонецЦикла;
				Иначе
					ЛогШаговВыполнения.Добавить(
						НСтр("ru='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(rss): Отключена работа с новостями через интернет.
							|Обновление классификаторов не выполнялось.'"));
				КонецЕсли;
				...
				Если ОбъектХДТО.Свойства().Получить("channel") <> Неопределено Тогда
					...
					Иначе
						...
						ЛогШаговВыполнения.Добавить(
							СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru='В файле полученном для ленты новостей %1 отсутствует поле channel.lastBuildDate.
									|Все новости будут прочитаны и загружены, но это займет больше времени.'"),
								ЛентаНовостей));
					КонецЕсли;
				Иначе
					ЛогШаговВыполнения.Добавить(
		...
		ИначеЕсли НРег(ФорматНовостей) = "feed" Тогда
			...
			Иначе
				ЛогШаговВыполнения.Добавить(
					НСтр("ru='Обработка.УправлениеНовостями.ПарсингХМЛФайлаНовостей(atom): Отключена работа с новостями через интернет.
						|Обновление классификаторов не выполнялось.'"));
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
...
Процедура ЗагрузитьНовость(
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Если НРег(ФорматНовостей) = "rss" Тогда
			...
			Если ЗаписыватьНовость = Истина Тогда
				...
				ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
					...
					Попытка
						...
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Начало попытки записи новости'"));
						...
						ОбъектНовость.ДополнительныеСвойства.Вставить("ТребуетсяПересчетОтборов", Истина);
						...
						ЛогШаговВыполненияВложенный.Добавить(НСтр("ru='Успешная попытка записи новости'"));

					Исключение

						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru='Неудачная попытка записи новости по причине:
								|%1'"),
							ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
						ЛогШаговВыполнения.Добавить(ТекстСообщения);
			...
			Иначе
				...
				ЛогШаговВыполнения.Добавить(НСтр("ru='Новость пропущена'"));
		...
		ЛогШаговВыполнения.Добавить(РезультатЗагрузкиНовости);
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка новостей. Загрузка новости'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ТипЗнч(ДанныеХДТО) = ТипОбъектXDTO Тогда
		...
		Если ТипЗнч(СвойствоДвоичныеДанные) = ТипСвойствоXDTO Тогда
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Загрузка новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru='Новости. Загрузка новостей. Парсинг файлов новостей. Парсинг файла новостей. Парсинг новости. Чтение новости. Блок бинарных данных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Функция ПодключитьсяКВебСервисуОбновленияСтандартныхСписков(КонтекстВыполнения = Неопределено)
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		Попытка
			...
			Если ТипЗнч(Прокси) <> ТипWSПрокси Тогда
				ЛогШаговВыполнения.Добавить(НСтр("ru='Неудачное подключение к веб-сервису обновления стандартных списков'"));
			Иначе
				ЛогШаговВыполнения.Добавить(НСтр("ru='Успешное подключение к веб-сервису обновления стандартных списков'"));
			КонецЕсли;
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:Новости.Загрузка классификаторов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Новости. Загрузка классификаторов. Соединение с веб-сервисом обновления стандартных списков'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
