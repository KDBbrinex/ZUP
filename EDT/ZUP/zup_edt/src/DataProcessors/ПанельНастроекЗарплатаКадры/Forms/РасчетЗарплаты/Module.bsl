...
Процедура ОбновитьФормуПоНастройкам(ПрочиеНастройки)
	...
	Если РасчетыСБюджетомПоНДФЛДоступны И ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.КонфигурацииЗарплатаКадрыРасширенная") Тогда
		ТекстПодсказки = НСтр("ru = 'При включении этой настойки будет доступно получение отчета ""Ведомость расчетов с бюджетом по НДФЛ"".
...
Процедура ИспользоватьНачислениеЗарплатыПриИзменении(Элемент)
	...
	ЗаписатьНастройкиНаКлиенте("ИспользоватьСтатьиФинансирования,НастройкиСтатистикиПерсонала,НастройкиРасчетаЗарплаты,НастройкиРасчетаЗарплатыБазовые,НастройкиЗаймовСотрудникам,ИспользоватьНачислениеЗарплаты,РассчитыватьДокументыПриРедактировании");
...
&НаКлиенте
Процедура ВыполнятьРасчетЗарплатыБезОптимизацииПриИзменении(Элемент)
	
	Если Не ВыполнятьРасчетЗарплатыБезОптимизации Тогда
		ЗаписатьВыполнятьРасчетЗарплатыБезОптимизацииНаСервере(ВыполнятьРасчетЗарплатыБезОптимизации);
	Иначе
		Оповещение = Новый ОписаниеОповещения("ЗаписатьВыполнятьРасчетЗарплатыБезОптимизацииЗавершение", ЭтаФорма);
		ТекстВопроса = НСтр("ru = 'Внимание! При включении этой настройки некоторые возможности программы будут недоступны.
		|Вы не сможете зарегистрировать удержанный НДФЛ к перечислению при выплате аванса, отказаться от регистрации
		|удержанного НДФЛ к перечислению при выплате в межрасчетный период являющихся частью оплаты труда премий.
		|В документах регистрации разовых начислений будет недоступна возможность расчета зависимых
		|от базы начислений (""Районный коэффициент"", ""Северная надбавка"", и т.д.).
		|Включить настройку?'");
...
&НаКлиенте
Процедура ЗаписатьВыполнятьРасчетЗарплатыБезОптимизацииЗавершение(Результат, Параметры) Экспорт

	Если Результат <> Неопределено И Результат = КодВозвратаДиалога.Да Тогда
		ЗаписатьВыполнятьРасчетЗарплатыБезОптимизацииНаСервере(Истина);
	Иначе
		ВыполнятьРасчетЗарплатыБезОптимизации = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Процедура ЗаписатьВыполнятьРасчетЗарплатыБезОптимизацииНаСервере(ЗначениеНастройки)

	Константы.ВыполнятьРасчетЗарплатыБезОптимизации.Установить(ЗначениеНастройки);	
...
Процедура ПрочитатьНастройки()
	...
	ВестиРасчетыСБюджетомПоНДФЛПрежняя = ВестиРасчетыСБюджетомПоНДФЛ;
	ВыполнятьРасчетЗарплатыБезОптимизации = Константы.ВыполнятьРасчетЗарплатыБезОптимизации.Получить();
