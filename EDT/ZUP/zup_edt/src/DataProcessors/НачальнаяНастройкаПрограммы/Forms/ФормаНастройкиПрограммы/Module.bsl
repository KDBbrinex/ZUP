...
&НаСервере
Процедура ОбработкаОповещенияЗаписьНачальнойШтатнойРасстановки()

	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,"ГруппаРасчетПервойПоловиныМесяца","Видимость", ПолучитьФункциональнуюОпцию("ИспользоватьРасчетПервойПоловиныМесяца"));	
...
Процедура УчетнаяПолитикаПериодСтрокойНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗарплатаКадрыКлиент.ВводМесяцаНачалоВыбора(
	ЭтаФорма, ЭтаФорма, "НастройкиСистемыНалогообложения.Период", "УчетнаяПолитикаПериодСтрокой");
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "Запись_НачальнаяШтатнаяРасстановка" Тогда
		ОбработкаОповещенияЗаписьНачальнойШтатнойРасстановки();
	КонецЕсли;
