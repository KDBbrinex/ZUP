...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	Заголовок = НСтр("ru = 'Замещение на период'") + " " + ВидОтсутствия + " " + СотрудникИОРодительный;
...
Процедура УстановитьИнфоНадписьПодвал()
	...
	ИнфоНадписьПодвал = СтроковыеФункции.ФорматированнаяСтрока(
		НСтр("ru = 'Полную картину связей документов замещения можно получить, сформировав <a href = %1>отчет</a>.'"), 
		ЦепочкиДокументовФормы.ИнфоНавигационнаяСсылка("ОткрытьОтчет", ДокументОснованиеСсылка));
...
Процедура ЗаполнитьЗначенияЗаполненияНаСервере(ПараметрыФормы, ВидДокумента)
	...
	Если ВидДокумента = "Совмещение" Тогда
		ЗначенияЗаполнения.Вставить("ПричинаСовмещения", ПредопределенноеЗначение("Перечисление.ПричиныСовмещения.ИсполнениеОбязанностей"));
	КонецЕсли;
...
Функция ДолжностьПоШтатномуРасписаниюСотрудника(Сотрудник, ДатаНачала)
	...
	Иначе
		Возврат ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ШтатноеРасписание.ПустаяСсылка");
	КонецЕсли;
