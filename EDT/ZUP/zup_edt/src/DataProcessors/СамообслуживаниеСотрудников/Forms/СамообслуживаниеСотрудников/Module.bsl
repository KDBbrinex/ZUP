...
Процедура ПроинициализироватьФорму()
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеФизическихЛиц(Ложь, ФизическоеЛицо, ПоляДанных);
...
&НаСервере
Процедура ЗаполнитьОбразование(КадровыеДанные)
...
&НаСервере
Процедура ЗаполнитьПаспорт(КадровыеДанные)
...
&НаСервере
Процедура ЗаполнитьСостояниеВБраке(КадровыеДанные)
...
&НаСервере
Процедура ЗаполнитьКонтакты(КадровыеДанные)
...
&НаСервере
Процедура ЗаполнитьСоставСемьи()
	
	УстановитьПривилегированныйРежим(Истина);
	ПредставлениеСоставаСемьи = Справочники.РодственникиФизическихЛиц.ПредставлениеСоставаСемьи(ФизическоеЛицо);
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура СформироватьФормуГрафикаРаботыСотрудника()
	...
	Если МесяцГрафикаРаботы < НачалоМесяца(ТекущаяДатаСеанса())  Тогда 
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выбранная дата меньше текущей.'"), , "МесяцГрафикаРаботыСтрокой");
		Возврат;
	КонецЕсли;
...
Функция ПревышенРазмерЛимита()
	...
	Если ПревышенЛимит Тогда 
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения, , "РазмерПакетаЛьгот");
	КонецЕсли;
