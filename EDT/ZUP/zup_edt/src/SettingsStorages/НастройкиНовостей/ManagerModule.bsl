...
//    - Для этого ключа объекта ключ настроек представляет собой "ИмяМетаданных|ИмяФормы" (разделены вертикальной чертой);
//    - Отбор по пользователю - есть.
//
////////////////////////////////////////////////////////////////////////////////
...
Процедура ОбработкаЗагрузки(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ ПроверитьПраваДоступа("ОбработкаЗагрузки") Тогда
		Возврат;
	КонецЕсли;
	...
	Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором") // Структура
			...
			ИЛИ (КлючОбъекта = "СостояниеПодсистемы") // Структура
...
Процедура ОбработкаПолученияОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ ПроверитьПраваДоступа("ОбработкаПолученияОписания") Тогда
		Возврат;
	КонецЕсли;
	...
	Если (КлючОбъекта = "НастройкиПоказаНовостейНастроенныеАдминистратором")
			...
			ИЛИ (КлючОбъекта = "СостояниеПодсистемы")
...
Процедура ОбработкаСохранения(КлючОбъекта, КлючНастроек, Настройки, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ ПроверитьПраваДоступа("ОбработкаСохранения") Тогда
		Возврат;
	КонецЕсли;
		...
		УстановитьПривилегированныйРежим(Истина);
			...
			Исключение
				...
				ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
					НСтр("ru='БИП:Новости.Хранилище настроек новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
					НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	ИначеЕсли КлючОбъекта = "ОтборыПоЛентамНовостейНастроенныеАдминистратором" Тогда
		...
		Если НаборЗаписей.ПроверитьЗаполнение() Тогда
			НаборЗаписей.Записать(Истина); // КлючОбъекта = "ОтборыПоЛентамНовостейНастроенныеАдминистратором". // АПК:1327 Конкурентная работа маловероятна.
		Иначе
			ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru='БИП:Новости.Хранилище настроек новостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
				НСтр("ru='Новости. Хранилище настроек новостей. ОбработкаСохранения. Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Процедура ОбработкаУстановкиОписания(КлючОбъекта, КлючНастроек, ОписаниеНастроек, ИмяПользователяИБ)
	...
	Если НЕ ПроверитьПраваДоступа("ОбработкаУстановкиОписания") Тогда
		Возврат;
	КонецЕсли;
...
Функция ПровестиВалидациюНастроек(Настройки, КлючОбъекта, КлючНастроек) Экспорт
	...
	ИначеЕсли КлючОбъекта = "ОтключенныеПанелиКонтекстныхНовостей" Тогда
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			КонецЕсли;
			Если НЕ ПустаяСтрока(ИмяМетаданных) Тогда
				ПравильныеНастройки.Вставить("Метаданные", ИмяМетаданных);
			КонецЕсли;
			Если НЕ ПустаяСтрока(ИмяФормы) Тогда
				ПравильныеНастройки.Вставить("Форма", ИмяФормы);
			КонецЕсли;
		КонецЕсли;
	...
	ИначеЕсли КлючОбъекта = "СостояниеПодсистемы" Тогда
		// Для этого ключа объекта не действует КлючНастроек.
		// Обязательные поля:
		// - ТекущееСостояние        - Строка - Допустимые состояния:
		...
		// - ДатаУстановкиТекущегоСостояния             - Дата - когда было установлено текущее состояние (в UTC);
		// - ДатаПринудительногоСбросаТекущегоСостояния - Дата - когда состояние неактивности будет принудительно сброшено к Активно (в UTC).
		ТекущаяДатаДляУстановки = ТекущаяУниверсальнаяДата();
		Результат = Новый Структура;
			Результат.Вставить("ТекущееСостояние"                          , "Активна");
			Результат.Вставить("ДатаУстановкиТекущегоСостояния"            , ТекущаяДатаДляУстановки);
			Результат.Вставить("ДатаПринудительногоСбросаТекущегоСостояния", '00010101');
		...
		Если ТипЗнч(Настройки) = ТипСтруктура Тогда
			...
			//  правильный тип;
			//  правильный интервал значения (вхождение в список значений, минимум, максимум).
			Настройки.Свойство("ТекущееСостояние", Результат.ТекущееСостояние);
			Если (Результат.ТекущееСостояние <> "Активна")
				...
				Результат.ТекущееСостояние = "Активна";
			КонецЕсли;
			//
			Настройки.Свойство("ДатаУстановкиТекущегоСостояния", Результат.ДатаУстановкиТекущегоСостояния);
			Если ТипЗнч(Результат.ДатаУстановкиТекущегоСостояния) <> ТипДата
					ИЛИ (Результат.ДатаУстановкиТекущегоСостояния = '00010101') Тогда
				Результат.ДатаУстановкиТекущегоСостояния = ТекущаяДатаДляУстановки;
			КонецЕсли;
			//
			Настройки.Свойство("ДатаПринудительногоСбросаТекущегоСостояния", Результат.ДатаПринудительногоСбросаТекущегоСостояния);
			Если ТипЗнч(Результат.ДатаПринудительногоСбросаТекущегоСостояния) <> ТипДата Тогда
				Результат.ДатаПринудительногоСбросаТекущегоСостояния = '00010101';
			КонецЕсли;
			Если (Результат.ТекущееСостояние = "Активна") Тогда
				Результат.ДатаПринудительногоСбросаТекущегоСостояния = '00010101'; // Активна - основное рабочее состояние. Его некуда сбрасывать.
			ИначеЕсли (Результат.ТекущееСостояние = "ТребуетсяПересчетОтборовПослеДлительногоОжидания") Тогда
				ПредельнаяДата = ТекущаяДатаДляУстановки + 1*60*60;
				Если Результат.ДатаПринудительногоСбросаТекущегоСостояния > ПредельнаяДата Тогда
					Результат.ДатаПринудительногоСбросаТекущегоСостояния = ПредельнаяДата;
				КонецЕсли;
			ИначеЕсли (Результат.ТекущееСостояние = "СейчасЗагружаютсяНовости") Тогда
				ПредельнаяДата = ТекущаяДатаДляУстановки + 6*60*60;
				Если Результат.ДатаПринудительногоСбросаТекущегоСостояния > ПредельнаяДата Тогда
					Результат.ДатаПринудительногоСбросаТекущегоСостояния = ПредельнаяДата;
				КонецЕсли;
			КонецЕсли;
...
//
// Параметры:
//  ИмяПроцедуры - Имя вызывающей процедуры, для записи в журнал регистрации.
//
// Возвращаемое значение:
//   Булево - Истина, если права есть, Ложь если прав нет.
//
Функция ПроверитьПраваДоступа(ИмяПроцедуры)
	...
	Если (НЕ ОбработкаНовостейПовтИсп.ЕстьРолиЧтенияНовостей())
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Новости. Хранилище настроек новостей. %1. Нарушение прав доступа'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяПроцедуры), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru='Попытка вызова ХранилищаНастроек.НастройкиНовостей.%1, когда недоступна роль ЧтениеНовостей или ПолныеПрава и не включен привилегированный режим.'",
					ОбщегоНазначения.КодОсновногоЯзыка()),
				ИмяПроцедуры), // Комментарий
