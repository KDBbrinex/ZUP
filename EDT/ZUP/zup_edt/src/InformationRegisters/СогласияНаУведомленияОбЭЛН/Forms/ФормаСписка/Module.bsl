...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПодписатьсяНаУведомления.Видимость = ДоступенОбменЧерезСЭДО;
	Элементы.ОтписатьсяОтУведомлений.Видимость = ДоступенОбменЧерезСЭДО;
...
Процедура СписокВыбор(ТаблицаФормы, Сотрудник, ПолеФормы, СтандартнаяОбработка)
	...
	Если ПолеФормы = Элементы.СписокСогласие
		...
		СЭДОФССКлиент.ОткрытьСогласие(СтрокаТаблицы.Согласие, Организация, Сотрудник);
...
Процедура СоздатьСогласие(Команда)
	...
	Иначе
		МассивСогласий = СоздатьСогласия(Организация, Сотрудники.Массив);
...
Процедура СотрудникПодписалСогласие(Команда)
	...
	Если Сотрудники.Количество = 0 Тогда
		Возврат;
	КонецЕсли;
	КоличествоПроведено = ПровестиСогласия(Организация, Сотрудники.Массив, Истина);
...
Процедура СотрудникНеПланируетПодписыватьСогласие(Команда)
	...
	Если Сотрудники.Количество = 0 Тогда
		Возврат;
	КонецЕсли;
	КоличествоПроведено = ПровестиСогласия(Организация, Сотрудники.Массив, Ложь);
	Если КоличествоПроведено = 0 Тогда
		ЗаголовокОповещения = "";
...
Процедура СотрудникОтозвалСогласие(Команда)
	...
	Если Сотрудники.Количество = 0 Тогда
		Возврат;
	КонецЕсли;
	Если НетСогласийДляОтзыва(Организация, Сотрудники.Массив) Тогда
...
Процедура ПометитьНаУдалениеСогласие(Команда)
	...
	МассивСогласий = МассивСогласийДляУдаления(Организация, Сотрудники.Массив);
	...
	СЭДОФССКлиент.ИзменитьПометкуУдаленияСогласий(МассивСогласий, Истина);
КонецПроцедуры
...
Процедура ОбновитьНадписьСообщенияОжидаемыеОтФСС()
	...
	КоличествоОжидаемыхСообщений = РегистрыСведений.ПодпискиНаУведомленияОбЭЛН.КоличествоОжидаемыхСообщений(Организация);
...
Процедура ПослеОкончанияРедактированияКомментарияКОтзывам(Знач Комментарий, Знач Сотрудники) Экспорт
	...
	РезультатОтзыва = ОтозватьСогласия(Организация, Сотрудники, Комментарий);
...
Процедура ПослеОтветаНаВопросОбУдаленииСогласий(Ответ, Сотрудники) Экспорт
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		Возврат;
	КонецЕсли;
	КоличествоУдалено = УдалитьСогласияСотрудников(Организация, Сотрудники.Массив);
...
Функция СоздатьСогласия(Знач Организация, Знач МассивСотрудников)
	...
	Запрос = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ЗапросСФильтрами(Организация, МассивСотрудников, Состояния);
	...
	Для Каждого Сотрудник Из МассивСотрудников Цикл
		...
		Если СозданноеРанее <> Неопределено И ЗначениеЗаполнено(СозданноеРанее.Согласие) Тогда
			ДокуменОбъект = СозданноеРанее.Согласие.ПолучитьОбъект();
		Иначе
			ДокуменОбъект = Документы.СогласиеНаУведомлениеОбЭЛН.СоздатьДокумент();
			ДокуменОбъект.Дата = НачалоДня(ТекущаяДатаСеанса());
		КонецЕсли;
		ДокуменОбъект.Организация   = Организация;
		ДокуменОбъект.Сотрудник     = Сотрудник;
		ДокуменОбъект.Ответственный = Ответственный;
		ДокуменОбъект.ОтветственныйЗаОбработкуПерсональныхДанных = ОтветственныйЗаОбработкуПерсональныхДанных;
		ДокуменОбъект.ОбновитьВторичныеДанные();
		ДокуменОбъект.Записать(РежимЗаписиДокумента.Запись);
		
		СозданныеДокументы.Добавить(ДокуменОбъект.Ссылка);
...
&НаСервереБезКонтекста
Функция ПровестиСогласия(Знач Организация, Знач МассивСотрудников, Знач СотрудникПодписалСогласие)
	Состояния = Новый Массив;
	Состояния.Добавить(Перечисления.СостоянияСогласийНаУведомленияОбЭЛН.ОжидаетПодписания);
	...
	Для Каждого СозданноеРанее Из СозданныеСогласия Цикл
		ДокуменОбъект = СозданноеРанее.Согласие.ПолучитьОбъект();
		ДокуменОбъект.УстановитьПометкуУдаления(Ложь);
		ДокуменОбъект.СотрудникПодписалСогласие = СотрудникПодписалСогласие;
		ДокуменОбъект.ОбновитьВторичныеДанные();
		ДокуменОбъект.Записать(РежимЗаписиДокумента.Проведение);
	КонецЦикла;
...
&НаСервереБезКонтекста
Функция НетСогласийДляОтзыва(Знач Организация, Знач МассивСотрудников)
	Состояния = Новый Массив;
	Состояния.Добавить(Перечисления.СостоянияСогласийНаУведомленияОбЭЛН.Подписано);
	Запрос = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ЗапросСФильтрами(Организация, МассивСотрудников, Состояния);
...
&НаСервереБезКонтекста
Функция ОтозватьСогласия(Знач Организация, Знач Сотрудники, Знач Комментарий)
	Состояния = Новый Массив;
	Состояния.Добавить(Перечисления.СостоянияСогласийНаУведомленияОбЭЛН.Подписано);
	Запрос = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ЗапросСФильтрами(Организация, Сотрудники.Массив, Состояния);
	...
	Для Каждого СозданноеРанее Из СозданныеСогласия Цикл
		ДокуменОбъект = Документы.ОтзывСогласияНаУведомлениеОбЭЛН.СоздатьДокумент();
		ДокуменОбъект.Дата = НачалоДня(ТекущаяДатаСеанса());
		ДокуменОбъект.Организация = Организация;
		ДокуменОбъект.Сотрудник = СозданноеРанее.Сотрудник;
		ДокуменОбъект.Ответственный = Ответственный;
		ДокуменОбъект.Комментарий = Комментарий;
		Отказ = Не ДокуменОбъект.ПроверитьЗаполнение();
...
&НаСервереБезКонтекста
Функция МассивСогласийДляУдаления(Знач Организация, Знач МассивСотрудников)
	Запрос = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ЗапросСФильтрами(Организация, МассивСотрудников, Неопределено);
...
&НаСервереБезКонтекста
Функция УдалитьСогласияСотрудников(Знач Организация, Знач МассивСотрудников)
	Состояния = Новый Массив;
	Состояния.Добавить(Перечисления.СостоянияСогласийНаУведомленияОбЭЛН.ОжидаетПодписания);
	Состояния.Добавить(Перечисления.СостоянияСогласийНаУведомленияОбЭЛН.Подписано);
	...
	Для Каждого СозданноеРанее Из СозданныеРанееСогласия Цикл
		...
		КонецЕсли;
		ДокуменОбъект = СозданноеРанее.Согласие.ПолучитьОбъект();
		Если ДокуменОбъект.ПометкаУдаления Тогда
			Продолжить;
		КонецЕсли;
		ДокуменОбъект.УстановитьПометкуУдаления(Истина);
...
Функция ОбновитьСведенияОДействующихСогласияхНаСервере()
	Количество = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ОбновитьСведенияОСогласии(Организация, "*");
	ОбновитьСписокНаСервере();
	Возврат Количество;
КонецФункции
...
Процедура ИзменитьПодписку(ВключитьПодписку, Сотрудники = Неопределено)
	Если Сотрудники = Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	СЭДОФССКлиент.ИзменитьПодписку(ВключитьПодписку, Сотрудники.Массив);
КонецПроцедуры
