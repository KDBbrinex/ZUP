...
// Возвращает таблицу сотрудников организаций, по которым ожидается отключение подписок.
Функция ТребованияПоОтключениюПодписокНаЭЛН() Экспорт
	...
	|		ПО Подписки.Сотрудник = Согласия.Сотрудник
	|			И Подписки.Организация = Согласия.Организация
	|ГДЕ
...
// Возвращает таблицу сотрудников организаций, по которым ожидается включение подписок.
Функция ТребованияПоВключениюПодписокНаЭЛН() Экспорт
	...
	|		ПО Согласия.Сотрудник = Подписки.Сотрудник
	|			И Согласия.Организация = Подписки.Организация
	|ГДЕ
...
//
// Возвращаемое значение:
//   Число - Количество обновленных записей регистра (пар Организация + Сотрудник).
//
Функция ОбновитьСведенияОСогласии(Организация, СотрудникИлиМассивСотрудников) Экспорт
	Если Не ЗначениеЗаполнено(Организация) Тогда
		Возврат 0; // Для пустой организации нет согласий.
	КонецЕсли;
	
	Если ТипЗнч(СотрудникИлиМассивСотрудников) = Тип("Массив") Тогда
		МассивСотрудников = СотрудникИлиМассивСотрудников;
	ИначеЕсли СотрудникИлиМассивСотрудников = "*" Тогда
		МассивСотрудников = СотрудникиОрганизацииВключаяУволенных(Организация);
	ИначеЕсли Не ЗначениеЗаполнено(СотрудникИлиМассивСотрудников) Тогда
		Возврат 0; // Для пустого сотрудника нет согласий.
	Иначе
		МассивСотрудников = Новый Массив;
		МассивСотрудников.Добавить(СотрудникИлиМассивСотрудников);
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Истина);
	
	// Для получения актуальной пары согласие-отзыв фактически строится "срез последних" по согласиям и отзывам,
	// т.е. по каждому сотруднику выбирается последнее согласие и последний отзыв с даты последнего согласия.
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КадровыеДанные.ДатаУвольнения КАК ДатаУвольнения,
	|	КадровыеДанные.ПриказОбУвольнении КАК Ссылка,
	...
	|		ПО Согласия.Сотрудник = КадровыеДанные.Сотрудник
	|ГДЕ
	|	Согласия.Сотрудник В(&МассивСотрудников)
	|	И Согласия.Организация = &Организация
	|	И НЕ Согласия.ПометкаУдаления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПоследниеДатыСогласий.Сотрудник КАК Сотрудник,
	...
	|	ВТВсеСогласия.ФизическоеЛицо КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТПоследниеСогласия
	|ИЗ
	|	(ВЫБРАТЬ
	|		ВТСогласия.Сотрудник КАК Сотрудник,
	...
	|		ПО Подписанные.Сотрудник = ОжидающиеПодписания.Сотрудник
	...
	|		ПО Подписанные.Сотрудник = НеБудутПодписаны.Сотрудник
	|			И (Подписанные.СостояниеДокумента = &Состояние_Подписано)
	...
	|		И ОтзывыСогласий.Организация = &Организация
	|		И ОтзывыСогласий.Проведен
	|		И ОтзывыСогласий.Дата >= ЕСТЬNULL(ВТСогласия.ДатаПодписанного, &ПустаяДата)) КАК ВсеОтзывы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПервыеДатыОтзывовПослеПоследнихСогласий.Сотрудник КАК Сотрудник,
	|	ПервыеДатыОтзывовПослеПоследнихСогласий.Дата КАК Дата,
	|	ВТВсеОтзывы.Ссылка КАК Ссылка,
	|	ВТВсеОтзывы.ФизическоеЛицо КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТОтзывы
	|ИЗ
	|	(ВЫБРАТЬ
	|		Отзывы.Сотрудник КАК Сотрудник,
	...
	|	
	|	СГРУППИРОВАТЬ ПО
	|		Отзывы.Сотрудник) КАК ПервыеДатыОтзывовПослеПоследнихСогласий
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТВсеОтзывы КАК ВТВсеОтзывы
	|		ПО ПервыеДатыОтзывовПослеПоследнихСогласий.Сотрудник = ВТВсеОтзывы.Сотрудник
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(Истина, МассивСотрудников, "ДатаУвольнения, ПриказОбУвольнении");
	ПустаяДата = '00010101';
	МаксимальнаяДата = НачалоДня(ЗарплатаКадрыПериодическиеРегистры.МаксимальнаяДата());
	...
	Для Каждого СтрокаТаблицы Из Таблица Цикл
		...
		Набор = НачатьЗаписьНабора(Организация, СтрокаТаблицы.Сотрудник);
	...
	Для Каждого Сотрудник Из МассивСотрудников Цикл
		Если Таблица.Найти(Сотрудник, "Сотрудник") = Неопределено Тогда
...
Функция СотрудникиОрганизацииВключаяУволенных(Организация)
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
...
Функция ЗапросСФильтрами(Организация, МассивСотрудников, СостоянияСогласий) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Согласия.Сотрудник КАК Сотрудник,
...
Функция ПолучитьПаруСогласиеИОтзывНаДату(Организация, Сотрудник, Дата, ИсключаемоеСогласие = Неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|ГДЕ
	|	ПоследнееСогласиеНаДату.Сотрудник = &Сотрудник
	|	И ПоследнееСогласиеНаДату.Организация = &Организация
	...
	|		ПО (ОтзывСогласия.Сотрудник = &Сотрудник)
	|			И (ОтзывСогласия.Организация = &Организация)
