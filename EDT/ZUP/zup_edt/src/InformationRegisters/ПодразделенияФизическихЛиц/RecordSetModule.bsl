	...
	Для Каждого ЗаписьНабора Из ЭтотОбъект Цикл
		...
		Для Каждого ИмяПланаОбмена Из ПланыОбменаРИБ Цикл
			...
			Если ФизическоеЛицоОбъект = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ОрганизационнаяСтруктура.ОграничитьРегистрациюОбъектаОтборомПоПодразделениям(ИмяПланаОбмена, Отказ, Запрос.Текст, Запрос.Параметры, Истина, ФизическоеЛицоОбъект, ЗаписьНабора.Подразделение);
