...
Процедура ПриОткрытии(Отказ)
	...
	Если ДоступныЭлементыПланирования И Участники.Количество() = 0 Тогда
		...
		НоваяСтрока.Участник = ПользователиКлиент.ТекущийПользователь();
...
Функция РезультатВыбора()
	...
	Если Не ОтклонитьКандидатов Тогда
...
Процедура УстановитьВидимостьРешенияПоКандидатам()
	...
	Если Кандидаты.Количество() = 1 Тогда
		Элементы.ОтклонитьКандидатов.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Отклонить %1'"),
			ПодборПерсонала.СклонениеКандидата(Запись.Кандидат, 4));	
	КонецЕсли;
...
Процедура УстановитьСвойстваПолейРешенияПоКандидатам(Форма)
	...
	Элементы.КомментарийРешения.Доступность = Форма.ОтклонитьКандидатов;
...
Функция ВладелецШаблонов(Форма)
	Возврат ?(Форма.ОтклонитьКандидатов,
			ПредопределенноеЗначение("Перечисление.ИспользованиеШаблоновСообщенийПриПодбореПерсонала.ОтклонениеКандидата"),
			ПредопределенноеЗначение("Перечисление.ИспользованиеШаблоновСообщенийПриПодбореПерсонала.ИзменениеЭтапаРаботыСКандидатом"));
КонецФункции
