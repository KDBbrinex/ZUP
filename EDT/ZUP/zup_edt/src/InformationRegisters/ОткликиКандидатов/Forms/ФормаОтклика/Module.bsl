...
Процедура ЗарегистрироватьКандидата(Команда)
	...
	СозданКандидат = Ложь;
	ИнтеграцияРекрутинговыхСайтовКлиент.СоздатьКандидатаПоРезюме(
		Запись.ИдентификаторРезюме, Запись.Источник, Запись.Вакансия, СозданКандидат);
	...
	ПодборПерсоналаКлиент.ОткрытьФормуНовогоКандидата(, 
		Новый Структура("СтруктураРезюме", ОписаниеКандидата),
		Запись.ИдентификаторРезюме);
...
Процедура ЗаполнитьРезюмеКандидатаПоДаннымОтклика(ОписаниеКандидата)
	...
	Если ЗначениеЗаполнено(Запись.Адрес) Тогда
		...
		Описание.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресМестаПроживанияФизическиеЛица");
	...
	Если ЗначениеЗаполнено(Запись.Телефон) Тогда
		...
		Описание.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.ТелефонМобильныйФизическиеЛица");
	...
	Если ЗначениеЗаполнено(Запись.EMail) Тогда
		...
		Описание.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.EMailФизическиеЛица");
