	...
	Для Каждого ЗаписьНабора Из ЭтотОбъект Цикл
		...
		Для Каждого ИмяПланаОбмена Из ПланыОбменаРИБ Цикл
			...
			Если СотрудникОбъект = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			СинхронизацияДанныхЗарплатаКадрыСервер.ОграничитьРегистрациюОбъектаОтборомПоОрганизациям(ИмяПланаОбмена, Отказ, Запрос.Текст, Запрос.Параметры, Истина, СотрудникОбъект, ЗаписьНабора.Организация);
			...
			Если Получатели.Количество() > 0 Тогда
				...
				СинхронизацияДанныхЗарплатаКадрыСервер.ЗарегистрироватьСвязанныеРегистрыСведенийОбъекта(ИмяПланаОбмена, Отказ, ЗаписьНабора.Сотрудник, Ложь, Получатели);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
