...
Процедура НаборЗаписейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Иначе	
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке, , "НаборЗаписей[" + ИндексТекСтроки + "].Период", , Отказ);
	КонецЕсли;	
	
	Если НЕ Отказ И НЕ ЗначениеЗаполнено(ТекДанные.Состояние) Тогда
		СообщениеОбОшибке = НСтр("ru = 'Состояние не заполнено.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(СообщениеОбОшибке, , "НаборЗаписей[" + ИндексТекСтроки + "].Состояние", , Отказ);
	КонецЕсли;
...
Процедура КомандаОК(Команда)
	...
	Для Каждого ЗаписьНабора Из НаборЗаписей Цикл
		...
		Если ЕстьКонфликт Тогда
			...
			КонецЕсли;			
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "НаборЗаписей[" + НаборЗаписей.Индекс(ЗаписьНабора) + "].Состояние",, Отказ);
		КонецЕсли;
