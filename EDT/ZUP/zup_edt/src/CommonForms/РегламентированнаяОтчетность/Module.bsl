		...
		Если Поле.Имя = "ОтчетыСтатус" Тогда
			...
			Иначе
				...
				Иначе
					...
					Если НЕ ЗначениеЗаполнено(ИмяТипаДокументаРеестрДанныхЭЛН)
						ИЛИ ТипЗнч(Ссылка) <> Тип("ДокументСсылка." + ИмяТипаДокументаРеестрДанныхЭЛН) Тогда
...
Процедура ОтправитьОтчет(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанныеНаименованиеОтчета = "Движение средств по счету в банке за пределами РФ"
			...
			ОписаниеРегОтчета = ОписаниеРегОтчета(ТекущиеДанные.РегламентированныйОтчет);
		...
		Если ТекущиеДанныеНаименованиеОтчета = "Сведения о доле доходов от туристско-рекреационной деятельности" Тогда
			Если ОписаниеРегОтчета(ТекущиеДанные.РегламентированныйОтчет).ВыбраннаяФорма = "ФормаОтчета2018Кв1" Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
		...
		Если ВыполнитьСтандартнуюОтправкуОтчета Тогда
			...
			Если ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
				И ТекущиеДанныеНаименованиеОтчета <> "Подтверждение вида деятельности"
				ИЛИ ТекущиеДанныеНаименованиеОтчета = "Движение средств по счету в банке за пределами РФ"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.дв.ден.б.за пред.РФ") Тогда
...
Процедура ОтправитьУведомления(Команда)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		ИначеЕсли ТекущиеДанные.НеОтправляетсяВКонтролирующийОрган
			...
			ИЛИ ТекущиеДанныеНаименованиеОтчета = "Заявление о гибели или уничтожении объекта налогообложения по транспортному налогу"
				И НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.заяв.о гибели ТС") Тогда
...
Процедура СоздатьСверку(ВидУслуги)
	...
	ОткрытьФорму(ФормаДокумента, ПараметрыФормы, ЭтаФорма, Новый УникальныйИдентификатор);
...
Функция ВыполнениеКомандыОтменено(НаименованиеОтчета, Организация, НаименованиеКоманды = "")
	...
	ИначеЕсли НаименованиеОтчета = Строка(ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаПодключение"))
		ИЛИ НаименованиеОтчета = Строка(ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаОтключение"))
		ИЛИ НаименованиеОтчета = Строка(ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаСертификат")) Тогда
		
		Текст = НСтр("ru = 'Проверка заявления в интернете не предусмотрена'");
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТипЗнч(ПроверяемыйОбъект) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
			...
			Если Представление = "Движение средств по счету в банке за пределами РФ" Тогда
				Если ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2020Кв2" Тогда
					ПоказатьПредупреждение(, НСтр(
					...
					|законодательством. Форма сдается в бумажном виде'"));
					Возврат;
				ИначеЕсли ОписаниеОтчета.ВыбраннаяФорма = "ФормаОтчета2015Кв3" Тогда
					ПоказатьПредупреждение(, НСтр(
					"ru='Проверка в интернете формы отчета
					|в редакции постановления Правительства РФ
					|от 25.07.2015 № 761 не предусмотрена
		...
		ИначеЕсли ТипЗнч(ПроверяемыйОбъект) = Тип("СправочникСсылка.МакетыПенсионныхДел") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Проверка в интернете макетов пенсионных дел не предусмотрена'"));
			Возврат;
			
		ИначеЕсли ТипЗнч(ПроверяемыйОбъект) = Тип("СправочникСсылка.ЗаявлениеОНазначенииПенсии") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Проверка в интернете заявлений о назначении пенсии не предусмотрена'"));
			Возврат;
		...
		Если Представление = "Сведения о доле доходов от туристско-рекреационной деятельности" Тогда
			Если ОписаниеРегОтчета(ПроверяемыйОбъект).ВыбраннаяФорма = "ФормаОтчета2018Кв1" Тогда
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
				"ru='Выгрузка, проверка выгрузки, проверка в интернете, отправка в электронном виде, печать с двухмерным
				|штрихкодом PDF417 формы отчета ""%1"",
				|рекомендованной письмом ФНС России от 16.01.2019 № СД-4-3/441@, не предусмотрены'"), Представление);
				Сообщение.Сообщить();
				Возврат;
			КонецЕсли;
		КонецЕсли;
