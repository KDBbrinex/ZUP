...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы[ИмяТаблицыРезультатовРаспределения + "СтатьяРасходов"].АвтоОтметкаНезаполненного = РаботаВБюджетномУчреждении;
	
	Если Не РаботаВБюджетномУчреждении Тогда
		
		СтатьяОплатаТруда = ОтражениеЗарплатыВБухучетеРасширенный.СтатьяОплатаТруда();
		ТекстПустогоЗначения = "<"+СОКРЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяОплатаТруда, "Код"))+">";
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", ТекстПустогоЗначения);
		ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ПоясняющийТекст);
		ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.НеЗаполнено;
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РаспределениеРезультатов.СтатьяРасходов");
		ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить(); 
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("РаспределениеРезультатаСтатьяРасходов");
		ОформляемоеПоле = ЭлементУсловногоОформления.Поля.Элементы.Добавить(); 
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных("РаспределениеРезультатовСтатьяРасходов");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРаспределяемыйРезультат
...
Процедура РаспределяемыйРезультатПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.РаспределяемыйРезультат.ТекущиеДанные <> Неопределено Тогда
		...
		ИзменятьВидимостьПодразделениеУчетаЗатрат = ТекущиеДанные.ПоказыватьПодразделениеУчетаЗатрат;
		ИзменятьВидимостьСтатьяРасходов = Не ТекущиеДанные.НеПоказыватьСтатьюРасходов;
		...
		Иначе
			...
			ВидимостьПолейАналитикаПодробно = Не ТекущиеДанные.НеПоказыватьСтатьюРасходов
			 И ОбщегоНазначенияКлиентСервер.ЗначениеСвойстваЭлементаФормы(
				Элементы,
				ИмяТаблицыРезультатовРаспределения + "АналитикаПодробно",
				"Пометка");
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
				Элементы,
				ИмяТаблицыРезультатовРаспределения + "СтатьяРасходов",
				"Видимость",
				ВидимостьПолейАналитикаПодробно);
				
		КонецЕсли;
				
		ВидимостьПолейАналитикаПодробно = ТекущиеДанные.ПоказыватьПодразделениеУчетаЗатрат
			 И ОбщегоНазначенияКлиентСервер.ЗначениеСвойстваЭлементаФормы(
				Элементы,
				ИмяТаблицыРезультатовРаспределения + "АналитикаПодробно",
				"Пометка");
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРаспределениеРезультата
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыРаспределениеРезультатов
...
Процедура АналитикаПодробно(Команда)
	...
	Если Не РаботаВБюджетномУчреждении И ИзменятьВидимостьСтатьяРасходов Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			ИмяТаблицыРезультатовРаспределения + "СтатьяРасходов",
			"Видимость",
	...
	Если ИзменятьВидимостьПодразделениеУчетаЗатрат Тогда
	...
	Если ИзменятьВидимостьВидДоходаИсполнительногоПроизводства Тогда
	
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			ИмяТаблицыРезультатовРаспределения + "ВидДоходаИсполнительногоПроизводства",
			"Видимость",
			ВидимостьПолейАналитикаПодробно);	
...
Процедура ЗаполнитьВидОперацииРаспределяемогоРезультата()
	...
	Для каждого Строка Из РаспределяемыйРезультат Цикл
		...
		Если Не РаботаВБюджетномУчреждении Тогда
			Строка.НеПоказыватьСтатьюРасходов = (Строка.ВидОперации = Перечисления.ВидыОперацийПоЗарплате.НачисленоПроцентовПоЗайму);
		КонецЕсли;
...
Процедура УстановитьОтображениеЭлементовФормы()
	...
	ИменаКолонок = "СтатьяФинансирования,Результат";
	Если РаботаВБюджетномУчреждении Тогда
		ИменаКолонок = ИменаКолонок + ",СтатьяРасходов";
	КонецЕсли;
	
	ЕстьПодразделениеУчетаЗатрат = Ложь;
	ИзменятьВидимостьВидДоходаИсполнительногоПроизводства = Ложь;
	Если ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовНачислений" Тогда
	...
	ИначеЕсли ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовУдержаний" И Не БухучетПрочихДоходов Тогда
			ИменаКолонок =  ИменаКолонок + ",Сотрудник,Подразделение";
			ЕстьПодразделениеУчетаЗатрат = Не ВРаспределенииЕстьНДФЛ;
			ИзменятьВидимостьВидДоходаИсполнительногоПроизводства = Истина;
	КонецЕсли;
	...
	ИзменятьВидимостьПодразделениеУчетаЗатрат = ЕстьПодразделениеУчетаЗатрат И ОдинРезультат;
	ИзменятьВидимостьСтатьяРасходов = ОдинРезультат;
	
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Элементы,
			ИмяТаблицыРезультатовРаспределения + "АналитикаПодробно",
			"Видимость",
			ИзменятьВидимостьПодразделениеУчетаЗатрат
			Или ИзменятьВидимостьВидДоходаИсполнительногоПроизводства
			Или ИзменятьВидимостьСтатьяРасходов);
...
Функция ТекстСообщенияРезультатаРаспределения(РаспределяемаяСтрока, ОстатокРаспределения)
	...
	ОстатокРаспределенияАбсолютный = ?(РаспределяемаяСтрока.Результат < 0, - ОстатокРаспределения, ОстатокРаспределения);
		
	Если ОстатокРаспределенияАбсолютный = 0 Тогда
		ТекстСообщения = "";
		
	ИначеЕсли ОстатокРаспределенияАбсолютный > 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='%1 не распределено %2'"),
			РаспределяемаяСтрока.Представление, ОстатокРаспределения);
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
