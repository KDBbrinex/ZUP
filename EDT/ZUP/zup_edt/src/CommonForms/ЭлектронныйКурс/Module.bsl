...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ Отказ И Элементы.ОбластьПросмотра.Видимость И ТипЗнч(Элементы.ОбластьПросмотра.Документ) = Тип("COMОбъект") Тогда
...
Процедура ЗагрузитьФайлыСДискаВБазуПриОткрытии()
	...
	ИзменитьНаименованиеКурса();
	ФайлыДляЗагрузки = ЭлектронноеОбучениеСлужебныйКлиентСервер.ИскатьФайлы(Параметры.ПутьККаталогуСФайламиКурса, "*", Ложь);			
	ЗагрузитьФайлыСДискаВБазу();	
...
&НаКлиенте
Процедура ЗагрузитьФайлыСДискаВБазуПродолжение()
...
Процедура СоздатьВидеоИлиАудиоПродолжение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		СоздатьВидеоИлиАудиоОкончание(Неопределено, НоваяСтрока.Ссылка);
...
Процедура УстановитьЭлементДляПросмотра(ЭлементДляПерехода = Неопределено)	
	...
	ИдентификаторДанныхПубликации = ИзучениеЭлектронныхКурсовСлужебныйКлиентСервер.ИдентификаторДанныхПубликации(ЭлектронныйКурс, ЭлементДляПерехода);
	СтрокаПараметровСсылки = "?" + "date=" + Формат(ОбщегоНазначенияКлиент.ДатаУниверсальная(), "ДФ=ddMMyyyyhhmmss");
	...
	Данные64 = ЭлектронноеОбучениеСлужебныйКлиентСервер.Base64ИзСтроки(
		"{""base"": ""data"", ""storage"":""/"", ""container"":"""+ИдентификаторДанныхПубликации+"""}");
	Данные64 = СтрЗаменить(Данные64, "=", "");
	
	СтрокаПараметровСсылки = СтрокаПараметровСсылки + "&data64=" + Данные64;		
...
Процедура ЗакрытьРедакторЭлемента()
	...
	Если ДокументWordДоступен() Тогда		
		ЗакрытьДокументWord();			
	КонецЕсли;
...
&НаКлиенте
Процедура ПриЗаписиДокумента(ТекущийДокумент, ЭтоИнтерактивнаяЗапись, ОтменаОперации) Экспорт   
...
&НаКлиенте
Процедура ПриЗакрытииДокумента(ТекущийДокумент, ОтменаОперации) Экспорт   
...
&НаКлиенте
Процедура СохранитьПрезентациюPowerPointПослеЗаписиВБазу()	
...
Процедура СохранитьПрезентациюВБазу(ПутьКФайлуПрезентацииКурса)
	...
	ДополнительныеПараметрыСохранения.Вставить("ФайлыДляУдаления", ФайлыДляУдаления);
	ДополнительныеПараметрыСохранения.Вставить("ПутьКФайлуПрезентацииКурса", ПутьКФайлуПрезентацииКурса);
	ДополнительныеПараметрыСохранения.Вставить("ПараметрыЗагрузки", ПараметрыЗагрузки);
	ДополнительныеПараметрыСохранения.Вставить("НастройкиКартинок", НастройкиКартинок);
	
	ОписаниеОповещения = Новый ОписаниеОповещения("СохранитьПрезентациюВБазуПродолжение", ЭтотОбъект, ДополнительныеПараметрыСохранения);

	НачатьПомещениеФайлов(ОписаниеОповещения, ПараметрыЗагрузки.ПередаваемыеФайлы,, Ложь, ПараметрыЗагрузки.УникальныйИдентификатор);	
...
&НаКлиенте
Процедура СохранитьПрезентациюВБазуПродолжение(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	
	ФайлыДляУдаления = ДополнительныеПараметры.ФайлыДляУдаления;
	ПутьКФайлуПрезентацииКурса = ДополнительныеПараметры.ПутьКФайлуПрезентацииКурса;
	ПараметрыЗагрузки = ДополнительныеПараметры.ПараметрыЗагрузки;
	НастройкиКартинок = ДополнительныеПараметры.НастройкиКартинок;
	ПараметрыЗагрузки.ПереданныеФайлы = ПомещенныеФайлы;
	...
	СохранитьПрезентациюPowerPointПослеЗаписиВБазу();
