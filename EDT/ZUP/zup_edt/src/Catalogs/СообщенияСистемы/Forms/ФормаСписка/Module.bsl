...
Процедура УстановитьГруппировкуСписка()
	...
	ПоляГруппировки = Список.КомпоновщикНастроек.Настройки.Структура[0].ПоляГруппировки; // ПоляГруппировкиКомпоновкиДанных
	ГруппировкаПолучатель  = ПоляГруппировки.Элементы[0];
	ГруппировкаОтправитель = ПоляГруппировки.Элементы[1];
...
Процедура УдалитьСообщениеНепосредственно(Знач Сообщения)
	...
	Для Каждого Сообщение Из Сообщения Цикл
		...
		ОбъектСообщения = Сообщение.ПолучитьОбъект(); // СправочникОбъект.СообщенияСистемы
		
		Если ОбъектСообщения <> Неопределено Тогда
			
			ОбъектСообщения.Заблокировать();
			
			Отправитель = ОбъектСообщения.Отправитель;
			Если ЗначениеЗаполнено(Отправитель)
				И Отправитель <> ОбменСообщениямиВнутренний.ЭтотУзел() Тогда
				
				ОбъектСообщения.ОбменДанными.Получатели.Добавить(Отправитель);
