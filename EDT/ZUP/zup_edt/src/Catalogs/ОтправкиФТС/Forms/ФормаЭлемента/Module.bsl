...
Процедура ВыгрузитьКвитанцию(Команда)
	...
	ФайлКвитанции = ПолучитьАдресФайлаКвитанции(Объект.Ссылка);
	Адрес = ФайлКвитанции.Адрес;
	ИмяФайлаВыгрузкиДоТочки = Объект.ИмяФайлаВыгрузки;
	ПозицияТочки = СтрНайти(ИмяФайлаВыгрузкиДоТочки, ".");
	Если ПозицияТочки > 0 Тогда
		ИмяФайлаВыгрузкиДоТочки = Лев(ИмяФайлаВыгрузкиДоТочки, ПозицияТочки - 1);
	...
	ПолучитьФайл(ФайлКвитанции.Адрес,
		НСтр("ru = 'Квитанция'") + ИмяФайлаВыгрузкиДоТочки + "." + ФайлКвитанции.Расширение);
...
Функция ПолучитьАдресФайлаКвитанции(ОтправкаСсылка)
	...
	Если ТипЗнч(СтрКвитанция) = Тип("ДвоичныеДанные") Тогда
		...
		Результат.Расширение = "bin";
		
	Иначе
		Текст = Новый ЗаписьТекста(ВременныйФайл, КодировкаТекста.UTF8);
		Текст.Записать(СтрКвитанция);
		Текст.Закрыть();
		Результат.Расширение = "xml";
	КонецЕсли;
	
	Результат.Адрес = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ВременныйФайл));
