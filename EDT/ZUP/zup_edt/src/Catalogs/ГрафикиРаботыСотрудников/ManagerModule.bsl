...
Функция ЗапросДляПечатиГрафиков(МассивОбъектов, Год)
	...
	|ВЫБРАТЬ
	|	СУММА(ВЫБОР
	|			КОГДА ГрафикиРаботыСотрудников.УчитыватьНерабочиеДни
	|					И (ДанныеПроизводственногоКалендаря.Дата МЕЖДУ ПериодыНерабочихДнейКалендаря.ДатаНачала И ПериодыНерабочихДнейКалендаря.ДатаОкончания)
	|				ТОГДА 0
	...
	|					ИЛИ ДанныеПроизводственногоКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Нерабочий)
	...
	|					ИЛИ ГрафикиРаботыСотрудников.УчитыватьНерабочиеДни
	|						И (ДанныеПроизводственногоКалендаря.Дата МЕЖДУ ПериодыНерабочихДнейКалендаря.ДатаНачала И ПериодыНерабочихДнейКалендаря.ДатаОкончания)
	|				ТОГДА 1
	|			ИНАЧЕ 0
	...
	|	ВидыВремени.ВидУчетаВремени КАК ВидУчетаВремени,
	|	ВЫБОР
	...
	|	ГрафикиРаботыСотрудников.СокращеннаяРабочаяНеделя КАК СокращеннаяРабочаяНеделя,
	|	ГрафикиРаботыСотрудников.НеполныйРабочийДень КАК НеполныйРабочийДень,
	|	ГрафикиРаботыСотрудников.УчитыватьПраздники КАК УчитыватьПраздники,
	|	ГрафикиРаботыСотрудников.НеполноеРабочееВремя КАК НеполноеРабочееВремя,
	|	ГрафикиРаботыСотрудников.Ссылка КАК ГрафикРаботы,
	|	ВЫБОР
	...
	|	ВидыУчетаВремени.ВидУчетаВремени.Наименование КАК ВидУчетаВремениНаименование,
...
Процедура УстановитьПризнакУчитыватьНерабочиеДни(ПараметрыОбновления = Неопределено) Экспорт
	
	Если Не УчетРабочегоВремениРасширенный.ВыделятьНерабочиеДниОтдельнымВидомВремени() Тогда
	...
	Запрос.Текст = 
		...
		|	Справочник.ГрафикиРаботыСотрудников КАК ГрафикиРаботыСотрудников
		|ГДЕ
		|	ГрафикиРаботыСотрудников.УчитыватьНерабочиеДни";
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	Справочник.ГрафикиРаботыСотрудников КАК ГрафикиРаботыСотрудников
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.АвтоматическиОбновляемыеГрафикиРаботыСотрудников КАК АвтоматическиОбновляемыеГрафикиРаботыСотрудников
		|		ПО (АвтоматическиОбновляемыеГрафикиРаботыСотрудников.ГрафикРаботы = ГрафикиРаботыСотрудников.Ссылка)
		|			И (АвтоматическиОбновляемыеГрафикиРаботыСотрудников.Год = &Год)
		|ГДЕ
		|	НЕ ГрафикиРаботыСотрудников.УчитыватьНерабочиеДни";
	...
	Пока Выборка.Следующий() Цикл
		...
		ГрафикОбъект.УчитыватьНерабочиеДни = Истина;
		ОбновлениеИнформационнойБазы.ЗаписатьДанные(ГрафикОбъект);
	КонецЦикла;
