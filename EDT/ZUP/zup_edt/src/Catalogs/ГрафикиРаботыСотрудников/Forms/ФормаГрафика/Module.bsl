...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьПризнакАвтоОбновленияГрафика();
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	УчетРабочегоВремениРасширенный.ЗаписатьДанныеГрафика(ТекущийОбъект.Ссылка, ДанныеГрафика, НомерТекущегоГода, НЕ СреднемесячныеНормыВремениГрафиковРаботыСотрудников.УстановленыВРучную, Ложь);
	СохранитьСреднемесячныеНормыВремениГрафиковРаботыСотрудников(ЭтаФорма);
	СохранитьПризнакАвтоОбновления(ТекущийОбъект.Ссылка);
...
Процедура СреднемесячныеНормыВремениГрафиковРаботыСотрудниковСреднемесячноеЧислоЧасовПриИзменении(Элемент)
	...
	АвтоОбновлениеПоПроизводственномуКалендарю = Ложь;
...
&НаСервере
Процедура УстановитьПризнакАвтоОбновленияГрафика()
	
	АвтоОбновлениеПоПроизводственномуКалендарю = ВозможноАвтоОбновленияГрафика();
	
КонецПроцедуры

&НаСервере
Процедура УстановитьПризнакАвтоОбновленияГрафикаПослеЗаполнения()
	
	АвтоОбновлениеПоПроизводственномуКалендарю = Объект.СпособЗаполнения = Перечисления.СпособыЗаполненияГрафиковРаботыСотрудников.ПоНеделям
		Или Объект.УчитыватьПраздники;
	
КонецПроцедуры

&НаСервере
Процедура УстановитьВидимостьАвтоОбновленияГрафика()
	
	ВидимостьФлага = Объект.СпособЗаполнения = Перечисления.СпособыЗаполненияГрафиковРаботыСотрудников.ПоНеделям
		Или Объект.УчитыватьПраздники;
...
&НаСервере
Функция ВозможноАвтоОбновленияГрафика()
	
	Если Объект.СпособЗаполнения <> Перечисления.СпособыЗаполненияГрафиковРаботыСотрудников.ПоНеделям
		И Не Объект.УчитыватьПраздники Тогда
	...
	Запрос.Текст = 
		...
		|		ПО ГрафикиРаботыПоВидамВремени.ГрафикРаботы = АвтоматическиОбновляемыеГрафикиРаботыСотрудников.ГрафикРаботы
	...
	Запрос.УстановитьПараметр("НачалоГода", Дата(НомерТекущегоГода,1,1));
	Запрос.УстановитьПараметр("КонецГода", Дата(НомерТекущегоГода,12,31));
	...
	Для НомерМесяца = 1 По 12 Цикл
		...
		Для НомерДня = 1 По 31 Цикл
			...
			Если ЧасыПереходящие = 0 Тогда
				ПредставлениеПереходящихЧасов = "";
			Иначе
				ПредставлениеПереходящихЧасов = "(" + ?(Цел(ЧасыПереходящие) = ЧасыПереходящие, Формат(ЧасыПереходящие, "ЧДЦ=; ЧГ="), Формат(ЧасыПереходящие, "ЧДЦ=2; ЧГ=")) + ")";
			КонецЕсли;	
			
			
			СтрокаТаблицыПредставления["ПредставлениеСмена" + НомерДня] = КодТекущейСмены;			
			СтрокаТаблицыПредставления["ПредставлениеДень" + НомерДня] = ПредставлениеЧасов + ПредставлениеПереходящихЧасов;
...
Процедура УстановитьУсловноеОформление()
	Для НомерДня = 1 По 31 Цикл
		...
		ЭлементОформления.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветВыходногоДня);
		...
		ЭлементОтбора.Использование = Истина;
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДанныеГрафика.ВидДня" + НомерДня);
		ЭлементОтбора.ПравоеЗначение = Перечисления.ВидыДнейПроизводственногоКалендаря.Нерабочий;
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура СоздатьСтрокиТаблицыДанныхГрафика()
	Если Объект.ВидыВремени.Количество() = 0 Тогда
		...
		СтрокаВидВремени.ВидВремени = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Явка");
		
		СтрокаВидВремени = Объект.ВидыВремени.Добавить();
	КонецЕсли;	
	ПорядокИспользуемыхВидовВремени = ПорядокИспользуемыхВидовВремени();
	
	ИспользуемыеВидыВремени = ОбщегоНазначения.ВыгрузитьКолонку(Объект.ВидыВремени, "ВидВремени");
	Если УчетРабочегоВремениРасширенный.ВыделятьНерабочиеДниОтдельнымВидомВремени() Тогда
		ИспользуемыеВидыВремени.Добавить(
			ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ОплачиваемыеНерабочиеДни"));
	КонецЕсли;
	
	Для НомерМесяца = 1 По 12 Цикл
		Для Каждого ИспользуемыйВидВремени Из ИспользуемыеВидыВремени Цикл
			ПорядокВидаВремени = ПорядокИспользуемыхВидовВремени.Получить(ИспользуемыйВидВремени);
			ДобавитьСтрокуВТаблицуДанныхГрафика(НомерМесяца, ИспользуемыйВидВремени, ПорядокВидаВремени);	
		КонецЦикла;	
	КонецЦикла;		
	Элементы.ТабличноеПолеГрафикаВидВремени.Видимость = (ИспользуемыеВидыВремени.Количество() > 1);
	Элементы.ТабличноеПолеГрафика.ВысотаВСтрокахТаблицы = 12 * ИспользуемыеВидыВремени.Количество();
КонецПроцедуры	
...
Процедура ПрочитатьДанныеГрафикаВФорму()
	...
	УстановитьВидимостьАвтоОбновленияГрафика();
	УстановитьПризнакАвтоОбновленияГрафика();
...
Процедура ИзменитьСоставВидовВремени()
	...
	Для НомерМесяца = 1 По 12 Цикл
		Для Каждого ИспользуемыйВидВремени Из МассивВидовВремени Цикл
			СтруктураПоиска = Новый Структура("НомерМесяца, ВидВремени", НомерМесяца, ИспользуемыйВидВремени);
			ПорядокВидаВремени = ПорядокИспользуемыхВидовВремени.Получить(ИспользуемыйВидВремени);
			НайденныеСтроки = ДанныеГрафика.НайтиСтроки(СтруктураПоиска);
			Если НайденныеСтроки.Количество() = 0 Тогда
				ДобавитьСтрокуВТаблицуДанныхГрафика(НомерМесяца, ИспользуемыйВидВремени, ПорядокВидаВремени);
	...
	Элементы.ТабличноеПолеГрафикаВидВремени.Видимость = (МассивВидовВремени.Количество() > 1);
...
Функция ПорядокИспользуемыхВидовВремени()
	...
	Если УчетРабочегоВремениРасширенный.ВыделятьНерабочиеДниОтдельнымВидомВремени() Тогда
		ПорядокИспользуемыхВидовВремени.Вставить(
			ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.ОплачиваемыеНерабочиеДни"),
			ПорядокИспользуемыхВидовВремени.Количество() + 1);
	КонецЕсли;
...
Процедура ПолучитьНастройкиЗаполненияГрафика(ОбработкаОповещения) 
	...
	ПараметрыОткрываемойФормы.Вставить("УчитыватьНерабочиеДни", Объект.УчитыватьНерабочиеДни);
...
Процедура ЗаполнитьДанныеОбъектаИзНастроек(Настройки)
	...
	Если Объект.ВидыВремени.Количество() = 0 Тогда
		СтрокаВидаВремени = Объект.ВидыВремени.Добавить();
		СтрокаВидаВремени.ВидВремени = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыИспользованияРабочегоВремени.Явка");
	КонецЕсли;
	...
	Объект.УчитыватьНерабочиеДни = Настройки.УчитыватьНерабочиеДни;
...
Процедура ЗаполнитьГрафикНаСервере()
	...
	УстановитьПризнакАвтоОбновленияГрафикаПослеЗаполнения();
...
&НаСервере
Процедура СохранитьПризнакАвтоОбновления(Ссылка)
	
	НаборЗаписей = РегистрыСведений.АвтоматическиОбновляемыеГрафикиРаботыСотрудников.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ГрафикРаботы.Установить(Ссылка);
	НаборЗаписей.Отбор.Год.Установить(НомерТекущегоГода);
	
	Если АвтоОбновлениеПоПроизводственномуКалендарю Тогда
		НоваяСтрока = НаборЗаписей.Добавить();
		НоваяСтрока.ГрафикРаботы = Ссылка;
