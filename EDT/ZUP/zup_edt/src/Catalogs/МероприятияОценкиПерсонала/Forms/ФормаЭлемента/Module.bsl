...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОткрытыеВопросы
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОценивающиеИндивидуальные
...
Процедура ВопросыХарактеристикПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если НЕ ОтборПоХарактеристикеЗаполнен Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбрана характеристика'"),,,, Отказ);
	КонецЕсли;
...
Процедура ПриПолученииДанныхНаСервере(ЗначениеКопирования = Неопределено)
	...
	АдресВебСсылки = "e1cib/app/" + Метаданные.Обработки.ДоступныеАнкеты.Имя;
	
	СформироватьПодменюВидовEmail("Респонденты");
	СформироватьПодменюВидовEmail("Сотрудники");
	
	ЗаполнитьХарактеристикиИВопросы(ЗначениеКопирования);
...
Процедура УстановитьПараметрыВыбораДляОценивающиеОбщиеРольРеспондента()
	...
	Запрос.УстановитьПараметр("СамооценкаПараметр", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.РолиРеспондентаОценкиПерсонала.Самооценка"));
...
Функция ЗаписатьВариантОтветаНаСервере(ОткрытыйВопрос)
	...
	Исключение
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Анкетирование'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ПодготовитьТекстыПриглашенийИОтправитьАдресатам(МассивСтруктурРеспондентов)
	...
	Для каждого ПриглашаемыйРеспондент Из МассивСтруктурРеспондентов Цикл
		...
		ОценкаПерсонала.ДобавитьПереопределениеПолучателейСообщения(ПередаваемыеПараметры, ПриглашаемыйРеспондент);
		...
		Исключение
			ПриглашаемыйРеспондент.ОтправкаУспешно = Ложь;
			Продолжить;
		КонецПопытки;
...
Функция ПользовательскиеНастройкиОтчетаОценкиПерсонала(УстанавливаемыйОтбор)
	...
	Для Каждого КлючИЗначение Из УстанавливаемыйОтбор Цикл
...
&НаСервере
Процедура ЗаполнитьХарактеристикиИВопросы(ЗначениеКопирования = Неопределено)
	...
	ИсточникХарактеристикИВопросов = ?(ЗначениеКопирования = Неопределено, Объект.Ссылка, ЗначениеКопирования);
	
	Запрос.УстановитьПараметр("Мероприятие", ИсточникХарактеристикИВопросов);
	...
	Пока ВыборкаВопросыХарактеристики.Следующий() Цикл
		Если ВыборкаВопросыХарактеристики.Переопределена И Не ЗначениеЗаполнено(ЗначениеКопирования) Тогда
