...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеШтатнымРасписаниемФормы.УстановитьУсловноеОформлениеСпискаПодразделений(ЭтотОбъект, "Подразделения", "СформированоВПериодеВыбора", "РасформированоВПериодеВыбора");
	...
	Если Не ПоказыватьДоговорниковГПХ И РольСотрудника = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(Параметры.ТекущаяСтрока) Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(Филиал)
				Или ЗначениеЗаполнено(Подразделение) Тогда
...
Процедура ПодразделенияПриАктивизацииСтроки(Элемент)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("АктивацияПодразделенияВФормеВыбораСправочникаСотрудники");	
...
Процедура СписокВыборЗначения(Элемент, Значение, СтандартнаяОбработка)
	...
	ОценкаПроизводительностиКлиент.ЗамерВремени("ПодборСотрудникаВФормеВыбораСправочникаСотрудники");
...
Процедура СписокПриСохраненииПользовательскихНастроекНаСервере(Элемент, Настройки)
	...
	ИдентификаторыПользовательскихОтборов = Новый Соответствие;
	КоллекцииОтборов = Новый Массив;
	...
	Для Каждого КоллекцияОтборов Из КоллекцииОтборов Цикл
		...
		Для Каждого ЭлементОтбора Из КоллекцияОтборов Цикл
			...
			Если ТипЗнч(ЭлементОтбора) = Тип("ЭлементОтбораКомпоновкиДанных")
				...
				ИдентификаторыПользовательскихОтборов.Вставить(ЭлементОтбора.ИдентификаторПользовательскойНастройки, ЭлементОтбора);
	...
	Для каждого ОписаниеПараметра Из КоллекцияИсключаемыхПараметров Цикл
		...
		Для каждого Отборы Из КоллекцииОтборов Цикл
			...
			Для каждого ЭлементОтбора Из Отборы Цикл
				...
				ЭлементНастроки = ИдентификаторыПользовательскихОтборов.Получить(ЭлементОтбора.ИдентификаторПользовательскойНастройки);
				Если ЭлементНастроки <> Неопределено Тогда
					
					Если ЭлементНастроки.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ОписаниеПараметра.Ключ) Тогда
...
Процедура Создать(Команда)
	...
	Если ПользовательскиеОтборы <> Неопределено Тогда
		КоллекцииОтборов.Добавить(ПользовательскиеОтборы);
	КонецЕсли;
