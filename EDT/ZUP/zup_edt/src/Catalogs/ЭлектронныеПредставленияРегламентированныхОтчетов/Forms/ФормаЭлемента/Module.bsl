...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПрорисоватьФайлы(ОбъектовВАрхиве > 0);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если СвойстваФайлов <> Неопределено Тогда
		ТекстСообщения = "";
		КонтекстЭДОСервер = КонтекстЭДОСервер(ТекстСообщения);
...
Процедура ФайлыКомплектаПФРВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ФайлыКомплектаСсылкаПоказать" Тогда	
		Если Элемент.ТекущиеДанные.ВАрхиве Тогда 
			КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(, 17, 3);
		Иначе 
			ОткрытьФайл(Элемент);
		КонецЕсли;		
	КонецЕсли;
...
Процедура ПроверитьВИнтернете(Команда)
	...
	КонтекстЭДОКлиент.ПроверитьВИнтернетеНачало(Объект.Ссылка, Объект.Наименование, Не ЭтоУведомление, Объект.Организация, Объект.ТипПолучателя)
...
Процедура Печать(Команда)
	...
	Если ВТаблицахЕстьАрхивные() Тогда
		КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(, 17, 5);
		Возврат;
	КонецЕсли;
...
Процедура Отправить(Команда)
	...
	Если ВТаблицахЕстьАрхивные() Тогда
		КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(, 17, 2);
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция КонтекстЭДОСервер(ТекстСообщения = "")
...
Функция ПроверитьФайлыВыгрузкиУведомленияОКонтролируемыхСделках(ФайлыОтчетов)
			...
			Пока ФайлXML.ПрочитатьАтрибут() Цикл
				...
				Если ФайлXML.Имя = "ВерсФорм" Тогда
					...
					Если НоваяСтрокаТаблицы.ВерсФорм <> "5.02" И ФайлыОтчетов.Количество() > 1 Тогда
						...
						ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Файлы с версией, отличной от 5.02, не могут быть загружены комплектом'"));
						Возврат Ложь;
	...
	Если ТаблицаПроверкиИдФайлИсх.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='В файлах различаются значения в полях ИдФайлИсх, ИдФайлПерв. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Если КоличествоИзТэгаКолФайл <> КоличествоВыбранныхФайлов Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='В описании к файлам число файлов комплекта не соответствует числу выбранных файлов. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	
	ТаблицаПроверкиКолФайл.Свернуть("КолФайл");
	Если ТаблицаПроверкиКолФайл.Количество() > 1 Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru='В файлах различается значение в поле КолФайл. Загрузка файлов будет прервана'"));
		Возврат Ложь;
	КонецЕсли;
	...
	Для сч = 0 По КоличествоФайлов - 1 Цикл
		...
		Если НомерТекущегоФайла <> сч + 1 Тогда
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru='Не все файлы комплекта выбраны. Загрузка файлов будет прервана'"));
			Возврат Ложь;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗагрузитьЭлектронныеПредставления(МассивФайлов)
	...
	ЗагружаетсяНесколькоФайлов = МассивФайлов.Количество() > 1;
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		ЗагрузитьЭлектронноеПредставление(ВыбранныйФайл, ЗагружаетсяНесколькоФайлов);
	КонецЦикла;
	
КонецПроцедуры

&НаСервере
Процедура ЗагрузитьЭлектронноеПредставление(ВыбранныйФайл, ЗагружаетсяНесколькоФайлов)
	...
	Иначе
		ТипФайлаПФРилиБО = ОпределитьТипФайлаПФРилиБО(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов);
		ОпределитьФайлВТаблицеСвойствФайлов(ВыбранныйФайл.Имя, ТипФайлаПФРилиБО, ВыбранныйФайл.АдресДанных, Ложь);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ОпределитьТипФайлаПФРилиБО(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов)
	...
	// пытаемся определить, является ли файл уведомлением о невозможности предоставления документов в срок и если Да, то каким
	ТипФайлаОтчетностиУведомления = ОпределитьТипФайлаУведомления(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов);
	Если ТипФайлаОтчетностиУведомления <> Неопределено Тогда
		Возврат ТипФайлаОтчетностиУведомления;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция ОпределитьТипФайлаУведомления(ПолноеИмяФайла, ЗагружаетсяНесколькоФайлов)
	
	ИмяФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ПолноеИмяФайла).ИмяБезРасширения;
	ИмяФайлаПриведенное = ВРЕГ(ИмяФайла);
	Если Лев(ИмяФайлаПриведенное, 11) = "ON_UVNPDUS_" Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыФайловБухОтчетности.ОсновнойФайлОтчетности");
	ИначеЕсли ЗагружаетсяНесколькоФайлов И Лев(ИмяФайлаПриведенное, 3) = "PR_" Тогда
		Возврат ПредопределенноеЗначение("Перечисление.ТипыСодержимогоТранспортногоКонтейнера.Приложение");
	КонецЕсли;
...
Функция ОпределитьТипФайлаПФР(ПолноеИмяФайла)
	...
	ИначеЕсли СвойстваУзла.Получить("ОДВ-1") <> Неопределено ИЛИ СвойстваУзла.Получить("ОДВ_1") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-ТД") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_ТД") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("ЗПЭД") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("ЗОЭД") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("УПУП") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	ИначеЕсли СвойстваУзла.Получить("УПРУП") <> Неопределено Тогда
		Возврат Перечисления.ТипыФайловОтчетностиПФР.ПачкаИС;
	Иначе
...
Функция ПроанализироватьФайлВыгрузки(Адрес, КороткоеИмяФайла)
	...
	Если НЕ ЭтоАдресВременногоХранилища(Адрес) Тогда 
		ОбщегоНазначения.СообщитьПользователю("Не удалось открыть файл выгрузки!");
		Возврат Ложь;
	КонецЕсли;
	...
	ИначеЕсли ЭтоФайлВыгрузкиФСС(ПолноеИмяФайла) Тогда
		...
		РезультатАнализа = ПроанализироватьФайлВыгрузкиФСС(ПолноеИмяФайла, Адрес);
...
Функция ПроанализироватьФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("V1_", Справочники.ВидыОтправляемыхДокументов.ОбъемСобранногоВиноградаДляПроизводстваВинодельческойПродукции);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("V2_", Справочники.ВидыОтправляемыхДокументов.ОбъемВиноградаИспользованногоДляПроизводстваВинаИгристогоВинаШампанского);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("V3_", Справочники.ВидыОтправляемыхДокументов.ОбъемВиноградаИспользованногоДляПроизводстваЗащищенныхВин);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("F1_", Справочники.ВидыОтправляемыхДокументов.ОбъемПроизводстваПоставкиФармсубстанцииСпиртаЭтилового);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("F2_", Справочники.ВидыОтправляемыхДокументов.ОбъемИспользованияДляСобственныхНуждФармсубстанцииСпиртаЭтилового);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("F3_", Справочники.ВидыОтправляемыхДокументов.ОбъемПоставкиФармсубстанцииСпиртаЭтилового);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("09_", Справочники.ВидыОтправляемыхДокументов.ОбъемСобранногоВиноградаДляПроизводстваВинодельческойПродукции2019);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("10_", Справочники.ВидыОтправляемыхДокументов.ОбъемВиноградаИспользованногоДляПроизводстваВинаИгристогоВинаШампанского2019);
	СоответствиеПрефиксовВидамОтправляемыхДокументов.Вставить("11_", Справочники.ВидыОтправляемыхДокументов.ОбъемВиноградаИспользованногоДляПроизводстваЗащищенныхВин2019);
...
Функция ОпределитьПериодФСРАР(ДеревоXML)
	...
	Если УзелФормаОтч.Свойства().Получить("ПризПериодОтч") = Неопределено Тогда
		Если УзелФормаОтч.Свойства().Получить("НомФорм") <> Неопределено Тогда
			НомФорм = СокрЛП(УзелФормаОтч.НомФорм);
		Иначе
			НомФорм = Неопределено;
		КонецЕсли;
		
		Если НомФорм = "13" ИЛИ НомФорм = "14" ИЛИ НомФорм = "15"
			ИЛИ НомФорм = "39" ИЛИ НомФорм = "40" ИЛИ НомФорм = "41" Тогда
			ПризПериодОтч = Неопределено;
		Иначе
			Возврат Результат;
		КонецЕсли;
	Иначе
	...
	ИначеЕсли ПризПериодОтч = "0" Тогда
		...
		Результат.Вставить("Периодичность", Перечисления.Периодичность.Квартал);
	ИначеЕсли ПризПериодОтч = Неопределено Тогда
		Результат.Вставить("ДатаНачалаПериода", Дата(ОтчетныйГод, 1, 1));
		Результат.Вставить("ДатаОкончанияПериода", Дата(ОтчетныйГод, 12, 31));
...
Функция ЭтоФайлВыгрузкиФСРАР(ОсновнойФайл)
	...
	ВозможныеПрефиксы.Добавить("V1_");
	ВозможныеПрефиксы.Добавить("V2_");
	ВозможныеПрефиксы.Добавить("V3_");
	ВозможныеПрефиксы.Добавить("F1_");
	ВозможныеПрефиксы.Добавить("F2_");
	ВозможныеПрефиксы.Добавить("F3_");
	ВозможныеПрефиксы.Добавить("09_");
	ВозможныеПрефиксы.Добавить("10_");
	ВозможныеПрефиксы.Добавить("11_");
...
Функция ПроанализироватьФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Если СтрНайти(Префикс, "рсч_") > 0 Тогда
		Если СтрНайти(КороткоеИмяФайла, "_2008_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2009_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2010_") > 0
		ИЛИ СтрНайти(КороткоеИмяФайла, "_2011_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2012_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2013_") > 0
		ИЛИ СтрНайти(КороткоеИмяФайла, "_2014_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2015_") > 0
		ИЛИ СтрНайти(КороткоеИмяФайла, "_2016_1 квартал_") > 0 ИЛИ СтрНайти(КороткоеИмяФайла, "_2016_2 квартал_") > 0 Тогда
			ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.РасчетПлатыОкрСреда;
		Иначе
			ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.ДекларацияПлатаНВОС;
		КонецЕсли;
	...
	ИначеЕсли СтрНайти(Префикс, "2тпа_") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПВоздух;
	ИначеЕсли СтрНайти(Префикс, "2тпр_") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПРекультивация;
	ИначеЕсли СтрНайти(Префикс, "онв_") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.ЗаявлениеОРегистрацииОбъектаНВОС;
	ИначеЕсли СтрНайти(Префикс, "2тпр_") > 0 Тогда
		ВидОтправляемогоДокумента = Справочники.ВидыОтправляемыхДокументов.Форма2ТПРекультивация;
	Иначе
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не удалось определить тип файла выгрузки!'"));
		Возврат Ложь;
	КонецЕсли;
	ПараметрыИмпорта.Вставить("ВидОтчета", ВидОтправляемогоДокумента);
	...
	Если ТипЗнч(ДеревоXML.ORG_INFO) = Тип("СписокXDTO") Тогда
...
Функция ЭтоФайлВыгрузкиРПН(ОсновнойФайл)
	...
	Возврат (СтрНайти(Префикс, "рсч_") > 0
		...
		ИЛИ СтрНайти(Префикс, "2тпр_") > 0
		ИЛИ СтрНайти(Префикс, "2тпа_") > 0);
...
Функция ПроанализироватьФайлВыгрузкиФНС(ПолноеИмяФайла)
	...
	Если Расширение = "xml" Тогда
		...
		Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиРеестраНДС(КороткоеИмяФайла)
			ИЛИ КонтекстЭДОСервер.ЭтоФайлВыгрузкиРеестраАкцизов(КороткоеИмяФайла) Тогда
		...
		Если НЕ ЗначениеЗаполнено(НомКорр) Тогда
			НомКорр = ПоказателиВыгрузки["НомКор"];
		КонецЕсли;
		...
		Иначе 
			...
			Если ГодПериодОтч = Неопределено Тогда
				...
				Если ГодПериодОтч = Неопределено Тогда
					...
					Если ГодПериодОтч = Неопределено Тогда
						ГодПериодОтч = ПоказателиВыгрузки["ФинГод"];
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ГодПериодОтч) ИЛИ НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч) ИЛИ (ПериодОтч <> Неопределено И НЕ ЗначениеЗаполнено(ПериодОтч)) Тогда
				ОтчПерН = ПоказателиВыгрузки["ОтчПерН"];
				Если ЗначениеЗаполнено(ОтчПерН) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ОтчПерН, 1, 2))
					И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ОтчПерН, 4, 2))
					И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Сред(ОтчПерН, 7)) Тогда
					
					ПериодичностьВыгрузки = Перечисления.Периодичность.Квартал;
					ДатаНачалаПериодаОтчетаВыгрузки = Дата(Число(Сред(ОтчПерН, 7)), Число(Сред(ОтчПерН, 4, 2)),
						Число(Сред(ОтчПерН, 1, 2)));
					ДатаНачалаПериодаОтчетаВыгрузки = НачалоКвартала(ДатаНачалаПериодаОтчетаВыгрузки);
					ДатаОкончанияПериодаОтчетаВыгрузки = КонецКвартала(ДатаНачалаПериодаОтчетаВыгрузки);
	...
	ИначеЕсли Расширение = "txt" Тогда
		...
		Если ЧислоДокументов < 1 Тогда
			ОбщегоНазначения.СообщитьПользователю("Файл выгрузки должен содержать выгрузку хотя бы одного отчета!");
			Возврат Ложь;
		ИначеЕсли ЧислоДокументов > 1 Тогда
			ОбщегоНазначения.СообщитьПользователю("Файл выгрузки должен содержать выгрузку только одного отчета!");
			Возврат Ложь;
		КонецЕсли;
		ПоказателиВыгрузки = Новый Соответствие;
	...
	Если СтрНайти(ПолноеИмяФайла, "NO_NDS_") ИЛИ СтрНайти(ПолноеИмяФайла, "NO_NDS.") Тогда
		// декларация НДС 2015
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиФСС(ПолноеИмяФайла, ПреобразуемыйАдрес)
	...
	ЭтоРеестрЭЛН = (Лев(КороткоеИмяФайла, 8) = "ReestrLN"
		ИЛИ СтрНайти(КороткоеИмяФайла, "prParseReestrFile") > 0);
	ЭтоПодтверждениеВидаДеятельности = (Лев(КороткоеИмяФайла, 4) = "POVD");
	...
	ИначеЕсли ЭтоРеестрЭЛН Тогда
		РезультатАнализа = ПроанализироватьФайлВыгрузкиРеестраЭЛН(ПолноеИмяФайла, ПреобразуемыйАдрес);
	ИначеЕсли ЭтоПодтверждениеВидаДеятельности Тогда
		РезультатАнализа = ПроанализироватьФайлВыгрузкиПодтвержденияВидаДеятельности(ПолноеИмяФайла);
	Иначе	
...
Функция ПроанализироватьФайлВыгрузкиОтчет4ФСС(ПолноеИмяФайла)
	...
	Попытка
		...
		ОбъектЧтениеXML.Закрыть();
	Исключение
		ОбщегоНазначения.СообщитьПользователю("Файл """ + КороткоеИмяФайла + """ не является файлом выгрузки в формате XML!");
		Возврат Ложь;
	КонецПопытки;
...
Функция ПроанализироватьФайлВыгрузкиРеестраСведенийФСС(ПолноеИмяФайла)
	...
	ИначеЕсли МассивОрганизаций.Количество() > 1 Тогда	
		ОбщегоНазначения.СообщитьПользователю("Файл """ + КороткоеИмяФайла + """ не может быть загружен, поскольку содержит информацию по нескольким организациям!");
		Возврат Ложь;
	КонецЕсли;
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиРеестраЭЛН(ПолноеИмяФайла, ПреобразуемыйАдрес)
	
	// инициализируем переменные, содержащие двоичные данные и имя файла
	ОбъектФайл = Новый Файл(ПолноеИмяФайла);
	КороткоеИмяФайла = ОбъектФайл.Имя;
	ПолноеИмяСконвертированногоФайла = ПолноеИмяФайла;
	
	// считываем показатели файла
	ОпределятьКодировку = Лев(КороткоеИмяФайла, 17) = "prParseReestrFile";
	Для НомерПопытки = ?(ОпределятьКодировку, 2, 1) По 2 Цикл
		ИмяПоследнегоУзла = Неопределено;
		ОбъектЧтениеXML = Новый ЧтениеXML;
		Попытка
			Если НомерПопытки = 2 Тогда
				КонтекстЭДОСервер = КонтекстЭДОСервер();
				ДвоичныеДанныеФайла = Новый ДвоичныеДанные(ПолноеИмяФайла);
				КодировкаФайла = КонтекстЭДОСервер.ОпределитьКодировку(ДвоичныеДанныеФайла);
				ЧтениеТекстаВыгрузки = Новый ЧтениеТекста(ПолноеИмяФайла, КодировкаФайла);
				ТекстВыгрузки = ЧтениеТекстаВыгрузки.Прочитать();
				ЧтениеТекстаВыгрузки.Закрыть();
				Если Лев(ТекстВыгрузки, 46) = "<?xml version=""1.0"" encoding=""windows-1251"" ?>" Тогда
					ТекстВыгрузки = Сред(ТекстВыгрузки, 47);
					ТекстВыгрузки = СокрЛП(ТекстВыгрузки);
				ИначеЕсли Лев(ТекстВыгрузки, 45) = "<?xml version=""1.0"" encoding=""windows-1251""?>" Тогда
					ТекстВыгрузки = Сред(ТекстВыгрузки, 46);
					ТекстВыгрузки = СокрЛП(ТекстВыгрузки);
				КонецЕсли;
				ПолноеИмяСконвертированногоФайла = ПолучитьИмяВременногоФайла("xml");
				ЗаписьТекстаВыгрузки = Новый ЗаписьТекста(ПолноеИмяСконвертированногоФайла, "utf-8");
				ЗаписьТекстаВыгрузки.Записать(ТекстВыгрузки);
				ЗаписьТекстаВыгрузки.Закрыть();
				ОбъектЧтениеXML.ОткрытьФайл(ПолноеИмяСконвертированногоФайла);
			Иначе
				ОбъектЧтениеXML.ОткрытьФайл(ПолноеИмяФайла);
			КонецЕсли;
			ОбъектЧтениеXML.ИгнорироватьПробелы = Ложь;
			...
			Пока ОбъектЧтениеXML.Прочитать() Цикл
				Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
					ИмяПоследнегоУзла = ОбъектЧтениеXML.Имя;
					Пока ОбъектЧтениеXML.ПрочитатьАтрибут() Цикл
						Если ПоказателиВыгрузки[ОбъектЧтениеXML.Имя] = Неопределено Тогда
							ИмяПоказателя = ОбъектЧтениеXML.Имя;
							ПозицияРазделителя = СтрНайти(ИмяПоказателя, ":");
							Если ПозицияРазделителя > 0 Тогда
								ИмяПоказателя = Сред(ИмяПоказателя, ПозицияРазделителя + 1);
							КонецЕсли;
							ПоказателиВыгрузки.Вставить(ИмяПоказателя, ОбъектЧтениеXML.Значение);
						КонецЕсли;
					КонецЦикла;
					
				ИначеЕсли ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.Текст И ЗначениеЗаполнено(ИмяПоследнегоУзла) Тогда
					ИмяПоказателя = ИмяПоследнегоУзла;
					ПозицияРазделителя = СтрНайти(ИмяПоказателя, ":");
					Если ПозицияРазделителя > 0 Тогда
						ИмяПоказателя = Сред(ИмяПоказателя, ПозицияРазделителя + 1);
					КонецЕсли;
			...
			ОбъектЧтениеXML.Закрыть();
			Если НомерПопытки = 2 Тогда
				ДвоичныеДанныеВыгрузки = Новый ДвоичныеДанные(ПолноеИмяСконвертированногоФайла);
				ПреобразуемыйАдрес = ПоместитьВоВременноеХранилище(ДвоичныеДанныеВыгрузки, УникальныйИдентификатор);
			КонецЕсли;
			Прервать;
		Исключение
			Если НомерПопытки = 2 Тогда
	...
	// определение вида отчета
	ПараметрыИмпорта.Вставить("ВидОтчета", Справочники.ВидыОтправляемыхДокументов.РеестрДанныхЭЛНЗаполняемыхРаботодателем);
	
	// определение организации
	ОрганизацияВыгрузки = Неопределено;
	РегНомФСС = ?(ПоказателиВыгрузки["EMPL_REG_NO"] = Неопределено, "", ПоказателиВыгрузки["EMPL_REG_NO"]);
	Если ЗначениеЗаполнено(РегНомФСС) Тогда
		ОрганизацияВыгрузки = ОпределитьОрганизациюПоКодуФСС(РегНомФСС);
	КонецЕсли;
	...
	// определение органа ФСС
	КодПодчиненности = ПоказателиВыгрузки["EMPL_PARENT_NO"];
	Если КодПодчиненности <> Неопределено Тогда
		ПараметрыИмпорта.Вставить("Получатель", Лев(КодПодчиненности, 4));
	КонецЕсли;
	...
	ПараметрыИмпорта.Вставить("Периодичность", Перечисления.Периодичность.День);
	ПараметрыИмпорта.Вставить("ДатаНачалаПериода", ДатаСеанса);
	ПараметрыИмпорта.Вставить("ДатаОкончанияПериода", ДатаСеанса);
	
	// инициализируем текст на форме
	ЧтениеТекста = Новый ЧтениеТекста(ПолноеИмяСконвертированногоФайла, "windows-1251");
	ПараметрыИмпорта.Вставить("Текст", ЧтениеТекста.Прочитать());
	ЧтениеТекста.Закрыть();
	Если ПолноеИмяСконвертированногоФайла <> ПолноеИмяФайла Тогда
		ОперацииСФайламиЭДКО.УдалитьВременныйФайл(ПолноеИмяСконвертированногоФайла);
	КонецЕсли;
...
&НаСервере
Функция ПроанализироватьФайлВыгрузкиПодтвержденияВидаДеятельности(ПолноеИмяФайла)
	...
	Попытка
		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			...
			ИначеЕсли ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.Текст И ЗначениеЗаполнено(ИмяПоследнегоУзла) Тогда
				ПоказателиВыгрузки.Вставить(ИмяПоследнегоУзла, ОбъектЧтениеXML.Значение);
	...
	// определение вида отчета
	ПараметрыИмпорта.Вставить("ВидОтчета", Справочники.ВидыОтправляемыхДокументов.ПодтверждениеВидаДеятельности);
	
	// определение организации
	ОрганизацияВыгрузки = Неопределено;
	РегНомФСС = ?(ПоказателиВыгрузки["insurerRegNumber"] = Неопределено, "", ПоказателиВыгрузки["insurerRegNumber"]);
	Если ЗначениеЗаполнено(РегНомФСС) Тогда
		ОрганизацияВыгрузки = ОпределитьОрганизациюПоКодуФСС(РегНомФСС);
	КонецЕсли;
	...
	// определение органа ФСС
	КодПодчиненности = ПоказателиВыгрузки["kpsNum"];
	Если КодПодчиненности <> Неопределено Тогда
		ПараметрыИмпорта.Вставить("Получатель", Лев(КодПодчиненности, 4));
	КонецЕсли;
	
	// определение периода
	ГодПериодОтч = ПоказателиВыгрузки["year"];
	Если ЗначениеЗаполнено(ГодПериодОтч) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(ГодПериодОтч) Тогда
		ГодПериодОтчЧислом = Число(ГодПериодОтч);
		ГодПериодОтчДатой = Дата(ГодПериодОтчЧислом, 1, 1);
		ДатаНачалаПериодаОтчетаВыгрузки = НачалоГода(ГодПериодОтчДатой);
		ДатаОкончанияПериодаОтчетаВыгрузки = КонецГода(ГодПериодОтчДатой);
		
		ПараметрыИмпорта.Вставить("Периодичность", Перечисления.Периодичность.Год);
		ПараметрыИмпорта.Вставить("ДатаНачалаПериода", ДатаНачалаПериодаОтчетаВыгрузки);
		ПараметрыИмпорта.Вставить("ДатаОкончанияПериода", ДатаОкончанияПериодаОтчетаВыгрузки);
	КонецЕсли;
	...
	Иначе
		...
		Иначе
			...
			Если СдвигМесяцев = Неопределено Тогда
				ОшибкаОпределенияПериода = Истина;
			Иначе
				НомерПериодаОтчета = Число(?(СтрДлина(ПериодОтчета) = 4, Прав(ПериодОтчета, 2), ПериодОтчета));
...
Функция ОпределитьВидОтчетаПФР(ДеревоXML, ВидКомплектовОтправляемыхДокументов = Неопределено, КороткоеИмяФайла = "")
	...
	ИначеЕсли СвойстваУзла.Получить("СЗВ-СТАЖ") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_СТАЖ") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.СведенияОСтраховомСтажеЗастрахованныхЛицСЗВ_СТАЖ;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-КОРР") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_КОРР") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-ИСХ") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_ИСХ") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.СведенияОЗаработкеСтажеЗастрахованныхЛицСЗВ_ИСХ;
	ИначеЕсли СвойстваУзла.Получить("ОДВ-1") <> Неопределено ИЛИ СвойстваУзла.Получить("ОДВ_1") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.ОписьОДВ_1;
	ИначеЕсли СвойстваУзла.Получить("СЗВ-ТД") <> Неопределено ИЛИ СвойстваУзла.Получить("СЗВ_ТД") <> Неопределено Тогда
		Возврат Справочники.ВидыОтправляемыхДокументов.СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД;
	ИначеЕсли СвойстваУзла.Получить("ЗПЭД") <> Неопределено Тогда
...
Функция ОпределитьОрганизациюПФР(КороткоеИмяФайла, ДеревоXML)
	...
	Если УзелПачкаВходящихДокументов <> Неопределено Тогда
		...
		Для Каждого СвойствоУзла Из СвойстваУзла Цикл
			...
			ИначеЕсли СвойстваТекущегоУзла.Получить("Работодатель") <> Неопределено
				И ТекУзел.Работодатель.Свойства().Получить("РегНомер") <> Неопределено Тогда
				РегистрационныйНомерПФР = СокрЛП(ТекУзел.Работодатель.РегНомер);
			КонецЕсли;
...
Функция ОпределитьПериодПФР(КороткоеИмяФайла, ДеревоXML)
	...
	Для Каждого СвойствоУзла Из СвойстваУзла Цикл
		...
		Если СвойстваТекущегоУзла.Получить("ОтчетныйПериод") <> Неопределено Тогда
			...
			ИначеЕсли СвойстваУзлаОтчетныйПериод.Получить("КалендарныйГод") <> Неопределено
				ИЛИ СвойстваУзлаОтчетныйПериод.Получить("Год") <> Неопределено Тогда
				
				Если СвойстваУзлаОтчетныйПериод.Получить("КалендарныйГод") <> Неопределено Тогда
					СтрКалендарныйГод = СокрЛП(УзелОтчетныйПериод.КалендарныйГод);
				Иначе
					СтрКалендарныйГод = СокрЛП(УзелОтчетныйПериод.Год);
				КонецЕсли;
				...
				КодПериода = 0;
				Если СвойстваУзлаОтчетныйПериод.Получить("Код") <> Неопределено Тогда
					СтрКодПериода = СокрЛП(УзелОтчетныйПериод.Код);
					Если ЗначениеЗаполнено(СтрКодПериода) И СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрКодПериода) Тогда
						КодПериода = Число(СтрКодПериода);
					КонецЕсли;
				КонецЕсли;
				...
				Иначе
					Если КалендарныйГод >= 2014 Тогда
						Если КодПериода = 3 ИЛИ КодПериода = 6 ИЛИ КодПериода = 9 Тогда
							НомерКвартала = КодПериода / 3;
						Иначе
							НомерКвартала = 0;
						КонецЕсли;
					Иначе
						НомерКвартала = КодПериода;
					КонецЕсли;
					
					Если НомерКвартала > 0 И НомерКвартала * 3 <= 12 Тогда
						ДатаНачалаКвартала = НачалоКвартала(Дата(СтрКалендарныйГод, НомерКвартала * 3, 1));
						ДатаОкончанияКвартала = КонецКвартала(ДатаНачалаКвартала);
...
Функция ЭтоФайлВыгрузкиФНС(ОсновнойФайл)
	...
	ИЛИ ВРег(Лев(КороткоеИмяФайла, 3)) = "IU_" И Расширение = "xml"
	ИЛИ ВРег(Лев(КороткоеИмяФайла, 7)) = "1115104" И Расширение = "xml"
	ИЛИ КонтекстЭДОСервер.ЭтоФайлВыгрузкиРеестраНДС(КороткоеИмяФайла)
	ИЛИ КонтекстЭДОСервер.ЭтоФайлВыгрузкиРеестраАкцизов(КороткоеИмяФайла)
	Тогда
		Возврат Истина;
	Иначе
		Возврат Ложь;
...
Функция ЭтоФайлВыгрузкиФСС(ОсновнойФайл)
	...
	Если нрег(КороткоеИмяФайла) = "9999999999_9999.xml" 		// 4аФСС
		...
		ИЛИ Лев(КороткоеИмяФайла, 18) = "E_9999999999_9999_" 	// реестр больничных листов
		ИЛИ Лев(КороткоеИмяФайла, 8) = "ReestrLN" 				// реестр ЭЛН
		ИЛИ СтрНайти(КороткоеИмяФайла, "prParseReestrFile") > 0 // реестр ЭЛН, сторонний файл выгрузки
		ИЛИ Лев(КороткоеИмяФайла, 4) = "POVD" Тогда 			// подтверждение вида деятельности
...
Функция ПолучитьСоответствиеПериодОтчПериодичности()
	...
	СоответствиеПериодОтчПериодичности.Вставить("13", Перечисления.Периодичность.Квартал);
	Если ПериодичностьПолугодиеДоступна Тогда
		СоответствиеПериодОтчПериодичности.Вставить("14", Перечисления.Периодичность.Полугодие);
	КонецЕсли;
	СоответствиеПериодОтчПериодичности.Вставить("15", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("16", Перечисления.Периодичность.Год);
	...
	СоответствиеПериодОтчПериодичности.Вставить("25", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("26", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("27", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("28", Перечисления.Периодичность.Квартал);
	Если ПериодичностьПолугодиеДоступна Тогда
	...
	СоответствиеПериодОтчПериодичности.Вставить("57", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("58", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("59", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("60", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("61", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("62", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("63", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("64", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("65", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("66", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("67", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("68", Перечисления.Периодичность.Год);
	...
	СоответствиеПериодОтчПериодичности.Вставить("83", Перечисления.Периодичность.Квартал);
	Если ПериодичностьПолугодиеДоступна Тогда
		СоответствиеПериодОтчПериодичности.Вставить("84", Перечисления.Периодичность.Полугодие);
	КонецЕсли;
	СоответствиеПериодОтчПериодичности.Вставить("85", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("86", Перечисления.Периодичность.Год);
	СоответствиеПериодОтчПериодичности.Вставить("90", Перечисления.Периодичность.Год);
	СоответствиеПериодОтчПериодичности.Вставить("91", Перечисления.Периодичность.Квартал);
	Если ПериодичностьПолугодиеДоступна Тогда
		СоответствиеПериодОтчПериодичности.Вставить("92", Перечисления.Периодичность.Полугодие);
	КонецЕсли;
	СоответствиеПериодОтчПериодичности.Вставить("93", Перечисления.Периодичность.Квартал);
	СоответствиеПериодОтчПериодичности.Вставить("94", Перечисления.Периодичность.Год);
	СоответствиеПериодОтчПериодичности.Вставить("A1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("A0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("AA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("AB", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("B0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("BA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("BB", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("C0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("CA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("CB", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("D0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("DA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("DB", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("E0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("EA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("EB", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F1", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F2", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F3", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F4", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F5", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F6", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F7", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F8", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F9", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("F0", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("FA", Перечисления.Периодичность.Месяц);
	СоответствиеПериодОтчПериодичности.Вставить("FB", Перечисления.Периодичность.Месяц);
...
Функция ПолучитьСоответствиеПериодОтчМесяцуОкончания()
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("13", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("14", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("15", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("16", 12);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("25", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("26", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("27", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("28", 12);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("57", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("58", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("59", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("60", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("61", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("62", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("63", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("64", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("65", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("66", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("67", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("68", 12);
	...
	СоответствиеПериодОтчМесяцуОкончания.Вставить("83", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("84", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("85", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("86", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("90", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("91", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("92", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("93", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("94", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("A0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("AA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("AB", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("B0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("BB", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("C0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("CA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("CB", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("D0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("DB", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("E0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("EA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("EB", 12);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F1", 1);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F2", 2);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F3", 3);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F4", 4);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F5", 5);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F6", 6);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F7", 7);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F8", 8);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F9", 9);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("F0", 10);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("FA", 11);
	СоответствиеПериодОтчМесяцуОкончания.Вставить("FB", 12);
...
&НаСервере
Процедура ПрорисоватьФайлы(Знач ЕстьФайлыВАрхиве = Ложь)
	
	// определяем, какой файл следует прорисовать
	Если СвойстваФайлов.Количество() = 0 Тогда
		Если ЕстьФайлыВАрхиве Тогда 
			ПредставлениеТекст = "<файл электронного представления был ранее перемещен в архив. Просмотр содержимого невозможен>";
		Иначе
	...
	Иначе
		...
		Если НЕ (КонтекстЭДОСервер.ЭтоФайлВыгрузкиУведомленияОКонтролируемыхСделках(ОсновнойФайл.ИмяФайла) И СвойстваФайлов.Количество() > 1) Тогда
			Если ОсновнойФайл <> Неопределено И Не ОсновнойФайл.ВАрхиве Тогда
				...
				КонецЕсли;
			ИначеЕсли ОсновнойФайл <> Неопределено И ОсновнойФайл.ВАрхиве Тогда
				ПредставлениеТекст = "<файл электронного представления был ранее перемещен в архив. Просмотр содержимого невозможен>";
			Иначе
...
Процедура ОткрытьФайл(Элемент)
	...
	ДополнительныеПараметры = Новый Структура("ДвоичныеДанныеФайла, ИмяФайла, АдресФайла",
		ДвоичныеДанныеФайла, ИмяФайла, Элемент.ТекущиеДанные.АдресДанных);
...
Процедура ОткрытьФайлПослеОтветаНаВопросОБольшомРазмереФайла(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		РасширениеФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайла).Расширение;
		
		Если нрег(РасширениеФайла) = ".txt" ИЛИ нрег(РасширениеФайла) = ".xml" Тогда
...
Процедура ДобавитьФайлыКомплектаНаСервере(МассивФайлов, ЭтоДополнительныеФайлы = Ложь)
	...
	Для Каждого ВыбранныйФайл Из МассивФайлов Цикл
		...
		Если НЕ ЭтоДополнительныеФайлы Тогда
			...
			ИначеЕсли ДокументооборотСКОКлиентСервер.ЭтоВидОтправляемогоДокументаРеестраАкцизов(Объект.ВидОтчета) Тогда
				Если НЕ КонтекстЭДОСервер().ЭтоФайлВыгрузкиРеестраАкцизов(ВыбранныйФайл.Имя) Тогда
					
					ТекстСообщения = СтрШаблон(
						НСтр("ru = 'Файл ""%1"" не может быть отправлен в составе реестра по акцизам.'"),
						ВыбранныйФайл.Имя);
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения);
			...
			НовСтр.Тип = ОпределитьТипФайлаПФРилиБО(ПолноеИмяФайла, Истина);
			НовСтр.СсылкаПоказать = НСтр("ru = 'Открыть'");
...
&НаКлиенте
Функция ВТаблицахЕстьАрхивные()
	
	Для Каждого Стр Из ДополнительныеФайлы Цикл 
		Если Стр.ВАрхиве Тогда 
...
Процедура ФайлыКомплектаПередНачаломДобавления(Отказ, Копирование, ЭтоДополнительныеФайлы)
	...
	Если ВТаблицахЕстьАрхивные() Тогда
		КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(, 17, 6);
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой(Форма)
	...
	ЗагружаетсяНесколькоФайлов = 
		...
		ИЛИ ДокументооборотСКОКлиентСервер.ЭтоВидОтправляемогоДокументаРеестраАкцизов(Объект.ВидОтчета)
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НевозможностьПредоставленияДокументов");
	...
	Если Форма.ТипДокумента = ПредопределенноеЗначение("Перечисление.ТипыОтправляемыхДокументов.ЗаявлениеОВвозеТоваров")
		...
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.УведомлениеСколковоОсвобождениеОтОбязанностейНалогоплательщика")
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.ПодтверждениеВидаДеятельности") Тогда
	...
	ИначеЕсли ЗагружаетсяНесколькоФайлов
		...
		ИЛИ ДокументооборотСКОКлиентСервер.ЭтоВидОтправляемогоДокументаРеестраНДС(Объект.ВидОтчета)
		ИЛИ ДокументооборотСКОКлиентСервер.ЭтоВидОтправляемогоДокументаРеестраАкцизов(Объект.ВидОтчета)
		ИЛИ Объект.ВидОтчета = ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.НевозможностьПредоставленияДокументов")) Тогда
...
Функция ТекстСообщенияОНепройденномКонтролеНомеровКорректировок()
	...
	Если Объект.ВидОтчета = Справочники.ВидыОтправляемыхДокументов.НДС
		И ТипЗнч(Объект.Получатель) = Тип("СправочникСсылка.НалоговыеОрганы") Тогда
		...
		Объект.Организация, Объект.Получатель.Код, Объект.ДатаОкончания, Объект.Версия);
...
Функция ОсновнойИзВыбранныхФайлов(ВыбранныеФайлы)
	...
	Для Каждого ФайлОтчета Из ВыбранныеФайлы Цикл
		...
		ИначеЕсли КонтекстЭДОСервер.ЭтоФайлВыгрузкиРеестраАкцизов(КороткоеИмя)
			И Найти(КороткоеИмя, ".2_") = 0 Тогда
...
Функция ВыбранныеФайлыСНужнымРасширением(ВыбранныеФайлы)
	...
	РазрешеныНетекстовыеВложения = Ложь;
	...
	Для Каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
		...
		Если РасширениеВыбранногоФайла = ".txt"
			...
			ПриведенноеИмяФайла = ВРЕГ(ВыбранныйФайл.Имя);
			РазрешеныНетекстовыеВложения = (Лев(ПриведенноеИмяФайла, 11) = "ON_UVNPDUS_");
	...
	Если РазрешеныНетекстовыеВложения Тогда
		Для каждого ВыбранныйФайл Из ВыбранныеФайлы Цикл
			РасширениеВыбранногоФайла = нрег(ВыбранныйФайл.Расширение);
			
			Если РасширениеВыбранногоФайла <> ".txt"
				И РасширениеВыбранногоФайла <> ".xml" Тогда
...
&НаКлиенте
Процедура ФайлыКомплектаПФРПередНачаломИзменения(Элемент, Отказ)
...
&НаКлиенте
Процедура ФайлыКомплектаУведомленийПередНачаломИзменения(Элемент, Отказ)
