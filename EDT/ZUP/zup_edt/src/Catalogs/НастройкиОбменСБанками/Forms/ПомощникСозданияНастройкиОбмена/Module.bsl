...
Процедура ПутьКФайлуНастроекНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	#Если ВебКлиент Тогда
		...
		НачатьПомещениеФайла(ОписаниеОповещенияОЗавершении, АдресФайлаНастроекОбмена, , , УникальныйИдентификатор);
	#Иначе
...
Функция ЗапускЗаданияПоПолучениюНастроекОбменаНаСервере(Знач УникальныйИдентификатор, Знач ПараметрыПолученияНастроек)
	...
	ПараметрыПолученияНастроек.ИдентификаторСессии = ЭлектронноеВзаимодействиеСлужебныйВызовСервера.СтрокаИзДвоичныхДанных(
...
Процедура СоздатьПрограммыКриптографииНаСервере(Знач ОписанияПрограмм)
	...
	Для Каждого НастройкиПрограммы Из ОписанияПрограмм Цикл
		Если НастройкиПрограммы.ИмяПрограммы = "Microsoft Enhanced Cryptographic Provider v1.0"
			ИЛИ Не НастройкиПрограммы.Установлена Тогда
			Продолжить;
		КонецЕсли;

		ОписаниеПрограммы = ЭлектроннаяПодпись.НовоеОписаниеПрограммы();
		ЗаполнитьЗначенияСвойств(ОписаниеПрограммы, НастройкиПрограммы);
