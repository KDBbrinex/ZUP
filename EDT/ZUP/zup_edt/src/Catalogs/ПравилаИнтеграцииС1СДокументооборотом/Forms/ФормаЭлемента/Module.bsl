...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка)
		И ЗначениеЗаполнено(Параметры.ТипОбъектаИС) Тогда
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаИС) Тогда 
		...
		Для Каждого Строка из ПравилаЗаполненияРеквизитовДО.ПолучитьЭлементы() Цикл
			Если Строка.ОбязательноеЗаполнение 
				И Не ПравилоВыгрузкиЗаполнено(Строка, ПравилаЗаполненияРеквизитовДО) Тогда
				
				Отказ = Истина;
				
				Элементы.ПравилаЗаполненияРеквизитовДО.ТекущаяСтрока
...
Процедура ТипОбъектаИСНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ТипОбъектаИСНачалоВыбораЗавершение", ЭтаФорма);
...
Процедура ТипОбъектаДОНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Оповещение = Новый ОписаниеОповещения("ТипОбъектаДОНачалоВыбораЗавершение", 
		ЭтаФорма, НачальныйТипОбъектаДО);
...
Процедура ПравилаЗаполненияРеквизитовДОПередНачаломИзменения(Элемент, Отказ)
	...
	Если Родитель <> Неопределено
		И Родитель.Вариант = ВариантВыражение() Тогда
...
Процедура ПравилаЗаполненияРеквизитовДОПередУдалением(Элемент, Отказ)
	...
	Если ТекущиеДанные.ИмяРеквизитаОбъектаДО = "documentType"
		И ТекущиеДанные.ЗаполненВШаблоне Тогда
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПравилаЗаполненияРеквизитовДОПередУдалениемЗавершение", ЭтаФорма, ТекущиеДанные);
...
Процедура ПравилаЗаполненияРеквизитовИСПередНачаломИзменения(Элемент, Отказ)
	...
	Оповещение = Новый ОписаниеОповещения("ПравилаЗаполненияРеквизитовИСПередНачаломИзмененияЗавершение", ЭтаФорма, ТекущиеДанные);
	
	ОткрытьФорму(ОткрываемаяФорма, ПараметрыФормы, ЭтаФорма,,,, Оповещение, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПравилаЗаполненияРеквизитовИСПередУдалением(Элемент, Отказ)
	...
	Оповещение = Новый ОписаниеОповещения(
		"ПравилаЗаполненияРеквизитовИСПередУдалениемЗавершение", ЭтаФорма, ТекущиеДанные);
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда 
		...
		Если ТекущиеДанные.ЭтоТаблица Тогда // очистим подчиненные, переставшие соответствовать
			...
			Для Каждого ПодчиненноеПравило Из ПодчиненныеПравила Цикл
				...
				Если ТекущиеДанные.Вариант = ВариантРеквизит()
					И ПодчиненноеПравило.Вариант = ВариантРеквизит() Тогда
					Если СтрНайти(ПодчиненноеПравило.ИмяРеквизитаОбъектаИС, ".") <> 0
						И Не СтрНачинаетсяС(ПодчиненноеПравило.ИмяРеквизитаОбъектаИС, ТекущиеДанные.ИмяРеквизитаОбъектаИС) Тогда
...
Процедура ЗаполнитьСписокПечатныхФорм()
	...
	Если НЕ ЗначениеЗаполнено(Объект.ТипФайловСохраненияПечатныхФорм) Тогда
		Объект.ТипФайловСохраненияПечатныхФорм = ИнтеграцияС1СДокументооборот.ТипФайлаСохраняемойПечатнойФормыПоУмолчанию();
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ТипОбъектаИС) Тогда 
		...
		ТаблицаПечатныхФорм = ИнтеграцияС1СДокументооборот.МенеджерыПечати(Объект.ТипОбъектаИС);
	...
	Элементы.ГруппаПрисоединяемыеПечатныеФормы.Видимость =
...
Процедура ЗаполнитьРеквизитыДокументооборота(ПриВыбореШаблона = Ложь)
	...
	Иначе
		...
		СтрокиВидДокумента = Объект.ПравилаЗаполненияРеквизитовДО.НайтиСтроки(ПараметрыОтбора);
		...
		ИначеЕсли РеквизитыШаблона <> Неопределено Тогда // Вид документа может быть указан и только в шаблоне.
			...
			ПараметрыОтбора.Удалить("Вариант");
		...
		Реквизиты = Справочники.ПравилаИнтеграцииС1СДокументооборотом.
			ПолучитьРеквизитыОбъектаДО(Объект.ТипОбъектаДО, ПараметрыЗаполнения);
		...
		Для Каждого Строка Из Реквизиты Цикл
			...
			Если РеквизитыШаблона <> Неопределено Тогда
				...
				Если НайденныеСтроки.Количество() > 0 Тогда // заполнен в шаблоне
					...
					НоваяСтрока.Обновлять = НоваяСтрока.Обновлять
						И Не Объект.ШаблонЗапрещаетИзменение;
						
					Если ПриВыбореШаблона 
						И НоваяСтрока.Вариант <> ВариантШаблон() Тогда
					...
					Иначе
						...
						Если НоваяСтрока.Вариант =
							ВариантЗначение() Тогда // предупредим о конфликте
...
Процедура ЗаполнитьРеквизитыПотребителя()
	...
	Реквизиты = Справочники.ПравилаИнтеграцииС1СДокументооборотом.
			ПолучитьРеквизитыОбъектаИС(Объект.ТипОбъектаИС);
...
Процедура ЗаполнитьПоУмолчаниюПриВыгрузкеНаСервере(НачальноеЗаполнение = Ложь)
	...
	Если СтрНайти(Объект.ТипОбъектаДО, Объект.ШаблонТип) = 0 Тогда
		Объект.Шаблон = "";
		Объект.ШаблонID = "";
		Объект.ШаблонТип = "";
	...
	Если СтрокиВидаДокумента.Количество() > 0 Тогда
		...
		Запрос.type = Объект.ТипОбъектаДО + "Type";
		Запрос.query = СписокУсловий;
		
		Результат = Прокси.execute(Запрос);
		ИнтеграцияС1СДокументооборот.ПроверитьВозвратВебСервиса(Прокси, Результат);
		
		Если Результат.items.Количество() = 1 И Не ЗначениеЗаполнено(СтрокаПравил.ЗначениеРеквизитаДОТип) Тогда
		...
		Если ЗначениеЗаполнено(СтрокаПравил.ЗначениеРеквизитаДОТип) И ДоступенФункционалКонтрагенты Тогда
			ЗапросЗначенийРеквизитов = ИнтеграцияС1СДокументооборот.СоздатьОбъект(Прокси,
				"DMGetDefaultValuesByDocumentTypeRequest");
			ВидXDTO = ИнтеграцияС1СДокументооборот.СоздатьObjectID(Прокси,
				СтрокаПравил.ЗначениеРеквизитаДОID,СтрокаПравил.ЗначениеРеквизитаДОТип);
			...
			Для Каждого СтрокаПравил Из ДеревоПравилВыгрузки.Строки Цикл
				Если ЗначениеЗаполнено(СтрокаПравил.ИмяРеквизитаОбъектаДО)
					И СвойстваОбъекта.Получить(СтрокаПравил.ИмяРеквизитаОбъектаДО) <> Неопределено Тогда
	...
	СтрокиГрифаДоступа = ДеревоПравилВыгрузки.Строки.НайтиСтроки(Новый Структура("ИмяРеквизитаОбъектаДО", "accessLevel"));
	
	Если СтрокиГрифаДоступа.Количество() > 0 И Не ЗначениеЗаполнено(СтрокиГрифаДоступа[0].ЗначениеРеквизитаДО) Тогда
...
Процедура ОчиститьПравилоЗаполненияПриВыгрузке(Правило)
	...
	Правило.ЗначениеРеквизитаДОID = Неопределено;
	Правило.ЗначениеРеквизитаДОТип = Неопределено;
	Правило.ВычисляемоеВыражение = Неопределено;
	Правило.ЗаполненВШаблоне = Ложь;
	Правило.ШаблонЗначение = Неопределено;
	Правило.ШаблонID = Неопределено;
	Правило.ШаблонТип = Неопределено;
	Правило.ШаблонПредставление = Неопределено;
	Правило.ЭтоДополнительныйРеквизитДО = Неопределено;
	Правило.ДополнительныйРеквизитДОID = Неопределено;
	Правило.ДополнительныйРеквизитДОТип = Неопределено;
...
Процедура ОчиститьПравилоЗаполненияПриЗагрузке(Правило)
	...
	Правило.Вариант = 
		ПредопределенноеЗначение("Перечисление.ВариантыПравилЗаполненияРеквизитов.ПустаяСсылка");
	Правило.ИмяРеквизитаОбъектаДО = Неопределено;
	Правило.ПредставлениеРеквизитаОбъектаДО = Неопределено;
	Правило.ЗначениеРеквизитаИС = Неопределено;
	Правило.ВычисляемоеВыражение = Неопределено;
	Правило.ЭтоДополнительныйРеквизитДО = Неопределено;
	Правило.ДополнительныйРеквизитДОID = Неопределено;
	Правило.ДополнительныйРеквизитДОТип = Неопределено;
	Правило.ЭтоДополнительныйРеквизитИС = Неопределено;
	Правило.ДополнительныйРеквизитИС = Неопределено;
	Правило.Ключевой = Ложь;
	Правило.Обновлять = Ложь;
	Правило.Картинка = 0;
...
Процедура ПроверитьПравилаЗаполненияРеквизитовПриВыгрузке(Отказ, ТипЗначенияРеквизита, ИдентификаторЗначенияРеквизита,
	...
	Если Не ЗаполненВШаблоне
		...
		ОбъектыXDTO = ИнтеграцияС1СДокументооборот.ПолучитьОбъект(
			ИнтеграцияС1СДокументооборотПовтИсп.ПолучитьПрокси(),
			ТипЗначенияРеквизита,
			ИдентификаторЗначенияРеквизита);
		Для Каждого ВидДокумента Из ОбъектыXDTO.objects Цикл
			Если ИнтеграцияС1СДокументооборот.СвойствоУстановлено(ВидДокумента, "templateRequired")
