...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Продукт         = ОбработкаНовостейПовтИсп.ПолучитьЗначениеПредопределеннойКатегории("ИмяКонфигурации"); // Только имя продукта.
	...
	ЭтотОбъект.КатегорииСПроизвольнымиУсловиями.ЗагрузитьЗначения(ЛокальныйСписокКатегорийТипаЧислоИлиДата.ВыгрузитьЗначения());
	...
	Если ИмяСобытия = "БуферОбмена: изменился состав сохраненных" Тогда
		...
		Если ТипЗнч(Источник) = ТипСтруктура Тогда
			Если (Источник.Свойство("Ссылка") = Истина)
				...
				Если СтрНайти(ВРег(Источник.КлючОбъекта), ВРег("Документы.Новости.")) = 1 Тогда
					ЛокальныйТекущийБуферОбмена = Прав(Источник.КлючОбъекта, СтрДлина(Источник.КлючОбъекта) - СтрДлина("Документы.Новости."));
...
Процедура ПримерНовостиДляВнешнихИсточниковПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ОбработкаНовостейКлиент.ОбработкаНажатияВТекстеНовости(ЭтотОбъект.Объект, ДанныеСобытия, СтандартнаяОбработка, ЭтотОбъект, Элемент);
...
Процедура Подключаемый_КомандаБуферОбмена_Копировать(Команда)
	...
	Если ВРег(Команда.Имя) = ВРег("КомандаБуферОбмена_Копировать_КатегорииПростые")
		...
		Иначе
			ПоказатьОповещениеПользователя(НСтр("ru='Выделите хотя бы одну строку'"), , , БиблиотекаКартинок.НовостиВнимание);
		КонецЕсли;
				...
				#Если НЕ ВебКлиент Тогда
					...
					Попытка
						...
						Если НЕ ПустаяСтрока(ПараметрыСхлопывающегосяБлока.УИНБлока) Тогда
							...
							Для С=1 По 5 Цикл
								ЭлементХТМЛТекст = ДокументХТМЛ.СоздатьТекстовыйУзел(НСтр("ru='Строка'") + " " + С + Символы.ПС);
...
Процедура СоздатьРеквизитыТекущегоБуфераОбмена(МассивБуферовОбмена)
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		НайденныеРеквизиты.Вставить("Найден_" + НайденныеРеквизиты, Ложь);
	КонецЦикла;
	...
	Для каждого ТекущийРеквизит Из ВсеРеквизиты Цикл
		Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
			Если ВРег(ТекущийРеквизит.Имя) = ВРег("ТекущийБуферОбмена_" + ТекущийБуферОбмена) Тогда
				НайденныеРеквизиты.Вставить("Найден_" + НайденныеРеквизиты, Истина);
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		Если НайденныеРеквизиты.Получить("Найден_" + НайденныеРеквизиты) = Истина Тогда
	...
	Для каждого ТекущийБуферОбмена Из МассивБуферовОбмена Цикл
		...
		Пока БылиУдаления = Истина Цикл
			...
			Для каждого ТекущаяКоманда Из ЭтотОбъект.Команды Цикл
				Если СтрНайти(ВРег(ТекущаяКоманда.Имя), ВРег("КомандаБуферОбмена_Вставить_СохраненныйБуферОбмена_" + ТекущийБуферОбмена + "_")) = 1 Тогда
...
Процедура ПослеЗагрузкиВнедреннойКартинки(Результат, АдресВХранилище, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат = Истина Тогда
		ЛокальныеДвоичныеДанныеФайла = ПолучитьИзВременногоХранилища(АдресВХранилище);
...
Функция ПолучитьПредставлениеНовостиXML()
	...
	ПредставлениеНовостиХМЛ = ИнтернетПоддержкаПользователей.ПредставлениеОбъектаХДТО(ОбъектХДТО, ФабрикаХДТО);
