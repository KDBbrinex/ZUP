...
Процедура ПередЗаписью(Отказ)
	...
	ИначеЕсли (СокрЛП(ВРег(УИННовости)) = ВРег("313f4928-a71c-4cef-96ee-a4b6106c1275"))
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru='Новости. Изменение данных. Новость. Перед записью'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
Процедура ПриЗаписи(Отказ)
	...
	// Здесь же: записать признак ТребуетсяПересчетОтборов, если его явно передали.
	УникальныйИдентификаторСсылки = СтрЗаменить(СокрЛП(Ссылка.УникальныйИдентификатор()), "-", "");
	ТребуетсяПересчетОтборов = Ложь;
	Если (ДополнительныеСвойства.Свойство("ТребуетсяПересчетОтборов"))
			И (ДополнительныеСвойства.ТребуетсяПересчетОтборов = Истина) Тогда
		ТребуетсяПересчетОтборов = Истина;
	КонецЕсли;
	Запись = РегистрыСведений.СвойстваНовостей.СоздатьМенеджерЗаписи();
		...
		Запись.ТребуетсяПересчетОтборов      = ТребуетсяПересчетОтборов;
