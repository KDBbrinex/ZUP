...
Процедура ОКПДТРКодНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.Должности.Форма.ВыборОКПДТР", СтруктураПараметров, ЭтотОбъект, , , , Оповещение, 
...
&НаКлиенте
Процедура ОКЗКодНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Оповещение = Новый ОписаниеОповещения("ПослеДобавленияОКЗИзКлассификатора", ЭтотОбъект);
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ОКЗКод", Объект.ОКЗКод);
	
	ОткрытьФорму("Обработка.КлассификаторОКЗ.Форма.ВыборОКЗ", СтруктураПараметров, ЭтотОбъект, , , , Оповещение, 
		РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ПослеДобавленияКодовИзКлассификатора(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		...
		Объект.ОКЗКод			 = Результат.ОКЗКод;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ПослеДобавленияОКЗИзКлассификатора(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		Объект.ОКЗКод			 = Результат.ОКЗКод;
	КонецЕсли;
КонецПроцедуры
