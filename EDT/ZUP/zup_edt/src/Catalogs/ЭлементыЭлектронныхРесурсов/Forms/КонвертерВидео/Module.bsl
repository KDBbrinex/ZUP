...
Процедура ТаблицаФайловВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.ЭлементыЭлектронныхРесурсов.Форма.КонвертерВидео", Новый Структура("ЭлементРесурса", ТекущаяСтрока.ЭлементРесурса), ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура СформироватьЗадания(Формат = Неопределено, ВыделенныеСтроки = Неопределено)
	...
	Иначе
		...
		ОткрытьФорму("Справочник.ЭлементыЭлектронныхРесурсов.Форма.НастройкиКонвертацииВидео",, ЭтотОбъект,,,,ОписаниеОповещенияВыбора);
	КонецЕсли;
...
Процедура УдалитьФорматыНаСервере(Знач ВыбранныеФорматы, Знач ВыбранныеФайлы)
	...
	Для каждого СвойстваФайла Из ВыбранныеФайлы Цикл	
		...
		Попытка
		    ...
		    ЭлементБлокировкиДанных.УстановитьЗначение("Ссылка", СвойстваФайла.ЭлементРесурса);
...
Процедура ПоискИнформацииОДлительности(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ТекстовыйДокумент = ЭлектронноеОбучениеСлужебныйКлиентСервер.ПрочитатьТекстовыйДокумент(НайденныеФайлы[0].ПолноеИмя, Ложь);
...
Процедура ОбновитьПрогрессКонвертации(СкопированныйФайл, ДополнительныеПараметры) Экспорт
	...
	ТекстовыйДокумент = ЭлектронноеОбучениеСлужебныйКлиентСервер.ПрочитатьТекстовыйДокумент(СкопированныйФайл);
	
	Если ТекстовыйДокумент = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось прочитать прогресс конвертации'");
	КонецЕсли;
...
Процедура ЗапуститьПриложениеКонвертации(ПараметрыКонвертации, РабочийКаталог, ОписаниеОповещения)
	...
	Если ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
		...
		ФайловаяСистемаКлиент.ЗапуститьПрограмму("sudo chmod +x " + РабочийКаталог + ПрограммаКонвертации.ИмяФайла, ПараметрыЗапускаПрограммы);
	КонецЕсли;
	
	ПараметрыЗапускаПрограммы = ФайловаяСистемаКлиент.ПараметрыЗапускаПрограммы();
	ПараметрыЗапускаПрограммы.ТекущийКаталог = РабочийКаталог;
	...
	ФайловаяСистемаКлиент.ЗапуститьПрограмму(РабочийКаталог + ПрограммаКонвертации.ИмяФайла + ПараметрыКонвертации, ПараметрыЗапускаПрограммы);
	...
	Для каждого Задание Из Задания Цикл
		...
		Если Задание.Выполняется Тогда
			...
			Если (ТипПлатформыНаКлиенте = ТипПлатформы.Windows_x86) ИЛИ (ТипПлатформыНаКлиенте = ТипПлатформы.Windows_x86_64) Тогда				
				...
				ПараметрыЗапускаПрограммы = ФайловаяСистемаКлиент.ПараметрыЗапускаПрограммы();
				ПараметрыЗапускаПрограммы.ТекущийКаталог = Задание.РабочийКаталог;
				ПараметрыЗапускаПрограммы.ДождатьсяЗавершения = Ложь;
				ПараметрыЗапускаПрограммы.ПолучитьПотокВывода = Ложь;
				ПараметрыЗапускаПрограммы.ПолучитьПотокОшибок = Ложь;
				
				ФайловаяСистемаКлиент.ЗапуститьПрограмму("taskkill /IM ffmpeg.exe /F", ПараметрыЗапускаПрограммы);
...
Функция ОписаниеПрограммыКонвертации(ТипПлатформыКлиента, РежимОтладки)
	...
	Если (ТипПлатформыКлиента = ТипПлатформы.Windows_x86) ИЛИ (ТипПлатформыКлиента = ТипПлатформы.Windows_x86_64) Тогда
		...
		СвойстваПрограммы.Код = "КомпонентаКонвертерВидеоДляWindows";
	...
	Если (ТипПлатформыКлиента = ТипПлатформы.Linux_x86) ИЛИ (ТипПлатформыКлиента = ТипПлатформы.Linux_x86_64) Тогда
		...
		СвойстваПрограммы = ШаблонСвойствПрограммыДляКонвертации();
		СвойстваПрограммы.Код = "КомпонентаКонвертерВидеоДляLinux";
	...
	Если (ТипПлатформыКлиента = ТипПлатформы.MacOS_x86) ИЛИ (ТипПлатформыКлиента = ТипПлатформы.MacOS_x86_64) Тогда
		...
		СвойстваПрограммы = ШаблонСвойствПрограммыДляКонвертации();
		СвойстваПрограммы.Код = "КомпонентаКонвертерВидеоДляMac";
		СвойстваПрограммы.ИмяФайла = ИмяПрограммыКонвертации;
		СвойстваПрограммы.РежимОтладки = РежимОтладки;
