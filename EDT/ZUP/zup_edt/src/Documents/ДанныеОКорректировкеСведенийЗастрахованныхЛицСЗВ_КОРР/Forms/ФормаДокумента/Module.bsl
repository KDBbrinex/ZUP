...
Процедура ОтчетныйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ПерсонифицированныйУчетКлиент.ПериодОтчетаРегулирование(Объект.ОтчетныйПериод, Объект.ОкончаниеОтчетногоПериода, ПериодСтрока, Направление, '20170101', '20171231');
...
Процедура КорректируемыйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	МинимальныйПериодОкончание = ?(ИзменениеБезОграничений, '00010101', '20171231');
	ПерсонифицированныйУчетКлиент.ПериодОтчетаРегулирование(Объект.КорректируемыйПериод, Объект.ОкончаниеКорректируемогоПериода, КорректируемыйПериодСтрока, Направление, МинимальныйПериод, МинимальныйПериодОкончание);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыНачисленныеУплаченныеВзносы
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыУплаченныеВзносыРасшифровка
...
Процедура ПриПолученииДанныхНаСервере()
	...
	ДанныеКорректируемогоПериода = ПерсонифицированныйУчетКлиентСервер.ДанныеОтчетногоПериода(Объект.КорректируемыйПериод, Объект.ОкончаниеКорректируемогоПериода);
	КорректируемыйПериодСтрока = ДанныеКорректируемогоПериода.Представление;
...
Процедура СотрудникиСотрудникПриИзмененииНаСервере()
	...
	ОкончаниеПериода = КонецДня(Объект.ОкончаниеКорректируемогоПериода);
...
Процедура ЗаполнитьДанныеОСтажеСотрудников(Сотрудники)
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Документы.ДанныеОКорректировкеСведенийЗастрахованныхЛицСЗВ_КОРР.СоздатьВТДанныеСтажаПФР(МенеджерВременныхТаблиц,
		Объект.Организация,  Объект.КорректируемыйПериод, Объект.ОкончаниеКорректируемогоПериода, СписокСотрудников, Объект.Ссылка);
	...
	РеквизитыСтроки = ОбщегоНазначения.СкопироватьРекурсивно(ПроверяемыеРеквизиты);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ПредыдущаяСтрока <> Неопределено 
				И КонецДня(ПредыдущаяСтрока.ДатаОкончанияПериода) + 1 = Выборка.ДатаНачалаПериода
				И ПерсонифицированныйУчет.ДанныеСтрокСовпадают(ПредыдущаяСтрока, Выборка, ПроверяемыеРеквизиты) Тогда 
				ПредыдущаяСтрока.ДатаОкончанияПериода = Выборка.ДатаОкончанияПериода;
				Продолжить;
			КонецЕсли;
			ПредыдущаяЗапись = ПредыдущаяОсновнаяЗапись();
			Если ПредыдущаяЗапись <> Неопределено 
				И КонецДня(ПредыдущаяЗапись.ДатаОкончанияПериода) + 1 = Выборка.ДатаНачалаПериода
