...
Функция РезультатЗапросаДанныхДляПечатиЗаявлений(МассивСсылок)
	...
	|	ЕСТЬNULL(СведенияОбЭЛН.Хеш, """") <> """" КАК ЭтоЭЛН
	|ИЗ
	|	Документ.ЗаявлениеСотрудникаНаВыплатуПособия КАК Заявление
...
Процедура ЗаполнитьВМакетеФИОЗаявителя(Макет, Заявление)
	...
	ФИО = ВРег(Заявление.ФамилияЗаявителя + " " + Заявление.ИмяЗаявителя + " " + Заявление.ОтчествоЗаявителя);
	ФИОВРодительномПадеже = "";
	ФизическиеЛицаЗарплатаКадры.Просклонять(ФИО, 2, ФИОВРодительномПадеже, , Заявление.ФизическоеЛицо);
...
Функция СтруктураАдресаВМашиночитаемомФормате_2012(Адрес)
	Результат = Новый Структура("Индекс, Регион, Район, НаселенныйПункт, Улица, Дом, Корпус, Строение, Квартира");
	...
	Если ТипЗнч(Корпуса) = Тип("Массив") Тогда
		Для Каждого Корпус Из Корпуса Цикл
			Если СтрСравнить(Корпус.ТипКорпуса, "СТРОЕНИЕ") = 0 Тогда
				Если Не ЗначениеЗаполнено(Результат.Строение) Тогда
					Результат.Строение = Корпус.Номер;
				КонецЕсли;
			ИначеЕсли СтрСравнить(Корпус.ТипКорпуса, "ЛИТЕР") = 0
	...
	КонецЕсли;
	Если Литера <> "" Тогда
		Если Результат.Корпус = "" И Результат.Строение <> "" Тогда
			Результат.Строение = Результат.Строение + Литера;
		Иначе
