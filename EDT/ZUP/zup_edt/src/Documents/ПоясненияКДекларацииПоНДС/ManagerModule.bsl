...
Функция ПредставлениеПериодаОтраженияЗаписи(СтрокаТаблицы) Экспорт
	
	ПредставлениеПериода = Новый Массив;
	Если ЗначениеЗаполнено(СтрокаТаблицы.ПериодОтраженияЗаписи) Тогда
		
		ПериодОтраженияЗаписи = СтрокаТаблицы.ПериодОтраженияЗаписи;
		
		Если СтрЧислоВхождений(ПериодОтраженияЗаписи, ";") = 1 
			И СтрЧислоВхождений(ПериодОтраженияЗаписи, "_") = 0 Тогда
			ПериодОтраженияЗаписи = СтрЗаменить(ПериодОтраженияЗаписи, ";", "_");
		КонецЕсли;
		
		МассивПериодов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПериодОтраженияЗаписи, ";", Истина);
		
		Для каждого ПериодИзМассива Из МассивПериодов Цикл
			
			ЧастиПериода = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПериодИзМассива, "_", Истина);
			
			Если ЧастиПериода.Количество() = 0 Тогда
				ВидПериода = "";
				Год = "";
			Иначе
				ВидПериода = Число(ЧастиПериода[0]);
				Год 	   = Число(ЧастиПериода[1]);
			КонецЕсли;
			
			Представление = "";
			Если Число(ВидПериода) <= 12 Тогда
				Представление = Формат(Дата(Год, ВидПериода, 1), "ДФ='MMMM yyyy'");
			Иначе
				Если ВидПериода = 21 Тогда
					Представление = "I квартал " + Формат(Год, "ЧГ=");
				ИначеЕсли ВидПериода = 22 Тогда
					Представление = "II квартал " + Формат(Год, "ЧГ=");
				ИначеЕсли ВидПериода = 23 Тогда
					Представление = "III квартал " + Формат(Год, "ЧГ=");
				ИначеЕсли ВидПериода = 24 Тогда
					Представление = "IV квартал " + Формат(Год, "ЧГ=");
				// при реорганизации (ликвидации)
				ИначеЕсли ВидПериода = 51 Тогда
					Представление = "I квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 54 Тогда
					Представление = "II квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 55 Тогда
					Представление = "III квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				ИначеЕсли ВидПериода = 56 Тогда
					Представление = "IV квартал " + Формат(Год, "ЧГ=") + " при реорганизации (ликвидации)";
				КонецЕсли;
			КонецЕсли;
			
			Если Представление <> "" Тогда
				ПредставлениеПериода.Добавить(Представление);
			КонецЕсли;
	...
	ПредставлениеПериода = СтрСоединить(ПредставлениеПериода, "; ");
...
Функция ОтправленныеРанееСтроки(Требование) Экспорт
	...
	ОтправленныеСтроки.Вставить("СведКС", Новый Соответствие());
	ОтправленныеСтроки.Вставить("НетКнигаПрод", Новый Соответствие());
	...
	|ВЫБРАТЬ
	|	""СведКС"",
	|	ОтветНаТребование.НомКС,
	|	ОтветНаТребование.Ссылка
	|ИЗ
	|	Документ.ПоясненияКДекларацииПоНДС.СведКС КАК ОтветНаТребование
	|ГДЕ
	|	ОтветНаТребование.Ссылка В(&Ответы)
	|	И ОтветНаТребование.ПоясненКС <> """"";
	...
	// В разделе 12 и НетКнигаПрод нет ссылки на номер строки декларации,
	...
	|ВЫБРАТЬ
	|	""НетКнигаПрод"",
	|	ОтветНаТребование.НомерСчетаФактуры,
	|	ОтветНаТребование.ДатаСчетаФактуры,
	|	ОтветНаТребование.Ссылка
	|ИЗ
	|	Документ.ПоясненияКДекларацииПоНДС.НетКнигаПрод КАК ОтветНаТребование
	|ГДЕ
	|	ОтветНаТребование.Ссылка В(&Ответы)
	|	И ОтветНаТребование.РезультатПроверки <> ЗНАЧЕНИЕ(Перечисление.РезультатПроверкиСтрокиДекларации.ПустаяСсылка)";
...
Функция ДанныеПоясненияКДекларацииПоНДС(Пояснение) Экспорт
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость20,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС20,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.Стоимость20Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродаж.СуммаНДС20Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость20,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС20,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.Стоимость20Расхождение,
	...
	|	ПоясненияКДекларацииПоНДСКнигаПродажДл.СуммаНДС20Расхождение,
	...
	|	ПоясненияКДекларацииПоНДССведКС.ФормулировкаНарушения,
	|	ПоясненияКДекларацииПоНДССведКС.НормаЗакона,
	|	ПоясненияКДекларацииПоНДССведКС.КонтрольноеCоотношение,
	|	ПоясненияКДекларацииПоНДССведКС.ДанныеДекларации
	|ИЗ
	...
	|	ПоясненияКДекларацииПоНДСНетКнигаПрод.НаименованиеКонтрагента
	|ИЗ
	...
	ДанныеПоясненияКДекларацииПоНДС.НетКнигаПрод.Колонки.Добавить("КлючСтроки", Новый ОписаниеТипов("Строка"));
	Для Каждого СтрокаНетКнигаПрод Из ДанныеПоясненияКДекларацииПоНДС.НетКнигаПрод Цикл
		СтрокаНетКнигаПрод.КлючСтроки = КлючСтрокиРаздела12(СтрокаНетКнигаПрод.НомерСчетаФактуры, СтрокаНетКнигаПрод.ДатаСчетаФактуры);
	КонецЦикла;
...
Процедура ОчиститьПустыеСтроки(ПоясненияКДекларацииПоНДС) Экспорт
	...
	Если ЗначениеЗаполнено(ПоясненияКДекларацииПоНДС) Тогда
		...
		Для Каждого ТабличнаяЧасть Из ТабличныеЧасти Цикл
			...
			Если ТабличнаяЧасть.Ключ = "СведКС" Тогда
				Отбор = Новый Структура("ПоясненКС", "");
			Иначе
				Отбор = Новый Структура("РезультатПроверки", Перечисления.РезультатПроверкиСтрокиДекларации.ПустаяСсылка());
			КонецЕсли;
			
			СтрокиДляУдаления = ПояснениеОбъект[ТабличнаяЧасть.Ключ].НайтиСтроки(Отбор);
...
Функция ТаблицаРаздела8() Экспорт
	...
	Раздел8.Колонки.Добавить("СпрКодОш", Новый ОписаниеТипов("Массив"));
	...
	Раздел8.Колонки.Добавить("ПериодЗап", Новый ОписаниеТипов("Массив"));
...
Функция ТаблицаРаздела9() Экспорт
	...
	Раздел9.Колонки.Добавить("СпрКодОш", Новый ОписаниеТипов("Массив"));
	...
	Раздел9.Колонки.Добавить("ПериодЗап", Новый ОписаниеТипов("Массив"));
...
Функция ТаблицаРаздела10() Экспорт
	...
	Раздел10.Колонки.Добавить("СпрКодОш", Новый ОписаниеТипов("Массив"));
	...
	Раздел10.Колонки.Добавить("ПериодЗап", Новый ОписаниеТипов("Массив"));
...
Функция ТаблицаРаздела11() Экспорт
	...
	Раздел11.Колонки.Добавить("СпрКодОш", Новый ОписаниеТипов("Массив"));	
	...
	Раздел11.Колонки.Добавить("ПериодЗап", Новый ОписаниеТипов("Массив"));
...
Функция ТаблицаРаздела12() Экспорт
	...
	Раздел12.Колонки.Добавить("СпрКодОш",       Новый ОписаниеТипов("Массив"));
	Раздел12.Колонки.Добавить("СвПокуп",        ОбщегоНазначения.ОписаниеТипаСтрока(12));
	Раздел12.Колонки.Добавить("ПериодЗап",      Новый ОписаниеТипов("Массив"));
	
	Возврат Раздел12;
	
КонецФункции

Функция ТаблицаНетКнигаПрод() Экспорт
	
	НетКнигаПрод = Новый ТаблицаЗначений;
	
	НетКнигаПрод.Колонки.Добавить("СвКонтрагент",   Новый ОписаниеТипов("Структура"));
	НетКнигаПрод.Колонки.Добавить("СведСчФ",        Новый ОписаниеТипов("Массив"));
	
	Возврат НетКнигаПрод;
	
КонецФункции

Функция ТаблицаТребованияСведКС() Экспорт
	
	СведКС = Новый ТаблицаЗначений;
	
	СведКС.Колонки.Добавить("НомерКС",   	ОбщегоНазначения.ОписаниеТипаСтрока(6));
	СведКС.Колонки.Добавить("ФормулНаруш",  ОбщегоНазначения.ОписаниеТипаСтрока(2500));
	СведКС.Колонки.Добавить("НормаЗакон",  	ОбщегоНазначения.ОписаниеТипаСтрока(1000));
	СведКС.Колонки.Добавить("КС",  			ОбщегоНазначения.ОписаниеТипаСтрока(1000));
	СведКС.Колонки.Добавить("ДанДеклар",  	ОбщегоНазначения.ОписаниеТипаСтрока(1000));
...
Функция СтруктураСвКонтрагент() Экспорт
	
	СвКонтрагент = Новый Структура;
	СвКонтрагент.Вставить("СведЮЛ_Каг", Новый Структура());
	СвКонтрагент.Вставить("СведИП_Каг", Новый Структура());
	
	Возврат СвКонтрагент;
	
КонецФункции

Функция СтруктураСведСчФ() Экспорт
	
	СвСведСчФ = Новый Структура;
	
	СвСведСчФ.Вставить("НомСчФПрод", "");
	СвСведСчФ.Вставить("ДатаСчФПрод", '00010101');
...
Процедура ЗаполнитьРасхожденияСтрокиРаздела9(ДанныеСтроки) Экспорт
	...
	ДанныеСтроки.Стоимость20Расхождение  = ДанныеСтроки.Стоимость20;
	...
	ДанныеСтроки.СуммаНДС20Расхождение   = ДанныеСтроки.СуммаНДС20;
...
Процедура ЗаполнитьРасхожденияСтрокиНетКнигаПрод(ДанныеСтроки) Экспорт
	
	ДанныеСтроки.ПокупательИННРасхождение 		= ДанныеСтроки.ПокупательИНН;
	ДанныеСтроки.ПокупательКППРасхождение 		= ДанныеСтроки.ПокупательКПП;
	ДанныеСтроки.ПосредникИННРасхождение 		= ДанныеСтроки.ПосредникИНН;
	ДанныеСтроки.ПосредникКППРасхождение 		= ДанныеСтроки.ПосредникКПП;
	ДанныеСтроки.КодВидаОперацииРасхождение 	= ДанныеСтроки.КодВидаОперации;
	ДанныеСтроки.НомерСчетаФактурыРасхождение 	= ДанныеСтроки.НомерСчетаФактуры;
	ДанныеСтроки.ДатаСчетаФактурыРасхождение 	= ДанныеСтроки.ДатаСчетаФактуры;
	ДанныеСтроки.РазделРасхождение 				= ДанныеСтроки.Раздел;
	ДанныеСтроки.СтоимостьРубВсегоРасхождение	= ДанныеСтроки.СтоимостьРубВсего;
...
Процедура ЗаполнитьРаздел8(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ЗаписьКнигиПокупок.КодОшибки                 = КодыОшибок(СтрокаТаблицыРаздела.СпрКодОш);
...
Процедура ЗаполнитьРаздел9(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ЗаписьКнигиПродаж.КодОшибки                 = КодыОшибок(СтрокаТаблицыРаздела.СпрКодОш);
		...
		ЗаписьКнигиПродаж.Стоимость20               = СтрокаТаблицыРаздела.СтоимПродСФ20;
		...
		ЗаписьКнигиПродаж.СуммаНДС20                = СтрокаТаблицыРаздела.СумНДССФ20;
		...
		Иначе
			ЗаполнитьЗначенияСвойств(ЗаписьКнигиПродаж, ДанныеКопированияСтроки, "РезультатПроверки, НомерСчетаФактурыРасхождение, ДатаСчетаФактурыРасхождение, НомерИсправленияСчетаФактурыРасхождение, ДатаИсправленияСчетаФактурыРасхождение,
					...
					|КодВалютыРасхождение, СтоимостьВсегоРасхождение, Стоимость20Расхождение, Стоимость18Расхождение, Стоимость10Расхождение, Стоимость0Расхождение, СуммаНДС20Расхождение, СуммаНДС18Расхождение, СуммаНДС10Расхождение, СтоимостьОсвобождаемаяРасхождение, КодВидаОперацииРасхождение, ПосредникИННРасхождение");
...
Процедура ЗаполнитьРаздел10(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ЗаписьЖурнала.КодОшибки                 = КодыОшибок(СтрокаТаблицыРаздела.СпрКодОш);
...
Процедура ЗаполнитьСведКС(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	
	ТаблицаРаздела = ДанныеТребования.СведКС;
	
	ОтправленныеРанееСтрокиРаздела = Неопределено;
	Если ОтправленныеРанееСтроки <> Неопределено Тогда
		ОтправленныеРанееСтрокиРаздела = ОтправленныеРанееСтроки.СведКС;
	КонецЕсли;
	
	ДанныеКопированияРаздела = Неопределено;
	Если ДанныеКопирования <> Неопределено Тогда
		ДанныеКопированияРаздела = ДанныеКопирования.СведКС;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		...
		ДанныеКопированияСтроки = Неопределено;
		Если ДанныеКопированияРаздела <> Неопределено Тогда
			ДанныеКопированияСтроки = ДанныеКопированияРаздела.Найти(СтрокаТаблицыРаздела.НомерКС, "НомКС");
		КонецЕсли;
		
		Если ДанныеКопированияСтроки = Неопределено
			И ОтправленныеРанееСтрокиРаздела <> Неопределено Тогда
			Если ОтправленныеРанееСтрокиРаздела.Получить(СтрокаТаблицыРаздела.НомерКС) <> Неопределено Тогда
				// Ответ по строке был дан ранее.
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		
		ЗаписьСведКС = Объект.СведКС.Добавить();
		ЗаписьСведКС.НомКС 					= СтрокаТаблицыРаздела.НомерКС;
		ЗаписьСведКС.ФормулировкаНарушения 	= СтрокаТаблицыРаздела.ФормулНаруш;
		ЗаписьСведКС.НормаЗакона 			= СтрокаТаблицыРаздела.НормаЗакон;
		ЗаписьСведКС.КонтрольноеCоотношение = СтрокаТаблицыРаздела.КС;
		ЗаписьСведКС.ДанныеДекларации 		= СтрокаТаблицыРаздела.ДанДеклар;
		
		Если ДанныеКопированияСтроки <> Неопределено Тогда
			ЗаполнитьЗначенияСвойств(
				ЗаписьСведКС, 
				ДанныеКопированияСтроки, 
				"НомКС, ПоясненКС, ФормулировкаНарушения, НормаЗакона, ДанныеДекларации, КонтрольноеCоотношение");
		КонецЕсли;
		
		Объект.СведКС.Сортировать("НомКС");
		
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьНетКнигаПрод(Объект, ДанныеТребования, ОтправленныеРанееСтроки, ДанныеКопирования) Экспорт
	
	ТаблицаРаздела = ДанныеТребования.НетКнигаПрод;
	
	ОтправленныеРанееСтрокиРаздела = Неопределено;
	Если ОтправленныеРанееСтроки <> Неопределено Тогда
		ОтправленныеРанееСтрокиРаздела = ОтправленныеРанееСтроки.НетКнигаПрод;
	КонецЕсли;
	
	ДанныеКопированияРаздела = Неопределено;
	Если ДанныеКопирования <> Неопределено Тогда
		ДанныеКопированияРаздела = ДанныеКопирования.НетКнигаПрод;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицыРаздела Из ТаблицаРаздела Цикл
		
		СведенияКонтрагента = СтрокаТаблицыРаздела.СвКонтрагент;
		ЭтоЮрЛицо  = СведенияКонтрагента.СведЮЛ_Каг.Количество() <> 0;
		
		СведСчФ = СтрокаТаблицыРаздела.СведСчФ;
		
		Для каждого СтрокаСведСчФ Из СведСчФ Цикл
			
			// Строка добавляется в раздел в том случае, если она раньше не была отправлена в другом пояснении
			// Но если это режим копирования и строка находится в документе копирования, то строка добавляется в любом случае
		
			КлючСтроки = КлючСтрокиРаздела12(СтрокаСведСчФ.НомСчФПрод, СтрокаСведСчФ.ДатаСчФПрод);
			...
			ЗаписьНетКнигаПрод = Объект.НетКнигаПрод.Добавить();
			ЗаписьНетКнигаПрод.НомерСчетаФактуры = СтрокаСведСчФ.НомСчФПрод;
			ЗаписьНетКнигаПрод.ДатаСчетаФактуры  = СтрокаСведСчФ.ДатаСчФПрод;
			
			Если ЭтоЮрЛицо Тогда
				
				ЗаписьНетКнигаПрод.ПокупательИНН = СведенияКонтрагента.СведЮЛ_Каг.ИННЮЛ;
				ЗаписьНетКнигаПрод.НаименованиеКонтрагента = СведенияКонтрагента.СведЮЛ_Каг.НаимОрг;
				
				// Необязательное поле
				Если СведенияКонтрагента.СведЮЛ_Каг.Свойство("КПП") Тогда
					ЗаписьНетКнигаПрод.ПокупательКПП = СведенияКонтрагента.СведЮЛ_Каг.КПП;
				КонецЕсли;
				
			Иначе
				ЗаписьНетКнигаПрод.ПокупательИНН = СведенияКонтрагента.СведИП_Каг.ИННФЛ;
				
				ФИО = Новый Массив;
				ФИО.Добавить(СокрЛП(СведенияКонтрагента.СведИП_Каг.ФИО.Фамилия));
				ФИО.Добавить(СокрЛП(СведенияКонтрагента.СведИП_Каг.ФИО.Имя));
				
				// Необязательное поле
				Если СведенияКонтрагента.СведИП_Каг.ФИО.Свойство("Отчество") Тогда
					ФИО.Добавить(СокрЛП(СведенияКонтрагента.СведИП_Каг.ФИО.Отчество));
				КонецЕсли;
				
				ФИО = СтрСоединить(ФИО, " ");
				
				ЗаписьНетКнигаПрод.НаименованиеКонтрагента = ФИО;
				
			КонецЕсли;
			
			Если ДанныеКопированияСтроки = Неопределено Тогда
				ЗаполнитьРасхожденияСтрокиНетКнигаПрод(ЗаписьНетКнигаПрод);
			Иначе
				ЗаполнитьЗначенияСвойств(ЗаписьНетКнигаПрод, ДанныеКопированияСтроки);
			КонецЕсли;
...
Функция ПолучитьДаннныеДляВыгрузки(ДокументСсылка)
	...
	Для Каждого СтрокаРаздела9 Из ДанныеПояснения.Раздел9 Цикл
		...
		Если СтрокаРаздела9.СуммаНДС20 <> СтрокаРаздела9.СуммаНДС20Расхождение
			ИЛИ СтрокаРаздела9.СуммаНДС18 <> СтрокаРаздела9.СуммаНДС18Расхождение 
	...
	Для Каждого СтрокаРаздела9_1 Из ДанныеПояснения.Раздел9_1 Цикл
		...
		Если СтрокаРаздела9_1.СуммаНДС20 <> СтрокаРаздела9_1.СуммаНДС20Расхождение 
			ИЛИ СтрокаРаздела9_1.СуммаНДС18 <> СтрокаРаздела9_1.СуммаНДС18Расхождение 
	...
	Для Каждого СтрокаРаздела10 Из ДанныеПояснения.Раздел10 Цикл
		...
		СписокПосредническаяДеятельностьРасхождение = ЗначенияИзТаблицыХраненияПоИдентификатору(ДанныеПояснения.ПосредническаяДеятельность, СтрокаРаздела10.ПосредническаяДеятельностьРасхождение, РеквизитыПосредническойДеятельности());
	...
	Для каждого СтрокаСведКС Из ДанныеПояснения.СведКС Цикл
		...
		Если ЗначениеЗаполнено(СтрокаСведКС.ПоясненКС) Тогда
...
Функция ПредставлениеОбщихКодовОшибок(Ошибка, ГрафаОшибки)
	
	ПредставлениеОшибки = "";

	Если Ошибка = "1" Тогда
		ПредставлениеОшибки = НСтр("ru = 'У контрагента нет записи об операции'");
	ИначеЕсли Ошибка = "2" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Операция отражена по разному в разделах 8 и 9'");
	ИначеЕсли Ошибка = "3" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Операция отражена по разному в разделах 10 и 11'")
	ИначеЕсли Ошибка = "4" Тогда
		// В каждом разделе для этой ошибки есть свои методы
	ИначеЕсли Ошибка = "5" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Дата счет-фактуры превышает отчетный период или отсутствует'");
	ИначеЕсли Ошибка = "6" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Заявлен вычет по счету-фактуре за пределами трёх лет'");
	ИначеЕсли Ошибка = "7" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Счет-фактура заявлен к вычету до даты регистрации налогоплательщика'");
	ИначеЕсли Ошибка = "8" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Некорректно указан код вида операции'");
	ИначеЕсли Ошибка = "9" Тогда
		ПредставлениеОшибки = НСтр("ru = 'Сумма НДС, указанная с отрицательным значением, превышает сумму НДС, указанную в записи по счету-фактуре, подлежащей аннулированию, либо отсутствует запись по счету-фактуре, подлежащая аннулированию'");
	Иначе
...
Функция ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, Раздел, ВерсияФоматаТребования = Неопределено)
	
	КодыОшибок = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(КодОшибки, ";");
	
	МассивОшибок = Новый Массив;
	Для Каждого Ошибка ИЗ КодыОшибок Цикл
		...
		Если Ошибка = "4" Тогда
			...
			Если Раздел = 8 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела8_Код4(ГрафаОшибки);
			ИначеЕсли Раздел = 9 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела9_Код4(ГрафаОшибки, ВерсияФоматаТребования);
			ИначеЕсли Раздел = 10 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела10_Код4(ГрафаОшибки);
			ИначеЕсли Раздел = 11 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела11_Код4(ГрафаОшибки);
			ИначеЕсли Раздел = 12 Тогда
				ПредставлениеОшибки = ПредставлениеОшибкиРаздела12_Код4(ГрафаОшибки);
			КонецЕсли;
				
		Иначе
			ПредставлениеОшибки = ПредставлениеОбщихКодовОшибок(Ошибка, ГрафаОшибки);
		КонецЕсли;
		
		ПредставлениеОшибки = СокрЛП(ПредставлениеОшибки);
		МассивОшибок.Добавить(ПредставлениеОшибки);
		
	КонецЦикла;
	
	МассивОшибок = СтрСоединить(МассивОшибок, Символы.ПС);
...
Функция ПредставлениеОшибкиРаздела8(КодОшибки, ГрафаОшибки) Экспорт
	...
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 8);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела8_Код4(ГрафаОшибки)
	...
	Для Каждого Графа Из МассивГраф Цикл
		...
		Иначе
			МассивПредставлений.Добавить(Графа);
		КонецЕсли;
	КонецЦикла;
	...
	КонецЕсли;
	ПредставлениеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(СообщениеОбОшибке, ГрафыОшибки);
	
	Возврат ПредставлениеОшибки;
	
КонецФункции

Функция ПредставлениеОшибкиРаздела9(КодОшибки, ГрафаОшибки, ВерсияФоматаТребования) Экспорт
	
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 9, ВерсияФоматаТребования);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела9_Код4(ГрафаОшибки, ВерсияФоматаТребования)
	
	БольшеИлиРавно_5_05 = ДокументооборотСКОКлиентСервер.СравнитьВерсииФормата(ВерсияФоматаТребования, "5.05") >= 0;
	...
	Для Каждого Графа Из МассивГраф Цикл
		...
		Иначе
			...
			Если БольшеИлиРавно_5_05 Тогда
				
				// В формате 5.0.5 добавлены колонки:
				// Стоимость (НДС 20%) и НДС 20%
				
				Если Графа = "17" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""Стоимость (НДС 20%)""'"));
				ИначеЕсли Графа = "18" Тогда
				...
				ИначеЕсли Графа = "21" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""НДС 20%""'"));
				ИначеЕсли Графа = "22" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""НДС 18%""'"));
				ИначеЕсли Графа = "23" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""НДС 10%""'"));
				ИначеЕсли Графа = "24" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""Стоимость (без НДС)""'"));
				ИначеЕсли Графа = "25" Тогда
					МассивПредставлений.Добавить(НСтр("ru = '""Период отражения записи""'"));
				КонецЕсли;
...
Функция ПредставлениеОшибкиРаздела10(КодОшибки, ГрафаОшибки) Экспорт
	...
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 10);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела10_Код4(ГрафаОшибки)
...
Функция ПредставлениеОшибкиРаздела11(КодОшибки, ГрафаОшибки) Экспорт
	...
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 11);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела11_Код4(ГрафаОшибки)
...
Функция ПредставлениеОшибкиРаздела12(КодОшибки, ГрафаОшибки) Экспорт
	...
	Возврат ПредставлениеОшибкиРазделов(КодОшибки, ГрафаОшибки, 12);
	
КонецФункции

Функция ПредставлениеОшибкиРаздела12_Код4(ГрафаОшибки)
...
Функция КодыОшибок(КодыОшибок)
	
	МассивКодовОшибок = Новый Массив;
	Для каждого КодОшибки Из КодыОшибок Цикл
		СтрокаКодаОшибки = СокрЛП(КодОшибки.КодОш);
		МассивКодовОшибок.Добавить(СтрокаКодаОшибки);
	КонецЦикла;
	
	МассивКодовОшибок = СтрСоединить(МассивКодовОшибок, ";");
	
	Возврат МассивКодовОшибок;
	
КонецФункции

Функция ГрафыОшибок(КодыОшибок)
	
	МассивГрафОшибок = Новый Массив;
	
	Для каждого КодОшибки Из КодыОшибок Цикл
		Если КодОшибки.Свойство("ГрафОш") Тогда
			ГрафОш = КодОшибки.ГрафОш;
			Для каждого СтрокаГрафОш Из ГрафОш Цикл
				СтрокаГрафыОшибки = СокрЛП(СтрокаГрафОш);
				МассивГрафОшибок.Добавить(СтрокаГрафыОшибки);
			КонецЦикла; 
		КонецЕсли;
	КонецЦикла; 
	
	МассивГрафОшибок = СтрСоединить(МассивГрафОшибок, ";");
	
	Возврат МассивГрафОшибок;
...
Функция ПериодОтраженияЗаписи(ПериодЗап)
	...
	Если ТипЗнч(ПериодЗап) = Тип("Массив") Тогда
		
		Результат = Новый Массив;
		Для каждого СтрокаПериодЗап Из ПериодЗап Цикл
			СтрокаПериодаОтраженияЗаписи = ПериодОтраженияПодзаписи(СтрокаПериодЗап);
			СтрокаПериодаОтраженияЗаписи = СокрЛП(СтрокаПериодаОтраженияЗаписи);
			Результат.Добавить(СтрокаПериодаОтраженияЗаписи);
		КонецЦикла;
		Результат = СтрСоединить(Результат, ";");
		Возврат Результат;
		
	Иначе
		Возврат ПериодОтраженияПодзаписи(ПериодЗап);
	КонецЕсли;
	
КонецФункции

Функция ПериодОтраженияПодзаписи(ПериодЗап)
	
	Если ТипЗнч(ПериодЗап) = Тип("Структура") И ПериодЗап.Свойство("ПериодСв") Тогда
		Возврат ПериодЗап.ПериодСв + "_" + ПериодЗап.ГодСв;
	Иначе
		Возврат "";
	КонецЕсли;
