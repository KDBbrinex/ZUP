...
Процедура Декорация1ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	КонтекстЭДОКлиент.СоздатьПакетСДопДокуменами(Объект.Требование);
КонецПроцедуры
...
&НаКлиенте
Процедура ОтправкаПоясняющихДокументов(Команда)
	КонтекстЭДОКлиент.СоздатьПакетСДопДокуменами(Объект.Требование);
КонецПроцедуры
...
Процедура БольшеНеПоказыватьИнформационнуюПанель()
	...
	ХранилищеОбщихНастроек.Сохранить(
		"ДокументооборотСКонтролирующимиОрганами_ПоясненияКДекларацииПоНДС_УбратьИнформационнуюПанель",
		,
		Истина);
...
Процедура УстановитьВидимостьИнформационнойПанели()
	...
	ПанельУбранаПользователем = ХранилищеОбщихНастроек.Загрузить("ДокументооборотСКонтролирующимиОрганами_ПоясненияКДекларацииПоНДС_УбратьИнформационнуюПанель") = Истина;
	
	Если ПанельУбранаПользователем Тогда
...
Процедура ПечатьТребования_ПослеВопроса(Результат, ВходящийКонтекст) Экспорт
	...
	ТолькоОтобранные = Результат = НСтр("ru = 'Только отобранные'");
	ТолькоТекущийРаздел = ВходящийКонтекст.ТолькоТекущийРаздел;
	ВходящийКонтекст.Вставить("ТолькоОтобранные", ТолькоОтобранные);
	
	Если (Модифицированность ИЛИ Параметры.Ключ.Пустая()) 
		И Не ЗаписатьОбъект("Печать", ВходящийКонтекст) ИЛИ Не ПроверитьЗаполнение() Тогда
		Возврат;
	КонецЕсли;
	...
	Если КонтекстЭДОСервер.ЭтоФайлВыгрузкиДекларацияНДС(СвойстваФайла.Имя) Тогда
		...
		Если НРег(СвойстваФайла.Расширение) = НРег(".xml") Тогда
			...
			Если КНД = Неопределено ИЛИ КНД = "" ИЛИ КНД <> "1151001" Тогда
				ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выбранный файл не является файлом декларации по НДС'"));
				Возврат Неопределено;
			КонецЕсли;
			...
			Если НЕ ДокументооборотСКОВызовСервера.ИспользуетсяРежимТестирования() Тогда
				...
				Если ОрганизацияВФайле <> Объект.Организация Тогда
					ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Организация в выбранном файле декларации по НДС не совпадает с организацией, указанной в требовании'"));
					Возврат Неопределено;
				КонецЕсли;
				...
				Если ВТребованииЕстьXMLФайл Тогда
					...
					Если НомКорр <> СвойстваДекларации.ТребованиеНомКорр Тогда
						ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Номер корректировки в выбранном файле декларации по НДС не совпадает с номером корректировки, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
					
					Период = ПоказателиВыгрузки["Период"];
					Если Период <> СвойстваДекларации.ТребованиеПериод Тогда
						ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Период в выбранном файле декларации по НДС не совпадает с периодом, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
					
					ОтчетГод = ПоказателиВыгрузки["ОтчетГод"];
					Если ОтчетГод <> СвойстваДекларации.ТребованиеОтчетГод Тогда
						ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Год в выбранном файле декларации по НДС не совпадает с годом, указанном в требовании'"));
						Возврат Неопределено;
					КонецЕсли;
		...
		Иначе
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Файл декларации по НДС должен иметь расширение xml'"));
			Возврат Неопределено;
		КонецЕсли;
...
&НаКлиенте
Функция ЗаписатьОбъект(ДействиеПослеЗаписи = "", ВходящийКонтекст = Неопределено)
	
	Результат = Истина;
	МассивПолей = Новый Массив;
	
	НайденныеСимволы = ПроверитьНеДопустимыеСимволы(Объект.ПояснИнОсн);
	Если НайденныеСимволы.Количество() > 0 Тогда
		МассивПолей.Добавить("Иные пояснения");
	КонецЕсли;
...
Процедура ЗаписатьОбъектЗавершение(РезультатПроверки, ДополнительныеПараметры) Экспорт
	...
	Если Успешно И ЗначениеЗаполнено(ДействиеПослеЗаписи) Тогда
		...
		ИначеЕсли ДействиеПослеЗаписи = "Печать" Тогда
			ПечатьТребования_ПослеВопроса(ВходящийКонтекст.ТолькоОтобранные, ВходящийКонтекст);
		КонецЕсли;
...
Процедура ПоказатьОшибкуСодержимогоПослеВопроса(РезультатВыбора, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВыбора = КодВозвратаДиалога.Да Тогда
		НедопустимыеСимволы = ПроверитьНеДопустимыеСимволы(Объект.ПояснИнОсн);
		Если НедопустимыеСимволы.Количество() > 0 Тогда
			Объект.ПояснИнОсн = ЗаменитьНеДопустимыеСимволы(Объект.ПояснИнОсн, НедопустимыеСимволы, " ");
		КонецЕсли;
