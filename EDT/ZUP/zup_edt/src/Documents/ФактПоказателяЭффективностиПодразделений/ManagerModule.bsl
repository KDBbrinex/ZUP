...
Процедура ПриЗаполненииОграниченияДоступа(Ограничение) Экспорт
	...
	// "НЕ Ложь В (Выбрать Первые 1 Ложь Из Документ.<ИмяДокумента>.Подразделения КАК Т1 ГДЕ Т.Ссылка = Т1.Ссылка И НЕ (",
	// "СтруктураПредприятия", "Т1.Подразделение", "))",
	// "","", "","","", "", "","", "", "","","", "","","", "","","", "","","", "","","", "","","", "","","", "","","",
	// "","","", "","","", "","","", "","","")
	Ограничение.Текст =
	"РазрешитьЧтениеИзменение
	|ГДЕ
	|	ДляВсехСтрок(ЗначениеРазрешено(Подразделения.Подразделение))";
КонецПроцедуры
...
Функция ДобавитьКомандыСозданияДокументов(КомандыСозданияДокументов, ДополнительныеПараметры) Экспорт
	...
	ОписаниеКоманды = ЗарплатаКадрыРасширенный.ОписаниеКомандыСозданияДокумента(
		"Документ.ФактПоказателяЭффективностиПодразделений",
		НСтр("ru = 'Факт показателя подразделений'"));
...
Функция ПолучитьДанныеДляПроведения(Ссылка)Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Ссылка.Период КАК Период,
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Ссылка.Горизонт КАК Горизонт,
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Подразделение КАК Подразделение,
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Ссылка.Показатель КАК Показатель,
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Значение КАК Значение
		|ИЗ
		...
		|ГДЕ
		|	ФактПоказателяЭффективностиПодразделенийПодразделения.Ссылка = &Ссылка";
