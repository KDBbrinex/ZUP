...
Функция ОписаниеРеквизитаКадровогоРешения() Экспорт
	Возврат Метаданные.Документы.ВосстановлениеВДолжности.Реквизиты.Решение;
КонецФункции
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ОтветственноеЛицоКадровик) Тогда
			...
			ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(Выборка.ОтветственноеЛицоКадровик),
				...
				Выборка.РаботникКадровойСлужбы);
			
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Выборка.ИсполнительФамилияИО) Тогда
			
			ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(Выборка.ИсполнительФамилияИО),
...
Функция ВыборкаДанныхДляПечатиДокументов(МассивОбъектов)
	...
	Запрос.Текст =
		...
		|	ФИОИсполнителя.ФамилияИО КАК ИсполнительФамилияИО,
...
Процедура ЗаполнитьДатуЗапретаРедактирования(ОбъектДокумента) Экспорт
	...
	ЗарплатаКадры.ЗаполнитьДатуЗапретаРедактирования(ОбъектДокумента, "ДатаВосстановления");
...
Процедура ЗаполнитьДатыЗапрета(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 100
		|	ВосстановлениеВДолжности.Ссылка КАК Ссылка,
		|	ВосстановлениеВДолжности.Дата КАК Дата
		|ИЗ
		|	Документ.ВосстановлениеВДолжности КАК ВосстановлениеВДолжности
		|ГДЕ
		|	ВосстановлениеВДолжности.ДатаЗапрета = ДАТАВРЕМЯ(1, 1, 1)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВосстановлениеВДолжности.Дата УБЫВ";
...
Функция ОписаниеПодписейДокумента() Экспорт 
	...
	ПодписиДокументов.ДобавитьОписаниеПодписейОрганизации(
		ОписаниеПодписей,
		"Руководитель, Бухгалтер, РаботникКадровойСлужбы, Исполнитель",
