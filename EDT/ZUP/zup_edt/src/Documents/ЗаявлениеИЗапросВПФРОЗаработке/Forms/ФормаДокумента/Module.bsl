...
Процедура УстановитьПредставлениеКИ()
	...
	Если Не ПустаяСтрока(Объект.АдресОрганизации) Тогда
		СтруктураАдреса = ЗарплатаКадры.СтруктураАдресаИзXML(
			Объект.АдресОрганизации,
			ПредопределенноеЗначение("Справочник.ВидыКонтактнойИнформации.ЮрАдресОрганизации"));
		УправлениеКонтактнойИнформациейКлиентСервер.СформироватьПредставлениеАдреса(СтруктураАдреса, АдресОрганизации);
	Иначе
		АдресОрганизации = НСтр("ru = '<Не заполнен>'");
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.ТелефонОрганизации) Тогда
		СтруктураТелефона = РаботаСАдресами.СведенияОбАдресе(Объект.ТелефонОрганизации);
		ТелефонОрганизации = СтруктураТелефона.Представление;
	Иначе
		ТелефонОрганизации = НСтр("ru = '<Не заполнен>'");
	КонецЕсли;
	...
	Если Не ПустаяСтрока(Объект.Телефон) Тогда
		...
		Телефон = УправлениеКонтактнойИнформациейКлиентСервер.СформироватьПредставлениеТелефона(
