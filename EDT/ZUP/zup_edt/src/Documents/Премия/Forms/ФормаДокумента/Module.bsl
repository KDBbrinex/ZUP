...
Процедура ПланируемаяДатаВыплатыПриИзмененииНаСервере()
	...
	Если Объект.РассчитыватьУдержания Или УчетНДФЛРасширенный.МежрасчетныйДокументИсчисляетНДФЛ(Объект.Организация, ОтложитьРасчетНалогаДоРасчетаЗарплатыВКонцеМесяца, Объект.ПорядокВыплаты, ОбязательныйРасчетНДФЛ) Тогда
...
Процедура ПорядокВыплатыПриИзмененииНаСервере(УстановитьПланируемуюДатуВыплаты = Истина)
	...
	КонецЕсли;
	Если Не УчетНДФЛРасширенный.МежрасчетныйДокументИсчисляетНДФЛ(Объект.Организация, ОтложитьРасчетНалогаДоРасчетаЗарплатыВКонцеМесяца, Объект.ПорядокВыплаты, ОбязательныйРасчетНДФЛ) Тогда
...
Процедура ЗаполнитьНалогиУдержания()
	...
	Если Не (Объект.РассчитыватьУдержания Или УчетНДФЛРасширенный.МежрасчетныйДокументИсчисляетНДФЛ(Объект.Организация, ОтложитьРасчетНалогаДоРасчетаЗарплатыВКонцеМесяца, Объект.ПорядокВыплаты, ОбязательныйРасчетНДФЛ)) Тогда
		Возврат;
	КонецЕсли; 
...
Процедура ЗаполнитьНалогиУдержанияСотрудника(ДанныеСтроки, ФизическоеЛицо = Неопределено, СотрудникиФизическогоЛица = Неопределено)
	...
	Для Каждого СтрокаНДФЛ Из СтрокиНДФЛ Цикл
		...
		ДанныеСтроки.ЗачтеноАвансовыхПлатежей = ДанныеСтроки.ЗачтеноАвансовыхПлатежей + СтрокаНДФЛ.ЗачтеноАвансовыхПлатежей;
	КонецЦикла;
...
Процедура ПриПолученииДанныхНаСервере()
	...
	ОбязательныйРасчетНДФЛ = УчетНДФЛРасширенный.ДоходыВУчетеНДФЛРегистрируютсяПоДатеВыплаты(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.ВидПремии));
	ОтложитьРасчетНалогаВМежрасчетахДоРасчетаЗарплаты = УчетНДФЛРасширенный.ОтложитьРасчетНалогаВМежрасчетахДоРасчетаЗарплаты(Объект.Организация);
...
Процедура УстановитьВидимостьРеквизитовНДФЛ()
	...
	ВидимостьРеквизитовНДФЛ = УчетНДФЛРасширенный.МежрасчетныйДокументИсчисляетНДФЛ(Объект.Организация, ОтложитьРасчетНалогаДоРасчетаЗарплатыВКонцеМесяца, Объект.ПорядокВыплаты, ОбязательныйРасчетНДФЛ);
...
Процедура УстановитьВидимостьКВыплате()
	...
	Элементы.НачисленияКВыплате.Видимость = (Объект.РассчитыватьУдержания И ОграниченияНаУровнеЗаписей.ЧтениеБезОграничений)
		Или УчетНДФЛРасширенный.МежрасчетныйДокументИсчисляетНДФЛ(Объект.Организация, ОтложитьРасчетНалогаДоРасчетаЗарплатыВКонцеМесяца, Объект.ПорядокВыплаты, ОбязательныйРасчетНДФЛ);
