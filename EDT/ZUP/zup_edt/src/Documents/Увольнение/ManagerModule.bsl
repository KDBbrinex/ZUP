			...
			Для Каждого СтрокаПериодовРегистрации Из РеквизитыДляПроведения.ПериодыРегистрации Цикл
				...
				Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
					...
					Если ПолучитьФункциональнуюОпцию("ИспользоватьУправленческуюЗарплату") Тогда
						...
						НачисленияПоСотрудникам = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(УправленческийУчет.НачисленияПоСотрудникам, РеквизитыДляПроведения.ПериодРегистрации, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.Сотрудники);
						...
						ОтработанноеВремяПоСотрудникам = РасчетЗарплатыРасширенный.СтрокиТаблицыДокументаПоПериодуРегистрации(УправленческийУчет.ОтработанноеВремяПоСотрудникам, РеквизитыДляПроведения.ПериодРегистрации, ПериодРегистрации, ЭтоПервыйПериод, СтрокаПериодовРегистрации.Сотрудники);
...
Процедура ЗаполнитьУведомлениеОПрекращенииОтпускаПоУходуЗаРебенкомПоОснованию(УвольнениеСсылка, РеквизитыУведомления) Экспорт
	...
	РеквизитыУведомления.Вставить("ТипПриказа", Перечисления.СостоянияСотрудника.Увольнение);
	...
	РеквизитыУведомления.Вставить("ДатаПрекращенияОплаты", РеквизитыУвольнения.ДатаУвольнения + 86400);
КонецПроцедуры
...
Процедура ДобавитьКомандыПечатиПакетаДокументов(КомандыПечати, ИдентификаторыПФ) Экспорт
	...
	Если ПравоДоступа("Просмотр", Метаданные.Документы.СправкаНДФЛ) Тогда
		...
		КомандаПечати.Обработчик = "УправлениеПечатьюБЗККлиент.ВыполнитьКомандуПечати";
		КомандаПечати.МенеджерПечати = "Документ.Увольнение";
		КомандаПечати.Идентификатор = "Раздел3РСВ";
	...
	Если ИдентификаторыПФ.Количество() > 1 Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИдентификаторыПФ, ИдентификаторыПФОсновногоМестаРаботы);
		КомандаПечати.Идентификатор = СтрСоединить(ИдентификаторыПФ, ",");
...
Процедура ПечатьПакетаДокументовПриУвольнении(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СправкаРасширенная")
		...
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СправкаРасширенная")
			...
			Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СправкаРасширенная") Тогда
				...
				Если МассивСправок.Количество() > 0 Тогда
					...
					Документы.ИсходящаяСправкаОЗаработкеДляРасчетаПособий.Печать(
			...
			Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_СТД_Р") Тогда
				...
				Если МассивСправок.Количество() > 0 Тогда
					...
					Документы.СведенияОТрудовойДеятельностиРаботникаСТД_Р.Печать(
						МассивСправок, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
		...
		Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ОтчетОПогашенииЗайма") Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.ДоговорЗайма) Тогда
					МассивСправок.Добавить(Выборка.ДоговорЗайма);
				КонецЕсли;
			...
			Если МассивСправок.Количество() > 0 Тогда
				...
				Документы.ДоговорЗаймаСотруднику.Печать(
					МассивСправок, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ФормаСВЗ_М") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			ПерсонифицированныйУчет.ПечатнаяФормаСЗВМПоДаннымДокументов(
				ОбъектыПечати, ВыборкаПоШапкеДокумента, ВыборкаПоРаботникиОрганизации, ВыборкаПоРаботникиОрганизации.Количество() > 1));
...
Функция РезультатЗапросаПоСотрудникамДляПечатиСЗВСТАЖ(МассивСсылок)
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТЗаписиОСтажеУвольнений
		|ИЗ
	...
	Запрос.Текст =
		...
		|	СотрудникиПериоды.Ссылка КАК Ссылка,
		|	СотрудникиПериоды.НомерСтроки КАК НомерСтроки,
		|	СотрудникиПериоды.ФизическоеЛицо КАК Сотрудник,
		|	КадровыеДанные.Фамилия КАК Фамилия,
		|	КадровыеДанные.Имя КАК Имя,
		|	КадровыеДанные.Отчество КАК Отчество,
		|	КадровыеДанные.СтраховойНомерПФР КАК СтраховойНомерПФР,
	...
	Возврат ПерсонифицированныйУчет.РезультатЗапросаПоСотрудникам(Запрос.МенеджерВременныхТаблиц, "ВТЗаписиОСтажеУвольнений");
...
//
Функция ДанныеДокументовДляПечатиРасчетаСреднегоЗаработка(МассивСсылок, ИмяМакета = "", ПоСтатьямФинансирования = Ложь) Экспорт
...
Процедура ПечатьРаздел3РСВ(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Раздел3РСВ") Тогда
		...
		Запрос.Текст =
			...
			|	КОНЕЦПЕРИОДА(Увольнение.ДатаУвольнения, КВАРТАЛ) КАК ДатаКонцаПериодаОтчета,
			|	Увольнение.ФизическоеЛицо КАК ФизическоеЛицо,
			|	1 КАК НомерСтроки
			...
			|	УвольнениеСпискомСотрудники.Ссылка.Организация,
			|	УвольнениеСпискомСотрудники.ДатаУвольнения,
			|	КОНЕЦПЕРИОДА(УвольнениеСпискомСотрудники.ДатаУвольнения, КВАРТАЛ),
			|	УвольнениеСпискомСотрудники.ФизическоеЛицо,
			|	УвольнениеСпискомСотрудники.НомерСтроки
			|ИЗ
			|	Документ.УвольнениеСписком.Сотрудники КАК УвольнениеСпискомСотрудники
		...
		Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
			...
			Пока Выборка.СледующийПоЗначениюПоля("ДатаКонцаПериодаОтчета") Цикл
				...
				ПараметрыПечати.Вставить("Организация", Выборка.Организация);
				ПараметрыПечати.Вставить("ДатаПодписи", Выборка.ДатаПодписи);
				ПараметрыПечати.Вставить("ДатаКонцаПериодаОтчета", Выборка.ДатаКонцаПериодаОтчета);
				ПараметрыПечати.Вставить("МассивФизическихЛиц", МассивФизическихЛиц);
				
				// При подготовке данных также используются 
				МакетыРСВ = ИнтерфейсыВзаимодействияБРО.ПерсонифицированныеСведенияОЗастрахованныхЛицахДляПечати(ПараметрыПечати);
		...
		Для Каждого СсылкаНаУвольнение Из МассивОбъектов Цикл
			
			Выборка = РезультатЗапроса[2].Выбрать();
			Если Выборка.НайтиСледующий(Новый Структура("Ссылка", СсылкаНаУвольнение)) Тогда
				
				НачалоВыводаМакета = ДокументРезультат.ВысотаТаблицы + 1;
				Пока Выборка.СледующийПоЗначениюПоля("НомерСтроки") Цикл
					
					Если Выборка.Ссылка <> СсылкаНаУвольнение Тогда
						Прервать;
					КонецЕсли;
					...
					Если ДатыКонцаПериодаОрганизаций <> Неопределено Тогда
						...
						Если МакетыРСВ <> Неопределено Тогда
							...
							Если Макеты <> Неопределено Тогда
								...
								Если НачалоВыводаМакета > ДокументРезультат.ВысотаТаблицы + 1 Тогда
									
									УправлениеПечатью.ЗадатьОбластьПечатиДокумента(
										ДокументРезультат, НачалоВыводаМакета, ОбъектыПечати, СсылкаНаУвольнение);
				...
				УправлениеПечатью.ЗадатьОбластьПечатиДокумента(
					ДокументРезультат, ПерваяСтрокаПечатнойФормы, ОбъектыПечати, Выборка.ФизическоеЛицо);
...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, МенеджерВременныхТаблиц, ПериодРегистрации, Организация, ПеремещаемыеСовместители = Неопределено, СтруктураВидовУчета = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		ДобавитьДанныеДляРеестраОтпусков(ДанныеДляПроведения, РеквизитыДляПроведения, МенеджерВременныхТаблиц);
...
Функция РеквизитыДляПроведения(ДокументСсылка) Экспорт
	...
	ВыборкаПоМесяцамНачалаУчета = Результаты[3].Выбрать();
	...
	РеквизитыДляПроведения.ФизическиеЛицаКРасчетуНДФЛиВзносов = ФизическиеЛицаКРасчетуНДФЛиВзносов;
	РеквизитыДляПроведения.ФизическиеЛицаУвольняемыеВСвязиСоСмертью = ФизическиеЛицаУвольняемыеВСвязиСоСмертью;
...
Процедура ДобавитьДанныеДляРеестраОтпусков(ДанныеДляПроведения, РеквизитыДляПроведения, МенеджерВременныхТаблиц)
	...
	Запрос.Текст = 
		...
		|	Документ.Увольнение.ДополнительныеОтпуска КАК УвольнениеДополнительныеОтпуска
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		|		ПО УвольнениеДополнительныеОтпуска.Ссылка.Сотрудник = КадровыеДанные.Сотрудник
		|ГДЕ
		|	УвольнениеДополнительныеОтпуска.Ссылка = &Ссылка
		...
		|	Документ.УвольнениеСписком.Сотрудники КАК УвольнениеСпискомСотрудники
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТКадровыеДанныеСотрудников КАК КадровыеДанные
		|		ПО УвольнениеСпискомСотрудники.Сотрудник = КадровыеДанные.Сотрудник
		|ГДЕ
		|	УвольнениеСпискомСотрудники.Ссылка = &Ссылка
		...
		|	ДанныеДокумента.НачалоПериодаЗаКоторыйПредоставляетсяОтпуск КАК НачалоПериодаЗаКоторыйПредоставляетсяОтпуск,
		|	ДанныеДокумента.КонецПериодаЗаКоторыйПредоставляетсяОтпуск КАК КонецПериодаЗаКоторыйПредоставляетсяОтпуск,
		|	ДанныеДокумента.КоличествоДнейОтпуска КАК КоличествоДнейОтпуска,
		|	ДанныеДокумента.ДатаНачалаПериодаОтсутствия КАК ДатаНачалаПериодаОтсутствия,
		|	ДанныеДокумента.ДатаОкончанияПериодаОтсутствия КАК ДатаОкончанияПериодаОтсутствия,
		...
		|	ДанныеДокумента.ИсправленныйДокумент КАК ИсправленныйДокумент
		|ИЗ
...
Процедура ОчиститьСведенияОДоходахУмершихСотрудников(Движения, ФизическиеЛицаУвольняемыеВСвязиСоСмертью)
	...
	УдалитьЗаписиДвижений(Движения.СведенияОДоходахНДФЛ, ФизическиеЛицаУвольняемыеВСвязиСоСмертью);
	УдалитьЗаписиДвижений(Движения.СведенияОДоходахСтраховыеВзносы, ФизическиеЛицаУвольняемыеВСвязиСоСмертью);
	УдалитьЗаписиДвижений(Движения.ДокументыУчтенныеПриРасчетеНДФЛ, ФизическиеЛицаУвольняемыеВСвязиСоСмертью);
	
КонецПроцедуры

Процедура УдалитьЗаписиДвижений(НаборЗаписей, ФизическиеЛицаУвольняемыеВСвязиСоСмертью)
...
Процедура СформироватьДвиженияМероприятийТрудовойДеятельности(НаборЗаписей, ДанныеДляПроведения) Экспорт
	...
	ЭлектронныеТрудовыеКнижки.СформироватьДвиженияМероприятийТрудовойДеятельности(НаборЗаписей, ДанныеДляПроведения, Истина);
