	...
	Если НовыйДокумент Тогда
		...
		Если Параметры.Свойство("СтрокаСписочногоДокумента") Тогда
			...
			Если Объект.Начисления.Количество() = 0
				И Не Объект.ФиксКоличествоДнейКомпенсацииОтпуска Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	КадровыйУчетФормыРасширенный.ЗапуститьОтложеннуюОбработкуДанных(ТекущийОбъект, Метаданные.Документы.Увольнение.Реквизиты.Сотрудник);
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.РежимЗаписи = РежимЗаписиДокумента.Проведение Тогда
		Если Объект.ДокументРассчитан Тогда
			ИмяОперации = "ПроведениеДокументаУвольнение";
		Иначе
			ИмяОперации = "ПроведениеДокументаУвольнениеКадры";
		КонецЕсли;	
	...
	ПараметрыОповещения.Вставить("Результат", ЭтотОбъект.СсылкаНаОбъект);
...
Процедура Подключаемый_РешениеИнфоНадписьОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АдаптацияУвольнение") Тогда
		...
		МодульАдаптацияУвольнениеКлиент.ОбработкаНавигационнойСсылкиРешения(ЭтотОбъект, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка);
	КонецЕсли;
...
Процедура ОткрытьДенежноеСодержание(Команда)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда
		...
		Модуль.ОткрытьФормуВводаСохраняемогоДенежногоСодержания(СтруктураОбъекта, "Увольнение","ЕжегодныйОтпуск", ЭтаФорма, Оповещение);
	КонецЕсли;
...
Функция ДанныеДокументаДляПроведения(ДанныеДляПроведения)
	...
	ДанныеДляПроведения = Документы.Увольнение.ДанныеДляПроведения(РеквизитыДляПроведения, МенеджерВременныхТаблиц, ДокументОбъект.ПериодРегистрации, ДокументОбъект.Организация);
...
Процедура ЗаполнитьНачисления(ПериодЗаполнения, ИсключаемыеРегистраторы, МенеджерРасчета, ТаблицаНачислений)
	...
	Если Объект.НачислитьЗарплату Тогда
		...
		ДанныеПерерасчета = ПерерасчетЗарплаты.ДанныеПерерасчетаПриУвольненииСотрудника(
			СсылкаНаОбъект, СсылкаНаИсправленныйДокумент, Сотрудники, Объект.Организация, Объект.ДатаУвольнения, ПериодЗаполнения, ИсключаемыеРегистраторы, МенеджерРасчета);
...
Процедура СформироватьЗаписиПерерасчета(МенеджерРасчета)
	...
	Если ЗначениеЗаполнено(СсылкаНаИсправленныйДокумент) Тогда
		...
		ИсправлениеДокументовРасчетЗарплаты.СформироватьДанныеПерерасчетаДляИсправления(Объект.Организация, Объект.ПериодРегистрации, СсылкаНаИсправленныйДокумент, Объект.Начисления, Объект.НачисленияПерерасчет, СсылкаНаОбъект, ОписаниеТаблицы, ПолучитьКонтролируемыеПоля(), ОписаниеДокумента(ЭтотОбъект), СотрудникДляИсправления, Объект.ДоначислитьЗарплатуПриНеобходимости);
...
Процедура ОбновитьУдержаноИтог(Форма)
	...
	Форма.УдержаноНДФЛ = Форма.Объект.НДФЛ.Итог("Налог") + Форма.Объект.ПогашениеЗаймов.Итог("НалогНаМатериальнуюВыгоду");
