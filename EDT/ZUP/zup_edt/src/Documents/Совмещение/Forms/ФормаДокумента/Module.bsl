...
Процедура КоличествоСтавокНажатие(Элемент, СтандартнаяОбработка)
	...
	КадровыйУчетРасширенныйКлиент.ИзменитьКоличествоСтавок(ЭтаФорма, "Объект.КоличествоСтавок", СтандартнаяОбработка, "КоличествоСтавокПредставление", Неопределено);
...
Процедура ЗаполнитьОтражениеВБухучетеПоСотруднику()
	...
	ОтражениеЗарплатыВБухучетеРасширенный.СоздатьВТСведенияОБухучетеЗарплатыСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТКадровыеДанныеСотрудников");
...
Процедура ПрочитатьДанныеОтсутствующегоСотрудника()
	...
	Если ЗначениеЗаполнено(Объект.ОтсутствующийСотрудник) Тогда
		...
		ДанныеСотрудника = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.ОтсутствующийСотрудник, "ФамилияИО,Пол");
		Если ДанныеСотрудника.Количество() > 0 Тогда
			ФизическиеЛицаЗарплатаКадры.Просклонять(ДанныеСотрудника[0].ФамилияИО, 2, ФамилияИООтсутствующегоСотрудника,
				ДанныеСотрудника[0].Пол, ДанныеСотрудника[0].ФизическоеЛицо)
		КонецЕсли;
...
Процедура ПрочитатьДанныеСовмещающегоСотрудника()
	...
	Если ЗначениеЗаполнено(Объект.СовмещающийСотрудник) Тогда
		...
		ДанныеСотрудника = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Объект.СовмещающийСотрудник, "ФамилияИО,Пол");
		Если ДанныеСотрудника.Количество() > 0 Тогда
			ФизическиеЛицаЗарплатаКадры.Просклонять(ДанныеСотрудника[0].ФамилияИО, 2, ФамилияИОСовмещающегоСотрудника,
				ДанныеСотрудника[0].Пол, ДанныеСотрудника[0].ФизическоеЛицо)
		КонецЕсли;
