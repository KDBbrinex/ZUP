...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ЮридическийАдресОрганизации = Неопределено;
...
Процедура ОбработкаЗаполнения(СсылкаОснования, ТекстЗаполнения, СтандартнаяОбработка)
	Если ТипЗнч(СсылкаОснования) = Тип("ДокументСсылка.ПриемНаРаботу") Тогда
		Значения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаОснования, "Организация, Сотрудник");
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если КадровыеДанные <> Неопределено
		...
		ЗарплатаКадрыОтображениеОшибок.СообщитьОбОшибке(Отказ, Текст, "Дата");
	КонецЕсли;
	
	// Проверка что на дату нет других действующих согласий.
	СогласиеИОтзыв = РегистрыСведений.СогласияНаУведомленияОбЭЛН.ПолучитьПаруСогласиеИОтзывНаДату(Организация, Сотрудник, Дата, Ссылка);
	Если СогласиеИОтзыв <> Неопределено 
		...
		СообщенияПользователюБЗК.СообщитьОбОшибкеВОбъекте(Отказ, ЭтотОбъект, "Дата", ТекстОшибки);
	КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	РегистрыСведений.СогласияНаУведомленияОбЭЛН.ОбновитьСведенияОСогласии(Организация, Сотрудник);
	
	ДоЗаписи = ДополнительныеСвойства.ЗначенияРеквизитовДоЗаписи;
	Если ДоЗаписи <> Неопределено
		И (ДоЗаписи.Сотрудник <> Сотрудник Или ДоЗаписи.Организация <> Организация) Тогда
		РегистрыСведений.СогласияНаУведомленияОбЭЛН.ОбновитьСведенияОСогласии(ДоЗаписи.Организация, ДоЗаписи.Сотрудник);
	КонецЕсли;
...
Функция КритичныеОшибкиЗаполнения() Экспорт
	...
	Если Не ПометкаУдаления Тогда
		...
		Если ЗначениеЗаполнено(ДругоеСогласиеНаДату) Тогда
			Текст = СтрШаблон(
				НСтр("ru = 'На %1 уже создано %2.'"),
				Формат(Дата, "ДЛФ=D"),
				ДругоеСогласиеНаДату);
			Ошибки.Добавить(СообщенияПользователюБЗК.СообщениеОбъекта(Текст, ЭтотОбъект, "Дата"));
		КонецЕсли;
	КонецЕсли;
...
Функция ОбновитьДанныеОрганизации(ПараметрыФиксации)
	РеквизитыДокумента = Новый Структура("НаименованиеОрганизации, ЮридическийАдресОрганизации");
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		ЗащитаПерсональныхДанныхПереопределяемый.ДополнитьДанныеОрганизацииОператораПерсональныхДанных(Организация, ДанныеОрганизации, Дата);
		// АПК:277-вкл
		РеквизитыДокумента.НаименованиеОрганизации = ДанныеОрганизации.НаименованиеОрганизации;
		РеквизитыДокумента.ЮридическийАдресОрганизации = ДанныеОрганизации.АдресОрганизации;
	КонецЕсли;
