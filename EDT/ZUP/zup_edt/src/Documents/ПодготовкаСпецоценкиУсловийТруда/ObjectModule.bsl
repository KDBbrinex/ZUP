...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		НайденныеСтроки = Сотрудники.НайтиСтроки(
...
Процедура ЗаполнитьПереченьРабочихМест() Экспорт
	...
	РезультатыЗапроса = ОхранаТруда.ДанныеПоРабочимМестам(Организация, ДатаНачала, ДатаОкончания);
	...
	Сотрудники.Загрузить(РезультатыЗапроса.Получить(РезультатыЗапроса.ВГраница()).Выгрузить());
	Если Комиссия.Количество() = 0 Тогда
		Комиссия.Загрузить(РезультатыЗапроса.Получить(РезультатыЗапроса.ВГраница()-2).Выгрузить());
		Если Комиссия.Количество() = 0 Тогда
			Если ЗначениеЗаполнено(Руководитель) Тогда
				НоваяСтрокаКомиссии = Комиссия.Добавить();
				НоваяСтрокаКомиссии.ЧленКомиссии = Руководитель;
				НоваяСтрокаКомиссии.Должность = ДолжностьРуководителя;
				НоваяСтрокаКомиссии.РольВКомиссии = Перечисления.РолиЧленовКомиссииОхраныТруда.Председатель;
			КонецЕсли;
			Если ЗначениеЗаполнено(ОтветственныйЗаОхрануТруда) Тогда
				НоваяСтрокаКомиссии = Комиссия.Добавить();
				НоваяСтрокаКомиссии.ЧленКомиссии = ОтветственныйЗаОхрануТруда;
				НоваяСтрокаКомиссии.Должность = ДолжностьОтветственногоЗаОхрануТруда;
