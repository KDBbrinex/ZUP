...
Функция ФайлДанныхЭД(СообщениеОбмена, ИдентификаторПечати = Неопределено, ДляПечати = Ложь) Экспорт
	...
	Если ДопИнформацияПоЭД <> Неопределено И ДопИнформацияПоЭД <> Неопределено
		...
		Если ОбменСБанкамиСлужебный.ЭтоЗарплатныйПроект(РеквизитыЭД.ВидЭД) Тогда
		...
		Иначе
			ТабличныйДокумент = СформироватьПечатнуюФормуЭД(
				СообщениеОбмена, РеквизитыЭД.ВидЭД, ДанныеЭД, ИдентификаторПечати, ДляПечати);
		КонецЕсли;
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|	Документ.СообщениеОбменСБанками КАК СообщениеОбменСБанками
		|ГДЕ
		|	НЕ СообщениеОбменСБанками.ПометкаУдаления
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ДополнительныеПараметры.ДополнительныеИсточникиДанных.Вставить("НастройкаОбмена");
	
	ВыборкаСообщенийОбмена = ОбновлениеИнформационнойБазы.ВыбратьСсылкиДляОбработки(
	...
	Пока ВыборкаСообщенийОбмена.Следующий() Цикл
		...
		Попытка
			...
			ЭлементБлокировки.УстановитьЗначение("Ссылка", ВыборкаСообщенийОбмена.Ссылка);
			ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
			Блокировка.Заблокировать();
			
			Объект = ВыборкаСообщенийОбмена.Ссылка.ПолучитьОбъект();
			...
			Иначе
				ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ВыборкаСообщенийОбмена.Ссылка);
			КонецЕсли;
		...
		Исключение
			...
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%Регистратор%", ВыборкаСообщенийОбмена.Ссылка);
...
// 
// Возвращаемое значение:
//  ТаблицаЗначение - информация для штампа. Содержит колонки:
			...
			Если ЗначениеЗаполнено(ПрисоединенныйФайл) Тогда
				...
				Для Каждого СтруктураПодписи Из ВыборкаПодписей Цикл
					...
					Если ЗначениеЗаполнено(СертификатПодписи) Тогда
						ЭлектронноеВзаимодействиеСлужебный.ОбновитьМаршрутПриПодписании(
...
Функция СформироватьПечатнуюФормуЭД(СообщениеОбмена, ВидЭД, ДанныеФайла, ИдентификаторПечати, ДляПечати)
	...
	Если ЗначениеЗаполнено(ИдентификаторДокумента) И ИдентификаторПечати <> "КомплектДокументов"  Тогда
		МакетИдентификатораДокумента = ЭлектронноеВзаимодействиеСлужебный.ПолучитьМакетИдентификатораДокумента();
...
Процедура ЗаполнитьТабличныйДокументПлатежноеТребование_ЭД(ТабличныйДокумент, Реквизиты, СообщениеОбмена)
	...
	ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, СообщениеОбмена);
...
Процедура ЗаполнитьТабличныйДокументПлатежноеПоручениеИсходящее_ЭД(ТабличныйДокумент, Реквизиты, СообщениеОбмена)
	...
	Если ЗначениеЗаполнено(Квитанция) Тогда
		...
		Если НЕ ЗначениеЗаполнено(ДатаПоступления) Тогда
			ДатаПоступления = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ДатаИсполненияБанком");
		КонецЕсли;
		Если НЕ ЗначениеЗаполнено(ДатаПоступления) Тогда
		...
		ДатаСписания = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ДатаСписанияСоСчетаПлательщика");
...
//
// Параметры:
//  СообщениеОбмена - ДокументСсылка.СообщенияОбменСБанками - сообщение обмена с банком
...
Процедура ДобавитьПлатежноеПоручениеИзВыписки(ТабличныйДокумент, Операция, ПлатежныеДокументыВИБ, ПрограммаБанка)
	...
	Если ЗначениеЗаполнено(ИсходноеПлатежноеПоручение) Тогда
		ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, ИсходноеПлатежноеПоручение);
	Иначе
		ДобавитьОбластьШтампа(ТабличныйДокумент, Макет, Операция, НаименованиеБанкаПлательщика, ПлательщикБИКБанка);
	КонецЕсли;
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
...
Процедура ДобавитьОбластьШтампа(ТабличныйДокумент, Макет, Операция, НаименованиеБанкаПлательщика, ПлательщикБИКБанка)
	...
	ДанныеШтампа = Новый Структура("НаименованиеБанка, Отделение, Офис, БИК, Статус, ДатаОперации");
...
Процедура ДобавитьПлатежноеТребованиеИзВыписки(ТабличныйДокумент, Операция, ПлатежныеДокументыВИБ)
	...
	Если ЗначениеЗаполнено(ИсходноеПлатежноеТребование) Тогда
		ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, ИсходноеПлатежноеТребование);
	Иначе
		ДобавитьОбластьШтампа(ТабличныйДокумент, Макет, Операция, НаименованиеБанкаПлательщика, ПлательщикБИКБанка);
	КонецЕсли;
	ТабличныйДокумент.ОриентацияСтраницы = ОриентацияСтраницы.Портрет;
...
Процедура ЗаполнитьОбластьОтметок(Макет, ТабличныйДокумент, СообщениеОбмена)
	...
	Если ЕстьШтамп ИЛИ ПодписанЭП ИЛИ ДокументОтправленВБанк Тогда
		...
		ИначеЕсли ДокументОтправленВБанк Тогда  // заполняется штамп по состоянию отправленного документа
			РеквизитыБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(РеквизитыСообщенияОбмена.Банк, "Наименование, Код");
			ОбластьШтамп = Макет.ПолучитьОбласть("Отметки|Штамп");
			ОбластьШтамп.Параметры.НаименованиеБанка = РеквизитыБанка.Наименование;
			ОбластьШтамп.Параметры.БИК = РеквизитыБанка.Код;
