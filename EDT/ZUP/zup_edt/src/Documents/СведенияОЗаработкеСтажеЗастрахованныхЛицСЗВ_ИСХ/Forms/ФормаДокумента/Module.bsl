...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ.Пустая() Тогда
		Если Не ЗначениеЗаполнено(Объект.ОтчетныйПериод) Тогда 
			...
			Объект.ОкончаниеОтчетногоПериода = '20160331';
		КонецЕсли;
...
Процедура ОтчетныйПериодРегулирование(Элемент, Направление, СтандартнаяОбработка)
	...
	ПерсонифицированныйУчетКлиент.ПериодОтчетаРегулирование(Объект.ОтчетныйПериод, Объект.ОкончаниеОтчетногоПериода, ПериодСтрока, Направление, , , '20160101', '20161231');
...
Процедура ЗаполнитьДанныеОСтажеСотрудников(ИдентификаторыСотрудников)
	...
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	Документы.СведенияОЗаработкеСтажеЗастрахованныхЛицСЗВ_ИСХ.СоздатьВТДанныеСтажаПФР(МенеджерВременныхТаблиц,
		Объект.Организация,  Объект.ОтчетныйПериод, Объект.ОкончаниеОтчетногоПериода, СписокСотрудников, Объект.Ссылка);
