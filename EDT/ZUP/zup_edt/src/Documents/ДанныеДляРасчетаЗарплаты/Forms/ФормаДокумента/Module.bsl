...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	Если Не Элементы.ПериодСтраницы.Видимость Тогда
		Объект.Период          = НачалоДня(Объект.Дата);
		Объект.ПериодОкончания = КонецДня(Объект.Дата);
	ИначеЕсли Не Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.ПериодСтраница Тогда
		Объект.ПериодОкончания = КонецМесяца(Объект.Период);
	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	Если НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяПоСотрудникам Тогда
		...
		СдельныйЗаработок = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СдельныйЗаработок");
		ДатаРегистрации = Объект.Дата;
		Если НастройкиВидаДокумента.ВыполненныеРаботыВводятсяСводно Тогда
	...
	Если НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты
		Или (НастройкиВидаДокумента.ВидыРаботЗаполняютсяВДокументе И Не НастройкиВидаДокумента.ВыполненныеРаботыВводятсяСводно) Тогда
	...
	Если НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты Тогда
		СоставКлючаПроверкиДублей.Добавить("Период");
		СоставКлючаПроверкиДублей.Добавить("ПериодОкончания");
	КонецЕсли;
	ПроверитьДублиСтрок("ДанныеСводно", СтрСоединить(СоставКлючаПроверкиДублей, ","), Отказ);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты И
		ВидыВремени.Количество() > 0 И
		Не НастройкиВидаДокумента.ВремяВводитсяСводно Тогда
		Период = Неопределено;
	...
	Если Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.ПериодСтраница Тогда
		Если ЗначениеЗаполнено(Объект.ПериодОкончания) И Объект.Период > Объект.ПериодОкончания Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(Текст, , "Период", "Объект", Отказ);
		КонецЕсли;
	КонецЕсли;
...
Процедура ДатаОкончанияПриИзмененииНаСервере()
	ЗаполнитьТарифныеСтавкиИОтработанноеВремя();
...
Процедура ДатаНачалаПриИзмененииНаСервере()
	...
	Объект.ИспользоватьПриРасчетеПервойПоловиныМесяца = 
		Объект.Период < НачалоМесяца(Объект.Период) + 15 * 86400;
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДанныеДетально
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыДанныеСводно
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВыполненныеРаботыПоСотрудникам
...
Процедура ИсполнителиРаботОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	ОбработкаПодбораСотрудниковНаСервере(ВыбранноеЗначение, ПутьКТаблицеСотрудников, Истина);
КонецПроцедуры

&НаКлиенте
Процедура ИсполнителиРаботСотрудникПриИзменении(Элемент)
	ВыделенныеСтроки = Элементы.ИсполнителиРабот.ВыделенныеСтроки;
	ЗаполнитьТарифныеСтавкиИОтработанноеВремя(ВыделенныеСтроки);
КонецПроцедуры
...
Процедура ДанныеСводноПодбор(Команда)
	Если НастройкиВидаДокумента.НесколькоОрганизаций Тогда
	...
	Если НастройкиВидаДокумента.НесколькоПодразделений Тогда
	...
	Если НастройкиВидаДокумента.НесколькоСотрудников Тогда
		ПараметрыОткрытия = Неопределено;
...
Процедура ЗаполнитьПредставленияВСтроках()
	Если Не ИспользуетсяТаблицаДетально Тогда
		Возврат;
	КонецЕсли;
...
Процедура НастроитьЭлементыВвода()
	...
	НастройкиВидаДокумента = Новый ФиксированнаяСтруктура(Справочники.ВидыДокументовВводДанныхДляРасчетаЗарплаты.НастройкиВидаДокумента(Объект.ВидДокумента));
	...
	Если ЗначениеЗаполнено(НастройкиВидаДокумента.Подсказка) Тогда
		Элементы.ПодсказкаЗаполняемаяВШаблонеДокумента.Видимость = Истина;
		Элементы.ПодсказкаЗаполняемаяВШаблонеДокумента.Заголовок = НастройкиВидаДокумента.Подсказка;
	Иначе
		Элементы.ПодсказкаЗаполняемаяВШаблонеДокумента.Видимость = Ложь;
	КонецЕсли;
	
	ЗаполнитьСведенияОПоказателях(НастройкиВидаДокумента);
	ЗаполнитьСведенияОВидахВремени();
	ЗаполнитьСведенияОВидахРабот();
	
	ЭтоВводПостоянныхПоказателей = НастройкиВидаДокумента.ЭтоВводПостоянныхПоказателей;
	...
	Элементы.Организация.Видимость = Не НастройкиВидаДокумента.НесколькоОрганизаций;
	Элементы.Подразделение.Видимость = НастройкиВидаДокумента.ПоказыватьПодразделение;
	Элементы.Сотрудник.Видимость = Не НастройкиВидаДокумента.НесколькоСотрудников
	...
	Если ЗначениеЗаполнено(НастройкиВидаДокумента.Организация)
		И ЗначениеЗаполнено(Объект.Организация) Тогда
		Элементы.Организация.Вид = ВидПоляФормы.ПолеНадписи;
		Элементы.Организация.Шрифт = ЖирныйШрифт;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(НастройкиВидаДокумента.Подразделение)
	...
	Если Элементы.Организация.Видимость
		И Не ЗначениеЗаполнено(НастройкиВидаДокумента.Организация) Тогда
		МассивПроверяемыхПолей.Вставить("Объект.Организация", "Организация");
	КонецЕсли;
	
	Если Элементы.Подразделение.Видимость
		И НастройкиВидаДокумента.ПроверятьЗаполнениеПодразделения
		И Не ЗначениеЗаполнено(НастройкиВидаДокумента.Подразделение) Тогда
	...
	Если НастройкиВидаДокумента.ВидыРаботЗаполняютсяВДокументе Тогда
		Страница = Элементы.ВидыРаботЗаполняютсяВДокументеСтраница;
	Иначе
		ИспользуетсяТаблицаДетально = Не НастройкиВидаДокумента.ЭтоВводПостоянныхПоказателей
			И НастройкиВидаДокумента.НесколькоСотрудников
			И (
				(ЕстьРазовыеПоказателиСотрудников
					И (ЕстьОперативныеПоказателиСотрудников
						...
						Или (ВидыВремени.Количество() > 0
							И Не НастройкиВидаДокумента.ВремяВводитсяСводно
							И НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты)))
				Или (ВидыВремени.Количество() > 0
						И Не НастройкиВидаДокумента.ВремяВводитсяСводно
						И НастройкиВидаДокумента.ВремяВводитсяЗаМесяц)
			);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		Если НастройкиВидаДокумента.НесколькоОрганизаций Тогда
			Если ИспользуетсяТаблицаДетально Тогда
				ЭлементОрганизация = Элементы.ДанныеДетальноОбъект;
			Иначе
				ЭлементОрганизация = Элементы.ДанныеСводноОбъект;
			КонецЕсли;
		Иначе
			ЭлементОрганизация = Элементы.Организация;
		КонецЕсли;
		Модуль = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		Модуль.ДокументДанныеДляРасчетаЗарплатыПриНастройкеЭлементовФормы(ЭтотОбъект, ЭлементОрганизация);
	КонецЕсли;
	...
	Иначе
		Если (ЕстьОперативныеПоказатели
				Или ВидыРабот.Количество() > 0
				Или (Не НастройкиВидаДокумента.ВремяВводитсяСводно И Не ИспользуетсяТаблицаДетально))
	...
	ВидимостьЗаполнить = Не НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты;
	...
	Элементы.ПериодСтраницы.Видимость = Не НастройкиВидаДокумента.ВидыРаботЗаполняютсяВДокументе
		И (Не НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты Или ИспользуетсяТаблицаДетально)
		Или НастройкиВидаДокумента.ВыполненныеРаботыВводятсяСводно;
	Элементы.ДанныеСводноГруппаПериод.Видимость = НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты И ЭтоВводПостоянныхПоказателей;
	Элементы.ДанныеСводноДата.Видимость = НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты И Не ЭтоВводПостоянныхПоказателей;
	
	Элементы.ДатаОкончания.Видимость = НастройкиВидаДокумента.ИспользоватьПериодОкончания Или НастройкиВидаДокумента.ВыполненныеРаботыВводятсяСводно;
	Элементы.ДанныеСводноПериодОкончания.Видимость = НастройкиВидаДокумента.ИспользоватьПериодОкончания;
	...
	Элементы.СтатьяФинансирования.Видимость =
		(НастройкиВидаДокумента.РежимВводаСтатьиФинансирования = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВШапке);
	Элементы.СпособОтраженияЗарплатыВБухучете.Видимость =
		(НастройкиВидаДокумента.РежимВводаСпособаОтраженияЗарплатыВБухучете = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВШапке);
	Элементы.ОтношениеКЕНВД.Видимость =
		(НастройкиВидаДокумента.РежимВводаОтношенияКЕНВД = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВШапке);
	Элементы.ПодразделениеДляУчетаЗатрат.Видимость =
		(НастройкиВидаДокумента.РежимВводаПодразделенияУчетаЗатрат = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВШапке);
	...
	Если НастройкиВидаДокумента.ВидыРаботЗаполняютсяВДокументе Тогда
		...
		Если НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяПоСотрудникам Тогда
			...
			НастроитьЭлементыРаспределенияВыполненныхРабот(НастройкиВидаДокумента);
		Иначе
			Элементы.ВыполненныеРаботыСтраницы.ТекущаяСтраница = Элементы.ВыполненныеРаботыВводятсяПоСотрудникамСтраница;
			НастроитьЭлементыВводаВыполненныхРаботПоСотрудникам(НастройкиВидаДокумента);
		КонецЕсли;
		Возврат;
	КонецЕсли;
	...
	Элементы.ДанныеСводно.ИзменятьПорядокСтрок = Не НастройкиВидаДокумента.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеСводно.ИзменятьСоставСтрок = Не НастройкиВидаДокумента.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеСводно.ПоложениеКоманднойПанели = ?(НастройкиВидаДокумента.ЗаполнятьСписокОбъектов,
														ПоложениеКоманднойПанелиЭлементаФормы.Нет,
														ПоложениеКоманднойПанелиЭлементаФормы.Авто);
	Элементы.ДанныеСводноОбъект.ТолькоПросмотр = НастройкиВидаДокумента.ЗаполнятьСписокОбъектов;
	Если НастройкиВидаДокумента.ЗаполнятьСписокОбъектов Тогда
		Элементы.ДанныеСводноГруппа.Заголовок = Неопределено;
	КонецЕсли;
	
	// ...И детальных данных по дням.
	Элементы.ДанныеДетально.ИзменятьСоставСтрок = Не НастройкиВидаДокумента.ЗаполнятьСписокОбъектов;
	Элементы.ДанныеДетально.ПоложениеКоманднойПанели = ?(НастройкиВидаДокумента.ЗаполнятьСписокОбъектов,
														ПоложениеКоманднойПанелиЭлементаФормы.Нет,
														ПоложениеКоманднойПанелиЭлементаФормы.Авто);
	Элементы.ДанныеДетальноОбъект.ТолькоПросмотр = НастройкиВидаДокумента.ЗаполнятьСписокОбъектов;
	...
	Элементы.ДанныеТаблицей.Видимость = ИспользуетсяТаблицаДетально
										Или НастройкиВидаДокумента.НесколькоОрганизаций
										Или НастройкиВидаДокумента.НесколькоПодразделений
										Или НастройкиВидаДокумента.НесколькоСотрудников
										Или (НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты И ЭтоВводПостоянныхПоказателей);
	...
	ОбъектыВСтроках = НастройкиВидаДокумента.НесколькоОрганизаций
					Или НастройкиВидаДокумента.НесколькоПодразделений
					Или НастройкиВидаДокумента.НесколькоСотрудников;
...
&НаСервере
Процедура НастроитьЭлементыВводаВыполненныхРаботПоСотрудникам(НастройкиВидаДокумента)
	Элементы.ВыполненныеРаботыПоСотрудникамПериод.Видимость = Не НастройкиВидаДокумента.ВыполненныеРаботыВводятсяСводно;
	
	Элементы.ВыполненныеРаботыПоСотрудникамСтатьяФинансирования.Видимость =
		(НастройкиВидаДокумента.РежимВводаСтатьиФинансирования = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти);
	Элементы.ВыполненныеРаботыПоСотрудникамСпособОтраженияЗарплатыВБухучете.Видимость =
		(НастройкиВидаДокумента.РежимВводаСпособаОтраженияЗарплатыВБухучете = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти);
	Элементы.ВыполненныеРаботыПоСотрудникамОтношениеКЕНВД.Видимость =
		(НастройкиВидаДокумента.РежимВводаОтношенияКЕНВД = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти);
	Элементы.ВыполненныеРаботыПоСотрудникамПодразделениеУчетаЗатрат.Видимость =
		(НастройкиВидаДокумента.РежимВводаПодразделенияУчетаЗатрат = Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти);
КонецПроцедуры

&НаСервере
Процедура НастроитьЭлементыРаспределенияВыполненныхРабот(НастройкиВидаДокумента)
	...
	Элементы.ИсполнителиРаботСуммаПоТарифу.Видимость = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСверхТарифа;
	
	Элементы.ИсполнителиРаботСотрудник.ОграничениеТипа = Новый ОписаниеТипов("СправочникСсылка.Сотрудники");
	Элементы.ИсполнителиРаботКоэффициент.Видимость = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСУчетомКоэффициентов;
	
	ЖирныйШрифт = Новый Шрифт(Элементы.ИсполнителиРабот.Шрифт, , , Истина);
	
	Элементы.ИсполнителиРаботТарифнаяСтавка.Видимость = НастройкиВидаДокумента.ПоказыватьТарифнуюСтавку;
	Если НастройкиВидаДокумента.ПоказыватьТарифнуюСтавку Тогда
	...
	Элементы.ИсполнителиРаботОтработанноеВремяГруппа.Видимость = НастройкиВидаДокумента.ПоказыватьОтработанноеВремя;
	Если НастройкиВидаДокумента.ПоказыватьОтработанноеВремя Тогда
		ЭлементОформления = УсловноеОформление.Элементы.Добавить();
		// Поля
		ОформляемоеПоле = ЭлементОформления.Поля.Элементы.Добавить();
...
Процедура УстановитьСвязиПараметровВыбора()
	...
	Если НастройкиВидаДокумента.ПоказыватьПодразделение Тогда
...
Процедура ЗаполнитьДанныеФормыПоОрганизации()
	...
	ЗапрашиваемыеЗначения = Новый Структура("Организация, МесяцРасчета", "Объект.Организация", "Объект.Период");
...
Процедура ЗаполнитьСписокОбъектов(Заполнять)
	...
	Если Не НастройкиВидаДокумента.ЗаполнятьСписокОбъектов И Не Заполнять Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПриИзмененииПериода()
	Если ИспользуетсяТаблицаДетально
		Или НастройкиВидаДокумента.НесколькоСотрудников И НастройкиВидаДокумента.ЗаполнятьСписокОбъектов Тогда
...
Процедура ПриИзмененииПериодаНаСервере()
	...
	Если НастройкиВидаДокумента.НесколькоСотрудников
		И НастройкиВидаДокумента.ЗаполнятьСписокОбъектов Тогда
...
Процедура УточнитьСписокСотрудников()
	...
	Если Не ЗначениеЗаполнено(Объект.Организация)
		Или НастройкиВидаДокумента.ПоказыватьПодразделение И Не ЗначениеЗаполнено(Объект.Подразделение) Тогда
		Возврат;
	КонецЕсли;
	...
	ИмяТаблицы = ?(ИспользуетсяТаблицаДетально, "ДанныеДетально", "ДанныеСводно");
...
&НаСервере
Процедура ОбработкаПодбораСотрудниковНаСервере(ВыбранноеЗначение, ПутьКТаблицеСотрудников, ЗаполнятьТаблицуИсполнителиРабот = Ложь)
	...
	Если ЗаполнятьТаблицуИсполнителиРабот Тогда
		ЗаполнитьТарифныеСтавкиИОтработанноеВремя(ИдентификаторыСтрок);
		РаспределитьСтоимостьРаботПоСотрудникам();
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура ЗаполнитьСведенияОПоказателях(НастройкиВидаДокумента)
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	Запрос.УстановитьПараметр("ЗначенияПоказателейВводятсяНаРазныеДаты", НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты);
	Запрос.УстановитьПараметр("НесколькоОрганизаций",   НастройкиВидаДокумента.НесколькоОрганизаций);
	Запрос.УстановитьПараметр("НесколькоПодразделений", НастройкиВидаДокумента.НесколькоПодразделений);
	Запрос.УстановитьПараметр("НесколькоСотрудников",   НастройкиВидаДокумента.НесколькоСотрудников);
...
Процедура ПрочитатьЗначенияПоказателей(ТекущийОбъект = Неопределено)
	...
	Для Каждого СтрокаОбъекты Из ТекущийОбъект.ЗначенияПоказателей Цикл
		...
		Если Не ИспользуетсяТаблицаДетально
			И НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты Тогда
	...
	Для Каждого СведенияОПоказателе Из НайденныеСтроки Цикл
		// Итоги по колонкам:
		ЗаполнитьИтогоПоКолонке(ЭтотОбъект, ?(ИспользуетсяТаблицаДетально, ДанныеДетально, ДанныеСводно), СведенияОПоказателе.ИмяРеквизита);
	КонецЦикла;
КонецПроцедуры
	...
	Для Каждого СведенияОПоказателе Из ПоказателиРасчетаЗарплаты Цикл
		...
		Иначе
			...
			Для Каждого СтрокаКоллекции Из ЭтотОбъект[СведенияОПоказателе.ИмяТаблицы] Цикл
				...
				Если НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты
					И Не ИспользуетсяТаблицаДетально Тогда
...
Функция ОбъектВводитсяВТаблице(НазначениеПоказателя)
	Возврат НазначениеПоказателя = Перечисления.НазначенияПоказателейРасчетаЗарплаты.ДляОрганизации
			И НастройкиВидаДокумента.НесколькоОрганизаций
		Или НазначениеПоказателя = Перечисления.НазначенияПоказателейРасчетаЗарплаты.ДляПодразделения
			И НастройкиВидаДокумента.НесколькоПодразделений
...
Процедура ПрочитатьДанныеОВремени(ТекущийОбъект = Неопределено)
	...
	// Заполняем таблицы формы, читая табличные части:
...
Процедура ЗаполнитьТабличныеЧастиВремени(ТекущийОбъект, Отказ)
	...
	Для Каждого СведенияОВидеВремени Из ВидыВремени Цикл
		...
		Если ПустаяСтрока(СведенияОВидеВремени.ИмяТаблицы) Тогда
			Если НастройкиВидаДокумента.ВремяВводитсяСводно Тогда
...
Процедура НастроитьЭлементыСводногоВводаДанныхОВремени(ИмяТаблицыДанных = "")
	ВводятсяТаблицей = НастройкиВидаДокумента.НесколькоСотрудников
		Или НастройкиВидаДокумента.ЗначенияПоказателейВводятсяНаРазныеДаты
		Или ВидыВремени.Количество() > 0 И НастройкиВидаДокумента.ВремяВводитсяЗаМесяц И НастройкиВидаДокумента.НесколькоСотрудников;
	...
	Если Не ИспользуетсяТаблицаДетально И ВводятсяТаблицей И ВидыВремени.Количество() > 0 Тогда
	...
	Для Каждого СтрокаВидаВремени Из ВидыВремени Цикл
		...
		ПоказыватьПолеДней = НастройкиВидаДокумента.ВремяВводитсяСводно;
	...
	Если Не ИспользуетсяТаблицаДетально И ВводятсяТаблицей И ВидыВремени.Количество() > 0 Тогда
		...
		ГруппаВидВремени = Элементы.Добавить(ИмяГруппыВидВремени, Тип("ГруппаФормы"), ГруппаСводноВремя);
		ГруппаВидВремени.Заголовок = ?(НастройкиВидаДокумента.ВремяВводитсяСводно, "Всего", НСтр("ru = 'Всего часов'"));
		ГруппаВидВремени.Вид = ВидГруппыФормы.ГруппаКолонок;
		ГруппаВидВремени.ОтображатьВШапке = Истина;
		ГруппаВидВремени.ГоризонтальноеПоложениеВШапке = ГоризонтальноеПоложениеЭлемента.Центр;
		ГруппаВидВремени.Группировка = ГруппировкаКолонок.Горизонтальная;
		...
		ПолеВводаИтогоЧасов.ОтображатьВШапке = НастройкиВидаДокумента.ВремяВводитсяСводно;
	...
	Для Каждого СведенияВидаРабот Из ВидыРабот Цикл
		...
		Иначе
			...
			Для Каждого СтрокаКоллекции Из ЭтотОбъект[СведенияВидаРабот.ИмяТаблицы] Цикл
				...
				КонецЕсли;
				НоваяСтрока = ТекущийОбъект.ВыполненныеРаботы.Добавить();
				Если Не НастройкиВидаДокумента.НесколькоСотрудников Тогда
					// Если в таблице используются даты, а не объекты.
...
Процедура РаспределитьСтоимостьРаботПоСотрудникам(ОтменятьИсправления = Ложь)
	УчитыватьКоэффициенты        = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСУчетомКоэффициентов;
	УчитыватьТарифныеСтавки      = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСУчетомТарифныхСтавок;
	УчитыватьОтработанноеВремя   = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСУчетомОтработанногоВремени;
	РаспределятьСуммуСверхТарифа = НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСверхТарифа;
...
Процедура ЗаполнитьСтрокуВыполненныхРабот(СтрокаТаблицы, ЗаполнятьРасценкуИСумму, ЗаполнятьБухучет)
	...
	Если ЗаполнятьБухучет И ЗначениеЗаполнено(СтрокаТаблицы.ВидРабот) Тогда
		...
		Если НастройкиВидаДокумента.РежимВводаСтатьиФинансирования <> Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти Тогда
			НастройкиУчета.СтатьяФинансирования = Неопределено;
		КонецЕсли;
		Если НастройкиВидаДокумента.РежимВводаСпособаОтраженияЗарплатыВБухучете <> Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти Тогда
			НастройкиУчета.СпособОтраженияЗарплатыВБухучете = Неопределено;
		КонецЕсли;
		Если НастройкиВидаДокумента.РежимВводаОтношенияКЕНВД <> Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти Тогда
			НастройкиУчета.ОтношениеКЕНВД = Неопределено;
		КонецЕсли;
		Если НастройкиВидаДокумента.РежимВводаПодразделенияУчетаЗатрат <> Перечисления.РежимыВводаБухучетаВДанныхДляРасчетаЗарплаты.ВводитьВТабличнойЧасти Тогда
...
&НаСервере
Процедура ЗаполнитьТарифныеСтавкиИОтработанноеВремя(ВыделенныеСтроки = Неопределено, ОтменятьИсправления = Ложь)
	ПараметрыЗаполнения = ПараметрыЗаполненияСведенийОСотрудниках(ВыделенныеСтроки, ОтменятьИсправления);
	Если ПараметрыЗаполнения.СтрокиТаблицы.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	ЗаполнитьПериодыРаботы(ПараметрыЗаполнения);
	ЗаполнитьОтработанноеВремя(ПараметрыЗаполнения);
	ЗаполнитьТарифныеСтавки(ПараметрыЗаполнения);
КонецПроцедуры

&НаСервере
Функция ПараметрыЗаполненияСведенийОСотрудниках(ВыделенныеСтроки, ОтменятьИсправления)
	ПараметрыЗаполнения = Новый Структура("СтрокиТаблицы, МассивСотрудников, МенеджерВТ, ДатыДокумента");
	ПараметрыЗаполнения.ДатыДокумента = ДатыДокумента();
	ПараметрыЗаполнения.СтрокиТаблицы = Новый Массив;
	ПараметрыЗаполнения.МассивСотрудников = Новый Массив;
	ПараметрыЗаполнения.МенеджерВТ = Новый МенеджерВременныхТаблиц;
	...
	Если ВыделенныеСтроки = Неопределено Тогда
		Для Каждого СтрокаТаблицы Из Объект.ЗначенияПоказателей Цикл
			...
			КонецЕсли;
			ПараметрыЗаполнения.СтрокиТаблицы.Добавить(СтрокаТаблицы);
			ПараметрыЗаполнения.МассивСотрудников.Добавить(СтрокаТаблицы.Объект);
		КонецЦикла;
	Иначе
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = ПараметрыЗаполнения.МенеджерВТ;
	Запрос.УстановитьПараметр("МассивСотрудников", ПараметрыЗаполнения.МассивСотрудников);
	Запрос.УстановитьПараметр("ДатаНачала",        ПараметрыЗаполнения.ДатыДокумента.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания",     ПараметрыЗаполнения.ДатыДокумента.ДатаОкончания);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Сотрудники.Ссылка КАК Сотрудник,
	...
	ПараметрыВТ.ДатаНачала         = ПараметрыЗаполнения.ДатыДокумента.ДатаНачала;
	ПараметрыВТ.ДатаОкончания      = ПараметрыЗаполнения.ДатыДокумента.ДатаОкончания;
	ПараметрыВТ.МесяцДатаНачала    = НачалоМесяца(ПараметрыЗаполнения.ДатыДокумента.ДатаНачала);
	ПараметрыВТ.МесяцДатаОкончания = КонецМесяца(ПараметрыЗаполнения.ДатыДокумента.ДатаОкончания);
	ПараметрыВТ.ДатаАктуальности   = ПараметрыЗаполнения.ДатыДокумента.ДатаНачала;
	ПараметрыВТ.Организация        = Объект.Организация;
	ПараметрыВТ.Подразделение      = Объект.Подразделение;
	
	УчетРабочегоВремениРасширенный.СоздатьВТДанныеУчетаВремениИСостоянийСотрудников(ПараметрыЗаполнения.МенеджерВТ, Истина, ПараметрыВТ);
	
	Возврат ПараметрыЗаполнения;
КонецФункции

&НаСервере
Процедура ЗаполнитьПериодыРаботы(ПараметрыЗаполнения)
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = ПараметрыЗаполнения.МенеджерВТ;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ДанныеУчетаВремени.Сотрудник КАК Сотрудник,
	|	МИНИМУМ(ДанныеУчетаВремени.Дата) КАК Период,
	|	МАКСИМУМ(ДанныеУчетаВремени.Дата) КАК ПериодОкончания
	|ИЗ
	...
	Для Каждого СтрокаТаблицы Из ПараметрыЗаполнения.СтрокиТаблицы Цикл
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(СтрокаТаблицы.Объект, "Сотрудник") Тогда
			СтрокаТаблицы.Период = Макс(Выборка.Период, ПараметрыЗаполнения.ДатыДокумента.ДатаНачала);
			СтрокаТаблицы.ПериодОкончания = Мин(Выборка.ПериодОкончания, ПараметрыЗаполнения.ДатыДокумента.ДатаОкончания);
		Иначе
			СтрокаТаблицы.Период = ПараметрыЗаполнения.ДатыДокумента.ДатаНачала;
			СтрокаТаблицы.ПериодОкончания = ПараметрыЗаполнения.ДатыДокумента.ДатаОкончания;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТарифныеСтавки(ПараметрыЗаполнения)
	Если Не НастройкиВидаДокумента.ПоказыватьТарифнуюСтавку Тогда
		Возврат;
	КонецЕсли;
	
	ДатыДокумента = ДатыДокумента();
	
	Фильтр = Новый Структура("ФиксТарифнаяСтавка", Ложь);
	ТаблицаСотрудников = Объект.ЗначенияПоказателей.Выгрузить(, "Объект, Период, ПериодОкончания");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТаблицаСотрудников.Объект КАК Сотрудник,
	|	ТаблицаСотрудников.ПериодОкончания КАК Период
	|ПОМЕСТИТЬ ВТСотрудники
	|ИЗ
	...
	Для Каждого СтрокаТаблицы Из ПараметрыЗаполнения.СтрокиТаблицы Цикл
		Если СтрокаТаблицы.ФиксТарифнаяСтавка Тогда
			Продолжить;
		КонецЕсли;
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(СтрокаТаблицы.Объект, "Сотрудник") Тогда
...
&НаСервере
Процедура ЗаполнитьОтработанноеВремя(ПараметрыЗаполнения)
	Если Не НастройкиВидаДокумента.ПоказыватьОтработанноеВремя Тогда
		Возврат;
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = ПараметрыЗаполнения.МенеджерВТ;
	...
	Для Каждого СтрокаТаблицы Из ПараметрыЗаполнения.СтрокиТаблицы Цикл
		Если СтрокаТаблицы.ФиксОтработанноеВремя Тогда
			Продолжить;
		КонецЕсли;
		Выборка.Сбросить();
		Если Выборка.НайтиСледующий(СтрокаТаблицы.Объект, "Сотрудник") Тогда
...
Процедура ЗаполнитьКоэффициентыРаспределенияВыполненныхРабот()
	Если Не НастройкиВидаДокумента.ВыполненныеРаботыРаспределяютсяСУчетомКоэффициентов Тогда
		Возврат;
	КонецЕсли;
...
&НаСервере
Функция ДатыДокумента()
	Результат = Новый Структура("ДатаНачала, ДатаОкончания");
	Если Элементы.ПериодСтраницы.Видимость Тогда
		Если Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.МесяцСтрокойСтраница Тогда
			Результат.ДатаНачала    = НачалоМесяца(Объект.Период);
			Результат.ДатаОкончания = КонецМесяца(Объект.Период);
		ИначеЕсли Элементы.ПериодСтраницы.ТекущаяСтраница = Элементы.ПериодСтраница
			И Элементы.ДатаОкончания.Видимость Тогда
			Результат.ДатаНачала    = НачалоДня(Объект.Период);
			Результат.ДатаОкончания = КонецДня(Объект.ПериодОкончания);
		Иначе
			Результат.ДатаНачала    = НачалоДня(Объект.Период);
			Результат.ДатаОкончания = КонецДня(Объект.Период);
		КонецЕсли;
	Иначе
		Результат.ДатаНачала    = НачалоДня(Объект.Дата);
		Результат.ДатаОкончания = КонецДня(Объект.Дата);
	КонецЕсли;
	Возврат Результат;
КонецФункции
...
Процедура ОтменитьВсеИсправленияНаСервере()
	ЗаполнитьТарифныеСтавкиИОтработанноеВремя(, Истина);
...
Процедура ОтменитьИсправленияСотрудникаНаСервере()
	...
	ЗаполнитьТарифныеСтавкиИОтработанноеВремя(ВыделенныеСтроки, Истина);
