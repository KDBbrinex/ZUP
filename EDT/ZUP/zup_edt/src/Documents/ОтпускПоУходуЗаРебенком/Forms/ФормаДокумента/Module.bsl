...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПрямыеВыплатыПособийСоциальногоСтрахования.ДобавитьУсловноеОформлениеЗаявленийВТаблицеДетей(ЭтотОбъект, "Объект", "ДанныеОДетях", "Заявление");
...
Процедура Подключаемый_ПланируемаяДатаВыплатыПриИзменении(Элемент)
	...
	РасчетЗарплатыРасширенныйКлиент.ПланируемаяДатаВыплатыПриИзменении(ЭтаФорма, ОписаниеДокумента(ПараметрыОписанияДокумента(ЭтотОбъект)));
...
Процедура ДанныеОДетяхВыбор(Элемент, ИдентификаторСтроки, КолонкаТаблицы, СтандартнаяОбработка)
	СтрокаТаблицы = Объект.ДанныеОДетях.НайтиПоИдентификатору(ИдентификаторСтроки);
...
Процедура ОбновитьДанныеПоСотруднику(СотрудникИзменился = Ложь)
	...
	КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(
		Истина, Объект.ОсновнойСотрудник, "Организация,Подразделение,Территория,ЛьготаПриНачисленииПособий,ДоляНеполногоРабочегоВремени", ВремяРегистрации);
...
Процедура ОбновитьДанныеОДетях()
	...
	ЗаявленияВведенныеНаОсновании = Запрос.Выполнить().Выгрузить();
	
	Для Каждого СтрокаТаблицы Из Объект.ДанныеОДетях Цикл
		СтрокаТаблицы.ПредставлениеОчередности = ПредставлениеОчередностиРебенка(СтрокаТаблицы.Очередность);
		НайденнаяСтрока = ЗаявленияВведенныеНаОсновании.Найти(СтрокаТаблицы.ИдентификаторСтроки, "ИдентификаторСтроки");
		Если НайденнаяСтрока <> Неопределено Тогда
			СтрокаТаблицы.Заявление = НайденнаяСтрока.Заявление;
		Иначе
