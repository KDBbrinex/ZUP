...
Процедура СоздатьСотрудника()
	...
	Если ДоступныПлановыеНачисления Тогда
		...
		ОтражениеЗарплатыНовогоСотрудникаВБухУчете();
...
&НаСервере
Процедура ОтражениеЗарплатыНовогоСотрудникаВБухУчете()
	
	БухучетСотрудника = ОтражениеЗарплатыВБухучетеРасширенный.НастройкаБухучетаЗарплатыСотрудника(Объект.Сотрудник, Объект.ДатаПриема);
	Если ЗначениеЗаполнено(БухучетСотрудника.Период) Тогда
		...
		НаборЗаписейБухУчета.Отбор.Сотрудник.Установить(Объект.НовыйСотрудник, Истина);
		Запись = НаборЗаписейБухУчета.Добавить();
		ЗаполнитьЗначенияСвойств(Запись,БухучетСотрудника);
		Запись.Сотрудник = Объект.НовыйСотрудник;
