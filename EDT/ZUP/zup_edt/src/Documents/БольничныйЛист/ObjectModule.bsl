...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ПособиеВыплачиваетсяФСС = ЭЛНКарантинПоКоронавирусу
		Или (ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(
				Организация,
				ПериодРегистрации)
			И ПрямыеВыплатыПособийСоциальногоСтрахования.КоллекцияСодержитПособияЗаСчетФСС(
				Начисления.ВыгрузитьКолонку("Начисление")));
...
Процедура ПроверитьЗаполнениеРеквизитовНеобходимыхДляРасчета(КонтейнерОшибок, ПроверкаПередРасчетом = Ложь)
	...
	ПроверитьРазбивкуНаНесколькоПериодовОплаты(КонтейнерОшибок);
	
КонецПроцедуры

Процедура ПроверитьРазбивкуНаНесколькоПериодовОплаты(КонтейнерОшибок)
	Если Не НазначитьПособие Тогда
		Возврат;
	КонецЕсли;
	Если Не Перечисления.ПричиныНетрудоспособности.ОтпускВытесняетБольничный(ПричинаНетрудоспособности) Тогда
		Возврат;
	КонецЕсли;
	Если ИсключаемыеПериоды.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	ДатыОплаты = Документы.БольничныйЛист.ОплачиваемыеДаты(ЭтотОбъект, Новый Соответствие);
	ПериодыОплаты = ОбщегоНазначенияБЗК.ПериодыИзМассиваДат(ДатыОплаты);
	Если ПериодыОплаты.Количество() < 2 Тогда
		Возврат;
	КонецЕсли;
	
	ДатаНачалаОкна    = ПериодыОплаты[0].ДатаОкончания;
	ДатаОкончанияОкна = ПериодыОплаты[ПериодыОплаты.ВГраница()].ДатаНачала;
	
	ПредставленияПериодов = Новый Массив;
	Для Каждого ИсключаемыйПериод Из ИсключаемыеПериоды Цикл
		Если ИсключаемыйПериод.ДатаНачала < ДатаОкончанияОкна
			И ИсключаемыйПериод.ДатаОкончания > ДатаНачалаОкна Тогда
			НаименованиеПериода = ОбщегоНазначенияБЗК.НаименованиеПериода(
	...
	ТекстСообщения = СтрШаблон(
		НСтр("ru = 'Неоплачиваемый период %1 разделяет оплату больничного, что недопустимо, поскольку формат электронного
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(КонтейнерОшибок, "ИсключаемыеПериоды", ТекстСообщения, "");
КонецПроцедуры
...
Процедура ЗаполнитьПоПервичномуБольничномуЛисту(СсылкаИлиНомерПервичногоБольничного) Экспорт
	
	ДанныеПервичногоБольничногоЛиста = УчетПособийСоциальногоСтрахования.ДанныеБольничногоЛиста(СсылкаИлиНомерПервичногоБольничного);
	...
	Иначе
		...
		ЗаполняемыеДанные = "Организация, Сотрудник, ПричинаНетрудоспособности, НазначитьПособие,
