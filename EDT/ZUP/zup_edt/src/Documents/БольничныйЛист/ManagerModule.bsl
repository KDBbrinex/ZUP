...
Функция ДанныеДляПроведения(Больничный, СтруктураВидовУчета, Кэш)
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Больничный.ФизическоеЛицо, Больничный.Организация, Больничный.Сотрудник, Больничный.ДатаНачала, Больничный.ДатаОкончания);
		...
		ЗаполнитьДанныеРеестраОтпусков(Больничный, ДанныеДляПроведения);
		ЗаполнитьДанныеУчетаПериодовУходаЗаБольнымиДетьми(Больничный, ДанныеДляПроведения);
...
Функция РеквизитыДляПроведения(ДокументСсылка)
	...
	ИсключаемыеПериоды = Результаты[3].Выгрузить();
	...
	РеквизитыДляПроведения.ИсключаемыеПериоды = ИсключаемыеПериоды;
...
Процедура ВывестиШапкуРасчетаПособия(КонтекстПечати, ВыборкаДляШапки, КадровыеДанныеСотрудников)
	...
	ЗаполнитьШапкуРасчетаПособия(ПараметрыШапки, ВыборкаДляШапки, КадровыеДанныеСотрудников);
...
Процедура ЗаполнитьШапкуРасчетаПособия(ПараметрыШапки, ВыборкаДляШапки, КадровыеДанныеСотрудников)
...
Процедура ВывестиШапкуЛисткаНетрудоспособности(КонтекстПечати, ВыборкаДляШапки, КадровыеДанныеСотрудников)
	// Определение локальных переменных.
	ТабличныйДокумент       = КонтекстПечати.ТабличныйДокумент;
	Макет                   = КонтекстПечати.Макет;
	ИменаПараметровОбластей = КонтекстПечати.ИменаПараметровОбластей;
...
Процедура ПриПечатиЛисткаНетрудоспособностиСРасчетомПособия(КонтекстПечати)
	...
	Пока ВыборкаДляШапки.Следующий() Цикл
		...
		ВывестиШапкуЛисткаНетрудоспособности(КонтекстПечати, ВыборкаДляШапки, КадровыеДанныеСотрудников);
...
Функция ДанныеДляРегистрацииВУчетаСтажаПФР(МассивСсылок) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		РеквизитыДляРегистрацииВУчетеСтажаПФР = РеквизитыДляРегистрацииВУчетеСтажаПФР(Выборка);
		Кэш = Новый Соответствие;
...
Функция РеквизитыДляРегистрацииВУчетеСтажаПФР(Выборка)
	
	РеквизитыДляРегистрацииВУчетеСтажаПФР = РеквизитыДляРегистрацииВУчетеСтажаПФРПустаяСтруктура();
	...
	Возврат РеквизитыДляРегистрацииВУчетеСтажаПФР;
	
КонецФункции

Функция РеквизитыДляРегистрацииВУчетеСтажаПФРПустаяСтруктура()
	
	РеквизитыДляРегистрацииВУчетеСтажаПФРПустаяСтруктура = Новый Структура("Сотрудник,
	| ДатаНачала, ДатаОкончания, НазначитьПособие, ДатаНачалаОплаты, ДатаОкончанияОплаты,
	| ПричинаНетрудоспособности, ДатаНачалаСобытия,
	| ВидСтажаНаПериодСОплатой, ВидСтажаНаПериодБезОплаты, Ссылка,
	| ИсключаемыеПериоды, ДатаНачалаПоловиннойОплаты, ДатаНарушенияРежима,
	| ВидНеоплачиваемогоВремени, ВидОплатыЗаСчетРаботодателя, ВидОплатыПособия");
	
	Возврат РеквизитыДляРегистрацииВУчетеСтажаПФРПустаяСтруктура;
...
Процедура ЗаполнитьРодственников(Документ)
	Если Не ЗначениеЗаполнено(Документ.ФизическоеЛицо) Тогда
		Документ.РодственникЗаКоторымОсуществляетсяУход1 = Справочники.РодственникиФизическихЛиц.ПустаяСсылка();
		Документ.РодственникЗаКоторымОсуществляетсяУход2 = Справочники.РодственникиФизическихЛиц.ПустаяСсылка();
		Возврат;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из НомераИФИОРодственников Цикл
		НомерРодственника = КлючИЗначение.Ключ;
		ФИОРодственника   = КлючИЗначение.Значение;
		Родственник = НайтиРодственникаВТаблице(Документ, Таблица, НомерРодственника, ФИОРодственника);
		Документ["РодственникЗаКоторымОсуществляетсяУход" + НомерРодственника] = Родственник;
	КонецЦикла;
КонецПроцедуры

// Подбор наиболее подходящих родственников физических лиц (в таблице - родственники, найденные по ФИО).
Функция НайтиРодственникаВТаблице(Документ, Таблица, НомерРодственника, ФИОРодственника) Экспорт
	Если Не ЗначениеЗаполнено(ФИОРодственника) Тогда
		Возврат Справочники.РодственникиФизическихЛиц.ПустаяСсылка();
	КонецЕсли;
	
	РодственникиНайденныеПоФИО = Таблица.Скопировать(Новый Структура("НомерРодственника", НомерРодственника));
...
Процедура ЗаполнитьПараметрыОплаты(Больничный, Кэш) Экспорт
	...
	Больничный.ДнейОплаты = МаксимальноеЧислоДнейОплаты(
		Больничный,
		Кэш["ОстатокОплачиваемыхДнейУходаЗаРодственником1"],
		Кэш["ОстатокОплачиваемыхДнейУходаЗаРодственником2"]);
...
Процедура ЗаполнитьИсключаемыеПериоды(Больничный, Кэш)
	...
	ДатыОтпусков = ДатыОтпусков(Больничный, Кэш);
	ПериодыОтпусков = ОбщегоНазначенияБЗК.ПериодыИзМассиваДат(ДатыОтпусков.ВыгрузитьКолонку("Дата"));
	Для Каждого ПериодОтпусков Из ПериодыОтпусков Цикл
		ИсключаемыйПериод = Больничный.ИсключаемыеПериоды.Добавить();
		ИсключаемыйПериод.Регистрировать = Ложь;
		ИсключаемыйПериод.ДатаНачала      = НачалоДня(Макс(ПериодОтпусков.ДатаНачала, Больничный.ДатаНачала));
		ИсключаемыйПериод.ДатаОкончания   = НачалоДня(Мин(ПериодОтпусков.ДатаОкончания, Больничный.ДатаОкончания));
		ИсключаемыйПериод.КалендарныхДней = ОбщегоНазначенияБЗК.КоличествоДнейВПериоде(
			ИсключаемыйПериод.ДатаНачала,
			ИсключаемыйПериод.ДатаОкончания);
	КонецЦикла;
...
Функция ДатыОтпусков(Больничный, Кэш) Экспорт
	Значение = Кэш["ДатыОтпусков"];
	Если Значение = Неопределено Тогда
		Значение = ДатыОтпусковБезКэша(Больничный);
		Кэш.Вставить("ДатыОтпусков", Значение);
	КонецЕсли;
	Возврат Значение;
КонецФункции

Функция ДатыОтпусковБезКэша(Больничный)
	Если Не ПериодСоответствуетТребованиям(Больничный.ДатаНачала, Больничный.ДатаОкончания)
		Или Не ОтпускВытесняетБольничный(Больничный) Тогда
...
Функция ПериодСоответствуетТребованиям(ДатаНачала, ДатаОкончания)
	Возврат ЗначениеЗаполнено(ДатаНачала) И ДатаНачала <= ДатаОкончания И Год(ДатаНачала) + 100 > Год(ДатаОкончания);
КонецФункции

Функция ОтпускВытесняетБольничный(Больничный)
	Возврат Перечисления.ПричиныНетрудоспособности.ОтпускВытесняетБольничный(Больничный.ПричинаНетрудоспособности);
КонецФункции

Функция МаксимальноеЧислоДнейОплаты(Больничный, ОстатокОплачиваемыхДнейУходаЗаРодственником1, ОстатокОплачиваемыхДнейУходаЗаРодственником2) Экспорт
	Если Больничный.АвтозаполнениеПериодовОплаты Тогда
	...
	Если Не ПериодСоответствуетТребованиям(ДатаНачала, ДатаОкончания)
	...
	МаксимумЗаСлучай = МаксимальноеКоличествоОплачиваемыхДнейПоПричинеНетрудоспособности(
	...
	// Корректируется на остаток дней ухода за родственниками.
	ЭтоУход = Перечисления.ПричиныНетрудоспособности.ЭтоУходЗаРодственником(Больничный.ПричинаНетрудоспособности);
	Если ЭтоУход
		И (ОстатокОплачиваемыхДнейУходаЗаРодственником1 <> Неопределено
			Или ОстатокОплачиваемыхДнейУходаЗаРодственником2 <> Неопределено) Тогда
		Остаток = 0;
		Если ОстатокОплачиваемыхДнейУходаЗаРодственником1 <> Неопределено Тогда
			Остаток = Остаток + ОстатокОплачиваемыхДнейУходаЗаРодственником1;
		КонецЕсли;
		Если ОстатокОплачиваемыхДнейУходаЗаРодственником2 <> Неопределено Тогда
			Остаток = Остаток + ОстатокОплачиваемыхДнейУходаЗаРодственником2;
		КонецЕсли;
		ДнейОплаты = Мин(ДнейОплаты, Остаток);
	КонецЕсли;
...
Функция ОплачиваемыеДатыБезКэша(Больничный, Кэш, НеучтенныеИсключаемыеДаты)
	...
	// Удаление дней, превышающих остаток оплачиваемых дней.
...
Функция ДатыНачислений(Больничный, Кэш)
	...
	Кэш.Вставить(Ключ, ДатыНачислений);
	
	Если Не ПериодСоответствуетТребованиям(Больничный.ДатаНачала, Больничный.ДатаОкончания) Тогда
	...
	Для Каждого Дата Из МассивДат Цикл
		...
		Иначе
			ДатаНачисления.ВидРасчета = Больничный.ВидОплатыПособия; // За счет ФСС.
		КонецЕсли;
...
Функция ПериодыСостояний(Больничный, Кэш)
	...
	ПериодыСостояний = ДатыНачислений.СкопироватьКолонки("Состояние");
	ПериодыСостояний.Колонки.Добавить("ДатаНачала",    ДатыНачислений.Колонки.Дата.ТипЗначения);
	ПериодыСостояний.Колонки.Добавить("ДатаОкончания", ДатыНачислений.Колонки.Дата.ТипЗначения);
	...
	Для Каждого ДатаНачисления Из ДатыНачислений Цикл
		...
		Если ТекущийПериод = Неопределено
			...
			ТекущийПериод = ПериодыСостояний.Добавить();
