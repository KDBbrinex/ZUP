...
// Создание нового документа "Справка 2-НДФЛ для передачи в налоговый орган".
//
// Параметры:
...
//  * Ссылка - ДокументСсылка.СправкиНДФЛДляПередачиВНалоговыйОрган, Неопределено - ссылка на созданный документ.
//  * Ошибки - Массив из Строка - сообщения об ошибках.
//
Функция НовыйДокумент(Организация, ОтчетныйПериод, Проводить = Истина) Экспорт
	
	ДанныеДляЗаполнения = НоваяСтруктураДляЗаполнения();
	
	ДокументОбъект = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.СоздатьДокумент();
	
	ДокументОбъект.Дата = ТекущаяДатаСеанса();
	ДокументОбъект.Организация = Организация;
	ДокументОбъект.НалоговыйПериодДата = ОтчетныйПериод;
	ДокументОбъект.НалоговыйПериод = Год(ОтчетныйПериод);
	ДокументОбъект.ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетность;
	РегистрацияВНалоговомОргане = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Организация, "РегистрацияВНалоговомОргане", 
	Истина);
	ДокументОбъект.РегистрацияВНалоговомОргане = РегистрацияВНалоговомОргане;
	ДанныеРегистрацииВНалоговомОргане = ЗарплатаКадрыВызовСервера.ДанныеРегистрацииВНалоговомОргане(
	Организация, РегистрацияВНалоговомОргане);
	ДокументОбъект.КППОтправки = ДанныеРегистрацииВНалоговомОргане.КПП;
	ДокументОбъект.КодИФНС = ДанныеРегистрацииВНалоговомОргане.Код;
	...
	ДокументОбъект.Телефон = ПолучитьТелефонОрганизации(Организация);
	
	ЗарплатаКадры.ЗаполнитьДанныеПодписанта(ДокументОбъект, "СправкуПодписал", "ДолжностьПодписавшегоЛица");
	
	ЗаполнитьДанные(ДокументОбъект, ДанныеДляЗаполнения);
	
	ЗаполнитьДокументПоДаннымЗаполнения(ДокументОбъект, ДанныеДляЗаполнения);
	УстановитьДополнительныеСвойстваДокумента(ДокументОбъект, ДанныеДляЗаполнения);
	...
	Результат.Вставить("Ошибки", ДокументОбъект.ДополнительныеСвойства.Ошибки);
...
// Возвращает актуальные на дату отчетного периода идентификаторы для печати Справки 2-НДФЛ и Реестра.
//
// Параметры:
//  ОтчетныйПериод  - Дата
// 
// Возвращаемое значение:
//  Строка - имя идентификатора макета для 2-НДФЛ, через запятую имя идентификатора макета Реестра.
//
Функция ИменаМакетов(ОтчетныйПериод = '00010101') Экспорт
	
	Макет2НДФЛ = "Форма2НДФЛ";
	
	Если ОтчетныйПериод >= '20110101' И ОтчетныйПериод < '20140101' Тогда
		МакетРеестра = "Реестр2011";
	ИначеЕсли ОтчетныйПериод >= '20140101' И ОтчетныйПериод < '20190101' Тогда
		МакетРеестра = "Реестр2014";
	Иначе
		МакетРеестра = "Реестр2019"
	КонецЕсли;
	
	Возврат СтрШаблон("%1,%2", Макет2НДФЛ, МакетРеестра);
...
Функция НомерПервойСправки(Дата, НалоговыйПериод, Организация, ИсключаемаяСсылка) Экспорт
	Запрос = Новый Запрос;
	...
	|	И СведенияОДоходахФизлицРаботникиОрганизации.Ссылка <> &ИсключаемаяСсылка";
...
Функция НомерПачкиДокумента(Знач НомерДокумента)
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	// Справка о доходах (2-НДФЛ с двухмерными штрихкодами)
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Обработчик = "УчетНДФЛКлиент.ВывестиМашиночитаемуюФормуСправки2НДФЛ";
	КомандаПечати.Идентификатор = "Форма2НДФЛ";
	КомандаПечати.Представление = НСтр("ru = 'Справка о доходах (2-НДФЛ) со штрихкодом PDF417'");
...
Процедура ЗаполнитьДанные(ДокументОбъект, ДанныеДляЗаполнения) Экспорт
	
	НомерСправки = НомерПервойСправки(ДокументОбъект.Дата, ДокументОбъект.НалоговыйПериод, ДокументОбъект.Организация, 
	ДокументОбъект.Ссылка);
	
	ДобавляемыеСотрудники = СотрудникиДляЗаполненияСправки(ДокументОбъект);
	
	Для каждого СтрокаСотрудника Из ДобавляемыеСотрудники Цикл
		СтрокаТаблицы = ДанныеДляЗаполнения.Сотрудники.Добавить();
		СтрокаТаблицы.Сотрудник = СтрокаСотрудника.Сотрудник;

		Если ДокументОбъект.НомерКорректировки = 0 Тогда
	...
	ПараметрыСправокНДФЛ.СведенияОДоходах 					= ДанныеДляЗаполнения.СведенияОДоходах;
	ПараметрыСправокНДФЛ.СведенияОВычетах 					= ДанныеДляЗаполнения.СведенияОВычетах;
	ПараметрыСправокНДФЛ.НалоговыйПериод					= ДокументОбъект.НалоговыйПериод;
	ПараметрыСправокНДФЛ.ДатаДокумента 						= ДокументОбъект.Дата;
	ПараметрыСправокНДФЛ.Организация 						= ДокументОбъект.Организация;
	ПараметрыСправокНДФЛ.РегистрацияВНалоговомОргане		= ДокументОбъект.РегистрацияВНалоговомОргане; 
	ПараметрыСправокНДФЛ.ИспользоватьНомераСправок 			= Истина;
	ПараметрыСправокНДФЛ.ЗаполнятьБезДоходовПоЦеннымБумагам = ДокументОбъект.НалоговыйПериод 
															>= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли())
															И ДокументОбъект.НеВключатьДоходыПоЦеннымБумагам;;
	ПараметрыСправокНДФЛ.УведомленияНОоПравеНаВычеты 		= ДанныеДляЗаполнения.УведомленияНОоПравеНаВычеты;
	ПараметрыСправокНДФЛ.КППОтправки 						= ДокументОбъект.КППОтправки;
			
	СправкиНДФЛ.ПрочитатьДанные(
	ДанныеДляЗаполнения.Сотрудники,
	ПараметрыСправокНДФЛ,
	Истина,
	ДокументОбъект.НомерКорректировки);
		
	ЗаполнитьСтавкиНДФЛВТаблицеСотрудники(ДанныеДляЗаполнения.Сотрудники);

КонецПроцедуры

Функция НоваяСтруктураДляЗаполнения() Экспорт
	
	СтруктураДляЗаполнения = Новый Структура;
	СтруктураДляЗаполнения.Вставить("СведенияОВычетах");
	СтруктураДляЗаполнения.Вставить("СведенияОДоходах");
	СтруктураДляЗаполнения.Вставить("Сотрудники");
	СтруктураДляЗаполнения.Вставить("УведомленияНОоПравеНаВычеты");
	СтруктураДляЗаполнения.Вставить("ФизическиеЛица");
	
	СоздатьТаблицыВСтруктуре(СтруктураДляЗаполнения);
	
	Возврат СтруктураДляЗаполнения;
	
КонецФункции

Функция НоваяТаблицаЗначенийПоТабличнойЧасти(ТабличнаяЧасть)
	
	Результат = Новый ТаблицаЗначений;
	
	МетаданныеТабличнаяЧасть = Метаданные.Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ТабличныеЧасти.Найти(
	ТабличнаяЧасть);
	
	Для каждого РеквизитТЧ Из МетаданныеТабличнаяЧасть.Реквизиты Цикл
		Результат.Колонки.Добавить(РеквизитТЧ.Имя, РеквизитТЧ.Тип);
	КонецЦикла;
...
Функция СоздатьТаблицыВСтруктуре(СтруктураДляЗаполнения)
	
	СтруктураДляЗаполнения.СведенияОВычетах = НоваяТаблицаСведенияОВычетах();
	СтруктураДляЗаполнения.СведенияОДоходах = НоваяТаблицаСведенияОДоходах();
	СтруктураДляЗаполнения.Сотрудники = НоваяТаблицаСотрудников();
	СтруктураДляЗаполнения.УведомленияНОоПравеНаВычеты = НоваяТаблицаЗначенийПоТабличнойЧасти("УведомленияНОоПравеНаВычеты");
	СтруктураДляЗаполнения.ФизическиеЛица = НоваяТаблицаЗначенийПоТабличнойЧасти("ФизическиеЛица");
	
КонецФункции

Функция НоваяТаблицаСотрудников() 
	
	ТаблицаСотрудников = НоваяТаблицаЗначенийПоТабличнойЧасти("Сотрудники");
	
	ТаблицаСотрудников.Колонки.Добавить("Ставка03", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка05", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка06", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка07", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка09", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка10", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка12", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка13", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка15", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка30", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	ТаблицаСотрудников.Колонки.Добавить("Ставка35", Новый ОписаниеТипов("ПеречислениеСсылка.НДФЛСтавки"));
	
	Возврат ТаблицаСотрудников;
	
КонецФункции

Функция НоваяТаблицаСведенияОВычетах()
	
	ТаблицаСведенияОВычетах = НоваяТаблицаЗначенийПоТабличнойЧасти("СведенияОВычетах");
	
	Возврат ТаблицаСведенияОВычетах;
	
КонецФункции

Функция НоваяТаблицаСведенияОДоходах()
	
	ТаблицаСведенияОДоходах = НоваяТаблицаЗначенийПоТабличнойЧасти("СведенияОДоходах");
	
	ТаблицаСведенияОДоходах.Колонки.Добавить("МесяцНалоговогоПериодаСтрокой", ОбщегоНазначения.ОписаниеТипаСтрока(8));
...
Процедура ЗаполнитьДокументПоДаннымЗаполнения(ДокументОбъект, ДанныеДляЗаполнения) Экспорт
	...
	ДокументОбъект.Сотрудники.Загрузить(ДанныеДляЗаполнения.Сотрудники);
	ДокументОбъект.ФизическиеЛица.Загрузить(ДанныеДляЗаполнения.ФизическиеЛица);
	ДокументОбъект.УведомленияНОоПравеНаВычеты.Загрузить(ДанныеДляЗаполнения.УведомленияНОоПравеНаВычеты);

КонецПроцедуры

Процедура ЗаполнитьСтавкиНДФЛВТаблицеСотрудники(ТаблицаСотрудников) Экспорт
	Для Каждого СтрокаТаблицы Из ТаблицаСотрудников Цикл
		ЗаполнитьСтавкиВСтрокеТаблицыСотрудники(СтрокаТаблицы);
	КонецЦикла;	
КонецПроцедуры	

Процедура ЗаполнитьСтавкиВСтрокеТаблицыСотрудники(СтрокаТаблицы) Экспорт
...
Функция СотрудникиДляЗаполненияСправки(ДокументОбъект)
	...
	Запрос.УстановитьПараметр("НалоговыйПериод", ДокументОбъект.НалоговыйПериод);
	Запрос.УстановитьПараметр("КонецНП",КонецГода(Дата(ДокументОбъект.НалоговыйПериод,1,1)));
	Запрос.УстановитьПараметр("НачалоНП", Дата(ДокументОбъект.НалоговыйПериод,1,1));
	Запрос.УстановитьПараметр("ДатаПодачиСведений", КонецДня(ДокументОбъект.Дата));
	Запрос.УстановитьПараметр("РегистрацияВНалоговомОргане", ДокументОбъект.РегистрацияВНалоговомОргане);
	Запрос.УстановитьПараметр("КППОтправки", ДокументОбъект.КППОтправки);
	Запрос.УстановитьПараметр("НачалоПредшествующегоГода", ДобавитьМесяц(Дата(ДокументОбъект.НалоговыйПериод,1,1), -12));
	...
	Если ДокументОбъект.НомерКорректировки = 99 Тогда // аннулирующие справки
		...
		|				ИЛИ СведенияОДоходахНДФЛ.МесяцНалоговогоПериода >= &ДатаОбъединенияВсехСведений)
		|
		|СГРУППИРОВАТЬ ПО
	...
	ИначеЕсли ДокументОбъект.НомерКорректировки > 0 Тогда
		
		Запрос.УстановитьПараметр("НомерПредыдущейКорректировки", ДокументОбъект.НомерКорректировки - 1);
		...
		|				ИЛИ ДанныеУчета.МесяцНалоговогоПериода >= &ДатаОбъединенияВсехСведений)";
		Запрос.Выполнить();
		
		Если ЗначениеЗаполнено(ДокументОбъект.РегистрацияВНалоговомОргане) И ЗначениеЗаполнено(ДокументОбъект.КППОтправки) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументОбъект.РегистрацияВНалоговомОргане, "КПП") <> ДокументОбъект.КППОтправки Тогда
			
			УчетНДФЛ.СформироватьВТФизическиеЛицаСУчетомПереездаВИнойНО(ДокументОбъект.КППОтправки, Запрос.МенеджерВременныхТаблиц, "ВТДоходы", "КонецГода");
	...
	Иначе // исходные (обычные) справки
		...
		Если ДокументОбъект.НалоговыйПериод >= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) И ДокументОбъект.НеВключатьДоходыПоЦеннымБумагам Тогда
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ДанныеУчета.ФизическоеЛицо КАК ФизическоеЛицо,
			|	ДанныеУчета.Подразделение КАК Подразделение,
		...
		|УПОРЯДОЧИТЬ ПО
		|	СотрудникНаименование";
		Если УчетНДФЛ.ВыводитьФорму2НДФЛ2019Года(ДокументОбъект.НалоговыйПериод, ДокументОбъект.Дата) Тогда
...
Функция ПолучитьТелефонОрганизации(Организация)
	
	Если Не ЗначениеЗаполнено(Организация) Тогда 
		Возврат Неопределено;
	КонецЕсли;
	
	ДанныеКИОрганизации = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(Организация, , "ТелОрганизации");
	
	Если ТипЗнч(ДанныеКИОрганизации) = Тип("Структура") И ДанныеКИОрганизации.Свойство("ТелОрганизации") Тогда
		Возврат ДанныеКИОрганизации.ТелОрганизации;
	КонецЕсли;	
	
КонецФункции

Процедура УстановитьДополнительныеСвойстваДокумента(ТекущийОбъект, ТаблицыЗаписей)
	
	Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ДобавитьДополнительныеСвойстваОДоходахИВычетах(
	...
	Для Каждого ДопСвойство Из ТекущийОбъект.ДополнительныеСвойства Цикл
		...
		НаборЗаписей.Загрузить(ТаблицыЗаписей[ДопСвойство.Ключ]);
	КонецЦикла;
