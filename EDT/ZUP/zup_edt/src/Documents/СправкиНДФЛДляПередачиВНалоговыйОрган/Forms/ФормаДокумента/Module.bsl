...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ПустаяСсылка() Тогда
		...
		ЗарплатаКадры.ЗаполнитьДанныеПодписанта(Объект, "СправкуПодписал", "ДолжностьПодписавшегоЛица");
	...
	УчетНДФЛФормы.СведенияОДоходахПриСозданииНаСервере(ЭтотОбъект, "ЗаписиСведенияОДоходах.КодДохода", "СведенияОДоходахКодВычета", "СведенияОДоходахСуммаВычета");
	
	Если ЗначениеЗаполнено(Объект.Организация) Тогда
		УстановитьИмяФайла();
	КонецЕсли;
	
	СправкиНДФЛФормы.ЗаполнитьСписокКонтролируемыхПолей(ЭтотОбъект);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		Если ДанныеТекущейСтроки <> Неопределено Тогда
			СправкиНДФЛФормыКлиентСервер.УстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, 
			ДанныеТекущейСтроки, ДокументПроведен);
		КонецЕсли;
	КонецЕсли;
	...
	Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ЗаполнитьСтавкиНДФЛВТаблицеСотрудники(Объект.Сотрудники);
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ИзмененыДанныеСправки2НДФЛ" И Источник = ЭтотОбъект Тогда
...
Процедура ДатаПриИзмененииНаСервере()
	...
	СправкиНДФЛФормы.УстановитьСписокВыбораРегистрацийВНО(ЭтотОбъект);
	...
	ПараметрыПредставления.СводнаяСправка = Объект.НалоговыйПериод 
		>= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) И Объект.НеВключатьДоходыПоЦеннымБумагам;
	СправкиНДФЛФормы.УстановитьИнфонадписьОписаниеДоходовОрганизации(ЭтотОбъект,,ПараметрыПредставления);
...
Процедура НалоговыйПериодПриИзменении(Элемент)
	...
	СправкиНДФЛФормыКлиент.УстановитьСтарыеЗначенияКонтролируемыхПолей(ДанныеТекущейСтроки, СтарыеЗначенияКонтролируемыхПолей, КонтролируемыеПоля, "ИТОГИ"); 
	СправкиНДФЛФормыКлиент.УстановитьСтарыеЗначенияКонтролируемыхПолей(ДанныеТекущейСтроки, СтарыеЗначенияКонтролируемыхПолей, КонтролируемыеПоля, "Уведомление");
...
Процедура НалоговыйПериодПриИзмененииНаСервере()
	...
	Если ИспользоватьИсходныеСправкиПриПроведении() Тогда
		...
		СправкиНДФЛФормы.ПрочитатьИсходныеСправки(
			Объект.Сотрудники,
			Объект.НалоговыйПериод, 
			Объект.Дата, 
			Объект.Организация, 
	...
	ПараметрыСправокНДФЛ.СведенияОДоходах 					= ЗаписиСведенияОДоходах;
	ПараметрыСправокНДФЛ.СведенияОВычетах 					= ЗаписиСведенияОВычетах;
	ПараметрыСправокНДФЛ.НалоговыйПериод					= Объект.НалоговыйПериод;
	ПараметрыСправокНДФЛ.ДатаДокумента 						= Объект.Дата;
	ПараметрыСправокНДФЛ.Организация 						= Объект.Организация;
	ПараметрыСправокНДФЛ.РегистрацияВНалоговомОргане		= Объект.РегистрацияВНалоговомОргане; 
	ПараметрыСправокНДФЛ.ИспользоватьНомераСправок 			= Истина;
	ПараметрыСправокНДФЛ.ЗаполнятьБезДоходовПоЦеннымБумагам = Объект.НалоговыйПериод 
															>= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) 
															И Объект.НеВключатьДоходыПоЦеннымБумагам;
	ПараметрыСправокНДФЛ.УведомленияНОоПравеНаВычеты 		= Объект.УведомленияНОоПравеНаВычеты;
	ПараметрыСправокНДФЛ.КППОтправки 						= Объект.КППОтправки;

	СправкиНДФЛ.ПрочитатьДанные(Объект.Сотрудники, ПараметрыСправокНДФЛ, Истина, Объект.НомерКорректировки);
	
	ЗаполнитьИнфоНадписиОДокументахУдостоверяющихЛичность(Объект.Сотрудники, Объект.НалоговыйПериод);
	
	СправкиНДФЛФормы.УстановитьЗаголовокПоляРегистрацияВНО(ЭтотОбъект, Истина);
...
Процедура НеВключатьДоходыПоЦеннымБумагамПриИзмененииНаСервере()
	...
	СправкиНДФЛ.ПрочитатьДанныеОДоходахИНалогах(ДанныеСправок, ПараметрыСправокНДФЛ);
...
Процедура ВидСправокПриИзменении(Элемент)
	...
	КонецЕсли;
	УстановитьВидимостьДанныхРеорганизации(ЭтотОбъект);
КонецПроцедуры
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Если Объект.Сотрудники.Количество() > 0 Тогда
		...
		СправкиНДФЛФормыКлиентСервер.УстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, ДанныеТекущейСтроки, 
		ДокументПроведен);
...
Процедура ПрочитатьДанныеСправок()
	...
	СправкиНДФЛ.ПрочитатьДанныеСотрудников(Объект.Сотрудники, Объект.НалоговыйПериод, Объект.Дата, 
		Не ДокументПроведен);
...
Процедура УстановитьДополнительныеСвойстваДокумента(ТекущийОбъект)
	...
	Для Каждого ДопСвойство Из ТекущийОбъект.ДополнительныеСвойства Цикл
		...
		НаборЗаписей.Загрузить(ЭтотОбъект["Записи" + ДопСвойство.Ключ].Выгрузить());
	КонецЦикла;
...
Процедура ЗаполнитьНаСервере()
	...
	ДанныеДляЗаполнения = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.НоваяСтруктураДляЗаполнения();
	Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ЗаполнитьДанные(Объект, ДанныеДляЗаполнения);
	
	ЗаписиСведенияОВычетах.Загрузить(ДанныеДляЗаполнения.СведенияОВычетах);
	ЗаписиСведенияОДоходах.Загрузить(ДанныеДляЗаполнения.СведенияОДоходах);
	
	Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ЗаполнитьДокументПоДаннымЗаполнения(Объект, ДанныеДляЗаполнения);
...
Процедура ОбработкаПодбораНаСервере(ДобавляемыеСотрудники)
	...
	Если Объект.Сотрудники.Количество() = 0 Тогда
		НомерСправки = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.НомерПервойСправки(Объект.Дата, 
		Объект.НалоговыйПериод, Объект.Организация, Объект.Ссылка);
	Иначе
	...
	Для каждого Сотрудник Из ДобавляемыеСотрудники Цикл
		...
		Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ЗаполнитьСтавкиВСтрокеТаблицыСотрудники(СтрокаТаблицы);
	...
	ПараметрыСправокНДФЛ.РегистрацияВНалоговомОргане		= Объект.РегистрацияВНалоговомОргане; 
	ПараметрыСправокНДФЛ.ИспользоватьНомераСправок 			= Истина;
	ПараметрыСправокНДФЛ.ЗаполнятьБезДоходовПоЦеннымБумагам = Объект.НалоговыйПериод >= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) 
															И Объект.НеВключатьДоходыПоЦеннымБумагам;
	ПараметрыСправокНДФЛ.УведомленияНОоПравеНаВычеты 		= Объект.УведомленияНОоПравеНаВычеты;
	ПараметрыСправокНДФЛ.КППОтправки 						= Объект.КППОтправки;
	
	СправкиНДФЛ.ПрочитатьДанные(ДанныеСправок, ПараметрыСправокНДФЛ, Истина, Объект.НомерКорректировки);
...
Функция НомерПервойСправки()
	Если Объект.Сотрудники.Количество() = 0 Тогда
		Возврат Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.НомерПервойСправки(Объект.Дата, Объект.НалоговыйПериод, Объект.Организация, Объект.Ссылка);
	Иначе
...
Процедура СотрудникПриИзмененииНаСервере(ТекущийСотрудник)
	...
	Если ДанныеТекущейСтроки <> Неопределено Тогда
		...
		СправкиНДФЛФормы.ОчиститьДанныеСправки(ДанныеТекущейСтроки);
		
		Если ДанныеТекущейСтроки.НомерСправки = 0 Тогда
			Если Объект.Сотрудники.Количество() = 1 Тогда
				ДанныеТекущейСтроки.НомерСправки = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.НомерПервойСправки(Объект.Дата, 
				Объект.НалоговыйПериод, Объект.Организация, Объект.Ссылка);
...
Процедура ПрочитатьДанныеФизическогоЛица(ФизическоеЛицо)
	...
	Если СтрокиПоСотруднику.Количество() > 0 Тогда
		СправкиНДФЛ.ПрочитатьДанныеСотрудников(СтрокиПоСотруднику, Объект.НалоговыйПериод, Объект.Дата, Истина);
...
Процедура ОтменитьИсправлениеДанныхСотрудника()
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		СправкиНДФЛФормы.ОтменитьИсправлениеДанныхСотрудника(ЭтотОбъект, ДанныеТекущейСтроки, Объект.НалоговыйПериод, Объект.Дата);
...
Процедура ОбновитьНалогиНаСервере()
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		СправкиНДФЛФормы.ОбновитьНалоги(ДанныеТекущейСтроки, ПараметрыСправокНДФЛ);
...
Процедура ПронумероватьСправкиНаСервере(ТекущийСотрудник, ТекущийНомерСправки)
	...
	НомерПервойСправки = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.НомерПервойСправки(Объект.Дата, Объект.НалоговыйПериод, Объект.Организация, Объект.Ссылка);
...
Функция ПоместитьДанныеСправкиВоВременноеХранилище(НоваяСтрока = Ложь)
	...
	ПараметрыДанныхСправки.ДанныеСотрудника					= ДанныеСотрудника;
	ПараметрыДанныхСправки.СтрокиСведенийОДоходах 			= СведенияОДоходахФизическихЛиц;
	ПараметрыДанныхСправки.СтрокиСведенийОВычетах			= СведенияОВычетахФизическихЛиц;
	ПараметрыДанныхСправки.НомерСправки						= ДанныеСотрудника.НомерСправки;
	ПараметрыДанныхСправки.НеВключатьДоходыПоЦеннымБумагам	= Объект.НалоговыйПериод 
															>= Год(УчетНДФЛ.ДатаПереводаДоходовПоЦеннымБумагамВДекларациюПоПрибыли()) 
															И Объект.НеВключатьДоходыПоЦеннымБумагам;
	ПараметрыДанныхСправки.УведомленияОПравеНаВычеты		= СведенияОбУведомлениях;
	ПараметрыДанныхСправки.ПоказыватьИсправляемуюСправку	= ИспользоватьИсходныеСправкиПриПроведении();
	ПараметрыДанныхСправки.ИсправляемаяСправка				= ИсправляемаяСправка;
	ПараметрыДанныхСправки.НомерКорректировки				= Объект.НомерКорректировки - 1;
...
Процедура ЗагрузитьДанныеСправкиИзХранилища(АдресДанныхСправкиВХранилище)
	...
	Если ДанныеСправки.НоваяСтрока Тогда
		...
		Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ЗаполнитьСтавкиВСтрокеТаблицыСотрудники(ДанныеСотрудника);
	Иначе	
		ДанныеСотрудника = Объект.Сотрудники.НайтиПоИдентификатору(Элементы.Сотрудники.ТекущаяСтрока);
	КонецЕсли;	
