...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ОснованиеДляПредставленияСправок = Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛ 
		Или ОснованиеДляПредставленияСправок = 
		Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛЗаРеорганизованнуюОрганизацию Тогда
		
		ДанныеДляПроведения = ДанныеДляПроведения();
		
		УчетНДФЛ.СформироватьНалогиПереданныеВНалоговыйОрган(Движения, Отказ, Организация, Дата, ДанныеДляПроведения.Налоги);
		
		УчетНачисленнойЗарплаты.ЗарегистрироватьНалогиПереданныеВНалоговыйОрган(Движения, Отказ, Организация, НачалоМесяца(Дата), 
		...
		ОтражениеЗарплатыВБухучете.СформироватьДвиженияПоДокументу(Движения, Отказ, Организация, НачалоМесяца(Дата), 
			ДанныеДляПроведения, СтрокаСписокТаблиц);
...
Функция ДанныеДляПроведения()
	...
	ЗадолженностьФизлиц = ДанныеОЗадолженности.Скопировать(,"РегистрацияВНалоговомОргане,ФизическоеЛицо,Сумма");
...
Процедура ПроверитьДанныеДокумента(Отказ) Экспорт
	...
	Если Сотрудники.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Не заполнен список сотрудников.'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Ссылка, "Объект.Сотрудники", , Отказ);
		Возврат;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		СтруктураДанныхНА = УчетНДФЛ.СправкиНДФЛДанныеНалоговогоАгента(
		Организация, 
		НалоговыйПериод, 
	...
	ТекстСообщения = "";
	Если ОснованиеДляПредставленияСправок 
		= Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ОНевозможностиУдержанияНДФЛЗаРеорганизованнуюОрганизацию 
		Или ОснованиеДляПредставленияСправок 
		= Перечисления.ОснованияДляПредставленияСправок2НДФЛ.ЕжегоднаяОтчетностьЗаРеорганизованнуюОрганизацию Тогда
		Если Не ЗначениеЗаполнено(ФормаРеорганизации) Тогда
			ТекстСообщения = НСтр("ru = 'Не указан код формы реорганизации.'");
			ОбщегоНазначенияБЗК.ДобавитьОшибкуЗаполненияДокумента(
			Ошибки, 
			ТекстСообщения, 
			ЭтотОбъект, 
			"Объект.ФормаРеорганизации", 
			"Объект", 
			Отказ); 
		КонецЕсли;
		Если Не ЗначениеЗаполнено(НаименованиеРеорганизованнойОрганизации) Тогда
			ТекстСообщения = НСтр("ru = 'Не указано наименование реорганизованной организации.'");
				...
				ТекстСообщения, 
				ЭтотОбъект, 
				"Объект.НаименованиеРеорганизованнойОрганизации", 
				"Объект", 
				Отказ); 
		КонецЕсли;
		Если ФормаРеорганизации <> "0" Тогда
			...
			ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.ИННСоответствуетТребованиям(
				ИННРеорганизованнойОрганизации, Истина, ТекстСообщения) Тогда
				ОбщегоНазначенияБЗК.ДобавитьОшибкуЗаполненияДокумента(
				Ошибки, 
				ТекстСообщения, 
				ЭтотОбъект, 
			...
			Если Не ЗначениеЗаполнено(КППРеорганизованнойОрганизации) Тогда
				...
				"Объект", 
				Отказ); 
			ИначеЕсли НЕ РегламентированныеДанныеКлиентСервер.КППСоответствуетТребованиям(КППРеорганизованнойОрганизации, 
	...
	Если ДополнительныеСвойства.Свойство("СведенияОДоходах") Тогда
		СведенияОДоходахФизическихЛиц = ДополнительныеСвойства.СведенияОДоходах.Выгрузить();
	...
	КонецЕсли;
	Если ДополнительныеСвойства.Свойство("СведенияОВычетах") Тогда
		СведенияОВычетахФизическихЛиц = ДополнительныеСвойства.СведенияОВычетах.Выгрузить();
	...
	СведенияОДоходахИВзносах = Документы.СправкиНДФЛДляПередачиВНалоговыйОрган.ТаблицыДанныхОДоходахИВычетовДокументов(Ссылка);
	Пока ВыборкаСотрудниковДляПроверки.СледующийПоЗначениюПоля("НомерСтроки") Цикл
		...
		ПараметрыПроверкиДанныхФизическихЛиц.ОшибкиДокумента			= Ошибки;
		...
		Если ВыборкаСотрудниковДляПроверки.ЕстьПовторяющиесяСправки Тогда
			...
			ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Сотрудники", 
			ВыборкаСотрудниковДляПроверки.НомерСтроки, "Сотрудник");
			ОбщегоНазначенияБЗК.ДобавитьОшибкуЗаполненияДокумента(
			Ошибки, 
			ТекстСообщения, 
			ЭтотОбъект, 
		...
		Если ВыборкаСотрудниковДляПроверки.ЕстьПовторяющиесяНомераСправок Тогда
			ПутьКДанным = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти("Сотрудники", 
			ВыборкаСотрудниковДляПроверки.НомерСтроки, "НомерСправки");
		...
		НСтр("ru = 'Сотрудник %1, справка № %2:'"), ВыборкаСотрудниковДляПроверки.СотрудникНаименование, 
	...
	Если НалоговыйПериод >= 2015  Тогда
		ДанныеПредставителя = ИнтерфейсыВзаимодействияБРОВызовСервера.СведенияОПредставителеПоРегистрацииВНалоговомОргане(
		РегистрацияВНалоговомОргане, Дата);
