...
Процедура СотрудникПриИзмененииНаСервере()
	СправкиНДФЛФормы.ОчиститьДанныеСправки(Объект, Истина);
...
&НаСервере
Процедура УстановитьВидимостьАдресов(ВидимостьАдресов)
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Адрес", "Видимость", ВидимостьАдресов);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "АдресЗаРубежом", "Видимость", ВидимостьАдресов);
КонецПроцедуры
...
Процедура ПрочитатьДанныеСправкиИзВременногоХранилища(АдресДанныхСправкиВХранилище)
	...
	ВидимостьАдресов = ДанныеСправки.НалоговыйПериод < 2019;
	УстановитьВидимостьАдресов(ВидимостьАдресов);
	Если Не ВидимостьАдресов Тогда
		ДанныеСправки.Удалить("Адрес");
		ДанныеСправки.Удалить("АдресЗаРубежом");
	КонецЕсли;	
	...
	Для каждого СтрокаДоходов Из ДанныеСправки.МассивСведенийОДоходах Цикл
		СтрокаСведенияОДоходах = Объект.СведенияОДоходах.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаСведенияОДоходах, СтрокаДоходов);
		СтрокаСведенияОДоходах.Ставка = Ставка;
	КонецЦикла;
...
Процедура ОбновитьНалогиНаСервере()
	...
	СправкиНДФЛФормы.ДляНалогаНаПрибыльПрочитатьДанныеОДоходахИНалогахСотрудника(
		ЭтотОбъект,
		Объект,
		Объект.СведенияОДоходах, 
		Объект.СведенияОВычетах, 
...
Процедура ЗаполнитьДанныеСправкиНаСервере()
	...
	ЗаполнитьТаблицуДоходыНалоги(Объект);
	СправкиНДФЛФормыКлиентСервер.УстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, Объект, ДокументПроведен, Истина);
	СправкиНДФЛФормыКлиентСервер.УстановитьПризнакНаличияГражданства(ЭтотОбъект, Объект);
КонецПроцедуры
...
Процедура ПрочитатьДанныеФизическогоЛица(ФизическоеЛицо)
	...
	СправкиНДФЛ.ПрочитатьДанныеСотрудников(
		ДанныеСправок, Объект.НалоговыйПериод, Объект.Дата, Не ДокументПроведен);
		УстановитьПредставлениеАдресов();
...
Процедура УстановитьПредставлениеАдресов()
	...
	Если Не ПустаяСтрока(Объект.Адрес) Тогда
		...
		Объект.Адрес, Справочники.ВидыКонтактнойИнформации.АдресПоПропискеФизическиеЛица);
		АдресПредставление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтруктураАдреса);
	Иначе
		АдресПредставление = НСтр("ru = '<Не заполнен>'");
	КонецЕсли;
	
	Если Не ПустаяСтрока(Объект.АдресЗарубежом) Тогда
		СтруктураАдреса = РаботаСАдресами.ПредыдущаяСтруктураКонтактнойИнформацииXML(
		Объект.АдресЗарубежом, Справочники.ВидыКонтактнойИнформации.АдресЗаПределамиРФФизическиеЛица);
		АдресЗарубежомПредставление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтруктураАдреса);
	Иначе
		АдресЗарубежомПредставление = НСтр("ru = '<Не заполнен>'");
	КонецЕсли;
...
Функция ПоместитьДанныеСправкиВХранилище()
	...
	ПараметрыДанныхСправки.Ставка					= Ставка;
