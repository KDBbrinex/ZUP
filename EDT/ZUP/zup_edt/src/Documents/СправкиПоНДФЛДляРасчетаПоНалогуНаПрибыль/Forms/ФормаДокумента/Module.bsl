...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Ключ = Документы.СправкиПоНДФЛДляРасчетаПоНалогуНаПрибыль.ПустаяСсылка() Тогда
		...
		Объект.НалоговыйПериод = Мин(Год(УчетНДФЛ.ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли()) - 1, Объект.НалоговыйПериод);

		ПриПолученииДанныхНаСервере();
	КонецЕсли;
	
	УчетНДФЛФормы.СведенияОДоходахПриСозданииНаСервере(ЭтотОбъект, "Объект.СведенияОДоходах.КодДохода", "СведенияОДоходахКодВычета", "СведенияОДоходахСуммаВычета");	
	
	СправкиНДФЛФормы.ЗаполнитьСписокКонтролируемыхПолей(ЭтотОбъект, Истина);
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Элементы.Сотрудники.ТекущаяСтрока <> Неопределено Тогда
		...
		СправкиНДФЛФормыКлиентСервер.УстановитьИнфонадписьИсправления(ИнфоНадписьИсправления, ДанныеТекущейСтроки, ДокументПроведен);
	КонецЕсли;
...
Процедура ДатаПриИзмененииНаСервере()
	...
	ПараметрыПредставления.СводнаяСправка = Истина;
	ПараметрыПредставления.ДляНалогаНаПрибыль = Истина;
...
Процедура НалоговыйПериодПриИзмененииНаСервере()
	...
	СправкиНДФЛФормы.ДляНалогаНаПрибыльПрочитатьДанные(
		Объект.Сотрудники, 
...
Процедура ОрганизацияПриИзмененииНаСервере()
	...
	СправкиНДФЛФормы.ДляНалогаНаПрибыльПрочитатьДанныеОДоходахИНалогах(
		ДанныеСправок, 
		Объект.СведенияОДоходах, 
		Объект.СведенияОВычетах, 
		Объект.НалоговыйПериод, 
...
Процедура ПриПолученииДанныхНаСервере()
	...
	Если Объект.НалоговыйПериод >= Год(УчетНДФЛ.ДатаВозвратаДоходовПоЦеннымБумагамИзДекларацииПоПрибыли()) Тогда
...
Процедура ПрочитатьДанныеФизическогоЛица(ФизическоеЛицо)
	...
	Если СтрокиПоСотруднику.Количество() > 0 Тогда
		...
		СтрокиПоСотруднику, Объект.НалоговыйПериод, Объект.Дата, Истина);
...
Процедура УстановитьПредставлениеАдресов(ДанныеСправок)
	...
	Для Каждого ДанныеСправкиПоСотруднику Из ДанныеСправок Цикл
		...
		Если Не ПустаяСтрока(ДанныеСправкиПоСотруднику.Адрес) Тогда
			...
			ДанныеСправкиПоСотруднику.АдресПредставление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтруктураАдреса);
		Иначе
		...
		Если Не ПустаяСтрока(ДанныеСправкиПоСотруднику.АдресЗарубежом) Тогда
			...
			ДанныеСправкиПоСотруднику.АдресЗарубежомПредставление = УправлениеКонтактнойИнформацией.ПредставлениеКонтактнойИнформации(СтруктураАдреса);
		Иначе
...
Функция ПоместитьДанныеСправкиВоВременноеХранилище(НоваяСтрока = Ложь)
	...
	ПараметрыДанныхСправки.СтрокиСведенийОДоходах	= СведенияОДоходах;
	ПараметрыДанныхСправки.СтрокиСведенийОВычетах	= СведенияОВычетах;
	ПараметрыДанныхСправки.НомерСправки				= ДанныеСотрудника.НомерСправки;
	ПараметрыДанныхСправки.Ставка					= ДанныеСотрудника.Ставка;
