...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПечатьВозможнаДляЦиклаОбмена = КонтекстЭДОСервер.ПечатьВозможнаДляЦиклаОбмена(Объект.ЦиклОбмена.Тип);
...
Процедура СодержимоеВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ВАрхиве = Элемент.ТекущиеДанные.ВАрхиве;
	Если ВАрхиве Тогда 
		КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(, 7, 3, Истина);
		Возврат;
	КонецЕсли;
	
	// отчетность ПФР
	Если ТипДокумента = "ФайлОтчетностиПФР" Тогда
		Если Прав(Элемент.ТекущиеДанные.Имя, 4) = ".der" Тогда
			ПоказатьКакСертификат(ВыбраннаяСтрока);
		Иначе
	...
	ИначеЕсли ТипДокумента = "ИзвещениеОПолучении" Тогда
		ПоказатьИзвещениеОПолучении(ВыбраннаяСтрока, Истина);
	...
	ИначеЕсли ТипДокумента = "УведомлениеОбОтказеОтчетаФСГС" Тогда
		ПоказатьПротоколФСГС(Элемент.ТекущиеДанные, Истина);
	ИначеЕсли ТипДокумента = "УведомлениеОбОтклоненииОтчетаФСГС" Тогда
		ПоказатьПротоколФСГС(Элемент.ТекущиеДанные, Истина);
	...
	ИначеЕсли ТипДокумента = "РассылкаФСГС" Тогда
		ПоказатьКакТекст(ВыбраннаяСтрока, "windows-1251");
	ИначеЕсли ТипДокумента = "ШаблонФСГС" Тогда
		ПоказатьКакXML(ВыбраннаяСтрока);
	ИначеЕсли ТипДокумента = "ОписаниеРассылкиШаблоновФСГС" Тогда
		ПоказатьКакXML(ВыбраннаяСтрока);
...
Процедура КомандаКонтейнер(Команда)
	...
	Если ВАрхиве > 0 Тогда
		ОписаниеОповещения  = ?(ВАрхиве = Всего, Неопределено, Новый ОписаниеОповещения("КомандаКонтейнерПродолжение", ЭтотОбъект));
		КонтекстЭДОКлиент.ПоказатьУведомлениеАрхивныхФайлов(ОписаниеОповещения, 25 + ?(ВАрхиве > 1, 1, 0), 0, ВАрхиве = Всего);
		Возврат;
	КонецЕсли;
	
	КомандаКонтейнерПродолжение(КодВозвратаДиалога.Да, Неопределено);
		
КонецПроцедуры

&НаКлиенте
Процедура КомандаКонтейнерПродолжение(Результат, ВхПараметры) Экспорт
...
Процедура КомандаРаспознать(Команда)
	...
	КонтекстЭДОКлиент.ОбновитьСтатусыСвязанногоОбъектаПриНеобходимости(Объект.ЦиклОбмена);
	ОбновитьДанные();
...
Процедура ОбновитьДанные()
				...
				Если ЗначениеЗаполнено(ИмяФайлаОшибки) Тогда
					...
					Если ДеревоXML <> Неопределено Тогда
						...
						Если УзелОписание = Неопределено Тогда
							УзелОписание = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(ДеревоXML,,, "Описание", "Э"); // УОПП
						КонецЕсли;
						Если УзелОписание <> Неопределено Тогда
						...
						Если УзелКодОшибки = Неопределено Тогда
							УзелКодОшибки = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(ДеревоXML,,, "КодОшибки", "Э"); // УОПП
						КонецЕсли;
						Если УзелКодОшибки <> Неопределено Тогда
						...
						РезультатПоиска = КонтекстЭДОСервер.НайтиСтрокиДереваЗначений(ДеревоXML, , МассивНайденныхСтрок, "ДопСв", "Э", Истина);
						
						Если РезультатПоиска <> Неопределено Тогда
							Для каждого НайденнаяСтрока Из МассивНайденныхСтрок Цикл
								ОписаниеОшибки = ОписаниеОшибки + Символы.ПС + НайденнаяСтрока.Значение;
							КонецЦикла;
						КонецЕсли;
	...
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.РассылкаФСГС Тогда
		...
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
		Элементы.ГруппаСостояниеСообщения.Видимость = Ложь;
	ИначеЕсли Объект.Тип = Перечисления.ТипыТранспортныхСообщений.РассылкаШаблоновФСГС Тогда
		Элементы.ГруппаСодержимоеСтраницы.ТекущаяСтраница = Элементы.СодержимоеДерево;
		ЗаполнитьДеревоСодержимого(Вложения, СодержимоеТранспортныхСообщений, Элементы.СодержимоеТранспортныхСообщений);
		Элементы.ГруппаСостояниеСообщения.Видимость = Ложь;
...
Процедура ЗаполнитьДеревоСодержимого(ВложенияТекущегоСообщения, Содержимое, ТПСодержимого)
	...
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтклоненииОтчетаФСГС);
	...
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ШаблонФСГС);
	ТипыДИВ.Добавить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ОписаниеРассылкиШаблоновФСГС);
	...
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтклоненииОтчетаФСГС, 1);
	...
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ШаблонФСГС, 3);
	ПриоритетыПрорисовкиДИВ.Вставить(Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ОписаниеРассылкиШаблоновФСГС, 3);
	...
	Если ИменаПодписанныхФайловИзДругихСообщений.Количество() > 0 Тогда
		...
		Если НЕ ЗначениеЗаполнено(СообщениеВладелец) Тогда
			...
			ИначеЕсли Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОПриемеВОбработкуОтчетаФСГС
				  ...
				  ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОтчетаФСГС
				  ИЛИ Объект.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтклоненииОтчетаФСГС Тогда
	...
	Для Каждого Вложение Из Вложения Цикл
		Если ЗначениеЗаполнено(Вложение.ПриоритетТипа) Тогда
			ОпределитьСтрокуВДеревеСодержимого(Содержимое, Вложение.ИмяФайла, Вложение.Тип, Вложение.СообщениеВладелец, , , , Вложение.ТипФайлаОтчетностиПФР, 1, Вложение.ВАрхиве);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Вложение Из Вложения Цикл
		Если Вложение.Тип = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.ЭлектронноЦифроваяПодпись Тогда
			...
			Если СтрПодписанныйФайл <> Неопределено Тогда
				ОпределитьСтрокуВДеревеСодержимого(СтрПодписанныйФайл, Вложение.ИмяФайла, Вложение.Тип, , Вложение.ЭЦПСертификат, Вложение.ЭЦПЭтоПодписьАбонента, Вложение.ЭЦПСтатусПроверки, 2, , Вложение.ВАрхиве);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОформитьСтрокуСодержимого(ДанныеСтроки)
		...
		ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтказеОтчетаФСГС Тогда
			ПредставлениеЯчейки = "Уведомление об отказе";
		ИначеЕсли ТипДанных = Перечисления.ТипыСодержимогоТранспортногоКонтейнера.УведомлениеОбОтклоненииОтчетаФСГС Тогда
			ПредставлениеЯчейки = "Уведомление об отклонении";
...
&НаСервере
Процедура ОпределитьСтрокуВДеревеСодержимого(ДеревоСодержимого, ИмяСодержимого, ТипСодержимого, СообщениеВладелец = Неопределено, СертификатПодписи = Неопределено, ПризнакПодписиАбонента = Неопределено, СтатусПроверкиПодписи = Неопределено, ТипФайлаОтчетности = Неопределено, ТекстОформленияУровень = 0, Знач ВАрхиве = Ложь)
...
Процедура ПолучитьТранспортныйКонтейнерНаСервере()
	...
	ПолученныеКонтейнеры = КонтекстЭДОСервер.ПолучитьТранспортныйКонтейнер(Объект.Ссылка, Истина, Истина);
	Для Каждого ПолученныйКонтейнер Из ПолученныеКонтейнеры Цикл
		...
		НовыйКонтейнер.ВАрхиве = ПолученныйКонтейнер.ВАрхиве;
	КонецЦикла;
...
Процедура ЗаполнитьВложенияТранспортногоСообщенияНаСервере()
	...
	Для Каждого ПолученноеВложение Из ПолученныеВложения Цикл
		...
		НовоеВложение.ВАрхиве = ПолученноеВложение.ВАрхиве;
	КонецЦикла;
...
Функция ПолучитьСписокКонтейнеров(ОбъектСсылка)
	...
	Контейнеры = КонтекстЭДОСервер.ПолучитьТранспортныйКонтейнер(ОбъектСсылка, Истина, Истина);
...
Функция ПолучитьСоответствиеПодтверждений(ТипСообщения)
	...
	КонтекстЭДОСервер = ДокументооборотСКОВызовСервера.ПолучитьОбработкуЭДО();
	СоответствиеПодтверждений = КонтекстЭДОСервер.ТипыСообщенийПоТипамОснований();
...
Процедура ПоказатьОтветНаЗапросИОС(СвойстваДокумента)
	...
	КонтекстЭДОКлиент.ПоказатьОтветНаЗапросИОСИзФормыТранспортногоСообщения(Объект.ЦиклОбмена, СообщениеВладелецФайла, ИмяФайлаДокумента);
...
Процедура ПоказатьПодтверждениеПолученияЗапросаПФР(СвойстваДокумента)
	...
	КонтекстЭДОКлиент.ПоказатьПодтверждениеПолученияЗапросаПФР(Объект.ЦиклОбмена, СообщениеВладелецФайла, ИмяФайлаДокумента);
...
Процедура ПоказатьПриложениеПФР(СвойстваДокумента)
	...
	КонтекстЭДОКлиент.ПоказатьПриложениеПФР(Объект.Ссылка, Результат);
...
Процедура ПоказатьПодтверждениеПолученияПисьмаПФР(СвойстваДокумента)
	...
	КонтекстЭДОКлиент.ПоказатьПодтверждениеПолученияПисьмаПФР(Объект.ЦиклОбмена, СообщениеВладелецФайла, ИмяФайлаДокумента);
...
Процедура ПоказатьКакXML(СвойстваДокумента, ПолеНастроекПечати = Неопределено)
	...
	Если ЭтоВеб ИЛИ ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() Тогда
...
&НаКлиенте
Процедура ПоказатьКакСертификат(СвойстваДокумента)
	...
	ДвДанныеСертификата = Вложения[0].Данные;
...
&НаКлиенте
Процедура ПоказатьИзвещениеОПолучении(ВыбраннаяСтрока, ПечатьВозможна = Ложь) Экспорт
	...
	КонтекстЭДОКлиент.ПоказатьИзвещениеОПолучении(Объект.ЦиклОбмена, Сообщение, ИмяФайлаИзвещения, ПечатьВозможнаДляЦиклаОбмена И ПечатьВозможна);
...
Процедура ПоказатьКвитанцияОПриемеДокумент(ВыбраннаяСтрока) Экспорт
	...
	Сообщение = ?(ЗначениеЗаполнено(СвойстваДокумента.СообщениеВладелец), СвойстваДокумента.СообщениеВладелец, Объект.Ссылка);
	ИмяФайлаДокумента = СвойстваДокумента.Имя;
	КонтекстЭДОКлиент.ПоказатьКвитанцияОПриемеДокумент(Объект.ЦиклОбмена, Сообщение, ИмяФайлаДокумента, Истина);
