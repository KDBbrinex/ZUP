...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	УчетНДФЛ.СформироватьПеречисленныйНалог(Ссылка, Движения, Отказ, Организация, ДатаПлатежа,
						ДанныеДляПроведения.ПеречисленныйНалог, ДанныеДляПроведения.ФизическиеЛица, РегистрацияВНалоговомОргане);
	
	// Регистрация обязательств.
	УчетНДФЛ.СформироватьНДФЛПеречисленный(Движения, Отказ);
	
	УплатаНДФЛКРаспределению = Движения.УплатаНДФЛНалоговымиАгентамиКРаспределению.ВыгрузитьКолонки();
	Для каждого СтрокаДвижений Из Движения.УплатаНДФЛНалоговымиАгентамиКРаспределению Цикл
		НоваяСтрока = УплатаНДФЛКРаспределению.Добавить();
		...
		КонецЕсли;
	КонецЦикла;
	УплатаНДФЛКРаспределению.Свернуть("Период,Организация,РегистрацияВНалоговомОргане","Сумма");
	УчетНДФЛ.СформироватьНДФЛПеречисленный(Движения, Отказ, УплатаНДФЛКРаспределению);
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
		// Дополним обязательства сведениями по статьям.
		БазаРаспределения = Движения.НДФЛПеречисленный.Выгрузить();
		БазаРаспределения.Свернуть("ФизическоеЛицо,РегистрацияВНалоговомОргане,СтатьяФинансирования,СтатьяРасходов", "Сумма");
		БазаРаспределения.ЗаполнитьЗначения(СтатьяФинансирования, "СтатьяФинансирования");
		БазаРаспределения.ЗаполнитьЗначения(СтатьяРасходов, "СтатьяРасходов");
		...
		ДанныеДляДополнения.ПоляОтбора 			= "ФизическоеЛицо,РегистрацияВНалоговомОргане";
		ОтражениеЗарплатыВУчете.ДополнитьНДФЛПеречисленныйСведениямиОРаспределенииПоСтатьям(Движения, ДанныеДляДополнения);;
	КонецЕсли;
