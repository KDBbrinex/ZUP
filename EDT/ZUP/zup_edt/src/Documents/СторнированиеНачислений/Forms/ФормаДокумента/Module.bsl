...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	ИсправлениеДокументовЗарплатаКадрыКлиент.ОповеститьОСторнированииДокумента(
		...
		ОписаниеСторнируемогоДокумента.МесяцНачисленияИмя);
...
Процедура ДанныеВРеквизиты()
	...
	Если ЗначениеЗаполнено(Объект.СторнируемыйДокумент) Тогда
		...
		ИсправлениеДокументовЗарплатаКадры.ЗаполнитьПараметрыИсправляемогоДокумента(
			ПараметрыСторнируемогоДокумента, Объект.СторнируемыйДокумент, ОписаниеСторнируемогоДокумента.МесяцНачисленияИмя);
	КонецЕсли;
...
Процедура РассчитатьПослеЗаполнения(ДокументОбъект)
	...
	Иначе
		...
		ИспользованиеПериода = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ОбщегоНазначения.ВыгрузитьКолонку(ДокументОбъект.Сторнировано, "Начисление", Истина), "ИспользованиеПериода");
		...
		ДокументМенеджер = Документы[ДокументОбъект.СторнируемыйДокумент.Метаданные().Имя];
		ДокументСторнирован = ДокументМенеджер.СторнироватьПоУчетам(
			НаборыЗаписей,
...
Процедура ПериодРегистрацииПриИзменении(ЗадатьВопрос = Истина)
	...
	Если РазностьПериодов < 0 Тогда
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			Формат(ПараметрыСторнируемогоДокумента.ПериодРегистрации, "ДФ='ММММ гггг'"));
