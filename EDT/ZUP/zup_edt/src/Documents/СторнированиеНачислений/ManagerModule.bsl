...
Процедура ЗаполнитьСведенияОПособиях(РеквизитыДляПроведения, ДанныеДляПроведения)
	...
	ОписаниеСторнируемогоДокумента = ОписаниеСторнируемогоДокумента(РеквизитыДляПроведения.СторнируемыйДокумент);
	ИмяПериодРегистрации = ОписаниеСторнируемогоДокумента.МесяцНачисленияИмя;
	ИмяРеквизитаЭЛНКарантинПоКоронавирусу = ОписаниеСторнируемогоДокумента.ИмяРеквизитаЭЛНКарантинПоКоронавирусу;
	
	ИменаРеквизитов = "Организация," + ИмяПериодРегистрации;
	Если ЗначениеЗаполнено(ИмяРеквизитаЭЛНКарантинПоКоронавирусу) Тогда
		ИменаРеквизитов = ИменаРеквизитов + "," + ИмяРеквизитаЭЛНКарантинПоКоронавирусу;
	КонецЕсли;
	
	РеквизитыСторнируемогоДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		РеквизитыДляПроведения.СторнируемыйДокумент, ИменаРеквизитов);
	
	Если ЗначениеЗаполнено(ИмяРеквизитаЭЛНКарантинПоКоронавирусу) Тогда
		ЭЛНКарантинПоКоронавирусу = РеквизитыСторнируемогоДокумента[ИмяРеквизитаЭЛНКарантинПоКоронавирусу];
	Иначе
	...
	ПособиеПлатитУчастникПилотногоПроекта = ПрямыеВыплатыПособийСоциальногоСтрахования.ПособиеПлатитУчастникПилотногоПроекта(
		РеквизитыСторнируемогоДокумента.Организация,
		РеквизитыСторнируемогоДокумента[ИмяПериодРегистрации],
...
Функция РеквизитыДляПроведения(ДокументСсылка, СторнируемыйДокумент)
	...
	ОписаниеСторнируемогоДокумента = ОписаниеСторнируемогоДокумента(СторнируемыйДокумент);
	ИмяПериодРегистрации = ОписаниеСторнируемогоДокумента.МесяцНачисленияИмя;
	
	Если ОписаниеСторнируемогоДокумента.ЕстьДатаВыплаты Тогда
	...
	Иначе
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ").Дата", ")." + ИмяПериодРегистрации + ", МЕСЯЦ)");
	КонецЕсли;
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, " КАК Документ.Отпуск", " КАК " + ОписаниеСторнируемогоДокумента.ПолноеИмя);
...
Функция ОписаниеСторнируемогоДокумента(СторнируемыйДокумент) Экспорт
	...
	Описание.Вставить("ЕстьДатаВыплаты", Ложь);
	Описание.Вставить("ПолноеИмя", Неопределено);
	Описание.Вставить("ИмяРеквизитаЭЛНКарантинПоКоронавирусу", "ЭЛНКарантинПоКоронавирусу");
	...
	КонецЕсли;
	Если МетаданныеСторнируемогоДокумента.Реквизиты.Найти(Описание.ИмяРеквизитаЭЛНКарантинПоКоронавирусу) = Неопределено Тогда
		Описание.ИмяРеквизитаЭЛНКарантинПоКоронавирусу = "";
	КонецЕсли;
	
	Описание.ЕстьДатаВыплаты = МетаданныеСторнируемогоДокумента.Реквизиты.Найти("ПланируемаяДатаВыплаты") <> Неопределено;
	Описание.ПолноеИмя = МетаданныеСторнируемогоДокумента.ПолноеИмя();
