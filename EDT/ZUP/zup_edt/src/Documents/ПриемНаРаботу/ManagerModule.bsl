...
Процедура ДополнитьДаннымиДляПроведенияЗаявленийОВеденииТрудовыхКнижек(ДанныеДляПроведения, ДокументСсылка)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ПриемНаРаботу.ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки КАК Период,
		|	ПриемНаРаботу.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ПриемНаРаботу.Организация КАК Организация,
		|	ПриемНаРаботу.ВидЗаявленияОВыбореСпособаВеденияТрудовойКнижки КАК ВидЗаявления,
		...
		|	И ПриемНаРаботу.ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки <> ДАТАВРЕМЯ(1, 1, 1)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ПриемНаРаботуСпискомСотрудники.ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки,
		|	ПриемНаРаботуСпискомСотрудники.ФизическоеЛицо,
		|	ПриемНаРаботуСпискомСотрудники.Ссылка.Организация,
		|	ПриемНаРаботуСпискомСотрудники.ВидЗаявленияОВыбореСпособаВеденияТрудовойКнижки,
		...
		|	И ПриемНаРаботуСпискомСотрудники.ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки <> ДАТАВРЕМЯ(1, 1, 1)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Функция ДанныеДляПроведенияМероприятияТрудовойДеятельности(СсылкаНаДокумент, ТолькоПроведенные = Ложь) Экспорт
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(Запрос.МенеджерВременныхТаблиц, "ВТДанныеДокументов") Тогда
		...
		Запрос.Текст =
			...
			|	ДанныеДокументов.РазрядКатегория КАК РазрядКатегория
			|ИЗ
...
Функция ДанныеДляПроведения(ДокументСсылка, ТолькоБронированиеПозиции, НачисленияУтверждены, Организация)
	...
	Запрос.Текст =
		...
		|			ТОГДА ДОБАВИТЬКДАТЕ(Сотрудники.ДатаПриема, ДЕНЬ, &ДнейСохраненияБрони + 1)
...
Процедура ОбработкаПроверкиЗаполнения(ДокументОбъект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Для каждого ПроверяемаяСтруктура Из ПроверяемыеСтруктуры Цикл
		...
		Если ПроверяемаяСтруктура.ОтразитьВТрудовойКнижке Тогда
			...
			Если ЗначениеЗаполнено(ПроверяемаяСтруктура.ВидЗаявленияОВыбореСпособаВеденияТрудовойКнижки)
				И Не ЗначениеЗаполнено(ПроверяемаяСтруктура.ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки) Тогда
				
				ТекстСообщения = НСтр("ru = 'Не указана дата заявления о выборе способа ведения трудовой книжки'");
				ПутьКДанным = ПутьКДанным + "ДатаЗаявленияОВыбореСпособаВеденияТрудовойКнижки";
				
			ИначеЕсли Не ЗначениеЗаполнено(ПроверяемаяСтруктура.ВидЗаявленияОВыбореСпособаВеденияТрудовойКнижки)
