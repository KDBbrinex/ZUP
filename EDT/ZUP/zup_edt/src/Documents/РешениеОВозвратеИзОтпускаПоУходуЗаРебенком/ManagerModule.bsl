...
Функция ПараметрыЗаполненияМероприятий(Объект) Экспорт
	...
	ПараметрыЗаполнения.ДатаСобытия = Объект.ДатаВозврата;
	...
	Если ДанныеСотрудников.Количество() > 0 Тогда
		...
		ПараметрыЗаполнения.Позиция = ТекущиеКадровыеДанныеСотрудника.ДолжностьПоШтатномуРасписанию;
		ПараметрыЗаполнения.Подразделение = ТекущиеКадровыеДанныеСотрудника.Подразделение;
		ПараметрыЗаполнения.Должность = ТекущиеКадровыеДанныеСотрудника.Должность;
...
Функция ВидСобытияАдаптацииУвольнения() Экспорт
	...
	Возврат АдаптацияУвольнение.СобытиеВозвратИзОтпускаПоУходуЗаРебенком();
...
Функция ПоляПроверкиУникальности() Экспорт
	...
	ПоляПроверки.Добавить("ДокументОснование");
