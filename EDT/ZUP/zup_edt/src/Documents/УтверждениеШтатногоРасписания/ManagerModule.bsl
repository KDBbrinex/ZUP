...
Функция ПечатнаяФормаПриказа(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеДляПечати.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(ОбластьПодвал.Параметры.ФИОИсполнителя) Тогда
			РезультатСклонения = "";
			Если ФизическиеЛицаЗарплатаКадры.Просклонять(Строка(ОбластьПодвал.Параметры.ФИОИсполнителя), 4, РезультатСклонения, ДанныеДляПечати.ПолИсполнителя, ДанныеДляПечати.ФИОИсполнителя) Тогда
