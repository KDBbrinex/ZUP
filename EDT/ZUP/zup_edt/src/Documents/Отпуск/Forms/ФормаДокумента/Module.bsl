...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НовыйДокумент Тогда
		...
		Если ЗначениеЗаполнено(Объект.Сотрудник) Тогда
			УстановитьКоличествоДнейОсновногоОтпуска(ОстаткиОтпусков.ОписаниеВидаОтпуска(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.Основной")));
	...
	Если Не Параметры.Ключ.Пустая() Тогда
		...
		Если Параметры.ПерезаполнитьНачисления 
			Или (ВыполнятьПредварительныйРасчетДокумента Или ЗначениеЗаполнено(Объект.ДокументЗаполнения)) И РасчетНеобходимоВыполнить Тогда 
...
Процедура СпроситьПользователяОПродленииОтпуска()
	...
	КоличествоДнейПересеченияОсновногоОтпуска = КоличествоДнейПересеченияОсновногоОтпуска();
	
	Если КоличествоДнейПересеченияОсновногоОтпуска = 0 Тогда
		ПересечениеДопОтпуска = ПересечениеДопОтпуска();
		Если ПересечениеДопОтпуска.КоличествоДнейПересечения <> Неопределено Тогда
...
Процедура ОтменаПродленияОсновногоОтпускаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"НадписьПродлитьОтпуск", 
			"Заголовок", 
			СтрШаблон(НСтр("ru='Продлить основной отпуск на %1 дн...'"), КоличествоДнейПересеченияОсновногоОтпуска()));
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"Страницы",
			"ТекущаяСтраница",
			Элементы.ПредоставлениеОтпускаСтраница);
...
Процедура ОтменаПродленияДопОтпускаЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ДополнительныеОтпускаКоличествоДнейПриИзмененииНаСервере(ИдентификаторПродленногоДопОтпуска);
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, 
			"НадписьПродлитьОтпуск", 
			"Заголовок", 
			СтрШаблон(НСтр("ru='Продлить дополнительный отпуск на %1 дн...'"), ПересечениеДопОтпуска().КоличествоДнейПересечения));
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"Страницы",
			"ТекущаяСтраница",
			Элементы.ПредоставлениеОтпускаСтраница);
...
Процедура ПродлитьДополнительныйОтпускНаСервере(ПродлениеДопОтпуска);
	...
	ДополнительныеОтпускаКоличествоДнейПриИзмененииНаСервере(ПродлениеДопОтпуска.ИдентификаторСтроки);
...
&НаСервере
Функция КоличествоДнейПересеченияОсновногоОтпуска()
	КоличествоДнейПересеченияОсновногоОтпуска = 0;
	Если ОбщегоНазначенияБЗК.ИнтервалыПересекаются(ДатаНачалаБольничного, ДатаОкончанияБольничного,
		Объект.ДатаНачалаОсновногоОтпуска, Объект.ДатаОкончанияОсновногоОтпуска) Тогда
		
		ДатаОкончанияОтпуска = Макс(ДатаОкончанияБольничного, Объект.ДатаОкончанияОсновногоОтпуска);
		КоличествоДнейПересеченияОсновногоОтпуска = ПолучитьКоличествоДнейПересеченияСОтпуском(
			Объект.ДатаНачалаОсновногоОтпуска, ДатаОкончанияОтпуска, ДатаНачалаБольничного, ДатаОкончанияБольничного);
...
Функция ПересечениеДопОтпуска()
	...
	Для Каждого СтрокаДополнительногоОтпуска Из Объект.ДополнительныеОтпуска Цикл
		Если ВидыЕжегодныхОплачиваемыхОтпусков.Найти(СтрокаДополнительногоОтпуска.ВидОтпуска) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ОбщегоНазначенияБЗК.ИнтервалыПересекаются(ДатаНачалаБольничного, ДатаОкончанияБольничного,
			СтрокаДополнительногоОтпуска.ДатаНачала, СтрокаДополнительногоОтпуска.ДатаОкончания) Тогда
			ДатаОкончанияДопОтпуска = Макс(ДатаОкончанияБольничного, СтрокаДополнительногоОтпуска.ДатаОкончания);
			ПересечениеДопОтпуска.Вставить("КоличествоДнейПересечения", ПолучитьКоличествоДнейПересеченияСОтпуском(
				...
				ДатаНачалаБольничного, ДатаОкончанияБольничного));
...
Функция КоличествоДнейПриостановкиОтпуска(ОсновнойОтпуск = Истина) 
	...
	Для Каждого Пересечение Из ПересечениеДопОтпуска Цикл
		Если Пересечение.Ключ = "КоличествоДнейПересечения" Тогда
			КоличествоДнейПересеченияСДопОтпусками = Пересечение.Значение;
		КонецЕсли;
	КонецЦикла;
	
	Возврат КоличествоДнейПересеченияСДопОтпусками;
...
Процедура ДополнитьНачисленияВыплачиваемыеКОтпуску(ТаблицаНачислений, МенеджерРасчета, ПериодРасчетаЗарплаты)
	...
	// нужно определить периоды действия для единоразовых начислений
	...
	Если (Объект.ПредоставитьЕдиновременнуюВыплатуКОтпуску И ЗначениеЗаполнено(Объект.ВидРасчетаЕдиновременнойВыплатыКОтпуску) 
		Или Объект.ПредоставитьМатериальнуюПомощьПриОтпуске) 
		И ЗначениеЗаполнено(Объект.ИсправленныйДокумент) Тогда
		РеквизитыИсправления = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ИсправленныйДокумент, "ПериодРегистрации,ПредоставитьЕдиновременнуюВыплатуКОтпуску,ВидРасчетаЕдиновременнойВыплатыКОтпуску,ПредоставитьМатериальнуюПомощьПриОтпуске");
		// Если единовременная выплата начислялась и будет начисляться - сохраним ее период в документе-исправлении
		Если Объект.ПредоставитьЕдиновременнуюВыплатуКОтпуску И ЗначениеЗаполнено(Объект.ВидРасчетаЕдиновременнойВыплатыКОтпуску) 
			И РеквизитыИсправления.ПредоставитьЕдиновременнуюВыплатуКОтпуску И ЗначениеЗаполнено(РеквизитыИсправления.ВидРасчетаЕдиновременнойВыплатыКОтпуску) Тогда
			ПериодРегистрацииЕдиновременнойВыплаты = РеквизитыИсправления.ПериодРегистрации;
		КонецЕсли;
		// Если матпомощь начислялась и будет начисляться - сохраним ее период в документе-исправлении
		Если Объект.ПредоставитьМатериальнуюПомощьПриОтпуске И РеквизитыИсправления.ПредоставитьМатериальнуюПомощьПриОтпуске Тогда
			ПериодРегистрацииМатериальнойПомощи = РеквизитыИсправления.ПериодРегистрации;
		КонецЕсли;
	КонецЕсли;
		...
		Если Объект.РасчетДенежногоСодержания Тогда
			Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба.РасчетДенежногоСодержания") Тогда 
				...
				Если НормаДней <> 0 Тогда 
					Форма.РассчитаноОтпускЧАЭС = КоличествоОплачиваемыхДнейПоВидуОтпуска(Объект, ВидОтпускаПострадавшимВАварииЧАЭС) * Объект.СохраняемоеДенежноеСодержание / НормаДней;	
				КонецЕсли;	
			КонецЕсли;
		Иначе
			Форма.РассчитаноОтпускЧАЭС = КоличествоОплачиваемыхДнейПоВидуОтпуска(Объект, ВидОтпускаПострадавшимВАварииЧАЭС) * Объект.СреднийЗаработок;	
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьПериодПриостановки(СсылкаНаБольничный, Идентификатор)
	...
	ДанныеБольничногоЛиста = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаБольничный, "ДатаНачала, ДатаОкончания");
	Если Не (ОбщегоНазначенияБЗК.ДатаВИнтервале(ДанныеБольничногоЛиста.ДатаНачала, 
		Объект.ДатаНачалаПериодаОтсутствия, Объект.ДатаОкончанияПериодаОтсутствия) 
		Или ОбщегоНазначенияБЗК.ДатаВИнтервале(ДанныеБольничногоЛиста.ДатаОкончания, 
		Объект.ДатаНачалаПериодаОтсутствия, Объект.ДатаОкончанияПериодаОтсутствия)) Тогда
		
		СообщениеПользователю = СтрШаблон(НСтр("ru='%1 не пересекается с данным отпуском'"), СсылкаНаБольничный);
	...
	ПериодПересечения = ОбщегоНазначенияБЗК.ПериодПересеченияДвухИнтервалов(ДанныеБольничногоЛиста.ДатаНачала, 
		ДанныеБольничногоЛиста.ДатаОкончания,Объект.ДатаНачалаПериодаОтсутствия, Объект.ДатаОкончанияПериодаОтсутствия);
