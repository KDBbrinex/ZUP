...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
				...
				Если ЗначениеЗаполнено(ДанныеМероприятия.ДатаОтмены) Тогда
					Если ДанныеМероприятия.ДатаОтмены > Дата Тогда
						...
						ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СсылкаНаДокумент,
							"Объект.Сотрудники[" + Формат(ДанныеСотрудника.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник" , , Отказ);
					КонецЕсли;
					Если ДанныеМероприятия.ДатаОтмены < ДанныеМероприятия.ДатаМероприятия Тогда
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = '%1: Дата отмены мероприятия не может быть меньше даты мероприятия.'"), ДанныеСотрудника.Сотрудник);
						ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, СсылкаНаДокумент,
							"Объект.Сотрудники[" + Формат(ДанныеСотрудника.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "].Сотрудник" , , Отказ);
					КонецЕсли;
				КонецЕсли;
	...
	Запрос.Текст =
		...
		|	МероприятияТрудовойДеятельностиПереданные.Регистратор КАК ДругойРегистратор
		|ИЗ
		|	ВТТаблицаСотрудников КАК ТаблицаСотрудников
		...
		|			ПО ТаблицаМероприятий.Сотрудник = МероприятияТрудовойДеятельностиПрочие.ФизическоеЛицо
		...
		|			ПО ТаблицаМероприятий.Сотрудник = МероприятияТрудовойДеятельности.ФизическоеЛицо
		...
		|			ПО ТаблицаМероприятий.ИдМероприятия = МероприятияТрудовойДеятельностиПереданные.ИдМероприятия
		|				И (ВЫБОР
		|					КОГДА ТаблицаМероприятий.ДатаОтмены = ДАТАВРЕМЯ(1, 1, 1)
		|						ТОГДА ЛОЖЬ
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ = МероприятияТрудовойДеятельностиПереданные.Отменено)
		|				И (МероприятияТрудовойДеятельностиПереданные.Регистратор <> &Ссылка)
		...
		|			ИЛИ НЕ МероприятияТрудовойДеятельностиПереданные.ИдМероприятия ЕСТЬ NULL)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
