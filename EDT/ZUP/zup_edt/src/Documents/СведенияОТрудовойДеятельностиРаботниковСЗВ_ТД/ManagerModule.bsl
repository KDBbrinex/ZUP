...
Функция ВыборкаПоСотрудникам(МассивОбъектов)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА СведенияОТрудовойДеятельностиРаботниковСЗВ_ТДМероприятия.ТрудоваяФункция.Описание = """"
		|			ТОГДА СведенияОТрудовойДеятельностиРаботниковСЗВ_ТДМероприятия.ТрудоваяФункция
		|		ИНАЧЕ СведенияОТрудовойДеятельностиРаботниковСЗВ_ТДМероприятия.ТрудоваяФункция.Описание
		...
		|			ПО СведенияОТрудовойДеятельностиРаботниковСЗВ_ТДМероприятия.ИдМероприятия = МероприятияТрудовойДеятельностиПереданные.ИдМероприятия
	...
	Пока ВыборкаПоРаботникам.СледующийПоЗначениюПоля("НомерСтрокиСотрудника") Цикл
		...
		Пока ВыборкаПоРаботникам.Следующий() Цикл
			...
			Иначе
				...
				НаборЗаписейМероприятие.ВидПР.Значение = СокрЛП(ВыборкаПоРаботникам.ТрудоваяФункцияОписание);
...
Процедура ЗаполнитьВДвиженияхПризнакПринятоВПФР(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	Документ.СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД КАК СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МероприятияТрудовойДеятельностиПереданные КАК МероприятияТрудовойДеятельностиПереданные
		|		ПО СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД.Ссылка = МероприятияТрудовойДеятельностиПереданные.Регистратор
		|			И (НЕ МероприятияТрудовойДеятельностиПереданные.ПринятоВПФР)
		|ГДЕ
...
Процедура ОбновитьДвиженияПереданныхМероприятийНеПринятыхВПФР(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ МероприятияТрудовойДеятельностиПереданные.Регистратор В (&МассивОбновленных)
		|
		|СГРУППИРОВАТЬ ПО
		|	МероприятияТрудовойДеятельностиПереданные.Регистратор
		|
		|ИМЕЮЩИЕ
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ МероприятияТрудовойДеятельностиПереданные.ПринятоВПФР) > 1";
...
Функция ДанныеДляПроведенияДокумента(СсылкаНаДокумент) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД.Ссылка = &Ссылка
		|	И СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД.ПринятоВПФР
...
Функция ДанныеДляПроведенияПоМероприятияТрудовойДеятельностиПереданные(СсылкаНаДокумент, ТолькоПроведенные = Ложь) Экспорт
	...
	Запрос.Текст =
		...
		|	КОНЕЦ КАК Отменено,
		|	ЕСТЬNULL(ВЫБОР
		|			КОГДА ТаблицаДокумента.ТрудоваяФункция.Описание = """"
		|				ТОГДА ТаблицаДокумента.ТрудоваяФункция.Наименование
		|			ИНАЧЕ ТаблицаДокумента.ТрудоваяФункция.Описание
		|		КОНЕЦ, """") КАК ТрудоваяФункцияОписание,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МАКСИМУМ(ДанныеМероприятий.ПринятоВПФР) КАК ПринятоВПФР,
		...
		|			И ДанныеМероприятий.ПринятоВПФР = ПринятостьМероприятий.ПринятоВПФР
		|ГДЕ
		|	ПринятостьМероприятий.ПринятоВПФР
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка";
...
Процедура СформироватьНаборыЗаписейМероприятияТрудовойДеятельностиПереданные(ПараметрыОбновления) Экспорт
	...
	Запрос.Текст =
		...
		|	Документ.СведенияОТрудовойДеятельностиРаботниковСЗВ_ТД.Мероприятия КАК ТаблицаДокумента
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.МероприятияТрудовойДеятельностиПереданные КАК МероприятияТрудовойДеятельностиПереданные
		|		ПО ТаблицаДокумента.ИдМероприятия = МероприятияТрудовойДеятельностиПереданные.ИдМероприятия
		|			И (ВЫБОР
		|				КОГДА ТаблицаДокумента.ДатаОтмены = ДАТАВРЕМЯ(1, 1, 1)
		|					ТОГДА ЛОЖЬ
		|				ИНАЧЕ ИСТИНА
...
Функция ЗаполнитьПоДаннымФайла(ДокументОбъект, АдресДанныхСЗВТД, ИмяФайла, Отказ) Экспорт
	...
	ДокументОбъект.Дата = ДатаИзСтроки(ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(ДеревоXML, "СЗВ_ТД/ДатаЗаполнения"));
	...
	Для Каждого ОбъектЗЛ Из ЗЛ Цикл
		...
		СтрокаСотрудники.ДатаРождения = ДатаИзСтроки(ОбъектЗЛ.ДатаРождения);
		...
		Если ЗначениеЗаполнено(СтрокаСотрудники.Сотрудник) Тогда
			...
			Если Заявления <> Неопределено Тогда
				...
				Если ЗначениеЗаполнено(ВидЗаявления) Тогда
					...
					СтрокаЗаявленияСотрудников.Дата = ДатаИзСтроки(ДатаЗаявления);
			...
			Если Мероприятия <> Неопределено Тогда
				...
				Для Каждого ОбъектМероприятия Из Мероприятия Цикл
					...
					СтрокаМероприятия.ДатаМероприятия = ДатаИзСтроки(ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(ОбъектМероприятия, "Дата"));
					СтрокаМероприятия.ВидМероприятия = ВидМероприятия(ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(ОбъектМероприятия, "Вид"));
					...
					Если Основания.Количество() > 0 Тогда
						...
						СтрокаМероприятия.ДатаДокументаОснования = ДатаИзСтроки(ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(Основания[0], "Дата"));
					...
					Если Основания.Количество() > 1 Тогда
						...
						СтрокаМероприятия.ДатаВторогоДокументаОснования = ДатаИзСтроки(ЭлектронныеТрудовыеКнижки.ПолучитьСвойствоXDTO(Основания[1], "Дата"));
...
Функция ВидМероприятия(НомерВидаМероприятия)
...
Функция ДозаполнитьМероприятияДокумента(ДокументОбъект, Отказ)
	...
	Запрос.Текст =
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	МероприятияТрудовойДеятельности.ФизическоеЛицо КАК Сотрудник,
		...
		|		ПО ФизическиеЛицаМероприятий.ФизическоеЛицо = МероприятияТрудовойДеятельности.ФизическоеЛицо
		|			И ФизическиеЛицаМероприятий.ИдМероприятия = МероприятияТрудовойДеятельности.ИдМероприятия
		|			И (ВЫБОР
		...
		|	МероприятияТрудовойДеятельностиПрочие.НаименованиеВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.ДатаВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.СерияВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.НомерВторогоДокументаОснования,
		...
		|		ПО ФизическиеЛицаМероприятий.ФизическоеЛицо = МероприятияТрудовойДеятельностиПрочие.ФизическоеЛицо
		|			И ФизическиеЛицаМероприятий.ИдМероприятия = МероприятияТрудовойДеятельностиПрочие.ИдМероприятия";
...
Функция ОснованиеУвольнения(Часть, Статья, Пункт, Подпункт)
