...
Процедура ОбработкаЗаполнения(ДокументОснование, СтандартнаяОбработка)
	...
	Если СотрудникиСоСреднимЗаработком.Количество() > 0 Тогда
		...
		|				И Сотрудник В (&СотрудникиСоСреднимЗаработком)) КАК НачисленияУдержанияПоСотрудникамОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ДанныеУвольнения.Сотрудник КАК ГрафикРаботы,
		...
		Пока Выборка.Следующий() Цикл
			...
			СуммаВыплаты = Макс(КоличествоДнейЧасов * СреднийЗаработок - Выборка.ВыходноеПособие, 0);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если РаспределятьРезультатыРасчета Тогда
		ОтражениеЗарплатыВБухучетеРасширенный.ПроверитьРезультатыРаспределенияНачисленийУдержанийОбъекта(
			ЭтотОбъект, "НачисленияУдержанияВзносы,Удержания,НДФЛ", Отказ, ВидВыплаты);
	КонецЕсли;
