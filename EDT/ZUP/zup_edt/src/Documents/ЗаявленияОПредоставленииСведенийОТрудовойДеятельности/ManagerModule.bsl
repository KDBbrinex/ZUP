...
Функция ПечатнаяФормаПредоставленииСведенийОТрудовойДеятельности(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Для Каждого СтрокаДанных Из ДанныеПечати Цикл
		Если СтрокаДанных.Ссылка <> ТекущаяСсылка Тогда
			ТекущаяСсылка = СтрокаДанных.Ссылка;
			НомерСтрокиНачало = ДокументРезультат.ВысотаТаблицы + 1;
			ЭтоСледующийДокумент = Истина;
		КонецЕсли;
		Если ДокументРезультат.ВысотаТаблицы > 0 Тогда
			ДокументРезультат.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		ДокументРезультат.Вывести(ТабличныйДокументОбластьЗаявления(Макет, СтрокаДанных));
		Если ЭтоСледующийДокумент Тогда
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ДокументРезультат, НомерСтрокиНачало, ОбъектыПечати, СтрокаДанных.Ссылка);
			ЭтоСледующийДокумент = Ложь;
		КонецЕсли;
	КонецЦикла;
