...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ФактПроживанияВКрымуВидимость = Параметры.ФактПроживанияВКрымуВидимость;
...
&НаКлиенте
Процедура ФактПроживанияВКрымуПриИзменении(Элемент)
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ФактПроживанияВКрыму", "Видимость", ФактПроживанияВКрымуВидимость);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПачкаСЗВ_6_4ОбъектЗаписиОСтажеОсобыеУсловияТруда", "ВыбиратьТип", ФактПроживанияВКрымуВидимость);
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "ПачкаСЗВ_6_4ОбъектЗаписиОСтажеОснованиеВыслугиЛет", "ВыбиратьТип", ФактПроживанияВКрымуВидимость);
...
Процедура УстановитьПараметрыВыбораВидаДеятельности()
	...
	Если ФактПроживанияВКрыму Тогда 
		Элементы.ЗаписиОСтажеВидДеятельности.ПараметрыВыбора = Новый ФиксированныйМассив(Новый Массив);
		Возврат;
	КонецЕсли;
	
	ВидыДеятельностиКрым = Новый Массив;
	ВидыДеятельностиКрым.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОбщественноПолезнойДеятельностиСЗВК.ДОГКРЫМ"));
	ВидыДеятельностиКрым.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОбщественноПолезнойДеятельностиСЗВК.ИПКРЫМ"));
	ВидыДеятельностиКрым.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОбщественноПолезнойДеятельностиСЗВК.ТДКРЫМ"));
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("ВидыДеятельностиКрым", ВидыДеятельностиКрым);
	
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВидыОбщественноПолезнойДеятельностиСЗВК.Ссылка КАК Ссылка
	               |ИЗ
	               |	Справочник.ВидыОбщественноПолезнойДеятельностиСЗВК КАК ВидыОбщественноПолезнойДеятельностиСЗВК
	               |ГДЕ
	               |	НЕ ВидыОбщественноПолезнойДеятельностиСЗВК.Ссылка В (&ВидыДеятельностиКрым)";
	
	ВидыДеятельности = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	
	ПараметрыВыбора = Новый Массив;
	ПараметрВыбора = Новый ПараметрВыбора("Отбор.Ссылка", Новый ФиксированныйМассив(ВидыДеятельности));
	ПараметрыВыбора.Добавить(ПараметрВыбора);
	
	Элементы.ЗаписиОСтажеВидДеятельности.ПараметрыВыбора = Новый ФиксированныйМассив(ПараметрыВыбора);
...
Процедура ДанныеИзВременногоХранилищаВДанныеФормы(АдресВоВременномХранилище)
	...
	ФактПроживанияВКрыму = ДанныеДокументаПоСотруднику.ФактПроживанияВКрыму;
...
Функция ДанныеСотрудникаВоВременноеХранилище()		
	...
	ДанныеСотрудника.Вставить("ФактПроживанияВКрыму", ФактПроживанияВКрыму);
...
Процедура ЗаписиОСтажеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда 
		ПерсонифицированныйУчетКлиентСервер.УстановитьЗаголовкиВСтрокеТаблицы(ЭтотОбъект, ТекущиеДанные, ОписаниеКолонокЗаголовковТаблицыЗаписиОСтаже(ЭтотОбъект));
	КонецЕсли;
	
	Если НоваяСтрока И Не Копирование И Не ФактПроживанияВКрымуВидимость Тогда 
		ТекущиеДанные.ОсобыеУсловияТруда = ПредопределенноеЗначение("Справочник.ОсобыеУсловияТрудаДляСЗВКПФР.ПустаяСсылка");
		ТекущиеДанные.ОснованиеВыслугиЛет = ПредопределенноеЗначение("Справочник.ОснованияДосрочногоНазначенияПенсииДляСЗВК.ПустаяСсылка");
	КонецЕсли;
