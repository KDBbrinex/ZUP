...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ПерсонифицированныйУчет.ПроверитьДанныеСотрудников(Сотрудники, Дата(1, 1, 1), Дата(2099, 1, 1), ЗаписиОСтаже, Отказ);
...
Функция СформироватьЗапросДляПроверки()
	...
	|	ПачкаДокументовСЗВ_КСотрудники.ФактПроживанияВКрыму
	...
	|	СотрудникиДокумента.ФактПроживанияВКрыму,
	...
	|	Сотрудники.ФактПроживанияВКрыму,
			...
			Если ЗначениеЗаполнено(ВыборкаЗаписиСтажа.НомерСтрокиСтаж) Тогда
				...
				Пока ВыборкаЗаписиСтажа.СледующийПоЗначениюПоля("НомерСтрокиСтаж") Цикл
					...
					Иначе//Обе даты заполнены
						...
						ДатаОкончанияМакс = ?(Запись.ФактПроживанияВКрыму, '20151231', '20011231');
						Если Запись.ДатаОкончанияПериода > ДатаОкончанияМакс Тогда
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'дата окончания периода должна быть не позднее %1'"), Формат(ДатаОкончанияМакс, "ДЛФ=ДД"));
							СообщитьОбОшибкеСтажа(ТекстОшибки, ВыборкаЗаписиСтажа, Отказ);
						КонецЕсли;
