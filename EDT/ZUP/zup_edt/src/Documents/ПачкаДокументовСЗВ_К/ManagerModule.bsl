...
Функция СформироватьЗапросПоЗаписямСтажаДляПечати(МассивОбъектов)
	...
	|	СведенияСЗВКСотрудники.ФактПроживанияВКрыму КАК ФактПроживанияВКрыму,
	...
	|	СведенияСЗВКСотрудники.ФактПроживанияВКрыму,
...
Функция СформироватьЗапросПоШапкеДляПечати(МассивСсылок)
	...
	|	ПачкаДокументовСЗВ_К.СоставПачки КАК СоставПачки,
	|	ПачкаДокументовСЗВ_К.ТипСведений КАК ТипСведений,
	...
	|	ПачкаДокументовСЗВ_К.Дата КАК Дата,
	|	ПачкаДокументовСЗВ_К.ОкончаниеОтчетногоПериода КАК ОкончаниеОтчетногоПериода,
	|	ПачкаДокументовСЗВ_К.НомерПачки КАК НомерПачки
	...
	|	ПачкаДокументовСЗВ_К.РегистрацияВНалоговомОргане КАК РегистрацияВНалоговомОргане,
	|	ПачкаДокументовСЗВ_К.СоставПачки КАК СоставПачки,
	|	ПачкаДокументовСЗВ_К.ТипСведений КАК ТипСведений,
...
Функция СформироватьЗапросДляПечатиАДВ_1(МассивСсылок)
	...
	|	ДанныеОПачке.НомерПачки КАК НомерПачки,
	|	ДанныеОПачке.ДокументовВПачке КАК ДокументовВПачке,
	|	ЕСТЬNULL(ФИОРуководителейПоследние.РасшифровкаПодписи, """") КАК Руководитель,
	|	ДанныеОПачке.Должность КАК Должность,
	|	ДанныеОПачке.Дата КАК Дата,
...
Функция СформироватьВыходнойФайл(ВыборкаПоШапкеДокумента, ВыборкаПоЗаписямСтажа)
	...
	Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
		...
		ИначеЕсли ВыборкаПоШапкеДокумента.ТипСведений = "ОТМН" Тогда
		...
		Если ВыборкаПоЗаписямСтажа.ФактПроживанияВКрыму Тогда
			НаборЗаписейСЗВК.ФактПроживанияКрым.Значение = "1";
		Иначе
			НаборЗаписейСЗВК.Удалить("ФактПроживанияКрым");
		КонецЕсли;
		...
		Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерПериодаТрудовойДеятельности") Цикл
			...
			Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерОсновнойЗаписи") Цикл
				...
				ВыводитьОсобенностиУчета = ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.ТерриториальныеУсловияКод)
					Или ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.ОсобыеУсловияТрудаКод)
					Или ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.ОснованиеИсчисляемогоСтажаКод)
				    Или ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.ТретийПараметрИсчисляемогоСтажа)
					Или ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.ОснованиеВыслугиЛетКод)
					Или ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.Должность);
					
				ОсновнаяСтрокаСЗВК.ОсобенностиУчета.НеВыводитьВФайл = Не ВыводитьОсобенностиУчета;
...
Функция СформироватьЗапросПоЗаписямСтажа(Ссылка)
	...
	|	СведенияСЗВКСотрудники.Имя КАК Имя,
	|	СведенияСЗВКСотрудники.Отчество КАК Отчество,
	|	СведенияСЗВКСотрудники.ТерриториальныеУсловияНа_31_12_2001Код КАК ТерриториальныеУсловияНа_31_12_2001Код,
	|	СведенияСЗВКСотрудники.РайонныйКоэффициентНа_31_12_2001 КАК РайонныйКоэффициентНа_31_12_2001,
	|	СведенияСЗВКСотрудники.ФактПроживанияВКрыму КАК ФактПроживанияВКрыму,
	|	ЗаписиОСтаже.НомерПериодаТрудовойДеятельности КАК НомерПериодаТрудовойДеятельности,
	|	ЗаписиОСтаже.НомерОсновнойЗаписи КАК НомерОсновнойЗаписи,
	|	ЗаписиОСтаже.НомерДополнительнойЗаписи КАК НомерДополнительнойЗаписи,
	...
	|	ЗаписиОСтаже.ВторойПараметрВыслугиЛет КАК ВторойПараметрВыслугиЛет,
	|	ЗаписиОСтаже.ТретийПараметрВыслугиЛет КАК ТретийПараметрВыслугиЛет,
	|	ЗаписиОСтаже.Должность КАК Должность
	|ИЗ
	|	ВТСотрудникиДокумента КАК СведенияСЗВКСотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПачкаДокументовСЗВ_К.ЗаписиОСтаже КАК ЗаписиОСтаже
	|		ПО СведенияСЗВКСотрудники.Сотрудник = ЗаписиОСтаже.Сотрудник
...
Функция СформироватьПечатнуюФормуСЗВ_К(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаПоШапкеДокумента.Следующий() Цикл 
		...
		ИспользоватьФорму2019 = ПерсонифицированныйУчет.ОрганизацияЗарегистрированаВКрыму(ВыборкаПоШапкеДокумента.КПП, ВыборкаПоШапкеДокумента.РегистрацияВНалоговомОргане)
			Или (ДатаПостановления485п <> Неопределено И ДатаПостановления485п <= ВыборкаПоШапкеДокумента.Дата);
		Если ИспользоватьФорму2019 Тогда
			ВывестиПечатнуюФормуСЗВ_К(ТабличныйДокумент, ВыборкаПоШапкеДокумента, ВыборкаПоЗаписямСтажа, ТаблицаИтоговогоСтажа, НомерДокумента);
			УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабличныйДокумент, НачалоФормы, ОбъектыПечати, ВыборкаПоШапкеДокумента.Ссылка);
			Продолжить;
		КонецЕсли;
		...
		Если ВыборкаПоЗаписямСтажа.НайтиСледующий(СтруктураОтбора) Тогда  
			...
			Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
				...
				ОбластьШапка.Параметры.Исходная = ВыборкаПоШапкеДокумента.ТипСведений = "ИСХД";
				ОбластьШапка.Параметры.Корректирующая = ВыборкаПоШапкеДокумента.ТипСведений = "КОРР";
				ОбластьШапка.Параметры.Отменяющая = ВыборкаПоШапкеДокумента.ТипСведений = "ОТМН";
...
Процедура ВывестиПечатнуюФормуСЗВ_К(ТабличныйДокумент, ВыборкаПоШапкеДокумента, ВыборкаПоЗаписямСтажа, ТаблицаИтоговогоСтажа, НомерДокумента)
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ПачкаДокументовСЗВ_К.ПФ_MXL_ФормаСЗВ_К_2019");
	...
	Если ВыборкаПоЗаписямСтажа.НайтиСледующий(СтруктураОтбора) Тогда  
		...
		Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерСтрокиСотрудник") Цикл
			...
			ОбластьШапка.Параметры.ДатаРождения = ПерсонифицированныйУчет.ДатаВОтчет(ВыборкаПоЗаписямСтажа.ДатаРождения);
			ОбластьШапка.Параметры.ТерриториальныеУсловия = СокрЛП(ВРег(ВыборкаПоЗаписямСтажа.ТерриториальныеУсловияНа_31_12_2001Код)) + ВыборкаПоЗаписямСтажа.РайонныйКоэффициентНа_31_12_2001;
			ОбластьШапка.Параметры.ФактПроживанияВКрыму = ?(ВыборкаПоЗаписямСтажа.ФактПроживанияВКрыму, "Х", "");
			
			ТабличныйДокумент.Вывести(ОбластьШапка);
			ТабличныйДокумент.Вывести(ОбластьШапкаПериоды);
			
			НомерСтрокиВариантСЗВК = ТабличныйДокумент.ВысотаТаблицы - 14;
			...
			Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерПериодаТрудовойДеятельности") Цикл
				...
				Если ЗначениеЗаполнено(ВыборкаПоЗаписямСтажа.НомерПериодаТрудовойДеятельности) Тогда
					...
					// Контроль выхода за пределы листа
					Если Не ТабличныйДокумент.ПроверитьВывод(ОбластьШапкаДЕЯТ) Тогда
						Если КолВоЛистов = 1 Тогда
							ТабличныйДокумент.Область(НомерСтрокиВариантСЗВК,2).Текст =	"Форма СЗВ-К (начало)";
							ТабличныйДокумент.Область(НомерСтрокиВариантСЗВК,13).Текст = "Лист 1";
						КонецЕсли;
					...
					Пока ВыборкаПоЗаписямСтажа.СледующийПоЗначениюПоля("НомерОсновнойЗаписи") Цикл
						...
						Если Не ТабличныйДокумент.ПроверитьВывод(ОбластьСтрокаСтажа) Тогда
							Если КолВоЛистов = 1 Тогда
								ТабличныйДокумент.Область(НомерСтрокиВариантСЗВК,2).Текст	= "Форма СЗВ-К (начало)";
								ТабличныйДокумент.Область(НомерСтрокиВариантСЗВК,13).Текст	= "Лист 1";
							КонецЕсли;
...
Функция СформироватьПечатнуюФормуАДВ_6_1(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаДокументов.СледующийПоЗначениюПоля("Документ") Цикл
		...
		ИспользоватьФорму2019 = ПерсонифицированныйУчет.ОрганизацияЗарегистрированаВКрыму(ВыборкаДокументов.КПП, ВыборкаДокументов.РегистрацияВНалоговомОргане)
			Или (ДатаПостановления485п <> Неопределено И ДатаПостановления485п <= ВыборкаДокументов.Дата);
		МакетОписи = УправлениеПечатью.МакетПечатнойФормы(?(ИспользоватьФорму2019,
			"ОбщийМакет.ПФ_MXL_ФормаАДВ_6_1_2019", "ОбщийМакет.ПФ_MXL_ФормаАДВ_6_1_2017"));
