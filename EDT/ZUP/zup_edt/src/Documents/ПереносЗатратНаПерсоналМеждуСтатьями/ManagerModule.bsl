...
Процедура ПровестиПоУчетам(ДокументСсылка, РежимПроведения, Отказ, ВидыУчетов = Неопределено, Движения = Неопределено, Объект = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Если ДанныеДляПроведения.НДФЛ <> Неопределено И ДанныеДляПроведения.НДФЛ.Количество() > 0 Тогда
			УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛИКорректировкиВыплаты(ДанныеДляПроведенияУчетЗарплаты, Отказ, ДанныеДляПроведения.НДФЛ, Неопределено);
		КонецЕсли;
...
Функция УдержанияПоСотрудникам(РеквизитыДляПроведения, Сотрудники, НачисленияСотрудников)
	...
	КонецЦикла;
	ОтражениеЗарплатыВБухучетеРасширенный.СоздатьВТБухучетНачисленийПоТаблицеБухучетНачислений(МенеджерВременныхТаблиц, БухучетНачислений, "ВТБухучетНачисленийВыходнаяТаблица");
