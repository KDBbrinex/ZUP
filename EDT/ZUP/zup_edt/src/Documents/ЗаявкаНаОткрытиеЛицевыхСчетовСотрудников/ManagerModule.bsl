...
// АПК:581-выкл Экспорт (возможно, избыточный) сделан для симметрии API
...
Процедура ЗаполнитьДанныеАдреса(СтрокаДанных, НазваниеКолонки)
	...
	Адрес = РаботаСАдресами.СведенияОбАдресе(СтрокаДанных[НазваниеКолонки]);
	
	СтрокаДанных["Индекс" + НазваниеКолонки] = Адрес.Индекс;
	
	СтрокаДанных["СтранаНазвание" + НазваниеКолонки] = Адрес.Страна;
	СтрокаДанных["СтранаКод"      + НазваниеКолонки] = Адрес.КодСтраны;
	
	СтрокаДанных["РегионНазвание"   + НазваниеКолонки] = Адрес.Регион;
	СтрокаДанных["РегионСокращение" + НазваниеКолонки] = Адрес.РегионТипКраткий;
	
	СтрокаДанных["РайонНазвание"   + НазваниеКолонки] = Адрес.Район;
	СтрокаДанных["РайонСокращение" + НазваниеКолонки] = Адрес.РайонТипКраткий;
	
	СтрокаДанных["ГородНазвание"   + НазваниеКолонки] = Адрес.Город;
	СтрокаДанных["ГородСокращение" + НазваниеКолонки] = Адрес.ГородТипКраткий;
	
	// НаселенныйПункт
	
	// Адрес может включать различные внутригородские территории, которые надо превратить в населенный пункт.
	// Когда адрес содержит несколько территорий, то представления вышестоящих добавляем к названию населенного пункта,
	// а сокращение берем по последнему населенному пункту в иерархии.
	// Когда же указано что-то одно, то считаем этот элемент адреса населенным пунктом.
	
	ВнутригородскиеТерритории = Новый Массив;
	ВнутригородскиеТерритории.Добавить("ВнутригородскойРайон");
	ВнутригородскиеТерритории.Добавить("НаселенныйПункт");
	ВнутригородскиеТерритории.Добавить("Территория");
	
	ПредставленияТерриторий = Новый Массив;
	НаселенныйПунктНазвание   = "";
	НаселенныйПунктСокращение = "";
	
	Для Каждого Территория Из ВнутригородскиеТерритории Цикл
		Если ЗначениеЗаполнено(Адрес[Территория]) Тогда
			НаселенныйПунктНазвание   = Адрес[Территория];
			НаселенныйПунктСокращение = Адрес[Территория + "ТипКраткий"];
			ПредставленияТерриторий.Добавить(НаселенныйПунктНазвание + " " + НаселенныйПунктСокращение);
		КонецЕсли;	
	КонецЦикла;
	
	Если ПредставленияТерриторий.Количество() > 1 Тогда
		 // В последнем по иерархии заменяем представление на название, чтобы убрать его сокращение.
		ПредставленияТерриторий[ПредставленияТерриторий.ВГраница()] = НаселенныйПунктНазвание;
		СтрокаДанных["НаселенныйПунктНазвание" + НазваниеКолонки] = СтрСоединить(ПредставленияТерриторий, ", ");
	Иначе	
		СтрокаДанных["НаселенныйПунктНазвание" + НазваниеКолонки] = НаселенныйПунктНазвание;
	КонецЕсли;	
	СтрокаДанных["НаселенныйПунктСокращение" + НазваниеКолонки] = НаселенныйПунктСокращение;
	
	// Конец НаселенныйПункт
	
	СтрокаДанных["УлицаНазвание"   + НазваниеКолонки] = Адрес.Улица;
	СтрокаДанных["УлицаСокращение" + НазваниеКолонки] = Адрес.УлицаТипКраткий;
	
	СтрокаДанных["Дом" + НазваниеКолонки] = Адрес.Здание.Номер;
	Если ЗначениеЗаполнено(Адрес.Корпуса) Тогда
		СтрокаДанных["Корпус"   + НазваниеКолонки] = Адрес.Корпуса[0].Номер;
	КонецЕсли;	
	Если ЗначениеЗаполнено(Адрес.Помещения) Тогда
		СтрокаДанных["Квартира" + НазваниеКолонки] = Адрес.Помещения[0].Номер;
	КонецЕсли;	
...
Процедура ЗаполнитьДанныеМестаРождения(СтрокаДанных, НазваниеКолонки)
	...
	СтрокаДанных["РегионНазвание"          + НазваниеКолонки] = СтруктураМестоРождения.Область;
	СтрокаДанных["РайонНазвание"           + НазваниеКолонки] = СтруктураМестоРождения.Район;
	СтрокаДанных["НаселенныйПунктНазвание" + НазваниеКолонки] = СтруктураМестоРождения.НаселенныйПункт;
