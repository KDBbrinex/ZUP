...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	КадровыеДанныеСотрудников = ДанныеСотрудников(ОбщегоНазначения.ВыгрузитьКолонку(ТекущийОбъект.Сотрудники, "Сотрудник"));
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = ЭтаФорма Тогда
		...
		Если ИмяСобытия = "ИзмененыЕжегодныеОтпуска" Тогда
			...
			ОбработатьИзменениеЕжегодныхОтпусков(Параметр);
...
Процедура СотрудникиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Сотрудник) Тогда
		...
		ИначеЕсли Поле.Имя = "СотрудникиПредставлениеПраваНаОтпуск" Тогда
			...
			Иначе	
				ПараметрыОткрытия.Вставить("ДатаОстатков", ДатаОстатковОтпусков(Объект.Месяц));
			КонецЕсли;
			...
			ПараметрыОткрытия.Вставить("ОтсутствияСдвигающиеРабочийГод", ОтсутствияСдвигающиеРабочийГодСотрудника(ТекущиеДанные.Сотрудник));
...
Процедура ЗаполнитьПоДаннымДолжностиПоШтатномуРасписанию(ТекущиеДанныеСтрокиСотрудники, ДанныеДолжностиПоШтатномуРасписанию, ОчищатьСтарыеЗначений, ЗаполнитьТолькоНачисления = Ложь)
	...
	Для Каждого ОписаниеПоказателя Из ДанныеДолжностиПоШтатномуРасписанию.НезависимыеПоказатели Цикл
...
Процедура ОбновитьПредставленияПолейТаблицыСотрудников()
	...
	КадровыеДанныеСотрудников = ДанныеСотрудников(ОбщегоНазначения.ВыгрузитьКолонку(Объект.Сотрудники, "Сотрудник"));
	Для каждого СтрокаСотрудника Из Объект.Сотрудники Цикл
		
		УстановитьКадровыеДанныеСтрокиСотрудника(СтрокаСотрудника, КадровыеДанныеСотрудников);
...
&НаСервере
Функция ДанныеСотрудников(Сотрудники)
	
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(
		Истина, Сотрудники, "ДатаПриема,ПриказОПриеме");
...
Процедура УдалитьДанныеСотрудника(УдаляемыйСотрудник)
	...
	УдалитьОтсутствияСдвигающиеРабочийГод(УдаляемыйСотрудник);
...
Процедура СотрудникиСотрудникПриИзмененииНаСервере(ИдентификаторСтрокиСотрудника, ТекущийСотрудник = Неопределено)
	...
	Если ТекущийСотрудник <> Неопределено
		И ТекущийСотрудник <> ТекущиеДанные.Сотрудник Тогда
		
		КадровыеДанныеСотрудников = ДанныеСотрудников(ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ТекущиеДанные.Сотрудник));
		УстановитьКадровыеДанныеСтрокиСотрудника(ТекущиеДанные, КадровыеДанныеСотрудников);
	...
	Если Объект.ЕжегодныеОтпуска.НайтиСтроки(СтруктураОтбораПоСотруднику).Количество() = 0 Тогда
		...
		Для каждого СтрокаБезСотрудника Из СтрокиБезСотрудника Цикл
			...
			СтрокаБезСотрудника.ДатаОстатка = ДатаОстатковОтпусков(Объект.Месяц);
		КонецЦикла;
...
Процедура СотрудникиОбработкаВыбораНаСервере(ВыбранныеСотрудники)
	...
	Для каждого Сотрудник Из СписокСотрудников Цикл
		...
		СотрудникиКДобавлению.Добавить(Сотрудник);
		
	КонецЦикла;
	
	Если СотрудникиКДобавлению.Количество() > 0 Тогда
		
		КадровыеДанныеСотрудников = ДанныеСотрудников(СотрудникиКДобавлению);
		
		Для каждого Сотрудник Из СотрудникиКДобавлению Цикл
			
			НоваяСтрокаСотрудников = Объект.Сотрудники.Добавить();
			НоваяСтрокаСотрудников.Сотрудник = Сотрудник;
			
			УстановитьКадровыеДанныеСтрокиСотрудника(НоваяСтрокаСотрудников, КадровыеДанныеСотрудников);
...
&НаСервере
Процедура ОбработатьИзменениеЕжегодныхОтпусков(Параметр)
	
	Сотрудник = Параметр.Сотрудник;
	ДатаОстатков = Параметр.ДатаОстатков;
	ЕжегодныеОтпуска = Параметр.ЕжегодныеОтпуска;
	ОстаткиОтпусковПоРабочимГодам = Параметр.ОстаткиОтпусковПоРабочимГодам;
	ОтсутствияСдвигающиеРабочийГод = Параметр.ОтсутствияСдвигающиеРабочийГод;
	
	ЗаполнитьОстаткиОтпусковПоРабочимГодам(Сотрудник, ДатаОстатков, ОстаткиОтпусковПоРабочимГодам);
	ЗаполнитьЕжегодныеОтпуска(Сотрудник, ДатаОстатков, ЕжегодныеОтпуска);
	ЗаполнитьОтсутствияСдвигающиеРабочийГод(Сотрудник, ДатаОстатков, ОтсутствияСдвигающиеРабочийГод);
	
КонецПроцедуры

&НаСервере
Функция ОтсутствияСдвигающиеРабочийГодСотрудника(Сотрудник)
	
	СтрокиОтсутствий = Объект.ОтсутствияСдвигающиеРабочийГод.Выгрузить(Новый Структура("Сотрудник", Сотрудник));
	Возврат ОбщегоНазначения.ТаблицаЗначенийВМассив(СтрокиОтсутствий);
	
КонецФункции

&НаСервере
Процедура ЗаполнитьОтсутствияСдвигающиеРабочийГод(Сотрудник, ДатаОстатков, ОтсутствияСдвигающиеРабочийГод)
	
	УдалитьОтсутствияСдвигающиеРабочийГод(Сотрудник);
	
	Для Каждого СведенияОбОтсутствии Из ОтсутствияСдвигающиеРабочийГод Цикл
...
&НаСервере
Процедура УдалитьОтсутствияСдвигающиеРабочийГод(Сотрудник)
	
	СтрокиОтсутствий = Объект.ОтсутствияСдвигающиеРабочийГод.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник));
	Для Каждого СтрокаОтсутствий Из СтрокиОтсутствий Цикл
		Объект.ОтсутствияСдвигающиеРабочийГод.Удалить(СтрокаОтсутствий);
	КонецЦикла;
...
Процедура ОбновитьФормуПоНастройкам()
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		"Видимость",
		ИспользоватьРасчетДенежногоДовольствияСотрудниковОргановИсполнительнойВласти Или Не РасстановкаВоеннослужащих);
...
&НаСервере
Процедура УстановитьКадровыеДанныеСтрокиСотрудника(СтрокаСотрудника, КадровыеДанныеСотрудников)
...
&НаКлиентеНаСервереБезКонтекста
Функция ДатаОстатковОтпусков(Месяц)
