...
Процедура УстановитьСвойстваЭлементовНДФЛ() 
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "УдержатьНалогПриВыплатеАванса", "Видимость", РассчитыватьНДФЛ);
...
Функция РассчитыватьНДФЛ()
	...
	Возврат Не ОтложитьРасчетНалогаВАвансеДоРасчетаЗарплаты И Не Объект.ИсчислятьНалогПриОкончательномРасчете Или Объект.ПланируемаяДатаВыплаты >= НачалоДня(КонецМесяца(Объект.МесяцНачисления));
