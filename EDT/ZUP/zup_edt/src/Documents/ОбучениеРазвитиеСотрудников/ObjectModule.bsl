...
Процедура ЗаполнитьПоДокументуПубликацияМероприятияОбученияРазвития(ДанныеЗаполнения)
	...
	Основание = ДанныеЗаполнения;
	ДанныеОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Основание, "Мероприятие, ДатаОкончания, ДатаНачалаСобытия");
	...
	РеквизитыМероприятия = Новый Массив;
	РеквизитыМероприятия.Добавить("Наименование");
	РеквизитыМероприятия.Добавить("Стоимость");
	РеквизитыМероприятия.Добавить("КоличествоУчебныхЧасов");
	РеквизитыМероприятия.Добавить("СпособОпределенияСтоимости");
	РеквизитыМероприятия.Добавить("Преподаватель");
	РеквизитыМероприятия.Добавить("Контрагент");
	РеквизитыМероприятия.Добавить("МестоПроведения");
	
	ДанныеМероприятия = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Мероприятие, РеквизитыМероприятия);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ДанныеМероприятия, "КоличествоУчебныхЧасов, Преподаватель, Контрагент, МестоПроведения");
	...
	// Заполнение табличной части "Расходы".
	ОбучениеРазвитие.ЗаполнитьТабличнуюЧастьРасходовПоМероприятию(ДанныеМероприятия, Расходы, Сотрудники.Количество());
	СуммаРасходов = Расходы.Итог("Сумма");
