...
Функция ПолучитьФормуПоУмолчанию() Экспорт 
	Возврат "Отчет.РегламентированноеУведомлениеФормаР13001.Форма.Форма2014_1";
КонецФункции
...
Функция ДанныеУведомления(ЭкземплярУведомления) Экспорт
	...
	СтруктураДанных.Вставить("НаименованиеСокращенное", "");
	...
	Если ЭкземплярУведомления.ИмяФормы = "Форма2014_1" Тогда
		...
		Если ДанныеУведомления.ДанныеУведомления.Свойство("ЛистА") Тогда
			
			НаименованиеПолное      = "А01000000";
			НаименованиеСокращенное = "А02000000";
			
			ЛистА = ДанныеУведомления.ДанныеУведомления.ЛистА;
			Если ЛистА.Свойство(НаименованиеПолное) Тогда
				СтруктураДанных.Вставить("НаименованиеПолное", ЛистА[НаименованиеПолное]);
			КонецЕсли;
			Если ЛистА.Свойство(НаименованиеСокращенное) Тогда
				СтруктураДанных.Вставить("НаименованиеСокращенное", ЛистА[НаименованиеСокращенное]);
			КонецЕсли;
		...
		Если ДанныеУведомления.ДанныеУведомления.Свойство("ЛистМ") Тогда
			
			КодСпособаРегистрации = "М04000000";
			
			ЛистМ = ДанныеУведомления.ДанныеУведомления.ЛистМ;
			
			Если ЛистМ.Свойство(КодСпособаРегистрации) Тогда
				СтруктураДанных.Вставить("СпособРегистрации", ЛистМ[КодСпособаРегистрации]);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ДанныеУведомления.ДанныеУведомления.Свойство("ЛистЛ1") Тогда
			
			КодОсновнойКодОКВЭД = "А01010000";
			
			ЛистЛ = ДанныеУведомления.ДанныеУведомления.ЛистЛ1;
			
			Если ЛистЛ.Свойство(КодОсновнойКодОКВЭД) Тогда
				СтруктураДанных.Вставить("ОсновнойКодОКВЭД", ЛистЛ[КодОсновнойКодОКВЭД]);
			КонецЕсли;
			
		КонецЕсли;
		
		Если ДанныеУведомления.ДанныеУведомления.Свойство("ЛистЛ2") Тогда
			
			КодОсновнойКодОКВЭД = "А01010000";
			
			ЛистЛ = ДанныеУведомления.ДанныеУведомления.ЛистЛ2;
...
Функция СформироватьСписокЛистовФорма2014_1(Объект, ЕстьВыходЗаГраницы) Экспорт
	...
	Для Каждого Стр Из СтруктураПараметров.ДеревоСтраниц.Строки Цикл
		...
		Если Стр.ИДНаименования = "ЛистЛ1" Или Стр.ИДНаименования = "ЛистЛ2" Тогда 
			Если Стр.ИДНаименования = "ЛистЛ1" Тогда 
			...
			ПечатнаяФорма = Отчеты.РегламентированноеУведомлениеФормаР13001.ПолучитьМакет("Печать_" + Стр.ИмяМакета);
			НомСтр = НомСтр + 1;
...
Процедура НапечататьСтруктуру(СтруктураДанныхСтраницы, НомСтр, ИмяМакета, ПечатнаяФорма, ЕстьВыходЗаГраницы)
	Если УведомлениеОСпецрежимахНалогообложения.СтраницаЗаполнена(СтруктураДанныхСтраницы) Тогда
		НомСтр = НомСтр + 1;
		МакетПФ = Отчеты.РегламентированноеУведомлениеФормаР13001.ПолучитьМакет(ИмяМакета);
...
Функция ИдентификаторФайлаЭлектронногоПредставления_Форма2014_1(СведенияОтправки)
	...
	Префикс = "RO_R13001_0000_0000_000000000000000_" + ДатаФормированияФайла + "_" + Строка(Новый УникальныйИдентификатор);
	Возврат Префикс;
КонецФункции
...
Функция ОсновныеСведенияЭлектронногоПредставления_Форма2014_1(Объект, УникальныйИдентификатор)
	...
	ОсновныеСведения.Вставить("ИзмУстав", ?(ЗначениеЗаполнено(Данные.ДанныеУведомления.Лист001.П02000000), "1", "0"));
...
Процедура ДополнитьПараметры(Параметры)
	Для Каждого Стр Из Параметры.ДанныеМногостраничныхРазделов.ЛистЕ Цикл 
		...
		Если ЗначениеЗаполнено(ПарамСтр.Е03050501) И ЗначениеЗаполнено(ПарамСтр.Е03050502) Тогда 
			ПарамСтр.Вставить("Е03050500", ПарамСтр.Е03050501 + "-" + ПарамСтр.Е03050502);
		КонецЕсли;
	КонецЦикла;
	
	Для Каждого Стр Из Параметры.ДанныеМногостраничныхРазделов.ЛистЖ2_4 Цикл 
		ПарамСтр = Стр.Значение;
		Если ЗначениеЗаполнено(ПарамСтр.Ж040203040501) И ЗначениеЗаполнено(ПарамСтр.Ж040203040502) Тогда 
			ПарамСтр.Вставить("Ж0402030405", ПарамСтр.Ж040203040501 + "-" + ПарамСтр.Ж040203040502);
		КонецЕсли;
	КонецЦикла;
	
	ПарамСтр =  Параметры.ДанныеУведомления.ЛистМ;
	Если ЗначениеЗаполнено(ПарамСтр.М03040501) И ЗначениеЗаполнено(ПарамСтр.М03040502) Тогда 
		ПарамСтр.Вставить("М03040500", ПарамСтр.М03040501 + "-" + ПарамСтр.М03040502);
	КонецЕсли;
КонецПроцедуры
...
Функция СформироватьДеревоСтраниц_Форма2014_1()
	...
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Форма2014_1_СтраницаГ";
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	...
	Стр001.МакетыПФ = "Печать_Форма2014_1_СтраницаД1;Печать_Форма2014_1_СтраницаД2";
	...
	Стр001.Наименование = "Стр. 1";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Форма2014_1_СтраницаЖ1";
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЛистЖ1";
	Стр001.МакетыПФ = "Печать_Форма2014_1_СтраницаЖ1";
	
	Стр001 = Стр001.Строки.Добавить();
	Стр001.Наименование = "Листы Ж 2-4";
	Стр001.ИндексКартинки = 1;
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Ложь;
	...
	Стр001.Наименование = "Стр. 1";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Форма2014_1_СтраницаЖ2_4";
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЛистЖ2_4";
	Стр001.МакетыПФ = "Печать_Форма2014_1_СтраницаЖ2;Печать_Форма2014_1_СтраницаЖ3;Печать_Форма2014_1_СтраницаЖ4";
	...
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Лист Л-1";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Форма2014_1_СтраницаЛ1";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Истина;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЛистЛ1";
	Стр001.МногострочныеЧасти.Добавить("МногострочнаяЧасть1");
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Лист Л-2";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Форма2014_1_СтраницаЛ2";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Истина;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "ЛистЛ2";
	...
	Стр001.МакетыПФ = "Печать_Форма2014_1_СтраницаМ1;Печать_Форма2014_1_СтраницаМ2;Печать_Форма2014_1_СтраницаМ3";
...
Функция СоздатьЗаполненноеУведомление(Параметры, СуществующийДокументСсылка = Неопределено) Экспорт
	...
	ИмяОтчета = "РегламентированноеУведомлениеФормаР13001";
	...
	Иначе
		...
		НовыйДок.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ФормаР13001;
	КонецЕсли;
...
Процедура СформироватьДеревоСтраницПоДанным(ДеревоСтраниц, ДанныеМногостраничныхРазделов) Экспорт 
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов["ЛистД"] Цикл 
		...
		Стр001Нов.МакетыПФ = "Печать_Форма2014_1_СтраницаД1;Печать_Форма2014_1_СтраницаД2";
	КонецЦикла;
	...
	НомерСтр = 0;
	Для Каждого Стр Из ДанныеМногостраничныхРазделов["ЛистЖ1"] Цикл 
		НомерСтр = НомерСтр + 1;
		Стр001Нов = Стр001.Строки.Добавить();
		Стр001Нов.Наименование = "Стр. " + Формат(НомерСтр, "ЧГ=");
		Стр001Нов.ИндексКартинки = 1;
		Стр001Нов.ИмяМакета = "Форма2014_1_СтраницаЖ1";
		Стр001Нов.Многостраничность = Истина;
		Стр001Нов.Многострочность = Ложь;
		Стр001Нов.УИД = Стр.Значение.УИД;
		Стр001Нов.ИДНаименования = "ЛистЖ1";
		Стр001Нов.МакетыПФ = "Печать_Форма2014_1_СтраницаЖ1";
		
		НомерСтр2 = 0;
		Для Каждого СтрПодч Из ДанныеМногостраничныхРазделов["ЛистЖ2_4"] Цикл 
			Если СтрПодч.Значение.УИДРодителя <> Стр.Значение.УИД Тогда 
				Продолжить;
			КонецЕсли;
			
			НомерСтр2 = НомерСтр2 + 1;
			Стр002Нов = Стр001Нов.Строки.Добавить();
			Стр002Нов.Наименование = "Стр. " + Формат(НомерСтр2, "ЧГ=");
			Стр002Нов.ИндексКартинки = 1;
			Стр002Нов.ИмяМакета = "Форма2014_1_СтраницаЖ2_4";
			Стр002Нов.Многостраничность = Истина;
			Стр002Нов.Многострочность = Ложь;
			Стр002Нов.УИД = СтрПодч.Значение.УИД;
			Стр002Нов.ИДНаименования = "ЛистЖ2_4";
			Стр002Нов.МакетыПФ = "Печать_Форма2014_1_СтраницаЖ2;Печать_Форма2014_1_СтраницаЖ3;Печать_Форма2014_1_СтраницаЖ4";
		КонецЦикла;
	КонецЦикла;
	...
	НомерСтр = 0;
	Для Каждого Стр Из ДанныеМногостраничныхРазделов["ЛистИ"] Цикл 
		НомерСтр = НомерСтр + 1;
		Стр001Нов = Стр001.Строки.Добавить();
		Стр001Нов.Наименование = "Стр. " + Формат(НомерСтр, "ЧГ=");
		Стр001Нов.ИндексКартинки = 1;
		Стр001Нов.ИмяМакета = "Форма2014_1_СтраницаИ";
		Стр001Нов.Многостраничность = Истина;
		Стр001Нов.Многострочность = Ложь;
		Стр001Нов.УИД = Стр.Значение.УИД;
		Стр001Нов.ИДНаименования = "ЛистИ";
	КонецЦикла;
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов["ЛистК"] Цикл 
		НомерСтр = НомерСтр + 1;
		Стр001Нов = Стр001.Строки.Добавить();	Стр001.ИндексКартинки = 1;
