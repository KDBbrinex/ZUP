...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//
	// Версия формы
	СтруктураРеквизитовФормы.мВерсияФормы = "29/07/2019";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2020Кв1_месяц", "АтрибВыгрузкиXML2020Кв1_квартал");
	РегламентированнаяОтчетность.ОбновитьШаблоныЧерезВебСервисНаСервере(ЭтотОбъект, ИмяАтрибутов);
...
#КонецЕсли
	...
	Если (НЕ ЗначениеЗаполнено(Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение)) ИЛИ (Форма.СтруктураРеквизитовФормы.мВПрограммеИзмененаОрганизация) Тогда
		...
		Иначе
			...
			Форма.ПолеТабличногоДокументаФормаОтчета.Области.ОргНазв.Значение = НаимЮЛПол + ?(ПустаяСтрока(Форма.СтруктураРеквизитовФормы.НаимОрганизацииСокр)
				, "", " (" + Форма.СтруктураРеквизитовФормы.НаимОрганизацииСокр + ")");
		КонецЕсли;
	КонецЕсли;
...
Процедура ПолеТабличногоДокументаФормаОтчетаВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	РегламентированнаяОтчетностьКлиент.ВыборОтчетаПредидущегоПериода(ЭтаФорма, Область, СтандартнаяОбработка);
	...
	ИначеЕсли Не СтруктураРеквизитовФормы.мАвтоВыборКодов Тогда 
		...
		Если ПравОбластьИмя = "А1" Или ПравОбластьИмя = "В1" Тогда 
			ПараметрВводаПоля = РегламентированнаяОтчетностьКлиентСервер.НайтиСвойстваПоказателя(ЭтаФорма, "OKVED2");
...
Процедура ОчиститьТабличноеПоле(Форма)
	...
	Форма.ПолеТабличногоДокументаФормаОтчета.Области["ВыбратьОтчетПрошлогоПериода"].Значение = "Выбор отчета предыдущего периода";
...
Процедура Расчет(Форма, ИмяСтраницыПанели = "") Экспорт
	...
	Области.П010000102.Значение = Области.П010000202.Значение + Области.П010000302.Значение + Области.П010000402.Значение + Области.П010000502.Значение + Области.П010000602.Значение
	                                         + Области.П010000702.Значение + Области.П010000802.Значение + Области.П010000902.Значение + Области.П010001002.Значение + Области.П010001102.Значение;
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтаФорма, "СхемаВыгрузкиXML2020Кв1");
...
Функция УниверсальнаяВыгрузкаXMLПроверкаДанныхНаСервере(ТекстВыгрузки, ПараметрыВыгрузки) Экспорт
	...
	ИмяАтрибутов = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "АтрибВыгрузкиXML2020Кв1_месяц", "АтрибВыгрузкиXML2020Кв1_квартал");
	ИмяШаблона = ?(СтруктураРеквизитовФормы.мПериодичность = Перечисления.Периодичность.Месяц, "ШаблонРосстата2020Кв1_месяц", "ШаблонРосстата2020Кв1_квартал");
	РегламентированнаяОтчетность.ИзменитьВерсиюФорматаСПроверкойСовместимостиШаблонов(ЭтотОбъект, ИмяАтрибутов, ИмяШаблона, ПараметрыВыгрузки);
...
Процедура ЗагрузитьПодготовленныеДанные()
	...
	ТекТабличноеПоле.Вывести(
		ОбъектОтчета(ЭтаФорма.ИмяФормы).ПолучитьМакет("ФормаОтчета2020Кв1_ФормаОтчета"));
...
Функция ПроверитьКонтрольныеСоотношенияИзШаблонаВФоне()
	...
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("ШаблонРосстата", ИмяШаблона);
	ПараметрыПроверкиКонтрольныхСоотношений.Вставить("Атрибуты", ИмяАтрибутов);
...
Функция СформироватьПараметрыВыбораОтчетаПредыдущегоПериода() Экспорт 
	...
	Парам.Вставить("ВидОтправляемогоДокумента", ПредопределенноеЗначение("Справочник.ВидыОтправляемыхДокументов.СтатистикаФормаП4"));
...
Процедура СформироватьСпискиВыбора()
	...
	РегламентированнаяОтчетность.ДобавитьСтрокуОписанияВвода(СвойстваПоказателей, "OKVED2",   512, , "Выбор кода ОКВЭД",   ТаблСписка);
КонецПроцедуры
