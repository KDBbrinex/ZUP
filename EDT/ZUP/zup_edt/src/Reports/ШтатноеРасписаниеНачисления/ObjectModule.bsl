...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Если КлючВарианта = "Т3" Тогда
		...
		Попытка
			...
			ЗначениеПараметраВРРайонныйКоэффициент.Значение = ПланыВидовРасчета.Начисления.НачислениеРайонныйКоэффициент();
...
Процедура ИнициализироватьОтчет() Экспорт
	...
	Если Не ОтчетИнициализирован Тогда
		...
		СоответствиеДополнительныхПолейПредставлениям = УправлениеШтатнымРасписанием.ДополнительныеПоляОтчетаШтатноеРасписание();
...
Процедура ВывестиТабличнуюЧастьТ3(ДокументРезультат, Данные, СписокНадбавок, Макет, ПредставлениеТарифовИНадбавок)
	...
	Для Каждого СтрокаДанных Из Данные Цикл
		...
		// Проверка вывода
		Если СетчикВыведенных + 1 = Данные.Количество() Тогда
			
			МассивОбластей = Новый Массив;
			МассивОбластей.Добавить(СтрокаДоНадбавок);
			МассивОбластей.Добавить(Макет.ПолучитьОбласть("ПодвалЛиста"));
			МассивОбластей.Добавить(Макет.ПолучитьОбласть("Подвал"));
			МассивОбластей.Добавить(Макет.ПолучитьОбласть("Подписи"));
		...
		Иначе
			...
			МассивОбластейСтроки.Добавить(СтрокаДоНадбавок);
			МассивОбластейСтроки.Добавить(Макет.ПолучитьОбласть("ПодвалЛиста"));
			
			Если НЕ ДокументРезультат.ПроверитьВывод(МассивОбластейСтроки) Тогда
				ВывестиШапкуТаблицы = Истина;
			КонецЕсли;
		...
		Если ВывестиШапкуТаблицы Тогда
			...
			ВывестиШапкуСтраницыТ3(ДокументРезультат, СписокНадбавок, Макет);
		...
		СетчикВыведенных = СетчикВыведенных + 1;
...
Процедура ВывестиШапкуСтраницыТ3(ДокументРезультат, СписокНадбавок, Макет)
	
	ЗаголовокТаблицыДоНадбавок = Макет.ПолучитьОбласть("ШапкаСтраницы|ДоНадбавок");
	ЗаголовокТаблицыНадбавки = Макет.ПолучитьОбласть("ШапкаСтраницы|Надбавка");
	ЗаголовокТаблицыПослеНадбавок = Макет.ПолучитьОбласть("ШапкаСтраницы|ПослеНадбавок");
	ДокументРезультат.Вывести(ЗаголовокТаблицыДоНадбавок, 2);
	НомерКолонки = 6;
	Для Каждого ДанныеНадбавки Из СписокНадбавок Цикл
		ЗаголовокТаблицыНадбавки.Параметры.НомерКолонки = НомерКолонки;
		...
		Если ЗначениеЗаполнено(СтрокаДанныхНадбавки.КоличествоСтавок) И СтрокаДанныхНадбавки.КоличествоСтавок <> 0 Тогда
			...
			Если ИспользоватьВилкуСтавокВШтатномРасписании Тогда
				...
				ПредставлениеРазмераВРублях = ПредставлениеЗначенийТ3(
					СтрокаДанныхНадбавки.РазмерМин,
					СтрокаДанныхНадбавки.РазмерМакс);
				
			Иначе
				
				ПредставлениеРазмераВРублях = ПредставлениеЗначенийТ3(
					СтрокаДанныхНадбавки.Размер,
					СтрокаДанныхНадбавки.Размер);
