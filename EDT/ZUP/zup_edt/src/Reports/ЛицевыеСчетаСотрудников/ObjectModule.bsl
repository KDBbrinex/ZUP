...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровки, СтандартнаяОбработка)
	...
	Попытка 
		...
		МакетКомпоновкиДанных = ЗарплатаКадрыОтчеты.МакетКомпоновкиДанныхДляКоллекцииЗначений(СхемаКомпоновкиДанных, НастройкиОтчета);
		...
		Для каждого СтрокаПоГодам Из ДанныеОтчета.Строки Цикл
			...
			Если МассивСотрудников.Количество() > 0 Тогда
				...
				ДополнительныеДанные.Вставить("ФактическиеОтпуска", ФактическиеОтпуска(КлючВарианта, ПараметрыОтчетов));
				...
				Для каждого СтрокаОрганизации Из СтрокаПоГодам.Строки Цикл
					Для каждого СтрокаПодразделения Из СтрокаОрганизации.Строки Цикл
						...
						Иначе
							...
							УкомплектоватьРасчетнымиЛистами = НастройкиОтчета.ПараметрыДанных.НайтиЗначениеПараметра(Новый ПараметрКомпоновкиДанных("УкомплектоватьРасчетнымиЛистами")).Значение;
							ВывестиСотрудниковПодразделенияТ54а(СтрокаПодразделения.Строки, ДокументРезультат, Макет, ДополнительныеДанные, КонецПериода, УкомплектоватьРасчетнымиЛистами);
...
Процедура ВывестиСтрокиОбщихДанныхТ54(СтрокаСотрудника, ДокументРезультат, Макет, ДополнительныеДанные)
	...
	КоллекцияФактическихОтпусков = ФактическиеОтпуска.ДанныеОтчета.Получить(СтрокаСотрудника.Сотрудник);
	Если КоллекцияФактическихОтпусков = Неопределено Тогда
	...
	Если МаксимальноеКоличествоСтрок > 0 Тогда
		...
		Для НомерСтроки = 0 По МаксимальноеКоличествоСтрок - 1 Цикл
			...
			Если НастройкиПечатныхФорм.ВыводитьПолнуюИерархиюПодразделений И ЗначениеЗаполнено(МакетСтрокаОбщая.Параметры.Подразделение) Тогда
				Если ТипЗнч(МакетСтрокаОбщая.Параметры.Подразделение) = Тип("СправочникСсылка.ПодключаемоеОборудование") Тогда
	...
	Для каждого СтрокаПериода Из СтрокаСотрудника.Строки Цикл
		...
		Если ДанныеНачислений <> Неопределено Тогда
			...
			Если СтрокиНачислений.Количество() > 0 Тогда
				...
				ЗаполнитьПараметрыПользовательскихПолейПоКоллекцииЗначений(МакетСтрокаПоПериодам, СтрокиНачислений, ДанныеНачислений.СоответствиеПользовательскихПолей);
				
				ТаблицаБольничныхСотрудника = ТаблицаБольничных.Скопировать(Новый Структура("Период,Сотрудник", СтрокаПериода.МесяцНачисления, СтрокаПериода.Сотрудник));
				...
				Для каждого СтрокаТаблицы Из ДанныеНачислений.ПорядокДопНачислений Цикл
					...
					Для Каждого СтрокаНачислений Из СтрокиНачислений Цикл
						...
						ЗначениеПоСтроке = СтрокаНачислений[ДанныеНачислений.СоответствиеПользовательскихПолей.Получить(СтрокаТаблицы.Имя)];
						Если ЗначениеЗаполнено(ЗначениеПоСтроке) Тогда
							СуммаЯчейки = СуммаЯчейки + ЗначениеПоСтроке;
						КонецЕсли;
...
Процедура ВывестиСотрудниковПодразделенияТ54а(СтрокиСотрудников, ДокументРезультат, Макет, ДополнительныеДанные, ДатаОтчета, УкомплектоватьРасчетнымиЛистами)
	...
	Для каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
		...
		Если УкомплектоватьРасчетнымиЛистами Тогда
			
			ДокументРезультат.ВывестиГоризонтальныйРазделительСтраниц();
			
			ПараметрыСтраницы = ЗарплатаКадрыОтчеты.ПараметрыСтраницыТабличногоДокумента(ДокументРезультат);
			
			ОтчетРасчетныйЛисток = ЗарплатаКадрыОтчеты.ОтчетРасчетныйЛисток();
			РасчетныйЛисток = Отчеты.АнализНачисленийИУдержаний.ДанныеРасчетныхЛистков(
				ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(СтрокаСотрудника.СотрудникФизическоеЛицо), СтрокаСотрудника.Организация,
				НачалоГода(СтрокаСотрудника.ГодНачисления), КонецГода(СтрокаСотрудника.ГодНачисления), ОтчетРасчетныйЛисток, ПараметрыСтраницы).ДокументРезультат;
			
			ОбластьРасчетныйЛисток = РасчетныйЛисток.Область(1,1, РасчетныйЛисток.ВысотаТаблицы, РасчетныйЛисток.ШиринаТаблицы);
			ПерваяСтрокаВыводаРасчетныхЛистков = ДокументРезультат.ВысотаТаблицы + 1;
			ДокументРезультат.Область(ПерваяСтрокаВыводаРасчетныхЛистков, , ПерваяСтрокаВыводаРасчетныхЛистков + РасчетныйЛисток.ВысотаТаблицы - 1, ).СоздатьФорматСтрок();
			Для НомерКолонки = 1 По РасчетныйЛисток.ШиринаТаблицы Цикл
				ДокументРезультат.Область(ПерваяСтрокаВыводаРасчетныхЛистков, НомерКолонки).ШиринаКолонки = РасчетныйЛисток.Область(1, НомерКолонки).ШиринаКолонки;
			КонецЦикла;
			
			ДокументРезультат.ВставитьОбласть(ОбластьРасчетныйЛисток, ДокументРезультат.Область(ПерваяСтрокаВыводаРасчетныхЛистков, 1));
			
			Для НомерСтроки = 1 По РасчетныйЛисток.ВысотаТаблицы Цикл
				
				ОбластьЛистка = РасчетныйЛисток.Область(НомерСтроки, ,НомерСтроки, );
				Если ОбластьЛистка.КонецСтраницы Тогда
					ДокументРезультат.Область(ПерваяСтрокаВыводаРасчетныхЛистков + НомерСтроки - 1, , ПерваяСтрокаВыводаРасчетныхЛистков + НомерСтроки - 1, ).КонецСтраницы = Истина;
				КонецЕсли;
...
Процедура ВывестиСтрокиКадровыеТ54а(СтрокаСотрудника, ДокументРезультат, Макет, ДополнительныеДанные)
	...
	ОписаниеГрафиковРаботы = ДанныеКадровойИстории.ОписаниеГрафиковРаботы;
		...
		Для НомерСтроки = 0 По МаксимальноеКоличествоСтрок - 1 Цикл
			...
			Если НомерСтроки <= КоллекцияКадровойИстории.Количество() - 1 Тогда
				...
				Если ЗначениеЗаполнено(СтрокаДанных.ГрафикРаботы) Тогда
					МакетСтроки.Параметры.УсловияТруда = ОписаниеГрафиковРаботы.Получить(СтрокаДанных.ГрафикРаботы);
				Иначе
			...
			Если НастройкиПечатныхФорм.ВыводитьПолнуюИерархиюПодразделений И ЗначениеЗаполнено(МакетСтроки.Параметры.Подразделение) Тогда
				Если ТипЗнч(МакетСтроки.Параметры.Подразделение) = Тип("СправочникСсылка.ПодразделенияОрганизаций") Тогда
...
Функция ДанныеНачислений(КлючВарианта, ПараметрыОтчетов)
	...
	ОтчетАнализНачисленийИУдержаний.ИнициализироватьНаборыДанных();
	...
	МакетКомпоновки = ЗарплатаКадрыОтчеты.МакетКомпоновкиДанныхДляКоллекцииЗначений(
		ОтчетАнализНачисленийИУдержаний.СхемаКомпоновкиДанных, ОтчетАнализНачисленийИУдержаний.КомпоновщикНастроек.Настройки);
...
Функция КадроваяИстория(КлючВарианта, ПараметрыОтчетов)
	...
	Если ЗначениеЗаполнено(ПараметрПериод) Тогда
		ПериодДатаНачала = НачалоГода(ПараметрПериод.ДатаНачала);
		ПериодДатаОкончания = ПараметрПериод.ДатаОкончания;
	Иначе
		ПериодДатаНачала = '00010101';
		ПериодДатаОкончания = '00010101';
	КонецЕсли;
	
	ПараметрыПостроения = КадровыйУчетРасширенный.ПараметрыПостроенияВТРеестрКадровыхПриказовПоСпискуСотрудников();
	ПараметрыПостроения.ДатаНачала = ПериодДатаНачала;
	ПараметрыПостроения.ДатаОкончания = ПериодДатаОкончания;
	ПараметрыПостроения.Сотрудники = ПараметрыОтчетов.Отбор.Сотрудник.ПравоеЗначение;
	ПараметрыПостроения.ВключатьСобытияУвольнений = Истина;
	
	Запрос = КадровыйУчетРасширенный.ЗапросВТРеестрКадровыхПриказов(Истина, ПараметрыПостроения, "ВТДанныеРеестра");
	...
	Запрос.Текст =
		...
		|	ДанныеРеестра.Сотрудник КАК Сотрудник
		|ПОМЕСТИТЬ ВТОтборСотрудниковДляТарифныхСтавокИГрафиковРаботы
		|ИЗ
		|	ВТДанныеРеестра КАК ДанныеРеестра";
	
	Запрос.Выполнить();
	
	Описатель = КадровыйУчет.ОписательВременныхТаблицДляСоздатьВТКадровыеДанныеСотрудников(Запрос.МенеджерВременныхТаблиц, "ВТОтборСотрудниковДляТарифныхСтавокИГрафиковРаботы");
	Описатель.ИмяВТКадровыеДанныеСотрудников = "ВТТарифныеСтавкиИГрафикиРаботы";
	
	КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(Описатель, Истина, "ТарифнаяСтавка,Надбавка,ГрафикРаботы", , Ложь);
	
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДанныеРеестра.Сотрудник КАК Сотрудник,
		|	ДанныеРеестра.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ДанныеРеестра.Организация КАК Организация,
		|	ДанныеРеестра.Регистратор КАК Регистратор,
		|	ДанныеРеестра.ДокументОснование КАК ДокументОснование,
		|	ДанныеРеестра.Период КАК Период,
		|	ДанныеРеестра.Подразделение КАК Подразделение,
		|	ДанныеРеестра.Должность КАК Должность,
		|	ДанныеРеестра.ДолжностьПоШтатномуРасписанию КАК ДолжностьПоШтатномуРасписанию,
		|	ДанныеРеестра.Разряд КАК Разряд,
		|	ДанныеРеестра.ВидСобытия КАК ВидСобытия,
		|	ДанныеРеестра.ВидДоговора КАК ВидДоговора,
		|	ЕСТЬNULL(ДанныеРеестра.ТарифнаяСтавкаНадбавка, ТарифныеСтавкиИГрафикиРаботы.ТарифнаяСтавка) КАК ТарифнаяСтавкаНадбавка,
		|	ДанныеРеестра.НомерПриказа КАК НомерПриказа,
		|	ДанныеРеестра.ДатаПриказа КАК ДатаПриказа,
		|	ДанныеРеестра.Основание КАК Основание,
		|	ДанныеРеестра.ДанныеДоНачалаУчета КАК ДанныеДоНачалаУчета,
		|	ТарифныеСтавкиИГрафикиРаботы.Надбавка КАК Надбавка,
		|	ТарифныеСтавкиИГрафикиРаботы.ГрафикРаботы КАК ГрафикРаботы
		|ИЗ
		|	ВТДанныеРеестра КАК ДанныеРеестра
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТТарифныеСтавкиИГрафикиРаботы КАК ТарифныеСтавкиИГрафикиРаботы
		|		ПО ДанныеРеестра.Период = ТарифныеСтавкиИГрафикиРаботы.Период
		|			И ДанныеРеестра.Сотрудник = ТарифныеСтавкиИГрафикиРаботы.Сотрудник
		|
		|УПОРЯДОЧИТЬ ПО
		|	Сотрудник,
		|	Период";
	...
	СтрокиБезРеквизитов = ДанныеРеестра.НайтиСтроки(Новый Структура("НомерПриказа", Неопределено));
	Если СтрокиБезРеквизитов.Количество() > 0 Тогда
		
		РеквизитыПриказов = КадровыйУчетРасширенный.РеквизитыКадровыхПриказов(ОбщегоНазначения.ВыгрузитьКолонку(СтрокиБезРеквизитов, "Регистратор", Истина));
		Для Каждого СтрокаБезРеквизитов Из СтрокиБезРеквизитов Цикл
			
			РеквизитыРегистратора = РеквизитыПриказов.Получить(СтрокаБезРеквизитов.Регистратор);
			Если РеквизитыРегистратора <> Неопределено Тогда
				
				СтрокаБезРеквизитов.НомерПриказа = РеквизитыРегистратора.НомерПриказа;
				СтрокаБезРеквизитов.ДатаПриказа = РеквизитыРегистратора.Дата;
	...
	ГрафикиРаботыСотрудников = Новый Соответствие;
	Для Каждого Сотрудник Из ПараметрыОтчетов.Отбор.Сотрудник.ПравоеЗначение Цикл
		
		КадроваяИсторияСотрудника = ПустаяТаблицаКадровойИстории();
		
		СтрокиРеестра = ДанныеРеестра.НайтиСтроки(Новый Структура("Сотрудник", Сотрудник));
		Для Каждого СтрокаРеестра Из СтрокиРеестра Цикл
			
			СтрокаТаблицыКадровойИстории = КадроваяИсторияСотрудника.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаТаблицыКадровойИстории, СтрокаРеестра);
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыКадровойИстории.ГрафикРаботы) Тогда
				ГрафикиРаботыСотрудников.Вставить(СтрокаТаблицыКадровойИстории.ГрафикРаботы);
			КонецЕсли;
			
		КонецЦикла;
		
		ДанныеСотрудников.Вставить(Сотрудник, КадроваяИсторияСотрудника);
		
	КонецЦикла;
	
	СтруктураВозврата.Вставить("ДанныеОтчета", ДанныеСотрудников);
	
	МассивГрафиков = ОбщегоНазначения.ВыгрузитьКолонку(ГрафикиРаботыСотрудников, "Ключ");
	Если МассивГрафиков.Количество() = 0 Тогда
		СтруктураВозврата.Вставить("ОписаниеГрафиковРаботы", Новый Соответствие);
	Иначе
		
		СтруктураВозврата.Вставить("ОписаниеГрафиковРаботы",
			Справочники.ГрафикиРаботыСотрудников.ТекстовоеОписаниеГрафиковРаботы(МассивГрафиков));
...
Функция ФактическиеОтпуска(КлючВарианта, ПараметрыОтчетов)
	...
	СтруктураВозврата = Новый Структура;
	СтруктураВозврата.Вставить("ДанныеОтчета", Новый Соответствие);
	...
	Для Каждого Сотрудник Из ПараметрыОтчетов.Отбор.Сотрудник.ПравоеЗначение Цикл
		...
		НоваяСтрокаТаблицаСотрудников.Период = ПараметрыОтчетов.Параметры.Период.ДатаОкончания;
		НоваяСтрокаТаблицаСотрудников.Сотрудник = Сотрудник;
	...
	КадровыйУчетРасширенный.СоздатьВТРеестрОтпусков(Запрос.МенеджерВременныхТаблиц, ТаблицаСотрудников, КлючВарианта);
	
	Запрос.УстановитьПараметр("ДатаНачала", ПараметрыОтчетов.Параметры.Период.ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", ПараметрыОтчетов.Параметры.Период.ДатаОкончания);
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(ОтпускаСотрудников.Дата, РеестрОтпусков.ДокументОснование.Дата) КАК ДатаПриказа,
		|	ЕСТЬNULL(ОтпускаСотрудников.Номер, РеестрОтпусков.ДокументОснование.Номер) КАК НомерПриказа
		|ИЗ
		|	ВТРеестрОтпусков КАК РеестрОтпусков
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ОтпускаСотрудников КАК ОтпускаСотрудников
		|		ПО РеестрОтпусков.ДокументОснование.ДокументЗаполнения = ОтпускаСотрудников.Ссылка
		|			И (НЕ ОтпускаСотрудников.ПометкаУдаления)
		|ГДЕ
		|	(РеестрОтпусков.ДатаС МЕЖДУ &ДатаНачала И &ДатаОкончания
		|			ИЛИ РеестрОтпусков.ДатаПо МЕЖДУ &ДатаНачала И &ДатаОкончания)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Сотрудник,
		|	ДатаПриказа,
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаРеестра = Новый Структура;
			
			СтрокаРеестра.Вставить("ВидЕжегодногоОтпуска", Выборка.ПредставлениеВидаОтпуска);
			СтрокаРеестра.Вставить("ПриказНаОтпускДата", Выборка.ДатаПриказа);
			СтрокаРеестра.Вставить("ПриказНаОтпускНомер", Выборка.НомерПриказа);
			СтрокаРеестра.Вставить("РабочийПериодС", Выборка.РабочийГодС);
			СтрокаРеестра.Вставить("РабочийПериодПо", Выборка.РабочийГодПо);
			СтрокаРеестра.Вставить("КоличествоДней", Выборка.ДнейОтпуска);
			СтрокаРеестра.Вставить("ДатаНачала", Выборка.ДатаС);
			СтрокаРеестра.Вставить("ДатаОкончания", Выборка.ДатаПо);
			
			Если НастройкиПечатныхФорм.УдалятьПрефиксыОрганизацииИИБИзНомеровКадровыхПриказов Тогда
				СтрокаРеестра.ПриказНаОтпускНомер = ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СтрокаРеестра.ПриказНаОтпускНомер, Истина, Истина);
			КонецЕсли;
			
			ДанныеСотрудника.Добавить(СтрокаРеестра);
			
		КонецЦикла;
		
		СтруктураВозврата.ДанныеОтчета.Вставить(Выборка.Сотрудник, ДанныеСотрудника);
...
Функция ПлановыеУдержания(КлючВарианта, ПараметрыОтчетов)
	...
	МакетКомпоновки = ЗарплатаКадрыОтчеты.МакетКомпоновкиДанныхДляКоллекцииЗначений(ОтчетыПлановыеУдержанияСотрудников.СхемаКомпоновкиДанных,
		ОтчетыПлановыеУдержанияСотрудников.КомпоновщикНастроек.Настройки);
...
Функция ДанныеОВычетах(ФизическиеЛицаОрганизаций, ПараметрыОтчетов)
	...
	ТекстОбъединения = 
	"ВЫБРАТЬ
	|	&Организация_Индекс КАК ГоловнаяОрганизация,
	|	ФизическиеЛица.Ссылка КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТФизическиеЛица
	|ИЗ
	|	Справочник.ФизическиеЛица КАК ФизическиеЛица
	|ГДЕ
	|	ФизическиеЛица.Ссылка В(&ФизическиеЛица_Индекс)";
	ТекстЗапроса = "";
	ИндексЗапроса = 1;
	Для Каждого ФизическиеЛицаОрганизации Из ФизическиеЛицаОрганизаций Цикл
		Если ИндексЗапроса > 1 Тогда 
			...
			|
			|";
			Если ИндексЗапроса = 2 Тогда 
				ТекстОбъединения = СтрЗаменить(ТекстОбъединения, "ПОМЕСТИТЬ ВТФизическиеЛица", "");
			КонецЕсли;
		КонецЕсли;
		ИндексЗапросаСтрокой = Формат(ИндексЗапроса, "ЧГ=");
		ТекстЗапроса = ТекстЗапроса + СтрЗаменить(ТекстОбъединения, "_Индекс", ИндексЗапросаСтрокой);
		
		Запрос.УстановитьПараметр("ФизическиеЛица" + ИндексЗапросаСтрокой, ФизическиеЛицаОрганизации.Значение);
		Запрос.УстановитьПараметр("Организация" + ИндексЗапросаСтрокой, ЗарплатаКадры.ГоловнаяОрганизация(ФизическиеЛицаОрганизации.Ключ));
	...
	ПредоставленныеВычеты = УчетНДФЛ.ПредоставленныеВычетыПоНДФЛДляАналитическогоОтчета(
		Истина, 
		Дата(НалоговыйПериод,1,1),
		КонецГода(Дата(НалоговыйПериод,1,1)),
		ТекущаяДатаСеанса(),
		Ложь,
		Запрос.МенеджерВременныхТаблиц,
		"ВТФизическиеЛица");
	ПредоставленныеВычеты.Свернуть("ГоловнаяОрганизация, ФизическоеЛицо, КодВычета", "Сумма");
	Отбор = Новый Структура("ГоловнаяОрганизация");
	Для Каждого ФизическиеЛицаОрганизации Из ФизическиеЛицаОрганизаций Цикл
		
		Отбор.ГоловнаяОрганизация = ЗарплатаКадры.ГоловнаяОрганизация(ФизическиеЛицаОрганизации.Ключ);
		Вычеты = ПредоставленныеВычеты.НайтиСтроки(Отбор);
		
		КоличествоВычетов = Вычеты.Количество();
		Если КоличествоВычетов > 0 Тогда
			
			ИтогиПоВычетам = Новый Соответствие;
			
			Для каждого Вычет Из Вычеты Цикл
				
				ОписаниеВычета = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru='%1 руб. (код %2)'"),
					Формат(Вычет.Сумма, "ЧДЦ=2"),
				...
				ВычетыОрганизаций = ЗначениеВозврата.Получить(Вычет.ФизическоеЛицо);
				Если ВычетыОрганизаций = Неопределено Тогда
					ВычетыОрганизаций = Новый Соответствие;
					ЗначениеВозврата.Вставить(Вычет.ФизическоеЛицо, ВычетыОрганизаций);
				КонецЕсли;
				...
				ИтогиПоОрганизациям = ИтогиПоВычетам.Получить(Вычет.ФизическоеЛицо);
				Если ИтогиПоОрганизациям = Неопределено Тогда
					ИтогиПоОрганизациям = Новый Соответствие;
					ИтогиПоВычетам.Вставить(Вычет.ФизическоеЛицо, ИтогиПоОрганизациям);
				КонецЕсли;
				...
				ИтогПоВычетам = ИтогПоВычетам + Вычет.Сумма;
...
Процедура ЗаполнитьПараметрыПользовательскихПолейПоКоллекцииЗначений(Макет, КоллекцияЗначений, СоответствиеПользовательскихПолей, ИменаЗаполняемыхПолей = "")
	
	Если ТипЗнч(КоллекцияЗначений) <> Тип("Массив") Тогда
		
		ЗарплатаКадрыОтчеты.ЗаполнитьПараметрыПользовательскихПолей(Макет, КоллекцияЗначений, СоответствиеПользовательскихПолей, ИменаЗаполняемыхПолей);
		Возврат;
	...
	Для каждого СоответствиеПользовательскогоПоля Из СоответствиеПользовательскихПолей Цикл
		...
		КлючиСтруктуры.Добавить(СоответствиеПользовательскогоПоля.Значение);
		
	КонецЦикла;
	
	КлючиСтруктурыСтрокой = СтрСоединить(КлючиСтруктуры, ",");
	СтруктураДанных = Новый Структура(КлючиСтруктурыСтрокой);
	Для Каждого Данные Из КоллекцияЗначений Цикл
		
		СтруктураСтроки = Новый Структура(КлючиСтруктурыСтрокой);
		ЗаполнитьЗначенияСвойств(СтруктураСтроки, Данные);
		
		Для Каждого ЭлементСтруктуры Из СтруктураСтроки Цикл
			
			Если ЗначениеЗаполнено(ЭлементСтруктуры.Значение) Тогда
				
				Если ТипЗнч(СтруктураДанных[ЭлементСтруктуры.Ключ]) = Тип("Число") Тогда
					СтруктураДанных[ЭлементСтруктуры.Ключ] = СтруктураДанных[ЭлементСтруктуры.Ключ] + ЭлементСтруктуры.Значение;
				Иначе
					СтруктураДанных[ЭлементСтруктуры.Ключ] = ЭлементСтруктуры.Значение;
				КонецЕсли;
