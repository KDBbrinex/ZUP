...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр.ПутьФормы = "Отчет.РегламентированноеУведомлениеОНаличииСчета.Форма.Форма2017_1";
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2017_1(Данные, УникальныйИдентификатор)
	...
	Если Не ЗначениеЗаполнено(Лист001.ДатаРазр) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указана дата выдачи разрешения", "Лист001", "ДатаРазр"));
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Лист001.НомРазр) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указан номер разрешения", "Лист001", "НомРазр"));
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Лист001.НаимОргРазр) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указан орган, выдавший разрешение", "Лист001", "НаимОргРазр"));
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Лист001.ДатаКонРазр) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указана дата окончания разрешения", "Лист001", "ДатаКонРазр"));
	КонецЕсли;
...
Процедура НапечататьСтруктуру(СтруктураДанныхСтраницы, НомСтр, ИмяМакета, ПечатнаяФорма, ИННКПП)
	Попытка
		МакетПФ = Отчеты.РегламентированноеУведомлениеОНаличииСчета.ПолучитьМакет(ИмяМакета);
	Исключение
		Возврат;
	КонецПопытки;
