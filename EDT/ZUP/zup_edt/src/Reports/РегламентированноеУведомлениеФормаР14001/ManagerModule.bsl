...
Функция СведенияИзФормРегистрации(ИмяФормы, ДанныеУведомления) Экспорт
	...
	Результат.Вставить("КодЗаявления", "Р14001");
	Данные = ДанныеУведомления.Получить();
	
	Лист001 = Данные.ДанныеУведомления.Лист001;
	Результат.Вставить("ОГРН", Лист001.П01010000);
	Результат.Вставить("НаимЮЛПолн", Лист001.П01030000);
	
	Результат.Вставить("Заявители", Новый Массив);
	Для Каждого ЛистР Из Данные.ДанныеМногостраничныхРазделов.ЛистР Цикл 
		Если Не УведомлениеОСпецрежимахНалогообложения.СтраницаЗаполнена(ЛистР.Значение) Тогда 
			Продолжить;
		КонецЕсли;
		
		СвЗаяв = Новый Структура;
		СвЗаяв.Вставить("Фамилия", ЛистР.Значение.Р04010100);
		СвЗаяв.Вставить("Имя", ЛистР.Значение.Р04010200);
		СвЗаяв.Вставить("Отчество", ЛистР.Значение.Р04010300);
		СвЗаяв.Вставить("ЭлектроннаяПочта", ЛистР.Значение.Р04060200);
