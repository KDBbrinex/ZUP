...
Функция РасчетныйЛисток(ФизическиеЛица, Организация, МесяцФормированияОтчета, ОтчетРасчетныйЛисток = Неопределено) Экспорт
	...
	Возврат ДанныеРасчетныхЛистков(ФизическиеЛица, Организация, МесяцФормированияОтчета, МесяцФормированияОтчета, ОтчетРасчетныйЛисток).ДокументРезультат;
	
КонецФункции

Функция ДанныеРасчетныхЛистков(ФизическиеЛица, Организация, ДатаНачала, ДатаОкончания, ОтчетРасчетныйЛисток = Неопределено, ПараметрыСтраницы = Неопределено) Экспорт
	...
	Иначе
		ЗаполнитьЗначенияСвойств(ДокументРезультат, ПараметрыСтраницы);
	КонецЕсли;
	...
	ПериодОтчета.ДатаНачала    = НачалоМесяца(ДатаНачала);
	ПериодОтчета.ДатаОкончания = КонецМесяца(ДатаОкончания);
	...
	Для каждого Параметр Из МакетКомпоновкиДанных.ЗначенияПараметров Цикл
		...
		Если Параметр.Имя = "ГруппировкаОтчета" Тогда
			Параметр.Значение = 2;
			ПараметрНастроек = ОтчетРасчетныйЛисток.КомпоновщикНастроек.Настройки.ПараметрыДанных.Элементы.Найти("ГруппировкаОтчета");
			Если ПараметрНастроек <> Неопределено Тогда
				
				ПараметрНастроек.Значение = Параметр.Значение;
				ПараметрНастроек.ИдентификаторПользовательскойНастройки = "";
	...
	ДанныеРасчетныхЛистков = Неопределено;
	ОтчетРасчетныйЛисток.КомпоновщикНастроек.ПользовательскиеНастройки.ДополнительныеСвойства.Свойство("ДанныеРасчетныхЛистков", ДанныеРасчетныхЛистков);
	
	Возврат Новый Структура("ДокументРезультат,ДанныеРасчетныхЛистков", ДокументРезультат, ДанныеРасчетныхЛистков);
