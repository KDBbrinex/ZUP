...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0606062";
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	РегламентированнаяОтчетность.ЗаполнитьНачальныеНастройки(ЭтотОбъект, "ТаблицаВариантыЗаполненияФормаОтчета");
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для НомКол = 1 По 5 Цикл
		мФормаОтчета.Области["П0000100001" + Формат(НомКол, "ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000100001" + Формат(НомКол, "ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;	
		
		мФормаОтчета.Области["П0000100008" + Формат(НомКол, "ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000100008" + Формат(НомКол, "ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;	
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для НомКол = 1 По 5 Цикл
		...
		Области["П00001000080" + НомКол].Значение = Области["П00001000090" + НомКол].Значение
			+ Области["П00001000120" + НомКол].Значение
			+ Области["П00001000130" + НомКол].Значение 
			+ Области["П00001000140" + НомКол].Значение;
		
		Области["П00001000010" + НомКол].Значение = Области["П00001000020" + НомКол].Значение
			+ Области["П00001000040" + НомКол].Значение + 
			+ Области["П00001000060" + НомКол].Значение + 
			+ Области["П00001000080" + НомКол].Значение;
...
Процедура ПоказатьПериод()
	...
	СтруктураРеквизитовФормы.СтрПериодОтчета = Формат(СтруктураРеквизитовФормы.мДатаНачалаПериодаОтчета, "ДФ = 'ММММ'")
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма № ЗП-физическая культура и спорт");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "ЗП-физическая культура и спорт");
