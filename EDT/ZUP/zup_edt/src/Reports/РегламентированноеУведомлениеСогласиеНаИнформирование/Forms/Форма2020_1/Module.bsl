...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.СогласиеНаИнформированиеОЗадолженности;
...
Функция ПолучитьТаблицуОсобыхПолей()
	...
	НовСтр.ПутьXML = "Файл/Документ/СоглНПИнф/Тлф";
...
Процедура ОсобаяОбработкаЭлементов(Путь, СтрЗагружаемоеЗначение, Страница) Экспорт 
	Если Путь = "Файл/Документ/СоглНПИнф/Тлф" Тогда
		Если СтрНачинаетсяС(СтрЗагружаемоеЗначение.ЗначениеЭлемента, "+7") Тогда 
			Страница.Тлф = Сред(СтрЗагружаемоеЗначение.ЗначениеЭлемента, 3);
		КонецЕсли;
	КонецЕсли;
КонецПроцедуры
