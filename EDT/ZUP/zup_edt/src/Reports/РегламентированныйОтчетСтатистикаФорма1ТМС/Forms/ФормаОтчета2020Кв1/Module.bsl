...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураРеквизитовФормы.мКодОтчета = "0606065";
...
Процедура ПоказатьПериод()
	...
	"Раздел 2. Численность работников, замещавших муниципальные должности и должности муниципальной службы,
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 7 Цикл
		...
		СуммаПоСтр = 0;
		Для Стр = 203 По 207 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000201202" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = СуммаПоСтр;
		
		Области["П0000201209" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = Области["П0000201201" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201202" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201210" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201211" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		
	КонецЦикла;
	
	Для Стр = 201 По 212 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 5 По 7 Цикл
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Стр = 201 По 212 Цикл
		Если Стр = 202 Или Стр = 209 Тогда
			Для Кол = 3 По 7 Цикл
...
Функция ПечатьНаСервере(ВидПечати)
	...
	СтруктураПараметров.Вставить("НаименованиеЛиста",  "Форма 1-Т (МС)");
	СтруктураПараметров.Вставить("ОриентацияЛиста",    "Ландшафт");
	СтруктураПараметров.Вставить("ИмяСтраницы",        "1-Т (МС)");
