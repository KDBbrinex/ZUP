...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ПереходНаЕСХН;
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		ДанныеУведомления.Титульная.ПрЗаяв = "3";
	КонецЕсли;
...
&НаКлиенте
Процедура ДоступностьОбласти(Обл, Доступность, Значение = Неопределено)
	Если Доступность Тогда
		Обл.ЦветФона = ЦФЖ;
		Обл.Защита = Ложь;
		Обл.Значение = ?(ЗначениеЗаполнено(Значение), Значение, Обл.Значение);
	Иначе
		Обл.ЦветФона = ЦФБ;
		Обл.Защита = Истина;
...
Процедура ДоступностьПолей()
	ПрЗаяв = ПредставлениеУведомления.Области.ПрЗаяв.Значение;
	Если ПрЗаяв = "2" Или ПрЗаяв = "3" Тогда 
		ДоступностьОбласти(ПредставлениеУведомления.Области.ИНН, Истина);
		ДоступностьОбласти(ПредставлениеУведомления.Области.КПП, ЭтоЮЛ);
	Иначе
		ДоступностьОбласти(ПредставлениеУведомления.Области.ИНН, Ложь);
		ДоступностьОбласти(ПредставлениеУведомления.Области.КПП, Ложь);
	КонецЕсли;
	
	Если ПрЗаяв = "3" Тогда
		ДоступностьОбласти(ПредставлениеУведомления.Области.ПрДатаПер, Ложь, "1");
	ИначеЕсли ПрЗаяв = "1" Или ПрЗаяв = "2" Тогда 
		ДоступностьОбласти(ПредставлениеУведомления.Области.ПрДатаПер, Ложь, "2");
	Иначе
		ДоступностьОбласти(ПредставлениеУведомления.Области.ПрДатаПер, Ложь, "");
	КонецЕсли;
	
	Если Не ЭтоЮЛ Тогда
		ДоступностьОбласти(ПредставлениеУведомления.Области.Наименование, Ложь, ПредставлениеУведомления.Области.Наименование.Значение);
	КонецЕсли;
КонецПроцедуры