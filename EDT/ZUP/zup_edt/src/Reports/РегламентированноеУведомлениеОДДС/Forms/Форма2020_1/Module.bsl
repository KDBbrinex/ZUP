...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Объект.ВидУведомления = Перечисления.ВидыУведомленийОСпецрежимахНалогообложения.ОтчетДДС;
	...
	Если НЕ ОнлайнСервисыРегламентированнойОтчетностиВызовСервера.СобытиеНаступило("Отпр.ув.ОДДС") Тогда
...
Процедура ЗаполнитьНачальныеДанные() Экспорт
	...
	Если ЭтоЮЛ Тогда
		...
		ДанныеУведомленияТитульный.Вставить("ТлфОсн", СведенияОбОрганизации.ТелОрганизации);
	...
	Иначе
		...
		ДанныеУведомленияТитульный.Вставить("ТлфОсн", СведенияОбОрганизации.ТелДом);
...
Процедура СформироватьДеревоСтраниц() Экспорт
	...
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Сведения о" + Символы.ПС + "счете и банке";
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Лист2";
	Стр001.Многостраничность = Ложь;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Лист2";
	Стр001.МакетыПФ = "Печать_Форма2020_1_Лист2";
	
	Стр001 = КорневойУровень.Добавить();
	Стр001.Наименование = "Сведения о движении"+символы.ПС+"денежных средств";
	Стр001.ИндексКартинки = 1;
	...
	Стр001.ИндексКартинки = 1;
	Стр001.ИмяМакета = "Лист3";
	Стр001.Многостраничность = Истина;
	Стр001.Многострочность = Ложь;
	Стр001.УИД = Новый УникальныйИдентификатор;
	Стр001.ИДНаименования = "Лист3";
	Стр001.МакетыПФ = "Печать_Форма2020_1_Лист3";
	
	СтрПрих = Стр001.ПолучитьЭлементы().Добавить();
	СтрПрих.Наименование = "Зачислено" + Символы.ПС + "денежных средств";
	СтрПрих.ИндексКартинки = 1;
	СтрПрих.ИмяМакета = "Лист4";
	СтрПрих.Многостраничность = Ложь;
	СтрПрих.Многострочность = Истина;
	СтрПрих.УИД = Новый УникальныйИдентификатор;
	СтрПрих.ИДНаименования = "Лист4";
	СтрПрих.МногострочныеЧасти.Добавить("МнгСтрПрих");
	СтрПрих.МакетыПФ = "Печать_Форма2020_1_Лист4";
	
	СтрРасх = Стр001.ПолучитьЭлементы().Добавить();
	СтрРасх.Наименование = "Списано" + Символы.ПС + "денежных средств";
	СтрРасх.ИндексКартинки = 1;
	СтрРасх.ИмяМакета = "Лист5";
	СтрРасх.Многостраничность = Ложь;
	СтрРасх.Многострочность = Истина;
	СтрРасх.УИД = Новый УникальныйИдентификатор;
	СтрРасх.ИДНаименования = "Лист5";
	СтрРасх.МногострочныеЧасти.Добавить("МнгСтрРасх");
	СтрРасх.МакетыПФ = "Печать_Форма2020_1_Лист5";
КонецПроцедуры
	...
	Если УИДТекущаяСтраница <> Элемент.ТекущиеДанные.УИД Тогда 
		...
		Иначе
			Если Элемент.ТекущиеДанные.ПолучитьРодителя() = Неопределено Тогда 
				ПоказатьТекущуюСтраницу(Элемент.ТекущиеДанные.ИмяМакета, Элемент.ТекущиеДанные.МногострочныеЧасти, ПредУИД, Неопределено);
			Иначе
				ПоказатьТекущуюСтраницу(Элемент.ТекущиеДанные.ИмяМакета, Элемент.ТекущиеДанные.МногострочныеЧасти, ПредУИД, Элемент.ТекущиеДанные.ПолучитьРодителя().УИД);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	
	Если Не Элемент.ТекущиеДанные.Многострочность Тогда 
		ТекущиеМногострочныеЧасти.Очистить();
	КонецЕсли;
	
	Если ТекущееИДНаименования = "Титульная" Тогда 
		Если ЭтоЮЛ Тогда
			ОблОГРН = ПредставлениеУведомления.Области["ОГРНИП"];
			ОблОГРН.Защита = Истина;
			ОблОГРН.ЦветФона = ЦФБ;
		Иначе
			ОблОГРН = ПредставлениеУведомления.Области["ОГРН"];
			ОблОГРН.Защита = Истина;
			ОблОГРН.ЦветФона = ЦФБ;
			ОблОГРН = ПредставлениеУведомления.Области["Наименование"];
			ОблОГРН.Защита = Истина;
			ОблОГРН.ЦветФона = ЦФБ;
			ОблОГРН = ПредставлениеУведомления.Области["КПП"];
			ОблОГРН.Защита = Истина;
			ОблОГРН.ЦветФона = ЦФБ;
			ОблОГРН.Значение = Неопределено;
		КонецЕсли;
		ДоступностьОбластьФайлКорректировки();
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДоступностьОбластьФайлКорректировки()
	ОблИдФайлКорр = ПредставлениеУведомления.Области["ИдФайлКорр"];
	Если Не ЗначениеЗаполнено(ПредставлениеУведомления.Области["НомКорр"].Значение) Тогда
		ОблИдФайлКорр.Защита = Истина;
		ОблИдФайлКорр.ЦветФона = ЦФБ;
		ОблИдФайлКорр.Значение = Неопределено;
		УведомлениеОСпецрежимахНалогообложенияКлиент.ПриИзмененииСодержимогоОбласти(ЭтотОбъект, ОблИдФайлКорр, Истина);
	Иначе
		ОблИдФайлКорр.Защита = Ложь;
		ОблИдФайлКорр.ЦветФона = ЦФЖ;
	КонецЕсли;
КонецПроцедуры
...
&НаСервере
Процедура ПоказатьТекущуюСтраницу(ИмяМакета, МногострочныеЧасти, ПредУИД, УИДРодителя)
...
Процедура ПредставлениеУведомленияПриИзмененииСодержимогоОбласти(Элемент, Область)
	Если Область.Имя = "НомКорр" Тогда
		ДоступностьОбластьФайлКорректировки();
	КонецЕсли;
...
Процедура ПредставлениеУведомленияВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ДоступнаПодсистемаВалюты И СтандартнаяОбработка И ЭтоОбластьВалюта(Область) Тогда
		СтандартнаяОбработка = Ложь;
		ДополнительныеПараметры = Новый Структура("Область, СтандартнаяОбработка, Элемент", Область, СтандартнаяОбработка, Элемент);
...
Процедура ОткрытьФормуВыбораВалютыЗавершение(Результат, ДополнительныеПараметры) Экспорт
	Если Результат <> Неопределено Тогда
		...
		Обл = ПредставлениеУведомления.Области["ОКВ"];
...
Функция ЭтоОбластьОКСМ(Область)
	Возврат (Область.Имя = "СтранаМН" Или Область.Имя = "КодСтран");
КонецФункции

&НаКлиенте
Функция ЭтоОбластьВалюта(Область)
	Возврат (Область.Имя = "ОКВ");
КонецФункции
...
Функция ПолучитьКартуМногостраничныхРазделов() Экспорт 
	...
	НовСтр.ИДНаименования = "Лист3";
	НовСтр2 = НовСтр.Строки.Добавить();
	НовСтр2.ИДНаименования = "Лист4";
	НовСтр2 = НовСтр.Строки.Добавить();
	НовСтр2.ИДНаименования = "Лист5";
