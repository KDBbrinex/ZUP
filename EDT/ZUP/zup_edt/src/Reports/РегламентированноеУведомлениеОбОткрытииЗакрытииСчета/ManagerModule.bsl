...
Функция ПолучитьТаблицуФорм() Экспорт 
	...
	Стр.ПутьФормы = "Отчет.РегламентированноеУведомлениеОбОткрытииЗакрытииСчета.Форма.Форма2017_1";
...
Процедура ЗаполнитьМногострочнуюЧасть_2019(ДанныеУведомления, ДеревоВыгрузки)
	Узел_КодВалСчет = УведомлениеОСпецрежимахНалогообложения.НайтиУзелВДеревеПоПути(ДеревоВыгрузки, "Документ/УвСчетОткр/СвСчетИно/КодВалСчет", "Код"); 
	
	Если Узел_КодВалСчет = Неопределено Тогда 
		Возврат;
	КонецЕсли;
...
Функция СформироватьСписокЛистовФорма2019_1(Объект) Экспорт
	...
	НапечататьСтруктуру(СтруктураПараметров.ДанныеУведомления["Лист001"], НомСтр, "Печать_Форма2019_1_Лист1", ПечатнаяФорма, ИННКПП);
