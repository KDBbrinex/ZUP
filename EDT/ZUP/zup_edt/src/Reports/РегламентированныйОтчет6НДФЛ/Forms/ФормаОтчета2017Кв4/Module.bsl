...
Процедура ВставитьСтроку(Команда)
	...
	Если Тип(Элементы.ТабличныйДокумент.ТекущаяОбласть) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
		...
		ИмяГруппы = ОпределитьИдМногострочногоРазделаПоАктивнойСтраницеНаКлиенте(ИмяРаздела);
		
		Если ИмяГруппы = Неопределено Тогда
			
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = НСтр("ru='Не выбран многострочный раздел, в котором необходимо вставить строку'");
			Сообщение.Сообщить();
		...
		ИндексВставляемойСтроки = Число(Сред(ИмяТекОбласти, 15)) - 1;
		
		ВставитьСтрокуНаСервере(ИмяРаздела, ИндексВставляемойСтроки,
		Элементы.РазделыОтчета.ТекущиеДанные.КолонкаНомерСтраницыРазделаОтчета);
		
		ИмяНовОбласти = мСтруктураИменаКолонокРазделов[ИмяГруппы][0].Значение + "_"
		+ Формат(ИндексВставляемойСтроки + 1, "ЧГ=");
...
&НаСервере
Процедура ВставитьСтрокуНаСервере(ИмяРаздела, ИндексВставляемойСтроки, НомерСтраницыРаздела)
	
	СохранитьДанныеРаздела(ИмяРаздела, Ложь, Ложь);
	
	ИмяГруппы = ОпределитьИдМногострочногоРазделаПоАктивнойСтраницеНаСервере(ИмяРаздела);
	ГруппаМнЧ = ЭтаФорма[ИмяГруппы];
	ГруппаМнЧ.Вставить(ИндексВставляемойСтроки);
	
	ВывестиМакетВТаблДокНаСервере(ИмяРаздела, НомерСтраницыРаздела);
...
Процедура ТабличныйДокументПриАктивизации(Элемент)
	...
	Элементы.ТабличныйДокументКонтекстноеМенюВставитьСтроку.Доступность = Ложь;
	
	ИмяГруппы = РазделТекущейОбласти(ЭтаФорма, Элемент.ТекущаяОбласть);
	
	Если ЗначениеЗаполнено(ИмяГруппы) Тогда
		Элементы.ТабличныйДокументКонтекстноеМенюДобавитьСтроку.Доступность = Истина;
		Элементы.ТабличныйДокументКонтекстноеМенюВставитьСтроку.Доступность = Истина;
...
Процедура ПроверитьКС_1_1(СтруктураРезультата)
	...
	Если СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета >= '20200101' 
		И СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета < '20200701' Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьКС_3_6(СтруктураРезультата)
	...
	// КС отключено по многочисленным обращениям пользователей
	...
	Настройки.ЭтоОшибка		= ПроверкаОбязательна();
...
Процедура ПроверитьКС_3_7(СтруктураРезультата)
	...
	Заголовки.Вставить("Условие"			, НСтр("ru='Отключено для 2020г в связи с короновирусом'"));
	Заголовки.Вставить("Комментарий"		, НСтр("ru=''"));
	Заголовки.Вставить("ТекстФормулы"		, НСтр("ru='Проверка Раздела 2 (КС 3.7)'"));
	Заголовки.Вставить("Нарушение"			, НСтр("ru='Ошибка ввода'"));
	Заголовки.Вставить("ОписаниеКС"			, 
		НСтр("ru='Срок перечисления налога (стр.120 Раздела 2) не может быть выходным или праздничным днем'"));
	Заголовки.Вставить("ШаблонДопСведений"	, НСтр("ru=''"));
	Заголовки.Вставить("ШаблонРасшифровки"	, НСтр("ru='%1 является рабочим днем'"));
	Заголовки.Вставить("ОписаниеОшибки"		, 
		НСтр("ru='Ошибочно указан срок перечисления налога'"));
	Настройки = ОбработатьНастройки(		, Заголовки, СтруктураРезультата);
	...
	ТаблицаВыходныхДней = Обработки.РегламентированнаяОтчетностьСоотношенияПоказателей.ДанныеПроизводственногоКалендаря(
		мДанныеОтчета.ДанныеКС.ГодОтчета);
	
	КолвоСтрокВМногострочнойЧасти = КоличествоСтрок(мДанныеОтчета.Раздел2, "П000020012001_");
	Для Инд = 1 По КолвоСтрокВМногострочнойЧасти Цикл
		
		Показатели = ЗаполнитьПоказатели("Р2С120", мДанныеОтчета.Раздел2, Инд);
		
		Если НЕ ЗначениеЗаполнено(Показатели.Р2С120.Значение) Тогда
			Продолжить;
		КонецЕсли;
		
		ЗарегистрироватьФормулу(Настройки);
		РасшифроватьПоказатели(Показатели, Настройки, "Раздел2", 1);
	
		Если ТаблицаВыходныхДней = Неопределено Тогда
			Настройки.ЭтоОшибка = ДеньНедели(Показатели.Р2С120.Значение) > 5;
		Иначе
			Настройки.ЭтоОшибка = НЕ ТаблицаВыходныхДней.Найти(Показатели.Р2С120.Значение) = Неопределено;
		КонецЕсли;
		
		Настройки.Расшифровка	= СтрЗаменить(Настройки.ШаблонРасшифровки, "%1", Показатели.Р2С120.ЗначениеСтр);
		ДополнитьСтруктуруОшибок(Настройки);
...
Процедура СформироватьВспомогательныеДанные()
	...
	ПредставлениеРазделов.Вставить("П00001000", "Раздела 1 6-НДФЛ");
	ПредставлениеРазделов.Вставить("П00002001", "Раздела 2 6-НДФЛ");
	ПредставлениеРазделов.Вставить("П00002000", "Приложения 2 к Декларации по налогу на прибыль");
	ПредставлениеРазделов.Вставить("П01111000", "подраздела 1.1 Приложения 1 к Разделу 1 Расчета по страховым взносам");
...
Функция	ПоказательПоСтроке(УсловныйКодПоказателя)
	...
	ИначеЕсли УсловныйКодПоказателя = "Р2С120" Тогда
		Результат.Вставить("Имя", "П000020012001_%1");
		Результат.Вставить("Представление", Нстр("ru='Срок перечисления налога'"));
...
Функция	ПредставлениеПоказателя(Показатель, ПолноеПредставление = Истина)
	...
	Иначе
		ПодстрокаРаздела	= мДанныеОтчета.ДанныеКС.ПредставлениеРазделов[Лев(Показатель.Имя, 9)];
