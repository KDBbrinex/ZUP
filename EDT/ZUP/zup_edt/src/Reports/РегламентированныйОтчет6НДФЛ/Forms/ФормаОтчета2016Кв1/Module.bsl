...
Процедура ЗаполнитьМакетМЧБ(ПараметрыПечати, МакетМЧБ, ОбщиеРеквизитыРазделов)
	...
	Для Каждого Показатель Из Показатели Цикл
		Если СтрНайти(Показатель.Имя, "ОКТМО") > 0 Тогда
			Показатель.Значение = ОбъектОтчета(ЭтаФорма.ИмяФормы).КодМуниципальногоОбразованияПоОКТМО(Показатель.Значение);
...
Функция ПечатьНаСервере(ВидПечати)
	...
	РегламентированнаяОтчетностьКлиентСервер.ПроставлениеНомеровЛистов(ЭтаФорма, Неопределено, СтруктураРеквизитовФормы.СписокПечатаемыхЛистов);
		...
		Иначе
			...
			Если РегламентированнаяОтчетностьВызовСервера.ЭтоЮридическоеЛицо(СтруктураРеквизитовФормы.Организация) Тогда
				Если СтруктураРеквизитовФормы.Организация
					...
					ТабличныйДокумент.Области.ПоМесту.Значение = ?(ОрганизацияКрупнейшийНалогоплательщик(), "213", "212");
				Иначе
...
Функция СформироватьСтруктуруПараметров()
	...
	ПараметрыВыгрузки.Вставить("ОКТМО", ОбъектОтчета(ЭтаФорма.ИмяФормы).КодМуниципальногоОбразованияПоОКТМО(СокрЛП(Титульный.ОКТМО)));
...
Функция КодПоМестуДляЮрЛицаПослеВыбораКодаИФНС(СсылкаНаРегистрациюВНО)
	...
	Если СтруктураРеквизитовФормы.Организация
		...
		КодПоМесту = ?(ОрганизацияКрупнейшийНалогоплательщик(), "213", "212");
