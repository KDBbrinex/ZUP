...
Функция ПолучитьФормуПоУмолчанию() Экспорт 
	Возврат "Отчет.РегламентированноеУведомлениеПрекращениеЕСХН.Форма.Форма2017_1";
КонецФункции
...
Функция ПроверитьДокументСВыводомВТаблицу_Форма2017_1(Объект, Данные, УникальныйИдентификатор)
	...
	Если Не ЗначениеЗаполнено(Титульная.ДатаПрекр) Тогда 
		ТаблицаОшибок.Добавить(Документы.УведомлениеОСпецрежимахНалогообложения.ПолучитьПустуюСтруктуруНавигацииПоОшибкам("Не указана дата прекращения деятельности", "Титульная", "ДатаПрекр"));
	КонецЕсли;
...
Процедура НапечататьСтруктуру(СтруктураДанныхСтраницы, НомСтр, ИмяМакета, ПечатнаяФорма, ИННКПП)
	Попытка
		МакетПФ = Отчеты.РегламентированноеУведомлениеПрекращениеЕСХН.ПолучитьМакет(ИмяМакета);
	Исключение
		Возврат;
	КонецПопытки;
