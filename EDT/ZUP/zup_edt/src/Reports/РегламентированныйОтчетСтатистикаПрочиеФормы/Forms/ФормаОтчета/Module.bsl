...
Процедура ПодготовитьДанныеОтчетаПоШаблону(ДанныеШаблонаОтчета, Отказ = Ложь, ЗагружатьСписки = Истина);
	...
	Если ЗагружатьСписки Тогда 
		...
		Для каждого УзелСправочник Из УзлыСправочники Цикл
			...
			Если ДочерниеУзлыКоличество > 100000 Или ДочерниеУзлыКоличество = 0 Тогда 
...
Процедура РасчетНаКлиенте(ИмяСтраницыПанели = "", ОбластьИмя = "") Экспорт	
	...
	Если СинхронныйАвторасчет Тогда 
		Если ВРазделеЕстьАвторасчет Тогда 
			РасчетКСНаСервере(ИмяСтраницыПанели, ОбластьИмя);
		КонецЕсли;
	Иначе
		ОтключитьОбработчикОжидания("ПолныйПересчетКСНаСтранице");
		Если ВРазделеЕстьАвторасчет Тогда 
...
Функция СформироватьСкриптРасчета(ЯчейкиДляРасчета, ИмяТекущейСтраницы)
	...
	Для Инд = 0 По 1 Цикл
		Для Каждого КЗ Из ЯчейкиДляРасчета Цикл
			...
			Расчет = РасчитываемыеЯчейки.НайтиСтроки(Новый Структура("Раздел, ЗависимоеЗначение, ТипРасчета", ИмяТекущейСтраницы, КЗ.Ключ, 3-Инд));
			Если Расчет.Количество() = 1 Тогда
				ФормулаРасчета = Расчет[0].Формула;
				Для Каждого Парам Из Расчет[0].Параметры Цикл 
					ОбластьПараметр = ТабличныйДокумент.Области.Найти(Парам);
...
&НаСервере
Процедура РасчетКСНаСервере(ИмяТекущейСтраницы = "", ОбластьИмя = "")
	Если СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
		Возврат;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОбластьИмя) И ЗначениеЗаполнено(ИмяТекущейСтраницы) Тогда 
		ЯчейкиДляРасчета = Новый Соответствие;
		ЗаполнитьЗависимыеЯчейкиРек(ЯчейкиДляРасчета, ИмяТекущейСтраницы, ОбластьИмя);
...
Процедура ПроверкаКСВФоне(АдресВозвращаемыхПараметровДляПроверкиВФоне)
	...
	Если РезультатВыполнения.ЗаданиеВыполнено Тогда
		РегламентированнаяОтчетностьКлиент.ВывестиКонтрольныеСоотношенияПоШаблону(РезультатПроверкиКС, ОбработкаКС, ЭтотОбъект);
	Иначе
...
Функция ПроверитьКонтрольныеСоотношенияВОтчете() Экспорт
	...
	Для каждого КонтрольноеСоотношение Из ТаблицаКонтрольныхСоотношений Цикл
		...
		СоотношениеВыполнено = УниверсальныйОтчетСтатистики.РезультатПроверкиКонтрольногоСоотношения(ЭтотОбъект, Формула, Условие, УсловиеПериод, РазрядностьОкругления, ДопустимоеОтклонение);
