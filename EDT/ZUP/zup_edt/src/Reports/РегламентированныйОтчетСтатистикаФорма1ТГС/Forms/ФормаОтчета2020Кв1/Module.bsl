...
Процедура ПоказатьПериод()
	...
	ПолеТабличногоДокументаФормаОтчета.Области.ЗаголовокРаздел2.Значение =
	"Раздел 2. Численность работников, замещавших государственные должности и должности государственной гражданской службы,
	|получивших дополнительное профессиональное образование в " + ПериодОтчета + " году, человек";
...
Процедура Расчет(Форма, ИмяТекущейСтраницыПанели = "", ОбластьИмя = "")
	...
	Для Гр = 3 По 7 Цикл
		...
		Для Стр = 217 По 220 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000201216" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 212 По 215 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000201211" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 208 По 210 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000201207" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = СуммаПоСтр;
		
		СуммаПоСтр = 0;
		Для Стр = 204 По 206 Цикл
			СуммаПоСтр = СуммаПоСтр + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
		Области["П0000201203" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = СуммаПоСтр;
		
		Области["П0000201202" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = Области["П0000201203" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201207" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201211" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201216" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		
		Области["П0000201221" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение = Области["П0000201201" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201202" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201222" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение
			+ Области["П0000201223" + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
	...
	Для Стр = 201 По 224 Цикл
		СуммаПоГрафам = 0;
		Для Гр = 5 По 7 Цикл
			СуммаПоГрафам = СуммаПоГрафам + Области["П0000201" + Стр + Формат(Гр, "ЧЦ=2; ЧВН=")].Значение;
		КонецЦикла;
...
Процедура УстановитьЗащитуИЦветФонаОбластейАвторассчитываемыхПоказателей()
	...
	Для Стр = 102 По 103 Цикл
		мФормаОтчета.Области["П0000101" + Стр + "04"].ЦветФона = ЦветФонаЗначение;
		мФормаОтчета.Области["П0000101" + Стр + "04"].Защита   = Не ОтклАвтоРасчетЗначение;
	КонецЦикла;
	
	Для Стр = 201 По 224 Цикл
		Если СтрокаАвторасчета(Стр) Тогда
			Для Кол = 3 По 7 Цикл
				мФормаОтчета.Области["П0000201" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].ЦветФона = ЦветФонаЗначение;
				мФормаОтчета.Области["П0000201" + Стр + Формат(Кол,"ЧЦ=2; ЧВН=")].Защита   = Не ОтклАвтоРасчетЗначение;
			КонецЦикла;
		Иначе
			мФормаОтчета.Области["П0000201" + Стр + "04"].ЦветФона = ЦветФонаЗначение;
			мФормаОтчета.Области["П0000201" + Стр + "04"].Защита   = Не ОтклАвтоРасчетЗначение;
		КонецЕсли;
	КонецЦикла;
...
Функция ПечатьНаСервере(ВидПечати)
	...
	ПечатнаяФорма.Вывести(ТабличныйДокумент.ПолучитьОбласть("РазделПодписи"));
	СтруктураРеквизитовФормы.мПечатныеФормы.Добавить(ПечатнаяФорма, "Форма 1-Т (ГС)");
...
Функция СравнениеОтчетовНаСервере(ДанныеДляСравнения)
	...
	ДеревоДляСравнения = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"]);
...
Функция ЗагрузкаОтчетаИзФайлаНаСервере(ДанныеДляЗагрузки)
	...
	ДеревоДляЗагрузки = РегламентированнаяОтчетность.ПолучитьДеревоВыгрузки(ЭтотОбъект, СтруктураРеквизитовФормы["ИмяМакетаСхемыВыгрузки"]);
...
Функция СформироватьСтруктуруПараметров() Экспорт 
	...
	ДопАтрибуты = РегламентированнаяОтчетность.СформироватьСтруктуруДопАтрибутов(ИмяФормы, СтруктураРеквизитовФормы["ИмяМакетаАтрибутовВыгрузки"]);
