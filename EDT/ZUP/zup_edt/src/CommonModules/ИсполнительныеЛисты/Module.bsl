...
Процедура РассчитатьУдержанияПоИсполнительнымЛистам(СпособРасчета, СтрокиРасчета, МенеджерВременныхТаблиц, ИсключаемыйРегистратор, МесяцНачисления, ПогашениеЗадолженности = Неопределено) Экспорт
	...
	Пока Выборка.СледующийПоЗначениюПоля("ИсполнительныйДокумент") Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			КонецЕсли;
			РасчетнаяБаза = РасчетнаяБазаИсполнительногоЛиста - Макс(ИсчисленныйПоИсполнительномуЛистуНДФЛ, 0);
			Если Выборка.СпособРасчета = Перечисления.СпособыРасчетаУдержанияПоИсполнительномуДокументу.ФиксированнойСуммой Тогда
...
Функция УдержанияПоИсполнительнымДокументамДляПлатежейПоРезультатамРасчета(Организации, МесяцНачисления) Экспорт
	...
	|	Удержания.СуммаУдержанияОборот КАК Сумма,
	|	Удержания.СуммаВознагражденияПлатежногоАгентаОборот КАК ВознаграждениеАгента,
	|	Удержания.ИсполнительныйДокумент КАК ИсполнительныйДокумент
	|ПОМЕСТИТЬ ВТУдержания
	|ИЗ
	|	РегистрНакопления.УдержанияПоИсполнительнымДокументам.Обороты(&ПериодРегистрации, КОНЕЦПЕРИОДА(&ПериодРегистрации, МЕСЯЦ), Месяц, Организация В (&Организации)) КАК Удержания
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура СоздатьВТОсталосьУдержатьПоДокументам(МенеджерВременныхТаблиц, ИсключаемыйРегистратор, МесяцНачисления)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	               ...
	               |	МИНИМУМ(ДанныеДляРасчета.Месяц) КАК МесяцНачала,
	               |	МАКСИМУМ(ДанныеДляРасчета.Месяц) КАК МесяцОкончания
	               ...
	               |			И УдержанияДоПределаПоСотрудникам.Удержание = УдержанияДоПредела.Удержание
	               |			И УдержанияДоПределаПоСотрудникам.ДокументОснование = УдержанияДоПредела.ДокументОснование
	               |			И (УдержанияДоПределаПоСотрудникам.Период МЕЖДУ УдержанияДоПредела.МесяцНачала И УдержанияДоПредела.МесяцОкончания)
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	УдержанияДоПредела.Организация КАК Организация,
	               |	УдержанияДоПредела.ФизическоеЛицо КАК ФизическоеЛицо,
	               |	УдержанияДоПредела.Удержание КАК Удержание,
	               |	УдержанияДоПредела.ДокументОснование КАК ДокументОснование,
...
Функция УдержанныеСуммыФизическихЛицСУчетомОграничений(ФизлицаДокументы)
	...
	Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
		Пока Выборка.СледующийПоЗначениюПоля("ИсполнительныйДокумент") Цикл
			...
			СуммаОграничения = Выборка.СуммаОграничения;
			Пока Выборка.Следующий() Цикл
