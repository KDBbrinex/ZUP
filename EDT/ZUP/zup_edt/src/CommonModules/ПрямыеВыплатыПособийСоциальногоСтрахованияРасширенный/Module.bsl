...
Функция ДанныеЗаполненияЗаявленияВФССОВозмещенииВыплатРодителямДетейИнвалидов(Документ, ОплатаДнейУходаЗаДетьмиИнвалидами) Экспорт
	...
	Если ЗначениеЗаполнено(ОплатаДнейУходаЗаДетьмиИнвалидами) Тогда		
		...
		|	ОплатаДнейУходаЗаДетьмиИнвалидами.ДатаНачалаСобытия КАК ДатаНачалаСобытия,
	...
	|		ПО ВТДокументы.Ссылка = ОплатаДнейУходаЗаДетьмиИнвалидами.Ссылка
	...
	Запрос.Текст = Запрос.Текст +
	"ВЫБРАТЬ
	|	Оплаты.ДатаНачалаСобытия КАК ДатаНачалаСобытия,
	|	Оплаты.ФизическоеЛицо КАК ФизическоеЛицо
	|ПОМЕСТИТЬ ВТДатыСобытийФизическихЛиц
	|ИЗ
	|	ВТДокументы КАК Оплаты
	|
	|СГРУППИРОВАТЬ ПО
	|	Оплаты.ДатаНачалаСобытия,
	...
	|ИЗ
	|	ВТДатыСобытийФизическихЛиц КАК Оплаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ПособияПоСоциальномуСтрахованию КАК Пособия
	|		ПО Оплаты.ФизическоеЛицо = Пособия.ФизическоеЛицо
	|			И Оплаты.ДатаНачалаСобытия = Пособия.ДатаСтраховогоСлучая
	...
	|		ПО Оплаты.Сотрудник = ОплатыДругихЗаявлений.Сотрудник
	...
	|		КОГДА ОплатаДнейУходаЗаДетьмиИнвалидами.СреднедневнойЗаработок > 0
	|			ТОГДА ОплатаДнейУходаЗаДетьмиИнвалидами.СреднедневнойЗаработок
	...
	|				И ВТДокументы.ДатаНачалаСобытия = СуммыВзносовДоступные.ДатаНачалаСобытия
	...
	|				И ВТДокументы.ДатаНачалаСобытия = СуммыВзносовУчтенные.ДатаНачалаСобытия
