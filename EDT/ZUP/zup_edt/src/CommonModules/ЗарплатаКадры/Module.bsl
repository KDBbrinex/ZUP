...
Процедура ПриДобавленииПереименованийОбъектовМетаданных(Итог) Экспорт
	...
	ЗарплатаКадрыВнутренний.ПриДобавленииПереименованийОбъектовМетаданных(Итог);
...
Процедура ПриЗаполненииВидовДоступа(ВидыДоступа) Экспорт
	...
	ЗарплатаКадрыВнутренний.ПриЗаполненииВидовДоступа(ВидыДоступа);
...
Процедура ПриЗаполненииИспользованияВидаДоступа(ИмяВидаДоступа, Использование) Экспорт
	...
	ЗарплатаКадрыВнутренний.ПриЗаполненииИспользованияВидаДоступа(ИмяВидаДоступа, Использование);
...
Функция ВыгрузитьДокумент(Ссылка, УникальныйИдентификатор = Неопределено) Экспорт
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.СправкиНДФЛДляПередачиВНалоговыйОрган") Тогда
		ИнформацияОФайле = ПолучитьДанныеФайла(Ссылка, УникальныйИдентификатор, Истина);
...
Функция ПолучитьКодПодчиненностиРеестраСведенийНаВыплатуПособийФСС(Ссылка) Экспорт
	КодПодчиненности = "";
	// АПК:280-выкл Обработка исключения не нужна, попытка выполняется на случай отсутствия реквизита
	Попытка
...
Процедура ПослеЗагрузкиУведомленийОНовыхСообщенияхСЭДО(Организация, Уведомления, ОбработанныеУведомления) Экспорт
	...
	УчетПособийСоциальногоСтрахования.ПослеЗагрузкиУведомленийОНовыхСообщенияхСЭДО(Организация, Уведомления, ОбработанныеУведомления);
...
Процедура ПослеРасшифровкиСообщенияСЭДО(Организация, Сообщение, Результат) Экспорт
	...
	УчетПособийСоциальногоСтрахования.ПослеРасшифровкиСообщенияСЭДО(Организация, Сообщение, Результат);
...
// 
// Параметры:
//	ГруппыПроверок - ТаблицаЗначений - см. КонтрольВеденияУчетаПереопределяемый.ПриОпределенииПроверок.ГруппыПроверок
//
// Возвращаемое значение:
...
Функция ОбновитьКлассификатор(ТекстXML, ПолучатьДанныеИзСервиса = Истина) Экспорт
	...
	ТаблицаСоСсылками = ПолныеИменаВСсылки(ТаблицаБезСсылок, НаборЗаписей.ВыгрузитьКолонки(), ТаблицаОшибок);
...
Процедура ОбновитьНаименованияФизЛицаИСотрудника(ТаблицаФизЛиц) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ФИОФизЛицПоследнее.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ФИОФизЛицПоследнее.Фамилия КАК Фамилия,
	|	ФИОФизЛицПоследнее.Имя КАК Имя,
	|	ФИОФизЛицПоследнее.Отчество КАК Отчество,
