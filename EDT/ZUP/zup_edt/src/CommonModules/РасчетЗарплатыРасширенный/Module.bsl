...
//			Сумма
//
Процедура СформироватьДвиженияУдержанийДоПределаПоСотрудникам(Движения, Отказ, ПериодРегистрации, УдержанияДоПределаПоСотрудникам) Экспорт 
...
Процедура СформироватьДвиженияКорректировкиВыплатыПоВременнойТаблицеНачислений(Движения, Отказ, Организация, ПериодРегистрации, ПорядокВыплаты, ДанныеДляПроведения, Записывать = Ложь, РегистрироватьСуммыКорректировкиВыплаты = Неопределено) Экспорт 
	...
	Если РегистрироватьСуммыКорректировкиВыплаты Тогда
		...
		УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛИКорректировкиВыплаты(ДанныеДляПроведенияУчетЗарплаты, Отказ, Неопределено, ДанныеДляПроведения.КорректировкиВыплатыПоСотрудникам, Записывать);
...
Функция ПерезаполнитьДанныеФормы(Форма, ОписаниеДокумента, Знач СотрудникиФизическиеЛица = Неопределено, СохранятьИсправления = Истина, ИспользоватьДлительнуюОперацию = Ложь, ПерезаполнятьВедущиеТаблицы = Истина, СотрудникиПериодДействияПерерасчет = Неопределено) Экспорт
	...
	Если ОписаниеДокумента.ЭтоНачислениеЗарплаты
		...
		СтруктураПараметров.ИспользоватьВоеннуюСлужбу = Форма.ИспользоватьВоеннуюСлужбу;
...
Процедура ПроверитьПересечениеФактическогоПериодаДействия(Параметры, Отказ = Ложь) Экспорт 
		...
		Если Не(СведенияОбИсправлении.Исправлен Или СведенияОбИсправлении.Сторнирован) Тогда 
			...
			Если Выборка.Количество() > 0 Тогда 
				...
				Пока Выборка.Следующий() Цикл 
					...
					ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Документ, "Начисления" + "[" + Формат(Выборка.НомерСтроки - 1, "ЧН=0; ЧГ=0") + "]." + "Результат", , Отказ);
				КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл 
		...
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Выборка.Регистратор, "Начисления" + "[" + Формат(Выборка.НомерСтрокиРегистр - 1, "ЧН=0; ЧГ=0") + "]." + "Результат", , Отказ);
	КонецЦикла;	
	...
	Пока Выборка.Следующий() Цикл 
		Если НомераСтрок[Выборка.НомерСтрокиДляПроверки] = Неопределено Тогда 
			...
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Документ, "Объект.Начисления" + "[" + Формат(Выборка.НомерСтрокиДляПроверки - 1, "ЧН=0; ЧГ=0") + "]." + ИмяКолонки, , Отказ);
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьНачисления(ДанныеДляПроведения, Документ, ТаблицаНачислений = "Начисления", ПолеДатыДействия = "Ссылка.МесяцНачисления", ПолеВидаНачисления = "Начисление", СписокФизическихЛиц = Неопределено, ПолеПериодРегистрации = "Ссылка.МесяцНачисления", СторнируемыйДокументКакСторно = Ложь) Экспорт
	...
	Иначе
		...
		Если МетаданныеДокумента.Реквизиты.Найти("СпособРасчетовСФизическимиЛицами") <> Неопределено Тогда
			...
			СпособРасчетов = ?(ЗначениеЗаполнено(СпособРасчетов), СпособРасчетов, Перечисления.СпособыРасчетовСФизическимиЛицами.ОплатаТруда);
...
Процедура ЗаполнитьДанныеНДФЛ(ДанныеДляПроведения, ДокументСсылка, ФизическиеЛица = Неопределено) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиФинансированияЗарплатаРасширенный") Тогда
		...
		|				ИЛИ РаспределениеУдержаний.ВидУдержания = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛДоходыКонтрагентов)
		|				ИЛИ РаспределениеУдержаний.ВидУдержания = ЗНАЧЕНИЕ(Перечисление.ВидыОсобыхНачисленийИУдержаний.НДФЛРасчетыСБывшимиСотрудниками)
		|				ТОГДА РаспределениеУдержаний.Результат
		|			ИНАЧЕ 0
		...
		|	РаспределениеУдержаний.ВидДоходаИсполнительногоПроизводства КАК ВидДоходаИсполнительногоПроизводства,
		|	РаспределениеУдержаний.НалогПоСтавке13 КАК НалогПоСтавке13,
		|	НДФЛ.*
		...
		|ГДЕ
		|	НДФЛ.НалогПоСтавке13 <> 0 ИЛИ НДФЛ.ЗачтеноАвансовыхПлатежейПоСтавке13 <> 0";
...
Процедура ЗаполнитьНастройкиРасчетаЗарплаты(ИспользоватьНачислениеЗарплаты = Неопределено) Экспорт
	...
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
	|		КОНЕЦ КАК ВыбиратьВидНачисленияКомпенсацииОтпускаВоеннослужащим,
	|		ВЫБОР
	|			КОГДА СУММА(ВЫБОР
	...
	УстановитьИспользованиеРасчетаПервойПоловиныМесяца(Настройки, ИспользоватьНачислениеЗарплаты);
...
Функция РасчетБазыНачислений(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабораНачислений, МенеджерДанныхУчетаВремени = Неопределено, МенеджерКадровогоУчета = Неопределено) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	РасчетБазыНачислений = Обработки.РасчетБазыНачисленийУдержанийАрхивный.Создать();	
	УстановитьПривилегированныйРежим(Ложь);
	
	РасчетБазыНачислений.УстановитьРассчитываемыйНаборНачислений(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабораНачислений, МенеджерДанныхУчетаВремени, МенеджерКадровогоУчета);
	
	Возврат РасчетБазыНачислений; 
КонецФункции

Функция РасчетБазыУдержаний(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабораУдержаний, МенеджерДанныхУчетаВремени = Неопределено, МенеджерКадровогоУчета = Неопределено) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	РасчетБазыУдержаний = Обработки.РасчетБазыНачисленийУдержанийАрхивный.Создать();	
	УстановитьПривилегированныйРежим(Ложь);
	
	РасчетБазыУдержаний.УстановитьРассчитываемыйНаборУдержаний(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабораУдержаний, МенеджерДанныхУчетаВремени, МенеджерКадровогоУчета);
...
Функция ТипыОснованийПлановыхНачислений() Экспорт
	ТипыОснованийПлановыхНачислений = Новый Массив;
	
	ТипыРегистраторовПлановыхНачислений = Метаданные.РегистрыСведений.ПлановыеНачисления.СтандартныеРеквизиты.Регистратор.Тип.Типы();
	ТипыОснования = Метаданные.ОпределяемыеТипы.ОснованиеНачисления.Тип.Типы();
	
	Для Каждого ТипОснования Из ТипыОснования Цикл
		Если ТипыРегистраторовПлановыхНачислений.Найти(ТипОснования) <> Неопределено Тогда
			ТипыОснованийПлановыхНачислений.Добавить(ТипОснования);
		КонецЕсли;
	КонецЦикла;	
	
	Возврат ТипыОснованийПлановыхНачислений;	
КонецФункции	
...
Функция КоэффициентыОплаченногоРабочегоВремениНачислений(МенеджерВременныхТаблиц) Экспорт
	...
	Запрос.Текст = 
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления.ВидыВремени КАК НачисленияВидыВремени
		|		ПО ВТНачисленияСотрудников.ВидРасчета = НачисленияВидыВремени.Ссылка
		|			И НачисленияВидыВремени.ВидВремени В (&РабочееВремя, &Явка)
...
Процедура ЗаполнитьИсходныйДокумент(ДокументОбъект)	Экспорт
	...
	Для Каждого ОписаниеТаблицы Из МетаданныеТаблиц Цикл
		Если ОписаниеТаблицы.Реквизиты.Найти("ИсходныйДокумент") <> Неопределено Тогда
			ЗаполнитьИсходныйДокументВТабличнойЧасти(ДокументОбъект, Ссылка, ОписаниеТаблицы);
		КонецЕсли;
	КонецЦикла;	
...
Процедура СформироватьЗадолженностьПоУдержаниямФизическихЛиц(Движения, ЗадолженностьПоУдержаниям, ЗаписыватьДвижения = Ложь) Экспорт 
...
Процедура СоздатьВТРасчетнаяБазаНачисленийПоВременнойТаблицеКаскадно(МенеджерВременныхТаблиц, ИсключаемыйРегистратор = Неопределено, МенеджерКадровогоУчета = Неопределено, МенеджерДанныхУчетаВремени = Неопределено) Экспорт
	Если ЗарплатаКадры.ВыполнятьРасчетЗарплатыБезОптимизации() Тогда
		СоздатьВТРасчетнаяБазаНачисленийПоВременнойТаблицеКаскадноАрхивный(
			МенеджерВременныхТаблиц,
...
Процедура СоздатьВТРасчетнаяБазаНачисленийПоВременнойТаблицеКаскадноАрхивный(МенеджерВременныхТаблиц, ИсключаемыйРегистратор = Неопределено, МенеджерКадровогоУчета = Неопределено, МенеджерДанныхУчетаВремени = Неопределено) Экспорт
	УдалитьВТ = Новый Массив;
	...
	ОписаниеРассчитываемогоНабора = ОписаниеРассчитываемогоНабораНачислений();
	ОписаниеРассчитываемогоНабора.ИмяВТРассчитываемыеЗаписи = "ВТОсновныеЗаписи";
	ОписаниеРассчитываемогоНабора.ИмяТаблицыБазовыеЗаписи = "ПланВидовРасчета.Начисления.БазовыеВидыРасчета";
	ОписаниеРассчитываемогоНабора.ИмяПоляОчередностьРасчета = "ВидРасчета.ОчередностьРасчета";

	ОписаниеТекущегоНабораРегистраНачисления = ОписаниеТекущегоНабораРегистраНачисления();
	...
	РасчетБазыНачислений = РасчетБазыНачислений(МенеджерВременныхТаблиц, ОписаниеРассчитываемогоНабора, МенеджерДанныхУчетаВремени, МенеджерКадровогоУчета);
	
	РасчетБазыНачислений.СоздатьВТРасчетнаяБазаНачислений(, ,"ВТРасчетнаяБаза");	
	...
	Пока Выборка.Следующий() Цикл
		...
		Для каждого СтрокаТаблицы Из СтрокиТаблицы Цикл
			...
			Если Не СтрокаТаблицы.ПроверкаНеобходимостиВосстановления
				...
				Или ПособиеПоУходуЗаРебенком И СтрокаТаблицы.СтрокаДвижений.Используется <> Выборка.Используется Тогда
...
Процедура ДополнитьЗапросВТПоказателейПоУчетуВремени(Запрос)
	...
	|	РабочееВремя.Дата,
	|	РабочееВремя.Часы,
	|	РабочееВремя.ПереходящаяЧастьТекущейСмены
	...
	|	ДниРаботы.Часы,
	|	ДниРаботы.ПереходящаяЧастьТекущейСмены
	...
	|			И ВЫБОР КОГДА ВТДниРаботыРасширенный.ПереходящаяЧастьТекущейСмены ТОГДА ДОБАВИТЬКДАТЕ(ВТДниРаботыРасширенный.Дата, ДЕНЬ, 1) ИНАЧЕ ВТДниРаботыРасширенный.Дата КОНЕЦ = ДанныеПроизводственногоКалендаря.Дата
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	СотрудникиГрафики.Сотрудник,
	|	ЕСТЬNULL(ВТНормаВремениСотрудников.НормаЧасов, 0)
	|ИЗ
	|	ВТСчитатьПоГрафику КАК СотрудникиГрафики
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТНормаВремениСотрудников КАК ВТНормаВремениСотрудников
	|		ПО СотрудникиГрафики.Сотрудник = ВТНормаВремениСотрудников.Сотрудник
...
Процедура ЗаполнитьИсходныйДокументВТабличнойЧасти(Объект, Регистратор, ОписаниеТаблицы)
	
	Для Каждого СтрокаТабличнойЧасти Из Объект[ОписаниеТаблицы.Имя] Цикл
		Если СтрокаТабличнойЧасти.ИсходныйДокумент = Неопределено Тогда
			СтрокаТабличнойЧасти.ИсходныйДокумент = Регистратор;
		КонецЕсли;
	КонецЦикла;
