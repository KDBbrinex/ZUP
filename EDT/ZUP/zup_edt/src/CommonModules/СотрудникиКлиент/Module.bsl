...
Процедура ПриИзмененииФИО(Форма, ФизическоеЛицо, МенеджерЗаписиФИО)
	...
	Если СтрНайти(ФИО, СокрЛП(МенеджерЗаписиФИО.Фамилия + " " + МенеджерЗаписиФИО.Имя + " " + МенеджерЗаписиФИО.Отчество)) = 0 Тогда
		ФизическоеЛицо.ФИО = СокрЛП(МенеджерЗаписиФИО.Фамилия + " " + МенеджерЗаписиФИО.Имя + " " + МенеджерЗаписиФИО.Отчество);
		ПоказатьПредупреждение(, НСтр("ru = 'Возможно, не корректно введено полное имя!
			|Полное имя не соответствует восстановленному по данным о фамилии, имени и отчеству.'"));
	КонецЕсли;
...
Функция ПараметрыДополнительнойФормы(ОписаниеДополнительнойФормы, Форма, ИзФормыСотрудника)
