...
Процедура ЗаписатьФискальнуюОперацию(ПараметрыФискализации) Экспорт
	ВызватьИсключение ТекстОшибки_Подсистема_ЧекопечатающиеУстройства();
КонецПроцедуры
...
//
// Параметры:
//  ПараметрыЧекаККТ - Параметры чека ККТ.
// 
...
//
Процедура ЗаписатьСтатусЧекаВОчереди(ПараметрыФискализации, СтатусЧека, ТекстОшибки = Неопределено) Экспорт
	ВызватьИсключение ТекстОшибки_Подсистема_ЧекопечатающиеУстройства();
КонецПроцедуры
...
Процедура ЗаписатьДанныеКодаТоварнойНоменклатуры(ЗаписьXML, ДанныеКодаТоварнойНоменклатуры)
	...
	КонецЕсли;
	Если Не ПустаяСтрока(ДанныеКодаТоварнойНоменклатуры.РевизитКодаТовара) Тогда // Ревизит кода товара.
					...
					Пока ЧтениеXML.Прочитать() И НЕ (ЧтениеXML.Имя = "Positions" И ЧтениеXML.ТипУзла = ТипУзлаXML.КонецЭлемента) Цикл
						...
						Если ЧтениеXML.Имя = "GoodCodeData" И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда   
							Если ФискальнаяСтрока <> Неопределено Тогда
								...
								ФискальнаяСтрока.ДанныеКодаТоварнойНоменклатуры.РевизитКодаТовара = ЗначениеXMLАтрибута(ЧтениеXML, "MarkingCode");
							КонецЕсли
						КонецЕсли;
...
Функция ТекстОшибки_Подсистема_ЧекопечатающиеУстройства()
	Возврат НСтр("ru = 'Подсистема ""Чекопечатающие устройства"" отсутствует в конфигурации'");
КонецФункции
