...
// Параметры:
//  Движения - коллекции наборов записей движений документа,
//				должна содержать коллекцию еще не записанного набора НДФЛПеречисленный.
//  ДанныеДляДополнения - структура см НовоеОписаниеПараметровДополненияНДФЛСтатьями.
//
...
Функция НалогиКУдержаниюПоСтатьям(НалогиКУдержанию, ПараметрыРаспределения) Экспорт
	...
	ФинансированиеЗадано = ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Или ЗначениеЗаполнено(Финансирование.СтатьяРасходов);
	
	// Получение базы НДФЛ для распределения по статьям.
	КолонкиОтбораБазы = "ФизическоеЛицо, ДокументОснование";
	ОснованияНДФЛ = НалогиКУдержанию.Скопировать(, КолонкиОтбораБазы);
	ОснованияНДФЛ.Свернуть(КолонкиОтбораБазы);
	
	ОснованияВыплат = ТаблицаВыплат.Скопировать(, КолонкиОтбораБазы);
	ОснованияВыплат.Свернуть(КолонкиОтбораБазы);
	...
	// Колонки базы распределения: ФизическоеЛицо,Подразделение,ДокументОснование,КатегорияДохода,СтатьяФинансирования,СтатьяРасходов,Сумма.
	БазаРаспределения = ОтражениеЗарплатыВУчетеВнутренний.БазаРаспределенияНДФЛКУдержанию(ОснованияНДФЛ, ПараметрыРаспределения.НДФЛПоСотрудникам);
	...
	Если ФинансированиеЗадано Тогда
		
		ОтборФинансирование = Новый Структура;
		Если ЗначениеЗаполнено(Финансирование.СтатьяФинансирования) Тогда
			ОтборФинансирование.Вставить("СтатьяФинансирования", Финансирование.СтатьяФинансирования);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Финансирование.СтатьяРасходов) Тогда
			ОтборФинансирование.Вставить("СтатьяРасходов", Финансирование.СтатьяРасходов);
		КонецЕсли;
		...
		// Колонки УжеУдержаноНДФЛ: КолонкиОтбораУжеУдержано, Сумма.
		УжеУдержаноНДФЛ = УчетНДФЛ.ЗарегистрированоНДФЛКПеречислениюПоДокументамОснованиям(ТаблицаОтбораУжеУдержаноНДФЛ, ИсключаемыйРегистратор, ДатаОперации);
		Если УжеРаспределено <> Неопределено Тогда
			ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(УжеРаспределено, УжеУдержаноНДФЛ);
			УжеУдержаноНДФЛ.Свернуть(КолонкиОтбораУжеУдержано, "Сумма");
		КонецЕсли;
		УжеУдержаноНДФЛ.Индексы.Добавить(КолонкиОтбораУжеУдержано);
		
	Иначе
		
		КолонкиОтбораБазы = КолонкиОтбораБазы + ",Подразделение,КатегорияДохода";
	...
	Для каждого ФизическоеЛицо Из ФизическиеЛица Цикл
		...
		Для каждого СтрокаПоДокументу Из ДокументыОснования Цикл
			...
			// Строки НДФЛ к удержанию по документу основанию, которые необходимо распределить.
			СтрокиНДФЛ = НалогиКУдержанию.НайтиСтроки(ОтборПоДокументу);
			
			Если ФинансированиеЗадано И СтрокиОснованийВыплат.Количество() > 0 Тогда
				
				// Строки базы НДФЛ, отобранные по документу основанию.
				...
				Для Каждого СтрокаРаспределения Из РаспределениеДокумента Цикл
					...
					Для каждого ЭлементКоллекции Из ОтборФинансирование Цикл
			...
			Иначе
				...
				Для каждого СтрокаНДФЛ Из СтрокиНДФЛ Цикл
					...
					СтрокиРаспределения = БазаРаспределения.НайтиСтроки(ОтборБазы);
	...
	|СтатьяФинансирования,
	|СтатьяРасходов";
	НоваяТаблицаНДФЛ.Свернуть(КолонкиГруппировки, "Сумма");
	НайденныеСтроки = НоваяТаблицаНДФЛ.НайтиСтроки(Новый Структура("Сумма",0));
	Для каждого СтрокаТЗ Из НайденныеСтроки Цикл
...
Функция ТаблицаНДФЛДополненнаяСтатьями(НаборЗаписей, БазаРаспределения, ПоляОтбора, КолонкиСтатей) Экспорт
	...
	Для каждого СтрокаДвижений Из НаборЗаписей Цикл
		...
		ИначеЕсли СтрокиРаспределения.Количество() > 1 Тогда
			...
			Коэффициенты = ОбщегоНазначения.ВыгрузитьКолонку(СтрокиРаспределения, "Сумма");
			// распределяем суммы с учетом ранее удержанного
			РаспределенныеСуммы = ЗарплатаКадры.РаспределитьСуммуПропорциональноБазе(СтрокаДвижений.Сумма, Коэффициенты, 0);
...
Функция НовоеОписаниеПараметровРаспределенияНалогиКУдержанию() Экспорт
	...
	ПараметрыРаспределения.Финансирование = Новый Структура("СтатьяФинансирования,СтатьяРасходов");
	// Параметр определяет, что распределяется НДФЛ сотрудников, иначе - контрагентов.
