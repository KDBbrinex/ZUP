...
Процедура ПровестиПоУчетам(Объект, Отказ) Экспорт
	...
	Если Не РегистрироватьВзаиморасчеты Тогда
		...
		ДанныеДляДополнения.БазаРаспределения 	= ДанныеДляПроведения.НДФЛПоСотрудникам;
		...
		Если Объект.ПеречислениеНДФЛВыполнено Тогда
			...
			ОтражениеЗарплатыВУчете.ДополнитьНДФЛПеречисленныйСведениямиОРаспределенииПоСтатьям(Объект.Движения, ДанныеДляДополнения);
	...
	Для Каждого СтрокаНДФЛ Из Объект.НДФЛ Цикл
		...
		Если ПараметрыРасчета.РасчетНДФЛНарастающимИтогомСНачалаГода Тогда
			...
			Если СтрокаТаблицы <> Неопределено Тогда
				СтрокаТаблицы.НДФЛ = СтрокаТаблицы.НДФЛ + СтрокаНДФЛ.Налог;
				СтрокаТаблицы.ЗачтеноАвансовыхПлатежей = СтрокаТаблицы.ЗачтеноАвансовыхПлатежей + СтрокаНДФЛ.ЗачтеноАвансовыхПлатежей;
			КонецЕсли;
		КонецЕсли;
...
Процедура РаспределитьНДФЛПоСтатьям(Объект, ПараметрыРасчета)
	...
	Для Каждого СтрокаНДФЛ Из ПараметрыРасчета.РезультатРасчета.НДФЛ Цикл
		...
		СтрокаНДФЛ.РаспределениеПоСтатьям = ТаблицаРаспределениеРезультатовУдержаний.Скопировать();
		...
		Для Каждого СтрокаРаспределения Из СтрокаНачислений.РаспределениеПоСтатьям Цикл
			СтрокаРаспределенияНДФЛ = СтрокаНДФЛ.РаспределениеПоСтатьям.Добавить();
		...
		Если СтрокаНДФЛ.РаспределениеПоСтатьям.Количество() > 0 Тогда
			ЗарплатаКадры.РазнестиСуммуПоБазе(СтрокаНДФЛ.Налог, СтрокаНДФЛ.РаспределениеПоСтатьям, "Результат", 0);
		КонецЕсли;
...
#КонецОбласти
	...
	Для Каждого СтрокаРезультатаРасчетаНДФЛ Из РезультатРасчетаНДФЛ.НДФЛ Цикл
		...
		Если СтрокаНачислений <> Неопределено Тогда
			СтрокаНачислений.НДФЛ = СтрокаНачислений.НДФЛ + СтрокаРезультатаРасчетаНДФЛ.Налог;
			СтрокаНачислений.ЗачтеноАвансовыхПлатежей = СтрокаНачислений.ЗачтеноАвансовыхПлатежей + СтрокаРезультатаРасчетаНДФЛ.ЗачтеноАвансовыхПлатежей;
		КонецЕсли;
...
Процедура РассчитатьУдержания(Объект, ПараметрыРасчета)
	...
	НДФЛДляПроведения.Колонки.Добавить("НачислениеУдержание", Новый ОписаниеТипов("ПеречислениеСсылка.ВидыОсобыхНачисленийИУдержаний"));
			...
			Если СтрокиНДФЛ <> Неопределено Тогда
				Для Каждого СтрокаНДФЛ Из СтрокиНДФЛ Цикл
					Если СтрокаНДФЛ.РаспределениеПоСтатьям.Количество() = 0 Тогда
						...
						НоваяСтрокаНДФЛ.НачислениеУдержание = Перечисления.ВидыОсобыхНачисленийИУдержаний.НДФЛДоходыКонтрагентов;
					Иначе
...
Функция ДанныеДляПроведения(Объект)
	...
	Если ПараметрыРасчета.РасчетНДФЛНарастающимИтогомСНачалаГода Тогда
		...
		// Добавим поля для учета НДФЛ контрагентов
		...
		Если Объект.РаспределятьРезультатыРасчета Тогда
			Для Каждого СтрокаНДФЛ Из ДанныеДляПроведения.НДФЛПоСотрудникам Цикл
				СтрокаНДФЛ.Сумма = СтрокаНДФЛ.НалогПоСтавке13;
				СтрокаНДФЛ.НачислениеУдержание = ВидНДФЛ;
			КонецЦикла;
		Иначе
			...
			Для Каждого СтрокаНДФЛ Из ДанныеДляПроведения.НДФЛ Цикл
				...
				Если СтрокаНачислений = Неопределено Тогда
					...
					СтрокаНДФЛСотрудника.НачислениеУдержание = ВидНДФЛ;
				...
				Если РаспределениеПоСтатьям.Количество() > 0 Тогда
					...
					Для Каждого СтрокаРаспределения Из РаспределениеПоСтатьям Цикл
						...
						СтрокаНДФЛСотрудника.НалогПоСтавке13 = СтрокаРаспределения.Результат;
	...
	Иначе
		...
		Для Каждого СтрокаНачислений Из ПараметрыРасчета.РезультатРасчета.НачисленияУдержанияВзносы Цикл
			...
			НоваяСтрокаНДФЛ.КатегорияДохода = Перечисления.КатегорииДоходовНДФЛ.ПрочиеДоходы;
			...
			Если СтрокаНачислений.РаспределениеПоСтатьям <> Неопределено И СтрокаНачислений.РаспределениеПоСтатьям.Количество() > 0 Тогда 
				...
				Для Каждого СтрокаРаспределения Из РаспределениеДоходов Цикл
					...
					КонецЕсли;
					НоваяСтрокаНДФЛСотрудников.НачислениеУдержание = ВидНДФЛ;
