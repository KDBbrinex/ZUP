...
Функция ДанныеСотрудников(ПараметрыПодключения, МассивОтбора)
	...
	Если Не ВедетсяШтатноеРасписание Тогда
		КадровыеДанные = КадровыеДанные + ",Подразделение";
	КонецЕсли;
...
Функция СпособыВыплатыЗарплатыПоРегистратору(РегистраторыВыплаты)

	Возврат КабинетСотрудникаВнутренний.СпособыВыплатыЗарплатыПоРегистратору(РегистраторыВыплаты);
...
Функция ДанныеРасчетныхЛистов(Организация, МесяцРасчетныхЛистов, СписокФизическихЛиц)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Сотрудники", ОбщегоНазначения.ВыгрузитьКолонку(ДанныеРасчетныхЛистов, "Сотрудник", Истина));
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ПубликуемыеОбъектыКабинетСотрудника.ОбъектПубликации В(&Сотрудники)";
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	ПубликуемыеСотрудники = Новый Соответствие;
	ФизическиеЛицаСотрудники = Новый Соответствие;
	Пока Выборка.Следующий() Цикл
		ПубликуемыеСотрудники.Вставить(Выборка.Сотрудник, Истина);
		ФизическиеЛицаСотрудники.Вставить(Выборка.ФизическоеЛицо, Выборка.Сотрудник);
	КонецЦикла;
	...
	СпособыВыплатыЗарплатыПоРегистратору = СпособыВыплатыЗарплатыПоРегистратору(РегистраторыВыплаты);
	...
	Для Каждого СтрокаДанных Из ДанныеРасчетныхЛистов Цикл
		Если Не ЗначениеЗаполнено(СтрокаДанных.ВидРасчета) И Не ЗначениеЗаполнено(СтрокаДанных.Группа) Тогда
			...
			Продолжить;
		КонецЕсли;
		ОбработатьСтрокуДанных(СтрокаДанных, НачисленияТарифнойСтавки, СпособыВыплатыЗарплатыПоРегистратору);
		Если ПубликуемыеСотрудники[СтрокаДанных.Сотрудник] = Неопределено Тогда
			СтрокаДанных.Сотрудник = ФизическиеЛицаСотрудники[СтрокаДанных.ФизическоеЛицо];
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбработатьСтрокуДанных(СтрокаДанных, НачисленияТарифнойСтавки, СпособыВыплатыЗарплатыПоРегистратору)
	...
	ИначеЕсли СтрокаДанных.Группа = ПредопределенноеЗначение("Перечисление.ГруппыНачисленияУдержанияВыплаты.Выплачено") Тогда
		...
		СтрокаДанных.specificSalaryComponent = "";
		Если СпособыВыплатыЗарплатыПоРегистратору[СтрокаДанных.РегистраторВыплаты] ="НаКарту" Тогда
		...
		ИначеЕсли СпособыВыплатыЗарплатыПоРегистратору[СтрокаДанных.РегистраторВыплаты] ="НаСчет" Тогда
			СтрокаДанных.specificSalaryComponent = "bankPayment";
...
Функция ПрименяемыеВычеты(СписокФизическихЛиц, ДатаАктуальности)
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	|	ИмущественныеВычетыНДФЛ.ПрименятьВычетыС КАК Период,
...
Процедура ОбработатьЗапросыСправокСРаботы(ПараметрыПодключения, МассивОбъектов)
	...
	ПараметрыПолучения.СписокФизическихЛиц = ОбщегоНазначения.ВыгрузитьКолонку(ТаблицаДанных, "ФизическоеЛицо", Истина);
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Попытка
			...
			Если ЗаявкаКабинета <> Неопределено Тогда
				...
				КонецЦикла;
				ЗаявкаКабинета.ФайлОтвета = ФайлСправкиСМестаРаботы(СтрокаТаблицы.ФизическоеЛицо, СотрудникиФизическогоЛица);
				ЗаявкаКабинета.Записать();
				ЗаявкаКабинета.Старт();
			КонецЕсли;
			ЗафиксироватьТранзакцию();
			...
			Если ЗаявкаКабинета <> Неопределено Тогда
				...
				Если СтрокаТаблицы.ИзменитьДокумент Тогда
					Если ЗначениеЗаполнено(СтрокаТаблицы.ДокументВид) Тогда
						Описание = Описание + Символы.ПС + СтрШаблон(НСтр("ru = 'Вид документа: %1'"), ВидыДокументовСервиса(СтрокаТаблицы.ДокументВид));
					КонецЕсли;
		...
		Попытка
			...
			Если ЗаявкаКабинета <> Неопределено Тогда
				...
				Если СтрокаТаблицы.ЭтоЛичныйВычет Тогда
					ШаблонОписанияВычета = ШаблонОписанияЛичный;
				ИначеЕсли СтрокаТаблицы.ВычетыНаДетей.Количество() > 0 Тогда
					ШаблонОписанияВычета = ШаблонОписанияНаДетей;
				Иначе
					ШаблонОписанияВычета = ШаблонОписания;
				КонецЕсли;
				ОписаниеЗадачи = СтрШаблон(ШаблонОписанияВычета,
										Строка(СтрокаТаблицы.ФизическоеЛицо),
										Строка(СтрокаТаблицы.Организация),
										Строка(МесяцПрименения),
										Строка(СтрокаТаблицы.Комментарий));
		...
		Попытка
			...
			Если ЗаявкаКабинета <> Неопределено Тогда
				...
				Если СтрокаТаблицы.ВесьДень Тогда
					ДатаНачала 		= Формат(СтрокаТаблицы.ДатаНачала, "ДФ=dd.MM.yyyy");
					ДатаОкончания 	= Формат(СтрокаТаблицы.ДатаОкончания, "ДФ=dd.MM.yyyy");
				...
				Иначе
					Если ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.Опоздание Тогда
						ДатаСобытия   = Формат(СтрокаТаблицы.ДатаОкончания, "ДФ=dd.MM.yyyy");
					...
					ИначеЕсли ПричинаОтсутствия = Перечисления.ПричиныОтсутствийЗаявокКабинетСотрудника.ЛичныеДела Тогда
						ДатаСобытия   = Формат(СтрокаТаблицы.ДатаНачала, "ДФ=dd.MM.yyyy");
	...
	Для каждого СтрокаТЗ Из КадровыеДанные Цикл
		...
		Иначе	
			...
			Если СтрокаТЗ.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Прием
					...
					Или СтрокаТЗ.ВидСобытия = Перечисления.ВидыКадровыхСобытий.ВосстановлениеВДолжности
					Или СтрокаТЗ.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение Тогда
				// если это не увольнение, добавим в список актуальных рабочих мест
				ЗаполнитьЗначенияСвойств(КадровыеДанныеДругихСотрудников.Добавить(), СтрокаТЗ);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ОбменССервисомКабинетСотрудника() Экспорт
	...
	Если Не Настройки.ТестовыйТарифАктивирован Тогда
		Если Не ОбщегоНазначения.РазделениеВключено() Тогда
			КабинетСотрудникаМенеджерСервиса.АктивироватьТестовыйТариф();
		КонецЕсли;
	ИначеЕсли Не Настройки.СообщениеОбАктивацииТестовогоТарифаОтправлено Тогда
