...
//
// Параметры:
//  Форма                                    - ФормаКлиентскогоПриложения - форма, где произошло нажатие на гиперссылку;
...
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма "ПанельАдминистрированияБСП".
//
...
Процедура ПроверитьНеобходимостьВыходаДляСозданияРезервнойКопииВОблачныйАрхивПослеВыбора(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если ВыполнитьРезервноеКопирование Тогда
		// АПК:278-выкл Завершение работы системы для выполнения резервного копирования.
			...
			Если ИнформацияОКлиенте.ЭтоАдминистраторWindows = Истина Тогда
				Если НЕ ПустаяСтрока(лкКаталогУстановкиАгентаКопирования) Тогда
					...
					Попытка
						...
						Процесс         = ОбъектОболочка.Exec(СтрокаКоманды); // АПК:534 Прямой запуск приложения для чтения консоли.
...
// Работает асинхронно. После выполнения удаляет все временные файлы.
//
// Параметры:
//  СтрокаКоманды           - Строка - командная строка;
//  ТекущийКаталог          - Строка - каталог запуска;
//  ДополнительныеПараметры - Структура - дополнительные параметры с ключами:
//    * УдалитьФайлы - Массив из Строка - массив имен файлов для удаления.
...
//
Процедура ВыполнитьСтрокуКомандыАсинхронно(СтрокаКоманды, ТекущийКаталог, ДополнительныеПараметры, КонтекстВыполнения = Неопределено) Экспорт
...
#Область ЗапускПриложения
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхивКлиент.ВыполнитьСтрокуКомандыАсинхронно/ЗапускКомандногоФайла", // Идентификатор.
		НСтр("ru='ОблачныйАрхивКлиент.Выполнение командной строки (асинхронно) / Запуск командного файла'"));
		...
		ИмяКомандногоФайла = ПолучитьИмяВременногоФайла("cmd"); // АПК:441 Отсутствует удаление файла.
		ИмяФайлаЛога       = ПолучитьИмяВременногоФайла("stdout.log"); // АПК:441 Отсутствует удаление файла.
		ИмяФайлаОшибок     = ПолучитьИмяВременногоФайла("stderr.err"); // АПК:441 Отсутствует удаление файла.
		...
		ДополнительныеПараметры.УдалитьФайлы.Добавить(ИмяКомандногоФайла);
		ДополнительныеПараметры.УдалитьФайлы.Добавить(ИмяФайлаЛога);
		ДополнительныеПараметры.УдалитьФайлы.Добавить(ИмяФайлаОшибок);

		ДополнительныеПараметры.Вставить("ИмяФайлаЛога"  , ИмяФайлаЛога);
		ДополнительныеПараметры.Вставить("ИмяФайлаОшибок", ИмяФайлаОшибок);

		ОписаниеОповещения = Новый ОписаниеОповещения("ПослеВыполненияСтрокиКомандыАсинхронно", ОблачныйАрхивКлиент, ДополнительныеПараметры);
		...
		ФайловаяСистемаКлиент.ЗапуститьПрограмму(ИмяКомандногоФайла, ПараметрыЗапускаПрограммы);

	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru='Запуск командного файла.
			|Строка команды: %1
			|Код возврата: неизвестен для асинхронного запуска
			|Содержимое командного файла:
			|%2
			|Поток вывода: неизвестен для асинхронного запуска
			|Поток ошибок: неизвестен для асинхронного запуска'"),
		СтрокаКоманды,
		Результат.ТекстКомандногоФайла);
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗарегистрироватьРезультатВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхивКлиент.ВыполнитьСтрокуКомандыАсинхронно/ЗапускВВебКлиенте", // Идентификатор.
		НСтр("ru='ОблачныйАрхивКлиент.Выполнение командной строки / Запуск в веб-клиенте'"),
		0,
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ИнтернетПоддержкаПользователейВызовСервера.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Работа с внешними утилитами'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Облачный архив. Работа с внешними утилитами. Запуск произвольной команды'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()), // ИдентификаторШага.
...
// Вызывается асинхронно после ВыполнитьСтрокуКомандыАсинхронно.
//
// Параметры:
//  Результат               - Структура - в этот параметр будут возвращены результаты выполнения программы. Ключи:
//   * ПотокВывода        - Строка;
//   * ПотокОшибок        - Строка;
//   * ПриложениеЗапущено - Булево;
//   * ОписаниеОшибки     - Строка;
//   * КодВозврата        - Число;
//  ДополнительныеПараметры - Структура - дополнительные параметры с ключами:
//    * УдалитьФайлы   - Массив из Строка - массив имен файлов для удаления;
//    * ИмяФайлаЛога   - Строка - имя файла с потоком сообщений;
//    * ИмяФайлаОшибок - Строка - имя файла с потоком ошибок.
//
Процедура ПослеВыполненияСтрокиКомандыАсинхронно(Результат, ДополнительныеПараметры) Экспорт
...
#Область ЧтениеПотоков

	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхивКлиент.ПослеВыполненияСтрокиКомандыАсинхронно/ЧтениеПотоков", // Идентификатор.
		НСтр("ru='ОблачныйАрхивКлиент.ПослеВыполненияСтрокиКомандыАсинхронно / Чтение потоков'"));

	Попытка
		ЧтениеТекста = Новый ЧтениеТекста(ДополнительныеПараметры.ИмяФайлаЛога, , , , Ложь);
	...
	Попытка
		ЧтениеТекста = Новый ЧтениеТекста(ДополнительныеПараметры.ИмяФайлаОшибок, , , , Ложь);
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ПотокВывода,
			ДополнительныеПараметры.ИмяФайлаОшибок,
			ПотокОшибок),
		Неопределено);
...
#Область УдалениеВременныхФайлов
	...
	ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		КонтекстВыполнения,
		"ОблачныйАрхивКлиент.ПослеВыполненияСтрокиКомандыАсинхронно/УдалитьВременныеФайлы", // Идентификатор.
		НСтр("ru='ОблачныйАрхивКлиент.ПослеВыполненияСтрокиКомандыАсинхронно / Удаление временных файлов'"));

		ПодробныеСообщения = Новый Массив;
		Для Каждого ТекущееИмяФайла Из ДополнительныеПараметры.УдалитьФайлы Цикл
			Попытка
				УдалитьФайлы(ТекущееИмяФайла);
				ПодробныеСообщения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='Удаление временного файла [%1] прошло успешно'"),
						ТекущееИмяФайла));
			Исключение
				ИнформацияОбОшибке = ИнформацияОбОшибке();
				ПодробныеСообщения.Добавить(
					СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru='При удалении файла [%1] произошла ошибка:
							|%2'"),
						ТекущееИмяФайла,
						ПодробноеПредставлениеОшибки(ИнформацияОбОшибке)));
			КонецПопытки;
	...
	ИнтернетПоддержкаПользователейКлиентСервер.ЗавершитьРегистрациюРезультатаВыполненияОперации(
		...
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Удаление временных файлов.
				|Подробности:
				|%1'"),
			СтрСоединить(ПодробныеСообщения, Символы.ПС)),
		Неопределено);
