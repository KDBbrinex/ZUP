...
// Процедура удаляет предельные суммы удержаний для строк перерасчетов.
//
// Параметры
...
//
Процедура УдалитьПредельныеСуммыПерерассчитываемыхУдержаний(ПредельныеСуммыУдержаний, МенеджерРасчетаЗарплаты) Экспорт
	...
	ОбщегоНазначенияБЗК.ДобавитьИндексКоллекции(ПредельныеСуммыУдержаний, "Организация, ФизическоеЛицо, Удержание, ДокументОснование");
	СтруктураСтроки = Новый Структура("Организация, ФизическоеЛицо, Удержание, ДокументОснование");
	
	Для Каждого СтрокаПерерасчетовУдержаний Из ТаблицаПерерассчитываемыхУдержаний Цикл
		ЗаполнитьЗначенияСвойств(СтруктураСтроки, СтрокаПерерасчетовУдержаний);
		СтруктураСтроки.Организация = СтрокаПерерасчетовУдержаний.ГоловнаяОрганизация;
		СтрокиПредельныхСумм = ПредельныеСуммыУдержаний.НайтиСтроки(СтруктураСтроки);
		Для Каждого СтрокаПредельнойСуммы Из СтрокиПредельныхСумм Цикл
			ПредельныеСуммыУдержаний.Удалить(СтрокаПредельнойСуммы);
		КонецЦикла;
	КонецЦикла;
...
Процедура СвернутьНачисленияПерерасчеты(НачисленияПерерасчет) Экспорт
	...
	КлючТекущейЗаписи = Новый Структура("Сотрудник,Подразделение,Начисление,ДокументОснование,КодВычета,ПериодДействия,ГрафикРаботы");
...
Процедура ЗаполнитьДанныеПерерасчетаНачислений(ДанныеПерерасчета, Организация, Подразделение = Неопределено, МесяцНачисления, Сотрудники, СотрудникиПериодДействияПерерасчет, ИсключаемыеРегистраторы, РежимДоначисления, МенеджерРасчета)
	...
	Если ЗарплатаКадры.ВТСодержитСтроки(МенеджерВременныхТаблиц, "ВТСторнируемыеРегистраторы") Тогда
		...
		ЗаполнитьСторнированиеПособий(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели);
	КонецЕсли;
...
Процедура ЗаполнитьСторнированиеПособий(МенеджерВременныхТаблиц, Организация, ДанныеПерерасчета, КатегорииНачисленийПособий, РаспределенияТерриторииПоказатели)
