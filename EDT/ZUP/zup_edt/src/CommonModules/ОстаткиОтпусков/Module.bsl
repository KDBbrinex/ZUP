...
Функция СправкаПоОтпускамСотрудника(Знач СписокСотрудников, Знач ДатаОстатков = '00010101', Знач ЭтоРасчетПриУвольнении = Ложь) Экспорт
	...
	Для Каждого Результат Из ТаблицаСобытий Цикл
		...
		Для каждого ВидОтпуска Из ВидыОтпусковСотрудника Цикл
			...
			ОбластьМакета.Параметры.КоличествоДней = ?(СтрокаДней= Неопределено, "0,00", Формат(СтрокаДней.КоличествоДней,ФорматЧисла));
			ТабДокумент.Присоединить(ОбластьМакета);
		КонецЦикла;
...
Функция ДоговорыСотрудниковСОтпускомВРабочихДнях(ТаблицаСотрудников) Экспорт 
	...
	Если ТребуютсяКадровыеДанные Тогда
		...
		КадровыйУчет.СоздатьВТКадровыеДанныеСотрудников(ОписательВТ, Ложь, "ЯвляетсяСудьей");
	...
	Для Каждого СтрокаРезультирующейТаблицы Из ДоговорыСотрудниковСОтпускомВРабочихДнях Цикл
		...
		Если КадровыеДанные <> Неопределено Тогда
			ЯвляетсяСудьей = КадровыеДанные.НайтиСтроки(СтруктураПоиска)[0].ЯвляетсяСудьей;
		КонецЕсли;	
...
Функция СпособыРасчетаОтпусковСотрудников(Сотрудники, ДатаАктуальности) Экспорт
	...
	КадровыеДанныеСотрудников = КадровыйУчет.КадровыеДанныеСотрудников(Истина, Сотрудники,
		"ДатаПриема,ДатаОкончания,СрочныйДоговор,СезонныйДоговор,КраткосрочныйТрудовойДоговор,ЯвляетсяСудьей", ДатаАктуальности);
	...
	Для Каждого КадровыеДанныеСотрудника Из КадровыеДанныеСотрудников Цикл
		...
		СпособРасчета = ?(КраткосрочныйТД Или СезонныйТД Или КадровыеДанныеСотрудника.ЯвляетсяСудьей, Перечисления.СпособыРасчетаНачислений.ОплатаОтпускаПоШестидневке, Перечисления.СпособыРасчетаНачислений.ОплатаОтпускаПоКалендарнымДням);
	...
	Для Каждого СотрудникБезПрава Из ИсходныеДанные.СотрудникиБезПраваНаОтпуск Цикл
		...
		Для Каждого СтрокаВидаОтпуска Из ТаблицаВидовОтпуска Цикл
			...
			Если ПраваОтпускаВРегистре.Количество() = 0 Тогда
				...
				ПерерывыПредоставленияРаздвигаютГод = Не (СтрокаВидаОтпуска.ВидЕжегодногоОтпуска = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.ОтпускЗаВредность")
					Или СтрокаВидаОтпуска.ВидЕжегодногоОтпуска = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыОтпусков.Северный"));
