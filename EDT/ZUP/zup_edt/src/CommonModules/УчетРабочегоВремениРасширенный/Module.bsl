...
Функция СоздатьМенеджерДанныхПоНаборуСотрудникиПериоды(МенеджерВременныхТаблиц, ДатаАктуальности, ОписаниеНабораСотрудников) Экспорт
	...
	Если ЗарплатаКадры.ВыполнятьРасчетЗарплатыБезОптимизации() Тогда
		МенеджерДанных = Обработки.МенеджерДанныхУчетаВремениСотрудниковАрхивный.Создать();
	Иначе
...
//  ОбработкаОбъект.МенеджерДанныхУчетаВремениСотрудников
//
Функция СоздатьМенеджерДанных(МенеджерВременныхТаблиц, ДатаАктуальности, УчитыватьНастройкуОптимизации = Истина) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	Если ЗарплатаКадры.ВыполнятьРасчетЗарплатыБезОптимизации() 
		И УчитыватьНастройкуОптимизации Тогда
...
Функция ОплачиваемыеКоронавирусныеПериоды(МенеджерВременныхТаблиц) Экспорт
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Сотрудники.Сотрудник КАК Сотрудник,
	|	НАЧАЛОПЕРИОДА(Сотрудники.Месяц, МЕСЯЦ) КАК ДатаНачала,
	|	КОНЕЦПЕРИОДА(Сотрудники.Месяц, МЕСЯЦ) КАК ДатаОкончания
...
//			
Процедура ЗаписатьДанныеГрафика(ГрафикРаботы, ДанныеГрафика, НомерТекущегоГода, СохранитьСреднемесячныеНормыВремени = Истина, СохранитьАвтоОбновлениеГрафика = Истина) Экспорт
	МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
...
Процедура ЗаполнитьГрафикиСотрудниковЗаГод(ДатаОбновления) Экспорт
	...
	Пока ВыборкаГрафиков.Следующий() Цикл
		Попытка
			...
			ЗаписатьДанныеГрафика(ВыборкаГрафиков.Ссылка, ДанныеГрафика, Год(ДатаОбновления));
		Исключение
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Функция ЗаполнитьОчереднойНабор(МенеджерВременныхТаблиц, Знач ПоследнийСотрудник = Неопределено) Экспорт 
	...
	Если КоличествоЗаписей = 45000 Тогда
		...
		ТекущийСотрудник = ДанныеДляЗаполнения[ИндексСтрокиТаблицы].ГрафикРаботы;
		Пока ИндексСтрокиТаблицы > 0 Цикл
			...
			Если СтрокаТаблицы.ГрафикРаботы = ТекущийСотрудник Тогда
				ДанныеДляЗаполнения.Удалить(ИндексСтрокиТаблицы);
			Иначе
				Прервать;
			КонецЕсли;
	...
	// установим блокировку в разрезе сотрудников, месяцев и периодов регистрации
