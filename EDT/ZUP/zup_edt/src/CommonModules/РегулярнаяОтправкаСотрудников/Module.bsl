...
Процедура ПриОпределенииНастроекРегламентныхЗаданий(Зависимости) Экспорт
	...
	Зависимость.РегламентноеЗадание = Метаданные.РегламентныеЗадания.ОтправкаСотрудниковБронированияКомандировок;
	Зависимость.ФункциональнаяОпция = Метаданные.ФункциональныеОпции.ИспользоватьАвтоматическуюОтправкуСотрудниковБронированияКомандировок;
...
Процедура ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны) Экспорт
	...
	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ОтправкаСотрудниковБронированияКомандировок.Имя);
...
Процедура ОтправитьСотрудниковВСистемуБронированияДлительнаяОперация(Параметры, АдресРезультата) Экспорт
	...
	ПоместитьВоВременноеХранилище(ПараметрыОтправкиСотрудников, АдресРезультата);
...
Процедура ЗарегистрироватьИзменениеКадровойИсторииДляБронированияКомандировок(Источник, Отказ, Замещение) Экспорт
	...
	Если ИспользуетсяОтборПоПодразделениям <> Неопределено Тогда
...
Процедура ПроверитьИзмененияПодразделенийИЗарегистрироватьНеобходимостьОтправки(СистемаБронирования, Источник, ИспользуетсяОтборПоПодразделениям)
	...
	Если ИспользуетсяОтборПоПодразделениям Тогда
		...
		ТаблицаОтправляемыхСотрудников = СотрудникиБронированияКомандировок(СистемаБронирования,
			...
			Если СтрокаИзменения.Добавление Тогда
				...
				Если СтрокаИзменения.НовоеЗначениеВидСобытия = Перечисления.ВидыКадровыхСобытий.Прием Тогда
					...
					Если НайденноеПодразделение <> Неопределено Тогда
						ДобавитьЭлементВСписок(МассивСотрудниковДляДобавления, ФизическоеЛицо);
					КонецЕсли;
				...
				ИначеЕсли СтрокаИзменения.НовоеЗначениеВидСобытия = Перечисления.ВидыКадровыхСобытий.Увольнение Тогда
					...
					Если СотрудникВНастройках <> Неопределено Тогда
						Если СотрудникВНастройках.Отправляется Тогда
							ДобавитьЭлементВСписок(МассивСотрудниковДляУдаления, ФизическоеЛицо);
						КонецЕсли;
					КонецЕсли;
...
Функция ПодготовитьМассивыИзмененныхСотрудниковОтправляемыхПоУсловию(НовыеНастройки)
	...
	НовыеПоУсловию = ТаблицаФизическихЛиц.ВыгрузитьКолонку("ФизическоеЛицо");
...
Процедура ЗаполнитьДанныеОтправляемыхСотрудников(СистемаБронирования, ДанныеФизическиЛиц, ТаблицаФизическихЛиц)
	...
	Для Каждого ЭлементФизическоеЛицо Из МассивФизическихЛиц Цикл
		...
		Если МестаРаботы[0].ДокументВид = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ Тогда

			ПаспортРФСерияНомер = СерияНомерДокументаСтрокой(МестаРаботы[0].ДокументСерия, МестаРаботы[0].ДокументНомер);

			ПаспортРФ = НовоеФизическоеЛицо.Документы.Добавить();
			ПаспортРФ.ФизическоеЛицо = ЭлементФизическоеЛицо;
			ПаспортРФ.ВидДокумента = МестаРаботы[0].ДокументВид;
			ПаспортРФ.НомерДокумента = ПаспортРФСерияНомер;
			ПаспортРФ.СрокДействияДокумента = МестаРаботы[0].ДокументСрокДействия;
...
Процедура ОтправитьСотрудниковБронированияКомандировокСписком(ПараметрыОтправки)
	...
	ДанныеФизическиЛиц = ОписаниеТаблицыОтправляемыхФизическихЛиц();
	...
	ЗаполнитьДанныеОтправляемыхСотрудников(СистемаБронирования, ДанныеФизическиЛиц, ТаблицаОтправляемыхФизическихЛиц);

	Попытка
		ИнтеграцияSmartway.ОтправитьСотрудников(СистемаБронирования, ДанныеФизическиЛиц, ПараметрыОтправки);
...
Процедура ДобавитьЭлементВСписок(Список, Элемент)
...
Процедура ПропускатьПроверкуРегулярнойОтправки(Форма, ДополнительныеСвойства) Экспорт
	...
	Если Форма.ДокументыФизическихЛиц.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ПаспортРФ Или
		Форма.ДокументыФизическихЛиц.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.ЗагранпаспортРФ Тогда
