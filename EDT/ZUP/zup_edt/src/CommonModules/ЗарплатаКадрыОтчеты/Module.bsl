...
Функция ВывестиТелоРасчетногоЛистка(ДокументРезультат, Макет, СтрокиГоловныхСотрудников, СоответствиеПользовательскихПолей, ДанныеПечатнойФормы, ДополнительныеПараметры)
		...
		Для каждого СтрокаСотрудника Из СтрокаГоловногоСотрудника.Строки Цикл
			...
			Если РабочееМесто.СтрокиНачислений.Количество() > 0 Тогда
				...
				Если СтрокаСотрудника.Сотрудник <> СтрокаСотрудника.ГоловнойСотрудник
					...
					СтрокаОтчетаПредставлениеРабочегоМеста = СтрокаОтчета.ПредставлениеРабочегоМеста;
			...
			Если СтрокаГруппы <> Неопределено Тогда
				...
				Для Каждого СтрокаВидаРасчета Из КоллекцииРазделов.Удержано Цикл
					...
					СтрокаДанныхРасчетногоЛистка.Сотрудник = СтрокаГруппы.Сотрудник;
				...
				Для каждого СтрокаВыплачено Из СтрокаГруппы.Строки Цикл
					...
					Для каждого СтрокаСРегистратором Из СтрокаВыплачено.Строки Цикл
						...
						Если СтрокаКоллекции = Неопределено Тогда
							...
							СтрокаДанныхРасчетногоЛистка = ДанныеРасчетныхЛистков.Добавить();
							ЗаполнитьЗначенияСвойств(СтрокаДанныхРасчетногоЛистка, СтрокаСРегистратором);
							СтрокаДанныхРасчетногоЛистка.РегистраторВыплаты = СтрокаСРегистратором.Регистратор;
							...
							Если СвойстваРегистратора <> Неопределено Тогда
								...
								Если НастройкиВывода.ДетализироватьВыплатыПоВедомостям Тогда
									...
									СтрокаКоллекции.ВидРасчета = ПредставлениеРегистраторовВзаиморасчетов(
										СтрокаСРегистратором.Регистратор, СтрокаСРегистратором.ВидРасчета, СвойстваРегистратора);
									
								Иначе
									СтрокаКоллекции.ВидРасчета = ПредставлениеВыплаты(СтрокаСРегистратором.ВидРасчета, СвойстваРегистратора)
								КонецЕсли;
			...
			Если СтрокаГруппы <> Неопределено Тогда
				...
				СтрокаДанныхРасчетногоЛистка.Сумма = ИтогиПоГруппам.СуммаДолгаНаНачалоМесяца;
			КонецЕсли;
			...
			Если СтрокаГруппы <> Неопределено Тогда
				...
				СтрокаДанныхРасчетногоЛистка = ДанныеРасчетныхЛистков.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаДанныхРасчетногоЛистка, СтрокаГруппы);
				СтрокаДанныхРасчетногоЛистка.Сумма = ИтогиПоГруппам.СуммаДолгаНаКонецМесяца;
			КонецЕсли;
	...
	СворачиваемыеКолонки = "ФизическоеЛицо,Группа,Сотрудник,ВидРасчета,ПриоритетВидаРасчета";
	Если НастройкиВывода.ДетализироватьВыплатыПоВедомостям Тогда
	...
	Для Каждого СтрокаОтчетаНачислено Из СтрокиОтчетаНачислено Цикл
		...
		СтрокаДанныхРасчетногоЛистка.ПредставлениеРабочегоМеста = СтрокаОтчетаПредставлениеРабочегоМеста;
		СтрокаДанныхРасчетногоЛистка.ПриоритетВидаРасчета =
			ПриоритетВидаРасчетаДанныхРасчетныхЛистков(СтрокаДанныхРасчетногоЛистка.ПриоритетВидаРасчета);
	...
	Если КоллекцииРазделов.Льготы.Количество() > 0 Тогда
		...
		ВывестиТелоСправочнойИнформации(ДокументРезультат, Макет, КоллекцииРазделов.Льготы, ДанныеРасчетныхЛистков,
	...
	Если КоллекцииРазделов.Справочно.Количество() > 0 Тогда
		...
		ВывестиТелоСправочнойИнформации(ДокументРезультат, Макет, КоллекцииРазделов.Справочно, ДанныеРасчетныхЛистков,
			ДополненныеПараметры(ДанныеПечатнойФормы, ЗначенияЗаполненияПолейГоловногоСотрудника));
...
Функция ПредставлениеРегистраторовВзаиморасчетов(Регистратор, ВидВзаиморасчетов, СвойстваРегистратора)
	...
	ПредставлениеРегистратора = ПредставлениеВыплаты(ВидВзаиморасчетов, СвойстваРегистратора);
	
	НомерДата = "№ " + ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(СвойстваРегистратора.Номер, Истина, Истина)
		+ " "  + НСтр("ru='от'") + " "
	...
	Если ТипЗнч(Регистратор) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВБанк") Тогда
		РегистраторСтрокой = НСтр("ru='Банк'") + ", " + НСтр("ru = 'вед'") + ". " + НомерДата;
	ИначеЕсли ТипЗнч(Регистратор) = Тип("ДокументСсылка.ВедомостьНаВыплатуЗарплатыВКассу") Тогда
		РегистраторСтрокой = НСтр("ru='Касса'") + ", "  + НСтр("ru = 'вед'") + ". " + НомерДата;
	ИначеЕсли СтрНайти(Строка(ТипЗнч(Регистратор)), ".Ведомость") Тогда
		РегистраторСтрокой = НСтр("ru='Ведомость'") + " " + НомерДата;
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(Регистратор)) Тогда
		РегистраторСтрокой = ОбщегоНазначения.ПредставлениеОбъекта(Регистратор.Метаданные()) + " " + НомерДата;
	Иначе
		РегистраторСтрокой = Строка(Регистратор);
	КонецЕсли;
	
	Возврат ПредставлениеРегистратора + " (" + РегистраторСтрокой + ")";
...
Процедура ВывестиТелоСправочнойИнформации(ДокументРезультат, Макет, КоллекцияСправочныхДанных, ДанныеРасчетныхЛистков, ДополнительныеПараметры)
	...
	Для каждого СтрокаСправочно Из КоллекцияСправочныхДанных Цикл
		...
		СтрокаДанныхРасчетногоЛистка = ДанныеРасчетныхЛистков.Добавить();
		ЗаполнитьЗначенияСвойств(СтрокаДанныхРасчетногоЛистка, СтрокаСправочно);
