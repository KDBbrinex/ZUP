...
Процедура ПолучитьДанныеВыпискиБанкаТекстовыйФормат(
	...
	Для Каждого СтруктураДанных Из МассивДанных Цикл
		...
		Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
			...
			ДобавитьНеПустойПараметр(ДеревоРазбора, СтрокаТЧ, Текст, "КодНазПлатежа", "КодВидаДохода");
...
Функция ДанныеВыпискиБанкаТаблицаЗначений(ЭлектронныеДокументы) Экспорт
	...
	Для Каждого СообщениеОбмена Из МассивСообщенийОбмена Цикл
		...
		Попытка
			...
			ДанныеЭД = РаботаСФайлами.ДвоичныеДанныеФайла(ПрисоединенныйФайл);
			...
			ПотокВПамяти = ДанныеЭД.ОткрытьПотокДляЧтения();
		...
		ИначеЕсли ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрисоединенныйФайл, "ПодписанЭП") Тогда
...
#Область ЭлектронноеВзаимодействие_Справочники_НастройкиОбменСБанками_ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.ИзменяемыеОбъекты = "Справочник.МаршрутыПодписания," + "Справочник.НастройкиОбменСБанками";
	Обработчик.БлокируемыеОбъекты = "Документ.СообщениеОбменСБанками,"
		+ "Справочник.НастройкиОбменСБанками";
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, ТаблицаОпераций)
	...
	ЭтоОбъявлениеНаВзносНаличными = Ложь;
	ЭтоЧек = Ложь;
	...
	ИначеЕсли ЗначениеСвойстваXDTO(Operation.PayDoc, "CashContribution") <> Неопределено Тогда
		...
		ЭтоОбъявлениеНаВзносНаличными = Истина;
	...
	Если Payer <> Неопределено Тогда
		...
		Если PayerBank <> Неопределено Тогда
			...
			БанкПлательщика.Вставить("БИК", ЗначениеСвойстваXDTO(PayerBank, "BIC"));
	...
	Если Payee <> Неопределено Тогда
		...
		Если PayeeBank <> Неопределено Тогда
			...
			БанкПолучателя.Вставить("БИК", ЗначениеСвойстваXDTO(PayeeBank, "BIC"));
	...
	Если ЭтоМемориальныйОрдер Тогда
		...
		МемориальныйОрдер.Вставить("СодержаниеОперации", СодержаниеОперации);
		Составитель = Новый Структура;
		Составитель.Вставить("КоррСчет", ЗначениеСвойстваXDTO(PayData, "Author.CorrespAcc"));
		Составитель.Вставить("БИК", ЗначениеСвойстваXDTO(PayData, "Author.BIC"));
		Составитель.Вставить("Наименование", ЗначениеСвойстваXDTO(PayData, "Author.Name"));
		Составитель.Вставить("Город", ЗначениеСвойстваXDTO(PayData, "Author.City"));
		МемориальныйОрдер.Вставить("Составитель", Составитель);
		МемориальныйОрдер.Вставить("НаименованиеСчетаДебет", ЗначениеСвойстваXDTO(PayData, "AccountNameDebit"));
		МемориальныйОрдер.Вставить("СчетДебет", ЗначениеСвойстваXDTO(PayData, "AccountDebit"));
		МемориальныйОрдер.Вставить("НаименованиеСчетаКредит", ЗначениеСвойстваXDTO(PayData, "AccountNameCredit"));
		МемориальныйОрдер.Вставить("СчетКредит", ЗначениеСвойстваXDTO(PayData, "AccountCredit"));
		МемориальныйОрдер.Вставить("СвободноеПоле9а", ЗначениеСвойстваXDTO(PayData, "SpareField9a"));
		МемориальныйОрдер.Вставить("СвободноеПоле14", ЗначениеСвойстваXDTO(PayData, "SpareField14"));
		МемориальныйОрдер.Вставить("СвободноеПоле15", ЗначениеСвойстваXDTO(PayData, "SpareField15"));
		МемориальныйОрдер.Вставить("СвободноеПоле20", ЗначениеСвойстваXDTO(PayData, "SpareField20"));
		...
		НовСтрока.НазначениеПлатежа = СодержаниеОперации;
		НовСтрока.МемориальныйОрдер = МемориальныйОрдер;
	КонецЕсли;
	
	Если ЭтоОбъявлениеНаВзносНаличными Тогда
		ОбъявлениеНаВзносНаличными = Новый Структура;
		ОтКого = Новый Структура;
		ОтКого.Вставить("ФИОВносителя", ЗначениеСвойстваXDTO(PayData, "Person.FullName"));
		ОтКого.Вставить("ДокументУдостоверяющийЛичность", ЗначениеСвойстваXDTO(PayData, "Person.IdentityDocument"));
		ОбъявлениеНаВзносНаличными.Вставить("ОтКого", ОтКого);
		ОбъявлениеНаВзносНаличными.Вставить("Символ", ЗначениеСвойстваXDTO(PayData, "Symbol"));
		ОбъявлениеНаВзносНаличными.Вставить("ИсточникПоступления", ЗначениеСвойстваXDTO(PayData, "Source"));
		НовСтрока.ОбъявлениеНаВзносНаличными = ОбъявлениеНаВзносНаличными;
	КонецЕсли;
	
	Если ЭтоЧек Тогда
		ДенежныйЧек = Новый Структура;
		Получатель = Новый Структура;
		Получатель.Вставить("ФИО", ЗначениеСвойстваXDTO(PayData, "Person.FullName"));
		Получатель.Вставить("ДокументУдостоверяющийЛичность", ЗначениеСвойстваXDTO(PayData, "Person.IdentityDocument"));
		ДенежныйЧек.Вставить("Получатель", Получатель);
		ДанныеБумажнойФормы = Новый Структура;
		ДанныеБумажнойФормы.Вставить("Серия", ЗначениеСвойстваXDTO(PayData, "DataPrinting.CheckSeries"));
		ДанныеБумажнойФормы.Вставить("Номер", ЗначениеСвойстваXDTO(PayData, "DataPrinting.CheckNumber"));
		ДенежныйЧек.Вставить("ДанныеБумажнойФормы", ДанныеБумажнойФормы);
		ДеталиДенежногоЧека = Новый ТаблицаЗначений;
		ДеталиДенежногоЧека.Колонки.Добавить("Символ");
		ДеталиДенежногоЧека.Колонки.Добавить("НаправлениеВыдачи");
		ДеталиДенежногоЧека.Колонки.Добавить("Сумма");
		
		Если НЕ ЗначениеСвойстваXDTO(PayData, "Details") = Неопределено Тогда
			Детали = ЗначениеСвойстваXDTO(PayData, "Details");
			Если ТипЗнч(Детали) = Тип("ОбъектXDTO") Тогда
				ПрочитатьДетальЧека(Детали, ДеталиДенежногоЧека);
			Иначе
				Для Каждого Деталь Из Детали Цикл
					ПрочитатьДетальЧека(Деталь, ДеталиДенежногоЧека);
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		
		ДенежныйЧек.Вставить("Детали", ДеталиДенежногоЧека);
		НовСтрока.ДенежныйЧек = ДенежныйЧек;
	...
	Если Stamp <> Неопределено Тогда
		ПараметрыШтампа = Новый Структура;
		ПараметрыШтампа.Вставить("НаименованиеБанка", ЗначениеСвойстваXDTO(Stamp, "Name"));
		ПараметрыШтампа.Вставить("Отделение", ЗначениеСвойстваXDTO(Stamp, "Branch"));
		ПараметрыШтампа.Вставить("БИК", ЗначениеСвойстваXDTO(Stamp, "BIC"));
		ПараметрыШтампа.Вставить("Статус", ЗначениеСвойстваXDTO(Stamp, "Status.Name"));
		ПараметрыШтампа.Вставить("Офис");
		ПараметрыШтампа.Вставить("ДатаОперации", НовСтрока.ДатаОперации);
...
Процедура ПрочитатьДетальЧека(ДетальЧека, ДеталиДенежногоЧека)
	
	НовСтрока = ДеталиДенежногоЧека.Добавить();
	НовСтрока.Символ = ЗначениеСвойстваXDTO(ДетальЧека, "Symbol");
	НовСтрока.НаправлениеВыдачи = ЗначениеСвойстваXDTO(ДетальЧека, "Purpose");
	НовСтрока.Сумма = ЗначениеСвойстваXDTO(ДетальЧека, "Sum")
