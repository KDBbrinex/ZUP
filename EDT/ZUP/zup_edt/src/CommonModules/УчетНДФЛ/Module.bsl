...
Процедура СформироватьУдержанныйНалогПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, УдержанныйНалог, РегистрацияВНалоговомОргане = Неопределено, Записывать = Ложь, ФормироватьРасчетыНалоговыхАгентовСБюджетом = Истина, Регистратор = Неопределено, ТаблицаМежрасчетныхВыплат = Неопределено) Экспорт
	...
	Для каждого СтрокаТЗ Из Результаты Цикл
		...
		КонецЕсли;
		Если ФормироватьРасчетыНалоговыхАгентовСБюджетом И НоваяСтрокаРасчетовНалогоплательщика.Сумма <> 0 Тогда
	...
	Если ФормироватьРасчетыНалоговыхАгентовСБюджетом И СоответствиеСтрок.Количество() > 0 Тогда
		Для каждого Элемент Из СоответствиеСтрок Цикл
			СтрокаРасчетовНалоговыхАгентовСБюджетом(Движения, Организация, ДатаОперации, ВидДвиженияНакопления.Приход, Элемент.Значение, Элемент.Ключ, , Элемент.Ключ.КатегорияДохода);
		КонецЦикла;
		Если Записывать Тогда
			Движения.РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.Записать();
			Движения.РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.Записывать = Ложь;
...
Процедура СформироватьИспользованныеАвансовыеПлатежиПоВременнойТаблице(Движения, Отказ, Организация, ДатаОперации, МенеджерВременныхТаблиц, Записывать = Ложь, ОкончательныйРасчет = Истина) Экспорт
	...
	|ГДЕ
	|	Вычеты.Сумма <> 0";
	Если МенеджерВременныхТаблиц.Таблицы["ВТДанныеОЗачтенныхПлатежах"].Колонки.Найти("КатегорияДохода") = Неопределено Тогда
...
Процедура СформироватьНДФЛКПеречислению(Движения, Отказ, КоллекцияДвижений = Неопределено) Экспорт
	...
	Для каждого СтрокаДвижений Из НайденныеСтроки Цикл
		Если СтрокаДвижений.Сумма = 0 Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СформироватьНДФЛПеречисленный(Движения, Отказ, НалогПеречисленный = Неопределено) Экспорт
	...
	Если НалогПеречисленный = Неопределено Тогда
		...
		Если Движения.Найти("РасчетыНалоговыхАгентовСБюджетомПоНДФЛ") <> Неопределено Тогда
			Отбор = Новый Структура("ВидДвижения", ВидДвиженияНакопления.Расход);
			НайденныеСтроки = Движения.РасчетыНалоговыхАгентовСБюджетомПоНДФЛ.Выгрузить().НайтиСтроки(Отбор);
		КонецЕсли;
	Иначе
...
Функция РассчитатьНалогПоОсновнойСтавке(ИсключаемыйРегистратор, Организация, МесяцРасчета, Знач СписокФизическихЛиц, Знач ПервыйМесяцНалоговогоПериода = Неопределено, Знач ПоследнийМесяцНалоговогоПериода = Неопределено, ОкончательныйРасчет = Истина, ДатаУдержанияИсчисленногоНалога = Неопределено, ДвиженияТекущегоРегистратора = Неопределено) Экспорт	
	...
	Если ПервыйМесяцНалоговогоПериода < КонецГода(ДатаИзмененияПорядкаИсчисленияНалогаДляИностранцев()) Тогда
		...
		Возврат	РассчитатьНалогПоОсновнойСтавке2018(ИсключаемыйРегистратор, Организация, МесяцРасчета, МенеджерВременныхТаблиц, ПервыйМесяцНалоговогоПериода, ПоследнийМесяцНалоговогоПериода, Истина, ОкончательныйРасчет, ДатаУдержанияИсчисленногоНалога)
	КонецЕсли;
	...
	ВспомогательнаяТаблицаМесяца = РезультатРасчета.СкопироватьКолонки("ФизическоеЛицо, МесяцПериодаПредоставленияВычета, МесяцНалоговогоПериода, Подразделение, КатегорияДохода, НалогПоСтавке13, ЗачтеноАвансовыхПлатежейПоСтавке13");
...
//				ФизическоеЛицо: должно быть непустым
//				МесяцНалоговогоПериода, должно быть непустым
//              колонки сумм исчисленного налога (НалогПоСтавке13) и сумм зачтенных авансовых платежей (для работающих
//              по патенту, ЗачтеноАвансовыхПлатежейПоСтавке13), их имена могут быть переданы дополнительно
//				может содержать другие колонки
//		ИмяКолонкиНалогаИсчисленного - строка -
//		ИмяКолонкиАвансаЗачтенного - строка -
//
...
Функция НалогКУдержаниюПоРезультатамРасчета(ИсключаемыйРегистратор, Организация, ДатаОперации, РезультатыРасчетов, ИмяКолонкиНалогаИсчисленного = "НалогПоСтавке13", ИмяКолонкиАвансаЗачтенного = "ЗачтеноАвансовыхПлатежейПоСтавке13") Экспорт
	...
	Налоги = РезультатыРасчетов.Скопировать(, "ФизическоеЛицо,МесяцНалоговогоПериода," + ИмяКолонкиНалогаИсчисленного + "," + ИмяКолонкиАвансаЗачтенного);
	...
	|	РасчетыНалогоплательщиковСБюджетомПоНДФЛОстатки.СуммаОстаток КАК НеудержаноНалога
	|ИЗ
...
Процедура ВписатьСуммыВыплаченногоДоходаВУдержанныеНалоги(МенеджерВременныхТаблиц, Регистратор, ДатаВыплаты = Неопределено, ДатаОперации = Неопределено) Экспорт
	...
	// Рассчитываемые показатели используются в отчетности только с 2016 года.
	...
	|ИЗ
	|	ВТДанныеОНалогах КАК СписокСотрудников
	|
	|ОБЪЕДИНИТЬ
	|
	|ВЫБРАТЬ
...
Функция РассчитатьУдержанныеНалоги(Ссылка, Организация, ДатаОперации, МенеджерТаблиц, ПериодРегистрации = Неопределено, ОкончательныйРасчет = Истина, ТаблицаВыплат = Неопределено) Экспорт
	...
	УточненнаяДатаОперации = ДатаОперацииПоДокументуВыплаты(ДатаОперации, ПериодРегистрации, ОкончательныйРасчет);
	НеОграничиватьСуммойКВыплате = ЗначениеЗаполнено(ПериодРегистрации) И УточненнаяДатаОперации >= ДатаИзмененияСроковУплатыНалога(); 
	...
	Если НеОграничиватьСуммойКВыплате Тогда
...
Процедура ПроставитьКрайнийСрокУплаты(КоллекцияСтрокДвижений, Организация) Экспорт
	...
	РабочиеДни = КалендарныеГрафики.БлижайшиеРабочиеДаты(КалендарныеГрафики.ОсновнойПроизводственныйКалендарь(), 
		ДатыУдержанияНалога, ПараметрыПолученияБлижайшихРабочихДат(Организация, УчитыватьОсобыеНерабочиеДни));
...
Процедура ЗаполнитьСведенияОПерсональныхДанных(ТаблицаСведений) Экспорт
	...
	НовыеСведения.ПоляДоступа		= "СуммыВозврата.Налог";
...
Функция ВычетыКДоходам(НалоговыйПериод) Экспорт
	...
	// описание кода 3010
	Если НалоговыйПериод > 2017 Тогда // с 2018 года
...
//				а также ДокументОснование, ПериодВзаиморасчетов, Сотрудник, СтатьяФинансирования, СтатьяРасходов, Подразделение
//
Процедура ЗарегистрироватьУдержанныйНалогПриВыплате(Движения, Отказ, Организация, ДатаОперации, ДатаВыплаты, МенеджерВременныхТаблиц, Регистратор, ТаблицаМежрасчетныхВыплат = Неопределено) Экспорт
...
Процедура РасчетПоказателей6НДФЛ(ИмяРасчета, НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные = Ложь, НомерКорректировки = 0) Экспорт
	...
	УчетНДФЛЗаполнениеОтчетности.РасчетПоказателей6НДФЛ(ИмяРасчета, НачалоПериода, ОкончаниеПериода, ДатаПодписи, Организация, РегистрацияВНалоговомОргане, ОписаниеТаблиц, ПодробныеДанные, НомерКорректировки)
	...
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Если ДоходыПоВидам.Количество() = 0 И ВычетыПоВидам.Количество() = 0 И Уведомления.Количество() = 0 Тогда
				...
				ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонПредупреждения, Ссылка, ?(ЗначениеЗаполнено(ФИО), ФИО, "<не указан>"));
...
Функция ДанныеОПерерасчетеНДФЛ(Организация, МесяцПерерасчета, НачалоПериода, ОкончаниеПериода, ФизическиеЛица = Неопределено, ДокументСсылка = Неопределено) Экспорт
	...
	Если ЗарплатаКадры.ВыполнятьРасчетЗарплатыБезОптимизации() Тогда
		Возврат ДанныеОПерерасчетеНДФЛБезОптимизации(Организация, МесяцПерерасчета, НачалоПериода, ОкончаниеПериода, ФизическиеЛица, ДокументСсылка)
	КонецЕсли;
...
Процедура ДополнитьДанныеУчетаСтавками(ДанныеУчета, ПорядокУчетаСтатусаНаКонецГода = "НеУчитыватьИзмененияСтатуса", ОкончаниеОтчетногоПериода = '00010101', СтатусыФизическихЛиц = Неопределено, Знач ПервыйМесяцНалоговогоПериода = '00010101', Знач ПоследнийМесяцНалоговогоПериода = '00010101')
	...
	СтруктураПоиска = Новый Структура("ФизическоеЛицо");
	СтруктураПоискаСтавок = Новый Структура("СтатусФизическогоЛица, КатегорияДохода, СтавкаНалогообложенияРезидента");
...
// Предназначена для дополнения рассчитанных сумм НДФЛ информацией о суммах подлежащих зачету авансовых платежей.
// Имеет дополнительный эффект: если в параметр ИсчисленоНДФЛЗаГод передано Неопределено, а итог по колонке 
// НалогПоСтавке13 в таблице СтрокиНДФЛЗаГод нулевой - эта таблица очищается. 
//
// Параметры:
//      СтрокиНДФЛЗаГод - Массив, ТаблицаЗначений - массив содержит строки таблицы значений, в обоих случаях обязательны
//                                                  колонки НалогПоСтавке13 и ЗачтеноАвансовыхПлатежейПоСтавке13
...
Процедура ВписатьАвансовыеПлатежиВРезультатРасчета(СтрокиНДФЛЗаГод, ОстаткиПоАвансовымПлатежам, ИсчисленоНДФЛЗаГод, ТекущееФизическоеЛицо, ТекущийНалоговыйПериод)
	...
	Если ИсчисленоНДФЛЗаГод = Неопределено Тогда  // вызов из расчета нерезидента, СтрокиНДФЛЗаГод являются таблицей значений
		ИсчисленоНДФЛЗаГод = СтрокиНДФЛЗаГод.Итог("НалогПоСтавке13");
		Если ИсчисленоНДФЛЗаГод = 0 Тогда
			СтрокиНДФЛЗаГод.Очистить();
			Возврат;
		Иначе
			СтрокиНДФЛЗаГод.Свернуть("ФизическоеЛицо, МесяцНалоговогоПериода, МесяцПериодаПредоставленияВычета, Подразделение, КатегорияДохода", "НалогПоСтавке13, КоличествоВычетов, ЗачтеноАвансовыхПлатежейПоСтавке13");
		КонецЕсли;
	...
	Для Каждого СтрокаНДФЛ Из СтрокиНДФЛЗаГод Цикл
		Если СтрокаНДФЛ.НалогПоСтавке13 * СуммаКЗачету > 0 Тогда
			Если СтрокаНДФЛ.НалогПоСтавке13 > 0 Тогда
				СтрокаНДФЛ.ЗачтеноАвансовыхПлатежейПоСтавке13 = Мин(СтрокаНДФЛ.НалогПоСтавке13, СуммаКЗачету);
			Иначе
				СтрокаНДФЛ.ЗачтеноАвансовыхПлатежейПоСтавке13 = Макс(СтрокаНДФЛ.НалогПоСтавке13, СуммаКЗачету);
			КонецЕсли;
			СуммаКЗачету = СуммаКЗачету - СтрокаНДФЛ.ЗачтеноАвансовыхПлатежейПоСтавке13;
			Если СуммаКЗачету = 0 Тогда
				Возврат;
			КонецЕсли;
		КонецЕсли;	
...
Процедура ЗаменитьДатыПодразделенияКатегории(ТаблицаПриемник, ТаблицаИсточник, ПараметрыЗаполненияРесурса, ДатыПодразделенияКатегории, ОсновнаяДатаПолученияДохода, ДополнительныеПоляЗаполнения = "")
	...
	Для каждого СтрокаТЗ Из ТаблицаИсточник Цикл
		Если СтрокаТЗ[ПараметрыЗаполненияРесурса.ИмяРесурса] <> 0 Тогда
			...
			СтрокаПараметровЗамены = ДатыПодразделенияКатегории.НайтиСтроки(СтруктураПоискаЗаменыИзмерений)[0]; 
...
Процедура ПереписатьМежрасчетныйНДФЛФизическогоЛицаВРезультат(РезультатыРасчетов, МежрасчетныйНДФЛ, ДатаУдержанияИсчисленногоНалога, МесяцРасчета, Знач ВсегоДоходов = 0)
	...
	Если МежрасчетныйНДФЛ.Итог("НалогПоСтавке13") * ВсегоДоходов >= 0 Тогда
...
Процедура ПереписатьЧастичныйРезультатРасчетаВРезультатПоФизическомуЛицу(РезультатРасчетаПоФизическомуЛицу, СтрокиНДФЛЗаГод, НДФЛПорцииРасчета, ВычетыПорцииРасчета, ОписаниеВычетов, ТекущийНалоговыйПериод = Неопределено, ПредоставленныеСтандартныеИСоциальныеВычеты = Неопределено, СтруктураПоискаСтандартныхВычетов = Неопределено, ИмущественныеВычеты = Неопределено, СтруктураПоискаИмущественныхВычетов = Неопределено, ПоследнийМесяцПериода = 12)
	...
	НДФЛПорцииРасчета.Свернуть("ФизическоеЛицо, МесяцНалоговогоПериода, МесяцПериодаПредоставленияВычета, Подразделение, КатегорияДохода", "НалогПоСтавке13, КоличествоВычетов, ЗачтеноАвансовыхПлатежейПоСтавке13");
	
	// Переписываем результаты очередной порции расчета в таблицу физ.лица
	Для Каждого СтрокаТЗ Из НДФЛПорцииРасчета Цикл
		Если СтрокаТЗ.НалогПоСтавке13 <> 0 Или СтрокаТЗ.КоличествоВычетов > 0 Тогда
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Если ЗарплатаКадры.АвтономнаяРаботаРазрешеноИзменениеДанных(Метаданные.Справочники.ВидыДоходовНДФЛ) Тогда
		...
		Обработчик = Обработчики.Добавить();
		Обработчик.Версия = "3.1.12.99";
		Обработчик.Процедура = "Справочники.ВидыДоходовНДФЛ.СоздатьКодДохода2020";
		Обработчик.ОбщиеДанные = Истина;
...
Функция ДанныеОПерерасчетеНДФЛБезОптимизации(Организация, МесяцПерерасчета, НачалоПериода, ОкончаниеПериода, ФизическиеЛица = Неопределено, ДокументСсылка = Неопределено)
	
	Возврат УчетНДФЛАрхивныеАлгоритмы.ДанныеОПерерасчетеНДФЛ(Организация, МесяцПерерасчета, НачалоПериода, ОкончаниеПериода, ФизическиеЛица, ДокументСсылка)
