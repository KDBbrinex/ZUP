...
Процедура СоздатьВТБухучетНачисленийБезДоговоровГПХ(ИсходныеДанные, ИмяВТБухучетНачислений) Экспорт
	...
	АктуальныеСтатьиФинансирования = АктуальныеСтатьиФинансирования(Запрос);
	АктуальныеСпособыОтражения = АктуальныеСпособыОтражения(Запрос);
	ПараметрыАктуальнаяАналитика = Новый Структура;
	ПараметрыАктуальнаяАналитика.Вставить("АктуальныеСтатьиФинансирования", АктуальныеСтатьиФинансирования);
	ПараметрыАктуальнаяАналитика.Вставить("АктуальныеСпособыОтражения", АктуальныеСпособыОтражения);
	...
	СоздатьВТБухучетНачисленийПоБазеСреднегоЗаработка(Запрос, ИмяВТНачисленияИсходная, ПараметрыКоэффициенты, ПараметрыАктуальнаяАналитика);
	УдалитьВТ.Добавить("ВТБухучетНачисленийПоБазеСреднегоЗаработка");
	
	// ВТБухучетНачисленийПоФактическимНачислениям
	СоздатьВТБухучетНачисленийПоФактическимНачислениям(Запрос, ИмяВТНачисленияИсходная, ПараметрыАктуальнаяАналитика);
...
Функция АктуальныеСтатьиФинансирования(Запрос)
	...
	|	АктуальнаяСтатья.СтатьяФинансирования КАК АктуальнаяСтатьяФинансирования
	|ИЗ
	|	РегистрСведений.СтатьяФинансированияДляЗаменыУстаревшейАналитикиВБухучете.СрезПоследних(&ПериодРегистрации, Организация = &Организация) КАК АктуальнаяСтатья
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтатьиФинансированияЗарплата КАК СтатьиФинансированияЗарплата
...
Функция АктуальныеСпособыОтражения(Запрос)
	...
	|	АктуальныйСпособОтражения.СпособОтраженияЗарплатыВБухучете КАК АктуальныйСпособОтраженияЗарплатыВБухучете
	|ИЗ
	|	РегистрСведений.СпособОтраженияДляЗаменыУстаревшейАналитикиВБухучете.СрезПоследних(&ПериодРегистрации, Организация = &Организация) КАК АктуальныйСпособОтражения
...
Процедура СоздатьВТБухучетНачисленийПоБазеСреднегоЗаработка(Запрос, ИмяВТНачисленияИсходная, ПараметрыКоэффициенты, ПараметрыАктуальнаяАналитика)
	...
	АктуальныеСтатьиФинансирования 	= ПараметрыАктуальнаяАналитика.АктуальныеСтатьиФинансирования;
	АктуальныеСпособыОтражения 		= ПараметрыАктуальнаяАналитика.АктуальныеСпособыОтражения;
	...
	Иначе
		...
		Для каждого СтрокаНачисления Из НачисленияПоБазе Цикл
			...
			Для Каждого СтрокаОтражения Из СтрокиОтражения Цикл
				...
				НоваяСтатьяФинансирования = СтрокаТаблицы.СтатьяФинансирования;
				Если ЗначениеЗаполнено(СтрокаОтражения.СтатьяФинансирования) Тогда
					АктуальнаяСтатья = АктуальныеСтатьиФинансирования[СтрокаОтражения.СтатьяФинансирования];
					НоваяСтатьяФинансирования = ?(ЗначениеЗаполнено(АктуальнаяСтатья),АктуальнаяСтатья,СтрокаОтражения.СтатьяФинансирования);
				КонецЕсли;
				
				НовыйСпособОтражения = СтрокаТаблицы.СпособОтраженияЗарплатыВБухучете;
				Если ЗначениеЗаполнено(СтрокаОтражения.СпособОтраженияЗарплатыВБухучете) Тогда
					АктуальныйСпособОтражения = АктуальныеСпособыОтражения[СтрокаОтражения.СпособОтраженияЗарплатыВБухучете];
					НовыйСпособОтражения = ?(ЗначениеЗаполнено(АктуальныйСпособОтражения),АктуальныйСпособОтражения,СтрокаОтражения.СпособОтраженияЗарплатыВБухучете);
				КонецЕсли;
...
Процедура СоздатьВТБухучетНачисленийПоФактическимНачислениям(Запрос, ИмяВТНачисленияИсходная, ПараметрыАктуальнаяАналитика)
		...
		Если Месяц(ПериодРегистрации) > 1 Тогда
			...
			Для каждого СтрокаНачисления Из НачисленияПоБазе Цикл
				...
				Для Каждого СтрокаОтражения Из СтрокиОтражения Цикл
					...
					Если ЗначениеЗаполнено(СтрокаОтражения.СтатьяФинансирования) Тогда
						АктуальнаяСтатья = АктуальныеСтатьиФинансирования[СтрокаОтражения.СтатьяФинансирования];
						СтрокаТаблицы.СтатьяФинансирования = ?(ЗначениеЗаполнено(АктуальнаяСтатья),АктуальнаяСтатья,СтрокаОтражения.СтатьяФинансирования);;
					КонецЕсли;
					
					Если СтрокаНачисления.ЭтоРасходыПоСтрахованиюРаботодатель Тогда
						Если ЗначениеЗаполнено(СтрокаОтражения.СпособОтраженияЗарплатыВБухучете) Тогда
							АктуальныйСпособОтражения = АктуальныеСпособыОтражения[СтрокаОтражения.СпособОтраженияЗарплатыВБухучете];
							СтрокаТаблицы.СпособОтраженияЗарплатыВБухучете = ?(ЗначениеЗаполнено(АктуальныйСпособОтражения),АктуальныйСпособОтражения,СтрокаОтражения.СпособОтраженияЗарплатыВБухучете);
						КонецЕсли;
					КонецЕсли;	
...
Процедура СоздатьВТБухучетНачисленияПоБазе(Запрос, УчитыватьСторноЗаписи, МенеджерКадровогоУчета, МенеджерДанныхУчетаВремени)
	...
	Иначе
		...
		Иначе
			...
			|ИЗ
			|	ВТНачисленияПоБазеИзРегистраРасчета КАК НачисленияПоБазе
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТРасчетнаяБаза КАК База
			|		ПО НачисленияПоБазе.Регистратор = База.Регистратор
			|			И НачисленияПоБазе.НомерСтроки = База.НомерСтроки
			|
			|СГРУППИРОВАТЬ ПО
		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				|				И БазаНачислений.ДатаНачалаБаза = БухучетНачислений.ДатаНачала
				|				И БазаНачислений.ДатаОкончанияБаза = БухучетНачислений.ДатаОкончания) КАК БазовыеНачисленияБУ
				|
				|СГРУППИРОВАТЬ ПО
...
Процедура ОбработатьСтрокиНачисленийИсключаемыеИзРаспределяемыхПоБазеСреднего(МенеджерВременныхТаблиц, ФондОплатыТрудаИСтраховыеВзносы, ПериодРегистрации, НачисленияИсключаемыеВПериодКомандировок)
	...
	Для каждого СтрокаНачисления Из НачисленияПоБазе Цикл
		...
		ИначеЕсли ДоляБазы = 0 Тогда
			...
			Для каждого СтрокаФОТ Из СтрокиФОТ Цикл
				УдаляемыеСтроки.Добавить(СтрокаФОТ);
			КонецЦикла;
	...
	Для каждого УдаляемаяСтрока Из УдаляемыеСтроки Цикл
		ФондОплатыТрудаИСтраховыеВзносы.Удалить(УдаляемаяСтрока);
	КонецЦикла;	
...
Функция ПроверитьРезультатыРаспределенияНачисленийУдержанийВСтрокеОбъекта(ПроверяемаяСтрока, СтрокиРаспределенияРезультатовРасчета, ИмяТаблицы, НачислениеУдержаниеВидОперации, РаботаВБюджетномУчреждении, ВидНачисленияВШапке = Неопределено)
	...
	ИначеЕсли ЭтоТаблицаСуммыВозврата Тогда
		Результат  = ПроверяемаяСтрока.Налог;
