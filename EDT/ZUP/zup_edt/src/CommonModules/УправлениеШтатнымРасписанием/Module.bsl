		...
		Пока Выборка.Следующий() Цикл
			...
			Иначе
				...
				ПозицияОбъект.Наименование = УправлениеШтатнымРасписаниемКлиентСервер.НаименованиеПозицииШтатногоРасписания(
					ПозицияОбъект.Подразделение, ПозицияОбъект.Должность);
...
Процедура РассчитатьФОТНесколькихПозиций(ОписаниеПозиций) Экспорт
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(СреднемесячныеНормыВремениПолногоГрафиковРаботыСотрудников.СреднемесячноеЧислоЧасов, СреднемесячныеНормыВремениГрафиковРаботыСотрудников.СреднемесячноеЧислоЧасов) КАК СреднемесячноеЧислоЧасовНорма,
		...
		|	ЕСТЬNULL(СреднемесячныеНормыВремениПолногоГрафиковРаботыСотрудников.СреднемесячноеЧислоДней, СреднемесячныеНормыВремениГрафиковРаботыСотрудников.СреднемесячноеЧислоДней) КАК СреднемесячноеЧислоДнейНормаПолногоГрафика,
		|	ВЫБОР
		...
		|	ЕСТЬNULL(СреднемесячныеЗначенияНормыВремени.СреднемесячноеЧислоДнейНормаПолногоГрафика, 1)
		|ИЗ
		|	ВТПозиции КАК Позиции
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСреднемесячныеЗначенияНормыВремени КАК СреднемесячныеЗначенияНормыВремени
		|		ПО Позиции.ГрафикРаботы = СреднемесячныеЗначенияНормыВремени.ГрафикРаботыСотрудников
		...
		|	Позиции.Сотрудник,
		|	&НормаЧасов,
		|	ЕСТЬNULL(СреднемесячныеЗначенияНормыВремени.СреднемесячноеЧислоЧасовНорма, 1)
		|ИЗ
		|	ВТПозиции КАК Позиции
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСреднемесячныеЗначенияНормыВремени КАК СреднемесячныеЗначенияНормыВремени
		|		ПО Позиции.ГрафикРаботы = СреднемесячныеЗначенияНормыВремени.ГрафикРаботыСотрудников
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(СреднемесячныеЗначенияНормыВремени.СреднемесячноеЧислоДнейНормаПолногоГрафика, 1) КАК НормаДнейПоГрафикуПолногоРабочегоВремени
		|ИЗ
		|	ВТПолныйСписокПозицийСНачислениями КАК ПолныйСписокПозицийСНачислениями
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТСреднемесячныеЗначенияНормыВремени КАК СреднемесячныеЗначенияНормыВремени
...
Функция РезультатЗаполненияПоТекущейКадровойРасстановке(ПараметрыВыполнения) Экспорт
	...
	Если МассивСтрокДляЗаполненияНачислений.Количество() > 0 Тогда
		...
		Для каждого СтрокаДляЗаполнения Из МассивСтрокДляЗаполненияНачислений Цикл
			...
			ДанныеПоСтроке.Начисления.Очистить();
			ДанныеПоСтроке.Показатели.Очистить();
...
Процедура ЗаполнитьДвиженияБронированийЗанятостиПозицийШтатногоРасписания(ПараметрыОбновления = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	ДОБАВИТЬКДАТЕ(УдалитьЗанятыеПозицииШтатногоРасписания.Период, ДЕНЬ, &ДнейСохраненияБрони + 1) КАК ДействуетДо
		|ИЗ
		|	РегистрНакопления.УдалитьЗанятыеПозицииШтатногоРасписания КАК УдалитьЗанятыеПозицииШтатногоРасписания
		|ГДЕ
...
Процедура ДобавитьОписаниеВерсионируемыхДанныхШтатногоРасписания(ОписаниеИсточников, ИдентификаторИсточника)
	...
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточника,
		"	ВЫБОР
		|		КОГДА " + ИдентификаторИсточника + ".КоличествоСтавок < 1
		|			ТОГДА " + ИдентификаторИсточника + ".ФОТМин * ВЫБОР
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели ЕСТЬ NULL
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ДлительностьРабочейНедели / ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели = 1
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				ИНАЧЕ 1
		|			КОНЕЦ
		|		ИНАЧЕ " + ИдентификаторИсточника + ".ФОТМин * " + ИдентификаторИсточника + ".КоличествоСтавок
		|	КОНЕЦ");
	...
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточника,
		"	ВЫБОР
		|		КОГДА " + ИдентификаторИсточника + ".КоличествоСтавок < 1
		|			ТОГДА " + ИдентификаторИсточника + ".ФОТМакс * ВЫБОР
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели ЕСТЬ NULL
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ДлительностьРабочейНедели / ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели = 1
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				ИНАЧЕ 1
		|			КОНЕЦ
		|		ИНАЧЕ " + ИдентификаторИсточника + ".ФОТМакс * " + ИдентификаторИсточника + ".КоличествоСтавок
		|	КОНЕЦ");
	...
	ДобавитьОписаниеСведенийШтатногоРасписания(ОписаниеИсточников, ИмяДанныхШР, ИдентификаторИсточника,
		"	ВЫБОР
		|		КОГДА " + ИдентификаторИсточника + ".КоличествоСтавок < 1
		|			ТОГДА " + ИдентификаторИсточника + ".ФОТ * ВЫБОР
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели ЕСТЬ NULL
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				КОГДА ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ДлительностьРабочейНедели / ВЫРАЗИТЬ(" + ИдентификаторИсточника + ".ГрафикРаботыСотрудников КАК Справочник.ГрафикиРаботыСотрудников).ГрафикПолногоРабочегоВремени.ДлительностьРабочейНедели = 1
		|					ТОГДА " + ИдентификаторИсточника + ".КоличествоСтавок
		|				ИНАЧЕ 1
		|			КОНЕЦ
		|		ИНАЧЕ " + ИдентификаторИсточника + ".ФОТ * " + ИдентификаторИсточника + ".КоличествоСтавок
		|	КОНЕЦ");
...
Процедура ОтключитьОбновлениеНастройкиПубликации(Объект)
