...
Функция ДанныеОНачисленияхДляРасчетаСреднегоЗаработкаПослеПереносаЗатрат(НачисленияСотрудников, ТаблицаПереносы) Экспорт
	...
	Для каждого ПорядокРасчета Из ИспользуемыеПорядкиРасчета Цикл
		...
		Для каждого СтрокаТЗ Из ВремТаблицаСписать Цикл
			...
			НормироватьСуммуДобавить = (?(СуммаТекущихДанных<0,-СуммаТекущихДанных,СуммаТекущихДанных) < ?(СтрокаТЗ.Сумма<0,-СтрокаТЗ.Сумма,СтрокаТЗ.Сумма));
...
Процедура ЗарегистрироватьНачисленияДляРасчетаОбщегоСреднегоЗаработка(Движения, Отказ, МенеджерВременныхТаблиц, ИсключатьВПериодКомандировок, ЗаписыватьДвижения = Ложь)
	...
	Если ИсключатьОплачиваемоеНерабочееВремяИСуммы(МенеджерВременныхТаблиц) Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	Начисления.Сотрудник КАК Сотрудник,
			|	НАЧАЛОПЕРИОДА(Начисления.ПериодДействия, МЕСЯЦ) КАК Месяц
		...
		Запрос.Текст = 
			...
			|	Коэффициенты.ВидРасчета КАК ВидРасчета,
			|	Коэффициенты.Месяц КАК Месяц,
			|	Коэффициенты.Сторно КАК Сторно,
			|	Коэффициенты.Коэффициент КАК Коэффициент
			|ПОМЕСТИТЬ ВТКоэффициентыПредварительно
			|ИЗ
			...
			|ИЗ
			|	ВТКоэффициентыПредварительно КАК Коэффициенты
			|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовРасчета.Начисления КАК Начисления
			|		ПО Коэффициенты.ВидРасчета = Начисления.Ссылка
			|			И (НЕ(Начисления.КатегорияНачисленияИлиНеоплаченногоВремени = ЗНАЧЕНИЕ(Перечисление.КатегорииНачисленийИНеоплаченногоВремени.Премия)
			|					И Начисления.КоличествоМесяцевБазовогоПериода > 1))
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|		ПО Начисления.Сотрудник = Коэффициенты.Сотрудник
			|			И Начисления.Начисление = Коэффициенты.ВидРасчета
			|			И (НАЧАЛОПЕРИОДА(Начисления.ПериодДействия, МЕСЯЦ) = Коэффициенты.Месяц)
			|			И (ВЫБОР
			|				КОГДА Начисления.Сторно
			|						ИЛИ Начисления.ФиксСторно
			|					ТОГДА ИСТИНА
			|				ИНАЧЕ ЛОЖЬ
			|			КОНЕЦ = Коэффициенты.Сторно)
			|
			|СГРУППИРОВАТЬ ПО
			|	Начисления.СоставнаяЧасть,
...
Процедура ИсключитьОплачиваемыеНерабочиеДни(МенеджерВременныхТаблиц, ИсключатьОплачиваемыеНерабочиеДни)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл
		ПериодыСотрудника = Периоды.НайтиСтроки(Новый Структура("Сотрудник", Выборка.Сотрудник));
		Пока Выборка.Следующий() Цикл
...
Процедура ДополнитьВТНачисленияОтражаемыеПоБазе(МенеджерВременныхТаблиц)
	...
	Запрос.Текст = 
		...
		|	*
		|ИЗ
		|	ВТРанееНачисленоПоБазовымРасчетам";
...
Процедура РаспределитьВТНачисленияПоБазе(МенеджерВременныхТаблиц, Регистратор, ИсключатьВПериодКомандировок)
	...
	Запрос.Текст = 
		...
		|	РаспределяемыеСуммы.ИдентификаторСтрокиДокумента КАК ИдентификаторСтрокиДокумента,
	...
	Запрос.Текст = 
		...
		|		ПО ОтборНачислений.Регистратор = РасчетнаяБаза.Регистратор
		|			И ОтборНачислений.НомерСтроки = РасчетнаяБаза.НомерСтроки
		|			И (ОтборНачислений.Регистратор <> &Регистратор)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТОтборНачислений КАК ДанныеДокумента
		|		ПО ОтборНачислений.Сотрудник = ДанныеДокумента.Сотрудник
		|			И ОтборНачислений.ИдентификаторСтрокиДокумента = ДанныеДокумента.ИдентификаторСтрокиДокумента
		|			И (ОтборНачислений.Регистратор <> &Регистратор)
		|			И (ДанныеДокумента.Регистратор = &Регистратор)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	РасчетнаяБазаПредварительно.Регистратор = &Регистратор";
	Запрос.Выполнить();
	...
	Запрос.Текст = 
		...
		|			И РаспределяемыеСуммыПредварительно.СтатьяФинансирования = РасчетнаяБазаВсего.СтатьяФинансирования
		|			И РаспределяемыеСуммыПредварительно.СпособОтраженияЗарплатыВБухучете = РасчетнаяБазаВсего.СпособОтраженияЗарплатыВБухучете
		|			И РаспределяемыеСуммыПредварительно.СтатьяРасходов = РасчетнаяБазаВсего.СтатьяРасходов
		...
		|			И РаспределяемыеСуммыПредварительно.СтатьяФинансирования = РасчетнаяБазаСреднегоЗаработка.СтатьяФинансирования
		|			И РаспределяемыеСуммыПредварительно.СпособОтраженияЗарплатыВБухучете = РасчетнаяБазаСреднегоЗаработка.СпособОтраженияЗарплатыВБухучете
		|			И РаспределяемыеСуммыПредварительно.СтатьяРасходов = РасчетнаяБазаСреднегоЗаработка.СтатьяРасходов
	...
	Запрос.Текст = 
		...
		|		ПО БазаРаспределения.ИдентификаторСтроки = РаспределяемыеСуммы.ИдентификаторСтроки
		|			И БазаРаспределения.СтатьяФинансирования = РаспределяемыеСуммы.СтатьяФинансирования
...
Процедура СоздатьВТПолныеСуммыПремий(МенеджерВременныхТаблиц, ИсключаемыйРегистратор)
	...
	Запрос.Текст = 
		...
		|		ДанныеРегистра.ИсходныйДокумент,
...
Процедура СоздатьВТДанныеОПремиях(МенеджерВременныхТаблиц, ИсключаемыйРегистратор)
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА ДанныеРегистра.ИсходныйДокумент <> ДанныеРегистра.Регистратор
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
