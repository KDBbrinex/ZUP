...
Процедура ПриНастройкеПодчиненногоУзлаРИБ() Экспорт
	...
	ЭтотУзел = ПланыОбмена[ПланыОбмена.ГлавныйУзел().Метаданные().Имя].ЭтотУзел();
...
Процедура ПринадлежностьФизлицаПередЗаписью(Источник) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.Организации") Тогда
		...
		КонецЕсли;
	Иначе
		СоставОбъекта = ЗарплатаКадрыСоставДокументов.СоставОбъекта(Источник.Ссылка);
...
Процедура ПринадлежностьФизлицаОрганизацииПриЗаписи(Источник) Экспорт
	...
	ЗарегистрироватьПринадлежностьФизическихЛиц(
		ДанныеОбъекта.МассивПринадлежностей, ДанныеОбъекта.МассивФизическихЛиц, Метаданные.РегистрыСведений.ОрганизацииФизическихЛиц, "Организация");
	ЗарегистрироватьУтраченныхФизическихЛицПриЗаписи(Источник, ДанныеОбъекта, Метаданные.РегистрыСведений.ОрганизацииФизическихЛицУтраченные, "Организация");
...
// См. СинхронизацияДанныхЗарплатаКадры.РегистрацияИзмененийНачальнойВыгрузкиДанных
...
Процедура ЗарегистрироватьПринадлежностьФизическихЛицОрганизаций(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если Не ИспользуютсяУзлыРИБСОтборомПоОрганизациям(Неопределено) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыОбновления <> Неопределено И ПараметрыОбновления.Свойство("ЗарегистрироватьУтраченныхФизическихЛицОрганизаций") И ПараметрыОбновления.ЗарегистрироватьУтраченныхФизическихЛицОрганизаций Тогда
		ЗарегистрироватьУтраченныхФизическихЛицОрганизаций();
	КонецЕсли;
	
	ОбновитьПринадлежностьФизическихЛиц = Истина;
	
	Пока ОбновитьПринадлежностьФизическихЛиц Цикл
...
Процедура ЗарегистрироватьПринадлежностьФизическихЛицПодразделений(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если Не СинхронизацияДанныхЗарплатаКадрыВызовПовтИсп.ИспользуютсяУзлыРИБСОтборомПоПодразделениям(Неопределено) Тогда
		Возврат;
	КонецЕсли;
	
	Если ПараметрыОбновления <> Неопределено И ПараметрыОбновления.Свойство("ЗарегистрироватьУтраченныхФизическихЛицПодразделений") И ПараметрыОбновления.ЗарегистрироватьУтраченныхФизическихЛицПодразделений Тогда
...
Процедура ЗарегистрироватьПринадлежностьФизическихЛиц(МассивПринадлежностей, МассивФизическихЛиц, МетаданныеРегистра, ИмяПринадлежности) Экспорт
	...
	Для каждого ФизическоеЛицо Из МассивФизическихЛиц Цикл
		Для Каждого Принадлежность Из МассивПринадлежностей Цикл
			Если Не ЗначениеЗаполнено(Принадлежность) Или Не ЗначениеЗаполнено(ФизическоеЛицо) Тогда
				Продолжить;
			КонецЕсли;
			НайденныеСвязи = СуществующиеСвязи.НайтиСтроки(Новый Структура("ФизическоеЛицо, Принадлежность", ФизическоеЛицо, Принадлежность));
			...
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений." + МетаданныеРегистра.Имя);
			ЭлементБлокировки.УстановитьЗначение("ФизическоеЛицо", ФизическоеЛицо);
			ЭлементБлокировки.УстановитьЗначение(ИмяПринадлежности, Принадлежность);
			
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связи физического лица %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), ФизическоеЛицо);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , ФизическоеЛицо, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
	...
	Для каждого ФизическоеЛицо Из МассивФизическихЛиц Цикл
		Для Каждого Принадлежность Из МассивПринадлежностей Цикл
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченной связи физического лица %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), ФизическоеЛицо);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , ФизическоеЛицо, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Продолжить;
			КонецПопытки;
...
Процедура ЗарегистрироватьОрганизацииПоСотрудникам(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если ПараметрыОбновления <> Неопределено И ПараметрыОбновления.Свойство("ЗарегистрироватьУтраченныеОрганизацииСотрудников") И ПараметрыОбновления.ЗарегистрироватьУтраченныеОрганизацииСотрудников Тогда
		ЗарегистрироватьУтраченныеОрганизацииСотрудников();
	КонецЕсли;
	
	ОбновитьОрганизацииСотрудников = Истина;
	
	Пока ОбновитьОрганизацииСотрудников Цикл
...
Процедура ЗарегистрироватьПодразделенияПоСотрудникам(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	Если ПараметрыОбновления <> Неопределено И ПараметрыОбновления.Свойство("ЗарегистрироватьУтраченныеПодразделенияСотрудников") И ПараметрыОбновления.ЗарегистрироватьУтраченныеПодразделенияСотрудников Тогда
		ЗарегистрироватьУтраченныеПодразделенияСотрудников();
	КонецЕсли;
	
	ОбновитьПодразделенияСотрудников = Истина;
	
	Пока ОбновитьПодразделенияСотрудников Цикл
...
Процедура ЗарегистрироватьПодразделенияСотрудников(МассивПодразделений, МассивСотрудников) Экспорт
	...
	ЗарегистрироватьПринадлежностьСотрудников(
		СуществующиеСвязи, МассивПодразделений, МассивСотрудников, Метаданные.РегистрыСведений.ПодразделенияСотрудников, "Подразделение");
	...
	Для каждого Сотрудник Из МассивСотрудников Цикл
		Для Каждого Принадлежность Из МассивПринадлежностей Цикл
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченной связи сотрудника %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), Сотрудник);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Сотрудник, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Продолжить;
			КонецПопытки;
...
Процедура ОбновлениеСвязейФизическихЛицСОрганизациямиИПодразделениями() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеСвязейФизическихЛицСОрганизациямиИПодразделениями);
	ОбновитьСвязиФизическихЛиц();
	
КонецПроцедуры

Процедура ОбновлениеСвязейСотрудниковСОрганизациямиИПодразделениями() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеСвязейСотрудниковСОрганизациямиИПодразделениями);
	ОбновитьСвязиСотрудников();
...
Процедура ЗарегистрироватьИзмененияПриОбработке(ИмяПланаОбмена, ПРО, Объект, Отказ, ТекстЗапроса, ПараметрыЗапроса, ИспользоватьКэш, Выгрузка) Экспорт
	...
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(Объект.Метаданные().ПолноеИмя());
	МенеджерОбъекта.ЗарегистрироватьИзмененияПриОбработке(ИмяПланаОбмена, ПРО, Объект, Отказ, ТекстЗапроса, ПараметрыЗапроса, ИспользоватьКэш, Выгрузка);
...
Процедура ЗарегистрироватьИзмененияПриОбработкеДоп(ИмяПланаОбмена, ПРО, Объект, Ссылка, Отказ, ТекстЗапроса, ПараметрыЗапроса, ИспользоватьКэш) Экспорт
	...
	МенеджерОбъекта.ЗарегистрироватьИзмененияПриОбработкеДоп(ИмяПланаОбмена, ПРО, Объект, Ссылка, Отказ, ТекстЗапроса, ПараметрыЗапроса, ИспользоватьКэш);
...
Процедура ЗарегистрироватьИзмененияПослеОбработки(ИмяПланаОбмена, ПРО, Объект, ОбъектМетаданных, Отказ, Получатели, Выгрузка) Экспорт
	...
	Если Не Отказ И Не Выгрузка И Получатели.Количество() > 0 Тогда
		МенеджерОбъекта.ЗарегистрироватьИзмененияПослеОбработки(ИмяПланаОбмена, ПРО, Объект, Отказ, Получатели, Выгрузка);
	КонецЕсли;
...
Процедура ПриПолученииСпискаШаблоновОчередиЗаданий(Шаблоны) Экспорт
	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ОбновлениеСвязейФизическихЛицСОрганизациямиИПодразделениями.Имя);
	Шаблоны.Добавить(Метаданные.РегламентныеЗадания.ОбновлениеСвязейСотрудниковСОрганизациямиИПодразделениями.Имя);
КонецПроцедуры
...
Процедура ЗарегистрироватьИзменениеКадровойИсторииПриЗаписи(Источник, Отказ, Замещение) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ОрганизационнаяСтруктура") Тогда
		...
		Модуль.ЗарегистрироватьИзменениеКадровойИсторииПриЗаписи(Источник, Отказ, Замещение);
	КонецЕсли;
...
Процедура ПриОпределенииПсевдонимовОбработчиков(СоответствиеИменПсевдонимам) Экспорт
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ОбновлениеСвязейФизическихЛицСОрганизациямиИПодразделениями.ИмяМетода);
	СоответствиеИменПсевдонимам.Вставить(Метаданные.РегламентныеЗадания.ОбновлениеСвязейСотрудниковСОрганизациямиИПодразделениями.ИмяМетода);
КонецПроцедуры
...
Процедура ОбновитьУзлыПлановОбменов(ПараметрыОбновления) Экспорт
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление планов обмена.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			Блокировка.Заблокировать();
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление плана обмена %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), Выборка.Ссылка);
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Выборка.Ссылка, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Возврат;
		КонецПопытки;
...
Процедура ЗарегистрироватьУтраченныхФизическихЛицОрганизаций()
	...
	РегистрироватьУтраченных = Истина;
	Пока РегистрироватьУтраченных Цикл
		...
		Попытка
			Блокировка.Заблокировать();
		Исключение
			ТекстСообщения = НСтр("ru = 'Обмен данными.Обновление утраченной связи физического лица.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Неопределено, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Прервать;
		КонецПопытки;
...
Процедура ЗарегистрироватьОрганизацииФизическихЛицУтраченные(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	ПроверитьПринадлежностьФизическихЛицУтраченные(
		Метаданные.РегистрыСведений.ОрганизацииФизическихЛиц, Метаданные.РегистрыСведений.ОрганизацииФизическихЛицУтраченные, "Организация");
...
Процедура ЗарегистрироватьПодразделенияФизическихЛицУтраченные(ПараметрыОбновления = Неопределено, АдресВоВременномХранилище = Неопределено) Экспорт
	...
	ПроверитьПринадлежностьФизическихЛицУтраченные(
		Метаданные.РегистрыСведений.ПодразделенияФизическихЛиц, Метаданные.РегистрыСведений.ПодразделенияФизическихЛицУтраченные, "Подразделение");
...
Процедура ОбновитьПринадлежностьФизическихЛицОрганизаций(ОбновитьПринадлежностьФизическихЛиц)
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченных организаций физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Если РезультатЗапроса.Пустой() Тогда
		ОбновитьПринадлежностьФизическихЛиц = Ложь;
		Возврат;
	КонецЕсли;
	...
	КоличествоОбработанныхФизическихЛиц = 0;
	Для Каждого СтрокаТаблицы Из ТаблицаПринадлежностиФизическихЛиц Цикл
		Если ТекущееФизическоеЛицо <> СтрокаТаблицы.ФизическоеЛицо Тогда
			Если НаборЗаписей <> Неопределено Тогда
				НаборЗаписей.Записать();
			КонецЕсли;
			
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОрганизацииФизическихЛиц");
			ЭлементБлокировки.УстановитьЗначение("ФизическоеЛицо", СтрокаТаблицы.ФизическоеЛицо);
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связей организаций физического лица %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаТаблицы.ФизическоеЛицо);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , СтрокаТаблицы.ФизическоеЛицо, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
			
			НаборЗаписей = РегистрыСведений.ОрганизацииФизическихЛиц.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.ФизическоеЛицо.Установить(СтрокаТаблицы.ФизическоеЛицо);
			ТекущееФизическоеЛицо = СтрокаТаблицы.ФизическоеЛицо;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаблицы.Принадлежность) И ЗначениеЗаполнено(СтрокаТаблицы.ФизическоеЛицо) Тогда
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.Организация = СтрокаТаблицы.Принадлежность;
			НоваяЗапись.ФизическоеЛицо = СтрокаТаблицы.ФизическоеЛицо;
			НоваяЗапись.ГоловнаяОрганизация = ЗарплатаКадры.ГоловнаяОрганизация(СтрокаТаблицы.Принадлежность);
		КонецЕсли;
		КоличествоОбработанныхФизическихЛиц = КоличествоОбработанныхФизическихЛиц + 1;
	КонецЦикла;
	Если НаборЗаписей <> Неопределено Тогда
		НаборЗаписей.Записать();
	КонецЕсли;
	...
	ОчиститьПринадлежностиФизическихЛиц(ТаблицаУтраченных, Метаданные.РегистрыСведений.ОрганизацииФизическихЛицУтраченные, "Организация");
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиОрганизацийФизическихЛиц");
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление настройки организаций физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	
	НаборЗаписей = РегистрыСведений.НастройкиОрганизацийФизическихЛиц.СоздатьНаборЗаписей();
	Если КоличествоОбработанныхФизическихЛиц <> 0 Тогда
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() > 0 Тогда
			НаборЗаписей[0].ФизическихЛицОбработано = НаборЗаписей[0].ФизическихЛицОбработано + КоличествоОбработанныхФизическихЛиц;
			НаборЗаписей.Записать();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьПринадлежностьФизическихЛицПодразделений(ОбновитьПринадлежностьФизическихЛиц)
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченных структурных подразделений физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаПринадлежностиФизическихЛиц Цикл
		Если ТекущееФизическоеЛицо <> СтрокаТаблицы.ФизическоеЛицо Тогда
			...
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ПодразделенияФизическихЛиц");
			ЭлементБлокировки.УстановитьЗначение("ФизическоеЛицо", СтрокаТаблицы.ФизическоеЛицо);
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связей структурных подразделений физического лица %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаТаблицы.ФизическоеЛицо);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , СтрокаТаблицы.ФизическоеЛицо, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
		...
		КонецЕсли;
		НоваяЗапись = НаборЗаписей.Добавить();
		НоваяЗапись.Подразделение = СтрокаТаблицы.Принадлежность;
		НоваяЗапись.ФизическоеЛицо = СтрокаТаблицы.ФизическоеЛицо;
		КоличествоОбработанныхФизическихЛиц = КоличествоОбработанныхФизическихЛиц + 1;
	КонецЦикла;
	Если НаборЗаписей <> Неопределено Тогда
	...
	ОчиститьПринадлежностиФизическихЛиц(ТаблицаУтраченных, Метаданные.РегистрыСведений.ПодразделенияФизическихЛицУтраченные, "Подразделение");
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиПодразделенийФизическихЛиц");
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление настройки структурных подразделений физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ЗарегистрироватьЗавершенияЗаполненияНастройкиОрганизацийФизическихЛиц()
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Завершение заполнения настройки организаций физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ЗарегистрироватьЗавершенияЗаполненияНастройкиПодразделенийФизическихЛиц()
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Завершение заполнения настройки структурных подразделений физических лиц.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ЗарегистрироватьУтраченныхФизическихЛицПриЗаписи(Источник, ДанныеОбъекта, МетаданныеРегистра, ИмяПринадлежности)
	...
	ЗарегистрироватьУтраченныхФизическихЛиц(МассивПринадлежностейУтраченные, МассивФизическихЛицУтраченные, МетаданныеРегистра, ИмяПринадлежности);
...
Процедура ЗарегистрироватьУтраченныеОрганизацииСотрудников()
	...
	Пока РегистрироватьУтраченных Цикл
		...
		Попытка
			Блокировка.Заблокировать();
		Исключение
			ТекстСообщения = НСтр("ru = 'Обмен данными.Обновление утраченной связи сотрудника.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Неопределено, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Прервать;
		КонецПопытки;
...
Процедура ЗарегистрироватьОрганизацииСотрудников(МассивОрганизаций, МассивСотрудников)
	...
	ЗарегистрироватьПринадлежностьСотрудников(
		СуществующиеСвязи, МассивОрганизаций, МассивСотрудников, Метаданные.РегистрыСведений.ОрганизацииСотрудников, "Организация");
	...
	Для каждого Сотрудник Из МассивСотрудников Цикл
		Для Каждого Принадлежность Из МассивПринадлежностей Цикл
			...
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связи сотрудника %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), Сотрудник);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Сотрудник, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
...
Процедура ОбновитьОрганизацииСотрудников(ОбновитьОрганизацииСотрудников)
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченных организаций сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Если РезультатЗапроса.Пустой() Тогда
		ОбновитьОрганизацииСотрудников = Ложь;
		Возврат;
	КонецЕсли;
	...
	НаборЗаписей = Неопределено;
	КоличествоОбработанныхСотрудников = 0;
	Для Каждого СтрокаТаблицы Из ТаблицаСотрудников Цикл
		Если ТекущийСотрудник <> СтрокаТаблицы.Сотрудник Тогда
			...
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ОрганизацииСотрудников");
			ЭлементБлокировки.УстановитьЗначение("Сотрудник", СтрокаТаблицы.Сотрудник);
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связей организаций сотрудника %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаТаблицы.Сотрудник);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , СтрокаТаблицы.Сотрудник, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаблицы.Принадлежность) И ЗначениеЗаполнено(СтрокаТаблицы.Сотрудник) Тогда
			НоваяЗапись = НаборЗаписей.Добавить();
			НоваяЗапись.Организация = СтрокаТаблицы.Принадлежность;
			НоваяЗапись.Сотрудник = СтрокаТаблицы.Сотрудник;
		КонецЕсли;
		КоличествоОбработанныхСотрудников = КоличествоОбработанныхСотрудников + 1;
	КонецЦикла;
	Если НаборЗаписей <> Неопределено Тогда
		НаборЗаписей.Записать();
	КонецЕсли;
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиОрганизацийСотрудников");
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление настройки организаций сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	
	НаборЗаписей = РегистрыСведений.НастройкиОрганизацийСотрудников.СоздатьНаборЗаписей();
	Если КоличествоОбработанныхСотрудников <> 0 Тогда
		НаборЗаписей.Прочитать();
		Если НаборЗаписей.Количество() > 0 Тогда
			НаборЗаписей[0].СотрудниковОбработано = НаборЗаписей[0].СотрудниковОбработано + КоличествоОбработанныхСотрудников;
			НаборЗаписей.Записать();
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьПодразделенияСотрудников(ОбновитьПодразделенияСотрудников)
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление утраченных структурных подразделений сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
	...
	Если РезультатЗапроса.Пустой() Тогда
		ОбновитьПодразделенияСотрудников = Ложь;
		Возврат;
	КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаСотрудников Цикл
		Если ТекущийСотрудник <> СтрокаТаблицы.Сотрудник Тогда
			...
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ПодразделенияСотрудников");
			ЭлементБлокировки.УстановитьЗначение("Сотрудник", СтрокаТаблицы.Сотрудник);
			Попытка
				Блокировка.Заблокировать();
			Исключение
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление связей структурных подразделений сотрудника %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), СтрокаТаблицы.Сотрудник);
				ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , СтрокаТаблицы.Сотрудник, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				Возврат;
			КонецПопытки;
	...
	Блокировка = Новый БлокировкаДанных;
	ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.НастройкиПодразделенийСотрудников");
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Обновление настройки структурных подразделений сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ЗарегистрироватьЗавершенияЗаполненияНастройкиОрганизацийСотрудников()
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Завершение заполнения настройки организаций сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ЗарегистрироватьЗавершенияЗаполненияНастройкиПодразделенийСотрудников()
	...
	Попытка
		Блокировка.Заблокировать();
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Обмен данными.Завершение заполнения настройки подразделений сотрудников.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()));
		ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , , ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		Возврат;
	КонецПопытки;
...
Процедура ПроверитьОрганизацииСотрудниковУтраченные()
	...
	Пока ОбновитьОрганизацииСотрудников Цикл
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить(Метаданные.РегистрыСведений.ОрганизацииСотрудниковУтраченные.ПолноеИмя());
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		Попытка
			Блокировка.Заблокировать();
		Исключение
			ТекстСообщения = НСтр("ru = 'Обмен данными.Обновление связи утраченной организации сотрудника.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Неопределено, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Прервать;
		КонецПопытки;
...
Процедура ПроверитьПодразделенияСотрудниковУтраченные()
	...
	Пока ОбновитьПодразделенияСотрудников Цикл
		...
		Блокировка = Новый БлокировкаДанных;
		ЭлементБлокировки = Блокировка.Добавить(Метаданные.РегистрыСведений.ПодразделенияСотрудниковУтраченные.ПолноеИмя());
		ЭлементБлокировки.ИсточникДанных = РезультатЗапроса;
		Попытка
			Блокировка.Заблокировать();
		Исключение
			ТекстСообщения = НСтр("ru = 'Обмен данными.Обновление связи утраченного структурного подразделения сотрудника.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(ТекстСообщения, УровеньЖурналаРегистрации.Ошибка, , Неопределено, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Прервать;
		КонецПопытки;
