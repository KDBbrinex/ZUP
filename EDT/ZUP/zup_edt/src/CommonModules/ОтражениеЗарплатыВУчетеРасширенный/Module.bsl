...
Функция СтраховыеВзносыДляПлатежейПоРезультатамРасчета(Организации, ПериодРегистрации) Экспорт
	...
	|ИЗ
	|	РегистрНакопления.СтраховыеВзносыПоФизическимЛицам.Обороты(&НачалоПериода, &КонецПериода, Месяц, Организация В (&Организации)) КАК СтраховыеВзносы";
	...
	Для каждого ИмяПоля Из СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(УчетСтраховыхВзносов.ОтражаемыеВУчетеВзносы(Истина)) Цикл
		ТекстПолейВзносов = ТекстПолейВзносов + "СтраховыеВзносы." + ИмяПоля + "Оборот КАК " + ИмяПоля + "," + Символы.ПС;
	КонецЦикла;
	...
	Возврат ОтражениеЗарплатыВУчете.ТаблицаСтраховыхВзносовПоВидамОпераций(Запрос.МенеджерВременныхТаблиц, ДополнительныеПоля);
...
Процедура ДополнитьСведенияОДоходахНДФЛСведениямиОРаспределенииПоСтатьям(Движения, Аванс = Ложь) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ПустыеЗначенияТерритории = ПустыеЗначенияТерриторииДляПараметраЗапросов();
...
Функция БазаРаспределенияНДФЛКУдержаниюСотрудников(ТаблицаУсловийОтбора)
	...
	БухучетНДФЛ.Свернуть("ФизическоеЛицо,ДокументОснование,Подразделение,КатегорияДохода,СтатьяФинансирования,СтатьяРасходов", "Сумма");
...
Процедура ДополнитьБазуУдержанийПоУмолчаниюСтатьями(БазаПоУмолчанию, Организация, ПериодРегистрации) Экспорт
	...
	Иначе
		...
		БазаПоУмолчанию = Запрос.Выполнить().Выгрузить();
...
//		Движения - коллекции наборов записей движений документа, должна содержать коллекции еще не записанных наборов
//			НДФЛПеречисленный
//		ДанныеДляДополнения - структура см ОтражениеЗарплатыВУчете.НовоеОписаниеПараметровДополненияНДФЛСтатьями.
//
Процедура ДополнитьНДФЛПеречисленныйСведениямиОРаспределенииПоСтатьям(Движения, ДанныеДляДополнения) Экспорт
	
	Если Движения.Найти("НДФЛПеречисленный") = Неопределено Тогда
		Возврат;
	КонецЕсли;
	...
	НаборЗаписей = Движения.НДФЛПеречисленный;
	
	Если НаборЗаписей.Количество() = 0 Или БазаРаспределения.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	КолонкиСтатей = "СтатьяФинансирования";
	Если ПолучитьФункциональнуюОпцию("РаботаВБюджетномУчреждении") Тогда
		КолонкиСтатей = КолонкиСтатей + ",СтатьяРасходов";
	КонецЕсли;
	
	НоваяТаблицаНДФЛ = ОтражениеЗарплатыВУчете.ТаблицаНДФЛДополненнаяСтатьями(НаборЗаписей, БазаРаспределения, ПоляОтбора, КолонкиСтатей);
