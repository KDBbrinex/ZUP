...
Функция ПредставлениеАдресаВФормате9Запятых(Знач АдресВФормате9Запятых, АнализироватьРегион = Ложь) Экспорт
	...
	Если (СтрЧислоВхождений(АдресВФормате9Запятых, ",") <> 9
		И СтрЧислоВхождений(АдресВФормате9Запятых, ",") <> 12)
		ИЛИ (Лев(АдресВФормате9Запятых, 3) <> "643"
		И Лев(АдресВФормате9Запятых, 3) <> "999") Тогда	
	...
	Для Сч = 0 По КоличествоЗапятых - 2 Цикл
		...
		СоставляющиеАдреса.Добавить(СокрЛП(Сред(АдресВФормате9Запятых, КоординатыЗапятых[Сч] + 1,
			КоординатыЗапятых[Сч + 1] - (КоординатыЗапятых[Сч] + 1))));
		
	КонецЦикла;
	
	СоставляющиеАдреса.Добавить(СокрЛП(Сред(АдресВФормате9Запятых,
		КоординатыЗапятых[КоличествоЗапятых - 1] + 1)));
	
	ТекПредставление = "";

	ТипДома = "дом";
	Если КоличествоЗапятых > 9 Тогда
		ТипДома = СокращенияОбъектовАдресацииАдресаРФ[
			?(ПустаяСтрока(СоставляющиеАдреса[10]), ТипДома, НРег(СоставляющиеАдреса[10]))];
	КонецЕсли;
	
	ТипКорпуса = "корпус";
	Если КоличествоЗапятых > 10 Тогда
		ТипКорпуса = СокращенияОбъектовАдресацииАдресаРФ[
			?(ПустаяСтрока(СоставляющиеАдреса[11]), ТипКорпуса, НРег(СоставляющиеАдреса[11]))];
	КонецЕсли;
	
	ТипКвартиры = "квартира";
	Если КоличествоЗапятых > 11 Тогда
		ТипКвартиры = СокращенияОбъектовАдресацииАдресаРФ[
			?(ПустаяСтрока(СоставляющиеАдреса[12]), ТипКвартиры, НРег(СоставляющиеАдреса[12]))];
	КонецЕсли;
...
Процедура ПоказатьКнопкиПечати(Форма) Экспорт
	...
	#Если НаКлиенте Тогда
		...
		Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
			...
			МодульПроцессыОбработкиДокументовКлиентСервер = ОбщегоНазначенияКлиент.ОбщийМодуль(
				"ПроцессыОбработкиДокументовКлиентСервер");
			
			МодульПроцессыОбработкиДокументовКлиентСервер.ПоказатьКнопкиПечатиРегламентированногоОтчета(Форма);
	...
	#Иначе
		...
		Если ОбщегоНазначения.ПодсистемаСуществует("ПроцессыОбработкиДокументов") Тогда
			...
			МодульПроцессыОбработкиДокументовКлиентСервер = ОбщегоНазначения.ОбщийМодуль(
				"ПроцессыОбработкиДокументовКлиентСервер");
...
Процедура ПроставитьСведенияОПредставителеОрганизации(Владелец, ФлагТолькоФЛ) Экспорт
	...
	СтруктураРеквизитовФормы = Владелец.СтруктураРеквизитовФормы;
	ДатаПодписи				 = Владелец.ДатаПодписи;
	ДанныеТитульногоЛиста	 = Владелец.ТабличныйДокумент;
				                                                          	
	ФИОУпПред = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОбОрганизации(
		СтруктураРеквизитовФормы.Организация,
		ДатаПодписи, "ФИОУпПред").ФИОУпПред;
		
	Если ТипЗнч(ДанныеТитульногоЛиста) = Тип("ТабличныйДокумент") Тогда
		
		КодИФНС = ДанныеТитульногоЛиста.Области.КодИМНС.Значение;
	
		КПП	= ДанныеТитульногоЛиста.Области.КПП1.Значение;
		
	ИначеЕсли ТипЗнч(ДанныеТитульногоЛиста) = Тип("Структура") Тогда
		
		КодИФНС = ДанныеТитульногоЛиста.КодИМНС;
	
		КПП	= ДанныеТитульногоЛиста.КПП1;
		
	КонецЕсли;

	СтруктураПредставителя = РегламентированнаяОтчетностьВызовСервера.ПолучитьСведенияОПредставителеОрганизации(
		СтруктураРеквизитовФормы.Организация,
		КодИФНС,
		КПП,
		ДатаПодписи);
		
	Если НЕ ((ПустаяСтрока(СтруктураПредставителя.ПредставительЮр))
	 	   И (ПустаяСтрока(СтруктураПредставителя.ПредставительФЛ))
		   И (ПустаяСтрока(СтруктураПредставителя.ДокументПредставителя))) Тогда
		
		СтруктураРеквизитовФормы.мПредставительФизЛицо = ПустаяСтрока(СтруктураПредставителя.ПредставительЮр);
		
		Если (НЕ СтруктураРеквизитовФормы.мПредставительФизЛицо)
		   И (ФлагТолькоФЛ) Тогда
			
			СтруктураРеквизитовФормы.мПредставительФизЛицо = Неопределено;
			СтруктураРеквизитовФормы.мНазваниеПредставителя = "";
			СтруктураРеквизитовФормы.мДокументПредставителя = "";
			СтруктураРеквизитовФормы.мФамилияПредставителя = "";
			СтруктураРеквизитовФормы.мИмяПредставителя = "";
			СтруктураРеквизитовФормы.мОтчествоПредставителя = "";
			
			Если ТипЗнч(ДанныеТитульногоЛиста) = Тип("ТабличныйДокумент") Тогда
				
				ДанныеТитульногоЛиста.Области.ОргУп.Значение 	 = "";
				ДанныеТитульногоЛиста.Области.ДокУпПред.Значение = "";
				
			ИначеЕсли ТипЗнч(ДанныеТитульногоЛиста) = Тип("Структура") Тогда
				
				ДанныеТитульногоЛиста.ОргУп 	= "";
				ДанныеТитульногоЛиста.ДокУпПред = "";
				
			КонецЕсли;

		Иначе
			
			СтруктураРеквизитовФормы.мНазваниеПредставителя = СокрЛП(СтруктураПредставителя.ПредставительЮр);
			СтруктураРеквизитовФормы.мДокументПредставителя = СокрЛП(СтруктураПредставителя.ДокументПредставителя);
			СтруктураРеквизитовФормы.мФамилияПредставителя = СокрЛП(СтруктураПредставителя.Фамилия);
			СтруктураРеквизитовФормы.мИмяПредставителя = СокрЛП(СтруктураПредставителя.Имя);
			СтруктураРеквизитовФормы.мОтчествоПредставителя = СокрЛП(СтруктураПредставителя.Отчество);
			
			Если ТипЗнч(ДанныеТитульногоЛиста) = Тип("ТабличныйДокумент") Тогда
			
				ДанныеТитульногоЛиста.Области.ОргУп.Значение = ?(СтруктураРеквизитовФормы.мПредставительФизЛицо,
												 				 СтруктураПредставителя.ПредставительФЛ,
												 				 СтруктураПредставителя.ПредставительЮр);
				ДанныеТитульногоЛиста.Области.ДокУпПред.Значение = СтруктураПредставителя.ДокументПредставителя;
				
			ИначеЕсли ТипЗнч(ДанныеТитульногоЛиста) = Тип("Структура") Тогда
				
				ДанныеТитульногоЛиста.ОргУп = ?(СтруктураРеквизитовФормы.мПредставительФизЛицо,
												 				 СтруктураПредставителя.ПредставительФЛ,
												 				 СтруктураПредставителя.ПредставительЮр);
				ДанныеТитульногоЛиста.ДокУпПред = СтруктураПредставителя.ДокументПредставителя;
	...
	Если ТипЗнч(ДанныеТитульногоЛиста) = Тип("ТабличныйДокумент") Тогда
		...
		Если ЗначениеЗаполнено(ДанныеТитульногоЛиста.Области.ОргУп.Значение) Тогда
			ДанныеТитульногоЛиста.Области.ОргДиректор.Значение = "";
		КонецЕсли;
		
	ИначеЕсли ТипЗнч(ДанныеТитульногоЛиста) = Тип("Структура") Тогда
		
		Если ЗначениеЗаполнено(ДанныеТитульногоЛиста.ОргУп) Тогда
			ДанныеТитульногоЛиста.ОргДиректор = "";
		КонецЕсли;
...
Функция СобратьДанныеТекущегоТаблПоля(Форма, ИмяТабличногоПоля) Экспорт 
	...
	Если ЗначениеЗаполнено(Форма.СтруктураРеквизитовФормы.мАдресвФорматеДляВыгрузки) Тогда
...
Процедура ПриИнициализацииФормыРегламентированногоОтчета(Форма, КонтролирующийОрган = "ФНС") Экспорт
	...
	ПриИнициализацииФормыРегламентированногоОтчетаОбщиеДействия(Форма, КонтролирующийОрган);
...
//
Процедура ПриИнициализацииФормыРегламентированногоОтчетаОбщиеДействия(Форма, КонтролирующийОрган)
	
	// Устанавливаем видимость ЭУ регламентированных отчетов из настроек.
	Если (СтрНачинаетсяС(Форма.ИмяФормы, "Отчет") ИЛИ СтрНачинаетсяС(Форма.ИмяФормы, "ВнешнийОтчет"))
		И СтрНайти(Форма.ИмяФормы, "Форма.ФормаОтчета") > 0 Тогда
		
		// Кнопка для загрузки данных отчета.
		ВидимостьКнопкиЗагрузить = Ложь;
		
		НастройкиЗагрузки = Неопределено;
		РегламентированнаяОтчетностьВызовСервера.ПравилаЗагрузкиДляФормы(Форма.ИмяФормы, НастройкиЗагрузки);
		
		РазрешитьЗагрузку = Неопределено;
		Если ТипЗнч(НастройкиЗагрузки) = Тип("Структура") И НастройкиЗагрузки.Свойство("РазрешитьЗагрузку", РазрешитьЗагрузку) Тогда
			ВидимостьКнопкиЗагрузить = (РазрешитьЗагрузку = Истина);
		КонецЕсли;
		
		КнопкаЗагрузить = Форма.Элементы.Найти("Загрузить");
		Если ТипЗнч(КнопкаЗагрузить) = Тип("КнопкаФормы") Тогда
			КнопкаЗагрузить.Видимость = ВидимостьКнопкиЗагрузить;
		КонецЕсли;
		
		// Кнопка для сравнения данных отчета.
		ВидимостьКнопкиСравнить = Ложь;
		
		НастройкиСравнения = Неопределено;
		РегламентированнаяОтчетностьВызовСервера.ПравилаСравненияДляФормы(Форма.ИмяФормы, НастройкиСравнения);
		
		РазрешитьСравнение = Неопределено;
		Если ТипЗнч(НастройкиСравнения) = Тип("Структура") И НастройкиСравнения.Свойство("РазрешитьСравнение", РазрешитьСравнение) Тогда
			ВидимостьКнопкиСравнить = (РазрешитьСравнение = Истина);
		КонецЕсли;
		
		КнопкаСравнить = Форма.Элементы.Найти("Сравнить");
		Если ТипЗнч(КнопкаСравнить) = Тип("КнопкаФормы") Тогда
			КнопкаСравнить.Видимость = ВидимостьКнопкиСравнить;
		КонецЕсли;
		
		#Если НаСервере Тогда
			
			// В случае формирования печатного бланка с PDF417 на сервере в формате PDF
			// для кнопки печати отображаем всплывающую подсказку.
			//
			Если Константы.ИспользоватьСервисФормированияМЧБсPDF417.Получить() Тогда
				
				КнопкаПечать = Форма.Элементы.Найти("Печать");
				
				Если НЕ КнопкаПечать = Неопределено Тогда
					
					КнопкаПечать.ОтображениеПодсказки = ОтображениеПодсказки.Авто;
					
				КонецЕсли;
				
				КомандаПоказатьСДвухмернымШтрихкодомPDF417 = Форма.Команды.Найти("ПоказатьСДвухмернымШтрихкодомPDF417");
				
				Если НЕ КомандаПоказатьСДвухмернымШтрихкодомPDF417 = Неопределено Тогда
					
					КомандаПоказатьСДвухмернымШтрихкодомPDF417.Подсказка
					= НСтр("ru='При сдаче отчета в бумажном виде, сохраните его на компьютер в виде PDF файла для последующей печати. '")
					+ НСтр("ru='Откройте этот файл и распечатайте его в 2-х экземплярах. На каждом экземпляре '")
					+ НСтр("ru='укажите дату, подпишите отчет и отнесите в свою налоговую инспекцию'");
...
Функция ОперацииСРегламентированнымОтчетом(ИмяОтчета, ИмяФормы) Экспорт
	...
	ОперацииСРеглОтчетом = НовыйНаборОперацийСРеглОтчетом();
	ОперацииСРеглОтчетом.АвтоФормированиеНаСервере         = Истина;
	ОперацииСРеглОтчетом.ВыгрузкаНаСервере                 = Истина;
	ОперацииСРеглОтчетом.ПечатьСоШтрихкодомPDF417НаСервере = Истина;
	
	// Регламентированные отчеты, поддерживаемые хотя бы одну операцию из шаблона.
	РеглОтчеты = Новый Соответствие;
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетУСН.ФормаОтчета2015Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетСведенияОСреднесписочнойЧисленностиРаботников.ФормаОтчета2007Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2017Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетНДС.ФормаОтчета2019Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход.ФормаОтчета2017Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход.ФормаОтчета2018Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход.ФормаОтчета2018Кв3"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетЕдиныйНалогНаВмененныйДоход.ФормаОтчета2018Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2016Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПрибыль.ФормаОтчета2019Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РасшифровкиБухгалтерскойОтчетности.ФормаОтчета2019Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	ОперацииСРеглОтчетом.ВыгрузкаНаСервере                 = Истина;
	ОперацииСРеглОтчетом.ПечатьСоШтрихкодомPDF417НаСервере = Истина;
	ОперацииСРеглОтчетом.ПечатьБезШтрихкодаPDF417НаСервере = Истина;
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2017Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетРасчетПоСтраховымВзносам.ФормаОтчета2020Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет6НДФЛ.ФормаОтчета2017Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет4ФСС.ФормаОтчета2017Кв3"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчет4ФСС.ФормаОтчета2017Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетПодтверждениеВидаДеятельности.ФормаОтчета2013Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	ОперацииСРеглОтчетом.ВыгрузкаНаСервере                 = Истина;
	ОперацииСРеглОтчетом.ПечатьСоШтрихкодомPDF417НаСервере = Истина;
	ОперацииСРеглОтчетом.ПолучениеФайловОтчета             = Истина;
	
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетБухОтчетностьМП.ФормаОтчета2015Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	...
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетБухОтчетность.ФормаОтчета2011Кв4"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	РеглОтчеты.Вставить(ВРег("РегламентированныйОтчетБухОтчетность.ФормаОтчета2019Кв1"),
		Новый ФиксированнаяСтруктура(ОперацииСРеглОтчетом));
	
	// Поиск регламентированного отчета в списке отчетов, поддерживаемых хотя бы одну операцию из шаблона.
	ОперацииСРеглОтчетом = РеглОтчеты.Получить(ВРег(СокрЛП(ИмяОтчета) + "." + СокрЛП(ИмяФормы)));
	
	Если ОперацииСРеглОтчетом = Неопределено Тогда
		ОперацииСРеглОтчетом = НовыйНаборОперацийСРеглОтчетом();
	КонецЕсли;
	
	Возврат ОперацииСРеглОтчетом;
...
Функция ДопускаетсяРучноеИзменениеСтатусаОбъекта(ТипКонтролирующегоОргана) Экспорт
	...
	Возврат ТипКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") ИЛИ
		ТипКонтролирующегоОргана = "Минобороны";
...
Функция СтатусыОбъектовРеглОтчетностиПриРучномВводе(ОбъектРеглОтчетности = Неопределено, ВидКонтролирующегоОргана = Неопределено) Экспорт
	...
	Параметры.Вставить("ИсточникОтчета", Неопределено);
	ЗаполнитьЗначенияСвойств(Параметры, ОбъектРеглОтчетности);
	
	Если ВидКонтролирующегоОргана = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") ИЛИ
		ВидКонтролирующегоОргана = "Минобороны" ИЛИ
		Параметры.ИсточникОтчета = "РегламентированныйОтчетИсполнениеКонтрактовГОЗ" Тогда
		
		#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
			МодульДокументооборотСМинобороны = ОбщегоНазначения.ОбщийМодуль("ДокументооборотСМинобороныКлиентСервер");
		#Иначе
			МодульДокументооборотСМинобороны = ОбщегоНазначенияКлиент.ОбщийМодуль("ДокументооборотСМинобороныКлиентСервер");
		#КонецЕсли 
			
		Статусы = МодульДокументооборотСМинобороны.СтатусыПриРучномВводе();
...
Функция ПредставлениеСтраницыРаздела3ФормыРСВ(СтруктураПараметров) Экспорт
	...
	Если СтруктураПараметров.Свойство("ПроверкаВыгрузкиСтраницы") Тогда
		...
		Иначе
			Возврат "Страница " + Формат(СтруктураПараметров.НомерСтраницы, "ЧГ=") + " (" + ПредстЗС + ")";
		КонецЕсли;
...
Процедура ИзменитьВариантыЗаполненияПоказателейТаблицы1_1Формы4ФСС(ВариантыЗаполненияТаблицы1_1,
																   СтруктураРеквизитовФормы) Экспорт
	
	Для Каждого ВариантЗаполненияТаблицы1_1 Из ВариантыЗаполненияТаблицы1_1 Цикл
		
		Если ВариантЗаполненияТаблицы1_1.КодПоказателя = "П000110000106" Тогда
			
			Если СтруктураРеквизитовФормы.ПериодОтчетаВходитВПервыйКвартал
			   И НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
			   
				ВариантЗаполненияТаблицы1_1.ВариантЗаполнения = "4";
				
			Иначе
				
				ВариантЗаполненияТаблицы1_1.ВариантЗаполнения = "0";
				
			КонецЕсли;
			
		ИначеЕсли ВариантЗаполненияТаблицы1_1.КодПоказателя = "П000110000107" Тогда
...
Процедура ИзменитьВариантыЗаполненияПоказателейТаблицы91Формы4ФСС(ВариантыЗаполненияТаблицы91,
																   СтруктураРеквизитовФормы) Экспорт
	
	Для Каждого ВариантЗаполненияТаблицы91 Из ВариантыЗаполненияТаблицы91 Цикл
		
		Если ВариантЗаполненияТаблицы91.КодПоказателя = "П000910000107" Тогда
			
			Если СтруктураРеквизитовФормы.ПериодОтчетаВходитВПервыйКвартал
			   И НЕ СтруктураРеквизитовФормы.ФлажокОтклАвтоРасчет Тогда
			   
				ВариантЗаполненияТаблицы91.ВариантЗаполнения = "4";
				
			Иначе
				
				ВариантЗаполненияТаблицы91.ВариантЗаполнения = СтруктураРеквизитовФормы.ВариантЗаполненияП000910000107;
				
			КонецЕсли;
			
		ИначеЕсли ВариантЗаполненияТаблицы91.КодПоказателя = "П000910000108" Тогда
			...
			Иначе
				...
				ВариантЗаполненияТаблицы91.ВариантЗаполнения = СтруктураРеквизитовФормы.ВариантЗаполненияП000910000108;
...
Функция КодСубъектаРФПоОКТМО(СоответствиеОКАТОСубъектамРФ, ОКТМО) Экспорт
	
	Результат = "99"; // По умолчанию - "Другие территории".
	
	Для Каждого СтрокаСоответствия Из СоответствиеОКАТОСубъектамРФ Цикл
...
Функция ДоступенМониторингИзмененияЗаконодательства(Период) Экспорт
	
	Возврат (Период >= Дата(2013, 1, 1));
...
Функция СокращенияОбъектовАдресацииАдресаРФ() Экспорт
	...
	Результат.Вставить(НСтр("ru = 'шахта'"), НСтр("ru = 'ш.'"));
	Результат.Вставить(НСтр("ru = 'зем. участок'"), НСтр("ru = 'з/у'"));
...
Функция ПолучитьАдресСтраницыСШаблонамиРосстата() Экспорт
	
	Возврат "https://www.gks.ru/monitoring";
	
КонецФункции

Функция НовыйНаборОперацийСРеглОтчетом()
	
	ОперацииСРеглОтчетом = Новый Структура;
	ОперацииСРеглОтчетом.Вставить("АвтоФормированиеНаСервере",         Ложь);
	ОперацииСРеглОтчетом.Вставить("ВыгрузкаНаСервере",                 Ложь);
	ОперацииСРеглОтчетом.Вставить("ПечатьСоШтрихкодомPDF417НаСервере", Ложь);
	ОперацииСРеглОтчетом.Вставить("ПечатьБезШтрихкодаPDF417НаСервере", Ложь);
	ОперацииСРеглОтчетом.Вставить("ПолучениеФайловОтчета",             Ложь);
