			...
			Если ТипЗнч(ЭлементКомПанели) = Тип("ГруппаФормы") Тогда
				Для Каждого ЭлементГруппы Из ЭлементКомПанели.ПодчиненныеЭлементы Цикл
					Если ТипЗнч(ЭлементГруппы) = Тип("ГруппаФормы")
							И (ЭлементГруппы.Заголовок = "Создать на основании"
								Или ЭлементГруппы.Заголовок = "Органайзер"
								Или ЭлементГруппы.Заголовок = "Печать") Тогда
...
Функция ЗначениеРеквизитаПоОбъектуXDTO(ОбъектXDTO, ТипРеквизита, ИмяРеквизита, ТипОбъекта, Объект,
	ЭтоДополнительноеСвойство = Ложь, Свойство = Неопределено, ПараметрыВыбора = Неопределено) Экспорт
	...
	Если Результат = Неопределено Тогда
		...
		Если ОбщегоНазначения.ЭтоСправочник(МетаданныеОбъекта) Тогда
			...
			Иначе // продолжим поиск по другим реквизитам
				...
				Если МетаданныеОбъекта.Владельцы.Количество() = 0
					Или Владелец <> Неопределено Тогда
...
//
// Параметры:
//   Форма - УправляемаяФорма - форма, реквизит которой заполняется.
...
Процедура ОбработкаЗаполненияДокументы(Источник, ДанныеЗаполнения, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		И ДанныеЗаполнения.Свойство("Объект1СДокументооборота") Тогда
...
// Параметры:
//   Прокси - объект для подключения к web-сервисам Документооборота
//   Задачи - массив идентификаторов задач Документооборота, принятие к исполнению которых нужно отменить
//
// Возвращаемое значение:
...
//     с данными объекта.
//
Процедура ЗаполнитьСвойствоXDTOизСтруктурыРеквизитов(Прокси, ОбъектXDTO, ИмяСвойства, СтруктураРеквизитов, ПутьДанных) Экспорт
	...
	Если ТипЗнч(ПутьДанных) = Тип("Структура") Тогда
		...
		Для Каждого КлючИЗначение Из ПутьДанных Цикл
			...
			Таблица = СвойствоКонтейнер[ИмяТаблицы];
...
Функция СоздатьОбъектДОПоПравилу(Прокси, ОбъектИС, Правило) Экспорт
	...
	ТипОбъектаДО = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Правило, "ТипОбъектаДО");
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьСвязанныеДокументы1СДокументооборота")
			...
			И Не ДанныеПравила.НеСоздаватьСвязиПоСсылкам Тогда
