...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПоВременнойТаблицеНачислений(Регистратор, Движения, Отказ, Организация, Дата, ПериодРегистрации, ПорядокВыплаты, ПланируемаяДатаВыплаты, ДанныеДляПроведения, Записывать = Ложь, РегистрироватьСуммыНалога = Неопределено, ДоходПолученНаТерриторииРФ = Ложь, ПеречислитьНалогПриВыплатеЗарплаты = Ложь, ИмяТаблицыФизлиц = "ВТФизическиеЛица", ИмяВТНачисления = "ВТНачисления", ЗасчитыватьДанныеАвансов = Ложь) Экспорт 
	...
	Если РегистрироватьСуммыНалога Тогда
		...
		УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛИКорректировкиВыплаты(ДанныеДляПроведенияУчетЗарплаты, Отказ, ДанныеДляПроведения.НДФЛПоСотрудникам, Неопределено, Записывать);
...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПоТаблицеДоходов(Регистратор, Движения, Отказ, Организация, Дата, МесяцНачисления, ПорядокВыплаты, ПланируемаяДатаВыплаты, ДанныеДляПроведения, ТаблицаДоходов, Записывать = Ложь, РегистрироватьСуммыНалога = Неопределено, ДоходПолученНаТерриторииРФ = Ложь) Экспорт 
	...
	Если РегистрироватьСуммыНалога Тогда
		...
		УчетНачисленнойЗарплаты.ЗарегистрироватьНДФЛИКорректировкиВыплаты(ДанныеДляПроведенияУчетЗарплаты, Отказ,
			ДанныеДляПроведения.НДФЛПоСотрудникам, Неопределено, Записывать);
...
Процедура ЗарегистрироватьДоходыИСуммыНДФЛПриНачисленииАванса(Регистратор, Движения, Отказ, Организация, Дата, ПериодРегистрации, ПланируемаяДатаВыплаты, ДанныеДляПроведения, Записывать = Ложь, ИмяТаблицыФизлиц = "ВТФизическиеЛица") Экспорт 
	...
	// регистрация доходов
	СформироватьДоходыНДФЛПоНачислениям(Движения, Отказ, Организация, ДатаОперацииПоНалогам, ПланируемаяДатаВыплаты, ДанныеДляПроведения.МенеджерВременныхТаблиц, ПериодРегистрации, , Ложь, , Регистратор);
	...
	СформироватьСоциальныеВычетыПоУдержаниям(Регистратор, Движения, Отказ, Организация, ДатаОперацииПоНалогам, ПериодРегистрации, ДанныеДляПроведения.УдержанияПоСотрудникам, , Ложь);
...
Процедура ДополнитьДвиженияСписаниемАванса(Регистратор, Движения, Организация, ДатаОперации, ПериодыРегистрацииСотрудников = Неопределено) Экспорт
	...
	Если Не ПроведениеСервер.ЕстьДвижения(Движения, "СведенияОДоходахНДФЛ") Тогда // Доходы этим документом не начислялась - обнулять авансовые движения не будем.
		Возврат
	КонецЕсли;	
	
	НаборЗаписейОДоходах = Движения.СведенияОДоходахНДФЛ;
	
	ФизическиеЛица = НаборЗаписейОДоходах.Выгрузить().Скопировать(Новый Структура("КатегорияДохода", Перечисления.КатегорииДоходовНДФЛ.ОплатаТруда)).ВыгрузитьКолонку("ФизическоеЛицо");
	Если ФизическиеЛица.Количество() = 0 Тогда // Зарплата этим документом не начислялась - обнулять авансовые движения не будем.
		Возврат
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ПериодыРегистрацииСотрудников) Тогда
		ВсегоМесяцев = ПериодыРегистрацииСотрудников.Количество();
		Месяцы = ПериодыРегистрацииСотрудников.ВыгрузитьКолонку("ПериодРегистрации");
	Иначе
		ВсегоМесяцев = 1;
		Месяцы = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(НачалоМесяца(НаборЗаписейОДоходах[0].Период));
	КонецЕсли;
	...
	|	НачислениеЗаПервуюПоловинуМесяца.МесяцНачисления КАК МесяцНачисления,
	|	НачислениеЗаПервуюПоловинуМесяца.Ссылка КАК ДокументОснование
	|ПОМЕСТИТЬ ВТДокументыОснования
	|ИЗ
	|	РегистрСведений.ДокументыУчтенныеПриРасчетеНДФЛ КАК ДокументыУчтенныеПриРасчетеНДФЛ
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.НачислениеЗаПервуюПоловинуМесяца КАК НачислениеЗаПервуюПоловинуМесяца
	|		ПО ДокументыУчтенныеПриРасчетеНДФЛ.Регистратор = НачислениеЗаПервуюПоловинуМесяца.Ссылка
	|ГДЕ
	|	НачислениеЗаПервуюПоловинуМесяца.Организация = &Организация
	|	И НачислениеЗаПервуюПоловинуМесяца.МесяцНачисления В(&Месяцы)
	|	И ДокументыУчтенныеПриРасчетеНДФЛ.ФизическоеЛицо В(&ФизическиеЛица)";
	// Проверяем, есть ли вообще зарегистрированные авансами доходы.
	Если Запрос.Выполнить().Выгрузить()[0].Количество = 0 Тогда
		Возврат
	КонецЕсли;
	...
	|	ДокументыОснования.МесяцНачисления КАК МесяцНачисления
	|ПОМЕСТИТЬ ВТФизическиеЛица
	|ИЗ
	...
	|	И СведенияОДоходахНДФЛ.ЗарегистрированоПриНачисленииАванса <> ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0)
	|	И СведенияОДоходахНДФЛ.Регистратор <> &Регистратор
	|
	|СГРУППИРОВАТЬ ПО
	|	ДокументыОснования.ФизическоеЛицо,
	|	ДокументыОснования.МесяцНачисления
	|
	|ИМЕЮЩИЕ
	|	СУММА(СведенияОДоходахНДФЛ.СуммаДохода - СведенияОДоходахНДФЛ.СуммаВычета) <> 0";
	// Проверяем, остались ли зарегистрированные авансами доходы.
	Если Запрос.Выполнить().Выгрузить()[0].Количество = 0 Тогда
		Возврат
	КонецЕсли;
	
	// Соберем данные для распределения дополнительных строк по подразделениям из текущих доходов.
	СоздаватьТаблицуРаспределения = Истина;
	Если ПроведениеСервер.ЕстьДвижения(Движения, "ДокументыУчтенныеПриРасчетеНДФЛ") Тогда
		Основания = Движения.ДокументыУчтенныеПриРасчетеНДФЛ.ВыгрузитьКолонку("ДокументОснование");
		ОбщегоНазначенияКлиентСервер.УдалитьВсеВхожденияЗначенияИзМассива(Основания, Регистратор);
		Если Основания.Количество() > 0 Тогда
			СоздаватьТаблицуРаспределения = Ложь;
			СоответствиеФизическихЛиц = ОбщегоНазначенияБЗККлиентСервер.МассивВСоответствие(Движения.ДокументыУчтенныеПриРасчетеНДФЛ.ВыгрузитьКолонку("ФизическоеЛицо"));
			...
			|	РегистрНакопления.СведенияОДоходахНДФЛ КАК СведенияОДоходахНДФЛ
			|ГДЕ
			|	СведенияОДоходахНДФЛ.Регистратор В(&Основания)";
			СтрокиРаспределения = Запрос.Выполнить().Выгрузить();
			СтрокиКУдалению = Новый Соответствие;
			Для каждого СтрокаТЗ Из СтрокиРаспределения Цикл
				Если СоответствиеФизическихЛиц[СтрокаТЗ.ФизическоеЛицо] = Неопределено Тогда
					СтрокиКУдалению.Вставить(СтрокаТЗ, Истина)
				КонецЕсли;
			КонецЦикла;
			Для Каждого УдаляемаяСтрока Из СтрокиКУдалению Цикл
				СтрокиРаспределения.Удалить(УдаляемаяСтрока.Ключ);		
			КонецЦикла;	
		КонецЕсли;
	КонецЕсли;
	Если СоздаватьТаблицуРаспределения Тогда
		СтрокиРаспределения = Новый ТаблицаЗначений;
		СтрокиРаспределения.Колонки.Добавить("ПериодРегистрации", Новый ОписаниеТипов("Дата"));
		СтрокиРаспределения.Колонки.Добавить("ФизическоеЛицо", Новый ОписаниеТипов("СправочникСсылка.ФизическиеЛица"));
		СтрокиРаспределения.Колонки.Добавить("СуммаДохода", Новый ОписаниеТипов("Число"));
		СтрокиРаспределения.Колонки.Добавить("Подразделение", Метаданные.ОпределяемыеТипы.ТерриторияВыполненияРаботВОрганизации.Тип);
		СтрокиРаспределения.Колонки.Добавить("ПодразделениеСотрудника", Новый ОписаниеТипов("СправочникСсылка.ПодразделенияОрганизаций"));
		СтрокиРаспределения.Колонки.Добавить("РегистрацияВНалоговомОргане", Новый ОписаниеТипов("СправочникСсылка.РегистрацииВНалоговомОргане"));
	КонецЕсли;
	Для каждого СтрокаТЗ Из Движения.СведенияОДоходахНДФЛ Цикл
	    НоваяСтрока = СтрокиРаспределения.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТЗ);
		НоваяСтрока.ПериодРегистрации = НачалоМесяца(СтрокаТЗ.Период);
	КонецЦикла;
	СтрокиРаспределения.Свернуть("ПериодРегистрации,ФизическоеЛицо,Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане", "СуммаДохода");
	СтрокиРаспределения.Индексы.Добавить("ФизическоеЛицо, ПериодРегистрации");
	// В таблице описания соберем для повторного использования все, что может понадобиться алгоритму распределения по базе.
	ОписаниеРаспределения = СтрокиРаспределения.Скопировать();
	ОписаниеРаспределения.Свернуть("ПериодРегистрации,ФизическоеЛицо", "СуммаДохода");
	ОписаниеРаспределения.Колонки.Добавить("НетРаспределения", Новый ОписаниеТипов("Булево"));
	ОписаниеРаспределения.Колонки.Добавить("ТребуетсяРаспределение", Новый ОписаниеТипов("Булево"));
	ОписаниеРаспределения.Колонки.Добавить("ТекущееРаспределение");
	ОписаниеРаспределения.Колонки.Добавить("СтрокаТекущегоРаспределения");
	ОписаниеРаспределения.Колонки.Добавить("Коэффициенты");
	ОписаниеРаспределения.Индексы.Добавить("ФизическоеЛицо, ПериодРегистрации");
	
	// Дополнительно определим остатки неудержанного налога и фактически выплаченные авансовые суммы 
	// для простановки документа-основания в компенсирующих записях.
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ФизическиеЛица.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ФизическиеЛица.МесяцНачисления КАК ПериодРегистрации,
	...
	|		ПО ФизическиеЛица.ФизическоеЛицо = ДокументыОснования.ФизическоеЛицо
	|			И ФизическиеЛица.МесяцНачисления = ДокументыОснования.МесяцНачисления
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	НачисленияАванса.ФизическоеЛицо КАК ФизическоеЛицо,
	|	НачисленияАванса.ДокументОснование КАК ДокументОснование,
	...
	|		КОНЕЦ) КАК ОсталосьНеУдержаноНалога,
	|	СУММА(РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СуммаВыплаченногоДохода) КАК СуммаВыплаченногоДохода
	|ПОМЕСТИТЬ ВТОстаткиНалога
	|ИЗ
	|	ВТДокументыКОбработке КАК НачисленияАванса
	...
	|		ПО НачисленияАванса.ФизическоеЛицо = РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ФизическоеЛицо
	|			И НачисленияАванса.ДокументОснование = РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ДокументОснование
	|			И (РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Регистратор <> &Регистратор)
	|
	|СГРУППИРОВАТЬ ПО
	|	НачисленияАванса.ФизическоеЛицо,
	|	НачисленияАванса.ДокументОснование,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ДокументыКОбработке.ФизическоеЛицо КАК ФизическоеЛицо,
	|	ДокументыКОбработке.ПериодРегистрации КАК ПериодРегистрации,
	|	ДокументыКОбработке.ДокументОснование КАК ДокументНачисленияАванса,
	|	ЕСТЬNULL(Остатки.СуммаВыплаченногоДохода, 0) КАК СуммаПоРегистратору,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СведенияОДоходахНДФЛ КАК СведенияОДоходахНДФЛ
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиНалога КАК Остатки
	|			ПО СведенияОДоходахНДФЛ.ФизическоеЛицо = Остатки.ФизическоеЛицо
	|				И СведенияОДоходахНДФЛ.ДокументОснование = Остатки.ДокументОснование
	|				И СведенияОДоходахНДФЛ.КодДохода.СтавкаНалогообложенияРезидента = Остатки.СтавкаНалогообложенияРезидента
	|				И СведенияОДоходахНДФЛ.ДатаПолученияДохода = Остатки.МесяцНалоговогоПериода
	|				И СведенияОДоходахНДФЛ.КатегорияДохода = Остатки.КатегорияДохода
	|				И СведенияОДоходахНДФЛ.РегистрацияВНалоговомОргане = Остатки.РегистрацияВНалоговомОргане
	|				И СведенияОДоходахНДФЛ.Подразделение = Остатки.Подразделение
	|		ПО ДокументыКОбработке.ФизическоеЛицо = СведенияОДоходахНДФЛ.ФизическоеЛицо
	|			И ДокументыКОбработке.ДокументОснование = СведенияОДоходахНДФЛ.ДокументОснование
	|			И (СведенияОДоходахНДФЛ.Регистратор <> &Регистратор)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ФизическоеЛицо,
	|	ПериодРегистрации, 
	|   ДокументНачисленияАванса,
	|   ДатаПолученияДохода,
	|   РегистрацияВНалоговомОргане,
	|   Подразделение,
	|   КатегорияДохода,
	...
	|		ПО ДокументыКОбработке.ФизическоеЛицо = ИмущественныеВычетыНДФЛ.ФизическоеЛицо
	|			И ДокументыКОбработке.ДокументОснование = ИмущественныеВычетыНДФЛ.Регистратор
	...
	|		ПО ДокументыКОбработке.ФизическоеЛицо = ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.ФизическоеЛицо
	|			И ДокументыКОбработке.ДокументОснование = ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ.Регистратор
	...
	|ИЗ
	|	ВТДокументыКОбработке КАК ДокументыКОбработке
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.АвансовыеПлатежиИностранцевПоНДФЛ КАК АвансовыеПлатежиИностранцевПоНДФЛ
	|		ПО ДокументыКОбработке.ФизическоеЛицо = АвансовыеПлатежиИностранцевПоНДФЛ.ФизическоеЛицо
	|			И ДокументыКОбработке.ДокументОснование = АвансовыеПлатежиИностранцевПоНДФЛ.Регистратор
	|			И (АвансовыеПлатежиИностранцевПоНДФЛ.Регистратор <> &Регистратор)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА ЕСТЬNULL(Остатки.ОсталосьНеУдержаноНалога, 0)
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК СуммаПоДокументуНачисленияАванса,
	|	ВЫБОР
	|		КОГДА РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Сумма - ЕСТЬNULL(Остатки.ОсталосьНеУдержаноНалога, 0)
	|		ИНАЧЕ РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Сумма
	|	КОНЕЦ КАК СуммаПоРегистратору,
	|	&Регистратор КАК ДокументОснование,
	|	ИСТИНА КАК ДатаПолученияДоходаФиксирована,
	|	*
	|ИЗ
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыНалогоплательщиковСБюджетомПоНДФЛ КАК РасчетыНалогоплательщиковСБюджетомПоНДФЛ
	|			ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиНалога КАК Остатки
	|			ПО РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ФизическоеЛицо = Остатки.ФизическоеЛицо
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ДокументОснование = Остатки.ДокументОснование
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.СтавкаНалогообложенияРезидента = Остатки.СтавкаНалогообложенияРезидента
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.МесяцНалоговогоПериода = Остатки.МесяцНалоговогоПериода
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.КатегорияДохода = Остатки.КатегорияДохода
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.РегистрацияВНалоговомОргане = Остатки.РегистрацияВНалоговомОргане
	|				И РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Подразделение = Остатки.Подразделение
	|		ПО ДокументыКОбработке.ФизическоеЛицо = РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ФизическоеЛицо
	|			И ДокументыКОбработке.ДокументОснование = РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ДокументОснование
	|			И (РасчетыНалогоплательщиковСБюджетомПоНДФЛ.Регистратор <> &Регистратор)
	|			И (РасчетыНалогоплательщиковСБюджетомПоНДФЛ.ВариантУдержания <> ЗНАЧЕНИЕ(Перечисление.ВариантыУдержанияНДФЛ.Удержано))";
	Результаты = Запрос.ВыполнитьПакет();
	
	СтруктураПоиска = Новый Структура("ФизическоеЛицо, ПериодРегистрации");
	
	// Обходя выборку, заполним таблицу описания данных распределения, которая поможет 
	// ускорить обработку последующих наборов данных.
	
	ТекущееФизическоеЛицо = Неопределено;
	ТекущийПериодРегистрации = '00010101';
	ТекущийДокументНачисленияАванса = Неопределено;
	ТекущаяДатаПолученияДохода = '00010101';
	ТекущаяРегистрацияВНалоговомОргане = Неопределено;
	ТекущееПодразделение = Неопределено;
	ТекущаяКатегорияДохода = Неопределено;
	ТекущаяСтавкаНалогообложенияРезидента = Неопределено;
	СуммаКРаспределению = 0;
	Выборка = Результаты[2].Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ТекущееФизическоеЛицо <> Выборка.ФизическоеЛицо Или ТекущийПериодРегистрации <> Выборка.ПериодРегистрации Тогда
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, Выборка);
			СтрокиОписания = ОписаниеРаспределения.НайтиСтроки(СтруктураПоиска); 
			Если СтрокиОписания.Количество() = 0 Тогда
				СтрокаОписания = ОписаниеРаспределения.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаОписания, СтруктураПоиска);
			Иначе
				СтрокаОписания = СтрокиОписания[0]
			КонецЕсли;
			НетРаспределения = СтрокаОписания.СуммаДохода = 0;
			Если НетРаспределения Тогда
				СтрокаОписания.НетРаспределения = Истина;
				Продолжить;
			КонецЕсли;
			ТекущееРаспределение = СтрокиРаспределения.Скопировать(СтруктураПоиска); 
			СтрокаОписания.ТекущееРаспределение = ТекущееРаспределение;
			ТребуетсяРаспределение = ТекущееРаспределение.Количество() > 1;
			СтрокаОписания.ТребуетсяРаспределение = ТребуетсяРаспределение;
			Если Не ТребуетсяРаспределение Тогда
				СтрокаТекущегоРаспределения = ТекущееРаспределение[0];
				СтрокаОписания.СтрокаТекущегоРаспределения = СтрокаТекущегоРаспределения;
			Иначе
				СтрокаОписания.Коэффициенты = ТекущееРаспределение.ВыгрузитьКолонку("СуммаДохода");	
				Коэффициенты = СтрокаОписания.Коэффициенты;	
			КонецЕсли;
		КонецЕсли;
		Если ТекущееФизическоеЛицо <> Выборка.ФизическоеЛицо Или ТекущийПериодРегистрации <> Выборка.ПериодРегистрации
			Или ТекущееПодразделение <> Выборка.Подразделение Или ТекущийДокументНачисленияАванса <> Выборка.ДокументНачисленияАванса 
			Или ТекущаяДатаПолученияДохода <> Выборка.ДатаПолученияДохода Или ТекущаяРегистрацияВНалоговомОргане <> Выборка.РегистрацияВНалоговомОргане 
			Или ТекущаяКатегорияДохода <> Выборка.КатегорияДохода Или ТекущаяСтавкаНалогообложенияРезидента <> Выборка.СтавкаНалогообложенияРезидента Тогда
			СуммаКРаспределению = Выборка.СуммаПоРегистратору;
		КонецЕсли;
		Если НетРаспределения Тогда
			Продолжить;
		ИначеЕсли Не ТребуетсяРаспределение Тогда
			Если СуммаКРаспределению = 0 Тогда
				НоваяСтрока = НаборЗаписейОДоходах.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.ДокументОснование = Выборка.ДокументНачисленияАванса;
				НоваяСтрока.СуммаДохода =  -НоваяСтрока.СуммаДохода;
				НоваяСтрока.СуммаВычета =  -НоваяСтрока.СуммаВычета;
			ИначеЕсли СуммаКРаспределению >= Выборка.СуммаДохода Тогда	
				СуммаКРаспределению = СуммаКРаспределению - Выборка.СуммаДохода;
				НоваяСтрока = НаборЗаписейОДоходах.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
			...
			Иначе	
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.СуммаДохода =  -СуммаКРаспределению;
				НоваяСтрока.СуммаВычета =  -Мин(НоваяСтрока.СуммаВычета, СуммаКРаспределению);
				...
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.ДокументОснование = Выборка.ДокументНачисленияАванса;
				НоваяСтрока.СуммаДохода =  -(НоваяСтрока.СуммаДохода - СуммаКРаспределению);
				НоваяСтрока.СуммаВычета =  -(НоваяСтрока.СуммаВычета - Мин(НоваяСтрока.СуммаВычета, СуммаКРаспределению));
			КонецЕсли;
		Иначе
			Если СуммаКРаспределению = 0 Тогда
				РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаДохода, Коэффициенты);
				Если Выборка.СуммаВычета <> 0 Тогда
					РезультатРаспределенияВычета = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаВычета, Коэффициенты);
				КонецЕсли;
			ИначеЕсли СуммаКРаспределению >= Выборка.СуммаДохода Тогда	
				РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаДохода, Коэффициенты);
				Если Выборка.СуммаВычета <> 0 Тогда
				...
				КонецЕсли;
			Иначе	
				РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(СуммаКРаспределению, Коэффициенты);
				Если Выборка.СуммаВычета <> 0 Тогда
					РезультатРаспределенияВычета = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Мин(НоваяСтрока.СуммаВычета, СуммаКРаспределению), Коэффициенты);
				КонецЕсли;
				РезультатРаспределенияПоДокументуАванса = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаДохода - СуммаКРаспределению, Коэффициенты);
				Если Выборка.СуммаВычета <> 0 Тогда
					РезультатРаспределенияВычетаПоДокументуАванса = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаВычета - Мин(НоваяСтрока.СуммаВычета, СуммаКРаспределению), Коэффициенты);
				КонецЕсли;
			КонецЕсли;
			Индекс = 0;
			Для каждого СтрокаТекущегоРаспределения Из ТекущееРаспределение Цикл
				Если СуммаКРаспределению <> 0 И СуммаКРаспределению < Выборка.СуммаДохода Тогда
					НоваяСтрока = НаборЗаписейОДоходах.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане");
					НоваяСтрока.Период = ДатаОперации;
					НоваяСтрока.СуммаДохода = -РезультатРаспределения[Индекс];
					Если Выборка.СуммаВычета <> 0 Тогда
						НоваяСтрока.СуммаВычета =  -РезультатРаспределенияВычета[Индекс];
					КонецЕсли;
					НоваяСтрока = НаборЗаписейОДоходах.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,ПодразделениеСотрудника,РегистрацияВНалоговомОргане");
					НоваяСтрока.Период = ДатаОперации;
					НоваяСтрока.ДокументОснование = Выборка.ДокументНачисленияАванса;
					НоваяСтрока.СуммаДохода = -РезультатРаспределенияПоДокументуАванса[Индекс];
					Если Выборка.СуммаВычета <> 0 Тогда
						НоваяСтрока.СуммаВычета =  -РезультатРаспределенияВычетаПоДокументуАванса[Индекс];
					КонецЕсли;
				Иначе
					НоваяСтрока = НаборЗаписейОДоходах.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
			...
			КонецЦикла;
		КонецЕсли;
		НаборЗаписейОДоходах.Записывать = Истина
	КонецЦикла;
	...
	Пока Выборка.Следующий() Цикл
		Если ТекущееФизическоеЛицо <> Выборка.ФизическоеЛицо Или ТекущийПериодРегистрации <> Выборка.ПериодРегистрации Тогда
			ТекущееФизическоеЛицо = Выборка.ФизическоеЛицо;
			ТекущийПериодРегистрации = Выборка.ПериодРегистрации;
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, Выборка);
			СтрокиОписания = ОписаниеРаспределения.НайтиСтроки(СтруктураПоиска); 
			Если СтрокиОписания.Количество() = 0 Тогда
				СтрокаОписания = ОписаниеРаспределения.Добавить();
			...
			КонецЕсли;
		КонецЕсли;
		Если СтрокаОписания.НетРаспределения Тогда
			Продолжить;
		ИначеЕсли Не СтрокаОписания.ТребуетсяРаспределение Тогда
			Если Выборка.СуммаПоДокументуНачисленияАванса = 0 Тогда
				НоваяСтрока = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОписания.СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.Сумма =  -НоваяСтрока.Сумма;
			ИначеЕсли Выборка.СуммаПоРегистратору = 0 Тогда
				НоваяСтрока = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОписания.СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОписания.СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.Сумма =  -Выборка.СуммаПоРегистратору;
				НоваяСтрока = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаОписания.СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
				НоваяСтрока.Период = ДатаОперации;
				НоваяСтрока.ДокументОснование = Выборка.ДокументНачисленияАванса;
				НоваяСтрока.Сумма =  -Выборка.СуммаПоДокументуНачисленияАванса;
			КонецЕсли;
		Иначе
			РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.Сумма, СтрокаОписания.Коэффициенты, 0);
			Если Выборка.СуммаПоДокументуНачисленияАванса <> 0 И Выборка.СуммаПоРегистратору <> 0 Тогда
				РезультатРаспределенияПоРегистратору = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаПоРегистратору, СтрокаОписания.Коэффициенты, 0);
				РезультатРаспределенияПоДокументуНачисленияАванса = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.СуммаПоДокументуНачисленияАванса, СтрокаОписания.Коэффициенты, 0);
			КонецЕсли;
			Индекс = 0;
			Для каждого СтрокаТекущегоРаспределения Из СтрокаОписания.ТекущееРаспределение Цикл
				Если Выборка.СуммаПоДокументуНачисленияАванса = 0 Тогда
					НоваяСтрока = НаборЗаписей.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
					НоваяСтрока.Период = ДатаОперации;
				...
				Иначе
					...
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
					НоваяСтрока.Период = ДатаОперации;
					НоваяСтрока.Сумма =  -РезультатРаспределенияПоРегистратору[Индекс];
					НоваяСтрока = НаборЗаписей.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаТекущегоРаспределения, "Подразделение,РегистрацияВНалоговомОргане");
					НоваяСтрока.Период = ДатаОперации;
					НоваяСтрока.ДокументОснование = Выборка.ДокументНачисленияАванса;
					НоваяСтрока.Сумма =  -РезультатРаспределенияПоДокументуНачисленияАванса[Индекс];
				КонецЕсли;
				Индекс = Индекс + 1;
			КонецЦикла;
		КонецЕсли;
	...
	Для Сч = 3 По 5 Цикл
		...
		Если Результаты[Сч].Пустой() Тогда
			Продолжить;
		КонецЕсли;
		
		Если Сч = 3 Тогда
			НаборЗаписей = Движения.ИмущественныеВычетыНДФЛ;
		ИначеЕсли Сч = 4 Тогда
			НаборЗаписей = Движения.ПредоставленныеСтандартныеИСоциальныеВычетыНДФЛ;
		ИначеЕсли Сч = 5 Тогда
			НаборЗаписей = Движения.АвансовыеПлатежиИностранцевПоНДФЛ;
		КонецЕсли;
		
		ТекущееФизическоеЛицо = Неопределено;
		ТекущийПериодРегистрации = '00010101';
		Выборка = Результаты[Сч].Выбрать();
		Пока Выборка.Следующий() Цикл
			Если ТекущееФизическоеЛицо <> Выборка.ФизическоеЛицо Или ТекущийПериодРегистрации <> Выборка.ПериодРегистрации Тогда
				ТекущееФизическоеЛицо = Выборка.ФизическоеЛицо;
				ТекущийПериодРегистрации = Выборка.ПериодРегистрации;
			...
			ИначеЕсли Не СтрокаОписания.ТребуетсяРаспределение Тогда
				...
				НоваяСтрока.Сумма =  -НоваяСтрока.Сумма;
			Иначе
				РезультатРаспределения = ЗарплатаКадрыКлиентСервер.РаспределитьПропорциональноКоэффициентам(Выборка.Сумма, СтрокаОписания.Коэффициенты);
				Индекс = 0;
				Для каждого СтрокаТекущегоРаспределения Из СтрокаОписания.ТекущееРаспределение Цикл
					НоваяСтрока = НаборЗаписей.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
...
Функция МежрасчетныйДокументИсчисляетНДФЛ(Организация, ИсчислятьНалогПриОкончательномРасчете, ПорядокВыплаты, СведенияОНачислениях) Экспорт
...
Функция ОтложитьУдержаниеНалогаПоМежрасчетуВыплаченномуСАвансомДоВыплатыЗарплаты(Организация) Экспорт
...
Функция ОтложитьРасчетНалогаВМежрасчетахДоРасчетаЗарплаты(Организация) Экспорт
		...
		Если Не Строка.Сторно Тогда
			Если ДополнительнаяПроверкаФиксированныхСтрок Тогда
				ДатаПолученияДоходаФиксирована = Строка.ДатаПолученияДоходаФиксирована
					И УчетНДФЛ.ДатаПолученияДоходаДолжнаБытьЗафиксирована(Строка.КатегорияДохода);
			Иначе
				ДатаПолученияДоходаФиксирована = Строка.ДатаПолученияДоходаФиксирована;
			КонецЕсли;
