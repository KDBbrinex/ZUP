...
Процедура ПередСкрытиемПерсональныхДанныхСубъектов(Субъекты, ТаблицаИсключений, Отказ) Экспорт
	...
	ПричинаОтменыСкрытия = НСтр("ru = 'субъект получал доходы, облагаемые страховыми взносами'", Метаданные.ОсновнойЯзык.КодЯзыка);
	ПолучателиДоходов = ПолучателиДоходовОблагаемыхСтраховымиВзносами(СубъектыДляСкрытия);
...
// Возвращает из числа проверяемых физических лиц тех, которые получали доходы, облагаемые страховыми взносами.
//
// Параметры:
...
//
Функция ПолучателиДоходовОблагаемыхСтраховымиВзносами(ФизическиеЛица, Организация = Неопределено, ДатаНачалаПолучения = '00010101')
	...
	|	И СтраховыеВзносыИсчисленные.Период >= &ДатаНачалаПолучения
	...
	|	И СтраховыеВзносыСведенияОДоходах.Период >= &ДатаНачалаПолучения
	|	И &ОтборПоОрганизации";
