...
Процедура ПриЗаполненииРазрешенийНаДоступКВнешнимРесурсам(ЗапросыРазрешений) Экспорт
	...
	Разрешение = РаботаВБезопасномРежиме.РазрешениеНаИспользованиеИнтернетРесурса(
		"HTTPS",
		АдресСервераTelegram(),
		ПортСоединенияTelegram(),
		НСтр("ru = 'Telegram messenger (https://telegram.org/)'"));
...
Функция ОбработатьВходящийHTTPЗапрос(HTTPВходящийЗапрос) Экспорт
	...
	Если ПустаяСтрока(ТелоЗапроса) Тогда
		...
		Возврат СтруктураОтвета;
	КонецЕсли;
	СтруктураПакета = JSONВДанные(ТелоЗапроса);
...
Процедура УстановитьWebhook(Чатбот, Отказ) Экспорт
	
	ДанныеЧатбота = Чатботы.ДанныеЧатботов()[Чатбот];
	УстановитьWebhookTelegram(Чатбот, ДанныеЧатбота, Отказ);
	
КонецПроцедуры

Функция УдалитьWebhook(Чатбот, Отказ) Экспорт
	
	ДанныеЧатбота = Чатботы.ДанныеЧатботов()[Чатбот];
	УдалитьWebhookTelegram(Чатбот, ДанныеЧатбота, Отказ);
...
Процедура ОбработатьПринятоеСообщение(Чатбот, Чат, Отправитель, ПринятоеСообщение, СообщениеОбработано)
	...
	// АПК:1223-выкл В диалогах чатботов нужны личные обращения
...
Процедура ОбработатьКомандуИсправить(Чатбот, Чат, ПринятыйТекст, СостояниеЧатбота, КомандаОбработана);
	...
	Если СостояниеЧатбота = Перечисления.СостоянияЧатаЧатбота.ЗапросСохраненияОтклика Тогда
		ТекстСообщения = НСтр("ru = 'ОК, я еще раз уточню у вас все сведения.'"); // АПК:1223 В диалогах чатботов нужны личные обращения
		ОтправитьСообщениеЧатбота(Чатбот, Чат, ТекстСообщения);
		НачатьПриемОтклика(Чатбот, Чат);
		КомандаОбработана = Истина;
	КонецЕсли;
...
Функция СохранитьВыбраннуюВакансию(ВыбраннаяВакансия, Чатбот, Чат, Отправитель)
	...
	Если НайденныеСтроки.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Похоже, выбранная вами вакансия уже недоступна. Попробуйте, выбрать другую.'"); // АПК:1223 В диалогах чатботов нужны личные обращения
		ОтправитьСообщениеЧатбота(Чатбот, Чат, ТекстСообщения);
		ВывестиСписокВакансий(Чатбот, Чат);
		Возврат Неопределено;
	КонецЕсли;
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'ОК, вы выбрали вакансию *%1*. Сейчас я приму отклик на нее.'"), // АПК:1223 В диалогах чатботов нужны личные обращения
		ВыбраннаяВакансия);
	ОтправитьСообщениеЧатбота(Чатбот, Чат, ТекстСообщения, Новый Массив);
...
Процедура ПроверитьИмяСобеседника(Имя, Чатбот, Чат, Отказ)
	...
	Если Не СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(ИмяБезПробеловТире) 
...
Процедура ПроверитьНаПустуюСтроку(СтрокаДляПроверки, Чатбот, Чат, Отказ)
	...
	Если ПустаяСтрока(СокрЛП(СтрокаДляПроверки)) Тогда
		ТекстСообщения = НСтр("ru = 'Вы не ответили. Пожалуйста, попробуйте, еще раз.'"); // АПК:1223 В диалогах чатботов нужны личные обращения
		ОтправитьСообщениеЧатбота(Чатбот, Чат, ТекстСообщения);
		Отказ = Истина;
	КонецЕсли;
...
Процедура СохранитьВозрастСобеседника(Возраст, Чатбот, Чат, Отправитель, ВопросЗадан)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Понятно, вам %1. Принято.'"), ПредставлениеВозраста); // АПК:1223 В диалогах чатботов нужны личные обращения
	ОтправитьСообщениеЧатбота(Чатбот, Чат, ТекстСообщения, Новый Массив);
...
Процедура СохранитьОтклик(Чатбот, Чат, Отправитель)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Спасибо, %1! Ваш отклик принят и будет рассмотрен службой персонала.'"), // АПК:1223 В диалогах чатботов нужны личные обращения
...
Функция ПредставлениеПринятыхСведений(ПринятыеСведения, Чатбот)
	...
	Если ЗначениеЗаполнено(ПринятыеСведения.Имя) Тогда
		ДобавитьКПредставлению(Представление, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '*%1*'"), ПринятыеСведения.Имя));
	КонецЕсли;
	
	Если ДанныеЧатбота.ЗапрашиватьВозраст И ЗначениеЗаполнено(ПринятыеСведения.Возраст) Тогда
		ДобавитьКПредставлению(Представление, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'возраст *%1*'"), ПринятыеСведения.Возраст));
	КонецЕсли;
	
	Если ДанныеЧатбота.ЗапрашиватьНомерТелефона И ЗначениеЗаполнено(ПринятыеСведения.Телефон) Тогда
		ДобавитьКПредставлению(Представление, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'телефон *%1*'"), ПринятыеСведения.Телефон));
	КонецЕсли;
	
	Если ДанныеЧатбота.ЗапрашиватьАдресЭлектроннойПочты И ЗначениеЗаполнено(ПринятыеСведения.EMail) Тогда
		ДобавитьКПредставлению(Представление, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'e-mail *%1*'"), ПринятыеСведения.EMail));
	КонецЕсли;
	
	Если ДанныеЧатбота.ЗапрашиватьИнформациюОСебе И ЗначениеЗаполнено(ПринятыеСведения.ОСебе) Тогда
		ДобавитьКПредставлению(Представление, 
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'о себе: *%1*'"), ПринятыеСведения.ОСебе));
	КонецЕсли;
...
Процедура ДобавитьКПредставлению(Представление, ЧтоДобавить)
	Представление = Представление + ?(ПустаяСтрока(Представление), "", ", ") + ЧтоДобавить;
КонецПроцедуры
...
Процедура ЗапроситьОбновленияЧатботаTelegram(Чатбот, ЕстьОбновления = Ложь)
	...
	Если СтруктураОтвета = Неопределено Тогда
		ВызватьИсключение НСтр("ru = 'Не удалось получить обновления чатбота Telegram'");
	КонецЕсли;
...
Процедура УстановитьWebhookTelegram(Чатбот, ДанныеЧатбота, Отказ)
	...
	СтруктураОтвета = ОтправитьДанныеTelegram(ДанныеЧатбота["Токен"], "setWebhook", СтруктураДанных, Истина);
	...
	Если Не СтруктураОтвета.Свойство("ok") Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура УдалитьWebhookTelegram(Чатбот, ДанныеЧатбота, Отказ)
	
	Если ДанныеЧатбота["Тип"] <> Перечисления.ТипыЧатботов.Telegram Тогда
		Возврат;
	КонецЕсли;
	
	СтруктураОтвета = ОтправитьДанныеTelegram(ДанныеЧатбота["Токен"], "deleteWebhook", , Истина);
...
Процедура ПроверитьWebhookTelegram(Чатбот, ДанныеЧатбота, РезультатПроверки)
	...
	СтруктураОтвета = ОтправитьДанныеTelegram(ДанныеЧатбота["Токен"], "getWebhookInfo", , Истина);
...
Функция ОтправитьДанныеTelegram(Токен, ИмяМетода, СтруктураДанных = Неопределено, ПоказыватьСообщения = Ложь)
	...
	Исключение
		ЗарегистрироватьОшибкуTelegram(ИнформацияОбОшибке(), ПоказыватьСообщения);
		Возврат Неопределено;
	КонецПопытки;
...
Процедура ЗарегистрироватьОшибкуTelegram(ИнформацияОбОшибке, ПоказыватьСообщения = Ложь)
	
	ИмяСобытия = НСтр("ru = 'Обращение к Telegram'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ошибка при обращению к сервису Telegram. Обратитесь к администратору.
...
Функция СоединениеTelegram()
	...
	Прокси = ПолучениеФайловИзИнтернета.ПолучитьПрокси("http://" + АдресСервераTelegram());
