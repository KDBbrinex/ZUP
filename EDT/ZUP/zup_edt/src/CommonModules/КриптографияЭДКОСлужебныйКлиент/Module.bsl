...
Процедура СоздатьМенеджерКриптографииПослеОпределенияАлгоритма(Результат, ВходящийКонтекст) Экспорт
	...
	Иначе
		...
		РезультатЗаполнения = ЗаполнитьПараметрыКриптографии(Алгоритм, ПредлагатьУстановкуРасширения);
		ДоступныеКриптопровайдеры = РезультатЗаполнения.ДоступныеКриптопровайдеры;
		
		Если ДоступныеКриптопровайдеры = Неопределено Тогда
			ВывестиСообщениеПриНеобходимости(ВходящийКонтекст.ВыводитьСообщение, РезультатЗаполнения.ОписаниеОшибки);
			
		Иначе
			ТипКриптопровайдера = ПараметрыПриложения["КриптографияЭДКО.ТипКриптопровайдера"];
...
Процедура СоздатьМенеджерКриптографииПослеПолученияСпискаКриптопровайдеров(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		Если КриптопровайдерУстановленВСистеме Тогда
			Если ВходящийКонтекст.ПараметрыКриптографии = Неопределено Тогда
				ПараметрыПриложения.Вставить("КриптографияЭДКО.ТипКриптопровайдера", ТекущийКриптопровайдер.ТипКриптопровайдера);
			КонецЕсли;
			РезультатВыполнения = Новый Структура("Алгоритм, ТипКриптопровайдераВладельца", ТекущийКриптопровайдер.Алгоритм,
...
Функция СгенерироватьИмяКонтейнера(ОсноваИмени) Экспорт
	...
	ПрефиксИмени = ?(ОбщегоНазначенияКлиент.ЭтоLinuxКлиент(), "\\.\HDIMAGE\", "");
...
Процедура СоздатьКонтейнерИЗапросНаСертификат(
	...
	Флаг = 67108864 + 65536; // Квалифицированная ЭП, экспортируемый ключ
...
Процедура ПолучитьСертификатыПослеПолученияСертификатов(Результат, Параметры, ВходящийКонтекст) Экспорт
	...
	Сертификаты = КриптографияЭДКОСлужебныйВызовСервера.ПреобразоватьСертификатыНаСервере(Результат);
...
Процедура НайтиСертификатыПоОтпечаткам(
	...
	Если ОсталосьНайти.Количество() > 0 Тогда
		...
		КонецЕсли;
		КриптографияЭДКОКлиент.ПолучитьСертификаты(Оповещение, Хранилище);
...
Процедура ВыбратьСертификат(ОповещениеОЗавершении, ЭтоЭлектроннаяПодписьВМоделиСервиса, НачальноеЗначениеВыбора, ХранилищеСертификатов, 
		ПоказыватьПросроченные, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора, ОтпечаткиЗапрещенныхСертификатов) Экспорт
	
	ПараметрыФормы = Новый Структура("Хранилище, ПоказыватьПросроченные, НачальноеЗначениеВыбора, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора, ОтпечаткиЗапрещенныхСертификатов",
									ХранилищеСертификатов, ПоказыватьПросроченные, НачальноеЗначениеВыбора, МножественныйВыбор, Отпечатки, ЗапретитьВыборПросроченных, БезВозможностиВыбора, ОтпечаткиЗапрещенныхСертификатов);
...
Процедура ВыбратьСертификатПослеВыбора(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат = Неопределено Тогда
		ВыполнитьОбработкуОповещения(
			ВходящийКонтекст.ОповещениеОЗавершении,
			КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Ложь,, ВходящийКонтекст));
	Иначе
		ВыполнитьОбработкуОповещения(
			ВходящийКонтекст.ОповещениеОЗавершении,
			КриптографияЭДКОКлиентСервер.ПодготовитьРезультат(Истина, "ВыбранноеЗначение", Результат, ВходящийКонтекст));
	КонецЕсли;
...
Процедура ПодписатьPKCS7ПослеСозданияМенеджераКриптографии(Результат, ВходящийКонтекст) Экспорт
	...
	Если Результат.Выполнено Тогда
		ВходящийКонтекст.Вставить("МенеджерКриптографии", 			Результат.МенеджерКриптографии);
		ВходящийКонтекст.Вставить("Алгоритм", 						Результат.Алгоритм);
		ВходящийКонтекст.Вставить("ТипКриптопровайдераВладельца", 	Результат.ТипКриптопровайдераВладельца);
...
Процедура ПроверитьПодписьPKCS7(
	...
	ВходящиеПараметры = ПараметрыВМассив(5, ЭтоЭлектроннаяПодписьВМоделиСервиса, ФайлПодписи, ФайлДанных, ИмпортироватьСертификаты, Алгоритм);
...
Процедура ЗашифроватьPKCS7(
	...
	ВходящиеПараметры = ПараметрыВМассив(5, ИсходныйФайл, Сертификаты, ИмяФайлаИлиРасширениеИлиАдрес, КлючевойКонтейнер, Алгоритм);
	
	ТипыПодходятДляЭПСервиса = (ТипЗнч(ИсходныйФайл) = Тип("Строка") И КриптографияЭДКОКлиентСервер.ЭтоАдресВоВременномХранилище(ИсходныйФайл))
	...
	Если СертификатыВЗащищенномХранилищеНаСервере И ТипыПодходятДляЭПСервиса И 
	...
	Иначе
		Если Алгоритм = "*" Тогда
			ПараметрыОпределенияАлгоритма = Новый Структура("Сертификат", ?(Сертификаты.Количество() >= 1, Сертификаты[0], Неопределено));
		Иначе
			ПараметрыОпределенияАлгоритма = Новый Структура("Алгоритм", Алгоритм);
		КонецЕсли;
		ПредварительноСоздатьМенеджерКриптографии(
...
Процедура ЗашифроватьPKCS7ПослеВыполнения(Результат, Параметры, ВходящийКонтекст) Экспорт
	...
	ИначеЕсли КриптографияЭДКОКлиентСервер.ЭтоАдресВоВременномХранилище(ВходящийКонтекст.ВходящиеПараметры[0])
...
Процедура РасшифроватьPKCS7(
	...
	Если Сертификат = Неопределено Или СертификатВХранилище Тогда
		...
		ПараметрыЛокальнойРасшифровки.Вставить("ВходящиеПараметры", ВходящиеПараметры);
		ПараметрыЛокальнойРасшифровки.Вставить("МенеджерКриптографии", МенеджерКриптографии);
		ПараметрыЛокальнойРасшифровки.Вставить("Алгоритм", Алгоритм);
...
Процедура ХешироватьДанныеСервисПередЗавершением(Результат, ВходящийКонтекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если ТипЗнч(Хеш) = Тип("ДвоичныеДанные") Тогда
			...
			Если ВходящийКонтекст.АлгоритмХешированияВМоделиСервиса <> "GOST R 34.11-94" Тогда
...
Процедура ОпределитьАлгоритмПослеЭкспортаСертификата(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Выполнено Тогда
		...
		СвойстваСертификата = КриптографияЭДКОСлужебныйВызовСервера.ПолучитьСвойстваСертификата(
			Результат.СтрокаBase64, Настройки);
...
Функция ОписаниеОшибкиНеУдалосьПодключитьВнешнююКомпонентуКриптографииНеНастроенПуть()
	...
	Возврат НСтр("ru='Не удалось подключить внешнюю компоненту для работы с криптографией: не настроен путь к программе криптографии.'");
...
Процедура ПредварительноСоздатьМенеджерКриптографии(
	...
	Если МенеджерКриптографии <> Неопределено Тогда
		...
		КонтекстДляАлгоритма = Новый Структура("Алгоритм", Алгоритм);
...
Функция ЗаполнитьПараметрыКриптографии(Алгоритм = "", ВыводитьДиалоги = Ложь)
...
Процедура ЗаполнитьПарольДоступаКЗакрытомуКлючу(ОповещениеОЗавершении, Сертификат, МенеджерКриптографии)
	...
	Если ОбщегоНазначенияКлиент.ЭтоLinuxКлиент() И НЕ ПоддерживаетсяЗапросПароляВнешнейКомпонентой Тогда
