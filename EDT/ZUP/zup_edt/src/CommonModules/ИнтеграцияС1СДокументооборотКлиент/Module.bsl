...
Процедура НачатьСозданиеСвязанногоОбъектаДО(ОбъектИС, ОписаниеОповещения, ОткрытьФорму = Ложь) Экспорт
	...
	Если Правила.Количество() = 0 Тогда
		...
		СозданныеПравила = ИнтеграцияС1СДокументооборотВызовСервераПереопределяемый.СоздатьПравилаИнтеграцииАвтоматически(ОбъектИС);
...
Процедура ОткрытьСостоянияДокумента(Форма, ТолькоПросмотр = Ложь) Экспорт
	...
	Состояния.Вставить("Регистрация");
	Состояния.Вставить("Рассмотрение");
	Состояния.Вставить("Согласование");
	Состояния.Вставить("Утверждение");
	Если Форма.Тип = "DMInternalDocument" Тогда
		Состояния.Вставить("Подписание");
	КонецЕсли;
	Состояния.Вставить("Исполнение");
...
Процедура ОткрытьСостоянияДокументаЗавершение(Результат, Форма) Экспорт
	...
	Реквизиты.Вставить("Регистрация");
	Реквизиты.Вставить("Рассмотрение");
	Реквизиты.Вставить("Согласование");
	Реквизиты.Вставить("Утверждение");
	Если Форма.Тип = "DMInternalDocument" Тогда
		Реквизиты.Вставить("Подписание");
	КонецЕсли;
	Реквизиты.Вставить("Исполнение");
...
Процедура НачатьСогласованиеПослеВопроса(Результат, Параметры) Экспорт
	...
	Если ПодходящиеШаблоны.Количество() = 0 Тогда
		...
		ОткрытьОбъект("DMBusinessProcessApproval", Неопределено, , 
...
Процедура ПолучениеФайлаПослеПолученияИзДокументооборота(Параметры) Экспорт
	...
	Если Не ПолучитьФайлы(ПолучаемыеФайлы,,, Ложь) Тогда // не обрабатываемая ошибка получения
		ПолучитьФайлыНачало(Параметры);
	КонецЕсли;
...
Процедура ЗакончитьРедактированиеСРасширениемПослеПомещения(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
	...
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		Параметры.Вставить("Текст", ИзвлечьТекст(Параметры.СведенияОРедактировании.ПолныйПуть));
	Иначе
...
Процедура СохранитьКакПослеДиалогаВыбораФайла(Результат, Параметры) Экспорт
	...
	Иначе
		...
		Параметры.Вставить("ПолныйПуть", Параметры.Каталог + ПолучитьРазделительПути() + Параметры.ИмяФайла);
	КонецЕсли;
	
	Параметры.Вставить("ПолноеИмяФайла", Параметры.ПолныйПуть);
...
Процедура ОбновитьИзФайлаНаДиске(Идентификатор, Имя, Расширение, УникальныйИдентификаторФормы,
	...
	Если Не РасширениеРаботыСФайламиПодключено() Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения(
			...
			Параметры);
		Попытка
			НачатьПомещениеФайла(ОписаниеОповещения,,,Истина, Параметры.УникальныйИдентификаторФормы);
		Исключение
			ТекстПредупреждения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	...
	Если ИнтеграцияС1СДокументооборотКлиентПовтИсп.ДоступенФункционалВерсииСервиса("1.4.9.1") Тогда
		...
		Иначе
			ИмяФайла = СведенияОРедактировании.РекомендуемаяПапка + ПолучитьРазделительПути() + ИмяФайла;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьИзФайлаНаДискеПослеПомещенияФайлаСРасширением(Результат, Адрес, ПолныйПуть, Параметры) Экспорт
	...
	НастройкиДокументооборот = ИнтеграцияС1СДокументооборотКлиентПовтИсп.ПолучитьНастройки();
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		Параметры.Вставить("Текст", ИзвлечьТекст(ПолныйПуть));
	Иначе
		Параметры.Вставить("Текст", "");
	КонецЕсли;
...
//   ОписаниеОповещения - описание оповещения, вызываемого при успешном создании файла.
//
...
Процедура СоздатьФайлСДиска(УникальныйИдентификаторФормы, ID, Тип, Представление,
	Владелец, ОписаниеОповещения = Неопределено, ЯвляетсяСканКопией = Ложь) Экспорт
...
Процедура СоздатьФайлСДискаПослеПомещенияФайла(ПомещенныеФайлы, Параметры) Экспорт
	...
	НастройкиДокументооборот = ИнтеграцияС1СДокументооборотКлиентПовтИсп.ПолучитьНастройки();
	Если НастройкиДокументооборот.НужноИзвлечьТекст Тогда
		Параметры.Вставить("Текст", ИзвлечьТекст(Параметры.ПолноеИмяФайла));
	Иначе
		Параметры.Вставить("Текст", "");
	КонецЕсли;
...
#Если Не ВебКлиент И НЕ МобильныйКлиент Тогда
	...
	Если РасширениеФайлаВСписке Тогда
		Возврат РаботаСФайламиСлужебныйКлиентСервер.ИзвлечьТекстИзТекстовогоФайла(ПолноеИмяФайла,
			Неопределено, 
			Отказ);
	КонецЕсли;
...
//	ДанныеПодписейФайла - Массив - существующие подписи
...
//
// Параметры:
//	 ОписаниеОповещенияОЗакрытии - ОписаниеОповещения - описание оповещения при закрытии формы
	...
	Для каждого ОбъектДляПодписи Из МассивОбъектовДляПодписи Цикл
		...
		ИначеЕсли ОбъектДляПодписи.ОбъектТип = "DMFile" Тогда
			...
			Если Строки.Количество() = 1 Тогда
				ПредставлениеФайла = НСтр("ru = 'Файл: '") + Строки[0].Наименование;
				ДанныеДляПодписи.Вставить("Представление",
...
//
Функция ОбщиеНастройкиЭП() Экспорт
