...
Процедура ПарсингТекста(СтруктураРеквизитов, Знач ТекстРезюме, Знач ШаблонРезюме) Экспорт
	...
	СтруктураДанныхКандидата.ИсточникИнформации = ШаблонРезюме;
...
//  Структура - структура с данными переданного вложения, если тип параметра не 
			...
			Если ПравилоПоиска.ЯвляетсяЗначением = Истина Тогда
				Пока ПозНач > 0 Цикл
					...
					Если ПозКон = 0 Тогда // Если в строке нет переноса строки, то берем весь оставшийся текст после текущего блока.
						ЗначениеИзТекста = СокрЛП(Сред(ТекстДляРаспознавания, ПозНач + СтрДлина(КлючПоиска)));
						Прервать;
					ИначеЕсли СтрДлина(КлючПоиска) <> (ПозКон-ПозНач) Тогда
...
Процедура ПолучитьСоставБлокаОпытаРаботыДляHH(СоставБлока)
	...
	ДобавитьСтрокуБлокаОпытаРаботы(СоставБлока, 3, "Организация",	"ПроизвольнаяСтрока",	Истина, "", "", 1);
...
Функция СписокСоответствийПола()
	...
	СоответствиеКлассификаторов.Вставить("мужчина", ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Мужской"));
	СоответствиеКлассификаторов.Вставить("мужской", ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Мужской"));
	
	СоответствиеКлассификаторов.Вставить("женщина", ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Женский"));
	СоответствиеКлассификаторов.Вставить("женский", ПредопределенноеЗначение("Перечисление.ПолФизическогоЛица.Женский"));
...
Функция СписокСоответствийГражданства()
	...
	СоответствиеКлассификаторов.Вставить("рф", ПредопределенноеЗначение("Справочник.СтраныМира.Россия"));
	СоответствиеКлассификаторов.Вставить("россия", ПредопределенноеЗначение("Справочник.СтраныМира.Россия"));
	СоответствиеКлассификаторов.Вставить("русский", ПредопределенноеЗначение("Справочник.СтраныМира.Россия"));
