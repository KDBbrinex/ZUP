...
Функция МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицыВидовРасчета, ИмяПВР = "Начисления", РежимРаботы = 0) Экспорт
	...
	Если ОписаниеТаблицыВидовРасчета.Свойство("КоличествоОтображаемыхПоказателей") Тогда
		Возврат ОписаниеТаблицыВидовРасчета.КоличествоОтображаемыхПоказателей;
	КонецЕсли;
...
Процедура УстановитьКомментарииДействийСНачислениями(Форма, ОписаниеТаблицыВидовРасчета, РежимРаботы, ДатаСобытия = Неопределено, ДатаОкончания = Неопределено) Экспорт
	...
	Для Каждого СтрокаНачисления Из Начисления Цикл
		...
		УстановитьКомментарииДействийСНачислением(СтрокаНачисления,
			МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицыВидовРасчета, , РежимРаботы),
...
Процедура УстановитьОтметкуЗапрашиватьПриВводе(ЗапрашиваемыеПоказатели, Показатели) Экспорт
	...
	Для Каждого СтрокаПоказателей Из Показатели Цикл
		СтрокаПоказателей.ЗапрашиватьПриВводе = 
			СтрокаПоказателей.ЗапрашиватьПриВводе Или ЗапрашиваемыеПоказатели.Найти(СтрокаПоказателей.Показатель) <> Неопределено;
	КонецЦикла;
...
// Возвращает описание существующих в системе многофункциональных документов
// в виде соответствия Тип - Описание
//
// Возвращаемое значение:
// 	Соответствие - содержит:
// 		* Ключ     - Тип       - тип многофункционального документа
// 		* Значение - Структура - см. ОписаниеМногофункциональногоДокумента()
//
Функция ТипыМногофункциональныхДокументов() Экспорт 
	...
	МногофункциональныеДокументы.Вставить(Тип("ДокументСсылка.ПрогулНеявкаСписком"), ОписаниеДокумента);
	...
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.Подработки") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			МногофункциональныеДокументы, 
			ОбщегоНазначенияБЗККлиентСервер.ОбщийМодуль("ПодработкиКлиентСервер").ТипыМногофункциональныхДокументов(), 
			Истина);
	КонецЕсли;
	
	// АттестацииСотрудников
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.АттестацииСотрудников") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			МногофункциональныеДокументы, 
			ОбщегоНазначенияБЗККлиентСервер.ОбщийМодуль("АттестацииСотрудниковКлиентСервер").ТипыМногофункциональныхДокументов(), 
			Истина);
	КонецЕсли;
	...
	Если ОбщегоНазначенияБЗККлиентСервер.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		ОбщегоНазначенияКлиентСервер.ДополнитьСоответствие(
			МногофункциональныеДокументы, 
			ОбщегоНазначенияБЗККлиентСервер.ОбщийМодуль("ГосударственнаяСлужбаКлиентСервер").ТипыМногофункциональныхДокументов(), 
			Истина);
	КонецЕсли;
...
// Конструктор описания многофункционального документа
...
// 		* РеквизитСостояние            - Строка               - Имя реквизита с признаком утверждения.
// 		                                                        Реквизит должен иметь тип Булево.
// 		* ВторойОтветственный          - Строка, Неопределено - Имя реквизита со вторым ответственным.
// 		                                                        Реквизит должен иметь тип СправочникСсылка.Пользователи
// 		* ВидУчета                     - Строка - Имя учета, к которому относится документ.
// 		                                          Допустимые значения:
// 		                                              "НачисленнаяЗарплата", 
// 		                                              "ПлановыеНачисления", 
// 		                                              "РабочееВремя", 
// 		                                              "КадровыеДанные"
// 		* СообщениеДокументНеУтвержден - Строка - сообщение о том, что документ не утвержден
//
Функция ОписаниеМногофункциональногоДокумента() Экспорт
	
	ОписаниеМногофункциональногоДокумента = Новый Структура;
	ОписаниеМногофункциональногоДокумента.Вставить("РеквизитСостояние",            "");
	ОписаниеМногофункциональногоДокумента.Вставить("ВторойОтветственный",          Неопределено);
	ОписаниеМногофункциональногоДокумента.Вставить("ВидУчета",                     "");
	ОписаниеМногофункциональногоДокумента.Вставить("СообщениеДокументНеУтвержден", "");
	
	Возврат ОписаниеМногофункциональногоДокумента;
...
Функция ИменаКолонокТаблицыРаспределенияРезультатовРасчета(ОписаниеТаблицы) Экспорт
	...
	Если ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовНачислений" Тогда
		ИменаКолонок = ИменаКолонокТаблицыРаспределенияРезультатовРасчетаНачислений();
	ИначеЕсли ИмяТаблицыРаспределениеРезультатов = "РаспределениеРезультатовУдержаний" Тогда
		ИменаКолонок = ИменаКолонокТаблицыРаспределенияРезультатовРасчетаУдержаний();
	КонецЕсли;
	
	Возврат ИменаКолонок;
КонецФункции

Функция ИменаКолонокТаблицыРаспределенияРезультатовРасчетаНачислений() Экспорт
	Возврат ИменаКолонокТаблицыРаспределенияРезультатовРасчетаОбщие() + ",Территория,СпособОтраженияЗарплатыВБухучете,ОблагаетсяЕНВД,ПодразделениеУчетаЗатрат"; 
КонецФункции
...
Функция ПредопределенныйЭлемент(ПолноеИмяПредопределенного) Экспорт
	...
	#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
		ПредопределенноеЗначение = ОбщегоНазначения.ПредопределенныйЭлемент(ПолноеИмяПредопределенного);
	#Иначе
		ПредопределенноеЗначение = ОбщегоНазначенияКлиент.ПредопределенныйЭлемент(ПолноеИмяПредопределенного);
	#КонецЕсли
...
Процедура ВводНачисленийИнициализироватьСтрокиНачислений(Форма, СтрокиНачислений, ОписаниеТаблицы, РежимРаботы, Сотрудник = Неопределено, ДатаСведений = Неопределено) Экспорт
	...
	Для Каждого СтрокаНачислений Из СтрокиНачислений Цикл
		...
		Если МаксимальноеЧислоПоказателей > МаксимальноеКоличествоПоказателейПоОписаниюТаблицы(Форма, ОписаниеТаблицы,, РежимРаботы) Тогда
			ТекстПредупреждения = ТекстПредупрежденияОписаниеНачисленийИзменилось();
...
Функция ТекстПредупрежденияОписаниеНачисленийИзменилось() Экспорт
	...
	ТекстПредупреждения = НСтр("ru = 'За время работы с документом существенно изменилось описание начислений. 
                                |Это привело к тому, что необходимо завершить работу с документом и повторно открыть его или перечитать данные документа (команда «Перечитать» из меню «Еще»).'");
...
Функция ПродолжительностьСтажа(Знач ОписаниеСтажа, Знач ДатаАктуальности) Экспорт 
	...
	Если ДатаАктуальности = ОписаниеСтажа.ДатаОтсчета Или ОписаниеСтажа.Прерван Тогда
		Возврат ПродолжительностьСтажаПоДаннымОСтаже(ОписаниеСтажа.Лет, ОписаниеСтажа.Месяцев, ОписаниеСтажа.Дней);
	КонецЕсли;
...
Функция ПродолжительностьСтажаПоДаннымОСтаже(Лет = 0, Месяцев = 0, Дней = 0) Экспорт
	
	ПродолжительностьСтажа = Новый Структура("Лет, Месяцев, Дней", 0, 0, 0);
	Если Лет <> 0 Или Месяцев <> 0 Или Дней <> 0 Тогда
		
		РазмерМесяцев = Месяцев + Лет * 12;
		
		ПродолжительностьСтажа.Лет = Цел(РазмерМесяцев / 12);
		ПродолжительностьСтажа.Месяцев = РазмерМесяцев - ПродолжительностьСтажа.Лет * 12;
		ПродолжительностьСтажа.Дней = Дней;
...
Процедура УстановитьСостояниеКнопкиФормыРассчитать(Форма, РасчетНеобходимоВыполнить, ИмяКнопкиРассчитать = "Рассчитать") Экспорт
	...
	Если РасчетНеобходимоВыполнить Тогда
		...
		ЦветФонаКнопки = ЦветСтиля("ВыделеннаяКнопкаФон");
	Иначе
		КартинкаКнопки = БиблиотекаКартинок.ПересчитатьЗарплатуСерая;
		ЦветФонаКнопки = ЦветСтиля("ЦветФонаКнопки");
	КонецЕсли;
...
Функция ИменаКолонокТаблицыРаспределенияРезультатовРасчетаУдержаний()
	Возврат ИменаКолонокТаблицыРаспределенияРезультатовРасчетаОбщие() + ",ВидУдержания,Сотрудник,Подразделение,ПодразделениеУчетаЗатрат,ВидДоходаИсполнительногоПроизводства"; 
КонецФункции

Функция ИменаКолонокТаблицыРаспределенияРезультатовРасчетаОбщие()
	Возврат "СтатьяФинансирования,СтатьяРасходов,КодСтатьиФинансирования,Результат";
КонецФункции	
