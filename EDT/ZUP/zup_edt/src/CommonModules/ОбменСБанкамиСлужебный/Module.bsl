...
Процедура СинхронизироватьДокументыСбербанк(Параметры, АдресРезультата) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Параметры.НастройкаОбмена, "ИдентификаторОрганизации, Организация, Банк");
	ТекстЗапроса = ОбменСБанкамиСлужебныйВызовСервера.ТекстЗапросаНочнойВыписки(Параметры.НастройкаОбмена, ИдентификаторЗапроса, РеквизитыНастройкиОбмена.ИдентификаторОрганизации);
	Данные = Новый Массив;
	Данные.Добавить(ТекстЗапроса);
	...
	Для каждого ЭлементКоллекции Из ЗапросыВыписок Цикл
		СтрокаXML = ОбменСБанкамиСлужебныйВызовСервера.ПакетXMLСбербанка(ЭлементКоллекции, Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ФродПараметры, Параметры.ТекущаяСессия.Логин);
	...
	ОтправитьДанныеВСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, Данные, Тикеты, ТребуетсяАутентификация);
	...
	ПолучитьИзвещенияПоТикетамСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, РеквизитыНастройкиОбмена.ИдентификаторОрганизации, МассивТикетов, Результат.КоличествоПолученных);

	ПолучитьСтатусыДокументовСбербанк(Параметры.НастройкаОбмена, Параметры.ТекущаяСессия.ИдентификаторСессии, Результат.КоличествоПолученных);
...
Функция ЗапросыВыписокСбербанк(Знач НастройкаОбмена) Экспорт
	...
	РеквизитыНастройкиОбмена = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(НастройкаОбмена, "Организация, Банк, ИспользуетсяКриптография");
	...
	Для Счетчик = 1 По КоличествоДней Цикл
		Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
			...
			Если РеквизитыНастройкиОбмена.ИспользуетсяКриптография Тогда
				ДайджестBase64 = ДайджестСбербанк(Перечисления.ВидыЭДОбменСБанками.ЗапросВыписки, АдресФайла, НастройкаОбмена);
...
Процедура ОбработатьЭлектронныеДокументы(Параметры, АдресРезультата) Экспорт
	...
	// Основной запрос - выборка из временных таблиц:
		...
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПроверкаПользователя", "Сертификаты.Пользователь В (&ПустойПользователь, &ТекущийПользователь, &ПользовательНеУказан)");
		КонецЕсли;
		...
		Если Выборка.Количество() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ВыборкаПоСертификатам.Количество() = 0
						И ЗначениеЗаполнено(СообщениеОбмена) Тогда
				...
				Пока ВыборкаПоСертификатам.Следующий() Цикл
					...
					Если ПервыйСертификат Тогда
						...
						КонецЕсли;
						ИДМассиваСообщений = ИДБазовый + Строка(ВыборкаПоСертификатам.НастройкаОбмена.УникальныйИдентификатор());
				...
				КонецЕсли;
			КонецЦикла;
		ИначеЕсли МассивСсылокНаОбъект.Количество() = 1 И НастройкиОбмена.ПараметрыОбмена.Подписывать Тогда
			СообщениеОбмена = СообщениеОбменаПоВладельцу(МассивСсылокНаОбъект.Получить(0));
			Если ЗначениеЗаполнено(СообщениеОбмена) 
				...
				СертификатыМаршрута = Результат[8].Выгрузить();
				...
				НастройкаОбмена = НастройкиОбъектов.Получить(МассивСсылокНаОбъект.Получить(0)).НастройкаОбмена;
	...
	Если ЭлектронноеВзаимодействиеКлиентСервер.ЕстьДействие(Действия, "Отправить") Тогда
		...
		Иначе
			Запрос.Текст = СтрЗаменить(Запрос.Текст,
				"&ПроверкаПользователя", "Сертификаты.Пользователь В (&ПустойПользователь, &ТекущийПользователь, &ПользовательНеУказан)");
			Запрос.УстановитьПараметр("ТекущийПользователь", Пользователи.АвторизованныйПользователь());
			Запрос.УстановитьПараметр("ПустойПользователь", Справочники.Пользователи.ПустаяСсылка());
			Запрос.УстановитьПараметр("ПользовательНеУказан", Пользователи.СсылкаНеуказанногоПользователя());
		КонецЕсли;
		...
		СоотвНастроекОбменаИСоответствийСертификатовИПараметров = СоотвНастроекОбменаИСоответствийСертификатовИПараметровДляАвторизацииСервер(МассивНастроекОбмена, СтМассивовСтруктурСертификатов, СоотвСертификатовИПаролей);
		...
		Если ТЗ_ЭД_КОтправке.Количество() > 0 Тогда
			...
			Для Каждого НастройкаОбмена Из МассивНастроекОбмена Цикл
				...
				Иначе
					...
					ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ИсходныеДанные.МассивСообщенийТребующихПодтверждение, МассивСообщенийТребующихПодтверждение, Истина);
				...
				// Если есть активная сессия, то документ отправляется в банк без возврата на клиента.
					...
					Если ТекущаяСессия <> Неопределено Тогда
						...
						Если НЕ РезультатОтправки.ТребуетсяАутентификация Тогда
							...
							ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ДанныеДляОтправки.МассивСообщенийТребующихПодтверждение, ТребуютSMS);
						КонецЕсли;
			...
			Для Каждого Элемент Из СтруктураКОтправке Цикл
				...
				Если ТипЗнч(МассивСообщенийОбмена) = Тип("Массив")
...
Процедура СохранитьСообщениеОбмена(Знач ПараметрыСообщения, НовоеСообщениеСсылка = Неопределено) Экспорт
	...
	Если ЕстьТакоеСообщениеВИнформационнойБазе(ПараметрыСообщения) Тогда
		Возврат;
	КонецЕсли;
	...
	Если ПараметрыСообщения.Свойство("АдресФайлаВоВременномХранилище") Тогда
		Название = СтроковыеФункции.СтрокаЛатиницей(Строка(НовоеСообщениеСсылка));
...
Функция ОтправкаПакетовВБанк(МассивПакетов, ИдентификаторСессии = Неопределено, ТекстСообщения = "") Экспорт
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетов Цикл
		...
		Попытка
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ПакетОбменСБанками, Перечисления.СтатусыПакетовЭД.Отправлен, СтруктураЭД);
			...
			Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
				...
				Если ЕстьПлатежныйДокументТребующийПодтверждениеВЛК(СообщенияОбмена, ВыводитьОкноПодтверждения) Тогда
					СтруктураВозврата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК.Добавить(РеквизитыНастройкиОбмена.Банк);
				КонецЕсли;
			...
			Если ЗначениеЗаполнено(ТекстОшибки) Тогда
				...
				ОбменСБанкамиСлужебныйВызовСервера.УстановитьСтатусПакета(ПакетОбменСБанками, Перечисления.СтатусыПакетовЭД.Отменен);
		...
		Исключение
			...
			ОбменСБанкамиСлужебныйВызовСервера.ОбновитьСтатусыДокументовПакетаЭДО(ПакетОбменСБанками, Перечисления.СтатусыПакетовЭД.Отменен, СтруктураРеквизитов);
		КонецПопытки;
		...
		Попытка
			...
			Иначе
				...
				ОбменСБанкамиСлужебныйВызовСервера.ОбработатьОтветБанкаНаОтправкуДокументаAsync(ОтветБанка, ПакетОбменСБанками, ВремФайл);
			КонецЕсли;
		Исключение
	...
	Если МассивОтправленныхСообщенийОбмена.Количество() Тогда
		ОтправленныеДокументы = ОбменСБанкамиСлужебныйВызовСервера.ВладельцыСообщенийОбмена(МассивОтправленныхСообщенийОбмена);
		СтруктураВозврата.Вставить("ОтправленныеДокументы", ОтправленныеДокументы);
	КонецЕсли;

	СтруктураВозврата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК = ОбщегоНазначенияКлиентСервер.СвернутьМассив(СтруктураВозврата.ПодтверждениеПлатежейВБанке.БанкиТребующиеПодтверждениеПлатежаВЛК);
...
//
Функция ОтправитьPOSTЗапрос(АдресСервера, Ресурс, Заголовки, Данные,
		ПолучитьТелоКакСтроку = Ложь, Таймаут = 60,
	...
	Возврат ОтправитьPOSTЗапросРекурсивно(ПараметрыОтправки, ПолучитьТелоКакСтроку, Таймаут, ПолучитьЛог, НастройкаОбмена, Перенаправления)
КонецФункции
...
Функция ДействующиеПараметрыОбмена(СсылкаНаВладельца, СообщениеОбмена = Неопределено, ВидЭД = Неопределено) Экспорт
	...
	НастройкаОбмена = ОбменСБанками.НастройкаОбмена(
		СтруктураВозврата.ПараметрыЭД.Организация, СтруктураВозврата.ПараметрыЭД.Банк);
...
//
Процедура СохранитьДатуПоследнейОперации(НастройкаОбмена, ДатаПоследнейОперации) Экспорт
...
// Параметры:
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//  СтруктураРазбораФайла - Структура - содержит дерево разбора и текущую строку разбора.
//
Процедура ОпределитьИсполненныеПлатежныеПоручения(НастройкаОбмена, СтруктураРазбораФайла) Экспорт

	МассивВнешнихИдентификаторов = МассивИдентификаторовВыпискиБанка(НастройкаОбмена, СтруктураРазбораФайла);
...
//  АдресХранилища - Строка - адрес временного хранилища для помещения результата.
//
Процедура ОтправитьЗапросВыпискиАсинхронныйОбмен(СтруктураПараметров,
...
Функция СформироватьXMLФайлыДокументов(МассивОбъектовДляВыгрузки, НастройкиОбъектов, ДопПараметры = "")
...
Процедура ПрочитатьОперациюВыписки(ЭД, Operation, НовыйЭД)
	...
	РеквизитыПлатежа.Добавить(ОбменСБанкамиКлиентСервер.ЧислоИзСтроки(PayData.Sum), "Сумма");
	...
	КодВидаДохода = ЗначениеСвойстваXDTO(PayData, "IncomeTypeCode");
	Если КодВидаДохода <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(КодВидаДохода, "КодВидаДохода");
	КонецЕсли;
	...
	Если НазначениеПлатежа <> Неопределено Тогда
		...
		Для Сч = 1 По КоличествоСтрокНП Цикл
			РеквизитыПлатежа.Добавить(СтрПолучитьСтроку(НазначениеПлатежа, Сч), "НазначениеПлатежа"
				+ Сч);
		КонецЦикла;
	КонецЕсли;
	...
	Иначе
		...
		РеквизитыПлатежа.Добавить(СтатусСоставителя, "СтатусСоставителя");
		...
		РеквизитыПлатежа.Добавить(ЗначениеЗаполнено(СтатусСоставителя), "ЭтоПлатежВБюджет");
	КонецЕсли;
	...
	Если УсловиеОплаты <> Неопределено Тогда
		...
		Для Сч = 1 По КоличествоСтрок Цикл
			РеквизитыПлатежа.Добавить(СтрПолучитьСтроку(УсловиеОплаты, Сч), "УсловиеОплаты"
				+ Сч);
		КонецЦикла;
	КонецЕсли;
	...
	Если СрокАкцепта <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(ОбменСБанкамиКлиентСервер.ЧислоИзСтроки(СрокАкцепта), "СрокАкцепта");
	КонецЕсли;
	...
	Если СуммаОстаткаПлатежа <> Неопределено Тогда
		РеквизитыПлатежа.Добавить(ОбменСБанкамиКлиентСервер.ЧислоИзСтроки(СуммаОстаткаПлатежа), "СуммаОстаткаПлатежа");
	КонецЕсли;
	...
	Если ЭтоМемориальныйОрдер Тогда
		...
		Если Payee <> Неопределено Тогда
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payee, "Name"), СторонаБанка
				+ "Наименование");
			РеквизитыПлатежа.Добавить(Payee.BIC, СторонаБанка + "БИКБанка");
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payee, "Name"), СторонаБанка
				+ "НаименованиеБанка");
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payee, "City"), СторонаБанка
				+ "ГородБанка");
			РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(Payee, "CorrespAcc"), СторонаБанка
				+ "КоррСчетБанка");
		КонецЕсли;
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(PayData, "AccountDebit"), "ПлательщикСчет");

		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Recipient, "name"), СторонаКлиента
			+ "Наименование");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Recipient, "inn"), СторонаКлиента
			+ "ИНН");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Recipient, "kpp"), СторонаКлиента
			+ "КПП");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(PayData, "AccountCredit"), "ПолучательСчет");
		РеквизитыПлатежа.Добавить(ЭД.Data.Bank.Bic, СторонаКлиента + "БИКБанка");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Data.Bank, "Name"), СторонаКлиента
			+ "НаименованиеБанка");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Data.Bank, "City"), СторонаКлиента
			+ "ГородБанка");
		РеквизитыПлатежа.Добавить(ЗначениеСвойстваXDTO(ЭД.Data.Bank, "CorrespAcc"), СторонаКлиента
			+ "КоррСчетБанка");
...
Процедура УстановитьЗначениеXDTO(ОбъектXDTO, ИмяСвойства, Значение,
		ТекстОшибки)
...
Процедура ОбработатьОтветИзБанка(
	...
	Если ЧтениеXML.Прочитать() И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента
		...
		КонецЦикла;
	Иначе
		СтруктураРазбораФайла = СформироватьДеревоРазбора(Перечисления.ВидыЭДОбменСБанками.ВыпискаБанка, Сообщение);
		...
		ДатаНачала = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ДатаНачала");
		Если ЗначениеЗаполнено(ДатаНачала) Тогда
			ПараметрыСообщения.Вставить("ДатаНачала", ДатаНачала);
		КонецЕсли;
		ДатаОкончания = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ДатаКонца");
		...
		СохранитьШтампыБанка(РеквизитыОтправленногоСообщения.НастройкаОбмена, СтруктураРазбораФайла);
		ОпределитьИсполненныеПлатежныеПоручения(РеквизитыОтправленногоСообщения.НастройкаОбмена, СтруктураРазбораФайла);
		ДатаКонца = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ДатаКонца");
		СохранитьДатуПоследнейОперации(РеквизитыОтправленногоСообщения.НастройкаОбмена, ДатаКонца);
...
Функция ОтправитьPOSTЗапросРекурсивно(ПараметрыОтправки,
		ПолучитьТелоКакСтроку, Таймаут, ПолучитьЛог, НастройкаОбмена,
		Перенаправления)
...
Процедура ПрочитатьВыпискуБанкаАсинхронныйОбмен(НастройкаОбмена,
		ПакетОбменСБанками, ДвоичныеДанныеФайла, СоздаватьОперацииВыписки,
		НовоеСообщениеОбмена)
	...
	СтруктураРазбораФайла = СформироватьДеревоРазбора(
		Перечисления.ВидыЭДОбменСБанками.ВыпискаБанка, ДвоичныеДанныеФайла);

	ДеревоРазбора = СтруктураРазбораФайла.ДеревоРазбора;
	СтрокаОбъекта = СтруктураРазбораФайла.СтрокаОбъекта;

	ИдентификаторЗапроса = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ИдентификаторЗапроса");
	...
	Если ЗначениеЗаполнено(ИдентификаторЗапроса) Тогда
		...
		Если НЕ ЗначениеЗаполнено(СообщениеЗапрос) Тогда
			...
			ПодробнаяИнформация = СтрШаблон(ПодробнаяИнформация, ПакетОбменСБанками, ИдентификаторЗапроса);
			ОбработатьОшибку(ВидОперации, ПодробнаяИнформация, , ПакетОбменСБанками);
		КонецЕсли;
	КонецЕсли;
	...
	СтруктураРеквизитов.Вставить("Направление", Перечисления.НаправленияЭД.Входящий);
	СтруктураРеквизитов.Вставить("АдресФайлаВоВременномХранилище", АдресВХранилище);
	ВнешнийИдентификатор = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаОбъекта, "ИдентификаторДокумента");
	...
	СохранитьШтампыБанка(НастройкаОбмена, СтруктураРазбораФайла);
	ОпределитьИсполненныеПлатежныеПоручения(НастройкаОбмена, СтруктураРазбораФайла);
	СохранитьДатуПоследнейОперации(НастройкаОбмена, ДатаОкончания);
...
Функция ИмяПоляДляОтображенияОшибкиПоМаршруту(НастройкаОбмена,
	...
	ИмяПоляОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("ИсходящиеДокументы[%1].МаршрутПодписания", НомерСтроки
		- 1);
...
Процедура ПроверитьВозможностьПередачиДанныхПлатежногоПоручения(
	...
	Если ЗначениеЗаполнено(КодВидаДохода) Тогда
		Если (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
			...
			ШаблонОшибки = НСтр("ru = 'Не поддерживается передача реквизита Код вида дохода на стороне банка.
									|Текущая версия формата: %1, требуется 2.2.2 или выше.
		...
		ИначеЕсли ПрограммаБанка = Перечисления.ПрограммыБанка.АльфаБанкОнлайн Тогда
			ТекстОшибки = НСтр("ru = 'Не поддерживается получение реквизита Код вида дохода на стороне банка.
									|Необходимо перейти на типовой способ обмена с банком.
	...
	Если (ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен
		...
		ШаблонОшибки = НСтр("ru = 'Не поддерживается передача реквизита Код выплат на стороне банка.
								|Текущая версия формата: %1, требуется 2.1.1 или выше.
...
Функция ПолучитьОбъектНастройкиОбмена(НастройкаОбмена, Организация, Банк)
	...
	Иначе
		СуществующаяНастройкаОбмена = ОбменСБанками.НастройкаОбмена(Организация, Банк, Ложь);
	КонецЕсли;
...
Функция ЕстьПлатежныйДокументТребующийПодтверждениеВЛК(СообщенияОбмена,
		ВыводитьОкноПодтверждения)
...
Процедура СохранитьШтампыБанка(НастройкаОбмена, СтруктураРазбораФайла)
	...
	Для Каждого СтрокаТЧ Из СтрокиТЧ Цикл
		...
		ИдПлатежа = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ИдПлатежа");
		Если ЗначениеЗаполнено(ДатаСписано) И ЗначениеЗаполнено(ИдПлатежа) Тогда
			...
			Если ЗначениеЗаполнено(ПлатежныйДокумент) Тогда
				ДополнительныеДанные = Новый Структура("ДатаСписания", ДатаСписано);
				ШтампБанкаСтатус = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ШтампБанкаСтатус");
				Если ЗначениеЗаполнено(ШтампБанкаСтатус) Тогда
					ШтампБанкаНаименование = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ШтампБанкаНаименование");
					ШтампБанкаОтделение = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ШтампБанкаОтделение");
					ШтампБанкаБИК = ЗначениеРеквизитаСтрокиДереваРазбора(ДеревоРазбора, СтрокаТЧ, "ШтампБанкаБИК");
					...
					ДополнительныеДанные.Вставить("ДанныеШтампа", ДанныеШтампа);
				КонецЕсли;
...
Процедура СформироватьПоручениеНаПереводВалютыСбербанк(СсылкаНаДокумент,
	...
	ЗаполнитьСвойствоXDTO(СекцияСтранаБенефициара, "digital", КодСтраныБенефициара, Истина, ТекстОшибки);
	ISOКодСтраныБенефициара = ЗначениеРеквизитаВДереве(ДеревоДанных, "РеквизитыПолучателя.АдресСтруктурированный.Страна.ISOКод");
	ЗаполнитьСвойствоXDTO(СекцияСтранаБенефициара, "iso2", ISOКодСтраныБенефициара, Истина, ТекстОшибки);
...
//
Процедура ОтметитьИсполненныеПлатежныеДокументы(НастройкаОбмена, ПрограммаБанка,
		МассивВнешнихИдентификаторов)
...
Функция МассивИдентификаторовВыпискиБанка(НастройкаОбмена, СтруктураДанных)
...
Процедура СохранитьЗапрос(Метод, НастройкаОбмена, URL, Соединение, HTTPЗапрос)
	...
	Для каждого ЭлементКоллекции Из HTTPЗапрос.Заголовки Цикл
		...
		Если Не ЗначениеЗаполнено(НастройкаОбмена)
				И ЭлементКоллекции.Ключ = "Authorization" Тогда
			МассивСтрок.Добавить("***********");
		Иначе
