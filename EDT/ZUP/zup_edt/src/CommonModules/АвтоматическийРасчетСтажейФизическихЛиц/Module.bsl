...
//
Процедура ПроверитьСтажиФизическихЛиц(МенеджерВременныхТаблиц, ОписаниеФильтра) Экспорт 
	ОписаниеРегистра = ЗарплатаКадрыОбщиеНаборыДанных.ОписаниеРегистра("СтажиФизическихЛицКПересчету");
	...
	|			И (&ФильтрПериод_ >= СтажиФизическихЛицКПересчету.НачалоПериода)";
	...
	ОписаниеИспользованияФильтра = ЗарплатаКадрыПериодическиеРегистры.ОписаниеИспользованиеФильтра("ТаблицаОтборов", "СтажиФизическихЛицКПересчету");
...
Процедура ПриЗаполненииСписковСОграничениемДоступа(Списки) Экспорт
	...
	Списки.Вставить(Метаданные.РегистрыСведений.СтажиФизическихЛицКПересчету, Истина);
...
Процедура ЗарегистрироватьПересчетСтажейФизическихЛиц(Источник, Отказ, Замещение) Экспорт
	...
	ТаблицаИзменившихсяДанных = Источник.ТаблицаИзменившихсяДанныхНабора();
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц;
	ЗарплатаКадры.СоздатьВТПоТаблицеЗначений(Запрос.МенеджерВременныхТаблиц, ТаблицаИзменившихсяДанных, "ВТИзменения");
	...
	Запрос.Текст =
		...
		|ПОМЕСТИТЬ ВТПериодыФизическихЛицКПересчетуСтажей
		|ИЗ
		|	ВТИзменения КАК ИзмененияРегистра
		|
		|СГРУППИРОВАТЬ ПО
...
Процедура ОбновлениеСтажейФизическихЛиц() Экспорт
	...
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбновлениеСтажейФизическихЛиц);
...
Процедура ОбновитьНакопленныеСтажиФизическихЛицПоВременнойТаблице(МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
		...
		|ВЫБРАТЬ
		|	ВЫБОР
		|		КОГДА НакопленныеСтажиФизическихЛиц.Прерван
		|			ТОГДА ДОБАВИТЬКДАТЕ(ПервыеДатыКРасчету.ДатаНачала, ДЕНЬ, -1)
		|		ИНАЧЕ ПервыеДатыКРасчету.ДатаНачала
	...
	Запрос.Текст =
		...
		|			И (ВЫБОР
		|				КОГДА НакопленныеСтажиФизическихЛиц.Прерван
		|					ТОГДА ДОБАВИТЬКДАТЕ(НакопленныеСтажиФизическихЛиц.ДатаНачала, ДЕНЬ, -1)
		|				ИНАЧЕ НакопленныеСтажиФизическихЛиц.ДатаНачала
		|			КОНЕЦ = РасчетСтажа.ДатаРасчетаСтажа)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ФизическоеЛицо,
		|	ДатаНачала";
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Запрос.Текст =
			...
			|		ПО НакопленныеСтажиФизическихЛиц.ФизическоеЛицо = СтажиФизическихЛиц.ФизическоеЛицо
			|			И НакопленныеСтажиФизическихЛиц.ВидСтажа = СтажиФизическихЛиц.ВидСтажа
			|			И НакопленныеСтажиФизическихЛиц.ДатаНачала < СтажиФизическихЛиц.ДатаОтсчета";
		...
		Пока Выборка.СледующийПоЗначениюПоля("ФизическоеЛицо") Цикл
			...
			Исключение
				...
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось обновить накопленные стажи %1.Ошибка блокировки'", ОбщегоНазначения.КодОсновногоЯзыка()), Выборка.ФизическоеЛицо);
			...
			Пока Выборка.СледующийПоЗначениюПоля("ДатаНачала") Цикл
				...
				Пока Выборка.Следующий() Цикл
					...
					Если Выборка.Рассчитать Тогда
						
						ОписаниеСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.СведенияОСтаже();
						
						Если СведенияОПредыдущемСтаже.Прерван Тогда
							ОписаниеСтажа.ДатаОтсчета = НачалоДня(НачалоДня(Выборка.ДатаНачала) - 1);
							ДатаНачала = Выборка.ДатаНачала;
						Иначе
							ОписаниеСтажа.ДатаОтсчета = Выборка.ДатаСведений;
							ДатаНачала = НачалоДня(НачалоДня(Выборка.ДатаНачала) - 1);
						КонецЕсли;
						...
						ПродолжительностьСтажа = ЗарплатаКадрыРасширенныйКлиентСервер.ПродолжительностьСтажа(ОписаниеСтажа, ДатаНачала);
