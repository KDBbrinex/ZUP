...
Процедура ФизическиеЛицаПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.РеестрГосслужащих") Тогда
		...
		МодульРеестрГосслужащих.ФизическиеЛицаПриСозданииНаСервере(Форма, "ГруппаДополнительныеДанные");
	КонецЕсли;
...
Функция ПараметрыОтбораПоПериодуПараметровОткрытияФормыСписка(Параметры) Экспорт
	...
	ПараметрыОтбора.Вставить("ОтбиратьРаботающихВПериоде", Истина);
		...
		Если Параметры.Отбор.Свойство("ДатаПримененияОтбора") Тогда
			...
			Если НЕ ОтборПоПериодуКадровыхДанных Тогда
				...
				Если ЗначениеЗаполнено(Параметры.Отбор.ДатаПримененияОтбора) Тогда
					...
					ПараметрыОтбора.ОтбиратьРаботающихВПериоде = Ложь;
...
Процедура СохранитьДанныеДополнительнойФормы(Форма, ИмяФормы, Отказ, ТекущийОбъект) Экспорт
	...
	ИначеЕсли ИмяФормы = "Справочник.ФизическиеЛица.Форма.ТрудоваяДеятельность" Тогда
		...
		СохранитьНаборЗаписей(Форма, "СтажиФизическихЛиц", "Лет,Месяцев,ТребуетЗаписи");
		СохранитьНаборЗаписей(Форма, "НаградыФизическихЛиц");
		СохранитьНаборЗаписей(Форма, "ТрудоваяДеятельностьФизическихЛиц");
		
		УстановитьПривилегированныйРежим(Истина);
		СохранитьНаборЗаписей(Форма, "ВидыСтажаТрудовойДеятельностиФизическихЛиц");
		УстановитьПривилегированныйРежим(Ложь);
...
Функция ОписаниеМенюВводаНаОсновании(ПараметрыПостроения) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АдаптацияУвольнение") Тогда
		...
		МодульАдаптацияУвольнение.ДобавитьПодменюРешенияПоКадровымИзменениям(ОписаниеМеню);
	КонецЕсли;
...
Процедура ЗаполнитьИсториюВзаимоотношений(История, ФизическиеЛица) Экспорт
	...
	ПараметрыСотрудников.КадровыеДанные = "Организация, ДатаПриема, ДатаУвольнения,"
		+ ?(ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание"), "ДолжностьПоШтатномуРасписанию", "Должность, Подразделение");
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.АнализТекучестиПерсонала") Тогда
		...
		ПричиныУвольнения = МодульАнализТекучестиПерсонала.ПричиныУвольненияСотрудников(Сотрудники.ВыгрузитьКолонку("Сотрудник"));
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ПодборПерсонала") Тогда
		МодульПодборПерсонала = ОбщегоНазначения.ОбщийМодуль("ПодборПерсонала");
		МодульПодборПерсонала.ДополнитьИсториюВзаимоотношенийФизическихЛицКандидатами(История, ФизическиеЛица);
	КонецЕсли;
