...
Функция ДанныеКадровыйРезервДляРезультатыАттестацииСотрудников(ДанныеДляПроведения) Экспорт
	...
	Запрос.УстановитьПараметр("РекомендуетсяВКадровыйРезерв", ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.РешенияАттестационныхКомиссий.РекомендуетсяВКадровыйРезерв"));
...
Процедура ПроверитьЗаполнениеВидаРезерваВТабличнойЧасти(ДокументОбъект, ИмяТабличнойЧасти, ПроверяемыеРеквизиты, Отказ, ПроверкаВнутриТЧ = Ложь) Экспорт
	...
	Для каждого СтрокаТЧ Из ТабличнаяЧастьДокумента Цикл
		...
		Если ПроверкаВнутриТЧ Тогда
			Если НЕ СтрокаТЧ.РешениеКомиссии = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.РешенияАттестационныхКомиссий.РекомендуетсяВКадровыйРезерв") Тогда
				Продолжить;
			КонецЕсли;
