...
Процедура СобратьДанныеДляНачислениеРезерваОтпусков(Регистратор, Организация, ПериодРегистрации, РасчетРезерваОтпусков, ОценочныеОбязательства, ОценочныеОбязательстваПоСотрудникам) Экспорт
	...
	ДоляРезервовОтпусков         = ДоляРезерваОтпусков(ПериодРегистрации, Организация, Регистратор);
	ТаблицаНакопленныхРезервов   = НакопленоРезерваПоВсейОрганизации(ПериодРегистрации, Организации, Регистратор, Истина);
	УстановитьПривилегированныйРежим(Истина);
	
	Если ФормироватьРезервОтпусковБУ
		И НормативОтчисленийВРезервОтпусков = Перечисления.МетодыНачисленияРезервовОтпусков.НормативныйМетод Тогда
	...
	КонецЕсли;
	ПредельнаяВеличинаОтчисленийОрганизации = Окр(ДоляРезервовОтпусков * ПредельнаяВеличинаОтчисленийТекущегоМесяца, 2);
...
Процедура СуммыРезервовТекущегоМесяцаНормативныйМетод(ПараметрыРасчетаОбязательств, СтруктураПараметровРасчета, Реквизиты, ВидУчета = "")
	...
	Если ((СтруктураПараметровРасчета.ФормироватьРезервОтпусковБУ И СтруктураПараметровРасчета.МетодНачисленияРезерваОтпусков =  Перечисления.МетодыНачисленияРезервовОтпусков.НормативныйМетод))
		...
		Если СтруктураПараметровРасчета["ВсегоНачисленоРезервов" + ВидУчета] > СтруктураПараметровРасчета.ПредельнаяВеличинаОтчисленийОрганизации Тогда
			Разница = Мин(СтруктураПараметровРасчета["ВсегоНачисленоРезервов" + ВидУчета], СтруктураПараметровРасчета.ПредельнаяВеличинаОтчисленийОрганизации);
...
Функция ДоляРезерваОтпусков(Период, Организация, Регистратор = Неопределено)
	...
	Если ФондОплатыТрудаИВзносы <> 0 Тогда
		...
		Если ВсегоНакопленоРезервов < ПредельнаяВеличинаОтчисленийВРезервОтпусков Тогда
			ДоляРезервовОтпусков = Окр(ДоляРезервовОтпусковОрганизации, 8);
		КонецЕсли;
