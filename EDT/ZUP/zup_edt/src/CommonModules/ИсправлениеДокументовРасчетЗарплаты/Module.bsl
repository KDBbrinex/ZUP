...
Процедура ЗаполнитьНачисленияПерерасчетПоНачисленияЗаднимЧислом(Организация, ПериодРегистрации,
	...
	Запрос.Текст =
		...
		|						ТОГДА Начисления.ДокументОснование = ДублирующиеНачисления.ДокументОснование
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ)
		...
		|				И (ВЫБОР
		|					КОГДА ДублирующиеНачисления.РассчитыватьПоРазовымНачислениямДокумента
		|						ТОГДА Начисления.РегистраторРазовогоНачисления = ДублирующиеНачисления.РегистраторРазовогоНачисления
		|					ИНАЧЕ ИСТИНА
		|				КОНЕЦ)
		...
		|					ТОГДА ВозможныеЗаписиДляПерерасчета.РегистраторРазовогоНачисления = ВозможныеЗаписиДляПерерасчетаВспом.РегистраторРазовогоНачисления
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		|			И (ВЫБОР
		|				КОГДА ТИПЗНАЧЕНИЯ(ВозможныеЗаписиДляПерерасчета.ДокументОснование) В (&ТипыОснований)
		|					ТОГДА ВозможныеЗаписиДляПерерасчета.ДокументОснование = ВозможныеЗаписиДляПерерасчетаВспом.ДокументОснование
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		...
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	НачисленияПериоды.Сотрудник КАК Сотрудник,
		|	НачисленияПериоды.ВидРасчета КАК ВидРасчета,
		|	НачисленияПериоды.ПериодДействия КАК ПериодДействия
	...
	Запрос.Текст =
		...
		|			И (Начисления.Результат <> 0 ИЛИ (НЕ НачисленияПериоды.ЭтоДополняющееНачисление))
		|			И (Начисления.ПериодРегистрации <= &ПериодРегистрации)
	...
	Результаты = РезультатЗапроса[0].Выгрузить();
	РезультатыПоказателей = РезультатЗапроса[1].Выгрузить();
	
	РаспределениеНачислений = ПолучитьРаспределениеНачислений(Результаты, РезультатыПоказателей, ВременныйРегистратор);
	...
	ПоказателиОтдельно = Показатели <> Неопределено;
	
	Для Каждого Строка Из РезультатыПоказателей Цикл
		Если ТекущийИдентификаторСтроки <> Строка.ИдентификаторСтроки
			Или ТекущийСотрудник <> Строка.Сотрудник
			Или ТекущийРегистратор <> Строка.Регистратор
			Или ТекущийПериодДействияНачало <> Строка.ПериодДействияНачало Тогда
		...
		ТекущийИдентификаторСтроки = Строка.ИдентификаторСтроки;
		ТекущийСотрудник = Строка.Сотрудник;
		ТекущийРегистратор = Строка.Регистратор;
		ТекущийПериодДействияНачало = Строка.ПериодДействияНачало;
	...
	ТекущийСотрудник = Неопределено;
	ТекущийПериодДействияНачало = Неопределено;
	ТекущееЗначениеРасчетПоДоходамДокумента = Неопределено;
	ТекущийРегистраторРазовогоДохода = Неопределено;
	...
	Для Каждого Строка Из Результаты Цикл
		...
		Если Не Строка.УточнятьПоНаличиюРазовогоНачисления
			...
			Если (ТекущийИдентификаторСтроки <> Строка.ИдентификаторСтроки 
				...
				Если Строка.Результат<>0 Или Строка.РезультатВТомЧислеЗаСчетФБ<>0 
					...
					НовыйПерерасчетСторно.ДатаНачала = Строка.ПериодДействияНачало;
					НовыйПерерасчетСторно.ДатаОкончания = Строка.ПериодДействияКонец;
					НовыйПерерасчетСторно.НачалоБазовогоПериода = Строка.БазовыйПериодНачало;
					НовыйПерерасчетСторно.ОкончаниеБазовогоПериода = Строка.БазовыйПериодКонец;
					НовыйПерерасчетСторно.Начисление = Строка.ВидРасчета;
					
					НовыйПерерасчетСторно.Результат = - Строка.Результат;
					НовыйПерерасчетСторно.РанееНачислено = - Строка.РанееНачислено;
					НовыйПерерасчетСторно.РезультатВТомЧислеЗаСчетФБ = - Строка.РезультатВТомЧислеЗаСчетФБ;
					НовыйПерерасчетСторно.ОтработаноДней = - Строка.ОтработаноДней;
					НовыйПерерасчетСторно.ОтработаноЧасов = - Строка.ОтработаноЧасов;
					НовыйПерерасчетСторно.ФиксРасчет = Истина;
					НовыйПерерасчетСторно.ФиксСторно = Истина;
					НовыйПерерасчетСторно.ПериодРегистрацииВремени = Строка.ПериодРегистрацииВремениСторноЗаписей;
					НовыйПерерасчетСторно.ГрафикРаботы = Строка.ГрафикРаботыСторноЗаписей;
					НовыйПерерасчетСторно.ГрафикРаботыНорма = Строка.ГрафикРаботыНормаСторноЗаписей;
					НовыйПерерасчетСторно.ВремяВЦеломЗаПериод = Строка.ВремяВЦеломЗаПериодСторноЗаписей;
				...
				Если ТекущийВидРасчета <> Строка.ВидРасчета 
					Или ТекущийДокументОснование <> Строка.ДокументОснование 
					Или ТекущийПериодДействияНачало <> Строка.ПериодДействияНачало
					Или ТекущийСотрудник <> Строка.Сотрудник 
					Или ТекущееЗначениеРасчетПоДоходамДокумента <> Строка.РассчитыватьПоРазовымНачислениямДокумента
					Или Строка.РегистраторРазовогоНачисления <> ТекущийРегистраторРазовогоДохода Тогда
			...
			ТекущийИдентификаторСтроки = Строка.ИдентификаторСтроки;
			ТекущийРегистратор = Строка.Регистратор;
			ТекущийВидРасчета = Строка.ВидРасчета;
			ТекущийДокументОснование = Строка.ДокументОснование;
			ТекущийСотрудник = Строка.Сотрудник;
			ТекущийПериодДействияНачало = Строка.ПериодДействияНачало;
			
			ТекущееЗначениеРасчетПоДоходамДокумента = Строка.РассчитыватьПоРазовымНачислениямДокумента;
			ТекущийРегистраторРазовогоДохода = Строка.РегистраторРазовогоНачисления;
		КонецЕсли;	
...
Функция НовыйНачисленияДокумента()
	...
	НачисленияДокумента.Колонки.Добавить("ПериодДействияНачало", Новый ОписаниеТипов("Дата"));
	НачисленияДокумента.Колонки.Добавить("ПериодДействияКонец", Новый ОписаниеТипов("Дата"));
	НачисленияДокумента.Колонки.Добавить("ИдентификаторСтрокиВидаРасчета", Новый ОписаниеТипов("Число"));

	Возврат НачисленияДокумента
КонецФункции	

Функция НачисленияИсправляемогоДокумента(ИсправляемыйДокумент, ОписаниеТаблицДокумента, Сотрудники = Неопределено)
	НачисленияДокумента = НовыйНачисленияДокумента();
	...
	Для Каждого ОписаниеТекущейТаблицы Из ОписаниеТаблицДокумента Цикл		
		...
		Если ОписаниеТекущейТаблицы.СодержитПолеВидРасчета Тогда
			...
			ВыражениеПоляПериодДействияНачало = СтрЗаменить(ВыражениеПоляПериодДействияНачало, "&ИспользованиеПериода", "Начисления." + ОписаниеТекущейТаблицы.ИмяРеквизитаВидРасчета + ".ИспользованиеПериода");
			...
			ВыражениеПоляПериодДействияКонец = СтрЗаменить(ВыражениеПоляПериодДействияКонец, "&ИспользованиеПериода", "Начисления." + ОписаниеТекущейТаблицы.ИмяРеквизитаВидРасчета + ".ИспользованиеПериода");
		...
		Пока Выборка.Следующий() Цикл
			СтрокаНачисленийДокумента = НачисленияДокумента.Добавить();
...
Функция НачисленияТекущегоДокумента(ИсправляемыйДокумент, ПериодРегистрации, ТаблицыНачислений, ОписаниеТаблиц)
	НачисленияТекущегоДокумента = НовыйНачисленияДокумента();
...
Процедура УчестьПредыдущиеИсправленияДокумента(ИсправляемыйДокумент, НачисленияИсправляемогоДокумента)
...
// и в случае если был добавлен, то добавляем и зависимое начисление в коллекцию перерасчетов
//
Функция НеобходимоДобавлятьПерерасчетЗависимого(СтрокаНачисления, РаннееДобавленныеНачисления)
...
Функция ПериодИсправляемогоДокумента(НачисленияИсправляемогоДокумента)
	ПериодИсправляемогоДокумента = Новый Структура();
	ПериодИсправляемогоДокумента.Вставить("ДатаНачала", ЗарплатаКадрыПериодическиеРегистры.МаксимальнаяДата());
	ПериодИсправляемогоДокумента.Вставить("ДатаОкончания", '00010101');
...
//
Функция НеобходимоДоначислятьЗарплату(ИсправляемыйДокумент, НачисленияИсправляемогоДокумента)
...
Функция ПериодыВосстановленияНачислений(ПериодИсправляемогоДокумента, НачисленияТекущегоДокумента)
...
Функция ВосстанавливаемыеНачисления(ДокументСсылка, ИсправляемыйДокумент, ПериодРегистрации, Организация, СотрудникиИсправляемогоДокумента, ПериодыВосстановленияНачислений)
	МенеджерРасчета = РасчетЗарплатыРасширенный.СоздатьМенеджерРасчета(ПериодРегистрации, Организация);
...
Процедура СформироватьДанныеПерерасчетаДляИсправленияСлужебный(Организация, ПериодРегистрации, ИсправленныйДокумент,
	...
	ТребуетсяВосстановлениеНачислений = ДоначислитьЗарплатуПриНеобходимости
		И НеобходимоДоначислятьЗарплату(ИсправленныйДокумент, НачисленияИсправляемогоДокумента);
...
Функция ПолучитьРаспределениеНачислений(Результаты, РезультатыПоказателей, ИсключаемыйРегистратор)
	...
	СтрокиДляРаспределения = Результаты.Скопировать(, "Регистратор, ИдентификаторСтроки");
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(РезультатыПоказателей, СтрокиДляРаспределения);
