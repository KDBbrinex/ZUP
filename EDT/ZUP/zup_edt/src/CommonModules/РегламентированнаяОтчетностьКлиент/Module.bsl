...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПослеФормированияВыгрузки(Результат, ДополнительныеПараметры) Экспорт
	...
	ОО = Новый ОписаниеОповещения("ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВВебКлиентеПослеПолученияФайлов", ЭтотОбъект, Новый Структура("ВремКаталог, ВремФайл, ИмяКнопки, Форма", ВремКаталог, ВремФайл, ИмяКнопки, Форма));
	НачатьПолучениеФайлов(ОО, ПолучаемыеФайлы, , Ложь);
КонецПроцедуры
...
Процедура ПредставлениеНуля(ДополнительныеПараметры) Экспорт
	...
	Если Область.СодержитЗначение И Область.ТипЗначения.СодержитТип(Тип("Число")) Тогда
		Если Область.Значение = 0 Тогда
			...
			ДополнительныеПараметры = Новый Структура("Область, Элемент, Форма", Область, Элемент, Форма);
			...
			# Если ВебКлиент Тогда
				Форма.ПоказатьВыборИзСписка(ОписаниеОповещения, Список, Элемент);
			#Иначе
...
Процедура ПолеТабличногоДокументаФормаОтчетаПриИзмененииСодержимогоОбластиЗавершение(СписокЭлемент, ДополнительныеПараметры) Экспорт
	...
	Элемент = ДополнительныеПараметры.Элемент;
	Форма = ДополнительныеПараметры.Форма;
	ДополнительныеПараметры.Свойство("ИмяСтраницыПанели", ИмяСтраницыПанели);
	...
	Форма.РасчетНаКлиенте(ИмяСтраницыПанели, ОбластьИмя);
	Форма.Модифицированность = Истина;
КонецПроцедуры
...
Процедура СохранитьОтчетИОткрытьФормуПрисоединенныеФайлы(Форма) Экспорт
	...
	Если ТипЗнч(Форма) = Тип("ФормаКлиентскогоПриложения") Тогда
		...
		Если СтрНайти(Форма.ИмяФормы, "Отчет.РегламентированныйОтчет") > 0 Тогда
...
Процедура ИзменитьСтатусОтправки(ПараметрыИзменения) Экспорт
	...
	Форма = ПараметрыИзменения.Форма;
	
	СписокСтатусов = Новый СписокЗначений;
	
	СписокСтатусов.ЗагрузитьЗначения(
		РегламентированнаяОтчетностьКлиентСервер.СтатусыОбъектовРеглОтчетностиПриРучномВводе(Форма, ПараметрыИзменения.КонтролирующийОрган));
	
	ТекСтатус = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	НСтр("ru = '%1'"), Форма.ТекущийЭлемент.Заголовок);
	...
	Оп = Новый ОписаниеОповещения("ИзменитьСтатусОтправкиПослеВыбораИзСписка", ЭтотОбъект, ПараметрыИзменения);
	
	Если ТекЭлемСпискаСтатусов = Неопределено Тогда
		Форма.ПоказатьВыборИзСписка(Оп, СписокСтатусов);
	Иначе
		Форма.ПоказатьВыборИзСписка(Оп, СписокСтатусов, , ТекЭлемСпискаСтатусов);
	КонецЕсли;
...
Процедура ОбработатьОшибкуМодуляПечати(ОписаниеОшибки, ДистрибутивКомпонентыПечатиМашиночитаемыхФорм, ВремКаталог)
	...
	ПоказатьВопрос(ОписаниеОповещения,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='%1'"), ТекстПредупреждения),
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеЗавершение(ТаксДокПринт, ВременныйКаталог, ИмяВременногоФайла, Выгрузка)
	...
	Попытка
		...
		ОО = Новый ОписаниеОповещения("ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПослеПолученияФайлов", ЭтотОбъект, Новый Структура("ИмяВременногоФайла, ВременныйКаталог, ТаксДокПринт", ИмяВременногоФайла, ВременныйКаталог, ТаксДокПринт));
		НачатьПолучениеФайлов(ОО, ПолучаемыеФайлы, , Ложь);
	Исключение
		Возврат;
	КонецПопытки;
...
Процедура ВывестиМашиночитаемуюФормуУведомленияОСпецрежимахВТонкомКлиентеПослеПолученияФайлов(ПолученныеФайлы, ПараметрыСохранения) Экспорт
	ИмяВременногоФайла = ПараметрыСохранения.ИмяВременногоФайла;
	ВременныйКаталог = ПараметрыСохранения.ВременныйКаталог;
	ТаксДокПринт = ПараметрыСохранения.ТаксДокПринт;
...
// Вызывается из процедуры "ИзменитьСтатусОтправки".
//
Процедура ИзменитьСтатусОтправкиПослеВыбораИзСписка(ВыбСтатусЭлемент, ПараметрыИзменения) Экспорт
	...
	Если ВыбСтатусЭлемент <> Неопределено
		...
		Если ВыбСтатусЭлемент.Значение = НСтр("ru = 'Сдано'") ИЛИ 
			ВыбСтатусЭлемент.Значение = НСтр("ru = 'Сдано, есть замечания'") Тогда
			...
			Если РегламентированнаяОтчетностьВызовСервера.ПодключенДокументооборотСКонтролирующимОрганом(
				...
				Оп = Новый ОписаниеОповещения("ИзменитьСтатусОтправкиПослеВыбораИзСпискаЗавершение",
				ЭтотОбъект, ПараметрыИзменения);
				
				ТекстВопроса = НСтр("ru='Вы уверены, что отчет уже сдан?'");
				Если ПараметрыИзменения.Свойство("ТекстВопроса") Тогда
					ТекстВопроса = ПараметрыИзменения.ТекстВопроса;
				КонецЕсли;
		...
		ИзменитьСтатусОтправкиПослеВыбораИзСпискаЗавершение(КодВозвратаДиалога.Да, ПараметрыИзменения);
		
	КонецЕсли;
	
КонецПроцедуры

// Вызывается из процедуры "ИзменитьСтатусОтправкиПослеВыбораИзСписка".
//
Процедура ИзменитьСтатусОтправкиПослеВыбораИзСпискаЗавершение(РезультатВопроса, ПараметрыИзменения) Экспорт
	...
	Форма.Элементы.НаименованиеЭтапа.Заголовок = ПараметрыИзменения.НовСтатус;
	
	// не устанавливаем признак модифицированности для Минобороны если изменили статус
	КонтролирующийОрган = Неопределено;
	Если НЕ ПараметрыИзменения.Свойство("КонтролирующийОрган", КонтролирующийОрган) ИЛИ КонтролирующийОрган <> ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.Минобороны") Тогда
		Форма.Модифицированность = Истина;
	КонецЕсли;
