...
//
Функция КоличествоСекундВременногоИнтервалаSCORM(Знач ВременнойИнтервал)
...
Функция ДатаИзВременногоИнтервала(Знач ВременнойИнтервал)
...
// Внимание: в отличие от интервала, время не может превышать 23.59.59.
Функция ДатаИзКоличестваСекунд(Знач СуммаСекунд)
...
Функция ПолнаяСтрокаИзКоличестваСекунд(Знач СуммаСекунд, ГраницаИнтервала = Неопределено) Экспорт
	...
	Если СтруктураИнтервала.КоличествоЛет > 0 Тогда // 1
		...
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоЛет,, ПарПредмета) + ",";
	КонецЕсли;
	
	Если СтруктураИнтервала.КоличествоМесяцев > 0 Тогда // 2
		ПарПредмета="месяц,месяца,месяцев,м,,,,,0";
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоМесяцев,, ПарПредмета) + ",";
	КонецЕсли;
	
	Если СтруктураИнтервала.КоличествоДней > 0 Тогда // 3
		ПарПредмета="день,дня,дней,м,,,,,0";
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоДней,, ПарПредмета) + ",";
	КонецЕсли;
	
	Если СтруктураИнтервала.КоличествоЧасов > 0 Тогда // 4
		ПарПредмета="час,часа,часов,м,,,,,0";
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоЧасов,, ПарПредмета) + ",";
	КонецЕсли;
	
	Если СтруктураИнтервала.КоличествоМинут > 0 Тогда // 5
		ПарПредмета="минута,минуты,минут,ж,,,,,0";
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоМинут,, ПарПредмета) + ",";
	КонецЕсли;
	
	Если СтруктураИнтервала.КоличествоСекунд > 0 Тогда // 6
		ПарПредмета="секунда,секунды,секунд,ж,,,,,0";
		Строка = Строка + ЧислоПрописью(СтруктураИнтервала.КоличествоСекунд,, ПарПредмета) + ",";
	КонецЕсли;
