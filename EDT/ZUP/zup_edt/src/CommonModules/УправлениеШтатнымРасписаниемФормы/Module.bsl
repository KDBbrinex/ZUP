...
Процедура ПрочитатьДополнительныеДанныеДолжностиДляПозицииШтатногоРасписания(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		...
		МодульГосударственнаяСлужба.ПрочитатьДополнительныеДанныеДолжностиДляПозицииШтатногоРасписания(Форма);
	КонецЕсли;
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		...
		МодульОхранаТрудаФормы.ПрочитатьДополнительныеЗначенияПозицииШтатногоРасписания(Форма, ?(Форма.ВнешниеДанные, Форма.РедактируемаяПозиция, Форма.СсылкаНаОбъект));
	КонецЕсли;
...
Процедура ДополнитьФормуЭлементаСправочникаШтатноеРасписание(Форма) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыПриложения.ГосударственнаяСлужба") Тогда
		МодульГосударственнаяСлужба = ОбщегоНазначения.ОбщийМодуль("ГосударственнаяСлужба");
		МодульГосударственнаяСлужба.ДополнитьФормуЭлементаСправочникаШтатноеРасписание(Форма);
	КонецЕсли;
	
	// Подсистема "Охрана труда"
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.ОхранаТруда") Тогда
		МодульОхранаТрудаФормы = ОбщегоНазначения.ОбщийМодуль("ОхранаТрудаФормы");
		МодульОхранаТрудаФормы.ДополнитьФормуПозицииШтатногоРасписания(Форма);
	КонецЕсли;
	
КонецПроцедуры

Процедура УстановитьУсловноеОформлениеСпискаДолжностей(Форма, ИмяСписка = "Список", ИмяРеквизитаВведена="ВведенаВШтатноеРасписание", ИмяРеквизитаИсключена = "ИсключенаИзШтатногоРасписания") Экспорт
	
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьШтатноеРасписание") Тогда
		Возврат
	КонецЕсли;
	
	УсловноеОформлениеКД = Форма[ИмяСписка].КомпоновщикНастроек.Настройки.УсловноеОформление;
	УсловноеОформлениеКД.ИдентификаторПользовательскойНастройки = "ОсновноеОформление";

	ТекущийШрифт = Форма.Элементы[ИмяСписка].Шрифт;
	...
	ЭлементОформления.Представление = НСтр("ru='Зачеркивать исключенные должности.'");
		
	ОтборЭлемента = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяРеквизитаИсключена);
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Представление = НСтр("ru = 'Выделять не введенные должности.'");
	...
	ОтборЭлемента = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяРеквизитаВведена);
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
...
Процедура УстановитьУсловноеОформлениеСпискаПодразделений(Форма, ИмяСписка = "Список", ИмяРеквизитаСформировано = "Сформировано", ИмяРеквизитаРасформировано = "Расформировано") Экспорт
	...
	ЭлементОформления = УсловноеОформлениеКД.Элементы.Добавить();
	ЭлементОформления.Представление = НСтр("ru='Зачеркивать расформированные подразделения.'");
	
	ОтборЭлемента = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяРеквизитаРасформировано);
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Истина;
	...
	ПолеЭлемента = ЭлементОформления.Поля.Элементы.Добавить();
	ЭлементОформления.Представление = НСтр("ru='Выделять жирным не сформированные подразделения.'");

	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных("Наименование");
	
	ОтборЭлемента = ЭлементОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ИмяРеквизитаСформировано);
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Ложь;
...
Процедура ЗаполнитьИтоговыйФОТПоПозициям(Форма, Позиции) Экспорт
	...
	Если Не Форма.ДоступноЧтениеНачисленийШтатногоРасписания Тогда
		Возврат;
	КонецЕсли;
	
	СтрокиСНеПолнымиСтавками = Новый Массив;
	ГрафикиРаботыПозиций = Новый Соответствие;
	Для каждого СтрокаПозиции Из Позиции Цикл
		
		Если СтрокаПозиции.КоличествоСтавок < 1
			И ЗначениеЗаполнено(СтрокаПозиции.ГрафикРаботыСотрудников) Тогда
			
			СтрокиСНеПолнымиСтавками.Добавить(СтрокаПозиции);
			ГрафикиРаботыПозиций.Вставить(СтрокаПозиции.ГрафикРаботыСотрудников);
			Продолжить;
	...
	Если СтрокиСНеПолнымиСтавками.Количество() > 0 Тогда
		
		ДанныеГрафиков = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ОбщегоНазначения.ВыгрузитьКолонку(ГрафикиРаботыПозиций, "Ключ"), "ГрафикПолногоРабочегоВремени");
		
		Для Каждого СтрокаПозиции Из СтрокиСНеПолнымиСтавками Цикл
			
			Если ЗначениеЗаполнено(ДанныеГрафиков.Получить(СтрокаПозиции.ГрафикРаботыСотрудников)) Тогда
				СтрокаПозиции.ФОТПоПозиции = СтрокаПозиции.ФОТ;
				СтрокаПозиции.ФОТПоПозицииМин = СтрокаПозиции.ФОТМин;
				СтрокаПозиции.ФОТПоПозицииМакс = СтрокаПозиции.ФОТМакс;
			Иначе
				РассчитатьИтогиФОТПоСтрокеПозиции(Форма, СтрокаПозиции);
			КонецЕсли;
...
Процедура ПрочитатьДанныеПозицииВФорму(Форма, Объект, АдресДанныхПозиции) Экспорт
	...
	УдалитьНачисленияИЕжегодныеОтпуска(Форма.Элементы.Позиции.ВыделенныеСтроки, Объект);
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		МодульУправленческаяЗарплатаФормы = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		МодульУправленческаяЗарплатаФормы.ПрочитатьДанныеПозицииВФорму(Форма, Объект, НовыеДанныеПозиции);
	КонецЕсли;
...
Процедура ЗаполнитьТекущиеДанныеПозиции(Объект, ТекущиеДанные, ДатаВступленияВСилу)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		МодульУправленческаяЗарплатаФормы = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		МодульУправленческаяЗарплатаФормы.ЗаполнитьТекущиеДанныеПозиции(Объект, ТекущиеДанные, ДанныеТекущейПозиции);
	КонецЕсли;
...
Процедура ПрочитатьТекущиеДанныеПозицииШтатногоРасписанияВФормуСправочника(Форма, Ссылка, ДатаАктуальности = '00010101', РедактированиеДанных = Ложь, ИсключаемыйРегистратор = Неопределено) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЗарплатаКадрыКорпоративнаяПодсистемы.УправленческаяЗарплата") Тогда
		МодульУправленческаяЗарплатаФормы = ОбщегоНазначения.ОбщийМодуль("УправленческаяЗарплатаФормы");
		МодульУправленческаяЗарплатаФормы.ПрочитатьТекущиеДанныеПозицииШтатногоРасписанияВФормуСправочника(Форма, ДанныеПозиции, РедактированиеДанных);
	КонецЕсли;
...
Процедура ЗаполнитьНачисленияПоказателиСтрокиПозиции(Форма, СтрокаПозиции, Начисления, Показатели) Экспорт
	...
	УстановитьЗначенияПоказателейСтрокиПозиции(Форма, СведенияОПоказателяхФормы, СтрокаПозиции);
	...
	Если СтрокиПоказателей.Количество() > 0 Тогда
		УстановитьЗначенияПоказателейСтрокиПозиции(Форма, СведенияОПоказателяхФормы, СтрокаПозиции, СтрокиПоказателей);
	КонецЕсли;
...
Процедура УстановитьЗначенияПоказателейСтрокиПозиции(Форма, СведенияОПоказателяхФормы, СтрокаПозиции, СтрокиПоказателей = Неопределено)
