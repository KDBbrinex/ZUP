...
Функция ДанныеТрудовойДеятельностиСотрудниковПоТаблицеОтборов(МенеджерВременныхТаблиц, ТолькоРазрешенные, Организация, МероприятияВсехСотрудников = Ложь, ИсключаемыйРегистратор = Неопределено)
	...
	Запрос.Текст =
		...
		|	ЕСТЬNULL(КадровыеДанныеСотрудников.ДатаПриема, ЕСТЬNULL(КадровыеДанные.ДатаПриема, ЛОЖЬ)) КАК ДатаПриемаСотрудника,
...
Функция ЗапросВТМероприятияТрудовойДеятельности(ТолькоРазрешенные, ИмяВТМероприятияТрудовойДеятельности, ИсключаемыйРегистратор = Неопределено)
	...
	Запрос.Текст =
		...
		|	МероприятияТрудовойДеятельностиПрочие.НаименованиеВторогоДокументаОснования КАК НаименованиеВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.ДатаВторогоДокументаОснования КАК ДатаВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.СерияВторогоДокументаОснования КАК СерияВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельностиПрочие.НомерВторогоДокументаОснования КАК НомерВторогоДокументаОснования,
		...
		|	МероприятияТрудовойДеятельности.НаименованиеВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельности.ДатаВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельности.СерияВторогоДокументаОснования,
		|	МероприятияТрудовойДеятельности.НомерВторогоДокументаОснования,
...
Функция ЗапросВТМероприятияТрудовойДеятельностиС01012021(ТолькоРазрешенные, ИмяВТМероприятияТрудовойДеятельности, ИсключаемыйРегистратор = Неопределено)
	...
	ТекстЗапросаС01012021 =
		...
		|			И МероприятияТрудовойДеятельности.ДатаДокументаОснования <= СотрудникиПериодыДанных.ОкончаниеПериода
		|ГДЕ
		|	МероприятияТрудовойДеятельности.ДатаОтмены = ДАТАВРЕМЯ(1, 1, 1)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|		ПО МероприятияТрудовойДеятельности.Сотрудник = СотрудникиПериодыДанных.Сотрудник
		|			И МероприятияТрудовойДеятельности.ДатаДокументаОснования <= СотрудникиПериодыДанных.ОкончаниеПериода
		|			И (МероприятияТрудовойДеятельности.ДатаОтмены МЕЖДУ &ДатаНачалаУчета И СотрудникиПериодыДанных.ОкончаниеПериода)
		|ГДЕ
		|	МероприятияТрудовойДеятельности.ДатаОтмены > МероприятияТрудовойДеятельности.ДатаДокументаОснования
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
