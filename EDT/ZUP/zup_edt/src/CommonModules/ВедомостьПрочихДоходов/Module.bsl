	...
	Если ПолучитьФункциональнуюОпцию("ПроверятьЗаполнениеФинансированияВВедомостях") Тогда
		...
		Для Каждого СтрокаСостава Из ДокументОбъект.Состав Цикл
			...
			Если ОшибкаФинансированияСтроки Тогда
				...
				ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, ДокументОбъект, СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Состав[%1].ФизическоеЛицо", СтрокаСостава.НомерСтроки-1),, Отказ);
			КонецЕсли;	
		КонецЦикла;
...
Процедура ЗарегистрироватьПеречислениеНДФЛ(Ведомость, Отказ = Ложь)
	...
	ПоляОтбора = "ФизическоеЛицо,РегистрацияВНалоговомОргане";
	// Базой для распределения по статьям будут движения НДФЛКПеречислению.
	БазаРаспределения = Ведомость.Движения.НДФЛКПеречислению.Выгрузить();
	...
	ДанныеДляДополнения.ПоляОтбора 			= ПоляОтбора;
	ОтражениеЗарплатыВУчете.ДополнитьНДФЛПеречисленныйСведениямиОРаспределенииПоСтатьям(Ведомость.Движения, ДанныеДляДополнения);
