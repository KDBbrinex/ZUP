...
Процедура ЗаполнитьРасшифровкуПлатежаПоВедомости_НДФЛ(ДанныеИБ, ДанныеXDTO)
	...
	ДанныеXDTO.Вставить("РасшифровкаПлатежа", Запрос.Выполнить().Выгрузить());
КонецПроцедуры
Процедура ЗаполнитьРасшифровкуПлатежаПоПлатежноеПоручение_НДФЛ(ДанныеИБ, ДанныеXDTO)
...
Процедура ПодготовитьДанныеКонтактнаяИнформация(ДанныеИБ, ДанныеXDTO)
	...
	Для Каждого СтрокаКИ Из ДанныеИБ.КонтактнаяИнформация Цикл
		...
		Если СвойстваВидаКИ.Предопределенный
			И Не СтрокаКИ.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.ДругаяИнформацияОрганизации")
			И Не СтрокаКИ.Вид = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ВидыКонтактнойИнформации.АдресЗаПределамиРФФизическиеЛица") Тогда
