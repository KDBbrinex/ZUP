...
// Устарела. Возвращает разделитель пути файловой системы.
//
// Параметры:
//   Платформа - Неопределено (устарела):
...
Процедура НачатьАвтоматическуюНастройкуИнтеграции(ИмяТипаОбъекта) Экспорт
	...
	СозданныеПравила = ИнтеграцияС1СДокументооборотВызовСервераПереопределяемый.
		СоздатьПравилаИнтеграцииАвтоматически(ИмяТипаОбъекта);
	...
	Если СозданныеПравила.Количество() > 0 Тогда
		...
		Для Каждого СозданноеПравило Из СозданныеПравила Цикл
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Создание:'"),
				ПолучитьНавигационнуюСсылку(СозданноеПравило),
				Строка(СозданноеПравило),
...
//
// Параметры:
//   ПараметрыВыполненияКоманды - Параметры выполнения команды ОткрытьПанель.
