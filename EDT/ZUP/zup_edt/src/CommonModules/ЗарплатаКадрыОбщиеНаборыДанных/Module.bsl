...
Функция ПараметрыЗапроса(ТекстЗапроса, ИмяИсточникаДанных) Экспорт
	...
	Если ПозицияСловаГДЕ > 0 Тогда
		...
		ПозицияИндексроать = СтрНайти(СтрокаПараметров, "ИНДЕКСИРОВАТЬ ПО");
		Если ПозицияИндексроать > 0 Тогда
			СтрокаПараметров = СокрЛП(Лев(СтрокаПараметров, ПозицияИндексроать - 1));
		КонецЕсли;
