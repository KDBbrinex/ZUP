...
Процедура ОбновитьКонтактнуюИнформациюПослеЗагрузкиОбъекта(Объект, ОбъектМодифицирован) Экспорт
	...
	Для Каждого КИ Из Объект.КонтактнаяИнформация Цикл
		КИ.ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(КИ.ЗначенияПолей, ,КИ.Вид);
	КонецЦикла;
...
Процедура ПриемНаРаботуСпискомЧерновик_ПослеЗагрузки(Отказ,Объект,ПараметрыОбъекта,Параметры) Экспорт
	...
	ЗарплатаКадрыРасширенный.ОбработкаЗаполненияМногофункциональногоДокумента(Объект);
...
Процедура НачислениеЗарплатыБП_ПослеЗагрузки(Объект) Экспорт
	...
	ПланируемаяДатаВыплаты = РасчетЗарплатыРасширенныйКлиентСервер.ПланируемаяДатаВыплатыЗарплаты(Объект.Организация, Объект.МесяцНачисления);
	Для каждого СтрокаТЧ Из Объект.НачисленияПоДоговорам Цикл
		СтрокаТЧ.ПланируемаяДатаВыплаты = ПланируемаяДатаВыплаты;
	КонецЦикла;	
	
КонецПроцедуры

Процедура ОтпускБезСохраненияОплаты_ПослеЗагрузки(Объект) Экспорт
