...
Процедура ОбработкаСобытия(НовостьСсылка, Форма, СписокПараметров) Экспорт
	...
	Если ТипЗнч(СписокПараметров) = ТипСписокЗначений Тогда
		НайденноеДействие = ИнтернетПоддержкаПользователейКлиент.НайтиЭлементСпискаЗначений(
			СписокПараметров,
		...
		Если НайденноеДействие <> Неопределено Тогда
			Если НайденноеДействие.Значение = "ОткрытьОбработку" Тогда
				ОткрываемаяФорма = ИнтернетПоддержкаПользователейКлиент.НайтиЭлементСпискаЗначений(
					СписокПараметров,
					Новый Структура("ВариантПоиска, ЗначениеПоиска",
						"ПоПредставлениюБезУчетаРегистра",
						"ИмяФормы"));
			...
			ИначеЕсли НайденноеДействие.Значение = "Предупреждение" Тогда
				Текст = ИнтернетПоддержкаПользователейКлиент.НайтиЭлементСпискаЗначений(
					СписокПараметров,
					Новый Структура("ВариантПоиска, ЗначениеПоиска",
						"ПоПредставлениюБезУчетаРегистра",
						"Текст"));
				Если ТипЗнч(Текст) = Тип("ЭлементСпискаЗначений") Тогда
					...
					Заголовок = ИнтернетПоддержкаПользователейКлиент.НайтиЭлементСпискаЗначений(
						СписокПараметров,
						Новый Структура("ВариантПоиска, ЗначениеПоиска",
							"ПоПредставлениюБезУчетаРегистра",
							"Заголовок"));
	...
	ИнформацияПереходаПоСсылке.Добавить(
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Структура гиперссылки:
				|%1'"),
			ИнтернетПоддержкаПользователейКлиентСервер.ПредставлениеЗначения(СтруктураГиперссылки, "=", Символы.ПС)));
