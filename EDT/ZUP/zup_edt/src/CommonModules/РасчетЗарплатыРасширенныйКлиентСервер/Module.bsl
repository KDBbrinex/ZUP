...
Функция ОписаниеРасчетногоДокумента() Экспорт
	...
	|НачисленияИмя,
	|НачисленияПерерасчетИмя,
	|НачисленияПерерасчетНулевыеСторноИмя,
	|ЗависимыеНачисленияИмя,
	|НачисленияПоДоговорамИмя,
	|ПособияИмя,
	...
	|УчтенныеПриРасчетеНДФЛДокументыИмя,
	...
	// Признак распределения незарплатных доходов.
	Описание.БухучетПрочихДоходов = Ложь;
...
Функция ОписаниеТаблицыРасчета() Экспорт
	...
	|СодержитПолеМестоПолученияДохода,
	|СодержитРегистраторРазовогоНачисления,
	|БухучетПрочихДоходов,
	...
	ОписаниеТаблицы.СодержитПолеМестоПолученияДохода = Ложь;
	ОписаниеТаблицы.СодержитРегистраторРазовогоНачисления = Ложь;
	
	// Признак распределения незарплатных доходов.
	ОписаниеТаблицы.БухучетПрочихДоходов = Ложь;
...
Функция ОписаниеТабличнойЧастиКорректировкиВыплаты() Экспорт
	...
	ОписаниеТабличнойЧасти = РасчетЗарплатыКлиентСерверБазовый.ОписаниеТабличнойЧастиКорректировкиВыплаты();
...
Функция ФормаПодробнееОРасчетеНДФЛОписаниеТаблицыКорректировкиВыплаты() Экспорт
	...
	ЗаполнитьЗначенияСвойств(Описание, РасчетЗарплатыКлиентСерверБазовый.ФормаПодробнееОРасчетеНДФЛОписаниеТаблицыКорректировкиВыплаты());
...
Функция ПланируемаяДатаВыплатыЗарплатыПоНастройкамПоПорядкуВыплаты(МесяцНачисления, Настройки, ПорядокВыплаты, ХарактерыВыплаты, ДатаДокумента) Экспорт
	...
	Если ПорядокВыплаты = ХарактерыВыплаты.Зарплата Тогда
		ПланируемаяДатаВыплаты = ПланируемаяДатаВыплатыЗарплатыПоНастройкам(МесяцНачисления, Настройки);
	ИначеЕсли ПорядокВыплаты = ХарактерыВыплаты.Аванс Тогда
		ПланируемаяДатаВыплаты = НачалоМесяца(МесяцНачисления) + (Настройки.ДатаВыплатыАвансаНеПозжеЧем - 1) * ДлинаСуток;
	Иначе
		ПланируемаяДатаВыплаты = НачалоМесяца(МесяцНачисления) + (Настройки.ДатаВыплатыМежрасчетаНеПозжеЧем - 1) * ДлинаСуток;
	КонецЕсли;
...
Функция ПланируемаяДатаВыплатыАванса(Организация, МесяцНачисления) Экспорт
	...
	Настройки = РасчетЗарплатыРасширенныйВызовСервера.СведенияОНастройкахЗарплатаКадрыРасширенная(Организация);
	Возврат НачалоМесяца(МесяцНачисления) + (Настройки.ДатаВыплатыАвансаНеПозжеЧем - 1) * 86400;
...
Процедура ДокументыВыполненияНачисленийУстановитьРежимОтображенияПодробно(Форма, ВидимостьПолейПодробно, ОписаниеТаблицыВидовРасчета, ИзменятьВидимостьПоказателей = Истина) Экспорт
	...
	Если ИзменятьВидимостьПоказателей Тогда
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		ОписаниеТаблицыВидовРасчета.ИмяТаблицы + "СкидкаПоВзносам",
		"Видимость",
		ВидимостьПолейПодробно);
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		ОписаниеТаблицыВидовРасчета.ИмяТаблицы + "МестоПолученияДохода",
		"Видимость",
		ВидимостьПолейПодробно);
		
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		Форма.Элементы,
		ОписаниеТаблицыВидовРасчета.ИмяТаблицы + "РанееНачислено",
		"Видимость",
		ВидимостьПолейПодробно);
	...
	Если ОписаниеТаблицыВидовРасчета.СодержитРегистраторРазовогоНачисления Тогда
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
			Форма.Элементы,
			ОписаниеТаблицыВидовРасчета.ИмяТаблицы + "РегистраторРазовогоНачисления",
			"Видимость",
			ВидимостьПолейПодробно);
	КонецЕсли;
...
Процедура ДокументыВыполненияНачисленийУстановитьВидимостьПолейОтображенияВремени(Форма, ОписаниеТаблицы, Видимость = Неопределено) Экспорт
	...
	ДокументыВыполненияНачисленийУстановитьВидимостьПолейНормыВремени(Форма, ОписаниеТаблицы, Видимость);
...
Функция РазмерностьАванса(СпособРасчетаАванса) Экспорт
	...
	Если СпособРасчетаАванса = ПредопределенноеЗначение("Перечисление.СпособыРасчетаАванса.РасчетомЗаПервуюПоловинуМесяца") Тогда
		РазмерностьАванса = ""
	Иначе
		РазмерностьАванса = РасчетЗарплатыКлиентСерверБазовый.РазмерностьАванса(СпособРасчетаАванса);
	КонецЕсли;
...
Функция ПредставлениеСпособаРасчетаАванса(СпособРасчетаАванса, СжатыйФормат) Экспорт
	...
	Иначе
		СтрокаПодстановки = РасчетЗарплатыКлиентСерверБазовый.ПредставлениеСпособаРасчетаАванса(СпособРасчетаАванса, СжатыйФормат);
	КонецЕсли;
...
Функция ИменаСпособовРасчетаАванса() Экспорт
	...
	ИменаСпособовРасчетаАванса = РасчетЗарплатыКлиентСерверБазовый.ИменаСпособовРасчетаАванса();
...
Функция СтрокаЗаполненаДляРасчета(Форма, ДанныеСтроки, ОписаниеТаблицы) Экспорт
	...
	Иначе
		...
		Если ОписаниеТаблицы.ИмяРеквизитаДатаОкончания <> Неопределено Тогда
			...
			#КонецЕсли
			РезультатПроверки = ПериодНачисленияЗаполненПравильно(
...
Функция СтрокиКРасчетуМногосотрудниковогоДокумента(Форма) Экспорт
	
	Возврат Форма.Объект.Сотрудники.НайтиСтроки(Новый Структура("ТребуетсяРасчет", Истина));
	
КонецФункции
 
Функция ТребуетсяРасчетМногосотрудниковогоДокумента(Форма) Экспорт
	
	СтрокиКРасчету = СтрокиКРасчетуМногосотрудниковогоДокумента(Форма);
