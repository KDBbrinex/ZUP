			...
			Для Каждого ПроверяемаяКолонка Из ПроверяемыеКолонки Цикл
				...
				Если СтатьиСтроки.Количество() > 1 Или СтатьиСтроки[0] <> ДокументОбъект[ПроверяемаяКолонка.Ключ] Тогда
					ОбщегоНазначения.СообщитьПользователю(
						...
						ДокументОбъект, 
						СтрШаблон("Состав[%1].ФизическоеЛицо", СтрокаСостава.НомерСтроки-1),, Отказ);
				КонецЕсли	
			КонецЦикла;	
		КонецЦикла;
...
Процедура ЗарегистрироватьПеречислениеНДФЛ(Ведомость, Отказ = Ложь)
	...
	ОтражениеЗарплатыВУчете.ДополнитьНДФЛПеречисленныйСведениямиОРаспределенииПоСтатьям(
		Ведомость.Движения, 
		ДанныеДляДополнения);
