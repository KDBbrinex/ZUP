	...
	Иначе
		...
		Если ИмяПВР = "Начисления" Тогда
			...
			Если ОбщиеДокументы.Найти(ТипЗнч(ДокументСсылка)) <> Неопределено Тогда 
				...
				ВидыДокументов.Добавить(Перечисления.ВидыДокументовНачисления.Командировка);
				...
				ТекстУсловия =
					"ВидыРасчета.СпособВыполненияНачисления <> ЗНАЧЕНИЕ(Перечисление.СпособыВыполненияНачислений.ПоОтдельномуДокументуДоОкончательногоРасчета)
					|	ИЛИ ВидыРасчета.ВидДокументаНачисления В (&ВидыДокументов)";
...
Функция ВыполнятьРасчетЗависимыхНачисленийПриРазовых() Экспорт 
	Возврат РасчетЗарплатыРасширенный.НастройкиРасчетаЗарплаты().ВыполнятьРасчетЗависимыхНачисленийПриРазовых;	
КонецФункции	
...
Функция ПоказателиПереопределяемогоРасчета() Экспорт
	...
	Показатели.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоСодержания"));
	Показатели.Добавить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.СреднемесячноеКоличествоКалендарныхДнейОтпускаГосслужащих"));
...
Функция ПоказателиПоТерриториямУсловиямТруда() Экспорт
	...
	Для Каждого ИмяПоказателя Из ИменаПоказателей Цикл
		Показатель = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты." + ИмяПоказателя);
