...
//    * МассивШагов - Массив из Структура - массив структур со следующими ключами
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'", ОбщегоНазначения.КодОсновногоЯзыка()), // Имя события.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Функция ДоступнаАктивацияАгентаКопированияНаКомпьютере(КонтекстВыполнения = Неопределено) Экспорт
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1. Начало'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НаименованиеПроцедурыФункции), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		, // Данные
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru='Облачный архив. Все обновления Облачного архива. %1. Конец'", ОбщегоНазначения.КодОсновногоЯзыка()),
			НаименованиеПроцедурыФункции), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
...
Функция ПолучитьПроксиВебСервисаАвторизации1С(КонтекстВыполнения = Неопределено)
	...
	Если ТипЗнч(Прокси) <> Тип("WSПрокси") Тогда
		...
		// Запись в журнал регистрации.
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Веб-сервисы'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Облачный архив. Веб-сервисы. Подключение к серверу'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Веб-сервисы'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Веб-сервисы. Удаление файла резервной копии'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
		...
		ИнтернетПоддержкаПользователейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
			...
			Иначе
				Если ЗащищенноеСоединениеКэш = Неопределено Тогда
					ЗащищенноеСоединениеКэш = ОбщегоНазначенияКлиентСервер.НовоеЗащищенноеСоединение(, Новый СертификатыУдостоверяющихЦентровОС);
				КонецЕсли;
...
Процедура ВсеОбновленияОблачногоАрхива() Экспорт
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Все обновления Облачного архива. Начало'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
	...
	Если МонопольныйРежим() Тогда
		...
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Все обновления Облачного архива'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Облачный архив. Все обновления Облачного архива. Конец'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияОблачногоАрхива, // ОбъектМетаданных
			, // Данные
...
Процедура ИзменитьИспользованиеРегламентныхЗаданий(Использовать) Экспорт
	...
	Иначе
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			...
			ТипЗнч(Использовать));
		ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru='Облачный архив. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
//
// Возвращаемое значение:
//   Массив из Число - массив чисел, дней недели (от 1 (пнд) до 7 (вск)).
//
...
//
// Возвращаемое значение:
//   Массив из Число - массив чисел, дней месяца (от 1 до 31 и 32 (last)).
//
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбщиеДанные. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
...
#КонецОбласти
	...
	ИнтернетПоддержкаПользователей.ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru='БИП:ОблачныйАрхив.Сервис и регламент'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru='Облачный архив. Сервис и регламент. ОбновлениеИнформационнойБазы. ОбластьДанных. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		УровеньЖурналаРегистрации.Информация, // УровеньЖурналаРегистрации.*
		, // ОбъектМетаданных
		(ШагВыполнения.ВремяОкончания - ШагВыполнения.ВремяНачала), // Данные
		ТекстСообщения, // Комментарий
		...
		Если ОблачныйАрхивПовтИсп.РазрешенаРаботаСОблачнымАрхивом() Тогда
			...
			ДлительныеОперации.ВыполнитьВФоне(
				...
				Новый Структура("МассивШагов",
					МассивШагов), // ПараметрыПроцедуры.
				ПараметрыВыполнения); // ПараметрыВыполнения
			// Возвращаемое значение "ВыполнитьВФоне" - Структура:
			//   * Статус - Строка - "Выполняется", "Выполнено", "Ошибка", "Отменено".
			//   * ИдентификаторЗадания - УникальныйИдентификатор;
			//   * АдресРезультата - Строка;
			//   * АдресДополнительногоРезультата - Строка;
...
//
Процедура ПанельАдминистрированияБСП_ПереключитьНаПравильнуюСтраницу(Форма, СобратьДанныеОКлиенте = Ложь)
...
//
Функция ЭтоАдминистраторWindows()
	...
	Иначе
		...
		ИначеЕсли (КлиентАгентКопированияНайденВРазрешенных = Истина)
			...
			ИначеЕсли ((КлиентСрокГодностиТекущегоАгентаКопирования > ДатаДляПроверок)
					И (КлиентСрокГодностиТекущегоАгентаКопирования <= (ДатаДляПроверок + 7*24*60*60))) Тогда // Станет неактуальным через неделю - ЖЕЛАТЕЛЬНО переустановить.
				Результат.Вставить("ТребуетсяУстановка", Ложь);
