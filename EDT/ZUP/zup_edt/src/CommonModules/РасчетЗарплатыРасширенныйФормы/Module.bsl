...
Процедура СортироватьПерерасчеты(НачисленияПерерасчет) Экспорт
	...
	ТаблицаСортировки.Колонки.Добавить("Идентификатор", Новый ОписаниеТипов("Число"));
	...
	Если НачисленияПерерасчет[0].Свойство("Сотрудник") Тогда
	...
	Если НачисленияПерерасчет[0].Свойство("Начисление") Тогда
	...
	Для Каждого СтрокаТаблицыФормы Из НачисленияПерерасчет Цикл
		...
		СтрокаТаблицыСортировки.Идентификатор = СтрокаТаблицыФормы.ПолучитьИдентификатор();
	КонецЦикла;	
	...
	Для Каждого ЭлементПорядка Из ТаблицаСортировки Цикл
		СтрокаТаблицыФормы = НачисленияПерерасчет.НайтиПоИдентификатору(ЭлементПорядка.Идентификатор);
...
Процедура СтрокаНДФЛВДанныеМенеджераРасчета(МенеджерРасчета, СтрокаИсточник, СтрокиПримененныеВычеты, ДанныеРезультатовРаспределения = Неопределено) Экспорт
	...
	ИначеЕсли МесяцыПериодаПредоставленияВычета.Количество() = 1 Тогда
		Для Каждого СтрокаВычета Из СтрокиПримененныеВычеты Цикл
			...
			СтрокаПриемник.МесяцПериодаПредоставленияВычета = СтрокаВычета.МесяцПериодаПредоставленияВычета;
		КонецЦикла; 
	Иначе
...
Функция ОбходТаблицыИдентификаторов(СоответствиеОбходовСотрудников, ПозицииСотрудников, Сотрудник)
...
Процедура УдалитьНеактуальныеСтрокиФормы(ТаблицаПриемник, СоответствиеОбходовСотрудников, ПозицииСотрудников)
