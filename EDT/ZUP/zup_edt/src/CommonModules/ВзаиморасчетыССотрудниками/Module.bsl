...
Процедура УпорядочитьДанныеВедомостейДляОплаты(ДанныеВедомостейДляОплаты) Экспорт
	...
	ПредставленияВедомостей = 
		ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ДанныеВедомостейДляОплаты.ВыгрузитьКолонку("Ведомость"),
			"Представление");
	ПредставленияФизЛиц = 
		ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
			ДанныеВедомостейДляОплаты.ВыгрузитьКолонку("ФизическоеЛицо"),
			"Представление");
			
	ДанныеВедомостейДляОплаты.Колонки.Добавить("ВедомостьПредставление",      Новый ОписаниеТипов("Строка"));
	ДанныеВедомостейДляОплаты.Колонки.Добавить("ФизическоеЛицоПредставление", Новый ОписаниеТипов("Строка"));
	Для Каждого Строка Из ДанныеВедомостейДляОплаты Цикл
		Строка.ВедомостьПредставление      = ПредставленияВедомостей[Строка.Ведомость];
		Строка.ФизическоеЛицоПредставление = ПредставленияФизЛиц[Строка.ФизическоеЛицо];
	КонецЦикла;	
	
	ДанныеВедомостейДляОплаты.Сортировать("ВедомостьПредставление, ФизическоеЛицоПредставление");
	
	ДанныеВедомостейДляОплаты.Колонки.Удалить("ВедомостьПредставление");
	ДанныеВедомостейДляОплаты.Колонки.Удалить("ФизическоеЛицоПредставление");
...
Процедура СоздатьВТСостояниеВыплат(ОписательВременныхТаблиц, ТолькоРазрешенные, Организация, ПериодРегистрации, ИгнорируемыеРегистраторы = Неопределено, ИмяВТСостояниеВыплат = "ВТСостояниеВыплат") Экспорт
	...
	ТекстЗапроса = "";
	Если ЧитатьРегистрЗарплатаКВыплате И ЧитатьРегистрЗарплатаКВыплатеАвансом Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ЗарплатаКВыплате.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
		...
		|				Отборы.ФизическоеЛицо КАК ФизическоеЛицо
		|			ИЗ
		|				ВТОтборДокументовДляЗарплатаКВыплате КАК Отборы)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|				Отборы.ФизическоеЛицо КАК ФизическоеЛицо
		|			ИЗ
		|				ВТОтборДокументовДляЗарплатаКВыплатеАвансом КАК Отборы)
		|
		|СГРУППИРОВАТЬ ПО
		|	ЗарплатаКВыплате.ФизическоеЛицо,
		|	ЗарплатаКВыплате.ДокументОснование,
		...
		Если ИгнорируемыеРегистраторы = Неопределено Или ИгнорируемыеРегистраторы.Количество()=0 Тогда
			ТекстЗапроса= СтрЗаменить(ТекстЗапроса, "&ИгнорируемыеРегистраторы", "ЛОЖЬ");
		Иначе
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&ИгнорируемыеРегистраторы", "ЗарплатаКВыплате.Регистратор В (&ИгнорируемыеРегистраторы)");
		КонецЕсли;	
		
	ИначеЕсли ЧитатьРегистрЗарплатаКВыплате Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ЗарплатаКВыплате.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
	...
	ИначеЕсли ЧитатьРегистрЗарплатаКВыплатеАвансом Тогда
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ЗарплатаКВыплате.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ЗарплатаКВыплате.ДокументОснование КАК ДокументОснование,
